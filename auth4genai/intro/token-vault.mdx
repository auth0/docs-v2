...
Once a user successfully connects and authorizes access to a supported external provider, Auth0 securely stores the following: 
...
1. **User authentication and consent:** The user authenticates with an external Identity Provider (e.g., Google) and grants your application permission to access their data by approving the requested OAuth scopes. The AI agent triggers the Connected Accounts flow and adds the external account to the user profile.
