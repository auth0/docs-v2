...
To be able to call your APIs on your users’ you might need to exchange a custom token for an Auth0 access token and verify it, this is called Token Exchange and in the context of MCP this can be done using Custom Token Exchange.

By the end of this quickstart, you should have an MCP server that can:

* Exchange a custom token for an Auth0 access token using RFC 8693
* Verify the access token with `jose` against your tenant JWKS, enforcing issuer, audience, and RS256
...
Copy the action ID by running the command below:

```shell
auth0 actions list
```

If you have more Actions in your tenant look for the one that matches the same name you used when you created the Action.

Now use the Action ID in the command below:

```shell
auth0 api post token-exchange-profiles -d '{
  "name": "fastmcp-cte-profile",
...
Open the Authentication menu on the navigation menu on the left, and add the `Client ID` and `Client Secret` from the TOD[client creation step](./call-your-apis-on-users-behalf#create-an-application-for-your-mcp-server) in the corresponding fields and click **Connect.**
...
<Frame>
  <img
    src="/img/mcp/greet_tool_call_with_cte.png"
    alt="WhoAmI tool call response in MCP Inspector"
  />
</Frame>

TODO: missing image of the the greet tool call with token exchange
