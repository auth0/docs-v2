...
export const CustomTocs = ({ sections, config = {} }) => {
  ...
  useEffect(() => {
    ...
    setItems(headings.map(h => ({ id: h.id, text: h.textContent.replace(/​ /g, '').trim() })));
        ...
        <ul className="text-[#767676] text-sm mt-4 flex flex-col gap-2">
          {items.map((i, idx) => (
            <li key={i.id}>
              ...
              {((config.elementWithStepper === idx) ||
                (config.elementWithStepper >= items.length && idx === items.length - 1)
               ) && steperItems.length > 0 && (
