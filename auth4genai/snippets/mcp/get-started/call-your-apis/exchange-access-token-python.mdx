...
This function uses the `get_token_by_exchange_profile` method of `ApiClient` from the `auth0-api-python` SDK  and, on a successful exchange, returns the new `accessToken` and its associated scopes. This method implements the [Custom Token Exchange](https://auth0.com/docs/authenticate/custom-token-exchange) flow.
...
self.client = ApiClient(ApiClientOptions(
    ...
    audience=audience, # API_AUTH0_AUDIENCE
    client_id=client_id, # MCP_AUTH0_CLIENT_ID
    client_secret=client_secret # MCP_AUTH0_CLIENT_SECRET
))
```

The client then gets attached to the request context, making it available to all tools.

### Using Token Exchange in MCP tools
Here's how it's implemented in the `greet` tool:

```python wrap lines highlight={10,11,12,13,14}
@mcp.tool(name="greet")
