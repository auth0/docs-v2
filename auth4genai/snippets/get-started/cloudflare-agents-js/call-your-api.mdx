...
<div id="manual-steps">
  <Steps>
    <Step title="Start from our Cloudflare Agents template">
...
```
    ...
    <Step title="About the dependencies">
The start kit is similar to the [Cloudflare Agents starter kit](https://github.com/cloudflare/agents-starter) but includes the following dependencies to integrate with Auth0 and AI SDK:
...
- `@auth0/ai-vercel`: [Auth0 AI SDK](https://github.com/auth0-lab/auth0-ai-js/tree/main/packages/ai-vercel) to provide building blocks for using Auth0 for AI Agents with the AI SDK.
- `@auth0/ai-cloudflare`: [Auth0 Cloudflare AI SDK](https://github.com/auth0-lab/auth0-ai-js/tree/main/packages/ai-cloudflare) to provide building blocks for using Auth0 for AI Agents with the Cloudflare Agents API.
...
In the root directory of your project, copy the `.dev.vars.example` into `.dev.vars` file and configure the Auth0 and OpenAI variables.
    ...
    <Step title="Define a tool to call your API">
In this step, you'll create a AI SDK tool to make the first-party API call to the Auth0 API. You will do the same for third-party APIs.
