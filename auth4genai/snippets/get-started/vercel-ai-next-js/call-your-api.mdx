...
In this step, you'll create a AI SDK tool to make the first-party API call. The tool fetches an access token to call the API.
...
The AI agent processes and runs the user's request through the AI pipeline, including the tool call. AI SDK simplifies this task with the `streamText()` method. Update the `/src/app/api/chat/route.ts` file with the following code:
