...
<div id="manual-steps">
  <Steps>
    <Step title="React SPA vs Next.js approach">
    ...
    <Step title="Set up React app + Hono API">
...
- `@auth0/ai-vercel`: [Auth0 AI SDK for AI SDK](https://github.com/auth0-lab/auth0-ai-js/tree/main/packages/ai-vercel) built for AI agents
- `ai`: Core [AI SDK](https://sdk.vercel.ai/docs) module
...
```
    ...
    <Step title="Enable step-up authorization">
...
```
    ...
    <Step title="Create chat server with Hono">
Create an AI-powered chat server using Hono, the Auth0 AI SDK, and the AI SDK that allows a user to chat with an AI assistant that can access their Google Calendar.

The Auth0 AI SDK's `withInterruptions()` function wraps the AI SDK's `streamText()` function, enabling the Hono server to handle interrupts, which are special responses from Token Vault. An interrupt is sent if a tool call requires a new or updated access token, for example, if a user needs to re-authenticate or a new permission is needed.
...
```
    ...
    <Step title="Handling interruptions">
...
That's it! You've successfully integrated federated connections with access tokens into your React SPA + AI SDK application.
    </Step>
  </Steps>
</div>
