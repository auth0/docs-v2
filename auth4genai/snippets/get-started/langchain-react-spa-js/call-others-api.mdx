...
When an agent tool requires access to a third-party service (like Google Calendar) that the user hasn't previously authorized, LangGraph can raise a `TokenVaultInterrupt`. The frontend handles these interrupts by displaying an authorization pop-up.
...
The `TokenVaultConsentPopup` component handles the OAuth flow for third-party connections.

Create `src/components/TokenVaultConsentPopup.tsx` to display the authorization prompt:

```tsx src/components/TokenVaultConsentPopup.tsx wrap lines
...
Create the Auth0 AI `withTokenVault` wrapper to securely fetch access tokens from Token Vault for your tools:
