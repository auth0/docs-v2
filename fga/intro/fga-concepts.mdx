---
title: Fine-Grained Authorization Concepts
permalink: /fga-concepts
...
# Fine-Grained Authorization (FGA) Concepts

The Auth0 FGA service answers [authorization](/intro/authorization-concepts#authentication-and-authorization) [checks](#what-is-a-check-request) by determining whether a **[relationship](#what-is-a-relation)** exists between an [object](#what-is-an-object) and a [user](#what-is-a-user). Checks reference your **[authorization model](#what-is-an-authorization-model)** against your **[relationship tuples](#what-is-a-relationship-tuple)** for authorization authority. Below are explanations of basic FGA concepts, like type and authorization model, and a [playground](https://play.fga.dev/) to test your knowledge.
...
Auth0 Fine Grained Authorization uses two different syntaxes to define the authorization model:

- A JSON syntax accepted by the Auth0 FGA API that closely follows the original syntax in the [Zanzibar Paper](https://research.google/pubs/pub48190/). For more information, see [Equivalent Zanzibar Concepts](./configuration-language.mdx#equivalent-zanzibar-concepts).
...
Click here to learn more about the [Auth0 FGA Configuration Language](./configuration-language).
...
A **store** is an Auth0 Fine Grained Authorization entity used to organize authorization check data.
...
A **relationship tuple** is a base tuple/triplet consisting of a user, relation, and object. Tuples may add an optional condition, like [Conditional Relationship Tuples](#what-is-a-conditional-relationship-tuple). Relationship tuples are written and stored in Auth0 FGA.
...
A **direct relationship** (R) between user X and object Y means the relationship tuple (user=X, relation=R, object=Y) exists, and the Auth0 FGA authorization model for that relation allows the direct relationship because of [direct relationship type restrictions](./configuration-language.mdx#direct-relationship-type-restrictions).
...
A **check request** is a call to the Auth0 Fine Grained Authorization check endpoint, returning whether the user has a certain relationship with an object.

</summary>

Check requests use the `check` methods in the Auth0 FGA SDKs ([JavaScript SDK](https://www.npmjs.com/package/@openfga/sdk)/[Go SDK](https://github.com/openfga/go-sdk)/[.NET SDK](https://www.nuget.org/packages/OpenFga.Sdk)) by manually calling the [check endpoint](/api/service#Relationship%20Queries/Check) using curl or in your code. The check endpoint responds with `{ "allowed": true }` if a relationship exists, and with `{ "allowed": false }` if the relationship does not.
...
A **list objects request** is a call to the Auth0 Fine Grained Authorization list objects endpoint that returns all objects of a given type that a user has a specified relationship with.

</summary>

List objects requests are completed using the `listobjects` methods in the Auth0 FGA SDKs ([JavaScript SDK](https://www.npmjs.com/package/@openfga/sdk)/[Go SDK](https://github.com/openfga/go-sdk)/[.NET SDK](https://www.nuget.org/packages/OpenFga.Sdk)) by manually calling the [list objects endpoint](/api/service#Relationship%20Queries/ListObjects) using curl or in your code.
...
<Tip>**Code example coming soon** — `ListObjectsRequestViewer` has not yet been ported to Mintlify. See the [OpenFGA SDK docs](https://openfga.dev/docs) for examples.</Tip>
...
A **list users request** is a call to the Auth0 Fine Grained Authorization list users endpoint that returns all users of a given type that have a specified relationship with an object.
...
<Tip>**Code example coming soon** — `ListUsersRequestViewer` has not yet been ported to Mintlify. See the [OpenFGA SDK docs](https://openfga.dev/docs) for examples.</Tip>
...
In Auth0 Fine Grained Authorization, type bound public access (represented by `<type>:*`) is a special Auth0 FGA syntax meaning "every object of [type]" when invoked as a user within a relationship tuple. For example, `user:*` represents every object of type `user` , including those not currently present in the system.
...
<CardGroup cols={2}>
  <Card title="Authorization Concepts" href="/intro/authorization-concepts">
    Learn about Authorization Concepts.
  </Card>
