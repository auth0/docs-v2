...
permalink: /modeling/getting-started
---
...
Creating a [Relationship Based Access Control (ReBAC)](/intro/authorization-concepts#what-is-relationship-based-access-control) authorization model might feel odd at first. Most of us tend to think about authorization models in terms of roles and permissions. After all, most software works like that. Your existing systems are likely built on a model using roles and permissions.

This guide outlines a process for defining your [authorization model](/intro/fga-concepts#what-is-an-authorization-model) with Auth0 FGA.
...
To define a ReBAC model in Auth0 FGA we recommend:
...
- Thinking about authorization starting from the resources, or objects as Auth0 FGA calls them.

If that sounds hard, don't worry! We'll guide you through it.

Auth0 FGA is built to quickly and reliably make [authorization checks](/intro/fga-concepts#what-is-a-check-request)
. This means providing an answer to a question: "Can user U perform action A on object O?"

ReBAC systems determine access from a [user's](/intro/fga-concepts#what-is-a-user) [relation](/intro/fga-concepts#what-is-a-relation) to an [object](/intro/fga-concepts#what-is-an-object). Authorization decisions are then yes or no answers to the question: "Does user U have relation R with object O?".

<Columns cols={2} style={{ marginTop: '2rem', marginBottom: '2rem' }}>
  <Card title="General Authorization Check" appearance="filled">
    "Can user <b>U</b> <u>perform an action</u> <b>A</b> on object <b>O</b>?"
  </Card>
  <Card title="Auth0 FGA (ReBAC) Authorization Check">
...
<div style={{ marginTop: '2rem', marginBottom: '2rem' }}>
  <Card
    icon={{ icon: <img src="/img/getting-started-fga-logo.svg" alt="FGAIcon" />, alignment: 'left' }}
...
![The starting point](/img/getting-started-diagram-01.svg)

### 01. Pick The Most Important Feature

![Pick the most important feature](/img/getting-started-diagram-02.svg)
...
<Info>
Roles don't "disappear" in ReBAC systems like Auth0 FGA. Your users might [have roles on a given object, rather than the entire system](./roles-and-permissions.mdx). But starting from the term "role" might lead you down the wrong path. Instead it is better to discover roles while you are modeling.
</Info>

Your feature description should include the [objects](/intro/fga-concepts#what-is-an-object), [users](/intro/fga-concepts#what-is-a-user) and [groups of users](/intro/fga-concepts#what-is-a-user) participating in the system. Sentences should look like this:
...
<Card appearance='filled' monoFontChildren>
...
![List the object types](/img/getting-started-diagram-03.svg)

Next make a list of the [types](/intro/fga-concepts#what-is-a-type) of objects in your system. You might be able to identify the objects in your system from your existing domain/database model.
...
<Columns cols={3} style={{ marginTop: '2rem', marginBottom: '2rem' }}>
  <Card icon={{ icon: <img src="/img/getting-started-icon-doc.svg" alt="DocIcon" />, alignment: 'left', label: 'Document' }} />
  <Card icon={{ icon: <img src="/img/getting-started-icon-dir.svg" alt="DirIcon" />, alignment: 'left', label: 'Folder' }} />
  <Card icon={{ icon: <img src="/img/getting-started-icon-org.svg" alt="OrgIcon" />, alignment: 'left', label: 'Organization' }} />
</Columns>
...
<div style={{ marginBottom: '2rem' }}>
  <Card centerTitle title="... IF {first noun} of a/the {second noun}" appearance="filled" />
</div>

Let's highlight those expressions in <span className="green-highlight-text">green</span>:

<div style={{ marginTop: '2rem', marginBottom: '2rem' }}>
<Card monoFontChildren appearance='filled'>
...
<Columns cols={5} style={{ marginTop: '2rem', marginBottom: '2rem' }}>
  <Card icon={{ alignment: 'left', label: 'User' }} />
  <Card icon={{ icon: <img src="/img/getting-started-icon-doc.svg" alt="DocIcon" />, alignment: 'left', label: 'Document' }} />
  <Card icon={{ icon: <img src="/img/getting-started-icon-dir.svg" alt="DirIcon" />, alignment: 'left', label: 'Folder' }} />
  <Card icon={{ icon: <img src="/img/getting-started-icon-org.svg" alt="OrgIcon" />, alignment: 'left', label: 'Organization' }} />
  <Card icon={{ icon: <img src="/img/getting-started-icon-drive.svg" alt="DriveIcon" />, alignment: 'left', label: 'Drive' }} />
</Columns>

Now that we have a list of object types we can start defining them using the [Auth0 FGA Configuration Language](../configuration-language):
...
You're now in the process of building a version you can use. The model above is not yet a valid authorization model accepted by Auth0 FGA.
</Info>
<Info>
**Important**
...
![List relations for those types](/img/getting-started-diagram-04.svg)

Each of the previously defined types has a set of relations. [Relations](/intro/fga-concepts#what-is-a-relation) are an important component in your model. After all, Auth0 FGA is a [Relationship Based Access Control (ReBAC)](/intro/authorization-concepts#what-is-relationship-based-access-control-rebac) system.
...
<Columns cols={4} equalWidth style={{ marginTop: '2rem', marginBottom: '2rem' }}>
<Card
  smallFontChildren
  icon={{ icon: <img src="/img/getting-started-icon-doc.svg" alt="DocIcon" />, alignment: 'middle', label: 'Document' }}
>
...
<Card
  smallFontChildren
  icon={{ icon: <img src="/img/getting-started-icon-dir.svg" alt="DirIcon" />, alignment: 'middle', label: 'Folder' }}
>
...
<Card
  smallFontChildren
  icon={{ icon: <img src="/img/getting-started-icon-org.svg" alt="OrgIcon" />, alignment: 'middle', label: 'Organization' }}
>
...
<Card
  smallFontChildren
  icon={{ icon: <img src="/img/getting-started-icon-drive.svg" alt="DriveIcon" />, alignment: 'middle', label: 'Drive' }}
>
...
<Info>
In Auth0 FGA, relations can only have alphanumeric characters, underscores and hyphens. We recommend using underscore (\_) to separate words and removing prepositions. E.g.: "can create a document" can become "can_create_document" or "create_document" if you are into brevity.
</Info>

Using the [Auth0 FGA Configuration Language](../configuration-language) we can enumerate the relations for each type:
...
![Define relations](/img/getting-started-diagram-05.svg)

We will use the [Auth0 FGA Configuration Language](../configuration-language) to create a [relation definition](/intro/fga-concepts#what-is-a-relation) for each of the relations we identified. At this stage we will encode the answers to the question we asked at the beginning of the document:.
...
<img src="/img/getting-started-icon-org.svg" alt="OrgIcon" />
...
The member relation is used to tell Auth0 FGA about the members of an organization.

<Info>
**Important**

Relation names in Auth0 FGA are arbitrary strings. There are no reserved relation names. You can use "member" or "part_of" or anything else to refer to a user that is part of a team/organization.
</Info>
...
We will specify this logic as part of [directly related user type](/intro/fga-concepts#what-is-a-directly-related-user-type).
...
The complete [type definition](/intro/fga-concepts#what-is-a-type-definition) for the **organization** type is:
...
<div style={{ display: 'inline-block' }}>
<img src="/img/getting-started-icon-doc.svg" alt="DocIcon" />
<div style={{ float: 'right', marginLeft: '16px' }}>
...
The owner relation is used to tell Auth0 FGA which users are owners of the document.

<Info>
**Important**

In the current version, there is no way to state that there is only one owner in the authorization model. The application must limit this [set of users](/intro/fga-concepts#what-is-a-user) to just one owner if that is a requirement.
</Info>
When a document is created, a relationship tuple will be stored in Auth0 FGA representing this relationship between owner and document. This is an example of a [user to object relationship](./direct-access.mdx).
...
Why? This [relation definition](/intro/fga-concepts#what-is-a-relation) states that:
...
The editor relation is used to tell Auth0 FGA which users are editors of the document.

When a user shares a document with another user or set of users as editor, a relationship tuple will be stored in Auth0 FGA representing this relationship between editor and document. This is an example of a [users to object relationship](./direct-access.mdx).
...
The parent relation is used to tell Auth0 FGA which folder or drive is the parent of the document.

<Warning>
**Important**

Relation names in Auth0 FGA are arbitrary strings. There are no reserved relation names. You can use "parent", "container" or "ancestor" to refer to a "parent folder".
</Warning>
...
When a document is created a relationship tuple will be stored in Auth0 FGA to represent this relationship between parent and document. The relation definition then should be:
...
You might have noticed that the "user" in the tuple is an object. This is a special syntax Auth0 FGA accepts in the "user" parameter to write [object to object relationships](./building-blocks/object-to-object-relationships.mdx). You can read more about writing data to manage object to object relationships in [Managing Relationships Between Objects](../interacting/managing-relationships-between-objects.mdx).
</Note>
##### Relation: can_share

We need to express the following in the [relation definition](/intro/fga-concepts#what-is-a-relation):
...
We can achieve that with the following definition using [Auth0 FGA Configuration Language](../configuration-language):
...
Similar to the [can_share relation](#relation-can_share), we can achieve that with the following definition using [Auth0 FGA Configuration Language](../configuration-language):
...
The complete [type definition](/intro/fga-concepts#what-is-a-type-definition) for the document type is:
...
<Note>
**The Auth0 FGA authorization model API and SDK only accepts JSON in its input. To convert from DSL to JSON, you may use the [FGA CLI](https://github.com/openfga/cli) to run `fga model transform`.**


</Note>
### 05. Test The Model

![Test the model](/img/getting-started-diagram-06.svg)
...
How? Remember from the introduction that **Auth0 Fine Grained Authorization's** main job is to answer the question:

<div style={{ marginTop: '2rem', marginBottom: '2rem' }}>
  <Card icon={{ icon: <img src="/img/getting-started-fga-logo.svg" alt="FGAIcon" />, alignment: 'left' }} title="Can user U, perform an action A on an object O?" />
</div>

The Auth0 FGA service does that by checking if a user has a particular relationship to an object, based on your authorization model and relationship tuples.
...
So we'll write some relationship tuples and assertions. An Auth0 FGA assertion takes one of these forms:
...
![Iterate](/img/getting-started-diagram-07.svg)
...
When defining the authorization model for your own system, you would continue iterating on the [authorization model](/intro/fga-concepts#what-is-an-authorization-model) with the [next feature](#01-pick-the-most-important-feature) and so on.
