...
<Info>
This tutorial will help you call your own API from an input-constrained device using the Device Authorization Flow. If you want to learn how the flow works and why you should use it, see [Device Authorization Flow](/get-started/authentication-and-authorization-flow/device-authorization-flow).
</Info>
...
* [Register the Application with Auth0](/get-started/auth0-overview/create-applications/native-apps).
  ...
  + Make sure the Application's [**Grant Types**](/get-started/applications/update-grant-types) include **Device Code**. This is also in the [Dashboard](https://manage.auth0.com/#), under **Application Settings > Advanced > Grant Types**.
  + If you want your Application to be able to use [Refresh Tokens](/secure/tokens/refresh-tokens), make sure the Application's [**Grant Types**](/get-started/applications/update-grant-types) include **Refresh Token**.
* Set up and enable at least one connection for the Application: [Database connections](/get-started/applications/set-up-database-connections), [Social connections](https://marketplace.auth0.com/features/social-connections)
* [Register your API with Auth0](/get-started/architecture-scenarios/mobile-api/part-2#create-the-api)

  + If you want your API to receive [Refresh Tokens](/secure/tokens/refresh-tokens) to allow it to obtain new tokens when the previous ones expire, enable **Allow Offline Access**.
* [Configure Device User Code Settings](/get-started/tenant-settings/configure-device-user-code-settings) to define the character set, format, and length of your randomly-generated user code.
...
<Info>
You can [configure the character set, format, and length of your randomly-generated user code](/get-started/tenant-settings/configure-device-user-code-settings) in your tenant settings.
...
* a configured [Rule](/customize/rules) denied access
...
<Warning>
You should validate your tokens before saving them. To learn how, see [Validate Access Tokens](/secure/tokens/access-tokens/validate-access-tokens) and [Validate ID Tokens](/secure/tokens/id-tokens/validate-id-tokens).
</Warning>

[Access Tokens](/secure/tokens/access-tokens) are used to call the [Auth0 Authentication API's `/userinfo` endpoint](https://auth0.com/docs/api/authentication#get-user-info) or another API. You will be able to use the Access Token to call `/userinfo` only if you included the `openid` scope. If you are calling your own API, the first thing your API will need to do is [verify the Access Token](/secure/tokens/access-tokens/validate-access-tokens).

[ID Tokens](/secure/tokens/id-tokens) contain user information that must be [decoded and extracted](/secure/tokens/id-tokens#id-token-payload). The `id_token` will only be present in the response if you included the `openid` scope.

[Refresh Tokens](/secure/tokens/refresh-tokens) are used to obtain a new Access Token or ID Token after the previous one has expired. The `refresh_token` will only be present in the response if you included the `offline_access` scope and enabled **Allow Offline Access** for your API in the Dashboard.
...
<AuthCodeBlock filename="C#" icon="file-code" language="javascript" lines="true">
AUTH0_SECRET='use [openssl rand -hex 32] to generate a 32 bytes value'
APP_BASE_URL='http://localhost:3000'
AUTH0_DOMAIN='https://{USER_DOMAIN}'
AUTH0_CLIENT_ID='{USER_CLIENT_ID}'
...
You can use [Rules](/customize/rules) to detect whether the current transaction is using the Device Authorization Flow. To do so, check the `context` object's `protocol` property:
...
[Tenant logs](/deploy-monitor/logs) are created for any interaction that takes place and can be used to troubleshoot issues.
...
* Have an [Auth0 application type](/get-started/applications) of **Native**
* Have the [**Token Endpoint Authentication Method**](/get-started/applications/application-settings) set to **None**
* Be [OIDC-conformant](/get-started/applications/application-settings#oauth)
* Not be created through [Dynamic Client Registration](/get-started/applications/dynamic-client-registration)

In addition, the Device Authorization Flow does not allow:

* [Social Connections](/authenticate/identity-providers) using [Auth0 developer keys](/authenticate/identity-providers/social-identity-providers/devkeys) unless you are using new [New Universal Login Experience](/authenticate/login/auth0-universal-login/universal-login-vs-classic-login/universal-experience).
...
* [The OAuth 2.0 protocol](/authenticate/protocols/oauth)
* [The OpenID Connect protocol](/authenticate/protocols/openid-connect-protocol)
* [Tokens](/secure/tokens)
* [Tenant Logs for Devices](/deploy-monitor/logs)
