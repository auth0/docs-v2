...
| --- | --- | --- |
| `acls_summary` | Network ACLs Summary | Shows the summary of the network ACLs matches for a period of time |
| `actions_execution_failed` | Actions Execution Failed | Execution of an Action failed |
| `acul_sdk_notice` | ACUL SDK Notice | ACUL SDK notification |
...
| `api_limit_warning` | Rate Limit Warning on the Authentication or Management APIs | The limit of requests to the Authentication or Management APIs in given time is about to be reached. |
| `appi` | Notice for API Peak Performance initiated | Increases the Elevated Public Authentication API Limits by the 100 RPS multiple purchased by the customer for up to 48 hours per month. For more information, see [Public performance rate limit policy](/docs/troubleshoot/customer-support/operational-policies/rate-limit-policy#public-performance). |
...
| `connected_accounts_connection_failed` | Failed Connected Account Connection | User failed to connect a third-party account |
| `connected_accounts_connection_succeeded` | Successful Connected Account Connection | User successfully connected a third-party account |
| `connected_accounts_delete_failed` | Failed Connected Account Delete | User failed to delete a connected third-party account |
| `connected_accounts_delete_succeeded` | Successful Connected Account Delete | User successfully deleted a connected third-party account |
| `cs` | Code Sent | Passwordless login code has been sent |
| `custom_domain_verification_failed` | Custom Domain Verification Failed | Custom Domain Verification Failed |
| `depnote` | Deprecation Notice | Deprecation Notice |
| `directory_sync_completed` | Inbound Directory Sync Completed | Inbound Directory Sync |
| `directory_sync_failed` | Inbound Directory Sync Failed |   |
| `directory_sync_started` | Inbound Directory Sync Started |   |
| `f` | Failed Login | Failed login. This is only emitted if the error is not covered by the `fp` or `fu` log types. |
| `failed_on_behalf_of_token_exchange` | Failed Exchange | Failed Exchange via On-Behalf-Of Token Exchange |
...
| `fcoa` | Failed cross-origin authentication | Failed Cross-Origin Authentication |
| `fcp` | Failed Change Password | Failed Change Password |
| `fcph` | Failed Post Change Password Hook | Failed Post Change Password Hook |
| `fcpn` | Failed Change Phone Number | Failed Change Phone Number |
| `fcpr` | Failed Change Password Request | Failed Change Password Request |
...
| `fdu` | Failed User Deletion | Failed User Deletion |
...
| `fecte` | Failed Exchange | Failed Exchange via Custom Token Exchange |
| `fede` | Failed Exchange | Failed to exchange Device Code for Access Token |
| `federated_logout_failed` | Failed Federated Logout | Failed to logout of the upstream Identity Provider |
...
| `feta` | Failed Exchange | Failed token exchange at Token Vault |
...
| `flows_execution_completed` | Flows Execution Completed | Flows Execution Completed |
| `flows_execution_failed` | Flows Execution Failed | Flows Execution Failed |
| `fn` | Failed Sending Notification | Failed to send notification |
| `forms_submission_failed` | Forms Submission Failed | Forms Submission Failed |
| `forms_submission_succeeded` | Forms Submission Succeeded | Forms Submission Succeeded |
| `fp` | Failed Login (Incorrect Password) | Failed login due to invalid password |
| `fpar` | Failed Pushed Authorization Request | Failed Push Authorization Request |
| `fpurh` | Failed Post User Registration Hook | Failed Post User Registration Hook |
| `fs` | Failed Signup | Failed Signup |
| `fsa` | Failed Silent Auth | Failed Silent Auth |
| `fu` | Failed Login (Invalid Email/Username) | Failed login due to invalid username |
...
| `idjag_exchange_failed` | Failed Exchange | Failed Exchange of Identity Assertion Authorization Grant JWT for Access Token |
| `idjag_exchange_succeeded` | Success Exchange | Successful Exchange of Identity Assertion Authorization Grant JWT for Access Token |
| `jwt_bearer_exchange_failed` | Failed Exchange | Failed Exchange of JWT Bearer Assertion for Access Token |
...
| `limit_mu` | Blocked IP Address | An IP address is blocked because it attempted too many failed logins without a successful login. Or an IP address is blocked because it attempted too many sign-ups, whether successful or failed. For more information, see [Attack Protection](/docs/secure/attack-protection). |
| `limit_sul` | Blocked Account | A user is temporarily prevented from logging in because they reached the maximum logins per time period from the same IP address. For more information, see [Attack Protection](/docs/secure/attack-protection). |
...
| `mfar` | MFA Required | A user has been prompted for multi-factor authentication (MFA). When using Adaptive MFA, Auth0 includes details about the risk assessment. Available in only the [Resource Owner Password Flow](/docs/get-started/authentication-and-authorization-flow/resource-owner-password-flow). |
| `mgmt_api_read` | Management API read Operation | API GET operation returning secrets completed successfully |
| `my_account_authentication_method_failed` | Failed authentication method operation in My Account API | Failed to complete authentication method operation in My Account API |
| `my_account_authentication_method_succeeded` | Successful authentication method operation in My Account API | Successfully completed authentication method operation in My Account API |
...
| `passkey_challenge_failed` | Passkey Challenge Failed | Native passkey challenge failed |
| `passkey_challenge_started` | Passkey Challenge Started | Native passkey challenge was successfully initiated |
...
| `reset_pwd_leak` | Breached Password on Reset | Someone behind the IP address `ip` attempted to reset with a leaked password. |
...
| `sce` | Success Change Email | Successful Change Email |
| `scoa` | Success cross-origin authentication | Successful Cross-Origin Authentication |
| `scp` | Success Change Password | Successful Change Password |
| `scpn` | Success Change Phone Number | Successful Change Phone Number |
| `scpr` | Success Change Password Request | Successful Change Password Request |
| `scu` | Success Change Username | Successful Change Username |
| `scv` | Success Credential Validation | Successful credential validation event. |
| `sd` | Success Delegation | Successful Delegation |
...
| `srrt` | Successfully revoked a refresh token | Successfully revoked a refresh token |
...
| `ss_sso_success` | Success SS-SSO Operation | Self-Service operation completed successfully |
| `ssa` | Success Silent Auth | Successful Silent Auth |
| `sscim` | Successful SCIM Operation | SCIM operation succeeded |
| `success_on_behalf_of_token_exchange` | Success Exchange | Successful Exchange via On-Behalf-Of Token Exchange |
...
| `too_many_records` | Max Amount of Authenticators | User has created the maximum amount of authenticators |
| `ublkdu` | User login block released | User block setup by anomaly detection has been released |
| `universal_logout_failed` | Failed Universal Logout request | Failed Universal Logout request |
| `universal_logout_succeeded` | Successful Universal Logout request | Successful Universal Logout request |
| `w` | Warning During Login | Filters for logs of type `warning` during login |
| `wn` | Warning Sending Notification | Warnings that have occurred during notification sending. |
