---
description: Compare Auth0 Private Cloud-managed service deployment options on AWS and Azure to choose a dedicated, isolated environment that meets your data-residency, performance, and guaranteed RPS requirements beyond the multi-tenant Public Cloud.
sidebarTitle: Overview
title: Private Cloud Deployments
---
The Private Cloud packages are managed services that you can use if you:

* Cannot use a multi-tenant Public Cloud service in your organization
* Have geographical deployment requirements where the Public Cloud option isn't feasible
* Require a guaranteed level of requests per second (RPS) or higher capacity than available on Public Cloud

The Auth0 private cloud can be deployed in the following ways:

| Option | Description |
| --- | --- |
| [**Private Cloud on AWS**](/docs/deploy-monitor/deploy-private-cloud/private-cloud-on-aws) | A dedicated, managed cloud service running on AWS providing isolation, higher performance, dev instances, Geo-HA add-ons and more |
| [**Private Cloud on Azure**](/docs/deploy-monitor/deploy-private-cloud/private-cloud-on-azure) | A dedicated cloud service running on Azure providing isolation, higher performance, dev instances, add-ons and more |

We also provide [add-on features](/docs/deploy-monitor/deploy-private-cloud/private-cloud-add-on-features) that can complement your Auth0 Private Cloud deployment.

All documentation for Private Cloud customers exists within this documentation site to provide the latest information.
Feature availability may vary depending on your Private Cloud version. For information on the latest feature releases and corresponding Private Cloud versions, see the [Auth0 Changelog](https://www.auth0.com/changelog).

If you have specific support requirements or need more information about the Professional Services we offer, please [contact Auth0 Sales](https://auth0.com/contact-us).
