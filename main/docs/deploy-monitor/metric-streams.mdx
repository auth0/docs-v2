...
<Card title="OTLP Push Integration" icon="chart-bar" href="#set-up-generic-otlp">
Set up OpenTelemetry Protocol with your preferred provider
</Card>
</CardGroup>
...
Stream Auth0 metrics to your preferred provider.

### Get provider credentials

1. To configure Auth0 to send metrics to your provider, you need:
* The protocol for sending metrics.
* The OTLP endpoint URL in which Auth0 sends data.
* An API key or Bearer token for access to your provider.

2. Review your provider's documentation for any additional configuration, such as creating an access token.
...
4. Enter the following:
  * **Stream Name**: Describes specific metrics transferred from Auth0.
  * **Protocol**: Choose the protocol your provider supports.
  * **Endpoint**: Enter your provider's OTLP endpoint URL.
  * **Authentication method**: Choose an authentication method for Auth0 to send metrics to your provider:
    * Basic: Provide a Username and Password.
    * Bearer Token: Provide the Bearer Token value from your provider in the **Authorization Token** field.
    * API Key: Provide the API Key value and header name for this value (e.g. `api-key`)
</Tab>
<Tab title="CLI">
```bash
auth0 api post metric-streams \
  --data '{
    ...
    "destination": {
      "type": "otlp",
      "configuration": {
        "endpoint": "PROVIDER_OTLP_ENDPOINT",
        "protocol": "http",
        "auth_type": "api_key",
        "api_key": "YOUR_PROVIDER_ACCESS_TOKEN",
        "auth_header_name": "YOUR_AUTH_HEADER"
      }
    }
  }'
```
