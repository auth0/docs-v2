...
# Set up metrics dtreaming via OpenTelemetry Connectors
...
<warning> When working with metric streams, it’s important to be aware that certain tags can potentially lead to high cardinality in your metrics data. Specifically, `client_id` and `connection_id` are known to have the potential for high cardinality in environments with a large number of unique clients or frequent connection changes. This can lead to increased monitoring costs.  
</warning>
