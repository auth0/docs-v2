...
Query all failed Management API requests.
...
```
sum(rate(auth0_api_request_count{
  api="MANAGEMENT",
  operation_result="FAILURE"
}[5m]))
```

Query failed Management API requests over 5 minutes.
...
If you receive `403 Forbidden` when creating or managing streams, verify your M2M application has the required `event_streams` permissions. See [Prerequisites](#prerequisites) for the complete list.
