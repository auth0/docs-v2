...
'og:title': Auth0 Security Bulletin for Rules
...
'twitter:description': Cases where improper custom rule code may create vulnerabilities
  in the authentication flow.
'twitter:title': Auth0 Security Bulletin for Rules
---
...
* Custom rule code containing conditional logic to enforce <Tooltip tip="Multi-factor authentication (MFA): User authentication process that uses a factor in addition to username and password such as a code via SMS." cta="View Glossary" href="/docs/glossary?term=multi-factor+authentication">multi-factor authentication</Tooltip> (MFA) may introduce the ability to bypass MFA altogether, especially when using silent authentication.
...
Within a single-page application (SPA), silent authentication enables new <Tooltip tip="Access Token: Authorization credential, in the form of an opaque string or JWT, used to access an API." cta="View Glossary" href="/docs/glossary?term=access+tokens">access tokens</Tooltip> to be issued without user interaction as long as the current session is valid against the <Tooltip tip="Authorization Server: Centralized server that contributes to defining the boundaries of a user’s access. For example, your authorization server can control the data, tasks, and features available to a user." cta="View Glossary" href="/docs/glossary?term=authorization+server">authorization server</Tooltip>.
...
Please refer to the [Check Domains Against Connection Aliases rule template](https://github.com/auth0/rules/blob/master/src/rules/check-domains-against-connection-aliases.js) for more information. Alternatively, in the **Rules** section of the <Tooltip tip="Auth0 Dashboard: Auth0's main product to configure your services." cta="View Glossary" href="/docs/glossary?term=Auth0+Dashboard">Auth0 Dashboard</Tooltip>, view the rule template named **Check if user email domain matches configured domain**.
