...
When the login completes, Auth0 returns an authorization code to your callback URL. Exchange it for tokens at the Auth0 `/token` endpoint:

```bash
curl --location 'https://<YOUR_AUTH0_DOMAIN>/oauth/token' \
--header 'Content-Type: application/x-www-form-urlencoded' \
--data-urlencode 'grant_type=authorization_code' \
--data-urlencode 'code=<AUTHORIZATION_CODE>' \
--data-urlencode 'client_id=<REQUESTING_APP_CLIENT_ID_IN_AUTH0>' \
--data-urlencode 'client_secret=<REQUESTING_APP_CLIENT_SECRET_IN_AUTH0>' \
--data-urlencode 'redirect_uri=<YOUR_CALLBACK_URL>'
```

Because the authorization request included the `offline_access` scope, Auth0 returns an access token, ID token, and refresh token issued by your Auth0 tenant. The Okta tokens are stored internally in Token Vault.
