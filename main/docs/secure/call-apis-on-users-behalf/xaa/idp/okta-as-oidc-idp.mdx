...
Once you've enabled Cross App Access, make sure the following features are toggled on under **Okta Admin > Settings > Features**.
...
Your Requesting application is a standard OIDC Application. To create a Requesting App in Okta:
...
Once you've created the Requesting Application, Okta assigns it an OIDC Client ID and Secret.

<Frame>![](/docs/images/xaa/xaa_okta_req_app_general.png)</Frame>

### Assign Requesting Application to Test Users

Allow your test user to log into the Requesting App in Okta.
...
Similar to the Requesting Application, your Resource Application is an OIDC Web Application.
...
To check that Resource Application's sign-in redirect URIs point back to your Auth0 tenant's `/login/callback` URL, go to **General > LOGIN**.

<Frame>![](/docs/images/xaa/xaa_okta_resource_app_details2.png)</Frame>

### Assign Resource Application to Test Users

Allow your test user to log into the Resource App in Okta.
...
Go to **Directory > AI Agents > Register AI Agent** and select **Register manually**. Name your agent and assign an owner.
...
In the **Application** section, select Resource application you created and set **AI agent's client ID registered in the app** to the `client_id` of the Agent0 application in your Auth0 tenant.
...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
    ...
    Users must log in with a XAA-enabled enterprise connection at least once so that user profile exists in Auth0.
