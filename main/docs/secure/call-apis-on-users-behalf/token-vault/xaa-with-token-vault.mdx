...
**If you want to build and test the full XAA flow from the Requesting App side**: Complete [Environment Setup](/docs/ai-agents-mcp/cross-app-access/requesting-app/set-up-xaa-test-environment) and [Okta as OIDC IdP](/docs/ai-agents-mcp/cross-app-access/requesting-app/idp/okta-as-oidc-idp) first, then return here to [configure your Token Vault application](#configure-your-token-vault-application).
...
Once you've configured both connections, proceed to [Configure your Token Vault application](#configure-your-token-vault-application) and [test the end-to-end flow](#test-the-end-to-end-flow).

## Configure your Token Vault application
...
2. Under **Advanced Settings > Grant Types**, enable **Authorization Code**, **Refresh Token**, and **Token Vault**.
...
If you already have Token Vault set up and are adding XAA support: You need to enable the Requesting App connection and Resource App connection for the application you just created. To learn more, read [Add XAA to an existing Token Vault integration](#add-xaa-to-an-existing-token-vault-integration).

<Tabs><Tab title="Auth0 Dashboard">

To enable the Okta Workforce connection or the Requesting App connection:

1. Navigate to **Authentication > Enterprise > Okta Workforce**, select the Okta Workforce connection, and select the **Applications** tab. Then, enable it for the application you just created.

To enable the OIDC connection or the Resource App connection:

1. Navigate to **Authentication > Enterprise > OpenID Connect (OIDC)**, select the OIDC connection, and select the **Applications** tab. Then, enable it for the application you just created.
...
Use the application you configured in [Configure your Token Vault application](#configure-your-token-vault-application) or an existing application with the Token Vault grant enabled.
