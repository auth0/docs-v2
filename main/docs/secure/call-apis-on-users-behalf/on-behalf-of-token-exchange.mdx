...
* Set `resource_server_identifier` to the resource server of the `aud` claim set in the incoming token (i.e. `https://incoming-token-api.example.com`), not the resource server of the downstream API. Auth0 uses the resource server identifier to validate the incoming token before performing the OBO token exchange.
...
To create a Custom API client with the same identifier as the resource server set in the incoming token, make a `POST` request to the [`/api/v2/clients`](https://auth0.com/docs/api/management/v2/clients/post-clients) endpoint with the following request body:
