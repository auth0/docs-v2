...
OBO token exchanges trigger the [`post-login` Action trigger](/docs/customize/actions/explore-triggers/post-login), where:

* The [`event.transaction.protocol`](/docs/actions/reference/post-login/post-login-event-object#param-protocol) is set to `oauth2-token-exchange`.
* The [`event.transaction.actor`](/docs/actions/reference/post-login/post-login-event-object#param-actor) tracks the complete delegation chain.
...
The delegation chain is limited to five nested levels. Because the exchange appends the current client as one more level, the OBO token exchange will fail if the subject token already has four nested `act` levels.
...
* Set `resource_server_identifier` to the audience of the incoming token (i.e. `https://incoming-token-api.example.com`), not the downstream API. Auth0 uses the resource server identifier to validate the incoming token before performing the OBO token exchange.

Because Custom API clients are first-party clients, make sure you [skip user consent](/docs/get-started/applications/third-party-applications/user-consent-and-third-party-applications#skip-consent-for-first-party-applications) for the APIs your first-party client needs to access.
...
To create a Custom API client with the same value as the `aud` claim in the incoming token, make a `POST` request to the [`/api/v2/clients`](https://auth0.com/docs/api/management/v2/clients/post-clients) endpoint with the following request body:
...
| `resource_server_identifier` | The unique identifier for your Custom API Client. Set to the audience of the incoming token i.e. `https://incoming-token-api.example.com`. |
...
2. Under **API Access**, find the downstream API (i.e., `https://downstream-api.example.com`) and select **Edit**.
...
| `audience` | `https://downstream-api.example.com` | Required. The identifier for the downstream service that will receive and validate the new token. |
...
* Makes the organization context available in the [`post-login` Actions trigger](/docs/actions/reference/post-login/post-login-event-object#event-organization) via the `event.organization` property