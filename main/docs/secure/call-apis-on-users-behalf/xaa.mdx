...
Both the Auth0 Resource App Authorization Server and the enterprise IdP must be configured for XAA to work. Start with [Environment Setup](/docs/secure/call-apis-on-users-behalf/xaa/set-up-xaa-test-environment) to configure the Auth0 side, then read the applicable guide in IdP Integration—[Okta as OIDC IdP](/docs/secure/call-apis-on-users-behalf/xaa/idp/okta-as-oidc-idp), [Okta as SAML IdP](/docs/secure/call-apis-on-users-behalf/xaa/idp/okta-as-saml-idp)—depending on the IdP and protocol you want to use to federate with it.
