...
XAA implements the [Identity Assertion Authorization Grant](https://datatracker.ietf.org/doc/draft-ietf-oauth-identity-assertion-authz-grant/), an in-progress OAuth extension that lets a Requesting App — an application or AI agent — obtain a secure token from the enterprise IdP to call another application's API (Resource App) on the end-user's behalf. This covers both app-to-app connections and agent-to-app connections, where an AI agent acts as an [MCP Client](https://modelcontextprotocol.io/) calling an MCP server exposed by the Resource App. To learn more, read [How it works](#how-it-works).
...
Both the Auth0 Resource App Authorization Server and the enterprise IdP must be configured for XAA to work. Start with [Environment Setup](/docs/secure/call-apis-on-users-behalf/xaa/set-up-xaa-test-environment) to configure the Auth0 side, then read the applicable guide in IdP Integration — [Okta as OIDC IdP](/docs/secure/call-apis-on-users-behalf/xaa/idp/okta-as-oidc-idp), [Okta as SAML IdP](/docs/secure/call-apis-on-users-behalf/xaa/idp/okta-as-saml-idp), [Generic OIDC IdP](/docs/secure/call-apis-on-users-behalf/xaa/idp/generic-oidc-idp), or [Generic SAML IdP](/docs/secure/call-apis-on-users-behalf/xaa/idp/generic-saml-idp) — depending on the IdP and protocol you want to use to federate with it.
...
<Note>
The Requesting App and Resource App can each use OIDC or SAML to federate with the enterprise IdP for this SSO step. Read [End-to-end Testing](/docs/secure/call-apis-on-users-behalf/xaa/end-to-end-testing) to understand the details of how the flow works in each case.
</Note>
