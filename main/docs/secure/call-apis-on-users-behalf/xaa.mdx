---
description: Connect and manage third-party apps and AI agents using enterprise admin controls.
...
If the enterprise IdP itself rate-limits a request during the XAA flow (for example, the refresh token or ID-JAG exchange with Okta), Auth0 returns a `429 Too Many Requests` response to your Requesting App and includes an `X-RateLimit-Upstream` header identifying the connection whose upstream IdP applied the rate limit. If the upstream IdP also returned a `Retry-After` value, Auth0 passes it along in the response; otherwise, it may not be present.
