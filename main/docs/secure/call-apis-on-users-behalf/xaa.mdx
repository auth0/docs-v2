...
- Connect AI agents to enterprise applications (agent-to-app): An employee uses an AI agent, implementing an MCP Client, to read from their calendar app and post an update in the enterprise messaging app. Instead of requiring the employee to go through redirection flows and consent prompts, the agent uses XAA to securely call the calendar and messaging app APIs, commonly exposed via MCP servers, if approved by the enterprise access policy.
...
The Auth0 resource Authorization Server and the enterprise IdP are configured separately: see [Environment setup](/docs/secure/call-apis-on-users-behalf/xaa/set-up-requesting-app) for the Auth0 side, and [Configure Okta as OIDC IdP](/docs/secure/call-apis-on-users-behalf/xaa/idp/okta-as-oidc-idp) for the IdP side.
...
<Note>
The Requesting App and Resource App can each use OIDC or SAML to federate with the enterprise IdP for this SSO step — see [Generic SAML IdP](/docs/secure/call-apis-on-users-behalf/xaa/idp/generic-saml-idp) for the SAML case.
</Note>
