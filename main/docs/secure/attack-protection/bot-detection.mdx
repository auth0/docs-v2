...
3. In the **Response** section, choose a bot detection response.
   ...
   If you enable the **Fail Open** toggle, when the Bot Detection service becomes unreacheable, the authentication process is prioritized and proceeds without a CAPTCHA challenge. The end user does not see an error message and they are not blocked from logging in or registering. 

The authentication process continues as if the challenge was successfully passed until the bot detection service is once again reachable and end users are appropriately challenged with CAPTCHA.
