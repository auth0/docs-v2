...
* Block new users from signing up with stolen credentials. This entirely blocks the username/password combination.
* Block anyone from logging in with stolen credentials. This entirely blocks the account and prevents bad actors from accessing your application. The user can’t log in until they change their password.
...
3. If your agreement with Auth0 includes Credential Guard, select **As soon as possible based on data received from the dark web, with Credential Guard**. Otherwise, leave **When breach data is published** selected.
