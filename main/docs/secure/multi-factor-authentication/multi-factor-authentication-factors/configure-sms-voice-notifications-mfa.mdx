---
description: Configure SMS and voice notifications as Auth0 MFA factors with Twilio, the built-in Auth0 provider, or a custom provider to deliver one-time codes.
...
After users are enrolled, the next time they authenticate they get the voice or SMS message at their registered phone.

## Use the Dashboard

1. Navigate to [**Auth0 Dashboard > Security > Multi-factor Auth**](https://manage.auth0.com/#/multifactor-auth).
2. Select **Phone Message** and enable the toggle switch at the top.
3. Choose if the phone message provider you configure under **[Branding > Phone Provider](https://manage.auth0.com/#/phone/templates/phone/provider)** is applicable for your tenant. If so, toggle on **Use Tenant-Level Messaging Provider**.
4. Choose your delivery method:
  * SMS: Verification code delivery by SMS
  * SMS and Voice: Verification code delivery by SMS or voice at the end user's discretion.
  * Voice: Verifcation code delivery by voice message.
5. Configure the code length and code lifetime:
  * Code Length: Enter a value between 6 and 10 digits. The default is 6 digits.
  * Code Lifetime: Enter a value between 30 and 3600 seconds. The default is 300 seconds or 5 minutes.
...
6. Configure your phone message provider, navigate to **[Branding > Phone Provider](https://manage.auth0.com/#/phone/templates/phone/provider)**:
   1. **Twilio**: Sends the messages using the [Twilio Programmable SMS API](https://www.twilio.com/sms) for SMS or [Twilio Programmable Voice API](https://www.twilio.com/voice) for voice. Use Twilio Life Credentials, not Test Credentials, which are only meant to send messages in a production environment. To learn more, read [Configure Twilio as a Phone Messaging Provider](/docs/customize/phone-messages/configure-phone-messaging-providers/configure-twilio-as-a-phone-messaging-provider).
   2. **Custom**: Sends the messages by invoking an Action in the [Send Phone Message Flow](/docs/customize/actions/explore-triggers/mfa-notifications-trigger).
   * [Configure a Custom Phone Provider with Terraform](/docs/customize/phone-messages/configure-phone-messaging-providers/configure-a-custom-phone-provider/configure-a-custom-phone-provider-with-terraform)
...
3. Configure the connection. Navigate to [**Auth0 Dashboard > Security > Multi-factor Auth**](https://manage.auth0.com/#/security/mfa) and choose **Phone Message**.
...
Auth0 limits a single user to send up to 10 SMS or voice messages per hour, and OTP flows via email or authenticators are limited to five requests every five minutes. (The burst rate is 10 but only 1 voice message per hour is sent for new requests.) To further protect your account, consider:

* Enabling [Suspicious IP Throttling](/docs/secure/attack-protection/suspicious-ip-throttling#signup-attempts). Auth0 blocks an IP if it attempts to do more than 50 signup requests per minute.
