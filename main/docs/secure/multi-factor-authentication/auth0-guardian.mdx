...
You can configure OTP code length and expiration time beyond the defaults (6 digits, 300 seconds). To learn more, read [Configure OTP settings](/docs/secure/multi-factor-authentication/multi-factor-authentication-factors/configure-sms-voice-notifications-mfa#configure-otp-settings).

You can also configure how often users are prompted for MFA from trusted browsers. To learn more, read [Change frequency of MFA prompts](/docs/secure/multi-factor-authentication/customize-mfa#change-frequency-of-mfa-prompts).
