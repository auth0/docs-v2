...
exports.onExecutePostLogin = async (event, api) => {
  ...
  // Example condition: prompt MFA only based on the NewDevice 
    ...
    // from an unknown device. When the confidenceScore is `low`, users should be prompted for MFA;

    switch (event.authentication.riskAssessment.assessments.NewDevice.confidenceScore) {
      case 'low':
      case 'medium':
