...
In some scenarios, you may not want to prompt the user for MFA each time they log in from the same browser. This is the Remember Me capability, which users can select as a checkbox on your login page and is enabled by default for Universal Login and Classic Login.

<Warning>
Customers are responsible for any diminishment in security posture resulting from a change to the "Remember Me" Session Behavior lifespan to a time period longer than Okta's standard recommended setup.
</Warning>
...
| --- | --- | --- |
| Idle Trusted Browser Lifetime | 7 days | 1 hour–30 days |
| Maximum Trusted Browser Lifetime | 30 days | 1 hour–90 days |
...
4. Select the **Remember browsers by default** checkbox.
...
**Required scopes:** `read:tenant_settings` (GET), `update:tenant_settings` (PUT)

To retrieve the current Guardian settings, send a `GET` request to `/api/v2/guardian/settings`:
...
To update the Guardian settings, send a `PUT` request to `/api/v2/guardian/settings`:
...
| --- | --- | --- | --- |
| `display_remember_me_checkbox` | boolean | `false` | Whether to display the **Remember this browser** checkbox on the MFA prompt in Universal Login. |
| `remember_me_default_value` | boolean | `false` | Whether the **Remember this browser** checkbox is checked by default. |
| `mfa_session_inactivity_timeout` | integer (seconds) | `604800` (7 days) | Duration of inactivity after which the user is prompted for MFA. Cannot exceed `mfa_session_overall_timeout`. |
| `mfa_session_overall_timeout` | integer (seconds) | `2592000` (30 days) | Maximum duration after which the user is prompted for MFA regardless of activity. |

  </Tab>
  <Tab title="Auth0 Actions">

You can set the [Auth0 Actions Post-Login API object](/docs/actions/reference/post-login/post-login-api-object) `allowRememberBrowser` property to `true`:
...
| Value | Description |
| --- | --- |
| `true` | When provider is set to `google-authenticator` or `duo`, the user is prompted for MFA once per session within your configured trusted browser lifetime. When provider is set to `guardian`, the MFA prompt displays the enrollment checkbox for users to choose whether or not to enroll. Defaults to `false`. |
| `false` | The user is prompted for MFA each time they authenticate. |
  </Tab>
</Tabs>

The time values are for active users. If a user is inactive for a period of time, their cookie will expire and they will be prompted for MFA on their next login attempt (regardless of the Remember Me setting).
