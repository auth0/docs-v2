...
If Remember Me is not enabled with the default of 30 days, you can set the [Auth0 Actions Post-Login API object](/docs/actions/reference/post-login/post-login-api-object) `allowRememberBrowser` property to `true`:
...
| Value | Description |
| --- | --- |
| `true` | When provider is set to `google-authenticator` or `duo`, the user is prompted for MFA once between 30 days depending on your configured preference. When provider is set to `guardian`, the MFA prompt displays the enrollment checkbox for users to choose whether or not to enroll. Defaults to `false`. To learn more, read [Customize Multi-Factor Authentication Pages ](/docs/secure/multi-factor-authentication/customize-mfa ) |
| `false` | The user is prompted for MFA each time they authenticate. |

The time values are for active users. If a user is inactive for a period of time, their cookie will expire and they will be prompted for MFA on their next login attempt (regardless if `allowRememberBrowser` is set to `true`). You can further configure configure the frequency of MFA prompts as well as one-time passwords with [Advanced Factor Configuration (AFC)](/docs/secure/multi-factor-authentication/customize-mfa/advanced-factor-configuration).
