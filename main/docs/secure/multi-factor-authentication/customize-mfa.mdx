...
| Value | Description |
| --- | --- |
| `true` | When provider is set to `google-authenticator` or `duo`, the user is prompted for MFA once between 30 days depending on your configured preference. When provider is set to `guardian`, the MFA prompt displays the enrollment checkbox for users to choose whether or not to enroll. Defaults to `false`. |
