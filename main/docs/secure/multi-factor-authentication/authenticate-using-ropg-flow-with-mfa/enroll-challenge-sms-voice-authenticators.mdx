...
To enroll with SMS or voice, they enroll with a phone number that can be challenged either with SMS or voice. Specific the parameters below to call the endpoint. The `oob_channels` parameter indicates how you want to send the one-time code to the user (SMS or voice).
...
Users should receive a message with a one-time code they need to provide to the application.
...
### Challenge user with a one-time code
...
Your application needs to prompt the user for the code sent in the message and should be set in the `binding_code` parameter. You can verify the code and get authentication tokens using the OAuth0 Token endpoint, specifying the `binding_code` and `oob_code` returned by the previous call:
