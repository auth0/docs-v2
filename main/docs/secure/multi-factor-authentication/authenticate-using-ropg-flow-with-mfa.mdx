...
The MFA API flow for ROPG also applies to the [Passwordless OTP grant](/docs/authenticate/passwordless/implement-login/embedded-login/relevant-api-endpoints) (`http://auth0.com/oauth/grant-type/passwordless/otp`) and [Refresh Token exchange](/docs/secure/tokens/refresh-tokens/use-refresh-tokens) (`oauth2-refresh-token`). The same endpoints, error responses, and challenge patterns apply across all three grant types.
</Callout>
