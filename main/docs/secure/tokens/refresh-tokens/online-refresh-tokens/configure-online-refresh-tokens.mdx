...
The response contains a new access token and optionally a new [ID token](/docs/secure/tokens/id-tokens):
...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
Auth0 does not issue a new `refresh_token` and as ORTs do not rotate, you continue using the same token for subsequent exchanges.
</Callout>
...
* Use the `event.refresh_token` object to determing if the token is an ORT.
