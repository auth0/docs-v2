...
You can **set** refresh token metadata during any of the supported flows above. However, existing metadata is only **available to read** via the `event.refresh_token.metadata` object during [refresh token exchange](/docs/secure/tokens/refresh-tokens/use-refresh-tokens) scenarios where `event.refresh_token` exists. At the initial login, `event.refresh_token` does not exist, so you can only set metadata—not read it.
</Callout>
