...
A single `post-login` Action can handle different `grant_type` scenarios using the `event.request.body.grant_type` object to manage claim persistence. The `event.refresh_token` object is only read available during refresh token exchanges.  
</Callout>
