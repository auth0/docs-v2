...
<Note>
When agents as principals is enabled for your tenant and agent subject claims are enabled for your API, access tokens can include additional identity claims (`client_profile`, `sub_profile`, and `act`) and an adjusted `sub` value. These claims follow the [OAuth 2.0 Entity Profiles](https://datatracker.ietf.org/doc/draft-mora-oauth-entity-profiles/) model and [RFC 8693](https://datatracker.ietf.org/doc/html/rfc8693). Agents as principals is an Early Access feature. These claims are added to the access token only; ID tokens are not affected.
</Note>
...
| `sub`                   | The subject claim indicates which user or application the access token was issued for:<br/>- For grants where an end user is involved (e.g., Authorization Code Flow), the `sub` claim is the `user_id`.<br/>- For Machine-to-Machine apps using Client Credentials (no end user), the `sub` claim is a unique identifier for the app.<br/>- For an agent-linked application using Client Credentials, when agent subject claims are enabled for the API, the `sub` claim is the agent identifier. In this case, and for non-agent Machine-to-Machine apps on an API with agent subject claims enabled, the `@clients` suffix is not appended.                                           | Yes                         | Yes                      | - User ID: `auth0\|6553da60a54af58e29493993`<br/>- Client ID with suffix: `awZfdIir8YFdGZWkvCejDoUb7SjTDicx@clients`<br/>- Agent ID: `agt_72jbvv7LfRKYp59gtRLtkn`        |
...
| `client_profile`        | Entity profile of the application requesting the access token. Present when agent subject claims are enabled for the API.                                                                                                                                                                                                                                                                                            | Case-specific               | Case-specific            | `native_app`, `web_app`, `browser_app`, `service`; `service ai_agent` for an agent-linked application                       |
| `sub_profile`           | Entity profile of the subject the access token was issued for. Present when agent subject claims are enabled for the API.                                                                                                                                                                                                                                                                                             | Case-specific               | Case-specific            | `user`, `service`, `ai_agent`, `service ai_agent`                                                                          |
| `act`                   | Actor claim ([RFC 8693](https://datatracker.ietf.org/doc/html/rfc8693)) identifying the party acting on behalf of the subject. When an agent acts on behalf of a user, the agent is the outermost actor and any prior delegation chain is preserved as a nested `act`. See [The act claim](/docs/secure/call-apis-on-users-behalf/on-behalf-of-token-exchange#the-act-claim).                                            | Case-specific               | Case-specific            | `{ "sub": "agt_72jbvv7LfRKYp59gtRLtkn", "sub_profile": "ai_agent", "client_id": "K1AUPhZq...", "iss": "https://tenant.auth0.com/" }` |
