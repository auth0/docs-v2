...
<Note>
When Agent as Principal is enabled for your tenant and agent subject claims are enabled for your API, access tokens can include additional identity claims (`sub_profile`, `client_profile`, and `act`) and an adjusted `sub` value. These claims are added to the access token only; ID tokens are not affected. To learn more, read [Agent identity in tokens](/docs/ai-agents-mcp/agent-as-principal/agent-identity-in-tokens).
</Note>
...
| `sub`                   | The subject claim indicates which user or application the access token was issued for:<br/>- For grants where an end user is involved (e.g., Authorization Code Flow), the `sub` claim is the `user_id`.<br/>- For Machine-to-Machine apps using Client Credentials (no end user), the `sub` claim is a unique identifier for the app.<br/>- For client credentials grants where the client is agent-linked, the `sub` claim is the agent's `agent_id`, or `external_agent_id` if one was set at creation. The `@clients` suffix is not appended in this case, nor for non-agent Machine-to-Machine apps on an API with agent subject claims enabled. See [Agent as Principal](/docs/ai-agents-mcp/agent-as-principal/agent-identity-in-tokens). | Yes                         | Yes                      | - User ID: `auth0\|6553da60a54af58e29493993`<br/>- Client ID with suffix: `awZfdIir8YFdGZWkvCejDoUb7SjTDicx@clients`<br/>- Agent ID: `agt_72jbvv7LfRKYp59gtRLtkn` |
...
| `sub_profile`           | Entity type of the subject. Issued when `agent_subject_claims: 'auth0-v1'` is enabled on the resource server and an agent-linked client is involved. See [Agent as Principal](/docs/ai-agents-mcp/agent-as-principal/agent-identity-in-tokens). Values: `user`, `ai_agent`, `service`, `browser_app`, `native_app`. | Case-specific               | Case-specific            | `"ai_agent"`                                                                                                                |
| `client_profile`        | Entity type of the requesting client. Issued under the same conditions as `sub_profile`. Multiple space-separated values permitted. See [Agent as Principal](/docs/ai-agents-mcp/agent-as-principal/agent-identity-in-tokens).                                                                                        | Case-specific               | Case-specific            | `"service ai_agent"`                                                                                                        |
| `act`                   | Actor claim per [RFC 8693](https://datatracker.ietf.org/doc/html/rfc8693). Present when an agent-linked client is involved in a token exchange or standard login flow. Contains `sub`, `sub_profile`, `client_id`, `client_profile`, and optionally a nested `act` for multi-hop delegation chains. When an agent acts on behalf of a user, the agent is the outermost actor and any prior delegation chain is preserved as a nested `act`. See [Agent as Principal](/docs/ai-agents-mcp/agent-as-principal/agent-identity-in-tokens) and [The `act` claim](/docs/secure/call-apis-on-users-behalf/on-behalf-of-token-exchange#the-act-claim).                    | Case-specific               | Case-specific            | `{"sub": "agt_1a2b3c", "sub_profile": "ai_agent", "client_id": "agent-client-id"}`                                         |
