...
<Note>
When agents as principals is enabled for your tenant and agent subject claims are enabled for your API, access tokens can include additional identity claims (`sub_profile`, `client_profile`, and `act`) and an adjusted `sub` value. These claims are added to the access token only; ID tokens are not affected. Agents as principals is an Early Access feature. To learn more, read [Agent identity in tokens](/docs/ai-agents-mcp/agents-as-principal/agent-identity-in-tokens).
</Note>
