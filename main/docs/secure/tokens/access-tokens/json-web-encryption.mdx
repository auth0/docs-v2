---
description: Encrypt access token payloads using JSON Web Encryption (JWE) so only the intended resource server can inspect the claims, protecting against data breaches and intermediary inspection.
