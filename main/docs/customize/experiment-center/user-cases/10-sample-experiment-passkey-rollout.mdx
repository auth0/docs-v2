---
title: "Sample Experiment: Passkey Rollout"
description: "End-to-end walkthrough of a gradual passkey rollout using a percentage-allocation experiment."
---

# Sample Experiment: Gradual Passkey Rollout

This guide walks through a real use case: rolling out passkey enrollment prompts to 10% of users, then scaling up based on results. This is the anchor use case for the Experiment Center.

## The scenario

You want to introduce passkey enrollment to your users but can't flip the feature on for everyone at once. A misconfigured rollout could confuse users or increase support load. The Experiment Center lets you start with 10% of users, observe what happens in tenant logs, and expand when you are confident.

## What you will build

- A feature flag with a `passkey_enabled` boolean parameter
- Two variations: control (`false`) and treatment (`true`)
- A percentage-allocation experiment (10% treatment, 90% control)
- A `post_login` Action that reads `event.experimentation` and conditionally prompts passkey enrollment
...
- A `post_login` Action you can deploy
...
Create a percentage-allocation experiment with a 10/90 split. The `subject: "device"` setting means assignment uses a device-level identifier, giving cross-session consistency for pre-authentication flows (Decision J).
...
Deploy the Action to your test tenant and set it as the `post_login` trigger.
...
## Ramping up: moving to a higher percentage
...
:::note
**Beta limitation:** Only one experiment can be active per tenant at a time (Decision H). You must complete or pause the current experiment before activating a new one.
:::
...
The Experiment Center does not provide built-in analytics at Beta. You analyze results in your own tools (Amplitude, Statsig, Snowflake, Datadog, etc.) using the enriched tenant log data.
