...
When an experiment is active and a variation is assigned, the Experiment Center injects experiment context into the [page template](/docs/customize/login-pages/universal-login/customize-templates#customize-universal-login-page-templates) rendering context automatically. 
Page templates do not require an opt-in step: experiment context is passed to all templates unconditionally when an experiment is active.
...
When you use [partials](https://auth0.com/docs/customize/login-pages/universal-login/customize-signup-and-login-prompts#use-the-auth0-dashboard-to-manage-partials) (sub-templates included from a main template), you pass the experiment context to the partial explicitly:
