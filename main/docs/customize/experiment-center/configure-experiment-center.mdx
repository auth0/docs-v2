...
The `experiment` field appears on every auth event where an experiment was active during the transaction. The `variation_id` tells you which variation this user was assigned to.

If you do not see the `experiment` field, check the [troubleshooting guide](/docs/customize/experiment-center/troubleshooting).
...
- Read the [Actions integration guide](/docs/customize/experiment-center/use-cases/actions-integration-guide) to use `event.experimentation` in post-login and registration triggers.
