...
description: "Troubleshoot Experiment Center errors with diagnostic steps and solutions."
---
...
Logs are not displaying the `experiment` field, or assignments are not appearing in tenant logs.
...
Only one experiment can be active per tenant. List active experiments and pause or complete any that are running:
...
Your tenant does not have the `experiment_center_beta` feature flag enabled. Contact the Auth0 Support to confirm your tenant is enrolled in Beta and the feature flag is ON.
