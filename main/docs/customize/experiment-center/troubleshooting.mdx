...
Logs are not displaying `details.experiment`, or assignments are not appearing in tenant logs.
...
## I don't see `details.experiment` in my tenant logs

**Check 1: Is the experiment active?**

The `details.experiment` object is only stamped on auth events when an experiment was active during the transaction. See the steps above to confirm the experiment is `active`.
...
If you are reading directly from the Auth0 Dashboard (Monitoring > Logs), the enriched fields appear in the event's raw JSON. Select the event to expand the full JSON payload and look for the `experiment` key under `details`.
