...
4. Select **Send Test Message** to test the tenant-level provider before using it in a production environment.
    <Callout icon="file-lines" color="#0EA5E9" iconType="regular">
    Invalid phone numbers return an error. To learn about phone validation, read [What validation occurs when using the /passwordless/start Endpoint for SMS](https://support.auth0.com/center/s/article/What-validation-occurs-when-using-the-passwordless-start-endpoint-for-SMS).
    </Callout>
