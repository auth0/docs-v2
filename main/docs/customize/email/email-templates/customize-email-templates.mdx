...
* [Configure an external SMTP email provider](/docs/customize/email/smtp-email-providers) rather than using Auth0's built-in SMTP email provider.
...
Message bodies have HTML content. Liquid is supported in every field (except **URL Lifetime**)

To learn about Liquid, view the [Liquid introduction](https://shopify.github.io/liquid/basics/introduction/) for a brief explanation of the three components of Liquid syntax: objects, tags, and filters.

View [supported Liquid syntax](TODO) and also the [variables available to each email template](TODO).
