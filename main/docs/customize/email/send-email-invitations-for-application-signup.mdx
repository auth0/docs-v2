...
This implementation relies on a user metadata property (named `user.app_metadata.needsInvitation` in these instructions) which indicates that the user has an outstanding invitation: when it's `true`, the user should receive the user invitation experience, and otherwise, they should receive the regular password reset experience.
...
To handle this, [create a post-login Action](/docs/customize/actions/explore-triggers/signup-and-login-triggers/login-trigger) that sets `user.app_metadata.needsInvitation` to `false` using `api.user.setAppMetadata`.
