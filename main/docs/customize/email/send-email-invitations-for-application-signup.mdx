...
* Create a `user.app_metadata` parameter to indicate that the user has an outstanding invitation. For example, set `user_app_metadata.needsInvitation` to `true`.

Configure your application to set your custom `user.app_metadata` invitation variable to `false` once the user successfully sets their password. This is so the user receives the password change experience in the future.
