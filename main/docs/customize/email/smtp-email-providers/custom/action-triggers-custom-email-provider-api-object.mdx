...
| `options.expires_at` | Optional number | The absolute expiry time in milliseconds since the Unix epoch. While cached records may be evicted earlier, they will never remain beyond the supplied `expires_at`. If both `options.expires_at` and `options.ttl` are included, the earlier expiry of the two is used.
