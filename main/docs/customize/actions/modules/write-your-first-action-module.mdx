---
mode: wide
description: Describes how to use Auth0 Actions Modules
    ...
    <Tabs>
      ...
      */}
      <Tab title="Dashboard">
        ...
        ```
    <Warning>
      **Action Modules do not have their own runtime.** They run in the runtime of the Action that includes them.

      When you build an Action Module, Auth0 builds it with whatever runtime is being used in the builder. However, when the module is executed, it uses the runtime of the Action that imports it.

      If your Action Module uses Node.js API calls that are deprecated in Node.js 22, Actions using Node.js 18 runtime will work fine, but Actions using Node.js 22 runtime will fail. Write Action Modules using Node.js APIs that are compatible with all runtime versions you plan to support.
    </Warning>
