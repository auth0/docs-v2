...
<Steps titleSize="h3">
  ...
  <Step title="Add a Secret" stepNumber={2}>
    ...
    <Tabs>
      ...
      <Tab title="Management API">
        Let’s store the external service URL as a Secret. Call [Update an Actions Module](/docs/api/management/v2/actions/patch-action-module) with the `update:actions` scope:
      ...
      <Tab title="Auth0 CLI">
        Let’s store the external service URL as a Secret. Call [Update an Actions Module](/docs/api/management/v2/actions/patch-action-module) with `auth0 api`:
