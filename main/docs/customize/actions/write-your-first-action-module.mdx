---
description: Learn how to write an Action Module, which includes choosing a flow, creating an Action Module and configuring it, and binding it to the flow.
...
* [Attach the Action Module to an Action](#attach-the-action-module-to-an-action)
* [Test the Action](#test-the-action)
* [Deploy the Action](#deploy-the-action)
* [See the results of an Action in Real-time Logs](#observe-actions-in-real-time-logs)
* [See the results of an Action in Tenant Logs](#observe-actions-in-tenant-logs)

## Prerequisites

Because this Action sends messages to a Slack channel, you need to [create an Incoming Webhook for a Slack Workspace](https://api.slack.com/messaging/webhooks). Once you have a Slack Webhook URL, you can continue with this guide.

## Create an Action Module

To get an Action Module available for Actions to use it, you need to create the Action Module and then add it to an Action.
...
2. Enter a **Name**, then select **Apply**.
...
You’ll see the Secret has been added to the Action and you can use the new secret via intelligent code complete by typing `event.secrets`.
...
Once a Secret has been created, its value is never revealed. Auth0 encrypts all Secrets and stores them securely.
...
3. Select **Create**, and the Dependency appends to the Action:
...
When you save this Action, the latest version of your dependency is resolved and replaced with a specific version number to keep future updates to the package from breaking your Action.
...
Now that the Action has been configured with a Secret and a dependency, you're ready to add custom logic.
...
You can use your Action to notify Slack. Add this code to your Action:
...
Now that the Action is working as expected, it’s time to deploy it.
