...
available in later transactions even if they are within their specified
lifetime.
...
Values stored in this cache are scoped to the tenant (or Action) in which they are set.
...
<em>Optional number</em>. The absolute expiry time in milliseconds
            since the Unix epoch. Records never persist beyond <code>expires_at</code>. If both <code>ttl</code> and <code>expires_at</code> are provided, the earlier expiry is used.
...
<em>Optional number</em>. The time-to-live value of this cache entry
            in milliseconds. Recirds never persist beyond this duration. If both <code>ttl</code> and <code>expires_at</code> are provided, the earlier expiry is used.
          </p>
