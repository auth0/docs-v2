...
Read Enterprise Groups synced from an Enterprise connection that the user is a member of. Requires Inbound SCIM or Google Workspace directory sync to be enabled for the connection.

### `api.groups.getUserGroups(options)`

Returns a paginated list of Enterprise Groups the current user belongs to. To add group data to tokens, see [`api.idToken.setCustomClaim`](https://auth0.com/docs/customize/actions/flows-and-triggers/login-flow/api-object#api-idtoken-setcustomclaim) and [`api.accessToken.setCustomClaim`](https://auth0.com/docs/customize/actions/flows-and-triggers/login-flow/api-object#api-accesstoken-setcustomclaim).

<ResponseField name="options" type="dictionary" post={["optional"]}>
 Optional. JSON object with pagination properties: `take` (integer, 1–100; defaults to 50) and `from` (pagination cursor from the `next` property of a previous response). 
</ResponseField>
...
Checks whether the current user belongs to any of the specified Enterprise Groups, and returns details about matching groups.

<ResponseField name="groups" type="array">
 An array of group identifiers (IDs or names) to check membership against. 
</ResponseField>

```js Check group memberships
