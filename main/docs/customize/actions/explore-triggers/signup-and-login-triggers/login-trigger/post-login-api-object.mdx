...
<td>
<p><em>Optional object</em>. An object that can contain the following optional fields: </p>
<ul>
<li><code>additionalFactors</code> - An array used to specify other factors a user can choose from when completing the MFA challenge. Supports the same values as the <code>type</code> field.</li>
<li><code>otpFallback</code> - When set to <code>false</code> and using <code>type: 'push-notification'</code>, the user cannot use the OTP fallback option of the push notification factor.</li>
</ul>
<p>Examples</p>
```js lines
// Using additionalFactors
api.authentication.challengeWith({
  type: 'otp'
}, {
  additionalFactors: [{
...
// Using otpFallback with push-notification
api.authentication.challengeWith({
  type: 'push-notification',
...
### `api.refreshToken.setMetadata(key, value)  [Early Access]`

Sets a key-value pair in the metadata object of the current session. Setting the value to `null` deletes the metadata key.
  ...
  <table class="table">
    ...
    <tbody>
      <tr>
        ...
        <td>
          <p>
            <em>String</em>. The key of the metadata property to be set.  No special characters and a maximum of 256 characters.
          </p>
        </td>
      </tr>
      <tr>
        ...
        <td>
          <p>
            <em>String</em>. The value of the metadata property. No special characters and a maximum of 256 characters. Setting the value to <code>null</code> removes the metadata property. 
          </p>
        </td>
      </tr>
    </tbody>
...
### `api.refreshToken.deleteMetadata(key) [Early Access]`

Deletes a key in the metadata object of the current session.
  ...
  <table class="table">
    ...
    <tbody>
      <tr>
        ...
        <td>
          <p>
            <em>String</em>. The key of the metadata property to delete.
          </p>
        </td>
      </tr>
    </tbody>
...
### `api.refreshToken.evictMetadata() [Early Access]`
...
### `api.session.setMetadata(key, value)  [Private Early Access]`
...
### `api.session.evictMetadata() [Private Early Access]`
