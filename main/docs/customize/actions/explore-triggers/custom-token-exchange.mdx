...
Issuing a Session Transfer Token with the actor in its context — so a delegated web session can be established for another user — is also supported. Read [Session Delegation](/docs/authenticate/single-sign-on/session-delegation) to learn more.
