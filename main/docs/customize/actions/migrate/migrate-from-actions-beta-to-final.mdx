...
Migrating an Action created during the period prior to [General Availability (GA)](/docs/troubleshoot/product-lifecycle/product-release-stages#general-availability) should typically involve the following steps:
...
3. For Actions that need to handle redirect callbacks, use the newly-exposed dedicated function. If you used code that relied on `event.protocol === 'redirect-callback'`, review the [Redirect with Actions](/docs/customize/actions/explore-triggers/signup-and-login-triggers/login-trigger/redirect-with-actions) page.
