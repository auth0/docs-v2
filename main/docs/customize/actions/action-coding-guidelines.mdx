...
* When using [`@auth0/actions`](https://www.npmjs.com/package/@auth0/actions?activeTab=readme) for [`Actions Unit Test`](/docs/customize/actions/unit-test/overview) implementation

  * Separate unit tests per function available at each trigger, for example: unit test separately `onExecutePostLogin` and `onContinuePostLogin`.
  * Don't test a sequence of functions or Actions, the framework provided is focused on doing this kind of test.
  * Use the `getDefaultArguments` to get default event and api object fixtures, mock the values and definitions in them when suitable.
  * Use the `loadAction` to handle the execution of each Action function in the unit test.
