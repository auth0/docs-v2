...
Data capture is available for: 
* Database connections authenticated by password. 
* <Tooltip tip="Passwordless: Form of authentication that does not rely on a password as the first factor." cta="View Glossary" href="/docs/glossary?term=passwordless">Passwordless</Tooltip> connections authenticated by email or SMS one-time password.
* Social connections.
...
When using Signup screens:

* Data from the `passkey-enrollment` and `passkey-enrollment-local` prompts are acessible on the Post Login trigger, and if a validation error is returned, the validation error is forwarded to the customer’s application error page. .  
