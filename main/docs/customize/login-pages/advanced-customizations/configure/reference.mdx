---
description: Reference for the ACUL rendering configuration object and its properties
...
You can manage the rendering configuration through the Auth0 Dashboard, Infrastructure-as-Code tools, or the Auth0 Management API. To learn how, read [Configure ACUL](/docs/customize/login-pages/advanced-customizations/configure/overview). 

For the full API schema, review the [`PATCH /v2/prompts/{prompt}/screen/{screen}/rendering`](/docs/api/management/v2/prompts/patch-rendering) endpoint.
...
Each value you want available in your screen must be explicitly listed in the `context_configuration` array. To learn more, read [Configure ACUL, Additional Data](https://auth0.com/docs/customize/login-pages/advanced-customizations/configure/overview#auth0-dashboard). 
...
* `user.app_metadata.YOUR_KEY` for user app metadata.
* `user.user_metadata.YOUR_KEY` for user metadata. 
