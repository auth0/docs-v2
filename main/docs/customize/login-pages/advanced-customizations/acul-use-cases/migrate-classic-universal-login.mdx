...
Auth0 CLI initializes your project with pre-built `login-id` and `login-password` authentication screens. The generated template includes a React-based development harness that provides local preview and debugging tools (including the UL Context Inspector via `DevScreenManager.tsx`). While your final deployed screens can use any framework or standalone JavaScript, the local development tooling uses React.

To learn more, read [auth0 aclu on GitHub](https://auth0.github.io/auth0-cli/auth0_acul_init.html). 

## Customize ACUL screens 

Customize the `login-id` and `login-password` screens to align to your organization standards. You can build your screens using your preferred framework (React, Vue, Angular, vanilla JavaScript, etc.).

You should consider the following:

  * Reuse existing CSS/theme and UI components from your framework of choice.
...
This command starts a local development server with the React-based Context Inspector harness that wraps your custom screens for debugging. The harness is only used during local development; your deployed screens run standalone.
