...
Auth0 CLI initializes your project with pre-built `login-id` and `login-password` authentication screens. The generated template uses React and includes development tools like the `DevScreenManager.tsx` and Context Inspector components.

<Callout icon="info-circle" color="#0EA5E9">
While ACUL supports any frontend framework or vanilla JavaScript, the CLI-generated templates are React-based to provide a complete starting point. You can adapt these screens to your preferred framework by using the [ACUL JavaScript SDK](https://auth0.github.io/auth0-acul-js/) directly or reference the [vanilla JavaScript examples](https://github.com/auth0-samples/auth0-acul-samples) in the sample repository.
</Callout>
...
You should consider the following:
  ...
  * Reuse existing CSS/theme and UI components (React components if using the template, or your framework's components if adapting).
...
This command uses the `DevScreenManager.tsx` component (included in the React template) to wrap your screens with the Context Inspector for debugging. If you're using a non-React framework, you'll need to manually integrate the Context Inspector or develop without it.
