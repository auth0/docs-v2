...
title: Add WebAuthn with Biometrics authentication to ACUL
...
You need:
	...
	* Enable [dentifier First + Biometrics](/docs/authenticate/login/auth0-universal-login/passwordless-login/webauthn-device-biometrics) 
...
To add support for WebAuthn with Device Biometrics authentication, using ACUL, you need to add a `POST` request to the `login-password` screen.
...
`POST` request example to add to the `login-password` screen:
