...
Local development
Local development with the Auth0 CLI command `auth0 aclu dev --connected` allows you to quickly iterate your ACUL screens without any delays and or backend dependencies. 

Live tenant integration testing
Live tenant integration with the Auth0 CLI command ``auth0 test login`` allows for End-to-End testing and verifies integration with your Auth0 tenant. 
...
It utilizes GitHub Actions to automatically:
	...
	* Build your custom login screen application
		* The workflow reads the [config/deploy_config.yml](YOUR-LINK-HERE) file to determine which screens are marked for deployment. 
		* If deployment targets are found, the workflow builds your ACUL application. For example, By using Vite and compiling the code into the `/dist` directory.
	...
	* Configure your Auth0 tenant 
		* The workflow uses [Auth0 CLI](https://auth0.github.io/auth0-cli/) with your M2M application.
...
* [config/screen-to-prompt-mapping.js](YOUR-LINK-HERE): Maps your internal screen directory names. For example, `mfa-sms-challenge` to the official Auth0 prompt names `mfa-sms`.
