---
description: Learn how to configure custom domains where Auth0 manages the TLS
  (SSL) certificates.
...
Auth0 recommends turning off CNAME flattening unless it's strictly necessary because CNAME flattening for Auth0 managed certificates is an unsupported configuration and may cause the custom domain to break without notice.

</Callout>

If you need to enable CNAME flattening for all subdomains managed by Cloudfare and also configure a specific subdomain to be an Auth0 custom domain, consider [delegating the subdomain](https://developers.cloudflare.com/dns/manage-dns-records/how-to/subdomains-outside-cloudflare/) for Auth0 to another DNS provider.
This will enable you to use CNAME flattening for all subdomains except the one used for Auth0.
