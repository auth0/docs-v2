...
### Scenario 1: Single-brand, single-domain
...
### Scenario 2: Multi-brand, separate domains
...
### Scenario 3: Multi-tenant with default domain

**Setup**: Multiple customers, with a [default custom domain](/docs/customize/custom-domains/multiple-custom-domains/default-domain) for shared services
...
### Pattern 1: Prompt for passkey enrollment per domain
...
### Pattern 2: Track passkey enrollment by domain
...
### Pattern 3: Domain-specific enrollment pages
...
### Pattern 4: Contextual enrollment prompts

Show passkey enrollment prompts based on user behavior:

```javascript
class PasskeyManager {
  constructor(auth0Client, customDomain) {
    this.auth0 = auth0Client;
    this.domain = customDomain;
    this.storageKey = `passkey_prompt_dismissed_${customDomain}`;
  }

  async shouldShowEnrollmentPrompt() {
    ...
    // Check if passkey enrolled for this domain
    const passkeyEnrolled = claims.passkey_enrolled &&
                             claims.passkey_domain === this.domain;

    if (passkeyEnrolled) {
      return false;
    }

    // Check login frequency (from user metadata)
    const loginCount = user.logins_count || 0;

    // Show prompt after 3rd login if passkey not enrolled
    return loginCount >= 3;
  }
  ...
  async showEnrollmentPrompt() {
    if (await this.shouldShowEnrollmentPrompt()) {
      // Show UI prompt
      const shouldEnroll = confirm(
        'Enable passkey for faster and more secure login?'
      );
...
<td>Use a default domain for most authentication, or guide users to enroll on each domain</td>
</tr>
<tr>
...
1. **Keep original domain active**: Maintain the original custom domain as the [default domain](/docs/customize/custom-domains/multiple-custom-domains/default-domain)
...
1. **Use a default domain**: Configure a [default custom domain](/docs/customize/custom-domains/multiple-custom-domains/default-domain) to minimize the number of domains requiring passkey enrollment
