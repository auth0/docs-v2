...
When using multiple custom domains with Back-Channel Logout (BCLO) or universal logout, your logout endpoint must handle tokens from all configured custom domains.

<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
Auth0 allows only one Back-Channel Logout URL per application, even when using multiple custom domains. Your endpoint must accept and validate logout tokens from any of your configured custom domain issuers.
</Callout>

**Example scenario:**

Your application has two brands, each with their own custom domain:
- `brand1.app.com` → `brand1.custom-domain.com` (Auth0 issuer)
- `brand2.app.com` → `brand2.custom-domain.com` (Auth0 issuer)

You configure a single Back-Channel Logout URL in your application settings:
```
https://brand1.app.com/back-channel-logout
```

This endpoint must handle logout tokens from both `brand1.custom-domain.com` and `brand2.custom-domain.com`.

**Implementation requirements:**

1. **Accept tokens from all configured issuers**: Your BCLO endpoint becomes a canonical endpoint that processes logout tokens regardless of which custom domain issued them.

2. **Validate token authenticity**:
   - Extract the `iss` (issuer) claim from the logout token
   - Validate the token signature against the JWKS endpoint for that issuer
   - Verify the token is valid and not expired

3. **Match subject and session**:
   - Match the `sub` (subject) and `sid` (session ID) claims to the user's session
   - Invalidate the session for that user across your application

4. **Handle universal logout**: The same approach applies to Auth0's universal logout (front-channel logout). Your logout handler must accept logout requests originating from any of your configured custom domains.

**SDK implementation:**

When implementing BCLO with Auth0 SDKs in a multiple custom domain setup:
- Do not restrict issuer validation to a single domain
- The SDK validates tokens by reading the `iss` claim and using it to fetch the appropriate JWKS
- Match sessions based on `sub` and `sid` rather than issuer alone

<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
The Auth0 SDK's BCLO implementation dynamically validates tokens by extracting the `iss` claim, using it as the domain, and validating the token against that issuer's JWKS. This allows the endpoint to handle logout tokens from multiple custom domains without requiring a pre-configured list of allowed issuers.
</Callout>
