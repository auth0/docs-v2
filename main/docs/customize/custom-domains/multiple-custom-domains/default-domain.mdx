...
curl --request PATCH \
  ...
  --data '{
    "domain": "custom_domain_name || canonical_domain_name",
    "is_default": true
  }'
```
```javascript Node.js
...
await management.customDomains.update(
  { id: '{customDomainId}' },
  { domain: 'custom_domain_name || canonical_domain_name', is_default: true }
);
```
```python Python
...
isDefault := true
customDomainName := "custom_domain_name || canonical_domain_name"
...
customDomain.setDomain("custom_domain_name || canonical_domain_name");
