...
For applications using [Lock for Web](https://github.com/auth0/lock):

```javascript
const lock = new Auth0Lock('YOUR_CLIENT_ID', 'login.example.com', {
  configurationBaseUrl: 'https://cdn.us.auth0.com',  // Required with custom domains
  auth: {
...
**Important**: The `configurationBaseUrl` parameter is required when using custom domains with Lock. Use the appropriate regional CDN URL:
- US: `https://cdn.us.auth0.com`
- EU: `https://cdn.eu.auth0.com`
- AU: `https://cdn.au.auth0.com`
- JP: `https://cdn.jp.auth0.com`
...
const getUsersWithCustomDomain = async (customDomain) => {
  ...
  const response = await axios.get('https://tenant.auth0.com/api/v2/users', {
    headers: {
      'Authorization': `Bearer ${token}`,
      'auth0-custom-domain': customDomain  // Specify custom domain
    }
  });
...
domain = 'tenant.auth0.com'  # Use canonical domain
