...
With MCD currently in Early Access (EA), we are aware of limitations to the functionality and existing feature set. Please see below for more information:
	...
	* **Custom Domains with Classic MFA and Guardian**: When using multiple custom domains with Classic MFA or Guardian, the associated notifications always use (and default to) canonical domain.
