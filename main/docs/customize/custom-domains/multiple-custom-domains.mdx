...
* Your tenant is on an Enterprise plan ([Public Cloud or Private Cloud deployments](/docs/deploy-monitor/deployment-options)). For more information, see [Manage Subscriptions](/docs/manage-subscriptions).
...
* EA allows up to 1,000 custom domains per tenant for experimentation and validation. Once MCD GA launches, the base entitlement of up to 20 custom domains will apply. If you have more than 20 domains configured during EA, you will need to purchase an additional add-on SKU to maintain those domains or remove excess domains before the GA transition. Contact your Auth0 account team to discuss add-on pricing.
* You must be able to prove ownership of the configured custom domains.

## Configure Multiple Custom Domains

Auth0 will enable the MCD feature on tenants requested by EA participants. You can then add MCD to a tenant with the <Tooltip tip="Management API: A product to allow customers to perform administrative tasks." cta="View Glossary" href="/docs/glossary?term=Management+API">Management API</Tooltip> or in the <Tooltip tip="Auth0 Dashboard: A web interface to allow customers to manage their Auth0 configuration." cta="View Glossary" href="/docs/glossary?term=Auth0+Dashboard">Auth0 Dashboard</Tooltip>.
...
If successful, you should see the following returned with the details of your new custom domain, including its verification status. The `status` is initially returned as `pending`.
...
With MCD currently in Early Access (EA), we are aware of limitations to the functionality and existing feature set. Please see below for more information:

	* The WebAuthn RPID is currently limited to a single domain configuration and is currently incompatible with customers using multiple custom domains.
