...
- **No-code visual builder**: Compose portal pages by dragging and dropping pre-built identity components. Configure layout, navigation, and behavior without writing code.
- **Reusable components**: Universal Portals uses the same [Universal Components](/docs/get-started/universal-components/universal-components-overview) library available for embedding directly in your own application, so you build once and reuse the same components across both delivery modes.
- **Custom components**: Extend beyond the pre-built library by building your own components for functionality unique to your use case, and publish them to a shared library for reuse across portals.
- **Granular permissions**: Control who can see each page (view-level) and who can interact with each element on it (component-level), enforced through claims.
- **Zero infrastructure to manage**: Portals are hosted by Auth0, so you get security best practices by default without hosting, building, or maintaining any of the underlying infrastructure yourself.
- **Public and private portals**: Serve both authenticated experiences, like My Account or My Organization, and anonymous experiences, like signup or KYC onboarding, from the same platform.
- **Standards-based**: Built on standard identity protocols rather than proprietary APIs.
- **Agent-ready**: Because portals are schema-first, portal pages and components can also be exposed as MCP resources, making them consumable by AI agents as well as end users.
