...
2. Locate the log stream you want to check in the list.
   ...
   * **Paused**: You have requested that we stop delivery attempts for the stream. You can click **Resume Stream** to change the status back to **Active**.
         <Callout icon="file-lines" color="#0EA5E9" iconType="regular">
           * When a log stream is paused, log events are not discarded. They are queued and will be delivered once the log stream is resumed.
           * The logs are held in the queue based on the tenant's [log retention](/docs/customize/log-streams#log-retention) period, if the stream is resumed within the retention window, all logs generated during the pause are delivered.
         </Callout>
