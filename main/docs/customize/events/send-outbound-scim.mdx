...
This implementation uses an event stream with our [outbound SCIM 2.0 user provisioning Action template](https://github.com/auth0/opensource-marketplace/blob/main/templates/outbound-scim-EVENT_STREAM/code.js).
...
* A SCIM server.
    ...
    * The SCIM server must support an `externalId eq` filter.
        ...
        If the server rejects the `externalId eq` filter, the Action falls back to filtering on `userName`, which the default attribute mapping sets to the user's email address. In this situation, the Action cannot track email changes because lookups on an updated email address don't match the original email address stored in `userName`.
