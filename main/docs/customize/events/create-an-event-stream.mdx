...
redirects:
  - "/docs/deploy-monitor/events/create-an-event-stream"
---

You can create event streams that listen for different [types of events](/docs/events), such as the creation of a new user profile. By subscribing to events and delivering them to a destination of your choice using event streams, you can facilitate a number of related use cases, including:
...
Before setting up an event stream, you need to identify the [event types](/docs/events) you want to monitor. Then, you will use your AWS account ID and region to set up your event stream, as demonstrated below.
...
Before setting up an event stream, you need to identify the [event types](/docs/events) you want to monitor. You will then use your webhook handler to create an event stream, as demonstrated below.
...
Before setting up an event stream, you need to identify the [event types](/docs/events) you want to monitor. This example uses the Auth0 CLI to create an event stream that subscribes to the `user.created` event, which triggers whenever your tenant registers a new user.
