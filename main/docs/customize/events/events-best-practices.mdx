...
redirects:
  - "/docs/deploy-monitor/events/events-best-practices"
---
...
To ensure you do not overwrite your data with stale information, check the `time` attribute of the incoming webhook against the timestamp of the data in your system. The `data.object` attribute in the payload may also include `created_at` and `updated_at` fields for operation-specific timestamps. See the event details in the [Event Catalog](/docs/events) for more information.
