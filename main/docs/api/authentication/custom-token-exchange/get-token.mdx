...
### Remarks
 ...
 - To request a Session Transfer Token instead of an access token, set `audience` to `urn:{yourDomain}:session_transfer`. Calling `setActor()` is required for this request; omitting it returns a `400` error: `setActor is required when requesting a Session Transfer Token`. Read [Session Delegation](/docs/authenticate/single-sign-on/session-delegation) to learn how to redeem the resulting token to establish a session on behalf of the subject user.

 - Requesting a Session Transfer Token requires the `cte_session_transfer_token` feature to be enabled for your tenant and client. If it isn't, the request returns a `400` error: `Session Transfer Tokens cannot be requested using Custom Token Exchange.`

 - Requesting a Session Transfer Token requires a confidential client — the calling application must authenticate with a Client Secret (or another confidential-client method). The `tokenEndpointAuthMethod: none` public-client option described above does not apply to Session Transfer Token requests.
