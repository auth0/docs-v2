...
### Remarks
 ...
 - Custom Token Exchange does not synthesize an agent actor. Even when agents as principals is enabled, the `act` claim is fully controlled by the Action through `setActor()`; Auth0 does not add an agent `act` automatically for this grant.
