...
Read [Custom Token Exchange](/docs/authenticate/custom-token-exchange) to understand Custom Token Exchange Profiles, how a profile maps to an Action, and how to enable this flow for your application.
 ...
 - Custom Token Exchange does not add an agent actor. Even when Agent as Principal is enabled, the `act` claim is fully controlled by the Action through `setActor()`; Auth0 does not add an agent `act` automatically for this grant.
