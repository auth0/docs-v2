...
<Note>
It is your responsibility to securely validate the user’s `subject_token`. See the [User Guide](/docs/authenticate/custom-token-exchange) for more details.
</Note>
...
Read [Custom Token Exchange](/docs/authenticate/custom-token-exchange) to understand Custom Token Exchange Profiles, how a profile maps to an Action, and how to enable this flow for your application.
 ...
 - When the Action calls `setActor()`, issued access tokens and ID tokens include an `act` claim representing the [delegation chain](/ai/docs/call-apis-on-users-behalf/on-behalf-of-token-exchange#the-act-claim). The `act` claim is also included in the `userinfo` response.
