...
- OBO token exchanges trigger the [`post-login` Action trigger](/docs/customize/actions/explore-triggers/post-login), where `event.transaction.protocol` is set to `oauth2-token-exchange` and `event.transaction.actor` tracks the complete delegation chain.
...
<ResponseField name="access_token" type="string">
  ...
  When an [agent-linked client performs](/docs/ai-agents-mcp/agent-as-principal) the exchange and agent subject claims are enabled for the downstream API, the agent is the outermost actor in the `act` claim, and the prior delegation chain from the subject token is preserved as the nested inner `act`. To learn more, read about [the act claim](/docs/secure/call-apis-on-users-behalf/on-behalf-of-token-exchange#the-act-claim).
</ResponseField>
