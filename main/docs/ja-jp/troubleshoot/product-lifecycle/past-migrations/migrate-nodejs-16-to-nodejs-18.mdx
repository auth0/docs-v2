...
## 一般的な注意点 {#general-considerations}

### ルールとフックを Actions に移行する {#migrate-rules-and-hooks-to-actions}
...
### マーケットプレイス連携 {#marketplace-integrations}

#### ソーシャル接続の連携 {#social-connections-integrations}
...
### 新しいカスタム Actions を作成 {#create-new-custom-actions}
...
### 既存の カスタム Actions をアップグレードする {#upgrade-existing-custom-actions}
...
### その他の拡張製品では Node 18 を選択する {#choose-node-18-for-other-extensibility-products}
...
## 既知の互換性のない変更 {#known-breaking-changes}

### Magic npm モジュール {#magic-npm-modules}
...
### Can I Require で削除されたモジュールバージョン {#can-i-require-module-versions-removed}
...
### TLS 接続ではデフォルトで安全な再ネゴシエーションが必須 {#secure-renegotiation-required-by-default-for-tls-connections}
