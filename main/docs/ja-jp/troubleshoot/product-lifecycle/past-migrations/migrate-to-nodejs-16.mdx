...
1. **［Extensibility（拡張性）］**パネルを使って開発テナントでNode 16を有効にします。このパネルは、Dashboardの[［Advanced Tenant Settings（テナントの詳細設定）］](%24%7Bmanage_url%7D/#/tenant/advanced)ページにあります。**［Runtime（ランタイム）］**ドロップダウンで**［Node 16］**を選択します。
2. **［Save（保存）］**をクリックします。
