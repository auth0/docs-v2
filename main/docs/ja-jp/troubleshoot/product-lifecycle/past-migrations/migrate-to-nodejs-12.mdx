...
### Node 12 ランタイムを有効にする {#enable-node-12-runtime}
...
ルール、Hooks、カスタムデータベーススクリプト、カスタムソーシャル接続については、[Management API](/docs/ja-jp/api) を使用して確認できます。これにより、テスト目的で項目を本番テナントから開発テナントへ移しやすくなります。

<Tooltip tip="Management API: お客様が管理タスクを実行できるようにする製品です。" cta="用語集を見る" href="/docs/ja-jp/glossary?term=Management+API">Management API</Tooltip> の Connections エンドポイントを使用すると、カスタムデータベーススクリプトは `options.customScripts` を使って取得または更新できます。同様に、カスタムソーシャル接続は `options.scripts.fetchUserProfile` で確認できます。
...
### 新しいURLを許可リストに追加 {#allowlist-new-urls}
...
### Authorization Extension のルールを再公開する {#republish-authorization-extension-rule}
...
### Delegated Administration の URL を設定する {#configure-delegated-administration-urls}
...
### SSO Dashboard の URL を設定する {#configure-sso-dashboard-urls}
...
### Extensions を更新する {#update-extensions}
...
### 固定したモジュールを更新する {#update-pinned-modules}
