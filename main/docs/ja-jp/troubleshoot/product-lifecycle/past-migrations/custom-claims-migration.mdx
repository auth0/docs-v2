...
### 制限対象のトークン audience {#restricted-token-audience}
...
## Auth0 および Webtask の名前空間に関する制限 {#restriction-on-auth0-and-webtask-namespaces}
...
### OIDC ユーザープロファイルのクレーム {#oidc-user-profile-claims}
...
### Auth0 ルールを使用した SAML2 add-on および Web Service Federation Protocol (WS-Fed) の属性マッピング {#saml2-add-on-and-web-service-federation-protocol-ws-fed-attribute-mapping-with-auth0-rules}
...
### トークンに非公開かつ名前空間なしのカスタムクレームを追加する {#add-private-non-namespace-claims-to-tokens}
...
### /userinfo に返される非公開かつ名前空間なしのカスタムクレーム {#private-non-namespace-claims-to-userinfo}
...
### テナントのログを確認する {#review-tenant-logs}
...
### SAML2 アドオンおよび Web Service Federation Protocol (Ws-Fed) 用のルールを修正する {#fix-auth0-rules-for-saml2-add-on-and-web-service-federation-protocol-ws-fed}
...
### 従来の動作を無効にする {#disable-legacy-behavior}
