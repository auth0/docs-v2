...
* `https://{yourDomain}/v2/logout`：`federated`クエリパラメーターを使用して、[IDプロバイダーからユーザーをログアウト](/logout/log-users-out-of-idps)
* `https://{yourDomain}/wsfed/{yourClientId}?wa=wsignout1.0`：`wreply`クエリを使用して、[WS-FedプロトコルでAuth0からログアウト](/protocols/ws-fed-protocol)
...
1. アプリケーションまたは証明書利用者が、Auth0の公開APIの[Authentication API:Logout](/api/authentication#logout)または[Authentication API:WS-Federation](/api/authentication#ws-federation)を使用して、ログアウトを開始する。
...
1. [［Auth0 Dashboard］>［Tenant Settings（テナントの設定）］>［Advanced（詳細設定）］](%24%7Bmanage_url%7D/#/tenant/advanced)に移動して、**［Migrations（移行）］**セクションを見つけます。
