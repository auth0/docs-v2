...
### 未移行のテナントにおける機能上の違い {#functional-differences-in-unmigrated-tenants}
...
## サイズ超過のユーザープロファイルを特定する {#identify-oversized-user-profiles}
...
### 上限のないユーザープロファイルデータに関する非推奨化ログの検索 {#query-uncapped-user-profile-data-deprecation-logs}
...
### ユーザープロファイルサイズの概算 {#ad-hoc-calculation-of-user-profile-size}
...
## サイズ超過のユーザープロファイルの根本原因に対処する {#address-the-root-cause-of-oversized-user-profiles}
...
### 外部アイデンティティプロバイダーから不要な属性が保存されないようにする {#prevent-storage-of-unneeded-attributes-from-external-identity-providers}
...
### 拡張を利用して追加のビジネスデータを動的に取得する {#use-extensibility-to-query-additional-business-data-dynamically}
...
### Enterprise Groups を使用してグループ情報を同期する {#use-enterprise-groups-to-sync-group-information}
...
## 移行を完了するためのオプトアウト {#opt-out-to-complete-migration}
