...
## どのような影響がありますか？ {#how-are-you-affected}

この移行による影響は、主に次の2点です。

### 1. Management API のデフォルト (非推奨化) {#1-management-api-default-the-deprecation}
...
### 2. Dynamic Client Registration (個別の設定) {#2-dynamic-client-registration-independent-configuration}
...
## 移行作業 {#migration-tasks}

### サードパーティアプリケーションを確認する {#review-your-third-party-applications}
...
### ステップ 1: 新しいサードパーティアプリケーションの作成方法を選択する {#step-1-choose-how-to-create-new-third-party-applications}
...
#### オプション A: 移行を完了する (推奨) {#option-a-complete-the-migration-recommended}

2026年10月23日までに移行を完了して、強化されたセキュリティ制御をデフォルトにしてください。この方法により、サードパーティアプリケーションを OAuth 2.1 のベストプラクティスに準拠させ、今後の機能に備えることができます。

##### 1. 強化されたセキュリティ制御をテストする {#1-test-enhanced-security-controls}
...
##### 2. デフォルト権限を設定する {#2-set-up-default-api-permissions}
...
##### 3. 互換性を検証する {#3-validate-compatibility}
...
##### 4. 移行を完了する {#4-complete-the-migration}
...
#### オプション B: 既存の動作をデフォルトとして維持する {#option-b-preserve-existing-behavior-as-the-default}
...
### ステップ 2: Dynamic Client Registration の扱いを選択する {#step-2-choose-how-to-handle-dynamic-client-registration}
...
#### 現在の DCR の動作を確認する {#review-current-dcr-behavior}
...
#### DCR セキュリティモードを設定する {#configure-dcr-security-mode}
...
## よくあるケース {#common-scenarios}

### シナリオ 1: 最新の OAuth を使用するパートナー連携 {#scenario-1-partner-integrations-using-modern-oauth}
...
### シナリオ 2: OIDC が必要なアプリケーション {#scenario-2-applications-requiring-oidc}
...
### シナリオ 3: Dynamic Client Registration (MCP、AI エージェント) {#scenario-3-dynamic-client-registration-mcp-ai-agents}
...
### シナリオ 4: Classic Login を使用するアプリケーション {#scenario-4-applications-using-classic-login}
...
### シナリオ5: Organizations を使用するサードパーティアプリケーション {#scenario-5-third-party-applications-with-organizations}
...
### 既存のアプリケーションのセキュリティモードは変更できますか？ {#can-i-change-the-security-mode-on-an-existing-application}

いいえ。`third_party_security_mode` はアプリケーションの作成時に設定されるため、後から変更することはできません。別のセキュリティモードを使用するには、新しいアプリケーションを作成してください。

### 既存のサードパーティアプリケーションはどうなりますか？ {#what-happens-to-my-existing-third-party-applications}

何も変わりません。既存のサードパーティアプリケーションは、現在とまったく同じように引き続き動作します。この移行で影響を受けるのは、新しく作成されるアプリケーションのデフォルト設定だけです。

### 同じテナント内で両方のセキュリティモードを使用できますか？ {#can-i-use-both-security-modes-in-the-same-tenant}

はい。一部のサードパーティアプリケーションには強化されたセキュリティ制御を適用し、他のアプリケーションには従来どおりの動作を維持できます。セキュリティモードはアプリケーションごとに設定されます。

### Dynamic Client Registration については？ {#what-about-dynamic-client-registration}
...
### 期限後も既存の動作でアプリケーションを作成できますか？ {#can-i-create-applications-with-existing-behavior-after-the-deadline}
...
### 今後の機能は既存の動作でも利用できますか？ {#will-future-features-work-with-existing-behavior}
