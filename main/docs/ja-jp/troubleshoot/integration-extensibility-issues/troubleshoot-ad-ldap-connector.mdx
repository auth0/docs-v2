...
[AD/LDAPコネクタ](/extensions/ad-ldap-connector)で問題が発生している場合は、以下をお読みいただいて、一般的な問題のトラブルシューティング方法を確認してください。
...
アクティブユーザーの数が増加し、上記のすべての手順が既に実行されている場合は、ホストマシンのハードウェア仕様をアップグレードするか、[高可用性環境](/extensions/ad-ldap-connector/ad-ldap-high-availability)を構成する必要があります。
...
コネクタをホストしているコンピューターがプロキシの背後にある場合は、`HTTP_PROXY`システム環境変数を設定するか、[AD/LDAPコネクタ構成ファイル](/extensions/ad-ldap-connector/ad-ldap-connector-config-file-schema)で`HTTP_PROXY`変数を設定できます。認証されたプロキシを使用する場合、URLは `http://USERNAME:PASSWORD@SERVER_URL:PORT`の形式にする必要があります。
...
サーバーがAuth0テナント（`https://${account.namespace}`）に接続できることを確認します。

これを確認するには、ブラウザを開き、`https://${account.namespace}/テスト`に移動します。
...
[カスタムドメイン](/custom-domains)の追加の構成を完了していない場合、このエラーが発生することがあります。

KerberosをサポートするAD/LDAP接続を使用するには、カスタムドメインで動作するように**［Ticket（チケット）］**エンドポイントを更新する必要があります。

1. [AD/LDAPコネクタ構成ファイル](/extensions/ad-ldap-connector/ad-ldap-connector-config-file-schema)を開きます。
2. `PROVISIONING_TICKET`構成変数を`https://${account.namespace}/p/ad/jUG0dN0R`に設定します。
...
[高可用性環境](/extensions/ad-ldap-connector/ad-ldap-high-availability)を構成しており、追加マシンでこのエラーが発生する場合、追加マシンの信頼されたルート認証機関が初期マシンの信頼されたルート認証機関と一致していることを確認してください。

## Auth0サポートにお問い合わせ

これらの問題を自分で解決できない場合は、[Auth0サポート](/support/open-and-manage-support-tickets)にお問い合わせください。

サポートチームが問題をトラブルシューティングできるように、サポートチケットに次の項目を含めてください。

1. 課題の説明。
2. [AD/LDAP構成ファイルのエクスポート](/extensions/ad-ldap-connector/import-export-ad-ldap-connector-configs)。
3. サービスログファイルのコピー:
