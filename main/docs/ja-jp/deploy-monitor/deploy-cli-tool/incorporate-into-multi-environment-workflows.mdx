...
## テナントと環境の対応 {#tenant-to-environment}
...
## リソース設定リポジトリ {#resource-configuration-repository}
...
## 環境固有の値 {#environment-specific-values}
...
## 環境ごとに設定ファイルを分ける {#separate-configuration-files}
...
### ファイル構成の例 {#example-file-structure}
...
## キーワード置換による動的な値 {#dynamic-values-with-keyword-replacement}
...
### config-dev.json の設定例 {#example-config-devjson}
...
### config-prod.json の設定例 {#example-config-prodjson}
