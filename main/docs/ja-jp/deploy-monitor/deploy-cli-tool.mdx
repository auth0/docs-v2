...
<Callout icon="📚">
完全な構成オプションと高度な使用方法については、[Deploy CLI構成リファレンス](https://github.com/auth0/auth0-deploy-cli/blob/master/docs/configuring-the-deploy-cli.md)を参照してください。
</Callout>
...
Deploy CLIが<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" href="/docs/ja-jp/glossary?term=management-api" tip="Management API: 顧客が管理タスクを実行できるようにするための製品。" cta="用語集の表示">Management API</Tooltip>を呼び出すためには、ツールの代理として呼び出しを行う、専用のAuth0アプリケーションが作成されなければなりません。
