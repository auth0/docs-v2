...
# Metric Streams (ベータ) {#metric-streams-beta}
...
## サポート対象の送信先 {#supported-destinations}
...
### メトリックストリームを作成 {#create-metric-stream}

<Tabs>
  <Tab title="Auth0 Dashboard">
    **メトリックストリーム に移動します。**

    1. [Auth0 Dashboard](https://manage.auth0.com) で、**Monitoring** &gt; **メトリックストリーム** に移動します。
    ...
    **ストリームを設定します。**
    ...
    これで メトリックストリーム が作成され、自動的に有効になります。
  </Tab>
...
## New Relicを設定する {#set-up-new-relic}

OpenTelemetry Protocol (OTLP) を使用して、Auth0のメトリクスをNew Relicに送信します。

### New Relicの資格情報を取得する {#get-new-relic-credentials}
...
### メトリックストリーム を作成 {#create-metric-stream-2}
...
### メトリックストリームを作成 {#create-metric-stream-3}
...
## メトリクスのカーディナリティを理解する {#understand-metric-cardinality}
...
### メトリクスが表示されない {#metrics-not-appearing}
...
### 監視コストが高い場合 {#high-monitoring-costs}
