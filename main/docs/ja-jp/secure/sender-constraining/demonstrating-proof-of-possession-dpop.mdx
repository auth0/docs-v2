...
## サポートされているアプリケーションのグラントタイプ {#supported-application-grant-types}
...
## Auth0 で DPoP を使用してトークンに送信者拘束を適用する {#sender-constrain-tokens-using-dpop-in-auth0}
...
## ステップ1: クライアントアプリケーションがDPoPキーペアを生成する {#step-1-client-application-generates-a-dpop-key-pair}
...
### JWTヘッダーのクレーム {#jwt-header-claims}
...
### JWT ペイロードクレーム {#jwt-payload-claims}
...
## ステップ 3: クライアントアプリケーションが DPoP-bound token をリクエストする {#step-3-client-application-requests-a-dpop-bound-token}
...
## ステップ 5: クライアントアプリケーションが DPoP-bound token と DPoP Proof JWT を使用して API を呼び出す {#step-5-client-application-calls-api-with-the-dpop-bound-token-and-dpop-proof-jwt}
...
## ステップ 6: DPoP を使用してトークンの更新を処理する {#step-6-handle-token-refresh-with-dpop}
