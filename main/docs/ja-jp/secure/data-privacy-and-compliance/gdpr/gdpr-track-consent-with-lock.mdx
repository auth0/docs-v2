...
## このフィールドには、Auth0が認証後にユーザーをリダイレクトできるURLのセットを入力します。サンプルアプリは`
...
## 分かりやすくするために、ここでは[同意フォームのサンプル]()を使用します。このフォームはホストする必要があります。また、フォームのURLは一般からのアクセスに公開されていなければなりません。手順2でフォームからAuth0にアクセス可能なURLを提供する必要があります。保護者の同意など、特別な同意プロンプトが必要な場合は、独自のカスタム同意フォームを作成する必要があります。国によって法律が異なるのでご注意ください。
...
2. Locate the Actions Code Editor, and select the **Secrets** (key) icon in its sidebar. Add the consent form URL as a Secret by creating a key/value pair:
   ...
   * 運用環境での使用に同時フォームへのリダイレクトをセットアップする際には、「[信頼済みのCallback URL]()」と「[データの整合性]()」でセキュリティ上の懸念事項を確認してください。
