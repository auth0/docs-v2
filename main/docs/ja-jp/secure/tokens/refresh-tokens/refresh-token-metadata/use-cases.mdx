...
## 永続的なカスタムクレームを作成する {#create-persistent-custom-claims}
...
## 一意のセッション ID を作成する {#create-a-unique-session-id}
...
## テナント識別子を作成する {#create-a-tenant-identifier}
...
## アップストリーム ID プロバイダー (IdP) からの一時データを管理する {#manage-transient-data-from-upstream-identity-providers-idps}
...
## セキュリティ強化と不正検知 {#enhance-security-and-fraud-detection}
...
## Management API を使用してメタデータにアクセスする {#access-metadata-with-the-management-api}
