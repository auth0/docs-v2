---
description: Actionで名前空間付きキーを使用してAuth0のアクセストークンとIDトークンにカスタムクレームを追加し、追加のユーザーまたはアプリケーションデータをAPIに渡します。
title: カスタムクレームを作成
---
...
### 制限対象外のクレーム {#non-restricted-claims}
...
## 名前空間付きガイドライン {#namespaced-guidelines}
...
## 非名前空間クレームのガイドライン {#non-namespaced-guidelines}
...
## カスタムクレームを作成する {#create-custom-claims}
