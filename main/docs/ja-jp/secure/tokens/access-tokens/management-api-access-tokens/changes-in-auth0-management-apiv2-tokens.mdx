...
新しい方法では、[クライアントの資格情報フロー](/login/flows/client-credentials-flow)が使用されます。新しいプロセスの使い方については、「[Management APIのアクセストークン](/security/tokens/access-tokens/management-api-access-tokens)」を参照してください。
...
新しい実装はこのようなリスクをもたらしません。構成し終わったら、Auth0 Dashboardにアクセスするか、`POST`要求を[Authentication APIの`/oauth/token`エンドポイント](/api/authentication#client-credentials)に対して行えばトークンを取得できます。
...
トークンの取得は、必ず「[Management APIのアクセストークン](/security/tokens/access-tokens/management-api-access-tokens)」に記載のプロセスだけに従ってください。