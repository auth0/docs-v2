...
## 利用可能な厳選ブロックリスト {#available-curated-blocklists}
...
## 厳選されたブロックリストを使用してルールを設定する {#configure-rules-with-curated-blocklists}

ルール定義内で [Network ACLs Management API](/docs/ja-jp/api/management/v2) の `auth0_managed` マッチャーを使用して、厳選されたブロックリストを設定します。

### 厳選されたブロックリストに基づくトラフィックのブロック {#block-traffic-from-curated-blocklists}
...
### 他のマッチャーと組み合わせる {#combine-with-other-matchers}
...
### 厳選ブロックリストのトラフィックを除外する {#exclude-curated-blocklist-traffic}
