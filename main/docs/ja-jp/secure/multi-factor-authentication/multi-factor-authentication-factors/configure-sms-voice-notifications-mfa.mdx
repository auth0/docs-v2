---
description: Twilio、Auth0 組み込みプロバイダー、またはカスタムプロバイダーを使用してワンタイムコードを配信する Auth0 MFA 認証要素として SMS と音声通知を設定します。
...
## Dashboard を使用する {#use-the-dashboard}
...
3. **[Branding &gt; Phone Provider](https://manage.auth0.com/#/phone/templates/phone/provider)** で設定する電話メッセージプロバイダーをテナントに適用するかどうかを選択します。適用する場合は、**Use Tenant-Level Messaging Provider** をオンにします。
4. 配信方法を選択します。
...
6. 電話メッセージプロバイダーを設定するには、**[Branding &gt; Phone Provider](https://manage.auth0.com/#/phone/templates/phone/provider)** に移動します。
   1. **Twilio**: SMS には [Twilio Programmable SMS API](https://www.twilio.com/sms)、音声には [Twilio Programmable Voice API](https://www.twilio.com/voice) を使用してメッセージを送信します。テスト用資格情報ではなく、Twilio の本番用資格情報を使用してください。詳細については、[Configure Twilio as a Phone Messaging Provider](/docs/ja-jp/customize/phone-messages/configure-phone-messaging-providers/configure-twilio-as-a-phone-messaging-provider) を参照してください。
   2. **Custom**: [Send Phone Message Flow](/docs/ja-jp/customize/actions/explore-triggers/mfa-notifications-trigger) の Action を呼び出してメッセージを送信します。
   * [Configure a Custom Phone Provider with Terraform](/docs/ja-jp/customize/phone-messages/configure-phone-messaging-providers/configure-a-custom-phone-provider/configure-a-custom-phone-provider-with-terraform)
   * [Configure a Custom Phone Provider with Twilio Verify](/docs/ja-jp/customize/phone-messages/configure-phone-messaging-providers/configure-a-custom-phone-provider/configure-a-custom-phone-provider-with-twilio-verify)

### Twilio の設定 {#twilio-configuration}
...
### SMS または音声メッセージのテンプレートをカスタマイズ {#customize-sms-or-voice-message-templates}
...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">Auth0 CLI を使用していますか？まだ設定していない場合は、このコマンドを実行する前に [CLI セッションをセットアップして認証してください](/docs/ja-jp/deploy-monitor/auth0-cli)。</Callout>
...
## 連携済みの SMS メッセージプロバイダー {#integrated-sms-messaging-providers}
...
## カスタム電話プロバイダー {#custom-phone-providers}
