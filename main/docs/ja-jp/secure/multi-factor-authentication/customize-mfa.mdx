...
## Actions で MFA ポリシーをカスタマイズする {#customize-mfa-policy-with-actions}
...
## 一般的なユースケースの例 {#common-use-case-examples}
...
### 特定のアプリケーションで MFA を必須にする {#require-mfa-for-specific-applications}
...
### 特定のユーザーに MFA を必須にする {#require-mfa-for-specific-users}
...
### MFAプロンプトの頻度を変更する {#change-frequency-of-mfa-prompts}
...
### ネットワーク外のユーザーに MFA を必須にする {#require-mfa-for-users-outside-of-the-network}
...
### リフレッシュトークンのリクエストではMFAを回避する {#bypass-mfa-for-refresh-token-requests}
