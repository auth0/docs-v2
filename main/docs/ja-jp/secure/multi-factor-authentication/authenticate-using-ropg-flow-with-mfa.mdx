...
[Auth0 MFA API](https://auth0.com/docs/api/authentication#multi-factor-authentication) を使用すると、<Tooltip tip="Resource Owner: 保護されたリソースへのアクセスを許可できるエンティティ（ユーザーやアプリケーションなど）。" cta="用語集を見る" href="/docs/ja-jp/glossary?term=Resource+Owner">Resource Owner</Tooltip> Password Grant または ROPG とも呼ばれる [Resource Owner Password Flow](/docs/ja-jp/get-started/authentication-and-authorization-flow/resource-owner-password-flow) を使って、<Tooltip tip="Resource Owner: 保護されたリソースへのアクセスを許可できるエンティティ（ユーザーやアプリケーションなど）。" cta="用語集を見る" href="/docs/ja-jp/glossary?term=MFA">MFA</Tooltip> が有効な場合に認証フローを完了できます。
...
上記のエラーが返されたら、ユーザーに MFA 認証要素が登録されているかどうかを確認する必要があります。前のセクションで取得した MFA トークンを使用して、[MFA 認証器エンドポイント](/docs/ja-jp/secure/multi-factor-authentication/manage-mfa-auth0-apis/manage-authenticator-factors-mfa-api) を呼び出します。
...
## MFA認証要素を登録する {#enroll-mfa-factor}
...
* [Push](/docs/ja-jp/secure/multi-factor-authentication/authenticate-using-ropg-flow-with-mfa/enroll-and-challenge-push-authenticators)
...
## MFA でユーザーにチャレンジする {#challenge-user-with-mfa}
...
### MFA OTPコードの制限事項 {#mfa-otp-code-limitations-and-restrictions}
...
`mfa_token` を使用して [`mfa/authenticator エンドポイント`](/docs/ja-jp/api/authentication/muti-factor-authentication/list-authenticators) を呼び出し、ユーザーが登録済みの認証要素をすべて一覧表示して、アプリケーションがサポートしている同じ種類のものを特定します。  また、チャレンジを発行するには、対応する `authenticator_type` も取得する必要があります:
...
[`request/mfa/challenge`](/docs/ja-jp/api/authentication/muti-factor-authentication/request-mfa-challenge) エンドポイントを呼び出して、MFA チャレンジを必須にします。
