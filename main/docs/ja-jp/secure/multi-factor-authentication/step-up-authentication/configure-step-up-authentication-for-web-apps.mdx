...
## MFA 向けに ID トークンを検証する {#validate-id-tokens-for-mfa}
...
#### AMRクレームの例外 {#amr-claim-exceptions}
...
### 例: MFAありの場合の値 {#example-values-with-mfa}
...
### 例: MFAなしの値 {#example-values-without-mfa}
...
## シナリオ: プッシュ通知による給与データへのアクセス {#scenario-salary-data-with-push-notifications}
...
ウェブアプリから要求されたときに、ユーザーに MFA での認証を求める Action を作成します。[Dashboard &gt; Actions &gt; Flows](https://manage.auth0.com/#/actions/flows) に移動し、次の内容を含む Action を作成します。
...
### ID トークンを検証する {#validate-id-token}
