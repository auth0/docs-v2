...
## Guardian iOS SDK のインストール {#install-guardian-ios-sdk}
...
## Guardianのプッシュ通知を有効にする {#enable-guardian-push-notifications}
...
#### 署名鍵と検証用キー {#signing-and-verification-keys}
...
### ログインリクエストを許可する {#allow-login-requests}
...
### ログインリクエストを拒否する {#reject-login-requests}
...
### モバイル専用のOTP登録を設定する {#set-up-mobile-only-otp-enrollment}
...
#### リソースを接続する {#connect-a-resource}

Auth0 Dashboard または Guardian SDK を使って、リソースを接続できます。

##### Auth0 Dashboard を使用する {#use-auth0-dashboard}
...
#### ワンタイムコードを入力 {#enter-one-time-code}
...
#### アプリでログインする {#log-in-with-your-app}
