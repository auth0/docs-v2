---
description: MFA ページのテーマプロパティをカスタマイズするための、MFA ホストページの設定オプションについて説明します。
title: MFA テーマの Language Dictionary
---

## デフォルトの場所 {#defaultlocation}
...
`languageDictionary` を使用すると、Classic Login エクスペリエンスでの<Tooltip tip="多要素認証（MFA）: SMSで送信されるコードなど、ユーザー名とパスワードに加えて別の要素を使用するユーザー認証プロセス。" cta="用語集を見る" href="/docs/ja-jp/glossary?term=MFA">MFA</Tooltip>プロセス内のさまざまな箇所のテキストを上書きできます。次の形式で指定します。
...
以下は、`smsEnrollmentConfirm` の `headerText` 属性を変更する例です。
...
...
  ...
  languageDictionary:{ // 言語辞書オプションを使用する
    smsEnrollmentConfirm:{  // 変更するカテゴリ/セクションを指定する
      headerText: "Some custom text - In order to confirm enrollment we need to confirm your phone. Please enter the received code." // 変更したい特定の属性に新しい値を設定する
    }
  }
...
利用可能なカテゴリとオプションの一覧は、以下をご覧ください。

### languageDictionary のオプション {#languagedictionary-options}
...
smsAuth: {
  codePlaceholder: 'Enter the code',
  headerText: 'Enter the code we\'ve just sent to your phone.',
  useRecoveryCode: 'Lost your device? <recovery>Use the recovery code</recovery>'
},
...
smsEnrollmentConfirm: {
  codePlaceholder: 'Enter the received code',
...
smsEnrollmentAddPhoneNumber: {
  ...
  // countryCodes: { 'US': '米国の翻訳', ... '<country code>': '<translation>' }
},
...
errorMessages: {
  ...
  invalidOtpFormat: 'OTP Code must have at least 4 numeric characters',
...
* [Classic Login の MFA のカスタマイズ](/docs/ja-jp/secure/multi-factor-authentication/customize-mfa/customize-mfa-classic-login)