...
### 順序付きフローとコンテキストベースのフロー {#sequenced-and-contextual-flows}
...
### post-login Action をテストする {#test-your-post-login-action}
...
## ユースケースの例 {#example-use-cases}

以下では、MFAフローをカスタマイズする際によくあるユースケースを紹介します。

### 現在の登録状況に基づいてチャレンジ方法を決定する {#use-current-enrollments-to-determine-challenge-method}
...
### ロールを使用してチャレンジ方法を判断する {#use-roles-to-determine-challenge-method}
...
### メタデータを使用してチャレンジ方法を決定する {#use-metadata-to-determine-challenge-method}
...
### ユーザーが別の認証方法を選択できるようにする {#allow-users-to-select-an-alternate-method-of-authentication}
...
### Adaptive MFA を使用してユーザーに追加認証を求めるタイミングを判断する {#use-adaptive-mfa-to-determine-when-to-challenge-users}
...
### Actions を使用してユーザーに追加認証を求める {#use-actions-to-challenge-users}
...
テナントログは、Auth0 Dashboard の [Monitoring &gt; Logs](https://manage.auth0.com/#/logs) で確認できます。また、[Management API](/docs/ja-jp/api/management/v2) を使用してログを取得することもできます。
...
3. ユーザーは、Actions で参照している認証要素に登録済みである必要があります。
   ...
   * 特定のユーザーでエラーが発生している場合は、そのユーザーの詳細を確認し、適切な認証要素に登録されていることを確認してください。[**Auth0 Dashboard &gt; User Management &gt; Users**](https://manage.auth0.com/#/users) に移動し、一覧から該当するユーザー名を選択します。
     ...
     * または、**Raw JSON tab** でユーザーの登録状況を確認することもできます。この情報は [Management API](/docs/ja-jp/api/management/v2/users/get-authentication-methods) から取得することも可能です。ただし、メール確認リンクで設定された Email 認証要素のような自動登録された認証器は、API には一覧表示されない点に注意してください。
   * ユーザーが適切な認証要素に登録されていない場合は、Management API の [authentication-methods](/docs/ja-jp/api/management/v2/users/post-authentication-methods) エンドポイントを使用して登録を作成できます。また、Auth0 Dashboard のユーザーのプロファイルページから直接、ユーザーの登録を管理することもできます。
