...
### Authenticatorをリストおよび削除する必要がある場合は、`)することをユーザーに求めます。ユーザーは、MFAを完了しないと認証要素をリスト・削除できません。
...
復旧コードを削除して新たに生成するには、Auth0の[Management APIのアクセストークン]()を取得し、Management APIの[復旧コード再生成エンドポイント]()を使用します。
