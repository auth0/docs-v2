...
## 攻撃対策の詳細 {#attack-protection-details}

Auth0 は、ルールの実行情報に加えて、Adaptive MFA の <Tooltip tip="攻撃対策: Auth0 が攻撃を検出して緩和するために提供する機能で、総当たり攻撃対策、不審な IP スロットリング、漏えいパスワードの検知、ボット検出、Adaptive MFA が含まれます。" cta="用語集を表示" href="/docs/ja-jp/glossary?term=attack+protection">攻撃対策</Tooltip> 情報もテナントログに含めます。ルールに表示されるものと同じデータ構造が、テナントログにも表示されます:
...
## MFAプロンプトの詳細 {#mfa-prompt-details}
...
## MFAプロバイダーの詳細 {#mfa-provider-details}
