...
description: Actions を使用して Auth0 Adaptive MFA をカスタマイズし、リスク評価の信頼度と詳細に基づいてログイン試行に対してチャレンジを要求するか、許可するか、ブロックするかを決定します。
...
## Adaptive MFA をカスタマイズするタイミング {#when-to-customize-adaptive-mfa}
...
## Action の結果パターン {#action-result-outcomes}
...
## Action テンプレート {#action-templates}
...
### Adaptive MFA テンプレート {#adaptive-mfa-template}
...
### MFA登録を必須にするテンプレート {#require-mfa-enrollment-template}
...
## Action のユースケース {#action-use-cases}
