...
Auth0 には、[Universal Login](/docs/ja-jp/authenticate/login/auth0-universal-login) を使用した、組み込みの <Tooltip tip="多要素認証（MFA）: SMS によるコードなど、ユーザー名とパスワードに加えて認証要素を使用するユーザー認証プロセス。" cta="用語集を見る" href="/docs/ja-jp/glossary?term=MFA">MFA</Tooltip> の登録および認証フローが用意されています。ですが、独自のユーザーインターフェースを作成したい場合は、[MFA API](/docs/ja-jp/secure/multi-factor-authentication/multi-factor-authentication-developer-resources/mfa-api) を使って実現することもできます。
...
* Auth0 Dashboard または [Management API](https://auth0.com/docs/api/management/v2#!/Guardian/put_factors_by_name) を使用して、[電話を認証要素として設定する](/docs/ja-jp/secure/multi-factor-authentication/multi-factor-authentication-factors/configure-sms-voice-notifications-mfa)。

## SMS または音声で登録する {#enroll-with-sms-or-voice}

### MFAトークンを取得する {#get-mfa-token}

登録を開始するタイミングに応じて、MFA API の利用に必要な <Tooltip tip="アクセストークン: API へのアクセスに使用する、opaque な文字列または JWT 形式の認可資格情報。" cta="用語集を見る" href="/docs/ja-jp/glossary?term=access+token">アクセストークン</Tooltip> は、いくつかの方法で取得できます。

* 認証時に登録する場合は、[Authenticate With Resource Owner Password Grant and MFA](/docs/ja-jp/secure/multi-factor-authentication/authenticate-using-ropg-flow-with-mfa) を参照してください。
* ユーザーが任意のタイミングで認証要素を登録できるようにする場合は、[Manage MFA Factor Enrollments](/docs/ja-jp/secure/multi-factor-authentication/manage-mfa-auth0-apis/manage-authenticator-factors-mfa-api) を参照してください。

### 認証器を登録する {#enroll-authenticator}

ユーザーの認証器を登録するには、MFA Associate エンドポイント に `POST` リクエストを送信します。この エンドポイント で必要なベアラートークンは、前の手順で取得した MFA トークンです。

SMS or 音声 で登録するには、SMS または 音声 のいずれかでチャレンジできる電話番号を登録します。エンドポイント を呼び出すには、以下のパラメーターを指定します。`oob_channels` パラメーターは、ユーザーに ワンタイムコード を送信する方法 (SMS または 音声) を示します。
...
### SMS または音声による登録を確認する {#confirm-sms-or-voice-enrollment}

ユーザーには、アプリケーションに入力する必要があるワンタイムコードを含むメッセージが届きます。

登録を完了するには、<Tooltip tip="Token Endpoint: 認可サーバー上でプログラムからトークンを要求するために使用されるエンドポイント。" cta="用語集を見る" href="/docs/ja-jp/glossary?term=OAuth">OAuth</Tooltip> <Tooltip tip="OAuth 2.0: 認可プロトコルとワークフローを定義する認可フレームワーク。" cta="用語集を見る" href="/docs/ja-jp/glossary?term=Token+endpoint">トークンエンドポイント</Tooltip> に `POST` リクエストを送信します。前のレスポンスで返された `oob_code` と、メッセージで受け取った値を含む `binding_code` を指定する必要があります。
...
## SMS または音声による認証チャレンジ {#challenge-with-sms-or-voice}

### MFAトークンを取得する {#get-mfa-token-2}

[MFA を使用して Resource Owner Password グラントで認証する](/docs/ja-jp/secure/multi-factor-authentication/authenticate-using-ropg-flow-with-mfa) の手順に従って、MFAトークンを取得します。

### 登録済みの認証器を取得する {#retrieve-enrolled-authenticators}

ユーザーにチャレンジを送信するには、チャレンジする認証要素の `authenticator_id` が必要です。MFA Authenticators エンドポイント を使用して、登録済みの認証器をすべて一覧表示できます。
...
### ワンタイムコードを使ってユーザーにチャレンジする {#challenge-user-with-a-one-time-code}
...
### 受信したコードで認証を完了する {#complete-authentication-using-received-code}
...
アプリケーションでは、メッセージで送信されたコードをユーザーに入力してもらい、その値を `binding_code` パラメーターに設定する必要があります。前の呼び出しで返された `binding_code` と `oob_code` を指定して OAuth0 トークンエンドポイントを使用すると、コードを検証し、認証トークンを取得できます。
...
`mfa_token` を使って [`mfa/authenticator`](/docs/ja-jp/api/authentication/muti-factor-authentication/list-authenticators) エンドポイントを呼び出し、ユーザーが登録済みのすべての認証要素の一覧を取得したうえで、アプリケーションがサポートしている種類と一致するものを特定します。さらに、チャレンジを発行するには、対応する `authenticator_type` も取得する必要があります:
...
[`request/mfa/challenge`](/docs/ja-jp/api/authentication/muti-factor-authentication/request-mfa-challenge) エンドポイントを呼び出して、MFAチャレンジを必須にします。

Auth0 Actions を使用すると、MFAフローをさらに柔軟にカスタマイズできます。詳しくは、[Actions トリガー: post-challenge - API Object](/docs/ja-jp/customize/actions/explore-triggers/password-reset-triggers/post-challenge-trigger/post-challenge-api-object) をご覧ください。
...
* [リカバリーコードによるチャレンジ](/docs/ja-jp/secure/multi-factor-authentication/authenticate-using-ropg-flow-with-mfa/challenge-with-recovery-codes)
...
* [Push 認証器の登録とチャレンジ](/docs/ja-jp/secure/multi-factor-authentication/authenticate-using-ropg-flow-with-mfa/enroll-and-challenge-push-authenticators)