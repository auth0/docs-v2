...
お使いのテナントで[リカバリーコードが有効](/docs/ja-jp/secure/multi-factor-authentication/configure-recovery-codes-for-mfa)になっている場合、Auth0 は、ユーザーが <Tooltip tip="多要素認証（MFA）: SMS によるコードなど、ユーザー名とパスワードに加えて認証要素を使用するユーザー認証プロセス。" cta="用語集を見る" href="/docs/ja-jp/glossary?term=multi-factor+authentication">多要素認証</Tooltip> (MFA) に登録すると、自動的にリカバリーコードを生成します。ユーザーはこのリカバリーコードを保存しておく必要があります。このコードは、MFA に使用しているデバイスやアカウントにアクセスできなくなった場合に、後から使用できます。
