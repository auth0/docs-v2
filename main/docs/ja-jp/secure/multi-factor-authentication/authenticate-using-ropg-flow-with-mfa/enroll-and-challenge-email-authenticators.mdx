...
Auth0 には、[Universal Login](/docs/ja-jp/authenticate/login/auth0-universal-login) を使用した組み込みの <Tooltip tip="多要素認証 (MFA): SMS で送信されるコードなど、ユーザー名とパスワードに加えて認証要素を使用するユーザー認証プロセス。" cta="用語集を見る" href="/docs/ja-jp/glossary?term=MFA">MFA</Tooltip> の登録および認証フローが用意されています。ただし、独自のユーザーインターフェイスを作成する場合は、[MFA API](/docs/ja-jp/secure/multi-factor-authentication/multi-factor-authentication-developer-resources/mfa-api) を使用して実現できます。
...
* [Email](/docs/ja-jp/secure/multi-factor-authentication/multi-factor-authentication-factors/configure-email-notifications-for-mfa) を認証要素として、Auth0 Dashboard または [Management API](https://auth0.com/docs/api/management/v2#!/Guardian/put_factors_by_name) で設定します。

## メールを登録する {#enroll-with-email}

ユーザーがプライマリーアイデンティティの確認済みメールアドレスに加えて、別のメールアドレスも登録できるようにするには、次の手順を行います。

### MFAトークンを取得する {#get-mfa-token}

登録を開始するタイミングに応じて、MFA API の使用に必要な<Tooltip tip="Access Token: API へのアクセスに使用される、不透明な文字列または JWT 形式の認可資格情報。" cta="用語集を表示" href="/docs/ja-jp/glossary?term=access+token">access token</Tooltip>は、さまざまな方法で取得できます。
...
### メール登録を確認する {#confirm-email-enrollment}

ユーザーには、アプリケーションに入力するワンタイムコードが記載されたメールが届きます。

<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  デフォルトのOTPの長さは6桁です。メールOTPコードの長さと有効期限は、Auth0 DashboardまたはManagement APIを使用して設定できます。詳細については、[OTP設定を構成する](/docs/ja-jp/secure/multi-factor-authentication/multi-factor-authentication-factors/configure-email-notifications-for-mfa#configure-otp-settings)を参照してください。
</Callout>
...
必要に応じて、ユーザーが受信するメールをカスタマイズできます。詳しくは、[メールテンプレートをカスタマイズする](/docs/ja-jp/customize/email/email-templates) を参照してください。

## メールによる認証チャレンジ {#challenge-with-email}

### MFAトークンを取得 {#get-mfa-token-2}

[Resource Owner Password グラントと MFA を使用して認証する](/docs/ja-jp/secure/multi-factor-authentication/authenticate-using-ropg-flow-with-mfa) で説明されている手順に従って、MFAトークンを取得します。
...
### OTP でユーザーに認証を求める {#challenge-user-with-otp}
...
`mfa_token` を使用して [`mfa/authenticator`](/docs/ja-jp/api/authentication/muti-factor-authentication/list-authenticators) endpoint を呼び出し、ユーザーが登録済みのすべての認証要素の一覧を取得したうえで、アプリケーションがサポートする同じ種類のものを特定します。チャレンジを発行するには、対応する `authenticator_type` も取得する必要があります:
...
[`request/mfa/challenge`](/docs/ja-jp/api/authentication/muti-factor-authentication/request-mfa-challenge) endpointを呼び出して、MFAチャレンジを必須にします。

Auth0 Actionsを使用すると、MFAフローをさらにカスタマイズできます。詳細については、[Actions トリガー: post-challenge - API オブジェクト](/docs/ja-jp/customize/actions/explore-triggers/password-reset-triggers/post-challenge-trigger/post-challenge-api-object)をご覧ください。
...
* [SMS および音声認証器の登録とチャレンジ](/docs/ja-jp/secure/multi-factor-authentication/authenticate-using-ropg-flow-with-mfa/enroll-challenge-sms-voice-authenticators)