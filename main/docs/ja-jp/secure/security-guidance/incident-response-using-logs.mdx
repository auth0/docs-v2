---
description: インシデント対応計画の一環として Auth0 テナントログを使用し、不審なログインの調査、侵害の範囲の特定、監査タイムラインの作成を行う方法を学びます。
...
## Auth0 のログを確認する {#check-auth0-logs}
...
### ログイン失敗のログイベントの例 {#example-of-failed-login-log-event}
...
## 侵害された可能性のあるユーザーアカウントを特定する {#identify-compromised-user-accounts}
...
## 侵害されたユーザーアカウントのアクティビティを確認する {#check-compromised-user-account-activity}
...
## Auth0 Dashboard でユーザーを削除またはブロックする {#delete-or-block-users-from-the-dashboard}
