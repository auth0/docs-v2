...
#### ご利用のAuth0サブスクリプションで自己管理カスタムドメインの設定が可能な場合は、カスタムドメインに静的IPアドレスを設定できます。自己管理カスタムドメインでは、ネットワークのエントリーポイントを制御し、IPアドレスを固定させることができます。サブスクリプションのプランに関する情報は、[Auth0の料金表]()をご覧ください。
...
* [Auth0 パブリッククラウド Service Endpoints](/docs/troubleshoot/customer-support/operational-policies/public-cloud-service-endpoints)
