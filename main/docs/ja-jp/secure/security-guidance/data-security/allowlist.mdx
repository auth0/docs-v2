...
* [Actions](/docs/ja-jp/customize/actions)
* [ルール](/docs/ja-jp/customize/rules)
* [フック](/docs/ja-jp/customize/hooks)
* [カスタムデータベースアクションスクリプト](/docs/ja-jp/authenticate/database-connections/custom-db/templates)
* [Log Streams](/docs/ja-jp/customize/log-streams)
* [外部 SMTP メールプロバイダー](/docs/ja-jp/customize/email/smtp-email-providers)
...
#### 機械可読なリスト {#machine-readable-list}
...
| `regions`         | object              | [Auth0 テナントリージョン](/docs/ja-jp/get-started/auth0-overview/create-tenants#region-locality-and-sub-locality)のコレクション。各テナントリージョンオブジェクトには、次のプロパティが含まれます。 * `ipv4_cidrs`: 文字列の配列。指定したリージョンの IPv4 CIDR 範囲の一覧。                                            |
...
#### 条件付き HTTP リクエストの例 (If-None-Match ヘッダー) {#example-of-conditional-http-request-if-none-match-header}
...
#### 条件付き HTTP リクエストの例 (If-Modified-Since ヘッダー) {#example-of-conditional-http-request-if-modified-since-header}
...
### リージョンごとの送信元 IP アドレス {#outbound-ip-addresses-by-region}
