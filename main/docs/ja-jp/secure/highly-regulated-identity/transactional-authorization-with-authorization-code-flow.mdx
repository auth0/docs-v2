...
### トランザクションの詳細を伝え、Auth0 にリダイレクトする {#communicate-transaction-details-and-redirect-to-auth0}
...
### 動的ポリシーを適用する {#apply-dynamic-policy}
...
### ユーザーにトランザクションの詳細の承認を求める {#challenge-the-user-to-get-transaction-details-approval}
...
さらに、SMS、Email、WebAuthn では、Auth0 がユーザーに表示する同意画面をカスタマイズして、authorization&#95;details やその他のトランザクションの詳細から表示したい情報を示すことができます。詳しくは、[Rich Authorization Requests を設定する](/docs/ja-jp/get-started/apis/configure-rich-authorization-requests)を参照してください。プッシュ通知にはこれは該当しません。トランザクションの詳細をエンドユーザーに表示するのはモバイルアプリケーションであるためです。
...
#### SMS、Email、または WebAuthn {#sms-email-or-webauthn}
