---
description: Bring Your Own Key (BYOK) を使用して Auth0 で独自の暗号化キーを管理し、テナントのデータを保護するキー階層を制御します。
sidebarTitle: 概要
...
* **自身のキーを管理する:** [Key Management Editor ロール](/docs/ja-jp/get-started/manage-dashboard-access/add-dashboard-users) を持つユーザーは、Auth0 KMS で Tenant Master Key のライフサイクルをカスタマイズできます。
* **Bring Your Own Key:** [Key Management Editor ロール](/docs/ja-jp/get-started/manage-dashboard-access/add-dashboard-users) を持つユーザーは、Auth0 Environment Root Key を置き換え、対応する Auth0 Cloud Hardware Security Module (HSM) に独自のラップ済み暗号化キーをインポートできます。

Auth0 Keys の詳細については、[Auth0 のキー階層](/docs/ja-jp/secure/highly-regulated-identity/customer-managed-keys#auth0-key-hierarchy) をご覧ください。

<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  Customer Managed Keys 機能は、Key Management Editor ロール を持つユーザーが利用できます。このロールはデフォルトではユーザーに付与されないため、テナントメンバーに明示的に付与する必要があります。詳細については、[Add Tenant Members](/docs/ja-jp/get-started/manage-dashboard-access/add-dashboard-users) をご覧ください。
</Callout>

## 独自のキーを管理する {#control-your-own-key}

Auth0 <Tooltip tip="Management API: 顧客が管理タスクを実行できるようにする製品。" cta="用語集を見る" href="/docs/ja-jp/glossary?term=Management+API">Management API</Tooltip> の [Rekey](https://auth0.com/docs/api/management/v2/keys/post-encryption-rekey) エンドポイントを使用すると、Tenant Master Key のライフサイクルを管理して、次のことができます。
...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  このエンドポイントを利用できるのは、Auth0 で特定の認可スコープが付与されたテナントメンバーのみです。デフォルトでは、これらのスコープは [Key Management Editor ロール](/docs/ja-jp/get-started/manage-dashboard-access/add-dashboard-users) を持つユーザーに付与されます。
</Callout>
...
Bring Your Own Key を有効にするには、[Auth0 Dashboard](/docs/ja-jp/secure/highly-regulated-identity/customer-managed-keys/customer-managed-keys-dashboard) または [Management API](/docs/ja-jp/secure/highly-regulated-identity/customer-managed-keys/customer-managed-keys-management-api) を使用できます。

## Customer Managed Keys のログイベントを監視する {#monitor-customer-managed-keys-log-events}
...
Customer Managed Keys の操作では、[テナントログ](/docs/ja-jp/deploy-monitor/logs) に次のログイベントが追加されます。
...
## Auth0 のキー階層 {#auth0-key-hierarchy}
...
### Environment Root Key {#environment-root-key}
...
### Tenant Master Keys {#tenant-master-keys}
