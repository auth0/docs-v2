...
## ステップ 1: 接続してアクセスを認可する {#step-1-connect-and-authorize-access}
...
## ステップ 2: SPA が Auth0 アクセストークンを使用してバックエンド API を呼び出す {#step-2-spa-calls-backend-api-with-auth0-access-token}
...
## ステップ 3: バックエンド API がアクセストークン交換を実行する {#step-3-backend-api-performs-access-token-exchange}
...
| `scope`                | <Badge color="yellow">早期アクセス</Badge> (任意) 外部プロバイダーのアクセストークンに要求するスコープをスペース区切りで指定します。これを使用すると、Connected Accounts フローで最初に付与されたスコープの一部を要求できます。指定しない場合、トークンには最初に付与されたすべてのスコープが含まれます。     |
...
## ステップ 4: Auth0 Authorization Server がアクセストークンを検証する {#step-4-auth0-authorization-server-validates-access-token}
