...
## クライアントアプリケーションを設定する {#configure-client-application}
...
## 署名付きJWTサブジェクトトークンを作成する {#create-signed-jwt-subject-token}
...
## 外部APIのアクセストークンをリクエストする {#request-token-for-external-api}

署名付き JWTを取得したら、外部APIのアクセストークンをリクエストできます。
...
| `scope`                | <Badge color="yellow">早期アクセス</Badge> (任意) 外部プロバイダーのアクセストークンにリクエストするスコープをスペースで区切ったリスト。リクエストしたスコープは、クライアントでこの接続に対して `token_vault_privileged_access.grants` に設定されているスコープとの共通部分になります。指定しない場合、この接続のグラントに設定されているすべてのスコープを含むトークンが発行されます。 |