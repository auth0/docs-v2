...
## ステップ 1: 接続してアクセスを承認する {#step-1-connect-and-authorize-access}
...
## ステップ2: リフレッシュトークン交換を実行する {#step-2-perform-refresh-token-exchange}
...
アプリケーションは、有効なAuth0リフレッシュトークンを使用して、Connected Accountsフローで付与されたスコープを持つGoogleのアクセストークンをToken Vaultにリクエストできます。このプロセスにより、ユーザーに接続の再認可を求めることなく、アプリケーションは新しいアクセストークンを取得できます。
...
| `requested_token_type` | 要求対象のトークンの種類。Token Vault の場合は、外部プロバイダーのアクセストークン、または `http://auth0.com/oauth/token-type/federated-connection-access-token` に設定します                                                             |
| `connection`           | 接続名。この場合は `google-oauth2` です。                                                                                                                                                               |
| `scope`                | <Badge color="yellow">早期アクセス</Badge> (省略可能) 外部プロバイダーのアクセストークンに要求するスコープを、スペース区切りのリストで指定します。これを使用して、Connected Accountsフローで最初に付与されたスコープのサブセットを要求します。指定しない場合、トークンには最初に付与されたすべてのスコープが含まれて発行されます。 |
...
## ステップ 3: Auth0 Authorization Server がリフレッシュトークンを検証 {#step-3-auth0-authorization-server-validates-refresh-token}
...
## 外部プロバイダーのリフレッシュトークンの有効期限ポリシー {#external-provider-refresh-token-expiration-policy}
