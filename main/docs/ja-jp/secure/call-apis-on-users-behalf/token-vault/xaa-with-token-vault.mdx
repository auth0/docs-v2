...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  **Requesting App側からXAAフロー全体を構築してテストする場合**：まず[環境の設定](/docs/ja-jp/ai-agents-mcp/cross-app-access/requesting-app/set-up-xaa-test-environment)と[OktaをOIDC IdPとして使用する](/docs/ja-jp/ai-agents-mcp/cross-app-access/requesting-app/idp/okta-as-oidc-idp)を完了してから、このページに戻って[Token Vaultアプリケーションを構成](#configure-your-token-vault-application)してください。

  **すでにToken Vaultを設定済みで、XAAのサポートを追加する場合**：[既存のToken Vault連携にXAAを追加する](#add-xaa-to-an-existing-token-vault-integration)に進んでください。
</Callout>

## XAA では何が異なるのか？ {#whats-different-with-xaa}
...
## 既存の Token Vault 連携に XAA を追加する {#add-xaa-to-an-existing-token-vault-integration}
...
### Requesting Appの接続を構成する {#configure-the-requesting-app-connection}
...
### Resource App の接続を構成する {#configure-the-resource-app-connection}
...
両方の接続を構成したら、[Token Vault アプリケーションを構成する](#configure-your-token-vault-application)と[エンドツーエンドのフローをテストする](#test-the-end-to-end-flow)に進んでください。

## Token Vault アプリケーションを構成する {#configure-your-token-vault-application}
...
<Tabs>
  <Tab title="Auth0 Dashboard">
    ...
    2. **Advanced Settings &gt; Grant Types** で、**認可コード**、**リフレッシュトークン**、**Token Vault** を有効にします。
    3. **変更を保存** を選択します。
  </Tab>
...
### アプリケーションでOkta接続を有効化する {#enable-okta-connections-for-the-application}
...
すでにToken Vaultを設定済みで、XAAのサポートを追加する場合：作成したアプリケーションに対して、Requesting App接続とResource App接続を有効化する必要があります。詳細については、[既存のToken Vault連携にXAAを追加する](#add-xaa-to-an-existing-token-vault-integration)をお読みください。

<Tabs>
  <Tab title="Auth0 Dashboard">
    Okta Workforce接続またはRequesting App接続を有効化するには：

    1. **[Authentication] &gt; [Enterprise] &gt; [Okta Workforce]** に移動し、Okta Workforce接続を選択して、**[Applications]** タブを選択します。次に、作成したアプリケーションで有効化します。

    OIDC接続またはResource App接続を有効化するには：

    1. **[Authentication] &gt; [Enterprise] &gt; [OpenID Connect (OIDC)]** に移動し、OIDC接続を選択して、**[Applications]** タブを選択します。次に、作成したアプリケーションで有効化します。
  </Tab>
...
## エンドツーエンドのフローをテストする {#test-the-end-to-end-flow}
...
[Token Vaultアプリケーションを構成する](#configure-your-token-vault-application)で構成したアプリケーション、またはToken Vaultグラントを有効にした既存のアプリケーションを使用してください。

### ステップ1：Auth0のリフレッシュトークンを取得する {#step-1-obtain-an-auth0-refresh-token}
...
#### 認可リクエストを開始する {#initiate-the-authorization-request}
...
#### 認可コードをリフレッシュトークンと交換する {#exchange-the-authorization-code-for-a-refresh-token}
...
### ステップ 2: Token Vault でリフレッシュトークンを交換する {#step-2-exchange-the-refresh-token-with-token-vault}
...
## 複数の Requesting App IdP に対応する {#handle-multiple-requesting-app-idps}
...
## Auth0 Organizations を XAA で使用する {#use-auth0-organizations-with-xaa}
