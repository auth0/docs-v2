---
description: Token Vault を設定して、リフレッシュトークン交換およびアクセストークン交換フローを含め、ユーザーに代わってサードパーティ API トークンを安全に保存および交換します。
...
## Connected Accounts for Token Vault を設定する {#configure-connected-accounts-for-token-vault}
...
## アプリケーションを設定 {#configure-application}
...
## トークン交換を設定する {#configure-token-exchange}
...
### リフレッシュトークン交換を設定する {#configure-refresh-token-exchange}
...
### アクセストークン交換を設定する {#configure-access-token-exchange}
...
#### バックエンド API を作成する {#create-backend-api}
...
#### カスタム API クライアント を作成する {#create-custom-api-client}
