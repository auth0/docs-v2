...
OBO token exchange では、[`post-login` Action トリガー](/docs/ja-jp/customize/actions/explore-triggers/signup-and-login-triggers/login-trigger)が実行されます。ここで:

* [`event.transaction.protocol`](/docs/ja-jp/customize/actions/explore-triggers/signup-and-login-triggers/login-trigger/post-login-event-object#param-protocol) は `oauth2-token-exchange` に設定されます。
* [`event.transaction.actor`](/docs/ja-jp/customize/actions/explore-triggers/signup-and-login-triggers/login-trigger/post-login-event-object#param-actor) は完全な委譲チェーンを追跡します。
...
OBO token exchangeを使用できるのは、リソースサーバーに関連付けられた カスタム API クライアントのみです。カスタム API クライアントは、リソースサーバーと同じ identifier を共有している場合、そのリソースサーバーにリンクされます。

カスタム API クライアントには、次の要件があります。

* `app_type` を `resource_server` に設定します。
* `resource_server_identifier` には、ダウンストリーム API ではなく、受信トークンの オーディエンス (例: `https://incoming-token-api.example.com`) を設定します。Auth0 は、OBO token exchangeを実行する前に、リソースサーバー identifier を使用して受信トークンを検証します。

カスタム API クライアントはファーストパーティクライアントであるため、ファーストパーティクライアントがアクセスする必要がある API では、必ず[ユーザーの同意をスキップ](/docs/ja-jp/get-started/applications/confidential-and-public-applications/user-consent-and-third-party-applications#skip-consent-for-first-party-applications)してください。
...
* [`post-login` Actions トリガー](/docs/ja-jp/customize/actions/explore-triggers/signup-and-login-triggers/login-trigger/post-login-event-object#event-organization)で、`event.organization` プロパティを通じて organization のコンテキストを利用できるようにする