...
<Callout color="#0EA5E9" icon="file-lines" icontype="regular">
...
Sender constraining is an <Tooltip cta="View Glossary" href="/docs/glossary?term=OAuth+2.0" tip="OAuth 2.0: Authorization framework that defines authorization protocols and workflows.">OAuth 2.0</Tooltip> and <Tooltip cta="View Glossary" href="/docs/glossary?term=OpenID" tip="OAuth 2.0: Authorization framework that defines authorization protocols and workflows.">OpenID</Tooltip> Connect (OIDC) security mechanism that cryptographically binds access and <Tooltip cta="View Glossary" href="/docs/glossary?term=refresh+tokens" tip="Refresh Token: Token used to obtain a renewed Access Token without forcing users to log in again.">refresh tokens</Tooltip> to the application that requested them, preventing token theft and misuse.

Traditionally, OAuth 2.0 <Tooltip cta="View Glossary" href="/docs/glossary?term=access+tokens" tip="Access Token: Authorization credential, in the form of an opaque string or JWT, used to access an API.">access tokens</Tooltip> are bearer tokens, which means whoever “bears” or possesses the token can use it. If a bearer token is stolen or leaked, an attacker can present it to a <Tooltip cta="View Glossary" href="/docs/glossary?term=resource+server" tip="Resource Server: Server hosting protected resources. Resource servers accept and respond to protected resource requests.">resource server</Tooltip> (API) and gain unauthorized access by impersonating the legitimate client application or user.
