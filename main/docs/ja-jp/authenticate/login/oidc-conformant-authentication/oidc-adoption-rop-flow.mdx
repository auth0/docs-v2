...
## アクセストークンの構造 (オプション) {#access-token-structure-optional}
...
## 標準のパスワードグラントリクエスト {#standard-password-grant-requests}
