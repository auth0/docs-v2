...
description: バックチャネルログアウトでは、ユーザーの SSO セッションが終了すると、すべての Relying Party にサーバー間のログアウトトークンが送信され、アプリケーションがそれぞれのセッションを終了できるようになります。
---
...
## バックチャネル通信 {#back-channel-communications}
...
### OIDC バックチャネル Logout 通信で使用されるトークン {#tokens-in-oidc-back-channel-logout-communications}
...
### ログアウトトークンストア {#logout-token-store}
