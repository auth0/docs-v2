---
description: SSO セッションを終了し、ブラウザーをアプリケーションのログアウト URL に戻すログアウトエンドポイントを介してユーザーをログアウトさせる方法を学びます。
...
## RP-Initiated Logout を設定する {#configure-rp-initiated-logout}
...
### endpoint discovery を有効にする {#enable-endpoint-discovery}
...
### OIDC Logout エンドポイントを呼び出す {#call-the-oidc-logout-endpoint}
...
#### OIDC Logout エンドポイントのパラメーター {#oidc-logout-endpoint-parameters}
...
#### id_token_hint パラメーター {#id_token_hint-parameter}
...
#### logout_hint パラメータ {#logout_hint-parameter}
...
#### post_logout_redirect_uri パラメーター {#post_logout_redirect_uri-parameter}
...
##### アプリケーションの Allowed Logout URLs を更新する {#update-application-allowed-logout-urls}
...
##### テナントの Allowed Logout URLs を更新 {#update-tenant-allowed-logout-urls}
...
##### Allowed Logout URLs のガイドライン {#allowed-logout-urls-guidelines}
...
##### post_logout_redirect_uri にクエリ文字列パラメーターを追加する {#add-query-string-parameters-to-post_logout_redirect_uri}
...
#### `ui_locales` パラメーター {#ui_locales-parameter}
...
#### `federated` パラメーター {#federated-parameter}
...
## ログアウト同意プロンプト {#logout-consent-prompt}
...
### ログアウトの同意プロンプトを無効にする {#disable-the-logout-consent-prompt}
