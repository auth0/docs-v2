...
## OIDC Back-Channel Logout Initiator の仕組み {#how-oidc-back-channel-logout-initiators-work}
...
## OIDC バックチャネルログアウトイニシエーターを設定する {#configure-oidc-back-channel-logout-initiators}
...
##### `selected_initiators` プロパティ {#selected_initiators-property}
...
##### アプリケーションを現在および今後のすべてのイニシエーターに登録する {#subscribe-an-application-to-all-current-and-future-initiators}
...
##### アプリケーションを password-changed イニシエーターのみに登録する (rp-logout と idp-logout が必要) {#subscribe-an-application-to-password-changed-initiator-only-rp-logout-and-idp-logout-are-required}
...
##### すべてのイニシエーターの購読を解除する (デフォルトは引き続き rp-logout) {#unsubscribe-all-initiators-rp-logout-remains-the-default}
...
#### 選択したイニシエータのみ {#selected-initiators-only}
...
#### サポート対象のすべてのイニシエーター {#all-supported-initiators}
