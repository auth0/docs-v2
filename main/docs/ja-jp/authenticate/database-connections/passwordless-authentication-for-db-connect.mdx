...
## 属性 (識別子) を設定する {#configure-attributes-identifiers}
...
## 新しいデータベース接続を作成する {#create-a-new-database-connection}
...
<Tabs>
  ...
  <Tab title="Management API" icon="code">
    ...
    <Tabs>
      <Tab title="Auth0 CLI">
        <Callout icon="file-lines" color="#0EA5E9" iconType="regular">Auth0 CLI を使用していますか？まだ設定していない場合は、このコマンドを実行する前に [CLI セッションをセットアップして認証します](/docs/ja-jp/deploy-monitor/auth0-cli)。</Callout>
...
## 既存の接続を更新する {#update-existing-connections}
...
## Auth0 Actionsを使用する {#use-auth0-actions}

データベース接続におけるパスワードレス認証要素についてさらに詳しく確認するには、Auth0 Actionsを使用して属性を設定します。

### Post-login トリガー {#post-login-trigger}

`post-login` トリガーは、ユーザーの認証後、認可サーバーがトークンを返す前に実行されます。[Auth0 Dashboard &gt; Actions &gt; Triggers &gt; Post Login](https://manage.auth0.com/#/actions/triggers) の `event.authentication` オブジェクトでは、次のメソッドを利用できます。

| Method        | Parameter | Description                               |
| ------------- | --------- | ----------------------------------------- |
| Email OTP       | `email`   | ユーザーを第1認証要素として認証するために使用されるEmail OTP。        |
| SMS によるPhone OTP | `sms`     | ユーザーを第1認証要素として認証するために使用されるPhone OTP (SMS) 。  |
| 音声通話によるPhone OTP | `tel`     | ユーザーを第1認証要素として認証するために使用されるPhone OTP (音声通話) 。 |
| パスワード         | `pwd`     | ユーザーを第1認証要素として認証するために使用されるパスワード。          |

<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  ユーザーがEmail OTP またはPhone OTP で認証されると、`email_verified` と `phone_verified` はユーザープロファイルに自動的に設定されます。以前に post-login Action を使ってこれらのフラグを手動で設定していた場合は、その回避策を削除できます。
</Callout>
