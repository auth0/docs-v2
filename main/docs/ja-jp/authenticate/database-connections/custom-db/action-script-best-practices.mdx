...
## 外部ユーザーストアへのアクセスを保護する {#secure-access-to-your-external-user-store}
...
### 保護された API を介してアクセスを提供する {#provide-access-with-a-protected-api}
...
## 非同期処理と外部呼び出しのタイムアウト {#time-out-async-and-external-calls}
...
## 無名関数を避ける {#avoid-anonymous-functions}
...
## アイデンティティプロバイダーのトークンを取得する {#retrieve-identity-provider-tokens}
