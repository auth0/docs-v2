---
title: Change Password データベースアクションスクリプトとテンプレート
sidebarTitle: Change Password
description: Change Password スクリプトは、管理者またはユーザーがパスワード変更ワークフローを開始すると実行されます。
---

Change Password スクリプトは、外部データベース内の指定したユーザーのパスワードを変更するための定義済み関数を実装します。この関数名は `changePassword` にすることをお勧めします。

このスクリプトは、[レガシー認証シナリオ](/docs/ja-jp/authenticate/database-connections/custom-db/overview-custom-db-connections)でのみ使用され、外部データベース内のユーザーのパスワードを変更する場合に必要です。これは、ユーザーが[パスワードリセットのワークフロー](/docs/ja-jp/customize/login-pages/classic-login/customize-password-reset-page)を実行したとき、または <Tooltip tip="Auth0 Dashboard: サービスを設定するための Auth0 の主要製品。" cta="用語集を見る" href="/docs/ja-jp/glossary?term=Auth0+Dashboard">Auth0 Dashboard</Tooltip> または Auth0 <Tooltip tip="Management API: お客様が管理タスクを実行できるようにする製品。" cta="用語集を見る" href="/docs/ja-jp/glossary?term=Management+API">Management API</Tooltip> からパスワード変更ワークフローが開始されたときに実行されます。
...
データ漏洩のリスクを防ぐため、`bcrypt` などの暗号学的ハッシュライブラリを使用してパスワード値をハッシュ化します。暗号化されていないパスワード認証情報をログに記録したり、保存または転送したりしないでください。
...
### 成功を返し、last_password_reset 属性を更新する {#return-a-success-and-update-last_password_reset-attribute}
...
## 言語別スクリプト例 {#language-specific-script-examples}
