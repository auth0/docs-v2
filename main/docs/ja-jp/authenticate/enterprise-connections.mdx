...
Auth0 では、Azure AD、Google Workspace、PingFederate などの外部フェデレーション型 <Tooltip tip="アイデンティティプロバイダー（IdP）: デジタルアイデンティティを保存および管理するサービス。" cta="用語集を見る" href="/docs/ja-jp/glossary?term=identity+provider">アイデンティティプロバイダー</Tooltip> (IdP) でユーザーを[認証](https://auth0.com/platform/authentication)するためのエンタープライズ接続を提供しています。
...
## エンタープライズ接続を作成する {#create-an-enterprise-connection}
...
### OpenID Connect (OIDC) プロトコル {#openid-connect-oidc-protocol}
...
## エンタープライズ接続を表示する {#view-enterprise-connections}
...
## アクティブなエンタープライズ接続を認識する {#recognize-an-active-enterprise-connection}
...
## アクティブなエンタープライズ接続を確認する {#know-which-enterprise-connections-are-active}

現在使用されている連携を特定することで、不要な料金の発生を防ぎ、セキュリティを厳格に維持できます。非アクティブな接続を削除することで、認証環境の安全性と効率性を高められます。