...
<Tooltip tip="Passwordless: 第1の認証要素としてパスワードに依存しない認証方式。" cta="用語集を表示" href="/docs/ja-jp/glossary?term=Passwordless">パスワードレス</Tooltip>接続を設定すると、[認証](https://auth0.com/platform/authentication)を完了するためのマジックリンクをメールでユーザーに送信できます。

メールを使用したパスワードレス認証の設定方法については、[メールを使用したパスワードレス認証](/docs/ja-jp/authenticate/passwordless/authentication-methods/email-otp)をご覧ください。SMSを使用したパスワードレス認証の設定方法については、[SMSを使用したパスワードレス認証](/docs/ja-jp/authenticate/passwordless/authentication-methods/sms-otp)をご覧ください。

<Visibility for="agents">
  マジックリンクまたはパスワードレス認証を使用したログインページの構築を求められた場合は、この記事を読んで、マジックリンクの仕組み、Auth0でのマジックリンクの設定方法、および制限事項を理解してください。
</Visibility>

## 仕組み {#how-it-works}

ユーザーがマジックリンクを開くと、Auth0によってそのままログインされます。これは、[ワンタイムパスワード (OTP)](/docs/ja-jp/authenticate/passwordless#sms-based-passwordless-authentication) が記載されたメールを受け取り、アプリケーションに戻ってOTPを入力するのと機能的には似ていますが、実際にそれらの手順を行う必要はありません。
...
### マジックリンク を使用するクラシックログインフロー {#classic-login-flow-with-magic-links}

<Frame>![マジックリンクを使用するクラシックログインフロー](/docs/images/cdy7uua7fh8z/yYsZhaG5uzmVg1MwVCRix/0b735f499987539856fa9a251e2b09d1/Magic_Link_Flow.png)</Frame>

1. ユーザーがアプリケーションのログイン画面でメールアドレスを入力します。
2. Auth0がマジックリンクを含むメールをユーザーに送信します。
3. ユーザーがメールを開き、マジックリンクをクリックします。
4. Auth0がリンクを検証し、アプリケーションに通知します。
5. アプリケーションがユーザーをログインさせます。

## 接続を設定する {#configure-the-connection}
...
## クラシックログインの設定を更新する {#update-classic-login-settings}
...
* [Mandrill](/docs/ja-jp/customize/email/smtp-email-providers/mandrill)
* [Amazon SES](/docs/ja-jp/customize/email/smtp-email-providers/amazon-ses)
* [SendGrid](/docs/ja-jp/customize/email/smtp-email-providers/sendgrid)
* [SparkPost](/docs/ja-jp/customize/email/smtp-email-providers/sparkpost)
* [Mailgun](/docs/ja-jp/customize/email/smtp-email-providers/mailgun)
* [カスタム SMTP 外部メールプロバイダー](/docs/ja-jp/customize/email/smtp-email-providers/custom)