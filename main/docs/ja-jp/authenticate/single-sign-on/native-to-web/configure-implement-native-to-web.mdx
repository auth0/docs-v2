...
## ネイティブからWebへのSSOを設定する {#configure-native-to-web-sso}
...
### Session Transfer Token の作成と管理 {#create-and-manage-session-transfer-tokens}
...
### ネイティブアプリケーションを設定する {#configure-native-applications}
...
### Web アプリケーションを設定する {#configure-web-applications}
...
## ネイティブからWebへのSSOを実装する {#implement-native-to-web-sso}
...
### Native アプリケーションで {#in-your-native-application}

#### Step 1: リフレッシュトークンを Session Transfer Token に交換する {#step-1-exchanging-a-refresh-token-for-a-session-transfer-token}
...
#### ステップ 2: URL またはクッキーを介して Session Transfer Token を送信する {#step-2-send-the-session-transfer-token-through-a-url-or-cookie}

設定されている `allowed_authentication_methods` に応じて、`session_transfer_token` を Web アプリケーションに送信する方法は 2 つあります。

##### オプション 1: `session_transfer_token` を Cookie として送信する {#option-1-send-the-session_transfer_token-as-a-cookie}
...
##### オプション 2: URLパラメーターとして session_transfer_token を送信する {#option-2-send-the-session_transfer_token-as-a-url-parameter}
...
* `session_transfer_token` を URLパラメーターとして [/authorize](https://auth0.com/docs/api/authentication/authorization-code-flow/authorize-application) エンドポイントに付加し、Web アプリケーションにログインする。session&#95;transfer&#95;token は有効で信頼されているため、Auth0 テナントは第1認証要素での認証を要求せずにユーザーを認証します。
...
### Web アプリケーションで {#in-your-web-application}
...
#### オプション 1: Web アプリケーションのリクエストに Session Transfer Token を追加する {#option-1-add-the-session-transfer-token-in-your-web-application-request}
...
#### オプション 2: Auth0 SDKs を使用して Session Transfer Token を Web アプリケーションに追加する {#option-2-add-the-session-transfer-token-to-web-applications-using-auth0-sdks}
...
## Organizations でのネイティブからWebへのSSO {#native-to-web-sso-with-organizations}
...
## Session Transfer Token with Actions {#session-transfer-token-with-actions}
...
### 親リフレッシュトークンのメタデータにアクセスする {#access-parent-refresh-token-metadata}
...
### Token Exchange のログ {#token-exchange-logs}
...
### セッション転送の検証に関する警告ログ {#session-transfer-validation-warning-logs}
