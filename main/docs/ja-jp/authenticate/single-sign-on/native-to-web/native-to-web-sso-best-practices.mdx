---
description: セッションの有効期間とリフレッシュトークンの処理に関するAuth0ネイティブからWebへのSSOのベストプラクティスを適用し、ネイティブアプリからブラウザーへのシームレスな移行を維持します。
...
## Post Login Actions を使用してセッションの有効期間を制限する {#use-post-login-actions-to-limit-session-lifetime}
...
## `session_transfer_token`をデバイスまたはIPアドレスに紐付ける {#bind-session_transfer_token-to-the-device-or-ip-address}
...
## クエリパラメータではなくセキュア Cookie を使用する {#use-secure-cookies-over-query-parameters}
...
## 必要な場合を除き、Webアプリにリフレッシュトークンを発行しないでください {#avoid-issuing-refresh-tokens-to-web-apps-unless-necessary}
...
## 必要に応じて Allow Refresh Tokens を有効にし、リフレッシュトークンを「オンライン」に設定する {#enable-allow-refresh-tokens-when-appropriate-to-set-the-refresh-tokens-as-online}
...
## enforce cascade revocation を有効にする {#enable-enforce-cascade-revocation}
