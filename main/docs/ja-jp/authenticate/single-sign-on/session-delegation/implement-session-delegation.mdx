...
セッション委譲フローの詳細な手順については、[エンドユーザーに代わって Web アプリケーションにアクセスするサポートエージェント](/docs/ja-jp/authenticate/custom-token-exchange/cte-example-use-case-support-agent-accessing-a-web-application-on-behalf-of-an-end-user)のユースケースを参照してください。
...
## Session Transfer Token を取得する {#get-a-session-transfer-token}
...
## Session Transfer Token を使用する {#redeem-the-session-transfer-token}
...
## 取得したトークンを処理する {#handle-the-resulting-tokens}
...
## 引き換えに失敗した場合のトラブルシューティング {#troubleshooting-a-failed-redemption}
