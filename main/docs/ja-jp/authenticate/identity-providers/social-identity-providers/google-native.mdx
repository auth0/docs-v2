...
## Android アプリケーション向けの Sign In with Google の設定 {#configuring-sign-in-with-google-for-android-applications}
...
### Google Cloud Console で資格情報を作成する {#create-credentials-in-the-google-cloud-console}
...
### Androidアプリケーションのコードを更新する {#update-code-in-the-android-application}
...
```go lines expandable
// トークンリプレイ攻撃対策のため、暗号学的に安全なnonceを生成する
val nonceBytes = ByteArray(16) // 128ビットのエントロピー
...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  `nonce` の使用について、以下の推奨事項を確認してください：

  * nonce の生成には CSPRNG を使用してください。汎用的な乱数ソースではなく、`java.security.SecureRandom` を直接呼び出してください。nonce には少なくとも 128 ビットのエントロピーが必要です。詳しくは、[OWASP Cryptographic Storage Cheat Sheet](https://cheatsheetseries.owasp.org/cheatsheets/Cryptographic_Storage_Cheat_Sheet.html#secure-random-number-generation)を参照してください。
  * 認証リクエストごとに新しい nonce を生成してください。nonce を再利用すると、リプレイ攻撃対策の効果が失われます。
  * nonce をログに記録しないでください。ログに記録すると、ログにアクセスできる誰もがその値を参照できるようになり、リプレイ攻撃対策の効果が失われます。
</Callout>

## 追加の認証シナリオ {#additional-authentication-scenarios}
...
### エンタープライズ フェデレーション {#enterprise-federation}
...
### トークン交換時のMFAに関する考慮事項 {#mfa-considerations-during-token-exchange}
