...
5. [Fetch User Profile Script](/docs/ja-jp/authenticate/identity-providers/social-identity-providers/oauth2#fetch-user-profile-script) を設定して、[TikTok の user&#95;info エンドポイント](https://developers.tiktok.com/doc/tiktok-api-v2-get-user-info/) からプロファイル情報を取得します。属性を Auth0 の正規化されたユーザープロファイルにマッピングします。
...
## Management API を使用してカスタムパラメーターを TikTok に渡す {#pass-customs-parameters-to-tiktok-with-the-management-api}
...
### `client_key` フィールドを設定する {#configure-the-client_key-field}
...
## アクセストークンのリクエスト {#access-token-request}
...
### TikTok 連携プロキシをデプロイする {#deploy-tiktok-integration-proxy}
