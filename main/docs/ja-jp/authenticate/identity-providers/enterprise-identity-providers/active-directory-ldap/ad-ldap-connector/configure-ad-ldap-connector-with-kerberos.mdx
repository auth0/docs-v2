...
## Active Directory を設定する {#configure-active-directory}
...
### Kerberos の自動検出範囲 {#auto-detected-range-for-kerberos}
...
## Lock による自動ログイン {#auto-login-with-lock}
...
## 実行時に Kerberos をバイパスする {#bypass-kerberos-at-runtime}
...
## Kerberos リクエストのログ記録を有効にする {#enable-kerberos-request-logging}

1. Kerberos リクエストの詳細なログ記録を有効にするには、システムレベルの環境変数 `DEBUG=kerberos-server` を追加します。
2. Connector を再起動します。
3. 再度ログインし、詳細を確認するためにログを確認します。

## Firefox での Kerberos サポート {#firefox-support-for-kerberos}
...
## Kerberos の HTTPS サポート {#https-support-for-kerberos}
