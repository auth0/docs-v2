...
| **Client ID（クライアント ID）** | 登録されたOktaアプリケーションの一意の識別子です。Okta管理コンソールで登録したアプリの**クライアントID**に保存した値を入力します。 |
| **Client Secret（クライアントシークレット）** | 登録されているOktaアプリケーションへのアクセスに使用される文字列です。Okta管理コンソールで登録したアプリの**クライアントシークレット**に保存した値を入力します。 |
