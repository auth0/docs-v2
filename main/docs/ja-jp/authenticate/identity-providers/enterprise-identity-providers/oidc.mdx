...
## OpenID Connect IDプロバイダーでアプリを設定する {#set-up-your-app-in-the-openid-connect-identity-provider}
...
### Auth0 Dashboard を使用してエンタープライズ接続を作成する {#create-an-enterprise-connection-using-the-auth0-dashboard}
...
#### ディスカバリーエンドポイントでフロントチャネルを使用する {#use-front-channel-with-discovery-endpoint}

<Callout icon="file-lines" color="#0EA5E9" iconType="regular">Auth0 CLI を使用している場合は、このコマンドを実行する前に[CLI セッションを設定して認証](/docs/ja-jp/deploy-monitor/auth0-cli)してください。</Callout>
...
#### ディスカバリーエンドポイントでバックチャネルを利用する {#use-back-channel-with-discovery-endpoint}
...
#### 発行者設定を指定してバックチャネルを使用する {#use-back-channel-specifying-issuer-settings}
...
#### 発行者設定を指定してフロントチャネル方式を使用する {#use-front-channel-specifying-issuer-settings}
...
## PKCE とクレームマッピングを設定する {#configure-pkce-and-claims-mapping}
...
## Auth0アプリケーションのエンタープライズ接続を有効にする {#enable-the-enterprise-connection-for-your-auth0-application}
...
## 発行者メタデータを手動で設定する {#manually-configure-issuer-metadata}

Issuer URL エンドポイントの **発行者の詳細を表示** をクリックすると、データを確認し、必要に応じて調整できます。

## Auth0 とフェデレーションを行う {#federate-with-auth0}
...
## グローバルトークン失効を設定する {#configure-global-token-revocation}
