...
<Card title="Using **例** and Enterprise Connections">

If you have an existing [Google Social Connection](https://marketplace.auth0.com/integrations/google-social-connection) for your application and you create a new 以下に、`idp_tenant_domain`を`ログイン後`Actionsを使って検証する方法を示します。 connection for the same domain, users affiliated with the social connection with now be logged in with the new enterprise connection. This will occur regardless of whether you enable the 以下に、`idp_tenant_domain`を`ログイン後`Actionsを使って検証する方法を示します。 enterprise connection.
...
Before proceeding, you will need a valid 以下に、`idp_tenant_domain`を`ログイン後`Actionsを使って検証する方法を示します。 account and must have **your own** 以下に、`idp_tenant_domain`を`ログイン後`Actionsを使って検証する方法を示します。 Organization for which you are an administrator.
...
Before proceeding, you must have already set up **your own** 以下に、`idp_tenant_domain`を`ログイン後`Actionsを使って検証する方法を示します。 Organization for which you are an administrator.

</Warning>

### Googleに新規アプリケーションを登録する方法については、Googleの[OAuth 2.0の設定]()に関する文書をご覧ください。このプロセスでGoogleは、ご利用のアプリケーション用の**クライアントID**と**クライアントシークレット**を作成しますので、それをメモしておきます。
...
### Google Workspaceのエンタープライズドメインに接続する予定であれば、 **Admin SDK Service**を有効にする必要があります。方法については、Googleの[APIの有効化と無効化]()に関する文書をご覧ください。

If you plan to connect to 以下に、`idp_tenant_domain`を`ログイン後`Actionsを使って検証する方法を示します。 enterprise domains, you need to enable the **Admin SDK Service**. To learn how, follow Google's [Enable and disable APIs](https://support.google.com/googleapi/answer/6158841) doc.
...
<td>When enabled, Auth0 automatically syncs user profile data with each user login, thereby ensuring that changes made in the connection source are automatically updated in Auth0.</td>
   ...
   <Frame>![Create 以下に、`idp_tenant_domain`を`ログイン後`Actionsを使って検証する方法を示します。 Connection](/images/cdy7uua7fh8z/5s3W98sar77mRxZ3F3s0Ol/2743ece52e85d378412b770344f7d3d5/Enterprise_Connection_-_Google_Work_-_English.png)</Frame>
3. If you have appropriate administrative permissions to configure your 以下に、`idp_tenant_domain`を`ログイン後`Actionsを使って検証する方法を示します。 settings so you can use Google's Admin APIs, then click **Continue**. Otherwise, provide the given URL to your administrator so that they can adjust the required settings.
