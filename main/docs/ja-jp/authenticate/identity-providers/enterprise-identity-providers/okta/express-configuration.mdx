...
## アプリケーションを Express Configuration 向けに設定する {#configure-your-application-for-express-configuration}
...
### Initiate Login URI テンプレートを使用してアプリケーションを登録する {#register-an-application-with-initiate-login-uri-template}
...
#### ユーザー属性プロファイル {#user-attribute-profile}
...
### 組織のログイン設定 {#organization-login-settings}
...
### 管理者のログインと同意の設定 {#administrator-login-and-consent-settings}
...
## ユーザーに権限を割り当てる {#assign-permissions-to-users}
...
### 既存のアプリケーションユーザーロールに権限を割り当てる {#assign-permissions-to-a-pre-existing-application-user-role}
...
### アプリケーションユーザーに新しいロールを割り当てる {#assign-a-new-role-to-application-users}
...
#### ロールに権限を割り当てる {#assign-permissions-to-the-role}
...
### post-login Action を使用して、ユーザー属性に基づいて権限を割り当てる {#assign-permissions-based-on-user-attributes-using-a-post-login-action}
...
## ホームレルムディスカバリーを有効にする {#enable-home-realm-discovery}
...
### 接続をまたいで管理者アカウントの一致を維持する {#maintain-admin-account-matching-across-connections}
...
## 1 つの統合で複数のアプリケーションを有効にする {#enable-multiple-applications-under-a-single-integration}
...
## OIN にインテグレーションを公開する {#publish-your-integration-to-the-oin}
...
### OIN にアプリケーションを登録する {#register-your-application-in-the-oin}
...
#### OIN 統合に Express Configuration を追加する {#add-express-configuration-to-your-oin-integration}
...
#### OIN の公開鍵を設定する {#configure-your-oin-public-key}
...
#### ​統合の設定を完了する {#complete-your-integration-configuration}
...
### 統合をテストして検証する {#test-and-verify-your-integration}
...
### 申請を完了する {#finalize-your-submission}
...
### 顧客利用開始フローの例 {#example-customer-enablement-flow}
...
#### Auth0 CLI の例 {#auth0-cli-example}
...
### 管理者アカウントを準備する際の推奨事項 {#recommendations-to-provision-admin-accounts}

既存のユーザーアカウントを使用する場合も、Express Configuration を有効にするために新しいアカウントを作成する場合も、以下の推奨事項に従ってください。

#### 勤務先のメールアドレスを使用する {#use-work-email}
...
### Express Configuration の使用状況を監視する {#monitor-express-configuration-usage}
...
## Management API リファレンス {#management-api-reference}
...
### Okta OIN Express Configuration System API を作成する {#create-the-okta-oin-express-configuration-system-api}
...
### User Attribute Profile を作成する {#create-user-attribute-profile}
...
### 接続プロファイルを作成する {#create-connection-profile}
...
### OIN Express Configuration Client を作成する {#create-oin-express-configuration-client}
...
### SaaS アプリケーションのプロパティを設定する {#configure-saas-application-properties}
...
### OIN クライアントアプリケーションにキーを割り当てる {#assign-keys-to-oin-client-application}
...
### テナントの同意プロンプトをカスタマイズする {#customize-your-tenants-consent-prompt}
