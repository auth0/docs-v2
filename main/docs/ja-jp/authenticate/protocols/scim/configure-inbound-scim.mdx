...
description: "Auth0 のエンタープライズ接続で Inbound SCIM を設定し、アップストリームアイデンティティプロバイダーが SCIM 2.0 プロトコルを介して Auth0 のユーザーをプロビジョニングおよびプロビジョニング解除できるようにします。"
---
...
## エンタープライズ接続で Inbound SCIM を有効にする {#enable-inbound-scim-for-an-enterprise-connection}
...
### Postman でテストする {#test-with-postman}
...
## サポートされている SCIM 操作 {#supported-scim-operations}
...
### SCIM エンドポイントとトークン {#scim-endpoints-and-tokens}
...
### セッションの失効とバックチャネルログアウト {#session-revocation-and-backchannel-logout}
...
| ルート属性のマッピング   | [ルート](/docs/ja-jp/manage-users/user-accounts/user-profiles/root-attributes) 属性にマッピングする場合は、[ユーザープロファイル属性](/docs/ja-jp/manage-users/user-accounts/user-profiles/user-profile-structure#user-profile-attributes) に記載されているルート属性のみが SCIM クエリで検索可能である点に注意してください。この一覧にない属性を検索可能にする必要がある場合は、app&#95;metadata に格納してください。 |
...
### ログイン時に追加属性を同期する {#sync-additional-attributes-at-login}
...
### ロールをユーザー属性として同期する {#sync-roles-as-a-user-attribute}
...
### 組織のメンバーシップ {#organization-membership}
...
### 組織のロールでグループを使用する {#use-groups-with-organizational-roles}
...
### Post-Login Actionでグループを使用する {#use-groups-in-a-post-login-action}
...
## ユーザーとグループを外部システムに同期する {#sync-users-and-groups-to-external-systems}
...
## デプロイのガイドライン {#deployment-guidelines}

### 効率的なセットアップのために連携ギャラリーを活用する {#leverage-integration-galleries-for-streamlined-setup}
...
### Auth0 Self-Service SSO とユーザープロビジョニングを使用する {#use-auth0-self-service-sso-and-user-provisioning}
...
### まずは下位環境で SCIM をテストする {#test-scim-in-lower-level-environments-first}

開発用またはステージング用の Auth0 テナントで連携を十分にテストするまでは、本番用の Auth0 テナントで SCIM を有効にしないでください。

### SCIMトークンを安全に送信する {#send-scim-tokens-securely}
