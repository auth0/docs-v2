...
## IdP テナントを設定する {#configure-idp-tenant}
...
## SAML シーケンスのテスト用ユーザーを作成する {#create-user-to-test-saml-sequence}
...
## サービスプロバイダー テナントを設定する {#configure-service-provider-tenant}
...
## IdP にサービスプロバイダーのメタデータを追加する {#add-service-provider-metadata-to-idp}
...
## SAML 接続をテストするためのアプリケーションを作成する {#create-application-to-test-saml-connection}
...
## サービスとアイデンティティプロバイダー間の接続をテストする {#test-connection-between-service-and-identity-provider}
...
## テスト用アプリケーションのWebページを作成する {#create-a-web-page-for-test-application}
...
## サンプルアプリケーションをテストする {#test-sample-application}
...
## テストシナリオのトラブルシューティング {#troubleshoot-test-scenario}
