...
## SAMLサービスプロバイダーとしてのAuth0 {#auth0-as-the-saml-service-provider}

これらのシナリオは、Auth0がSAMLサービスプロバイダーである場合に該当します。つまり、Auth0はSAML接続を作成してSAMLアイデンティティプロバイダーに接続します。

### SAML認証リクエストに署名する {#sign-the-saml-authentication-request}
...
#### deflate エンコーディングの有効化/無効化 {#enabledisable-deflate-encoding}
...
### カスタムキーを使用してリクエストに署名する {#use-a-custom-key-to-sign-requests}
...
### 署名付きの SAML 認証レスポンスを受信する {#receive-signed-saml-authentication-responses}
...
### 暗号化されたSAML認証アサーションを受信する {#receive-encrypted-saml-authentication-assertions}
...
#### コンテンツ復号 algorithmsを設定する {#set-content-decryption-algorithms}
...
### 鍵ペアを使用して暗号化されたレスポンスを復号する {#use-your-key-pair-to-decrypt-encrypted-responses}
...
## SAMLアイデンティティプロバイダーとしてのAuth0 {#auth0-as-the-saml-identity-provider}
...
### SAML レスポンス/アサーションに署名する {#sign-the-saml-responsesassertions}
...
#### SAML レスポンスの署名鍵を変更する {#change-the-signing-key-for-saml-responses}
...
#### テナント署名鍵のローテーション {#rotate-the-tenant-signing-key}
...
### 署名付きの SAML 認証リクエストを受信する {#receive-signed-saml-authentication-requests}
...
### 暗号化されたSAML認証アサーションを送信する {#send-encrypted-saml-authentication-assertions}
