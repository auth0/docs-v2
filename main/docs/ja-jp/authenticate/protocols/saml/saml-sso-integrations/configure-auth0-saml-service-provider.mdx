...
## IdPからメタデータと証明書を取得する {#get-metadata-and-certificate-from-the-idp}
...
## Auth0 で SAML エンタープライズ接続を作成する {#create-saml-enterprise-connection-in-auth0}
...
### プロキシゲートウェイ向けに SAML 接続を設定する {#configure-saml-connection-for-proxy-gateways}
...
### リクエストテンプレートをカスタマイズする {#customize-the-request-template}
...
| Name                             | Description                                                                                                                                                                                                                                                                                    |
| -------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AssertionConsumerServiceURL`    | ユーザーのサインイン後に IdP がレスポンスを送信する URL。これを使用する場合は、`AuthnRequest` に署名し、リクエストテンプレートに `ProtocolBinding` 属性を含めてください。                                                                                                                                                                                     |
...
## 接続のトラブルシューティング {#troubleshoot-connection}
