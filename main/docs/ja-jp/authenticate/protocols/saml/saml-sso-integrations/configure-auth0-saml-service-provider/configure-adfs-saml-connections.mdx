---
description: Auth0 をサービスプロバイダーとして、Microsoft Active Directory Federation Services (ADFS) へのカスタム SAML 接続を作成します。これにはクレームマッピングの設定も含まれます。
...
#### フェデレーション メタデータを使用して作成する {#create-using-federation-metadata}
...
### クレーム発行ポリシーの編集 {#edit-claim-issuance-policy}
...
### 署名証明書のエクスポート {#export-signing-certificate}
...
## Auth0 をサービスプロバイダーとして SAML 接続を作成する {#create-saml-connection-with-auth0-as-service-provider}
...
## 証明書利用者信頼 の編集 {#edit-relying-party-trust}
...
## 署名リクエストを有効にする (任意) {#enable-sign-requests-optional}
...
## 統合を有効にしてテストする {#enable-and-test-the-integration}
