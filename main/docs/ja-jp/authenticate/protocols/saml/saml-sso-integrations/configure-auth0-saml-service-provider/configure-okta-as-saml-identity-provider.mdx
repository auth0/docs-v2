...
## Okta SAML アプリ連携を設定する {#configure-okta-saml-app-integration}

Okta Developer Console で SAML アプリ連携を作成できます。

### アプリ連携を作成する {#create-app-integration}
...
### SSO URLを控え、証明書をダウンロードする {#record-sso-url-and-download-certificate}
...
## Auth0 で SAML 接続を設定する {#configure-saml-connection-in-auth0}
...
## Auth0で SAML Enterprise 接続を有効にする {#enable-saml-enterprise-connection-in-auth0}

Auth0 Dashboard で SAML Enterprise 接続を有効にできます。

### Organizations を使用する場合に SAML Enterprise 接続を有効にする {#enable-saml-enterprise-connection-when-using-organizations}
...
### Organizations を使用しない場合に SAML Enterprise 接続を有効にする {#enable-saml-enterprise-connection-when-not-using-organizations}
