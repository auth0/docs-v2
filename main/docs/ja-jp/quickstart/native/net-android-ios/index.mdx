...
## ユーザー情報にアクセスする {#access-user-information}

認証に成功すると、ログイン結果からユーザー情報にアクセスできます。

### 認証結果 {#authentication-result}
...
### すべてのクレームを確認する {#iterate-through-all-claims}
...
### カスタムスコープをリクエストする {#request-custom-scopes}
...
## トラブルシューティングと Advanced {#troubleshooting-advanced}

<Accordion title="よくある問題と解決策">
  ### ブラウザーからアプリにリダイレクトされない {#browser-doesnt-redirect-back-to-app}
  ...
  **解決策:**
  ...
  ### &quot;Invalid Callback URL&quot; エラーで認証に失敗する {#authentication-fails-with-invalid-callback-url-error}
  ...
  * コード内の Domain が Auth0 Dashboard の Domain と一致していることを確認します
  ...
  ### LoginAsync() が応答しない、または完了しない {#loginasync-hangs-or-never-completes}
  ...
  * ネットワーク接続を確認します
  ...
  ### エラー: &quot;Default App must use Token Endpoint Authentication Method &#39;None&#39;&quot; {#error-default-app-must-use-token-endpoint-authentication-method-none}
  ...
  **修正方法:**
  ...
  ### iOS: ブラウザーが開かない {#ios-browser-doesnt-open}
...
<Accordion title="本番環境での考慮事項">
  ...
  * **ProGuard/コード難読化**: Android でコード難読化を使用する場合は、適切なルールを追加します
  ...
  ### App Store の要件 {#app-store-requirements}
...
<Accordion title="高度な設定">
  ### カスタムスコープと audience {#custom-scopes-and-audience}
  ...
  ### プラットフォーム固有のブラウザ設定 {#platform-specific-browser-configuration}
