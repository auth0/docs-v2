...
<Recipe>
  <Content>
      ...
      - `com_auth0_domain`：Auth0テナントのドメインです。通常、Auth0 Dashboardにあるアプリケーションの［Settings（設定）］の［Domain（ドメイン）］フィールドで確認できます。[カスタムドメイン](/docs/ja-jp/customize/custom-domains)を使用している場合は、その値を代わりに設定してください。
