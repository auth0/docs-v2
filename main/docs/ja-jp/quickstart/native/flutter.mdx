...
<Accordion title="AI を使って Auth0 を統合" icon="microchip-ai" iconType="solid" defaultOpen>
  ...
  **次に、AI アシスタントに以下のように依頼します:**
  ...
  AI アシスタントが、Auth0 アプリケーションの作成、資格情報の取得、auth0&#95;flutter SDK 依存関係の追加、Android と iOS のコールバック URL の設定に加え、安全な資格情報ストレージを使用した Web Auth のログイン/ログアウト実装まで自動的に行います。[agent skills の完全なドキュメント →](/quickstart/agent-skills)
</Accordion>
...
## トラブルシューティングと高度な使い方 {#troubleshooting-advanced-usage}

<Accordion title="よくある問題と解決策">
  ### コールバックURLの不一致 {#callback-url-mismatch}
  ...
  **解決策:**
  ...
  ### Android: Chrome Custom Tab が開かない {#android-chrome-custom-tab-doesnt-open}
  ...
  **対処法:**
  ...
  ### iOS: 「App で開く」アラート {#ios-open-in-app-alert}
  ...
  * または `useEphemeralSession: true` を設定する (SSO を無効化) :
  ...
  ### ユーザーによって認証がキャンセルされた {#authentication-cancelled-by-user}
...
<Accordion title="資格情報の管理">
  ...
  ### 保存済みの資格情報を確認する {#check-for-stored-credentials}
  ...
  ### 保存済みの資格情報を取得する {#retrieve-stored-credentials}
...
<Accordion title="高度な Flutter 連携">
  ### 生体認証による資格情報のセキュリティ強化 {#enhanced-credential-security-with-biometrics}
...
<Accordion title="本番デプロイ">
  ### App Store 公開の準備 {#app-store-preparation}
