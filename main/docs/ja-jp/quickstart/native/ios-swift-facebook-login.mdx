...
<Accordion title="よくある問題と解決方法">
  ### 認証エラーでトークン交換に失敗する {#token-exchange-fails-with-authentication-error}
  ...
  **対処方法:**
  ...
  ### セッショントークンのリクエストでエラーが返る {#session-token-request-returns-an-error}
  ...
  **対処方法:**
  ...
  ### ユーザープロファイルの項目が不足している {#user-profile-fields-are-missing}
  ...
  * `fetchUserProfile` の `request.parameters["fields"]` 呼び出しに同じフィールドが含まれていることを確認します
  ...
  ### `Auth0.plist` が見つからない {#auth0plist-not-found}
...
<Accordion title="本番運用時の注意点">
  ...
  * **生体認証による保護**: 保存された資格情報を保護するため、`CredentialsManager` で生体認証を有効にします
  ...
  ### Facebook アプリの設定 {#facebook-app-configuration}
  ...
  * Login SDK を使用するアプリに対する Facebook の [Data Policy](https://developers.facebook.com/policy/) の要件を確認します
  ...
  ### App Store への申請 {#app-store-submission}
