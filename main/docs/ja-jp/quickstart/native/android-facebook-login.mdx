...
<Accordion title="よくある問題と解決策">
  ### token exchange が authentication エラーで失敗する {#token-exchange-fails-with-authentication-error}
  ...
  **解決策:**
  ...
  ### session token request がエラーを返す {#session-token-request-returns-an-error}
  ...
  **修正:**
  ...
  ### ユーザープロファイルのフィールドが不足している {#user-profile-fields-are-missing}
