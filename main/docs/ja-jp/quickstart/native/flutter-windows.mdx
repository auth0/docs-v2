...
description: このガイドでは、Auth0 Flutter SDK（Beta版）を使用して、Flutter Windows デスクトップアプリケーションに Auth0 を統合する方法を説明します。
mode: wide
---
...
<Note>
  ...
  * Auth0 アカウント — [無料でサインアップ](https://auth0.com/signup)。
  ...
  この機能は**Beta**版です (`auth0_flutter` 2.1.0-beta.1) 。API は一般提供前に変更される可能性があります。
</Note>
...
## トラブルシューティングと高度な使い方 {#troubleshoot-advanced-use}

<Accordion title="よくある問題と解決策">
  ### ブラウザーは開くが、アプリがコールバックを受信しない {#browser-opens-but-app-doesnt-receive-callback}
  ...
  **対処法:**
  ...
  ### 5 分後に認証がタイムアウトする {#authentication-times-out-after-5-minutes}
  ...
  **対処法:** アプリがコールバック URI を受信できていません。次を確認してください:
  ...
  ### URI を転送せずに 2 つ目のアプリ インスタンスが起動する {#second-app-instance-launches-instead-of-forwarding-uri}
  ...
  **対処法:**
  ...
  ### WindowsWebAuthentication が見つからない {#windowswebauthentication-not-found}
  ...
  **対処法:** `pubspec.yaml` で Beta 版が指定されていることを確認してください:
  ...
  ### Callback URL の不一致エラー {#callback-url-mismatch-error}
...
* [Flutter quickstart (Android, iOS, macOS, Web)](/docs/ja-jp/quickstart/native/flutter) — 他のプラットフォーム向けガイド。
* [Token Storage Best Practices](/docs/ja-jp/secure/security-guidance/data-security/token-storage) — 認証情報の安全な管理。
* [Auth0 Universal Login](/docs/ja-jp/authenticate/login/auth0-universal-login) — ログイン体験をカスタマイズ。