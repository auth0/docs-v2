...
<Steps>
  ...
  <Step title="アプリの資格情報を設定" stepNumber={4}>
    ...
    <Warning>
      `clientId` や `domain` の値を Swift のソースファイルに直接ハードコードしないでください。バージョン管理にコミットされてしまいます。実行時に環境変数またはビルド時の構成ファイルから読み込んでください。ほとんどのアプリでは `Auth0.plist` (このクイックスタートでのデフォルトの方法) を推奨します。
    </Warning>
...
## トラブルシューティングと高度な設定 {#troubleshooting-advanced}

<Accordion title="よくある問題と解決策">
  ### ビルドエラー: &#39;Auth0&#39; module not found {#build-errors-auth0-module-not-found}
  ...
  **解決策**:
  ...
  ### アプリがクラッシュする: &#39;Auth0.plist not found&#39; {#app-crashes-auth0plist-not-found}
  ...
  **修正方法**:
  ...
  ### ブラウザは開くがアプリに戻らない {#browser-opens-but-never-returns-to-app}
  ...
  **修正方法**:
  ...
  ### 毎回権限アラートが表示される {#permission-alert-appears-every-time}
...
<Accordion title="本番デプロイ">
  ### App Store 公開準備 {#app-store-preparation}
...
<Accordion title="高度な連携">
  ### biometrics による Keychain セキュリティの強化 {#enhanced-keychain-security-with-biometrics}
  ...
  ### トークンの自動更新 {#automatic-token-refresh}
  ...
  ### App Extensions 間での資格情報の共有 {#shared-credentials-across-app-extensions}
  ...
  **要件:**
  ...
  ### 認証フローオプションの比較 {#authentication-flow-options-comparison}
