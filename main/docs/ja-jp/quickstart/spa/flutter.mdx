...
<Accordion title="AI を使って Auth0 を統合する" icon="microchip-ai" iconType="solid" defaultOpen>
  Claude Code、Cursor、GitHub Copilot などの AI コーディングアシスタントを使えば、[agent skills](https://agentskills.io/home) を使用して数分で Auth0 の authentication を自動的に追加できます。
  ...
  **次に、AI アシスタントに次のように依頼します:**
  ...
  AI アシスタントは、Auth0 アプリケーションの作成、資格情報の取得、auth0&#95;flutter SDK 依存関係の追加、web/index.html の設定、コールバック URL の設定、ログイン/ログアウトフローの実装を自動的に行います。[agent skills の完全なドキュメント →](/quickstart/agent-skills)
</Accordion>
...
<Steps>
  ...
  <Step title="Auth0 Appを設定する" stepNumber={3}>
    ...
    Auth0 App は、CLI コマンドを実行して自動的に設定することも、Auth0 Dashboard から手動で設定することもできます。
    ...
    <Tabs>
      <Tab title="CLI">
        ...
        <CodeGroup>
          ```shellscript Mac
          # Auth0 CLI をインストールする（まだインストールしていない場合）
          brew tap auth0/auth0-cli && brew install auth0

          # Auth0 App を設定し、lib/auth_config.dart を生成する
...
### 「コールバック URL の不一致」エラー {#callback-url-mismatch-error}
...
### 認証が機能しない {#authentication-not-working}
...
### ページを再読み込みするとユーザーがログアウトされる {#user-logged-out-after-page-refresh}
...
### 「Invalid state」エラー {#invalid-state-error}
...
### ブラウザーコンソールでの CORS エラー {#cors-errors-in-browser-console}
...
* **[Flutter Native App Quickstart](/docs/ja-jp/quickstart/native/flutter/interactive)** - Auth0 を使ってネイティブモバイルアプリを構築する
