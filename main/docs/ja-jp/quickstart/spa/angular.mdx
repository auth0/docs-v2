...
<Accordion title="AI を使って Auth0 を統合する" icon="microchip-ai" iconType="solid" defaultOpen>
  ...
  **次に、AI アシスタントにこう依頼します。**
  ...
  AI アシスタントが、Auth0 アプリケーションの作成、資格情報の取得、`@auth0/auth0-angular` のインストール、ルートガードや HTTP インターセプターの作成、環境設定まで自動的に行います。[agent skills の完全なドキュメント →](/docs/ja-jp/quickstart/agent-skills)
</Accordion>
  ...
  <Step title="Auth0 Appをセットアップする" stepNumber={3}>
    ...
    <Tabs>
      ...
      <Tab title="CLI">
        ...
        <CodeGroup>
          ```shellscript Mac
          # Auth0 CLIをインストール（まだインストールしていない場合）
          brew tap auth0/auth0-cli && brew install auth0

          # Auth0アプリをセットアップし、環境ファイルを生成
        ...
        <Note>
          ...
          2. `http://localhost:4200` 用に設定されたAuth0 Single Page Applicationを作成します
          3. Auth0のドメインとクライアントIDを含む `src/environments/environment.ts` を生成します
        </Note>
      </Tab>
