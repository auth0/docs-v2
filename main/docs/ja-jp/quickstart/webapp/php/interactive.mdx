...
<QuickstartButtons githubLink="https://github.com/auth0-samples/auth0-php-web-app/tree/main/app" lang="ja-jp" />
...
<Recipe>
  <Content>
    ...
    <Section id={sections[1].id} title={sections[1].title} stepNumber="2">
      ...
      - `domain`：Auth0テナントのドメインです。通常、Auth0 Dashboardにあるアプリケーションの設定の［Domain（ドメイン）］フィールドで確認できます。[カスタムドメイン](/docs/ja-jp/customize/custom-domains)を使用している場合は、その値を代わりに設定してください。
    ...
    <Section id={sections[5].id} title={sections[5].title} stepNumber="6">
      ユーザーがログインやログアウトできるようになったら、認証済のユーザーに関連付けられた[プロファイル情報](/docs/ja-jp/manage-users/user-accounts/user-profiles)を取得できるようにしたいと考えるはずです。たとえば、ログインしたユーザーの名前やプロフィール写真をプロジェクトに表示したいかもしれません。
