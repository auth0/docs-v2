...
## 1. 新しいプロジェクトを作成する {#1-create-a-new-project}
...
## 2. Auth0 Express SDKをインストールする {#2-install-the-auth0-express-sdk}
...
## 3. Auth0 App を設定する {#3-setup-your-auth0-app}
...
## 4. ミドルウェアを設定する {#4-configure-the-middleware}
...
## 5. ログイン、ログアウト、プロファイルのルートを作成する {#5-create-login-logout-and-profile-routes}
...
<AccordionGroup>
  ...
  <Accordion title="クレームベース認可を使用する">
    ...
    <Note>
      `role` のようなクレームは、Auth0 ルールまたは Actions を使用してトークンに追加する必要があります。[カスタムクレームの追加方法について詳しくはこちら](/docs/ja-jp/manage-users/access-control/sample-use-cases-actions-with-authorization#add-user-roles-to-tokens)。
    </Note>
  </Accordion>
...
### ログイン後に「Invalid state」エラーが発生する {#invalid-state-error-after-login}
...
### 「req.oidc is undefined」 {#reqoidc-is-undefined}
...
### セッションが大きすぎる / Cookie エラー {#session-too-large-cookie-errors}
...
### Callback URL の不一致 {#callback-url-mismatch}
...
### 環境変数が読み込まれない {#environment-variables-not-loading}
...
* **[認可を追加する](/docs/ja-jp/manage-users/access-control/rbac)** - ロールベースのアクセス制御を実装する
* **[保護された API を呼び出す](/docs/ja-jp/secure/tokens/access-tokens/get-access-tokens)** - アクセストークンを使用してバックエンド API を呼び出す
* **[Universal Login をカスタマイズする](/docs/ja-jp/customize/universal-login-pages)** - ログイン画面をブランドに合わせる
* **[ソーシャル接続を追加する](/docs/ja-jp/connections/social)** - Google、GitHub、その他のソーシャルログインを有効にする
* **[MFA を実装する](/docs/ja-jp/secure/multi-factor-authentication)** - 多要素認証を追加する
