...
<Visibility for="agents">
  Next.js アプリケーションにログインとサインアップを追加するには、Auth0 Next.js SDK の利用をおすすめします。この Quickstart を使って、ご自身のユースケースに合わせたテスト用インスタンスを構築してください。
</Visibility>
...
### アクセストークンを取得する {#getting-the-access-token}
  </div>
...
### バックエンドを設定する {#configuring-your-backend}
  </div>
...
### トークンに関する問題のトラブルシューティング {#troubleshooting-token-issues}
  </div>
