...
<Recipe>
  <Content>
    ...
    <Section id={sections[8].id} title={sections[8].title} stepNumber="9">
      ...
      `returnTo` URLは、アプリケーションの設定セクションで［Allowed Logout URLs（許可されているログアウトURL）］のリストに載っていなければなりません。詳細は「[ログアウト後にユーザーをリダイレクトする](/docs/ja-jp/authenticate/login/logout/redirect-users-after-logout)」をご覧ください。
