...
import {GlossaryPage} from "/snippets/ja-jp/Glossary.jsx";
...
<GlossaryPage
  terms={[
      ...
      },
      {
        ...
        description:
          "計画、情報取得、推論、ツール呼び出しなどのアクションを実行できる自律型 AI エージェントを構築するためのソフトウェアツール。通常は大規模言語モデル（LLM）を活用します。",
      },
      {
        term: "Application",
        description:
      ...
      },
      {
        term: "GenAI",
        description:
          "生成AI。トランスフォーマー、GAN、拡散モデルなどの生成モデルを使用して、テキスト、画像、動画などの新しいコンテンツを生成します。エージェント型フレームワークと統合すると、GenAIモデルはコンテンツ生成にとどまらず、自律的に計画し、情報を取得し、ツールを呼び出せるようになります。",
      },
      {
      ...
      },
      {
        ...
        description:
          "大規模言語モデル（LLM）が、データベースや API などの外部ソースから取得した関連情報を取り込み、ユーザーの問い合わせに対して文脈に即したより正確な回答を提供するための技術。",
      },
      {
        term: "Role",
        description:
