...
<ParamField body="scope" type="string">
  <Badge color="yellow">早期アクセス</Badge> (任意) 外部プロバイダーのアクセストークンに要求するスコープをスペース区切りで指定したリストです。これを使用して、元々許可されたスコープのサブセットを要求します。指定しない場合、元々許可されたすべてのスコープを含むトークンが発行されます。特権ワーカートークン交換 (`subject_token_type` が `urn:ietf:params:oauth:token-type:jwt` の場合) では、要求されたスコープは、指定された接続の `token_vault_privileged_access.grants` に設定されたスコープとの共通部分になります。
</ParamField>
