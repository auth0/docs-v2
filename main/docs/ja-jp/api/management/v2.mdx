...
## リクエストの関連付け {#request-correlation}
...
### チェックポイントベースのページネーション {#checkpoint-based-pagination}
...
#### チェックポイント ID の有効期限 {#checkpoint-id-expiry}
...
#### 順方向のみの制約 {#forward-only-constraints}
...
### オフセットページネーションとチェックポイントページネーションの選び方 {#choosing-between-offset-and-checkpoint-pagination}
...
### ページネーションを扱う際のベストプラクティス {#best-practices-for-handling-pagination}
...
### アクセストークンを使ってテストする {#test-with-an-access-token}
