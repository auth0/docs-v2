...
## state バリデーションの仕組み {#how-state-validation-works}
...
## invalid state エラーの一般的な原因 {#common-causes-of-the-invalid-state-error}

以下では、invalid state エラーの一般的な原因と、実施できるトラブルシューティングの手順を紹介します。

### キャッシュされたコールバック URL {#cached-callback-urls}
...
### キャッシュされたCookieとURLパラメーター {#cached-cookies-and-url-parameters}
...
### Universal Login ページとリンクの作成 {#universal-login-page-and-link-building}
...
### コールバック URL に直接アクセスする {#visiting-the-callback-url-directly}
...
## invalid state エラーのトラブルシューティング {#troubleshoot-invalid-state-errors}
