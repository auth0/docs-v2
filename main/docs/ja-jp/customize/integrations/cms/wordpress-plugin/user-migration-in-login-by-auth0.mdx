...
ユーザー移行機能では、カスタムデータベース と呼ばれる Auth0 の中核機能と、Login by Auth0 WordPress Plugin の URL エンドポイントを組み合わせることで、ユーザーが既存の WordPress ユーザーアカウントを使って認証できるようになります。カスタムデータベースについて詳しくは、[カスタムデータベース接続](/docs/ja-jp/authenticate/database-connections/custom-db) をご覧ください。
...
## セットアップと設定 {#set-up-and-configuration}

ユーザー移行を設定する最も簡単な方法は、プラグインの初回インストール時に Setup Wizard を使用することです。手順の詳細については、[Login by Auth0 をインストールする](/docs/ja-jp/customize/integrations/cms/wordpress-plugin/install-login-by-auth0)をご覧ください。
...
1. [アプリケーションを作成して正しく設定](/docs/ja-jp/customize/integrations/cms/wordpress-plugin/configure-login-by-auth0)し、そのアプリケーション用の空のデータベース接続を作成して有効化します。これらは標準の Setup Wizard で作成したものをそのまま使用しても、一から新規作成してもかまいません。Setup Wizard について詳しくは、[Install Login by Auth0](/docs/ja-jp/customize/integrations/cms/wordpress-plugin/install-login-by-auth0)を参照してください。
...
### JSON の位置 0 に予期しない token &lt; があります {#unexpected-token-in-json-at-position-0}
...
### メールアドレスまたはパスワードが正しくありません {#wrong-email-or-password}
...
### Email を変更できない、またはユーザーデータが誤っている {#cannot-change-email-or-incorrect-user-data}
...
* 保持する必要のあるデータが Auth0 にある場合は、[User Import/Export Extension を使用して](/docs/ja-jp/manage-users/user-migration/user-import-export-extension)、ユーザーデータを調整できます。
