...
## AWS で外部アイデンティティプロバイダーを作成する {#create-an-external-identity-provider-in-aws}
...
## Auth0 の SAML SSO を設定する {#configure-saml-sso-for-auth0}
...
## AWS ロールをユーザーにマッピングする {#map-aws-role-to-a-user}
...
### 複数のロールをマッピングする {#map-multiple-roles}
...
## セッションの有効期限を設定する {#configure-session-expiration}
