...
### 従来型Webアプリケーションを作成する {#create-a-regular-web-application}
...
### 接続を作成して有効にする {#create-and-enable-a-connection}
...
## Azure の設定 {#configure-azure}

### Azure API Management Service を作成する {#create-azure-api-management-service}
...
### サンプル API をインポートする {#import-the-sample-api}
...
### OAuth 2.0 認可サーバーを設定する {#configure-an-oauth-20-authorization-server}
...
### Auth0 を使用するように Azure API を設定する {#configure-azure-api-to-use-auth0}
...
## Auth0 で Allowed Callback URL を設定する {#set-the-allowed-callback-url-in-auth0}
...
### アクセストークン用の JWT 検証ポリシーを設定する {#configure-a-jwt-validation-policy-for-access-tokens}
