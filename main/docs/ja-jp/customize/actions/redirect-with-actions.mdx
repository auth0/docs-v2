...
## 認証フローを再開する {#resume-the-authentication-flow}
...
## 外部サイトにデータを渡す {#pass-data-to-the-external-site}
...
### token が改ざんされていないことを確認する {#ensure-the-token-has-not-been-tampered-with}
...
## データを Auth0 に返す {#pass-data-back-to-auth0}
...
### 可能であればアプリのメタデータを使用する {#use-app-metadata-where-possible}
...
### Auth0 ユーザープロファイルに保存するデータは必要最小限にしてください {#be-selective-when-storing-data-on-the-auth0-user-profile}
...
## フロントチャネルでデータを送信する {#send-data-on-the-front-channel}
...
### 情報を Action に渡す {#pass-information-back-to-the-action}
...
## 独自の認証方法 {#custom-authentication-methods}
...
### リソース所有者エンドポイント {#resource-owner-endpoint}
...
### `prompt=none` のフロー {#flows-where-promptnone}
