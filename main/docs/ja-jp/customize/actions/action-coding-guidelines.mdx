...
* [アプリケーションメタデータ](/applications/configure-application-metadata)を使用して、アクションを実行するべきかどうかを判断するために、特定のアプリケーションに対してフィルター処理を行います。
...
* Authentication API呼び出し用のドメインには、`event.request.hostname`を使用します。デフォルトのAuth0テナントドメイン、または[カスタムドメイン](/custom-domains)を指定できます。
