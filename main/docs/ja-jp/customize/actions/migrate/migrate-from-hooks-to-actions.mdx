...
### Hook のコードを新しい Action にコピーする {#copy-hook-code-to-a-new-action}
...
### 依存関係を移行する {#convert-dependencies}
...
### データへのアクセス方法を変更する {#change-how-data-is-accessed}

フックでは、ユーザー、クライアント、リクエスト、そのほかのコンテキストデータに関する情報は、フック関数に渡される複数の引数に格納されています。Actions では、これらのデータは再構成され、`event` オブジェクトに移されています。多くのプロパティはそのまま移行されていますが、わかりやすくするために統合されたものもあります。

変換するフックの種類によって、`event` オブジェクトの内容は異なります。

* [Client Credentials Exchange - Actions イベントオブジェクト](/docs/ja-jp/customize/actions/reference/credentials-exchange/credentials-exchange-event-object)
...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  フックの `context` オブジェクトとは異なり、`event` オブジェクトのプロパティに保存または変更されたデータは、後続の Actions には引き継がれません。フックでこれらのプロパティにデータを設定してコア機能をトリガーしている場合は、Actions 間でデータを保持するために、[Machine to Machine](/docs/ja-jp/actions/reference/credentials-exchange/credentials-exchange-api-object) および [Pre User Registration](/docs/ja-jp/customize/actions/explore-triggers/signup-and-login-triggers/pre-user-registration-trigger/pre-user-registration-api-object) の Actions Flows で利用できる `api` インターフェースを使用する必要があります。
</Callout>

### コールバックを変換する {#convert-callbacks}
...
Client Credentials Exchange フック で アクセストークン に追加のクレームを設定していた場合:
...
これで、Actionsの[Client Credentials Exchange API オブジェクト](/docs/ja-jp/actions/reference/credentials-exchange/credentials-exchange-api-object)を使用できるようになりました：
