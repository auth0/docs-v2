...
**Note:** The 現在のトランザクションに関連付けられているユーザーを記述するオブジェクト。 object does not include any top-level attributes added by an external <Tooltip tip="Identity Provider (IdP): Service that stores and manages digital identities." cta="View Glossary" href="/docs/glossary?term=identity+provider">identity provider</Tooltip>.
