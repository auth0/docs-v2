...
### 特定のアプリケーションへのアクセスを平日のみに制限する {#allow-access-only-on-weekdays-for-a-specific-application}
...
### API を呼び出すすべてのユーザーのアクセスを拒否する {#deny-access-to-anyone-calling-an-api}
...
### ID トークンとアクセストークンにユーザーロールを追加する {#add-user-roles-to-id-and-access-tokens}
...
### ユーザープロファイルを拡張する {#enrich-the-user-profile}
...
### カスタムMFAポリシーを適用する {#enforce-custom-mfa-policy}
...
### パスキーで手間を軽減する {#reduce-friction-with-passkeys}
...
### ユーザーを外部サイトにリダイレクトする {#redirect-the-user-to-an-external-site}
...
### access token の scopes を変更する {#modify-scopes-on-an-access-token}
...
### 特定のJA3/JA4フィンガープリントからのアクセスを拒否する {#deny-access-to-specific-ja3ja4-fingerprints}
