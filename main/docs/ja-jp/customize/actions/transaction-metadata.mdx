...
Actions トランザクションメタデータを使用すると、トランザクションの間、[カスタムトークン交換](/docs/ja-jp/customize/actions/explore-triggers/custom-token-exchange) Action と [post-login](/docs/ja-jp/customize/actions/explore-triggers/signup-and-login-triggers/login-trigger) Action 間でカスタムメタデータを保存、参照、共有できます。
...
`api` オブジェクトと `event` オブジェクトは、[カスタムトークン交換 Actions](/docs/ja-jp/customize/actions/reference/custom-token-exchange/custom-token-exchange-api-object) でも使用できます。
...
### メタデータにすぐアクセスする {#access-metadata-immediately}
...
### 対応している値を設定する {#set-supported-values}
...
### Actions 間で値を共有する {#share-values-between-actions}
...
### カスタムトークン交換からPost-Loginへ値を共有する {#share-values-from-custom-token-exchange-to-post-login}

[カスタムトークン交換](/docs/ja-jp/customize/actions/explore-triggers/custom-token-exchange) Actionでキーと値のペアを設定し、同じトランザクション内のPost-Login Actionで読み取ります。たとえば、カスタムトークン交換Actionが`subject_token`または`actor_token`からすでに取得・処理した情報に基づいて、Post-Login Actionでカスタムクレームを設定する場合に便利です。

**カスタムトークン交換Action**

`subject_token`からクレームをデコードし、`api`オブジェクトの`setMetadata()`メソッドを使用してトランザクションメタデータに設定します。

```text lines
exports.onExecuteCustomTokenExchange = async (event, api) => {
  // subject_token は、Action 内ですでにデコードおよび検証されています
...
### 外部サイトへのリダイレクト時に値を保持する {#preserve-values-on-redirect-to-external-sites}
...
### Forms のレンダリング時に値を保持する {#preserve-values-on-forms-rendering}
...
### Forms と値を共有する {#share-values-with-forms}
