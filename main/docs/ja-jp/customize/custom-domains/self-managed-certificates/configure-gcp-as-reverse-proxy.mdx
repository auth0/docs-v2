...
1. [自己管理証明書を使ってカスタムドメインを構成する](/custom-domains/self-managed-certificates)作業がまだ済んでいない場合は、ここで行います。**［Origin Domain Name（オリジンのドメイン名）］**と`cname-api-key`の値を後で使用するためにメモしておきます。
2. [［Auth0 Dashboard］>［Tenant Settings（テナント設定）］>［Custom Domains（カスタムドメイン）］](%24%7Bmanage_url%7D/#/tenant/custom_domains)に移動して、DNSサーバーにTXTレコードを追加し、**［Verify（検証）］**をクリックしてドメインの所有権を確認します。
