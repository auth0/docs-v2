---
description: 一貫したユーザー体験を提供するために、テナントでデフォルトのカスタムドメインを設定する方法を紹介します。
...
## デフォルトのカスタムドメインとは何ですか？ {#what-is-a-default-custom-domain}
...
## デフォルトドメインを設定するメリット {#benefits-of-setting-a-default-domain}
...
## デフォルトドメインを設定する {#configure-a-default-domain}
...
## デフォルトドメイン指定を解除する {#remove-default-domain-designation}
...
## デフォルトドメインの使われ方 {#how-the-default-domain-is-used}
...
### 通知を発生させる Management API エンドポイント {#management-api-endpoints-that-trigger-notifications}
...
<AuthCodeGroup>
  ```bash Auth0 CLI カスタムドメインヘッダーなし（デフォルトを使用）
  ...
  ```bash Auth0 CLI カスタムドメインヘッダーあり（デフォルトを上書き）
  auth0 api post "tickets/password-change" \
    --data '{
      "user_id": "auth0|123456",
      "result_url": "https://example.com/password-changed"
...
## Actionsでデフォルトドメインを使用する {#using-the-default-domain-with-actions}
