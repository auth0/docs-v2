...
[Auth0 Actions](/docs/ja-jp/customize/actions) を使用すると、カスタムコードで認証をカスタマイズできます。[複数のカスタムドメイン](/docs/ja-jp/customize/custom-domains/multiple-custom-domains) を使用している場合は、Actions でカスタムドメインの情報にアクセスして、ドメイン固有のロジックを実装したり、ユーザーを組織に振り分けたり、アクセスポリシーを適用したり、通知をカスタマイズしたりできます。

## カスタムドメイン情報へのアクセス {#accessing-custom-domain-information}

Actions では、`event.custom_domain` オブジェクトを通じてカスタムドメインの情報にアクセスできます。このオブジェクトには、ドメイン名と、そのドメインに設定されたメタデータが含まれます。

### 利用可能なトリガー {#available-triggers}

`event.custom_domain` オブジェクトは、以下の Action トリガーで利用できます。

| トリガー                          | イベントオブジェクト の参照                                                                                               |
| ----------------------------- | -------------------------------------------------------------------------------------------------------------- |
| Credentials Exchange          | [イベントオブジェクト](/docs/ja-jp/actions/reference/credentials-exchange/credentials-exchange-event-object)                   |
| カスタムメールプロバイダー                 | [イベントオブジェクト](/docs/ja-jp/actions/reference/custom-email-provider/custom-email-provider-event-object)                 |
| カスタム電話プロバイダー                  | [イベントオブジェクト](/docs/ja-jp/actions/reference/custom-phone-provider/custom-phone-provider-event-object)                 |
| Post Login                    | [イベントオブジェクト](/docs/ja-jp/actions/reference/post-login/post-login-event-object)                                       |
| Post User Registration        | [イベントオブジェクト](/docs/ja-jp/actions/reference/post-user-registration/post-user-registration-event-object)               |
| Post Password Change          | [イベントオブジェクト](/docs/ja-jp/actions/reference/post-change-password/post-change-password-event-object)                   |
| Pre User Registration         | [イベントオブジェクト](/docs/ja-jp/actions/reference/pre-user-registration/pre-user-registration-event-object)                 |
| Reset Password Post Challenge | [イベントオブジェクト](/docs/ja-jp/actions/reference/password-reset-post-challenge/password-reset-post-challenge-event-object) |
...
### ドメインごとの条件分岐 {#conditional-logic-by-domain}
...
### メールドメインに基づくアクセス制御 {#email-domain-based-access-control}
...
### アプリケーション メタデータに基づくアクセス制御 {#application-metadata-based-access-control}
...
### 接続メタデータに基づくアクセス制御 {#connection-metadata-based-access-control}
...
### 組織のメタデータに基づくアクセス制御 {#organization-metadata-based-access-control}
...
### リージョン別のリクエスト {#region-based-requests}
...
### Actions のドキュメント {#actions-documentation}

* [Auth0 Actions の概要](/docs/ja-jp/customize/actions)
* [Action トリガーを確認する](/docs/ja-jp/customize/actions/explore-triggers)
* [Actions の制限事項](/docs/ja-jp/customize/actions/limitations)

### イベントオブジェクトのリファレンス {#event-object-references}
...
* [Organizations](/docs/ja-jp/manage-users/organizations)
* [カスタムメールプロバイダー](/docs/ja-jp/customize/email/smtp-email-providers/custom)