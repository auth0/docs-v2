...
## カスタムドメインでのパスキーの仕組み {#how-passkeys-work-with-custom-domains}

### WebAuthn Relying Party ID (RP ID) {#webauthn-relying-party-id-rp-id}
...
### ドメインごとの登録 {#per-domain-enrollment}
...
## パスキーのユーザーエクスペリエンスを理解する {#understanding-the-passkey-user-experience}

### 単一ブランド、単一ドメイン {#single-brand-single-domain}
...
### マルチブランド、別々のドメイン {#multi-brand-separate-domains}
...
### 共通ドメインを使うマルチテナント {#multi-tenant-with-common-domain}
...
### テナントでパスキーを有効にする {#enable-passkeys-for-your-tenant}
...
### パスキー用のカスタムドメインを設定する {#configure-custom-domains-for-passkeys}
...
### RP ID の設定を確認する {#verify-rp-id-configuration}
...
### ドメインごとにパスキー登録を促す {#prompt-for-passkey-enrollment-per-domain}
...
### ドメインごとのパスキー登録を管理する {#track-passkey-enrollment-by-domain}
...
### ドメインごとの登録ページ {#domain-specific-enrollment-pages}
...
### コンテキストに応じた登録プロンプト {#contextual-enrollment-prompts}
...
## ユーザーとのコミュニケーション {#user-communication}

### ドメインごとの登録についてユーザーに伝える {#inform-users-about-per-domain-enrollment}
...
## 制限事項と留意点 {#limitations-and-considerations}
...
### 関連オリジン (今後提供予定の機能) {#related-origins-future-feature}
...
## 移行パターン {#migration-scenarios}

### 単一のカスタムドメインから複数のカスタムドメインへの移行 {#migrating-from-single-custom-domain-to-multiple}
...
### カスタムドメイン間の移行 {#migrating-between-custom-domains}
...
### ドメインごとのパスキー登録をテストする {#test-passkey-enrollment-per-domain}
...
### カスタムドメインでPasskeyが機能しない {#passkey-not-working-on-custom-domain}
...
### 複数の登録に関してユーザーが混乱している {#user-confused-about-multiple-enrollments}
