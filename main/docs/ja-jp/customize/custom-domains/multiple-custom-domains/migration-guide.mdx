...
## 移行前チェックリスト {#pre-migration-checklist}
...
### 新しいカスタムドメインを追加する {#add-your-new-custom-domains}
...
### ドメインの所有権を確認する {#verify-domain-ownership}

新しいカスタムドメインごとに、ドメイン所有権の確認手続きを完了します。

#### Auth0 管理の証明書を使用する場合 {#for-auth0-managed-certificates}
...
#### 自己管理の証明書を使用する場合 {#for-self-managed-certificates}
...
### デフォルトドメインを設定する (任意) {#configure-default-domain-optional}

メールとAPI呼び出しで1つのドメインをデフォルトで使用する場合は、[そのドメインをデフォルトドメインとして設定します](/docs/ja-jp/customize/custom-domains/multiple-custom-domains/default-domain):
...
### アプリケーション設定を更新する {#update-application-configurations}
...
### メールテンプレートを更新する {#update-email-templates}
...
### ソーシャルIDプロバイダー (IdP) を更新する {#update-social-identity-providers-idp}
...
#### その他のプロバイダー {#other-providers}

設定済みのすべてのソーシャルプロバイダーについて、各プロバイダーのドキュメントに従ってリダイレクトURIを更新します。

### エンタープライズ接続を更新する (必要に応じて) {#update-enterprise-connections-if-applicable}
...
#### Azure AD 接続 {#azure-ad-connections}
...
#### ADFS 接続 {#adfs-connections}

ADFS の設定でエンドポイントを更新し、新しいカスタムドメインを使用してください。

### 認証をテストする {#test-authentication}
...
### 古いドメインの廃止 (該当する場合) {#decommission-old-domain-if-applicable}
...
## 既存のユーザーセッションへの対応 {#handling-existing-user-sessions}

カスタムドメインの移行時には、既存のユーザーセッションに影響が生じる場合があります。

### セッションに関する注意点 {#session-considerations}
...
## 移行後のベストプラクティス {#post-migration-best-practices}
