...
| 値 | 説明 |
| --- | --- |
| `MGMT_API_ACCESS_TOKEN` | `create:hooks`のスコープを持つ[Management APIのアクセストークン](/docs/ja-jp/api/management/v2/tokens)。 |
| `HOOK_ID` | シークレットを追加したいフックのID。 |
| `HOOK_SECRET_KEY` | 指定のフックに追加したいシークレットの名前。このエンドポイントはキー値ペアのオブジェクトを受け入れます。 |
| `HOOK_SECRET_VALUE` | 指定のフックに追加したいシークレットの値。このエンドポイントはキー値ペアのオブジェクトを受け入れます。 |