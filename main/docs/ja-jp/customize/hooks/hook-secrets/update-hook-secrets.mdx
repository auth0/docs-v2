...
| 値 | 説明 |
| --- | --- |
| `MGMT_API_ACCESS_TOKEN` | `update:hooks`スコープのあるManagement APIのアクセストークンです。詳細については、「[Management APIのアクセストークン](/docs/ja-jp/api/management/v2/tokens)」をお読みください。 |
| `HOOK_ID` | シークレットを更新するフックのIDです。 |
| `HOOK_SECRET_KEY` | 指定したフックで更新したいシークレットの名前です。このエンドポイントは、鍵と値のペアを受け付けます。 |
| `HOOK_SECRET_VALUE` | 指定したフックで更新したいシークレットの値です。このエンドポイントは、鍵と値のペアを受け付けます。 |
