...
| 値 | 説明 |
| --- | --- |
| `HOOK_ID` | シークレットを削除したいフックのID。 |
| `MGMT_API_ACCESS_TOKEN` | `update:hooks`のスコープを持つManagement APIのアクセストークン。詳細については、「[Management APIのアクセストークン](/docs/ja-jp/api/management/v2/tokens)」をお読みください。 |
| `HOOK_SECRET_NAME` | 指定のフックから削除したいシークレットの名前。このエンドポイントは削除するシークレット名の配列を受け入れます。 |