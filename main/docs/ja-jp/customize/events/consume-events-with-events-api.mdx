...
## Events API の仕組み {#how-the-events-api-works}
...
## Events API に接続する {#connect-to-the-events-api}

テナントの events エンドポイントへの SSE 接続を確立します。次の例では `curl` を使用します。

<Tabs>
  <Tab title="Auth0 CLI">
    <Callout icon="file-lines" color="#0EA5E9" iconType="regular">Auth0 CLI を使用する場合は、まだであればこのコマンドを実行する前に[CLI セッションをセットアップして認証してください](/docs/ja-jp/deploy-monitor/auth0-cli)。</Callout>

    ```bash
    auth0 api get "events"
    ```
  </Tab>
...
<Tabs>
  <Tab title="Auth0 CLI">
    ```bash
    auth0 api get "events" \
      -q "event_type=user.created" \
      -q "event_type=user.updated" \
      -q "from_timestamp=2025-06-01T00:00:00Z"
    ```
  </Tab>
...
## 切断後の再開 {#resume-after-a-disconnection}
...
* **`from` クエリパラメータ** — クライアントが `Last-Event-ID` ヘッダーをサポートしていない場合は、こちらを使用します。
...
## サーバー側での接続の切り替え {#server-side-connection-cycling}
...
## コンシューマーを実装する {#implement-a-consumer}
