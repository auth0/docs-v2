---
description: Auth0イベントストリームをテストし、Deliveriesエンドポイントをポーリングして配信の失敗を監視し、単一呼び出しおよび一括Redelivery APIで未配信のイベントを復旧して、イベントストリームの信頼性を維持します。
title: イベントのテスト、可観測性、障害復旧
---

以下のセクションでは、イベントストリームのテストとイベントの管理について説明します。

## イベントストリームをテストする {#test-the-event-stream}
...
### Deliveries エンドポイントのポーリング {#polling-deliveries-endpoint}
...
イベント配信は自動的に再試行されます。詳しくは、[Best Practices](/docs/ja-jp/customize/events/event-testing-observability-and-failure-recovery#batch-redelivery-of-failed-events)をご覧ください。

障害が発生した場合は、Redelivery API を使用してイベントを再配信してください。

### 失敗したイベントを 1 件再配信する {#redeliver-a-single-failed-event}
...
### 失敗したイベントの一括再配信 {#batch-redelivery-of-failed-events}
...
### 自動無効化の仕組み {#auto-disable-mechanism}
