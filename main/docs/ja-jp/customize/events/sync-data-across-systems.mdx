...
## アイデンティティデータを同期する理由 {#why-synchronize-identity-data}
...
* Events が有効な Auth0 テナント。利用可能なプランについて詳しくは、[Create an Event Stream](/docs/ja-jp/customize/events/create-an-event-stream) を参照してください。
* `user.created`、`user.updated`、`user.deleted` をサブスクライブしている有効な Event Stream。詳しくは、[Create an Event Stream](/docs/ja-jp/customize/events/create-an-event-stream) を参照してください。
* ハンドラーから書き込み可能な外部データストア (例: PostgreSQL、MySQL、データウェアハウス) 。

## Implement でデータ同期を行う {#implement-data-synchronization}
...
### user.created を処理する {#handle-usercreated}
...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  イベントは順番どおりに届かないことがあります。古いデータで新しいレコードが上書きされるのを防ぐため、更新を適用する前に必ずタイムスタンプを比較してください。詳しくは、[Events Best Practices](/docs/ja-jp/customize/events/events-best-practices)をご覧ください。
</Callout>
...
### タイプ別にイベントをルーティングする {#route-events-by-type}
...
<Tabs>
  <Tab title="Webhook">
    ...
    <Callout icon="file-lines" color="#0EA5E9" iconType="regular">
      HTTP `2XX` レスポンスは、できるだけ早く返してください。ハンドラーで時間のかかる処理を行う必要がある場合は、イベントを内部キューに入れて非同期で処理してください。詳しくは、[Events Best Practices](/docs/ja-jp/customize/events/events-best-practices) を参照してください。
    </Callout>
  </Tab>
...
## 重複や順序の問題に備える {#guard-against-duplicates-and-ordering-issues}
...
## 同期を確認する {#verify-synchronization}
