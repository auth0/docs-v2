...
実験がアクティブでバリエーションが割り当てられると、Experiment Center は [ページテンプレート](/docs/ja-jp/customize/login-pages/universal-login/customize-templates#customize-universal-login-page-templates) のレンダリング コンテキストに実験のコンテキストを自動的に注入します。ページテンプレートではオプトインの手順は不要です。実験がアクティブな場合、実験のコンテキストはすべてのテンプレートに無条件で渡されます。各テンプレートは、このコンテキストを `experiment` という名前のローカル変数として受け取ります。

## 利用可能なコンテキストフィールド {#available-context-fields}
...
### `experiment.config` の構造 {#experimentconfig-structure}
...
## テンプレートでパラメータを読み取る {#read-a-parameter-in-a-template}
...
## Partials を使用してサインアップとログインをカスタマイズする {#customize-signup-and-login-with-partials}
...
## 例: サインアップ見出し文言のバリエーション {#example-signup-headline-copy-variant}
...
## 例: 条件付きUI要素 {#example-conditional-ui-element}
...
## アクティブな実験がない場合の安全なデフォルト値 {#safe-defaults-when-no-experiment-is-active}
