...
| --- | --- | --- |
| `state` | Renders the current page's state value, which is opaque and used for security purposes.<br/><br/>To learn more about current screen information, review [Customize Univeral Login Page Templates](/docs/ja-jp/customize/login-pages/universal-login/customize-templates#current-screen-information). | `<input type='hidden' name='state' value='{{state}}'>` |
