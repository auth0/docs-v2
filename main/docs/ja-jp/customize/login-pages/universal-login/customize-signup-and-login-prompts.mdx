---
description: Auth0 Universal Login の Signup プロンプト と Login プロンプトにカスタムフィールドとテキストを追加する方法を説明します。 
sidebarTitle: 概要
title: Signup プロンプト と Login プロンプトをカスタマイズする
---
...
## Auth0 DashboardでPartialsを管理する {#use-the-auth0-dashboard-to-manage-partials}
...
## Partials をプログラムで管理する {#manage-partials-programmatically}
...
## Form 入力のスタイル設定とバリデーション {#style-and-validate-form-inputs}
...
### クライアントサイドのバリデーション {#client-side-validation}
...
## コンテンツをローカライズする {#localize-content}
...
### カスタムテキスト変数を作成または更新する {#create-or-update-a-custom-text-variable}
...
### Partial でカスタムテキスト変数を使用する {#use-a-custom-text-variable-in-a-partial}
...
## 取得したデータをバリデーションして保存する {#validate-and-save-captured-data}
...
### ユーザーのメタデータに保存 {#save-to-user-metadata}
