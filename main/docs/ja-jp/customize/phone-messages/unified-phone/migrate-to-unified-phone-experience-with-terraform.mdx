...
## 電話プロバイダーリソースを有効にし、レガシー設定を削除する {#enable-the-phone-provider-resource-and-remove-legacy-configuration}
...
### 統合エクスペリエンス フラグを設定する {#set-the-consolidated-experience-flag}
...
### テナントレベルの 電話プロバイダー を追加する {#add-the-tenant-level-phone-provider}
...
### auth0_branding_phone_notification_template リソースを追加する (任意) {#add-the-auth0_branding_phone_notification_template-resource-optional}
...
### Auth0 Guardian リソースの phone ブロックを整理する {#clean-up-the-auth0-guardian-resource-phone-block}
...
### terraform apply 中の 403 Forbidden エラー {#403-forbidden-errors-during-terraform-apply}
...
### 409 Conflict エラー: Message Type は設定された電話プロバイダーでサポートされていません {#409-conflict-error-message-type-is-not-supported-by-the-configured-phone-provider}
...
### Terraform apply エラー: Provider または options は指定できません {#terraform-apply-error-provider-or-options-cannot-be-specified}
...
### カスタム電話プロバイダーでメッセージが配信されない {#custom-phone-provider-not-delivering-messages}
...
### 移行後にOTPメッセージが届かない {#otp-messages-not-arriving-after-migration}
