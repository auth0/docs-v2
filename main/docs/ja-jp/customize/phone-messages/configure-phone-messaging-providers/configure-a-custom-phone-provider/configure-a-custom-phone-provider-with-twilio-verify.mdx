...
## Twilio Verifyサービスは[Twilio Console]()または[Verify REST API]()通して作成できます。Auth0のカスタム電話プロバイダーをTwilio Verifyと接続するには以下が必要です。
...
exports.onExecuteCustomPhoneProvider = async (event, api) => {
 const { TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN, **［Name（名前）］：**Twilio } = event.secrets;
