...
* [拡張機能へのAPIアクセスを有効にする](/docs/ja-jp/customize/extensions/authorization-extension/enable-api-access-to-authorization-extension): Authorization Extension のAPIエンドポイントとその使い方について説明します。
...
## 旧バージョンからのアップグレード {#upgrade-from-previous-versions}

### バージョン 2.6 以降からのアップグレード {#upgrade-from-version-26-or-later}

バージョン 2.6 以降からのアップグレードでは、互換性のない変更はなく、特別な対応も必要ありません。

### 2.6 より前のバージョンからアップグレードする {#upgrade-from-versions-before-26}
...
#### 拡張機能のAPIキーをローテーションします {#rotate-the-extensions-api-key}

1. **Auth0 Authorization** を選択して、拡張機能を開きます。
2. 拡張機能ダッシュボードの右上にあるドロップダウンメニューから、**Configuration** を選択します。
3. **API Key** セクションで **Rotate** を選択します。

#### 拡張機能のルールを再度公開する {#republish-the-extensions-rule}

**Publish Rule** を選択します。

#### 古い拡張機能ルールが存在する場合は削除する {#delete-the-old-extension-rule-if-it-exists}
