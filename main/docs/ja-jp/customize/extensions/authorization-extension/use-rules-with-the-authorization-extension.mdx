...
## 発行されたトークンにカスタムクレームを追加する {#add-custom-claims-to-the-issued-token}
...
## アプリケーションへのアクセスを制御する {#control-app-access}
...
### アプリのメタデータに必須ロールを設定する {#set-app-metadata-required-roles}
...
### アプリのロールを強制するルールを作成する {#create-rule-enforcing-app-roles}
