...
<Warning>
  カスタムフィールドを使用するには、[Custom Domains](/docs/ja-jp/customize/custom-domains) を有効にする必要があります。カスタムドメイン外でカスタムフィールドを含むフォームをレンダリングすると、エラーが表示されます。
</Warning>
...
カスタムフィールドのソースコード内で参照するキーと値のペアを追加します。キーと値のペアには、フォームフィールドの[変数](/docs/ja-jp/customize/forms/variables)を含めることができます。
...
複雑なバリデーション要件に対応するには、[flow](/docs/ja-jp/customize/forms/flows) を使用できます。
...
## カスタムフィールドハンドラー {#custom-field-handlers}
...
## カスタムフィールドの例 {#custom-field-examples}

以下のセクションでは、フォームに追加できるカスタムフィールドの例を紹介しています。

### 範囲入力カスタムフィールド {#range-input-custom-field}
...
### カラー入力のカスタムフィールド {#color-input-custom-field}
...
### API の値を使ったオートコンプリート入力のカスタムフィールド {#autocomplete-input-custom-field-using-values-from-api}
...
### APIの値を使用する動的なドロップダウンのカスタムフィールド {#dynamic-dropdown-custom-field-using-values-from-api}
...
### &#x20;(+) ボタンで項目を追加できる動的入力カスタムフィールド {#dynamic-input-custom-field-with-a-button-to-add-more-fields}
