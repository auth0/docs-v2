---
description: post-login または pre-user-registration Action を使用して、ログインまたはサインアップフロー中に Form をレンダリングする方法を説明します。
...
## 既存フィールドと非表示フィールドに値を設定する (クライアント側) {#populate-values-for-existing-fields-and-hidden-fields-client-side}
...
## 共有変数を使ってカスタムデータを渡す (サーバー側) {#inject-custom-data-with-shared-variables-server-side}
...
## Actions におけるフィールドと共有変数のデータ {#fields-and-shared-variables-data-in-actions}
...
## 制約事項と制限 {#restrictions-and-limitations}
