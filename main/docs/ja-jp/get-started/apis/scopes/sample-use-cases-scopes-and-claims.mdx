---
description: OAuth 2.0のスコープとJWTクレームが、ユーザーに代わってアプリケーションがアクセスできるデータや実行できる操作をどのように制御するかを、実践的な例を通じて学びます。
...
## ユーザーを認証して標準クレームをリクエストする {#authenticate-a-user-and-request-standard-claims}
...
## カスタム API へのアクセスをリクエストする {#request-custom-api-access}
...
## ユーザーを認証し、標準クレームとカスタム API へのアクセスをリクエストする {#authenticate-a-user-and-request-standard-claims-and-custom-api-access}
...
## トークンにカスタムクレームを追加する {#add-custom-claims-to-a-token}
