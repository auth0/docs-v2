---
description: Auth0 API でカスタムスコープを定義および使用して、アクセス制御を実装し、アプリケーション内の権限を適用します。
title: API スコープ
...
2. これらのアクセスレベルをカスタムスコープとして定義します。 (スコープについては、[スコープ](/docs/ja-jp/get-started/apis/scopes)を参照してください。)
3. 呼び出し元アプリケーションが利用できるように、これらのスコープを識別します。

## API スコープの使い方 {#ways-to-use-api-scopes}
...
アプリケーションのカスタム API アクセスを要求する方法を示す例については、[スコープとクレームのユースケース例](/docs/ja-jp/get-started/apis/scopes/sample-use-cases-scopes-and-claims) を参照してください。

### 例: サードパーティアプリケーションから呼び出される API {#example-an-api-called-by-a-third-party-application}
...
### 例: バックエンド サービスから呼び出される API {#example-an-api-called-by-a-back-end-service}
...
## API スコープを制限する {#limit-api-scopes}

アプリケーションは、API へのリクエストに、その API で定義されている任意のスコープを含めることができます。ただし、利用可能なすべてのスコープをリクエストできるようにするのではなく、[アプリケーション向け API アクセスポリシー](/docs/ja-jp/get-started/apis/api-access-policies-for-applications) を使用して、アプリケーションの API へのアクセス方法を制御できます。

## 詳しく見る {#learn-more}

* [スコープとクレームのユースケース例](/docs/ja-jp/get-started/apis/scopes/sample-use-cases-scopes-and-claims)
* [API のアクセス許可を追加する](/docs/ja-jp/get-started/apis/add-api-permissions)
...
* [テスト環境用の Management API アクセストークンを取得する](/docs/ja-jp/secure/tokens/access-tokens/management-api-access-tokens/get-management-api-access-tokens-for-testing)