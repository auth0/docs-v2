...
| 値 | 説明 |
| --- | --- |
| `API_ID` | アクセス許可を削除したいAPIのID。 |
| `MGMT_API_ACCESS_TOKEN` | `update:resource_servers`のスコープを持つ[Management APIのアクセストークン](https://auth0.com/docs/ja-jp/api/management/v2/tokens)。 |
| `PERMISSION_NAME` | 指定したAPIで保持したいアクセス許可の名前。 |
| `PERMISSION_DESC` | 指定したAPIで保持したいアクセス許可のわかりやすい説明。 |
