...
## Private Key JWT の設定 {#configure-private-key-jwt}
...
#### `private_key_jwt` 用の新しいアプリケーションを設定する {#configure-a-new-application-for-private_key_jwt}
...
#### 既存のアプリケーションを設定する {#configure-an-existing-application}
...
#### Client Secret 認証を使用するようにアプリケーションを設定する {#configure-an-application-to-use-client-secret-authentication}
...
#### 資格情報の有効期限を更新する {#update-credential-expiration}
...
#### `private_key_jwt` 用の新しいアプリケーションを設定する {#configure-a-new-application-for-private_key_jwt-2}
    ...
    レスポンスには、アプリケーションをリソースサーバーにリンクする `client_id` プロパティが含まれています。レスポンスにはまた、作成した認証情報のために生成された `kid` が含まれており、後で `client_assertion` を生成する際に使用されます。

    <Callout icon="file-lines" color="#0EA5E9" iconType="regular">
      Auth0 は、資格情報の `kid` を生成する際に、[JSON Web Key Thumbprint](https://datatracker.ietf.org/doc/html/rfc7638) 標準に従います。

      `kid` は、公開キーの JWK 表現の SHA256 ダイジェストを base64URL エンコードしたものです。
    </Callout>

#### 既存のアプリケーションを設定する {#configure-an-existing-application-2}
...
##### 認証情報リソースを作成する {#create-the-credential-resource}

    キーペアを生成したら、クレデンシャルリソースを作成します。Management APIの[`/clients`](https://auth0.com/docs/api/management/v2#!/Clients/post_clients)エンドポイントに対して、次のPOSTリクエストを行います。
    ...
    | `managementApiAccessToken` | [Management API 用のアクセストークン](/docs/ja-jp/secure/tokens/access-tokens/management-api-access-tokens) (スコープ `create:credentials` が付与されたもの) 。                       |
    | `pem`                      | PEM 形式でエンコードされた公開キー、または X.509 証明書。                                                                                                                       |
    | `expires_at`               | 任意。資格情報の有効期限を ISO 8601 形式で指定します。たとえば、`2020-08-20T19:10:06.299Z` です。有効期限を過ぎると、その資格情報は無効になります。                                                             |
    | `parse_expiry_from_cert`   | 省略可能。証明書が指定された場合に、Auth0 がその有効期限を解析することを示すブール値です。証明書が指定されていない場合、Auth0 はエラーを返します。`parse_expiry_from_cert` と `expires_at` は同時に指定できません。この場合、Auth0 はエラーを返します。 |
...
##### 認証情報を関連付ける {#associate-the-credential}
...
#### Client Secret 認証を使用するようにアプリケーションを設定する {#configure-an-application-to-use-client-secret-authentication-2}
    ...
    <Warning>
      認証方法を `client_secret` に設定すると、各アプリケーションを `client_secret` を使用して認証するよう更新するまで、`private_key_jwt` を使用した認証はできなくなります。
    </Warning>
    ...
    **例**
    ...
    | `managementApiAccessToken` | [Management API 用アクセストークン](/docs/ja-jp/secure/tokens/access-tokens/management-api-access-tokens) (スコープ `update:client` および `update:credentials` が付与されたもの) 。 |
    | `tokenEndpointAuthMethod`  | 最終的に使用する認証方法。たとえば、`client_secret_basic` または `client_secret_post`。                                                                                     |

#### 有効期限フィールドで資格情報をパッチする {#patch-credentials-with-an-expiration-field}
