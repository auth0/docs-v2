...
#### テナントでサポートするACRクレームを設定する {#configure-supported-acr-claims-for-the-tenant}
...
#### `scope` と `redirect_uri` を必須にする Action を追加する {#add-action-to-require-scope-and-redirect_uri}
...
#### &#x20;(FAPI2 プロファイルのみ) レスポンスで `iss` クレームを返すようにする {#fapi2-profiles-only-ensure-iss-claim-is-returned-in-responses}
