...
[`/Clients/delete_clients_by_id`](/docs/ja-jp/api/management/v2#!/Clients/delete_clients_by_id)エンドポイントに`DELETE`呼び出しを行います。必ず、`{yourClientId}`と`MGMT_API_ACCESS_TOKEN`のプレースホルダーの値を、それぞれご自身のクライアントIDと<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" href="/docs/ja-jp/glossary?term=management-api" tip="Management API: 顧客が管理タスクを実行できるようにするための製品。" cta="用語集の表示">Management API</Tooltip>のアクセストークンに置き換えてください。
...
| 値 | 説明 |
| --- | --- |
| `{yourClientId}` | 削除されるアプリケーションのID。 |
| `MGMT_API_ACCESS_TOKEN` | `delete:clients`スコープのある[Management APIの[アクセストークン](https://auth0.com/docs/ja-jp/api/management/v2/tokens)。 |
