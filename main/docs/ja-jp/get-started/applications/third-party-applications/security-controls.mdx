...
## OAuth 2.1 標準 {#oauth-21-standards}
...
## 明示的なAPI認可 {#explicit-api-authorization}
...
## マシン間 (クライアント資格情報) {#machine-to-machine-client-credentials}
...
## 制限付きクライアント設定 {#restricted-client-configuration}
...
## Client ID の形式 {#client-id-format}
...
## リフレッシュトークンの設定 {#refresh-token-settings}
...
## `/authorize` パラメーターの検証 {#authorize-parameter-validation}
