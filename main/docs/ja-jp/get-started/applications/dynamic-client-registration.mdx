...
## Dynamic Client Registration を有効にする {#enable-dynamic-client-registration}
...
## DCR クライアントの API アクセスを設定する {#configure-api-access-for-dcr-clients}
...
## アプリケーションを登録する {#register-an-application}
...
## テナントアクセス制御リスト (ACL) {#tenant-access-control-list-acl}
...
DCR の ACL ルールを設定するには、ACL ルールに `dynamic_client_registration` スコープを追加します。詳しくは、[テナント ACL Reference](/docs/ja-jp/secure/tenant-access-control-list/reference) をご覧ください。
...
## DCR の許容モード {#permissive-mode-for-dcr}
