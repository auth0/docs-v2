...
For ease of configuration and maximized security isolation, define アプリケーション in Auth0 independently. Doing so allows items like allowed callback URLs to be configured independently and, following the principle of least privilege, minimizes the potential exposure of any Client ID and Client Secret information.
...
For users that are authenticated via 接続 other than Database or Custom Database Connections, the user is provisioned to the external <Tooltip tip="Identity Provider (IdP): Service that stores and manages digital identities." cta="View Glossary" href="/docs/glossary?term=Identity+Provider">Identity Provider</Tooltip> (IdP) independent of Auth0 and in the normal way. On the other hand, users that are authenticated via Database or Custom Database 接続 can be provisioned in a number of different ways. The Auth0 Dashboard and Auth0 Management API can be used to create a user directly in your Auth0 Tenant. We also support [Automatic Migration](/docs/manage-users/user-migration/configure-automatic-migration-from-your-database) and [Bulk Migration](/docs/manage-users/user-migration/bulk-user-imports).

<Warning>

For a user to be provisioned via the Management API or the Dashboard, you must directly enable a Database or Custom データベース接続 for at least one Application. Enabling a Database or Custom データベース接続 for an Organization only will not suffice.

</Warning>

ユーザー are then associated with an Auth0 Organization by assigning them memberships, and an Auth0 Organization can be configured to [assign user Membership automatically](/docs/manage-users/organizations/configure-organizations/grant-just-in-time-membership) or [on a manual basis](/docs/manage-users/organizations/configure-organizations/assign-members).
...
<Frame>![Architecture Scenarios - MOA - Isolated Users, Shared Apps, Invitation（招待） Flow (Database Connection)](/images/cdy7uua7fh8z/2QMwMeBQ6U9TjLtbjPgNwe/1b436cd226b95355e73a93d5f9bc0be3/Isolated_Users__Shared_Apps__Invitation_Flow__Database_Connection_.png)</Frame>
...
<Frame>![Architecture Scenarios - MOA - Isolated Users, Shared Apps, Invitation（招待） Flow (Enterprise Connection)](/images/cdy7uua7fh8z/1IsdaHpprvIp17R5zYIg7q/c2c73360fb21657ab2ae98c0981b1cc6/Isolated_Users__Shared_Apps__Invitation_Flow__Enterprise_Connection_.png)</Frame>
