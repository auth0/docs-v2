...
次に、ユーザーの認証に使われる[［Connections（接続）］](/docs/ja-jp/connections)を定義します。この例では、Hoekstra & Associatesの関連ユーザーのために[［Database Connection（データベース接続）］](/docs/ja-jp/authenticate/database-connections)、MetaHexa Bankの関連ユーザーのためには[［Enterprise Connection（エンタープライズ接続）］](/docs/ja-jp/connections/identity-providers-enterprise) を定義します。
