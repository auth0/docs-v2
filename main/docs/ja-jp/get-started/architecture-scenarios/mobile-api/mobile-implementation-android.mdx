...
## ステップ1. アプリケーションを設定する {#step-1-set-up-the-application}
...
### 依存関係を設定する {#set-the-dependencies}
...
* [JWTDecode.Android](https://github.com/auth0/JWTDecode.Android): このパッケージは、<Tooltip tip="JSON Web Token (JWT): 2者間でクレームを安全に表現するために使われる標準的な ID トークン形式（多くの場合、アクセストークン形式としても使われます）。" cta="用語集を見る" href="/docs/ja-jp/glossary?term=JWTs">JWT</Tooltip>のデコードに役立ちます。
...
### マニフェストを更新する {#update-the-manifest}
...
### 設定値を指定する {#set-configuration-values}

`/res/values/strings.xml` にある `strings.xml` リソースで、Auth0 の <Tooltip tip="Client ID: Auth0 から登録済みリソースに付与される識別値。" cta="用語集を見る" href="/docs/ja-jp/glossary?term=Client+ID">Client ID</Tooltip>、Auth0 ドメイン、API の URL を設定します。
...
## ステップ 2. ユーザーを認可する {#step-2-authorize-the-user}

### マニフェストを更新する {#update-the-manifest-2}
...
### Login Activity を作成する {#create-the-login-activity}

`LoginActivity` ではユーザーの認可を処理し、ユーザーに最初に表示される画面として使用します。`WebAuthProvider` を初期化して認証を開始する `login()` メソッドを作成します。`WebAuthProvider` には、正しい scheme、<Tooltip tip="Audience: 発行されたトークンの対象者を一意に識別する識別子です。トークンでは aud という名前で表され、その値には ID Token の場合はアプリケーション（Client ID）、Access Token の場合は API（API Identifier）の ID が含まれます。" cta="用語集を見る" href="/docs/ja-jp/glossary?term=audience">audience</Tooltip>、および scope を必ず指定してください。この実装では、次を使用します。

* **scheme**: `demo`
* **audience**: `https://api.exampleco.com/timesheet`s (Node.JS API)
* **response&#95;type**: `code`
* **scope**: `create:timesheets read:timesheets openid profile email offline_access`。これらのスコープにより、Node.JS API に対して `POST` と `GET` を実行できるほか、ユーザープロフィールと <Tooltip tip="Refresh Token: ユーザーに再度ログインを求めることなく、新しい Access Token を取得するために使用されるトークンです。" cta="用語集を見る" href="/docs/ja-jp/glossary?term=Refresh+Token">Refresh Token</Tooltip> も取得できます。
...
### 資格情報を保存する {#store-the-credentials}
...
`login()` メソッドで `WebAuthProvider` を初期化する前に、`CredentialsManager` を作成しておけます。`AuthenticationAPIClient` を `CredentialsManager` に渡すと、有効期限が切れた <Tooltip tip="アクセストークン: API へのアクセスに使用される、opaque string または JWT の形式の認可資格情報。" cta="用語集を見る" href="/docs/ja-jp/glossary?term=Access+Tokens">アクセストークン</Tooltip> を更新できるようになります。
...
## ステップ 3. ユーザープロフィールの取得 {#step-3-get-the-user-profile}

### User モデルを作成する {#create-the-user-model}
...
### ユーザープロファイルを保存する {#store-the-user-profile}
...
次に、`LoginActivity` の `login()` メソッドを更新して、<Tooltip tip="ID Token: リソースへのアクセス用ではなく、クライアント自身のための認証情報です。" cta="用語集を見る" href="/docs/ja-jp/glossary?term=ID+Token">ID Token</Tooltip> を取得し、JWTDecode.Android ライブラリを使ってトークンからユーザープロファイルを取得します。続いて、そのユーザープロファイルを `UserProfileManager` に保存します。
...
## ステップ 4. スコープに基づいて UI 要素を条件付きで表示する {#step-4-display-ui-elements-conditionally-based-on-scope}
...
### スコープに基づいて承認メニューを表示する {#display-approval-menu-based-on-scope}
...
## ステップ 5：API を呼び出す {#step-5-call-the-api}

### マニフェストを更新する {#update-the-manifest-3}
...
### Timesheets Activity のレイアウトを作成する {#create-the-timesheets-activity-layouts}
...
### タイムシートモデルを作成する {#create-the-timesheet-model}
...
### タイムシートアダプターを作成する {#create-the-timesheet-adapter}
...
### Timesheet Activity を作成する {#create-the-timesheet-activity}
...
## ステップ 6. ユーザープロフィールを表示する {#step-6-view-the-user-profile}
...
### マニフェストを更新する {#update-the-manifest-4}
...
#### User Activity のレイアウトを作成する {#create-the-user-activity-layouts}
...
### URLからプロフィール画像を読み込む {#load-the-profile-picture-from-url}
...
### User Activity を作成する {#create-the-user-activity}
...
## ステップ 7. 新しいタイムシート用フォーム {#step-7-form-for-new-timesheets}

次に、新しいタイムシートのエントリを作成するための `FormActivity` とレイアウトを作成します。

### マニフェストを更新する {#update-the-manifest-5}
...
### Form Activity のレイアウトを作成する {#create-the-form-activity-layouts}
...
### Form Activity を作成する {#create-the-form-activity}
...
## アプリをテストする {#test-the-app}

先に進む前に、[Node.JS API を実装](/docs/ja-jp/get-started/architecture-scenarios/mobile-api/api-implementation-nodejs)していることを確認してください。
