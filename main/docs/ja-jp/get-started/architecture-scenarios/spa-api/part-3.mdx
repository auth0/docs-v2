...
* **clientID** ：Auth0のクライアントIdの値。これは[Dashboard](https://manage.auth0.com/#/applications})にある［Application（アプリケーション）］の［Settings（設定）］で取得できます。
* **domain** ：Auth0ドメインの値。これは[Dashboard](https://manage.auth0.com/#/applications})にある［Application（アプリケーション）］の［Settings（設定）］で取得できます。
