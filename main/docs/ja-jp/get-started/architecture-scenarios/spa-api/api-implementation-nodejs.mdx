...
# Node.js API の実装 (SPA + API) {#nodejs-api-implementation-spas-api}
...
## ステップ 1. API のエンドポイントを定義する {#step-1-define-the-api-endpoints}

Node.js API の構築には、[Express Webアプリケーションフレームワーク](http://expressjs.com/)を使用します。

### package.json ファイルを作成する {#create-a-packagejson-file}
...
### エンドポイントを実装する {#implement-the-endpoints}
...
## ステップ 2. API エンドポイントを保護する {#step-2-secure-the-api-endpoints}
...
## ステップ 3. アプリケーションの権限を確認する {#step-3-check-the-application-permissions}
...
## ステップ 4. ユーザーを識別する {#step-4-determine-the-user-identity}
