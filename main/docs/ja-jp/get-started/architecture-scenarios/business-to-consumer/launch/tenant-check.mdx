...
## 一般的なテナントの確認 {#general-tenant-check}

### テナント準備チェック {#tenant-preparation-check}
...
### テナントの関連付けの確認 {#tenant-association-check}
...
### 本番テナントを指定する {#specify-production-tenant}
...
### テナント本番チェック {#tenant-production-check}
...
### テナント設定の確認 {#tenant-settings-check}
...
#### レガシー機能フラグをオフにする {#legacy-feature-flags-off}
...
### カスタムドメイン名の設定 {#custom-domain-naming-set-up}
...
## アプリケーションと接続の設定の確認 {#application-and-connection-settings-check}
...
## ページのカスタマイズ確認 {#page-customization-check}
...
### パスワードリセットページ {#password-reset-page}
...
### API オフラインアクセス {#api-offline-access}
...
### アクセストークンの署名アルゴリズム {#access-token-signing-algorithm}
...
### API アクセストークンのバリデーション {#api-access-token-validation}
...
## メールテンプレートをカスタマイズ {#email-templates-customized}
...
## 攻撃対策の設定 {#attack-protection-configured}
