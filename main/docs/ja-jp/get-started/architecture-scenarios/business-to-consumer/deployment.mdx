---
title: B2C IAM のデプロイ自動化
sidebarTitle: デプロイ自動化
description: Deploy CLI をソース管理および環境固有の設定ファイルと併用して、B2C IAM 実装向けの Auth0 テナントのデプロイを自動化します。
---
...
<Warning>
  Deploy CLI Tool と source control extensions はどちらも破壊的な変更を引き起こす可能性があります。自動デプロイの合間に Dashboard で直接行った手動変更は失われる可能性があります。そのため、いずれかを使用する場合は、**すべての**変更をツール経由で参照される source control サブシステムからデプロイし、手動で変更しないようにする必要があります。
</Warning>
