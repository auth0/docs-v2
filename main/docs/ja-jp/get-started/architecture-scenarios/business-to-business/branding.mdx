...
## 組織ごとのログインのブランディング {#branding-login-by-organization}
...
組織のユーザー同士がすべて互いに分離されている場合は、Universal Login ページで、そのログインページがどの組織向けのものかを明確に示すことが重要です。これを行う最も簡単な方法は、[Organizations Feature](/docs/ja-jp/manage-users/organizations/create-first-organization) を使用することです。これにより、ロゴ、メインカラー、背景色を保存でき、アプリケーションから org&#95;id を渡すか、ユーザーが組織入力プロンプトで自分の組織を入力して組織が特定されると、それらを使って Universal Login ページのブランディングをカスタマイズできます。
...
## Password Reset ページのカスタマイズ {#password-reset-page-customization}
