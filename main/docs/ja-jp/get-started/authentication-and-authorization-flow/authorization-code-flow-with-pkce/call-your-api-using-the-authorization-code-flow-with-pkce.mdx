...
パラメーター must be stored securely since they allow a user to remain authenticated essentially forever.
...
サンプル実装の場合、[モバイル + API](/architecture-scenarios/application/mobile-api)アーキテクチャのシナリオを参照してください。この一連のチュートリアルには、付録に[GitHubで提供されているコードのサンプル]()があります。
