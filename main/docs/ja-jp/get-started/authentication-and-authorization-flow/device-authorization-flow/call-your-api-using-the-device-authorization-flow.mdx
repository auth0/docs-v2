...
#### デバイスコードURL への POST の例 {#example-post-to-device-code-url}
...
#### デバイスコードのレスポンス {#device-code-response}
...
### デバイスのアクティベーションをリクエストする {#request-device-activation}
...
#### token URL に token をリクエストする POST の例 {#example-request-token-post-to-token-url}
...
##### トークンリクエストのパラメータ {#token-request-parameters}
...
#### Token URL へのリフレッシュトークンの POST の例 {#example-refresh-token-post-to-token-url}
...
### Device Authorization Flow の使用を検出する {#detect-device-authorization-flow-use}
