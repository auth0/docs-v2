...
* [APIをAuth0に登録します](/docs/ja-jp/get-started/architecture-scenarios/mobile-api/part-2#create-the-api)。
...
<tr>
<td>`scope`</td>
<td>認可を要求する<a href="/docs/ja-jp/get-started/apis/scopes">スコープ</a>です。スペースで区切る必要があります。`profile`や`email`など、ユーザーに関する<a href="https://openid.net/specs/openid-connect-core-1_0.html#StandardClaims">標準OIDCスコープ</a>、<a href="/docs/ja-jp/secure/tokens/json-web-tokens/create-custom-claims">名前空間形式</a>に従った<a href="/docs/ja-jp/secure/tokens/json-web-tokens/json-web-token-claims#custom-claims">カスタムクレーム</a>、<a href="/docs/ja-jp/get-started/apis/scopes/api-scopes">ターゲットAPIがサポートするスコープ</a>（例：`read:contacts`）を要求できます。IDトークンの取得や、<a href="/docs/ja-jp/api/authentication#user-profile">/userinfoエンドポイント</a>を使ってユーザープロファイル情報を取得する際には、`openid`を含めます。リフレッシュトークンを取得するには`offline_access`を含めます（<a href="https://manage.auth0.com/#/apis">［API Settings（API設定）］</a>で__［Allow Offline Access（オフラインアクセスを許可する）］__が有効になっていることを確認します）。URLエンコードされている必要があります。</td>
</tr>
<tr>
<td>`audience`</td>
