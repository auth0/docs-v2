...
Auth0を構成し、[テナントメンバー](/manage-dashboard-access)に自身の[エンタープライズIDプロバイダー](/enterprise)（IdP）を使用して、シングルサインオン（SSO）を通じてAuth0 Dashboardに認証できるように構成できます。
...
[Auth0サポートを使用してチケットを開く](/support/open-and-manage-support-tickets)ことで、IdPの構成データを共有することができ、そうすることでSSO構成をセットアップすることができます。チケットを提出する際に以下の情報を含めてください。
...
2. IDトークンに[メールクレームを追加](/security/tokens/json-web-tokens/create-custom-claims)します。
...
エンタープライズテナントでAuth0 Teamsを使用する場合、Teamsアカウントに属するテナントに対してSSO認証を強制できます。テナント登録と管理についての詳細は、「[Auth0 Teams](/auth0-teams)」をお読みください。

1. 新しいブラウザーを開き、TeamsアカウントとIDを入力してください。`https://accounts.auth0.com/teams/{team-identifier}。
`
2. **［Security（セキュリティ）］**ページに移動します。
3. **［Enforce Single Sign On（シングルサインオンの強制）］**を選択することで、[セキュリティポリシーを構成](/auth0-teams/configure-security-policies)します。
...
1. テナントメンバーに対して、[新しいテナントメンバー招待状を作成します](/manage-dashboard-access/add-dashboard-users)。
...
HRDを有効にした状態でテナントメンバーを移行するには、[テナントメンバーの追加](/manage-dashboard-access/add-dashboard-users)に類似した手順を追う必要があります。
