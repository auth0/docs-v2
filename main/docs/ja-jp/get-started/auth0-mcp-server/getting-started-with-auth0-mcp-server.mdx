---
description: Auth0 Model Context Protocol (MCP) Server を AIクライアントと連携します。
title: Auth0 Model Context Protocol (MCP) Server の利用を始める
sidebarTitle: はじめに
---

import { ReleaseStageNotice } from "/snippets/ja-jp/ReleaseStageNotice.jsx"

<ReleaseStageNotice feature="Auth0 MCP Server" stage="beta" />

<Card title="始める前に">
  * システムに [Node.js v18 以降](https://nodejs.org/en/download) をインストールしておきます。
  * 管理者権限を持つ有効な [Auth0 アカウント](https://auth0.com/)を作成しておきます。
  * 少なくとも 1 つのサポート対象の MCP クライアント (たとえば [Claude Desktop](https://claude.ai/download)、[Cursor](https://www.cursor.com/downloads)、[Windsurf](https://www.windsurf.com/editor)、[VS Code](https://code.visualstudio.com/download)、[Claude Code](https://claude.ai/download)、または [Gemini CLI](https://github.com/google-gemini/gemini-cli)) をインストールしておきます。
</Card>

[Auth0 Model Context Protocol (MCP) Server](/docs/ja-jp/get-started/auth0-mcp-server) をインストールし、よく使われている複数の [MCP クライアント](https://modelcontextprotocol.io/clients) と連携することで、チームやユーザーの活用の幅を広げましょう。

## インストールと設定 {#installation-and-configuration}

ターミナルで次のいずれかのコマンドを実行して、Auth0 MCP Serverをインストールします。

### Claude Desktop の場合 (デフォルト) {#for-claude-desktop-default}

```bash lines
npx @auth0/auth0-mcp-server init
```

### Cursor の場合 {#for-cursor}

```bash lines
npx @auth0/auth0-mcp-server init --client cursor
```

### Windsurf向け {#for-windsurf}

```bash lines
npx @auth0/auth0-mcp-server init --client windsurf
```

### VS Code の場合 {#for-vs-code}

```bash lines
npx @auth0/auth0-mcp-server init --client vscode
```

設定スコープの選択を求められます。

* **Global**: すべての VS Code インスタンスで利用できます。
* **Workspace**: 特定のプロジェクトまたはリポジトリでのみ利用できます。

選択内容に応じて、コマンドが適切な `mcp.json` ファイルを自動的に設定します。

### Claude Code の場合 {#for-claude-code}

```bash lines
npx @auth0/auth0-mcp-server init --client claude-code
```

設定スコープを選択するよう求められます。

* **User**: `~/.claude.json` に書き込まれ、すべてのプロジェクトで利用できます。
* **Project**: 現在のプロジェクト内でのみ利用できます。

### Gemini CLI の場合 {#for-gemini-cli}

```bash lines
npx @auth0/auth0-mcp-server init --client gemini
```

### その他のMCPクライアント {#other-mcp-clients}

Auth0 MCP Serverを他のMCPクライアントで使用するには、この設定をクライアントに手動で追加し、変更を反映させるために再起動してください。

```json lines
{
  "mcpServers": {
    "auth0": {
      "command": "npx",
      "args": ["-y", "@auth0/auth0-mcp-server", "run"],
      "capabilities": ["tools"],
      "env": {
        "DEBUG": "auth0-mcp"
      }
    }
  }
}
```

## Auth0 で認証する {#authenticate-with-auth0}

<Tooltip tip="OAuth 2.0: 認可プロトコルとワークフローを定義する認可フレームワーク。" cta="用語集を見る" href="/docs/ja-jp/glossary?term=OAuth+2.0">OAuth 2.0</Tooltip> のデバイス<Tooltip tip="OAuth 2.0: 認可プロトコルとワークフローを定義する認可フレームワーク。" cta="用語集を見る" href="/docs/ja-jp/glossary?term=authorization+flow">認可フロー</Tooltip>を開始するため、ブラウザーが自動的に開きます。Auth0 アカウントにログインし、求められたアクセス許可を付与してください。

<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  資格情報はシステムのキーチェーンに安全に保存されます。必要に応じて、キーチェーン管理ツールで保存を確認できます。
</Callout>

## 連携を確認する {#verify-your-integration}

### Claude Desktop {#claude-desktop}

1. 新しくインストールした Auth0 MCP Server が確実に認識されるよう、Claude Desktop を再起動します。
2. Claude Desktop で **Tools** アイコンをクリックし、Auth0 のツール (たとえば `auth0_create_action` や `auth0_delete_application`) が **Available MCP Tools** の一覧に表示されていることを確認します。
3. Claude Desktop との会話を開始し、Auth0 テナントの管理についてサポートを求めます。

### Cursor {#cursor}

1. 新しくインストールした Auth0 MCP Server を Cursor が認識するよう、Cursor を再起動します。
2. Cursor で **設定 &gt; MCP** を開き、**MCP Servers** の一覧に **Auth0** が表示されていることを確認します。
3. Cursor との会話を開始し、Auth0 テナントの管理についてサポートを依頼します。

### Windsurf {#windsurf}

1. 新しくインストールした Auth0 MCP Server を Windsurf で認識させるため、Windsurf を再起動します。
2. Windsurf で **Windsurf 設定 &gt; Cascade** を開き、**Model Context Protocol (MCP) Servers** の一覧に **Auth0** が表示されていることを確認します。
3. Windsurf との会話を開始し、Auth0 テナントの管理についてサポートを依頼します。

### VS Code {#vs-code}

1. VS Code を再起動し、新しくインストールした Auth0 MCP Server が認識されるようにします。
2. VS Code で Copilot Chat をエージェントモードで開き、Auth0 のツールが利用できることを確認します。
3. 会話を開始し、Auth0 テナントの管理について支援を求めてみましょう。

### Claude Code {#claude-code}

1. Claude Code を再起動して、新しくインストールした Auth0 MCP Server が認識されるようにします。
2. セッション内で Auth0 のツール (例: `auth0_create_action` や `auth0_list_applications`) が利用できることを確認します。
3. Claude Code との会話を開始し、Auth0 テナントの管理について支援を依頼します。

### Gemini CLI {#gemini-cli}

1. Gemini CLI を再起動し、新しくインストールした Auth0 MCP Server が認識されるようにします。
2. Gemini CLI との対話を開始し、Auth0 テナントの管理について依頼してみましょう。

## 一般的なトラブルシューティング手順 {#common-troubleshooting-steps}

<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  接続に関する問題の多くは、Auth0 MCP Server を設定し、MCP クライアントを再起動することで解決できます。
</Callout>

#### コマンドラインヘルプを表示: {#get-command-line-help}

次のコマンドで、利用可能なコマンドの一覧と使用例を確認できます。

```bash lines
npx @auth0/auth0-mcp-server help
```

#### 認証に関する問題 {#authentication-issues}

Auth0 テナントで適切な権限が設定されていることを確認し、次の方法で再初期化をお試しください：

```bash lines
npx @auth0/auth0-mcp-server init
```

#### 接続エラー: {#connection-failures}

1. 次のコマンドで、MCP サーバーが実行中であることを確認します。

   ```ps lines
   ps aux | grep auth0-mcp
   ```

2. サーバーが実行中であることを確認したら、AI クライアントを再起動します。

#### Claude がサーバーに接続できないエラー: {#claude-cant-connect-to-the-server-error}

1. インストール後に Claude Desktop を再起動します。
2. 次のコマンドで、サーバーが実行中であることを確認します。

   ```ps lines
   ps aux | grep auth0-mcp
   ```

#### API エラー： {#api-errors}

Auth0のトークンの権限と有効期限を確認してください。詳細なログを確認するには、デバッグモードを有効にしてください (以下を参照) 。

## 設定の問題をデバッグする {#debug-configuration-issues}

#### デバッグモードを有効にする {#enable-debug-mode}

詳細なログを表示するには、デバッグモードを有効にします：

```bash lines
export DEBUG=auth0-mcp
```

#### MCPクライアントログのデバッグ {#debug-client-logs}

Claude Desktopで詳細なMCPクライアントログを取得できます：

```bash lines
# リアルタイムでログを追跡する
tail -n 20 -F ~/Library/Logs/Claude/mcp*.log
```

Auth0 MCP Server をデバッグモードで実行すると、MCP サーバーの詳細なログを取得できます:

```bash lines
DEBUG=auth0-mcp npx @auth0/auth0-mcp-server run
```

#### MCP Inspector を使用した高度なデバッグ {#advanced-debugging-using-mcp-inspector}

高度なトラブルシューティングには、MCP Inspector を使用してください。

```bash wrap lines
npx @modelcontextprotocol/inspector -e DEBUG='auth0-mcp' @auth0/auth0-mcp-server run__text in bold__
```

## 次のステップ {#next-steps}

[Auth0 Model Context Protocol (MCP) Server Tools Reference](/docs/ja-jp/get-started/auth0-mcp-server/auth0-mcp-tools-reference)にアクセスしてAuth0 MCP Serverの全機能を確認するか、実践的な例やユースケースについてはAuth0 Model Context Protocol Server Guidesをご覧ください。