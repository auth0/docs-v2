...
### VS Code の場合 {#for-vs-code}

```bash lines
npx @auth0/auth0-mcp-server init --client vscode
```

設定のスコープを選択するよう求められます：

* **グローバル**: すべての VS Code インスタンスで利用可能。
* **ワークスペース**: 特定のプロジェクトまたはリポジトリでのみ利用可能。

コマンドは選択内容に基づいて適切な `mcp.json` ファイルを自動的に設定します。

### Claude Code の場合 {#for-claude-code}
...
* **ユーザー**: `~/.claude.json` に書き込まれ、すべてのプロジェクトで利用可能。
* **プロジェクト**: 現在のプロジェクトでのみ利用可能。

### Gemini CLI の場合 {#for-gemini-cli}
...
1. 新しくインストールした Auth0 MCP Server を VS Code が認識するよう、VS Code を再起動します。
2. VS Code で Copilot Chat をエージェントモードで開き、Auth0 ツールが利用可能であることを確認します。
3. 会話を開始し、Auth0 テナントの管理についてサポートを求めます。

### Claude Code {#claude-code}

1. 新しくインストールした Auth0 MCP Server を Claude Code が認識するよう、Claude Code を再起動します。
2. Auth0 ツール (たとえば `auth0_create_action` や `auth0_list_applications`) がセッションで利用可能であることを確認します。
3. Claude Code との会話を開始し、Auth0 テナントの管理についてサポートを求めます。

### Gemini CLI {#gemini-cli}

1. 新しくインストールした Auth0 MCP Server を Gemini CLI が認識するよう、Gemini CLI を再起動します。
2. Gemini CLI との会話を開始し、Auth0 テナントの管理についてサポートを求めます。
