...
### ステップ 1: 高度なログ監視を設定する {#step-1-set-up-advanced-log-monitoring}
...
Claude は `auth0_list_logs` ツールを使用してログを取得・分析します。この分析により、通常の認証パターンのベースラインを把握できるため、異常を特定するうえで不可欠です。

### ステップ2: セキュリティ検知用のActionsを作成する {#step-2-create-security-detection-actions}
...
### ステップ 3: 自動対応Actionsを作成する {#step-3-create-automated-response-actions}
...
### ステップ 4: セキュリティ Actions をデプロイする {#step-4-deploy-the-security-actions}
...
### ステップ 5: カスタムのセキュリティチャレンジフォームを作成する {#step-5-create-custom-security-challenge-forms}

追加認証が必要な中リスクのログインには、状況をユーザーに説明するカスタムの[フォーム](/docs/ja-jp/customize/forms)を作成するとよいでしょう。
...
Claude は `auth0_create_form` ツールを使ってカスタムフォームを作成します。

## ステップ6: セキュリティイベントを監視する {#step-6-monitor-security-events}
...
## ステップ 7: セキュリティシステムを調整する {#step-7-fine-tune-the-security-system}
