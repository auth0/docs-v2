---
description: 各 Auth0 MCP Server ツールに必要な管理スコープと、初期化時に glob パターンを使用して設定する方法を学ぶ。
title: スコープを理解する
---
...
<Accordion title="管理操作のスコープ">
  ...
  | アプリケーションのクライアント許可を作成する                 | `auth0_create_application_grant` | `create:client_grants`    | アプリケーションによる API へのアクセスを承認し、マシンツーマシン認証フローを有効にします    |
  | アプリケーションのクライアント認証情報をファイルに保存する          | `auth0_save_credentials_to_file` | `read:client_credentials` | アプリケーションのクライアント認証情報を読み取り、ローカル環境ファイルに保存できます         |
