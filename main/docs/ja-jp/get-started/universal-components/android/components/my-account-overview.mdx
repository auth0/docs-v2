...
### My Account API を有効化する {#enable-the-my-account-api}

1. [Auth0 Dashboard &gt; アプリケーション &gt; APIs](https://manage.auth0.com/#/apis) に移動します。
2. **Activate My Account API** を選択して、テナントで有効化されていることを確認します。

### アプリケーションを作成し、My Account API の権限を設定する {#create-an-application-and-configure-my-account-api-permissions}
...
8. **Grant Access** を選択して、選択した権限を付与します。
...
### トークンプロバイダーを設定する {#configure-the-token-provider}
