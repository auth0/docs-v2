...
### MFA方式とスコープを有効にする {#enable-mfa-methods-and-scopes}
...
### MFA方式とスコープを有効にする {#enable-mfa-methods-and-scopes-2}
...
### MFA方式とスコープを有効にする {#enable-mfa-methods-and-scopes-3}
...
## コンポーネントをインストールする {#install-the-component-3}
    ...
    <Accordion title="レガシー Vercel registry（廃止）">
      既存の利用者は、**2026年9月17日**までVercelでホストされているパスを引き続き使用できます。
...
## TypeScript 型定義 {#typescript-definitions}
