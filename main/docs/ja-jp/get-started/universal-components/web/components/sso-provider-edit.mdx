...
## サードパーティアプリケーションアクセスとCross App Access {#third-party-application-access-and-cross-app-access}
...
### プロビジョニングタブの操作 {#provisioning-tab-actions}
...
### カスタマイズ用プロップス {#customization-props}
...
### Action のプロップス {#action-props-2}

    3つのタブでのユーザー操作は、それぞれのタブ名 (`sso`、`provisioning`、`domains`) の下に整理された複数のアクションプロパティによって処理されます。
...
### SSO タブのアクション {#sso-tab-actions-2}
    ...
    <table class="table">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            テナントから削除せずに、Organization からプロバイダーを削除します。
          </td>
        </tr>
      </tbody>
    </table>
...
### プロビジョニングタブの操作 {#provisioning-tab-actions-2}
    ...
    **型:** `ComponentAction<IdentityProvider, CreateIdpProvisioningScimTokenResponseContent>`
    ...
    アイデンティティプロバイダーがAuth0に対して認証を行うための新しいSCIMベアラートークンを生成します。
...
### ドメインタブの操作 {#domains-tab-actions-2}
...
### カスタマイズ用プロップス {#customization-props-2}
...
## 高度なカスタマイズ {#advanced-customization-2}

### 利用可能なフック {#available-hooks-2}
...
## インストール {#installation-3}
    ...
    <Accordion title="レガシー Vercel registry（廃止）">
      ...
      export default function App() {
        ...
        return (
          <Auth0Provider
            ...
            interactiveErrorHandler="popup" // Universal Login ポップアップでステップアップ認証チャレンジを処理するために必要
          >
            <Auth0ComponentProvider>
              <EditProviderPage />
            </Auth0ComponentProvider>
...
### SSO タブのアクション {#sso-tab-actions-3}
...
### プロビジョニングタブの操作 {#provisioning-tab-actions-3}

    `provisioning`プロップは、SCIMプロビジョニングタブの操作を設定します。このタブはSCIMプロトコルを使用したユーザーの自動プロビジョニングを管理します。
...
### ドメインタブの操作 {#domains-tab-actions-3}
...
### カスタマイズ用プロップス {#customization-props-3}
...
## 高度なカスタマイズ {#advanced-customization-3}

### 利用可能なフック {#available-hooks-3}
