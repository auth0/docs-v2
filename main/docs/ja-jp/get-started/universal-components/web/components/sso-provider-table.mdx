...
### 必須のプロップス {#required-props}

    必須のプロップスは、コンポーネントの動作の基盤となるものです。`SsoProviderTable`では、一般的なプロバイダー管理のワークフローに対応するために、両方のナビゲーションアクションが必要です。
    ...
    <table class="table">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            Provider の削除操作を非表示にします。デフォルト: <code>false</code>
          </td>
        </tr>
...
### Action のプロップス {#action-props}
    ...
    アクション系のプロップスは、基本的なナビゲーション以外のユーザー操作を扱います。これらは削除、除去、および有効化/無効化の操作を制御します。
...
### 主要なプロップス {#core-props}
    ...
    コアプロップはコンポーネントの動作に不可欠です。`SsoProviderTable` は、一般的なプロバイダー管理のワークフローを処理するために、両方のナビゲーションアクションを必要とします。
    ...
    **型:** `ComponentAction<void>`
    ...
    `createAction` プロパティは、ユーザーが &quot;Add Provider&quot; をクリックしたときにどこへ遷移させるかを制御するため、必須です。これがないと、テーブルはプロバイダー作成フローを開始する方法が分かりません。
...
## インストール {#installation-3}
    ...
    <Accordion title="レガシー Vercel レジストリ (廃止)">
...
### Action のプロップス {#action-props-3}
    ...
    アクション系のプロップスは、基本的なナビゲーションを超えるユーザー操作を扱います。これらは削除、除去、有効化／無効化の操作を制御します。
