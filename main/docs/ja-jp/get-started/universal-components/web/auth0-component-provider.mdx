...
### トークンの更新 {#token-renewal}

    デフォルトでは、Auth0 React SDK はサイレント認証を使用して新しいアクセストークンを取得します。アプリケーションでリフレッシュトークンによる更新を有効にする場合は、Auth0 アプリケーションでリフレッシュトークンを設定し、`Auth0Provider` に `useRefreshTokens={true}` を指定します:
...
### My Organization コンポーネントで使用する {#use-with-my-organization-components}

    My Organization コンポーネントは、My Organization API 用のユーザースコープのアクセストークンを要求します。API リクエストを行う際には、必要なオーディエンスとスコープが自動的に付与されます。
...
## 状態とパフォーマンス {#state-and-performance}

    すべてのAuth0コンポーネントでTanStack Queryのキャッシュを細かく調整できます。デフォルトでは、データは2分間最新の状態に保たれ、5分後にガベージコレクションされ、ウィンドウへのフォーカス時に再取得は行われません。
...
## プロキシモードを設定する {#configure-proxy-mode}

    Next.js アプリケーションでは、サーバーサイド認証のためにプロキシモードを使用します:
...
### proxy モードのプロパティ {#proxy-mode-properties}
...
## ユーザーエクスペリエンス {#user-experience-2}
    ...
    **toastSettings**
    ...
    <Accordion title="カスタムプロバイダー">
      ...
      <Callout icon="file-lines" color="#0EA5E9" iconType="regular">
        すべてのカスタムメソッドはオプションです。必要なものだけを実装してください。メソッドが受け取るのはメッセージテキストのみです。スタイリング、表示位置、表示時間は実装側で処理します。
      </Callout>
    </Accordion>

    ***

## 状態とパフォーマンス {#state-and-performance-2}
...
## ユーザーエクスペリエンス {#user-experience-3}
...
## 状態とパフォーマンス {#state-and-performance-3}
