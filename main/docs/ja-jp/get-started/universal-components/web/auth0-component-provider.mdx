...
## 状態とパフォーマンス {#state-and-performance}
    ...
    すべてのAuth0コンポーネントに対するTanStack Queryのキャッシュを細かく調整します。デフォルトではデータは2分間最新の状態に保たれ、5分後にガベージコレクションされ、ウィンドウフォーカス時の再フェッチはスキップされます。
