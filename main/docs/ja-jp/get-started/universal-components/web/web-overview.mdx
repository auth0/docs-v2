...
<Tabs>
  <Tab title="React">
## Universal Components をインストールする {#install-universal-components}
...
## Universal Components の設定 {#configure-universal-components}
...
## スタイルコンポーネント {#style-components}
  ...
  <Tab title="Next.js">
## Universal Components をインストールする {#install-universal-components-2}
...
## スタイルコンポーネント {#style-components-2}
  ...
  <Tab title="shadcn">
## Universal Components をインストールする {#install-universal-components-3}
    ...
    次に、GitHub Registry を使用して shadcn CLI 経由で Universal Components を個別にインストールします。
    ...
    <Callout icon="file-lines" color="#0EA5E9" iconType="regular">
      ...
      連携に必要な依存関係 `@auth0/universal-components-core` をインストールします。**shadcn CLI v2.5.0 以降** (GitHub Registry のサポート) が必要です。
    </Callout>

    <Callout icon="clock" color="#f59e0b" iconType="regular">
      **レガシー Vercel レジストリ**：Vercel でホストされているパス
      (`https://auth0-universal-components.vercel.app/r/...`) を使用している既存の利用者は、
      すぐに移行する必要はありません。Vercel レジストリは **2026 年 9 月 17 日**まで引き続き利用できます。
      新しいプロジェクトでは、上記の GitHub Registry のパスを使用してください。
    </Callout>
...
## Universal Components の設定 {#configure-universal-components-3}
...
## スタイルコンポーネント {#style-components-3}
...
<CardGroup cols={2}>
  ...
  <Card title="SaaS スターターリポジトリ" icon="github" href="https://github.com/auth0/saas-start-universal-components">
...
<CardGroup cols={2}>
  ...
  <Card title="セルフサービスのアカウントセキュリティインターフェースを構築する" icon="key" href="/docs/ja-jp/get-started/universal-components/web/components/my-account-overview">
    My Account API を使用して、ユーザーが自身の MFA 認証要素とパスキーを管理できるようにします。
  </Card>
</CardGroup>