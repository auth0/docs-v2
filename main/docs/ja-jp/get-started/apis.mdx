...
title: API
sidebarTitle: 概要
description: API の登録、アクセスポリシー、スコープ、トークン設定など、Auth0 で API を利用するための主要なトピックを紹介します。
validatedOn: 2026-08-12
---

[API](https://auth0.com/features/machine-to-machine) は外部リソースを表すエンティティであり、アプリケーションから送信される保護されたリソースへのリクエストを受け取り、それに応答できます。[OAuth2 specification](https://tools.ietf.org/html/rfc6749) では、API は **<Tooltip tip="リソースサーバー: 保護されたリソースをホストするサーバー。リソースサーバーは保護されたリソースへのリクエストを受け取り、応答します。" cta="用語集を見る" href="/docs/ja-jp/glossary?term=Resource+Server">リソースサーバー</Tooltip>** に相当します。

## API へのアクセスを認可する {#authorize-access-to-your-apis}
...
## アクセス制御による認可の決定 {#determine-authorization-through-access-control}
