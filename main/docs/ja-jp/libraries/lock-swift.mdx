...
### アプリケーションに組み込む {#integrate-with-your-application}
...
### Lock のインポート {#import-lock}

**Lock** が必要な場所でインポートします

`import lock`

### Auth0 の認証情報 {#auth0-credentials}
...
## Lock Classic の実装 {#implementation-of-lock-classic}

Lock Classic は、データベース接続、ソーシャル接続、エンタープライズ接続を使用して認証を行います。

### OIDC準拠モード {#oidc-conformant-mode}
...
## Auth0.Swift ライブラリを使用してユーザープロフィールにアクセスする {#use-auth0swift-library-to-access-user-profile}
...
## パスワードマネージャーのサポート {#password-manager-support}
