...
#### plist を使用して Client ID と Domain を設定する {#configure-client-id-and-domain-with-a-plist}
...
#### Client ID と Domain をコードで設定する {#configure-client-id-and-domain-programmatically}

##### Web Auth の場合 {#for-web-auth}
...
##### Authentication API クライアントの場合 {#for-the-authentication-api-client}
...
### Web Authを設定する (iOS / macOS) {#configure-web-auth-ios-macos}

#### コールバック URL とログアウト URL を設定する {#configure-callback-url-and-logout-url}
...
#### カスタム URL スキームを設定する {#configure-custom-url-scheme}
...
### Web Auth でのログイン (iOS / macOS) {#web-auth-login-ios-macos}
...
#### async/await を使う {#using-asyncawait}
...
### Web Auth ログアウト (iOS / macOS) {#web-auth-logout-ios-macos}
...
#### async/await を使う {#using-asyncawait-2}
...
### SSOアラートボックス (iOS / macOS) {#sso-alert-box-ios-macos}
