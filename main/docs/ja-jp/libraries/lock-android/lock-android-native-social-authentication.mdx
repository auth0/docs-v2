...
Google開発者コンソールで<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" href="/docs/ja-jp/glossary?term=oath2" tip="OAuth 2.0: 認可プロトコルとワークフローを定義する認可フレームワーク。" cta="用語集の表示">OAuth</Tooltip>資格情報を新たに作成する場合は、アプリケーションにサインする際に使う証明書のSHA-1を提供する必要があります。<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" href="/docs/ja-jp/glossary?term=auth0-dashboard" tip="Auth0 Dashboard: サービスを構成するためのAuth0の主製品。" cta="用語集の表示">Auth0 Dashboard</Tooltip>でアプリケーションの構成を完了する際は、SHA-256値も提供する必要があります。以下に、値を取得するためのターミナルコマンド例とその結果例を示します。
