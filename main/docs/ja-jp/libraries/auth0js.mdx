...
## v9 から v10 への移行 {#migrate-from-v9-to-v10}
...
## すぐに使えるサンプル {#ready-to-go-example}
...
#### 利用可能なパラメータ {#available-parameters}
...
#### ポップアップ認証の結果を処理する {#handle-popup-authentication-results}
...
### 組み込み認証でのシングルサインオン {#single-sign-on-with-embedded-authentication}
...
## パスワードレスログイン {#passwordless-login}
...
### パスワードレス認証を開始する {#start-passwordless-authentication}
...
### パスワードレス認証を完了する {#complete-passwordless-authentication}
...
## `authResult` を取得してユーザー情報を取得する {#extract-the-authresult-and-get-user-info}
...
## エラーコードと説明 {#error-codes-and-descriptions}
...
| `user_metadata` | 任意     | (JSON object) ユーザー情報に使用する追加属性。[user&#95;metadata](/docs/ja-jp/manage-users/user-accounts/metadata) に保存されます |
...
## checkSession を使用して新しいトークンを取得する {#using-checksession-to-acquire-new-tokens}
...
### checkSession() を使用したポーリング {#polling-with-checksession}
...
## パスワードリセットのリクエスト {#password-reset-requests}
...
### ユーザープロファイルを取得する {#get-the-user-profile}
...
### ユーザープロファイルを更新する {#update-the-user-profile}
