...
<ParamField body="countryCodes" type={<span><a href="/docs/ja-jp/libraries/acul/js-sdk/Screens/interfaces/CountryCodesMembers">CountryCodesMembers</a></span>}>
  `phone-identifier-enrollment` 画面での電話番号入力に使用する、`available` や `recommended` などの国コードデータを提供します。
</ParamField>
...
<ParamField body="screen" type={<span><a href="/docs/ja-jp/libraries/acul/js-sdk/Screens/interfaces/ScreenMembersOnPhoneIdentifierEnrollment">ScreenMembersOnPhoneIdentifierEnrollment</a></span>}>
  設定やコンテキストなど、`phone-identifier-enrollment` 画面に固有の詳細を含みます。`screen.data` オブジェクトでは、`phone`、`messageType`、`username` の3つのオプションフィールドを利用できます。現在の登録セッションに関連付けられたユーザー名を表示するには、`screen.data?.username` を使用します。
</ParamField>
