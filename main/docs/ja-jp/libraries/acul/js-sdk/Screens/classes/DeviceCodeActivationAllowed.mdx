...
description: "Universal Login の `device-code-activation-allowed` 画面をカスタマイズするために利用できる、すべてのプロパティとメソッドについて説明します。"
---

DeviceCodeActivationAllowed クラスは、`device-code-activation-allowed` 画面の機能を実装します。この画面は、デバイスコードの有効化が正常に承認されると表示されます。
...
DeviceCodeActivationAllowed画面マネージャーのインスタンスを作成します:
...
<ParamField body="client" type={<span><a href="/docs/ja-jp/libraries/acul/js-sdk/Screens/interfaces/ClientMembers">ClientMembers</a></span>}>
  `device-code-activation-allowed` 画面向けに、`id`、`name`、`logoUrl` などのクライアント関連の構成を提供します。
</ParamField>
...
<ParamField body="countryCodes" type={<span><a href="/docs/ja-jp/libraries/acul/js-sdk/Screens/interfaces/CountryCodesMembers">CountryCodesMembers</a></span>}>
  電話番号入力用に、`available` や `recommended` などの国コードデータを `device-code-activation-allowed` 画面向けに提供します。
</ParamField>
...
<ParamField body="screen" type={<span><a href="/docs/ja-jp/libraries/acul/js-sdk/Screens/interfaces/ScreenMembers">ScreenMembers</a></span>}>
  設定やコンテキストを含む、`device-code-activation-allowed` 画面固有の詳細を含みます。
</ParamField>
...
<ParamField body="transaction" type={<span><a href="/docs/ja-jp/libraries/acul/js-sdk/Screens/interfaces/TransactionMembers">TransactionMembers</a></span>}>
  アクティブな識別子やフローの状態など、`device-code-activation-allowed` 画面に固有のトランザクションデータを提供します。
</ParamField>
...
<ParamField body="untrustedData" type={<span><a href="/docs/ja-jp/libraries/acul/js-sdk/Screens/interfaces/UntrustedDataMembers">UntrustedDataMembers</a></span>}>
  デバイスコード有効化フロー中のユーザー入力など、SDK に渡された信頼されていないデータを扱います。
</ParamField>
