...
description: "Universal Login の `brute-force-protection-unblock` 画面をカスタマイズするために使用できる、すべてのプロパティとメソッドについて説明します。"
---

BruteForceProtectionUnblock クラスは、`brute-force-protection-unblock` 画面の機能を実装します。この画面は、ログイン試行の失敗が複数回発生したためにユーザーのアカウントがブロックされ、ブロック解除リクエストを送信する必要がある場合に表示されます。

<Frame>
  <img style={{maxHeight:"400px"}} src="https://mintlify.s3.us-west-1.amazonaws.com/auth0/docs/images/cdy7uua7fh8z/ACUL/brute-force-protection-unblock.png" alt="ACUL ブルートフォース対策のブロック解除" />
</Frame>

## コンストラクター {#constructors}

BruteForceProtectionUnblock画面マネージャーのインスタンスを作成します:
...
<ParamField body="client" type={<span><a href="/docs/ja-jp/libraries/acul/js-sdk/Screens/interfaces/ClientMembers">ClientMembers</a></span>}>
  `brute-force-protection-unblock` 画面向けに、`id`、`name`、`logoUrl` などのクライアント関連の構成を提供します。
</ParamField>
...
<ParamField body="countryCodes" type={<span><a href="/docs/ja-jp/libraries/acul/js-sdk/Screens/interfaces/CountryCodesMembers">CountryCodesMembers</a></span>}>
  `brute-force-protection-unblock` 画面向けに、`available` や `recommended` など、電話番号入力用の国コードデータを提供します。
</ParamField>
...
<ParamField body="screen" type={<span><a href="/docs/ja-jp/libraries/acul/js-sdk/Screens/interfaces/ScreenMembers">ScreenMembers</a></span>}>
  設定やコンテキストなど、`brute-force-protection-unblock` 画面に固有の詳細を含みます。
</ParamField>
...
<ParamField body="transaction" type={<span><a href="/docs/ja-jp/libraries/acul/js-sdk/Screens/interfaces/TransactionMembers">TransactionMembers</a></span>}>
  アクティブな識別子やフローの状態など、`brute-force-protection-unblock` 画面に固有のトランザクションデータを提供します。
</ParamField>
...
<ParamField body="unblockAccount" type="Promise<void>">
  このメソッドは、アカウントのブロック解除リクエストを送信します。
