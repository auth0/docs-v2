...
<ParamField body="countryCodes" type={<span><a href="/docs/ja-jp/libraries/acul/js-sdk/Screens/interfaces/CountryCodesMembers">CountryCodesMembers</a></span>}>
  `login-email-verification` 画面での電話番号入力用に、`available` や `recommended` などの国番号データを提供します。
</ParamField>
  ...
  **メソッド パラメーター**
  ...
  <Expandable title="パラメーター">
    ...
    <ParamField body="persist?" type="&#x22;session&#x22;">
      `'session'` に設定すると、選択した言語はセッション期間中維持されます。
    </ParamField>
  </Expandable>
</ParamField>
...
<ParamField body="continueWithCode" type="Promise<void>">
  ...
  **メソッド パラメーター**
  ...
  <Expandable title="パラメーター">
    <ParamField body="options">
      [ContinueWithCodeOptionPayload](/docs/ja-jp/libraries/acul/js-sdk/Screens/interfaces/ContinueWithCodeOptionPayload)。
    </ParamField>
...
<ParamField body="getErrors" type={<span><a href="/docs/ja-jp/libraries/acul/js-sdk/Screens/interfaces/Error">Error</a>[]</span>}>
  このメソッドは、コンテキストからトランザクション エラーの配列を取得します。エラーがない場合は空の配列を返します。トランザクション コンテキスト内のエラーオブジェクトの配列です。
</ParamField>

<ParamField body="resendCode" type="Promise<void>">
  このメソッドは、新しい確認コードをユーザーのメールアドレスに送信するよう要求します。
...
<ParamField body="resendManager" type={<span><a href="/docs/ja-jp/libraries/acul/js-sdk/Screens/interfaces/ResendControl">ResendControl</a></span>}>
  このメソッドは、タイムアウト管理付きの再送機能を提供します。再送を開始する `startResend()` メソッドを持つ [`ResendControl`](/docs/ja-jp/libraries/acul/js-sdk/Screens/interfaces/ResendControl) オブジェクトを返し、ユーザーが再度再送できるようになるまでクールダウン期間を設けます。
