...
<ParamField body="useScreen" type={<span>() =&gt; <a href="/docs/ja-jp/libraries/acul/react-sdk/API-Reference/Types/interfaces/ScreenMembersOnPhoneIdentifierEnrollment">ScreenMembersOnPhoneIdentifierEnrollment</a></span>}>
  このフックには、設定やコンテキストを含む `phone-identifier-enrollment` 画面固有の詳細が含まれます。

  `screen.data` オブジェクトは、`phone`、`messageType`、`username` の3つのオプションフィールドを公開します。現在の登録セッションに関連付けられたユーザー名を表示するには、`screen.data?.username` を使用します。
...
<ParamField body="useUntrustedData" type={<span>() =&gt; <a href="/docs/ja-jp/libraries/acul/react-sdk/API-Reference/Types/interfaces/UntrustedDataMembers">UntrustedDataMembers</a></span>}>
  このフックは、URL のクエリ文字列で事前に設定されたパラメータなど、画面に渡される信頼できないデータを処理します。
...
<ParamField body="usePhoneIdentifierEnrollment" type={<a href="/docs/ja-jp/libraries/acul/react-sdk/API-Reference/Types/interfaces/PhoneIdentifierEnrollmentMembers">PhoneIdentifierEnrollmentMembers</a>}>
