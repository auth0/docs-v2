...
<ParamField body="useCountryCodes" type={<span>() =&gt; <a href="/docs/ja-jp/libraries/acul/react-sdk/API-Reference/Types/interfaces/CountryCodesMembers">CountryCodesMembers</a></span>}>
  このフックは、`login` 画面で利用可能な電話の国番号と推奨される既定値を提供します。
  ...
  **メソッドパラメーター**
  ...
  <Expandable title="パラメーター">
    ...
    <ParamField body="identifier?" type="string">
      ユーザーの識別子 — `identifierType` に応じて、メールアドレス、ユーザー名、または電話番号。これか `username` のいずれかを指定します。両方を指定した場合は `identifier` が優先されます。
    </ParamField>

    <ParamField body="username?" type="string">
      ユーザーの識別子 (元の表記のまま) 。これか `identifier` のいずれかを指定します。
    </ParamField>
    ...
    <ParamField body="identifierType?" type={<span><a href="/docs/ja-jp/libraries/acul/react-sdk/API-Reference/Types/type-aliases/IdentifierType">IdentifierType</a></span>}>
      `identifier` (または `username`) が保持する内容。通常は `screen.data.activeIdentifierType` から読み取ります。省略すると、識別子は型指定なしで送信されます。
    </ParamField>
    ...
    <ParamField body="phoneCountryCode?" type="string">
      電話番号識別子に使用する ISO 3166-1 alpha-2 の国 (`countryCodes.available` 内の `code` から選択) 。`identifierType` が `phone` の場合に必要です。国内番号を渡してください。国番号はサーバー側で付加されます。
    </ParamField>
