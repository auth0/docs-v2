...
[独自のデータベースをIDプロバイダーとして使用する](/docs/ja-jp/authenticate/database-connections/custom-db)場合は、次のタスクを実行してユーザープロファイルを更新します。
...
[ユーザーの移行を有効化](/docs/ja-jp/authenticate/database-connections/migrating)していて、ユーザーがAuth0データベースへ移行済みの場合には、Auth0はユーザープロファイルについてデータベースを再度クエリすることはありません。そのため、カスタムデータベースでユーザについて行ったすべての変更は、Auth0には反映されません。
...
Auth0は、[データベース接続](/docs/ja-jp/authenticate/database-connections)から受け取ったユーザープロファイルをキャッシュしてから、クライアントアプリケーションに送信します。キャッシュはAuth0のデータベースに保管されて、ユーザーが認証するたびに更新されます。
