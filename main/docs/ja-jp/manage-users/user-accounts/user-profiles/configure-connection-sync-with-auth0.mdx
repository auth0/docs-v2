...
| 値 | 説明 |
| --- | --- |
| `CONNECTION_ID` | root属性の更新を許可する接続のID。 |
| `MGMT_API_ACCESS_TOKEN` | `update:connections`のスコープを持つ[Management APIのアクセストークン](https://auth0.com/docs/api/management/v2/tokens)。 |
| `ATTRIBUTE_UPDATE_VALUE` | ユーザープロファイルのroot属性の更新を許可する時間を示します。有効な値は`on_first_login`と`on_each_login`です。新しい接続では`on_each_login`が既定値になります。 |
