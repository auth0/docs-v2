...
## Auth0 Management APIの[ユーザーの更新]()エンドポイントを使用して、`app_metadata`や`user_metadata`フィールドを含む、認証済みユーザーのプロファイルデータを更新することができます。詳細については、「[Management APIを使用したメタデータの管理](/users/metadata/manage-metadata-api)」をお読みください。
