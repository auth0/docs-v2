...
| 値 | 説明 |
| --- | --- |
| `MGMT_API_ACCESS_TOKEN` | `create:users`スコープのある[Management APIのアクセストークン](/docs/ja-jp/api/management/v2/tokens)です。 |
| `CONNECTION_NAME` | 初回のユーザー情報の受信に使用した接続の名前です。 |
| `EMAIL_VALUE` | 作成するユーザーのメールアドレスです。 |
| `PASSWORD_VALUE` | 作成するユーザーのパスワードです。 |
| `GIVEN_NAME_VALUE` | 作成するユーザーの名です。 |
| `FAMILY_NAME_VALUE` | 作成するユーザーの姓です。 |
| `NAME_VALUE` | 作成するユーザーのフルネームです。 |
| `NICKNAME_VALUE` | 作成するユーザーのニックネームです。 |
| `PICTURE_VALUE` | 作成するユーザーの写真のURLです。 |
