...
Auth0 に保存させたくないユーザー属性がある場合は、[ユーザー属性を DenyList に追加](/docs/ja-jp/secure/security-guidance/data-security/denylist)できます。

### ユーザーデータの正規化 {#user-data-normalization}
...
### ユーザープロファイル属性マッピング {#user-profile-attribute-mapping}

#### AD/LDAP Connector {#adldap-connector}
...
### ユーザープロファイルのキャッシュ {#caching-user-profiles}
...
## カスタムユーザープロファイルデータ {#custom-user-profile-data}
...
## ユーザープロファイルデータを変更する {#change-user-profile-data}
...
## ユーザープロファイルへのアクセス {#access-user-profiles}
...
### アプリからユーザープロファイルにアクセスする {#access-user-profiles-from-apps}
...
### Management API からユーザープロファイルにアクセスする {#access-user-profiles-from-the-management-api}
...
## ユーザープロファイルとトークンの違い {#the-difference-between-user-profiles-and-tokens}
