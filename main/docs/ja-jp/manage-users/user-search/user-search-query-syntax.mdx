...
<td>`last_login`</td>
<td>日時</td>
<td>ユーザーが最後にログインした日時を示すタイムスタンプです。このプロパティが<a href="/docs/ja-jp/customize/actions">ルール</a>内から`user`オブジェクトを伴って実行される場合、値はルールを起動したログインと関連付けられます（ルールは実際のログインの後に実行されるからです）。</td>
</tr>
<tr>
<td>`last_ip`</td>
