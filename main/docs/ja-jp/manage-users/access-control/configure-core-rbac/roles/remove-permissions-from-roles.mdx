...
| 値 | 説明 |
| --- | --- |
| `ROLE_ID` | 権限を削除したいロールのID。 |
| `MGMT_API_ACCESS_TOKEN` | `update:roles`のスコープを持つ[Management APIのアクセストークン](https://auth0.com/docs/ja-jp/api/management/v2/tokens)。 |
| `API_ID` | 削除したい指定ロールの権限に関連するAPIのID。 |
| `PERMISSION_NAME` | 削除したい指定ロールの権限の名前。 |
