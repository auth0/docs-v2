...
| 値 | 説明 |
| --- | --- |
| `USER_ID` | 権限を割り当てたいユーザーのID。 |
| `MGMT_API_ACCESS_TOKEN` | `update:users`のスコープを持つ[Management APIのアクセストークン](https://auth0.com/docs/ja-jp/api/management/v2/tokens)。 |
| `API_IDENTIFIER` | 指定のユーザーに対して割り当てたい権限に関連するAPIの識別子。 |
| `PERMISSION_NAME` | 指定のユーザーに対して割り当てたい権限の名前。 |
