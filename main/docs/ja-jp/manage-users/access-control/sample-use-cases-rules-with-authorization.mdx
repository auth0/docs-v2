...
1. Authorization Core（認可コア）機能セットを使用して[DAEロールを作成します](/docs/ja-jp/manage-users/access-control/configure-core-rbac/roles/create-roles)。作成したロールの名前は必ず、[あらかじめ定義されたDAEロール](/docs/ja-jp/customize/extensions/delegated-administration-extension#assign-roles-to-users)と一致しなければなりません。
