---
description: Auth0 Actions を使用して、カスタムロジックでロールベースのアクセス制御を拡張し、ログイン時にユーザーのロールを評価し、アクセストークンの permissions を追加または削除します。
...
## 企業ネットワーク内からアクセスしているユーザーのみにアクセスを許可する {#allow-access-only-to-users-who-are-inside-the-corporate-network}
...
## トークンにユーザーのロールを追加する {#add-user-roles-to-tokens}
...
## 特定のJA3/JA4フィンガープリントへのアクセスを拒否する {#deny-access-to-specific-ja3ja4-fingerprints}
