...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  この機能は Free 計画ではご利用いただけません。
</Callout>

## サポート対象のファイル形式 {#supported-file-formats}
...
## 1. ユーザーエクスポート ジョブ を送信する {#1-submit-export-users-job}
...
<Tabs>
  <Tab title="Auth0 Dashboard">
    ...
    5. **Export Fields** セクションで、エクスポートに含めるフィールドを選択します:
       ...
       * **Include default fields** をオンにすると、[Auth0 正規化ユーザープロファイルスキーマ](/docs/ja-jp/manage-users/user-accounts/user-profiles/normalized-user-profiles)のフィールドが追加されます。
  ...
  <Tab title="Management API">
    ...
    ジョブの作成に成功すると、戻り値にはジョブ ID を含むジョブの情報が含まれます。
  </Tab>
</Tabs>

## 2. ジョブのステータスを確認してエクスポートをダウンロードする {#2-check-job-status-and-download-the-export}
