...
この機能は、Auth0 DashboardまたはManagement APIで有効にすることができます。

* **Auth0 Dashboard**：左のメニューから[［Settings（設定）］](/tenant-settings)を選択し、**［Advanced（詳細設定）］**タブを選択します。**［Settings（設定）］**セクションで**［Allow Organization Names in Authentication API（Authentication APIで組織名を許可する）］**トグルを有効にします。
