...
各[組織](/docs/ja-jp/manage-users/organizations/organizations-overview)ごとに特定の接続を有効にして、ユーザーに異なるログインオプションを提供できます。接続を有効にすると、その接続が組織のログインプロンプトに追加され、ユーザーはその接続を使用して認証し、アプリケーションにアクセスできるようになります。
...
## 組織のプロパティ {#organization-properties}
...
| Organization Admin のアクセスレベル | すべての接続タイプ    | このプロパティは、[My Organization API](/docs/ja-jp/manage-users/my-organization-api) を使用する際に、この接続に対して Organization Admin が持つアクセスレベルを決定します。使用可能な値は次のとおりです。<ul><li>**None**: tenant Admin が管理し、My Organization API では表示も編集もできません。</li> <li>**Read-only**: tenant Admin が管理し、My Organization API で表示できます。</li> <li>**Limited**: Organization Admin は、My Organization API を使用して、選択された機密性の低い設定を編集できます。</li> <li>**Full**: Organization Admin は、[Connection Profile](/docs/ja-jp/authenticate/enterprise-connections/connection-profile) で定義された制限に従って、この接続で許可されているすべての設定を編集できます。</li></ul>  |
| 組織メンバーのアクセスレベル              | すべての接続タイプ    | このプロパティは、[My Organization API](/docs/ja-jp/manage-users/my-organization-api) を使用する際に、この接続を介して認証されたメンバーを管理するために Organization Admin が持つアクセスレベルを決定します。使用可能な値は次のとおりです。<ul><li>**None**: Organization Admin は、My Organization API を使用して、この接続を介して認証されたメンバーを表示も管理もできません。</li> <li>**Read-only**: Organization Admin は、My Organization API を使用して、この接続を介して認証されたメンバーを表示できますが、変更はできません。</li> <li>**Limited**: Organization Admin は、この接続を介して認証されたメンバーに対して、選択されたメンバー管理操作を実行できます。</li> <li>**Full**: Organization Admin は、この接続を介して認証されたメンバーに対して、許可されているすべてのメンバー管理操作を実行できます。</li></ul> |
...
## Organization Connections を設定する {#configure-organization-connections}
...
#### Auth0 Dashboard {#auth0-dashboard}
    ...
    * **Organization Admin Access Level** を確認し、入力されていない場合は、My Organization API を使用する際にこの接続に対して Organization Admins に付与するアクセスレベルを選択します。
    * **Organization Member Access Level** を確認し、入力されていない場合は、My Organization API を使用する際に、この接続で認証されたメンバーの管理について Organization Admins に付与するアクセスレベルを選択します。
...
#### Management API {#management-api}
    ...
    * **My Organization API を使用する場合**: `{organizationAccessLevel}`、`{organizationMemberAccessLevel}`、`{organizationConnectionName}` は適切な値に置き換えます。
    ...
    | `{isEnabled}`                     | 組織の接続を有効または無効にします。接続を無効にしても、`show_as_button`、`assign_membership_on_login` などの値を保持できます。                                                                                                                                                                                                                                                                                                                                                                         |
    ...
    | `{organizationMemberAccessLevel}` | My Organization API を使用してこの接続で認証されたメンバーを管理する際に、組織の Admin がどのレベルのアクセス権を持つかを決定します。`none` に設定すると、この接続で認証されたメンバーは My Organization API からは表示もアクセスもできません。`readonly` に設定すると、組織の Admin はこの接続で認証されたメンバーを表示できますが、変更はできません。`limited` に設定すると、組織の Admin はこの接続で認証されたメンバーに対して、選択されたメンバー管理操作を実行できます。`full` に設定すると、組織の Admin はこの接続で認証されたメンバーに対して、許可されているすべてのメンバー管理操作を実行できます。                                                                                                    |
...
##### レスポンスのステータスコード {#response-status-codes}
