...
| 値 | 説明 |
| --- | --- |
| `ORG_ID` | ロールを削除したいメンバーの組織ID。 |
| `MGMT_API_ACCESS_TOKEN` | `delete:organization_member_roles`のスコープを持つ[Management APIのアクセストークン](/docs/ja-jp/tokens/management-api-access-tokens)。 |
| `USER_ID` | 指定のロールを削除したいユーザーのID。 |
| `ROLE_ID` | 指定された組織の指定のユーザーから削除したいロールのID。 |
...
| --- | --- | --- | --- |
| `204` |   | Roles successfully removed from organization（ロールが組織メンバーから削除されました） |   |
...
| `403` | `insufficient_scope` | Insufficient scope; expected any of（スコープ不足。次のいずれかを想定します）： `delete:organization_member_roles`. | 提供されたベアラートークンスコープで許可されていないフィールドのread/writeを試みました。 |
