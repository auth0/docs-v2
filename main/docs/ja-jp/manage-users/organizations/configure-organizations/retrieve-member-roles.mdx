...
| 値 | 説明 |
| --- | --- |
| `ORG_ID` | メンバーのロールを取得したい組織のID。 |
| `USER_ID` | ロールを取得したいメンバーのユーザーID。 |
| `MGMT_API_ACCESS_TOKEN` | `read:organization_member_roles`のスコープを持つ[Management APIのアクセストークン](/docs/ja-jp/tokens/management-api-access-tokens)。 |
...
| --- | --- | --- | --- |
| `200` |   | Roles successfully retrieved.（ロールを正常に取得しました。） |   |
...
| `403` | `insufficient_scope` | Insufficient scope; expected any of: `read:organization_member_roles`.（スコープが不足しています。`read:organization_member_roles`が必要です。） | 渡されたベアラートークンのスコープでは許可されないフィールドの読み出しや書き込みが試行されました。 |
