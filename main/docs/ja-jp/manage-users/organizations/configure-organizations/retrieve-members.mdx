...
| 値 | 説明 |
| --- | --- |
| `ORG_ID` | メンバーを取得したい組織のID。 |
| `MGMT_API_ACCESS_TOKEN` | スコープ`read:organization_members`を持つ[Management API](/docs/ja-jp/tokens/management-api-access-tokens)のアクセストークン。 |
...
| --- | --- | --- | --- |
| `200` |   | Members successfully retrieved（メンバーを取得しました） |   |
...
| `403` | `insufficient_scope` | Insufficient scope; expected any of（スコープ不足。次のいずれかを想定します）： `read:organization_members`. | 提供されたベアラートークンスコープで許可されていないフィールドのread/writeを試みました。 |
