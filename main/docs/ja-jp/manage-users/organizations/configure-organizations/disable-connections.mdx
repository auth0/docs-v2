...
| `MGMT_API_ACCESS_TOKEN` | スコープ`delete:organization_connections`を持つ[Management APIのアクセストークン](/docs/ja-jp/tokens/management-api-access-tokens)。 |
...
| `403` | `insufficient_scope` | スコープが不十分です。`delete:organization_connections`のいずれかが予期されています。 | 指定されたベアラートークンのスコープで許可されていないフィールドの読み取り/書き込みを試行しました。 |
