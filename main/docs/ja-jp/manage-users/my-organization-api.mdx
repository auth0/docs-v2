...
* 接続に関する [Cross App Access (XAA)](/docs/ja-jp/ai-agents-mcp/cross-app-access) リソースアプリケーションの設定
* メンバーの招待
* メンバーの管理
* メンバーへのロールの割り当てと削除
* 招待作成のためのユーザーストアの検出
...
## My Organization API を設定する {#set-up-my-organization-api}

### Auth0 Dashboard で My Organization API を有効化する {#activate-the-my-organization-api-in-auth0-dashboard}
...
### クライアントアプリケーションの属性を設定する {#configure-client-application-attributes}
...
| `connection_deletion_behavior`        | **Enum (allow, allow&#95;if&#95;empty).** このアプリケーションから My Organization API 経由でエンドユーザーが接続の削除を試みたときに、My Organization API がどのように動作するかを示します。enum の値と説明は次のとおりです: <p /> 1. `allow`: ユーザーが適切なスコープを持っている場合、接続を削除できます。その結果、その接続に由来するすべてのユーザーも削除されます。 <p /> 2.`allow_if_empty`: ユーザーが適切なスコープを持っている場合、接続内にユーザーが存在しないときに限り、接続を削除できます。ユーザーが存在する場合、My Organization API はエラーを返し、削除は実行されません。 |
| `third_party_client_access`           | **Object.** My Organization API を介して、この Organization 上のサードパーティアプリケーションへのアクセスについて Organization 管理者が設定できる値を制御します。形式: `{ "default_value": "block" \| "allow", "allowed_values": ["block", "allow"] }`。デフォルトは `{ "default_value": "block", "allowed_values": ["block"] }` であり、明示的に許可されるまで Organization 管理者はサードパーティアプリケーションへのアクセスを有効にできません。                                       |
| `invitation_landing_client_id`        | **String.** メンバー招待を受諾する際にユーザーがリダイレクトされるアプリケーションのクライアント ID です。アプリケーションが `create:my_org:member_invitations` スコープを使用する場合は必須です。指定したクライアントは、招待の作成時にその Organization で有効になっている必要があります。                                                                                                                                                                                              |
| `enforce_permission_ceiling`          | **Boolean.** 有効にすると、操作時点で自身の実効的な権限を超える権限を持つロールを Organization 管理者が割り当てられないようにします。デフォルトは `false` です。                                                                                                                                                                                                                                                                           |
| `enforce_self_assignment_restriction` | **Boolean.** 有効にすると、Organization 管理者が自分自身にロールを割り当てられないようにします。デフォルトは `false` です。                                                                                                                                                                                                                                                                                              |

[Connection Profile](/docs/ja-jp/authenticate/enterprise-connections/connection-profile) の `connection_profile_id` は、Organization 管理者が My Organization API を介して接続を [Cross App Access (XAA)](/docs/ja-jp/ai-agents-mcp/cross-app-access) リソースアプリケーションとして機能するように設定できるかどうかを制御します。

#### クライアントアプリケーション属性を設定する {#configure-client-application-attributes-2}
...
<Tabs>
  <Tab title="Auth0 Dashboard">
    ...
    4. 次の設定を行います。<br />
       ...
       1. 1 つ以上の IDプロバイダー を有効にします。Organization 管理者は、有効化されたプロバイダーの一覧から希望するオプションを選択できます。 <br />
       ...
       E. **任意**。Organization 管理者にサードパーティアクセスの設定を許可するかどうかを設定します。 <br />

       F. ユーザーアクセスの認可を Unauthorized、Authorized、または All に設定します。 <br />

       1. **Unauthorized**。権限は付与されません。 <br />
       2. **Authorized**。必要な権限を選択します。 <br />
       3. **All**。既存および今後追加されるすべての権限が含まれます。 <br />

       G. クライアント認証情報アクセスの認可を Unauthorized、Authorized、または All に設定します。 <br />

       1. **Unauthorized**。権限は付与されません。 <br />
       2. **Authorized**。必要な権限を選択します。 <br />
       3. **All**。既存および今後追加されるすべての権限が含まれます。 <br />

       H. **任意**。Invitation Landing Client を設定します。 <br />

       1. Organization のメンバーがメンバー招待を承諾する際にリダイレクトされるクライアントを選択します。アプリケーションが `create:my_org:member_invitations` スコープを使用する場合は必須です。 <br />

       I. **任意**。Permission Guardrails を設定します。 <br />

       1. Enforce Permission Ceiling: 有効にすると、Organization 管理者は自身の実効権限を超える権限を持つロールを割り当てられなくなります。 <br />
       2. Enforce Self-Assignment Restriction: 有効にすると、Organization 管理者は自分自身にロールを割り当てられなくなります。 <br />
    5. **Save** を選択します。
  </Tab>
...
## アクセストークンを生成する {#generate-an-access-token}

My Organization API は、サポートされている [OAuth 2.0フロー](/docs/ja-jp/get-started/authentication-and-authorization-flow) のいずれかを通じて取得した、ユーザーに紐付けられたアクセストークンでのみ呼び出せます。
...
### 認可コードフローの例 {#example-with-authorization-code-flow}
...
### PKCE を使用した Authorization Code フローの例 {#example-with-authorization-code-flow-with-pkce}
...
### IDトークン内のOrganization管理者権限 {#organization-admin-permissions-in-id-token}

ユーザーベースの認可リクエストに`/my-org`オーディエンスが指定されている場合、IDトークンには`urn:auth0:my_org_current_user_permissions` claimが自動的に含まれます。このclaimはマシンツーマシンフローでは利用できません。このclaimには、認証済みユーザーが割り当てられたロールを通じて保持するすべての`my_org:*`権限が含まれ、Embeddable UI Componentsが表示するアクションを判断する際に使用されます。
...
| `create:my_org:member_invitations`              | organizationのメンバー招待を作成する                               |
| `read:my_org:member_invitations`                | organizationのメンバー招待を参照する                               |
| `delete:my_org:member_invitations`              | organizationのメンバー招待を削除する                               |
| `read:my_org:members`                           | organizationのメンバーを一覧表示する                               |
| `delete:my_org:memberships`                     | 元になるユーザーを削除せずに、organizationからメンバーを削除する (メンバーシップを削除する)  |
| `read:my_org:member_roles`                      | organizationメンバーに割り当てられたロールを一覧表示する                     |
| `create:my_org:member_roles`                    | organizationメンバーにロールを割り当てる                             |
| `delete:my_org:member_roles`                    | organizationメンバーからロールの割り当てを解除する                        |
| `read:my_org:user_stores`                       | organizationのユーザーストアを参照する                              |

## エンドポイントリファレンス {#endpoint-reference}

My Organization API は、Configuration、組織の詳細、アイデンティティプロバイダー、ドメイン、プロビジョニング設定、SCIM トークン、メンバー招待、メンバー、メンバーロール、ユーザーストアに関するエンドポイントをサポートしています。スキーマやエラーコードなどを含むエンドポイントの完全なリファレンスについては、[API Explorer](https://auth0.com/docs/api/myorganization)を参照してください。

## SDK リファレンス {#sdk-reference}
...
### Organization ごとのレート制限 {#per-organization-rate-limits}
...
## クロスオリジン リクエスト {#cross-origin-requests}
...
これらのイベントコードを使用すると、API で管理されるすべてのリソース、具体的には設定、組織の詳細、IdP、ドメイン、メンバー、メンバー招待、メンバーロール、ユーザーストアにわたるアクティビティを追跡できます。ログイベントのスキーマの詳細については、[GitHubリポジトリ](https://github.com/auth0/auth0-log-schemas)を参照してください。
...
| `my_organization_api_member_succeeded`      | My Organization API Member Succeeded            | My Organization API サービスのメンバーリソースに対する API 呼び出しが成功したことを示します。ロールの割り当てと削除を含むすべてのメンバー操作が対象です。 |
| `my_organization_api_member_failed`         | My Organization API Member Failed               | My Organization API サービスのメンバーリソースに対する API 呼び出しが失敗したことを示します                                |
| `my_organization_api_invitation_succeeded`  | My Organization API Invitation Succeeded        | My Organization API サービスの招待リソースに対する API 呼び出しが成功したことを示します                                  |
| `my_organization_api_invitation_failed`     | My Organization API Invitation Failed           | My Organization API サービスの招待リソースに対する API 呼び出しが失敗したことを示します                                  |
| `my_organization_api_role_failed`           | My Organization API Role Failed                 | My Organization API サービスのロールリソースに対する API 呼び出しが失敗したことを示します                                 |
| `my_organization_api_user_stores_failed`    | My Organization API User Stores Failed          | My Organization API サービスのユーザーストアリソースに対する API 呼び出しが失敗したことを示します                             |

## 組織の接続の所有権 {#organization-connection-ownership}
...
| `organization_access_level`        | **Enum (null, none, readonly, limited, full)**。My Organization API の利用時にユーザーが持つアクセスの種類を決定します: <p /> 1. `null` <p /> 2. `none` - Tenant Administrator によって割り当てられた関連付けです。この接続は My Organization API 経由では表示も編集もできません。 <p /> 3. `readonly` - Tenant Administrator によって割り当てられた関連付けです。この接続は表示できますが、My Organization API 経由で編集することはできません。 <p /> 4. `limited` - Tenant Administrator によって割り当てられた関連付けです。My Organization API 経由で限定的な情報の参照と変更が可能です。 <p /> 5. `full` - Tenant Administrator によって割り当てられた関連付けです。My Organization API の利用者は、My Organization API の利用時に追加の制限なく接続を変更できます。注: organization&#95;access&#95;level にかかわらず、My Organization API による変更には常に Connection Profile による制限が適用されます。                                                                                   |
...
| `organization_member_access_level` | **Enum (none, readonly, limited, full)。** この接続を介して認証されたメンバーに対して Organization Admin が実行できる操作を制御します。組織ごとに Tenant Admin のみが設定できます。同じ基盤となる接続でも、異なる組織間で異なる値を設定できます。 <p /> 1. `none` — メンバーは Organization Admin には表示されません。すべての操作で 404 Not Found が返されます。 <p /> 2. `readonly` — Organization Admin はメンバーを表示および一覧表示し、そのロールを表示できます。変更は許可されません。 <p /> 3. `limited` — すべての読み取り専用操作に加え、組織からメンバーを削除し、ロールを割り当てまたは削除できます。 <p /> 4. `full` — すべての限定操作に加え、メンバーの基盤となるユーザーアカウントを削除し、メンバー属性にパッチを適用できます (User Attribute Profile によって管理されます) 。 <p /> デフォルト: `organization_access_level` が `full` の場合、`organization_member_access_level` のデフォルトは `full` です。それ以外の場合、デフォルトは `limited` です。Tenant Admin は `PATCH /api/v2/organizations/{id}/connections/{connectionId}` を介していつでもこの値を上書きできます。 |
...
* `organization_member_access_level` は、Management API 経由、または [Auth0 Dashboard &gt; Organizations](https://manage.auth0.com/#/organizations) の接続設定から構成できます。
