...
## My Account API を有効化する {#activate-the-my-account-api}
...
### デフォルトポリシーを有効にする {#enable-default-policy}
...
### 認証要件の階層 {#authentication-requirements-hierarchy}
...
### デフォルトポリシーの動作 {#default-policy-behavior}
...
## access token を取得する {#get-an-access-token}
...
### アクセストークンの例 {#access-token-examples}

#### 認可コードフローでのUniversal Login {#universal-login-with-authorization-code-flow}
...
#### Native パスキーを使用する埋め込みログイン {#embedded-login-with-native-passkeys}
...
## 認証方法を管理する {#manage-authentication-methods}
...
### 認証方法の管理例 {#manage-authentication-method-examples}

#### TOTP 認証器を登録する {#enroll-a-totp-authenticator}
...
#### 認証方法を一覧表示 {#list-authentication-methods}
...
#### 認証方法をDelete {#delete-an-authentication-method}
