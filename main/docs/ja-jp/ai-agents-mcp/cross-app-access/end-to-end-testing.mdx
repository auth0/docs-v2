...
XAAフローは[手動](#test-manually)でテストするか、[オンラインサンドボックス環境](#recommended-test-using-our-online-sandbox-environment)を使ってテストできます (推奨) 。XAAフローのテストが完了したら、[Resource AppをOkta Integration Network (OIN) に公開](#publish-your-resource-app-in-oin)して、エンタープライズ顧客がアプリを見つけやすくしましょう。

## &#x20;(推奨) オンラインサンドボックス環境でテストする {#recommended-test-using-our-online-sandbox-environment}

ブラウザーで Cross App Access をテストする場合は、[XAA.dev](https://xaa.dev/) の利用をおすすめします。[手順](https://developer.okta.com/blog/2026/07/03/cross-app-access-saml#configure-your-xaa-saml-app-in-okta)に従って、XAA.dev を使って Resource App をテストしてください。XAA.dev はホスト型の Requesting App として機能し、IdP 接続に応じて OIDC または SAML 経由で ID-JAG を取得して、Resource App と交換できます。

## 手動でテストする {#test-manually}

### ID-JAG を取得する {#obtain-the-id-jag}

ID-JAG の取得方法は、Requesting App がエンタープライズ IdP に対して SAML と OIDC のどちらを使用して認証するかによって、2 つに分かれます。

#### SAML Requesting App {#saml-requesting-app}
...
<div align="center">
  SAML レスポンス =&gt; SAML アサーション =&gt; refresh&#95;token =&gt; ID-JAG
</div>
...
#### OIDC Requesting App {#oidc-requesting-app}
...
### ID-JAG を Auth0 の `/token` エンドポイントに送信する {#send-id-jag-to-auth0s-token-endpoint}
...
## Resource AppをOINで公開する {#publish-your-resource-app-in-oin}

B2BおよびSaaSのproviderはResource Appを[Okta Integration Network (OIN)](https://www.okta.com/integrations/)に公開し、エンタープライズ顧客へのリーチを広げることができます。方法については、[How to Build and List Secure Cross App Access Connections on OIN](https://developer.okta.com/blog/2026/07/06/submit-oin-xaa)の手順に従ってください。

Resource AppをOINに公開する際に必要なフィールドの一覧は、Auth0ログの[idjag-exchange-succeeded](https://auth0.com/docs/tenant-logs/auth-token-exchange-success/idjag-exchange-succeeded)で確認してください。
