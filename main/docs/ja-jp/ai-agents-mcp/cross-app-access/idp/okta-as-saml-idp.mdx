...
## Okta テナントを作成して設定する {#create-and-configure-your-okta-tenant}
...
### Okta で Requesting App を作成する {#create-requesting-app-in-okta}
...
### Requesting App をテストユーザーに割り当てる {#assign-requesting-app-to-test-users}
...
### Okta で Resource App を作成する {#create-resource-app-in-okta}
...
### テストユーザーにResource Appを割り当てる {#assign-resource-app-to-test-users}
...
### Resource App で Cross-app Access (XAA) を有効にする {#enable-cross-app-access-xaa-in-resource-app}
...
## Okta に AI エージェントを登録する {#register-ai-agent-in-okta}

### AI エージェント を作成する {#create-ai-agent}
...
### エージェントの資格情報を追加する {#add-agent-credentials}
...
### エージェントの委譲を追加 {#add-agent-delegations}
...
### Agent Resource 接続を追加する {#add-agent-resource-connections}
...
### Agent を有効化する {#activate-the-agent}

**Agent&#39;s Actions** ドロップダウンで **Activate** を選択し、**MANAGED STATUS** が **Active** になっていることを確認します。

## Auth0 で SAML エンタープライズ接続を設定する {#configure-a-saml-enterprise-connection-in-auth0}
...
### Auth0 で接続をテストする {#test-connection-in-auth0}
...
## OIN で Resource App を公開する {#publish-your-resource-app-in-oin}
