...
sequenceDiagram
    ...
    participant Client
    participant Auth0
    User->>Client: User-initiated action
...
1. **User-initiated action**: The user initiates an action that requires the application to access an API.
...
<Note>
CIMD clients that use Private Key JWT authentication must [implement key rotation by generating a new key pair with a new, unique `kid`](#cimd-client-key-rotation-for-private-key-jwt-authentication).
</Note>
...
curl --location --request PATCH \
  'https://YOUR_AUTH0_DOMAIN/api/v2/clients/F8gx1EKvYaa54jOPmaLWpgoI90T' \
  --header 'Content-Type: application/json' \
  --header 'Authorization: Bearer YOUR_MANAGEMENT_API_TOKEN' \
  --data '{
