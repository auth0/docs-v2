...
<ReleaseStageNotice
  ...
  feature="Auth0 Dashboard Global Search"
  stage="beta"
  contact="Auth0 Support"
  terms="true"
/>

With the ability to have multiple configurations in one tenant, finding a specific application, user, or API could be difficult. Auth0 Dashboard provides intuitive search capabilities to mitigate long search times if your tenant has complex integrations.

Global Search is a command palette you can use to navigate and search with `CMD+K` for iOS or `CTRL+K` for Android. Global search options include:
...
<Frame>![Auth0 Dashboard Global Search example](main/docs/images/dashboard/dbsearch-test.png)</Frame>
