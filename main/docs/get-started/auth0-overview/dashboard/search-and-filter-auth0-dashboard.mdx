...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  **Search everything at once (Beta)**

  For a faster way to search across Applications, APIs, Organizations, and Users all at once, use Global Search — press Cmd+K (or Ctrl+K on Windows/Linux), or click the search icon in the top navigation bar from any dashboard page.
</Callout>

## How it works

The search bar on the [Applications](https://manage.auth0.com/#/applications), the [APIs](https://manage.auth0.com/#/apis), the [Roles](https://manage.auth0.com/#/roles), the [Custom Domains](https://manage.auth0.com/#/custom-domains-v2), [Organizations](https://manage.auth0.com/#/organizations), and the [Organization members](https://manage.auth0.com/#/organizations) section of Auth0 Dashboard uses a guided filter interface and Boolean search. The search bar functions as a dropdown menu that lists the filterable fields. When you select a field and enter a value, the filter applies the value as a removable tag above the results. You can combine up to five filters at a time.
...
| **Field** | **Description** | **Match type** |
|---|---|---|
| **Name** | The unique name identifier of the Organization | Partial match |
| **Display name** | The human-readable display name shown in the dashboard | Partial match |
| **ID** | The unique Organization identifier | Exact match |
...
| **App entitlement active** | Whether app entitlement is currently active for the organization | Boolean |

<Callout icon="file-lines" color="#0EA5E9" iconType="regular">                                                                                                                                                               
    **Third-party client access** and **App entitlement active** are Beta features and may not be available in all tenants.                                                                                                    
  </Callout>     
...
| **Field** | **Description** | **Match type** |
|---|---|---|
| **Name** | The name of the organization member | Partial match |
...
| **Field** | **Description** | **Match type** |
|---|---|---|
| **Domain** | The custom domain name | Partial match |
| **ID** | The unique custom domain identifier | Exact match |
| **Type** | The certificate type: Auth0-managed certificates or Self-managed certificates  | Enum selection |
