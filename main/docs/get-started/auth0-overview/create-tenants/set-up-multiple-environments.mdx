...
validatedOn: 2026-03-10
---
Development, staging, and production environments are easy to set up in Auth0. Simply create a new tenant for each environment to guarantee isolation between them. You can easily switch between tenants using the tenant selector dropdown in the top left of the Dashboard. You can also configure different administrators for each.
...
Through the [Management API v2](https://auth0.com/docs/api/management/v2), you can automate the migration of assets ([Actions](/docs/customize/actions), database [connections](/docs/authenticate/identity-providers), and so forth) between tenants.
...
The recommended way of working with code that you need to use/move from development to production is via [Actions](https://manage.auth0.com/#/actions) with environment variables or secrets. If you have not yet created a rule, you'll need to do so. (Otherwise, jump to step 4.)

1. Click **Create Your First Rule**.
2. Choose the **Empty rule** template from the list of available templates.
