...
redirects:
  - "/docs/best-practices/multi-tenant-apps-best-practices"
  - "/docs/saas-apps"
  - "/docs/tutorials/using-auth0-with-multi-tenant-apps"
  - "/docs/applications/concepts/multiple-tenants"
  - "/docs/design/using-auth0-with-multi-tenant-apps"
---
...
If Auth0 Organizations does not satisfy the requirements of your use case, you may consider the legacy solutions outlined below. For guidance on selecting the best approach for your specific requirements, please reach out to our [Professional Services](/docs/get-started/professional-services) team.

Legacy solutions include:
...
You'll need to track the tenants to which your users belong within your application. When a user logs into your application, you'll need to read that information and direct the user to the appropriate Auth0 application to complete authentication.

To enable a connection for multiple applications with the Auth0 <Tooltip tip="Management API: A product to allow customers to perform administrative tasks." cta="View Glossary" href="/docs/glossary?term=Management+API">Management API</Tooltip>, call the [Update enabled clients for a connection endpoint](https://auth0.com/docs/api/management/v2/connections/patch-clients), and pass the relevant connection ID.
...
This approach allows all of your users, regardless of which tenant they belong to, to log in using a uniform configuration (such as available connections).

To implement this, you could store tenant details in the user's Auth0 profile in the [`app_metadata` object](/docs/manage-users/user-accounts/metadata), using an identifier of your choice (for example, `"tenant": "customer-group-12345"`). After the user logs in, your application retrieves the `tenant` variable, and then displays a version appropriate to the returned value.
