...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td><code>https://auth0.com/llms.txt</code></td>
      <td>Concise summaries of all articles; best for quick context.</td>
    </tr>
    <tr>
      <td><code>llms-full.txt</code></td>
      <td><code>https://auth0.com/docs/llms-full.txt</code></td>
      <td>Full content of all articles; best for deep indexing.</td>
    </tr>
  </tbody>
</table>
...
2. Select **Add Connector** and enter the MCP Server URL: `https://auth0.com/docs/mcp`.
3. Select **Save**.

If successful, the Auth0 Docs MCP Server should show up under the list of available tools. To learn more, read OpenAI's [Connect from ChatGPT](https://developers.openai.com/apps-sdk/deploy/connect-chatgpt/) documentation.
...
If successful, the Auth0 Docs MCP Server should show up as a custom connector. To learn more, read Claude’s [Connect to remote MCP Servers](https://modelcontextprotocol.io/docs/tutorials/use-remote-mcp-server#connecting-to-a-remote-mcp-server) documentation.

</Tab><Tab title="Claude Code">
To connect to Claude Code to the Auth0 Docs MCP Server:
...
If successful, the Auth0 Docs MCP Server should show up under the list of available MCP servers. To learn more, read Claude’s [Installing MCP servers](https://docs.anthropic.com/en/docs/claude-code/mcp#installing-mcp-servers) documentation.
...
If successful, the Auth0 Docs MCP Server should show up under the list of available MCP servers. To learn more, read Codex’s [Model Context Protocol](https://developers.openai.com/codex/mcp) documentation.
...
2. Select **Add custom MCP** to edit your `mcp.json`.
...
To verify the setup, ask: `What tools do you have available?` in Cursor’s chat. Cursor should list the Auth0 Docs MCP Server as an available tool. To learn more, read Cursor’s [Installing MCP servers](https://cursor.com/docs/context/mcp#installing-mcp-servers) documentation.
...
4. Select **Global** or **Workspace**.
5. Press **Enter** to complete setup. Verify the Auth0 Docs MCP Server appears under **Installed MCP Servers** in your Extensions (⇧⌘X).
...
To verify the setup, ask: `What tools do you have available?` in Cascade’s chat. Cascade should list the Auth0 Docs MCP Server as an available tool. To learn more, read Windsurf’s [Model Context Protocol](https://docs.windsurf.com/plugins/cascade/mcp#model-context-protocol-mcp) documentation.
...
- Configure a Slack Social Connection. Use the Auth0 Docs MCP Server to find the correct way to map Slack's `team_id` into the user's ID token. Then, update my local user interface to include this `slack_team_id` field.
- How do I integrate Auth0 into an existing Vercel project?

### AI agents and MCP servers

- Add a tool call to my AI agent that can access the user’s Google calendar and answer questions about their schedule and meetings. Use the Auth0 Docs MCP Server for instructions on using Auth0 to secure login and the tool call to the user’s Google calendar.
