...
User → AWS Cognito → Auth0 B2B Connect → (domain discovery) → Enterprise IdP (for example, Okta)
...
- A Cognito App Client with an active Cognito Domain configured

## Configure Auth0 B2B Connect

Use the guided setup wizard to create a new B2B Connect integration for AWS Cognito.
...
1. Enter an **Integration Name** (for example, "AWS Cognito").
...
After the setup wizard completes, navigate to the **Settings** tab on the integration page. Copy the following values for use during your Cognito setup:
...
In the [AWS Cognito Console](https://console.aws.amazon.com/cognito/), select your user pool, navigate to **Authentication** in the left nav, select **Social and external providers**, and select **Add identity provider**.
...
1. Under **Attribute mapping**, select **Edit** and map the user attributes required by your application:
   ...
   - **OIDC**: Map `username` → `sub` and `email` → `email`
   - **SAML**: Map `email` → `http://schemas.xmlsoap.org/ws/2005/05/identity/claims/emailaddress`

2. Select **Save changes**.

### Allow users to sign in with the identity provider

1. In the AWS Cognito Console, select your user pool, navigate to **Applications** in the left nav, select **App clients**, and select your application.
