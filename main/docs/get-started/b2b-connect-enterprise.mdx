...
description: Use B2B Connect - Enterprise to add enterprise SSO, SCIM provisioning, and Universal Logout to your existing authorization server without replacing your    
...
<ReleaseStageNotice
    ...
    stage="ea"
    contact="your Technical Account Team or Auth0 Support"
    terms="true"
/>
...
B2B Connect - Enterprise supports the following [enterprise connections](/docs/authenticate/enterprise-connections):
* SAML
* OIDC
* Okta Workforce
...
<Frame>![Create B2B Integration wizard](/docs/images/dashboard/b2b-connect-wizard.png)</Frame>
...
3. Configure Auth0 as IdP: Provide values from your auth server so Auth0 can act as its identity provider:
    * OIDC: Issuer URL and your Application Callback URL. 
    ...
    Expand **Show individual endpoints** if your auth server does not support issuer discovery to get the Authorization URL, Token URL, Client ID, and Client Secret.
    </Callout>
...
B2B Connect - Enterprise supports third-party managed authorization servers that support federation over OIDC or SAML. Examples include:
...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
For information on the maximum number of SSEC profiles you can create per tenant, read [Self-Service Enterprise Configuration](/docs/authenticate/enterprise-connections/self-service-enterprise-configuration#how-it-works).
</Callout>
...
1. Select **+Create Organization** or select the **\<\>** symbol for API integration instructions.
...
6. Your customer administrator launches the self-service assistant via the ticket URL and follows the steps to configure their connection and complete domain verification.
...
Use [Auth0 Event](/docs/customize/events) to configure real-time notifications for lifecycle events, so you can keep your downstream identity store in sync with changes in Auth0. You can subscribe to events for users, Organizations, and enterprise connections.
...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
If you configure a [supported connection](#supported-enterprise-connections), you receive `connection.created`, `connection.updated`, and `connection.deleted` lifecycle events for all supported connection types. Use these events to keep your local domain map up to date as your B2B customers configure and modify their enterprise connections.
</Callout>
...
The B2B integration wizard configures your client with the organization and domain settings required for this to work automatically. For details on how login flows work with Organizations, read [Login flows for Organizations](/docs/manage-users/organizations/login-flows-for-organizations).

## Session management

### IPSIE session expiry (OIDC)

Auth0 B2B Connect - Enterprise supports the [IPSIE `session_expiry` claim](/docs/authenticate/enterprise-connections/session-expiry-enterprise-connections#configure-session-expiry-for-enterprise-connections-ipsie) for enterprise OIDC identity providers. When Auth0 receives a session expiry signal from an enterprise OIDC IdP, it includes the `session_expiry` claim in the token that returns to your authorization server. Use this claim to terminate downstream sessions in alignment with the enterprise IdP's session policy.

### DPoP

B2B Connect - Enterprise supports [Demonstrating Proof-of-Possession (DPoP)](/docs/secure/sender-constraining/demonstrating-proof-of-possession-dpop) for enterprise connections. DPoP binds tokens to the client that requested them, protecting against token theft and replay attacks.

If your existing authorization server also supports [DPoP](/docs/authenticate/enterprise-connections/enable-dpop-enterprise-connections), it can be used with the B2B Connect client when Auth0 is acting as the identity provider.

### Universal Logout

B2B Connect - Enterprise supports [Universal Logout](/docs/authenticate/login/logout/universal-logout) with Okta Workforce Identity, OIDC, and SAML connections you can enable in Auth0 Dashboard. Universal Logout cannot be configured by your B2B customers, but it must be enabled for them.

## Delegated administration

B2B Connect - Enterprise also supports delegated administration through the [My Organization API and UI components](/docs/manage-users/my-organization-api). With My Organization, you can build a self-service administration experience that lets your B2B customers manage their own Organization settings without contacting your team.

B2B customers using the My Organization interface can:
...
The My Organization UI components can be embedded in your existing admin portal or surfaced as a standalone page.

## Troubleshoot

### Connection list is empty

B2B integrations use [per-application access](/docs/manage-users/organizations/configure-organizations/grant-application-access#enable-per-application-access). If an Organization has enterprise connections enabled but has not been granted access to your B2B integration application, then your tenant administrators see no connections when viewing the Enterprise Connections tab in the Auth0 Dashboard.

To resolve this issue: navigate to **[Auth0 Dashboard > Organizations](https://manage.auth0.com/dashboard/#/organizations/list)** and select your application and navigate to the **Applications** tab. Verify the relevant B2B Integration is listed. If it is missing, you may need to **+ Add Application**.

### B2B customer did not receive self-service ticket

If a B2B customer does not receive their onboarding ticket, create a new ticket. In B2B Connect - Enterprise, navigate to the **[Organizations](https://manage.auth0.com/dashboard/#/b2b-integrations/organizations)** tab, select the relevant Organization, and generate a new onboarding ticket.

### Enterprise connection does not surface in the login flow

Review the following conditions:

1. Verify the enterprise connection is enabled on the correct Organization.
2. Confirm the Organization has been granted access to the B2B integration application (per-application access).
3. Check the user's email domain matches the verified domain associated with the Organization.
