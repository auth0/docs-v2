...
Using `audience` set to `urn:{yourDomain}:session_transfer` returns a Session Transfer Token instead of an access token, which your application can redeem to establish a web session as another user. Read [Session Delegation](/docs/authenticate/single-sign-on/session-delegation) to learn more.
