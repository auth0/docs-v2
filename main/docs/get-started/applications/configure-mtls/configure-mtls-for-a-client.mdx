---
description: Register a self-signed or CA-signed X.509 certificate on an Auth0 application to enable mTLS client authentication to the authorization server.
...
Because the certificate must be sent as a single-line JSON string value, every literal newline character in the PEM file must be JSON-escaped before you pass it to Auth0. Replace each line break with `\n` (or `\r\n` if the file uses Windows-style line endings). Many HTTP tools and libraries do not do this conversion for you, so verify the escaping before sending the request. For example:
...
* `$credential_certificate`: the content of the `cert.pem` file generated in the previous step
...
* `$client_name`**:** the name for the new client
* `$credential_name`**:** the name for the public key
* `$credential_certificate`: the content of the PEM file generated by the CA
...
Once you have [generated a key pair](/docs/secure/application-credentials/generate-rsa-key-pair) exclusively for mTLS, create the credential resource. Make the following POST request to the `/clients/$client_id/credentials` endpoint:
