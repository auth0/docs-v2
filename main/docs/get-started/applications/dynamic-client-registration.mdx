...
By default, Dynamic Client Registration is disabled for all tenants. To enable it:

<Tabs><Tab title="Auth0 Dashboard">

Go to [Dashboard > Settings > Advanced](https://manage.auth0.com/#/tenant/advanced) and enable **Dynamic Client Registration (DCR)**.

</Tab><Tab title="Management API">

Set the `enable_dynamic_client_registration` flag to `true` in your tenant's settings using the [`/api/v2/tenants/settings`](https://auth0.com/docs/api/management/v2#!/Tenants/patch_settings) endpoint.
...
To dynamically register an application, send an HTTP `POST` request to the registration endpoint: `https://{yourDomain}/oidc/register`. Because Auth0 supports Open Dynamic Registration, this endpoint accepts registration requests without an access token.
...
After registration, the application can initiate an [Authorization Code Flow with PKCE](/docs/get-started/authentication-and-authorization-flow/authorization-code-flow-with-pkce) using the `client_id` and configured `redirect_uris`. MCP clients such as Claude Code and VS Code use DCR to register automatically. To learn more, read [Register your MCP Client Application](https://auth0.com/ai/docs/mcp/guides/registering-your-mcp-client-application).

## Tenant Access Control List (ACL)

Auth0 provides [Tenant Access Control List (ACL)](/docs/secure/tenant-access-control-list) to manage traffic to the `/oidc/register` endpoint. You can restrict who can send DCR requests by configuring ACL rules based on:
...
The `/oidc/register` endpoint is rate-limited to **5 requests per second** per tenant. To learn more about rate limits and how to configure them, read [Rate Limit Configuration](/docs/troubleshoot/customer-support/operational-policies/rate-limit-configurations).
