...
To do so, go to [Dashboard > Settings > Advanced](https://manage.auth0.com/#/tenant/advanced) and enable **Dynamic Client Registration (DCR)**.
...
## DCR and third-party applications

All applications created through Dynamic Client Registration are third-party applications. DCR clients receive enhanced security controls including:

- `tpc_` client ID prefix
- PKCE enforcement for authorization code flows
- Only `authorization_code` and `refresh_token` grant types
- Restricted client properties
- Explicit API authorization required via [client grants](/docs/get-started/applications/application-access-to-apis-client-grants)

### API access for DCR clients

DCR clients need [default permissions for third-party applications](/docs/get-started/applications/application-access-to-apis-client-grants#default-permissions-for-third-party-applications) configured before they can access any APIs. Configure default permissions on each API that DCR clients need to access.

<Warning>
Before enabling DCR, configure default API permissions for third-party applications. Without them, dynamically registered clients cannot access any APIs.
</Warning>

MCP clients such as Claude Code and VS Code use DCR and are compatible with enhanced security controls. To learn more about MCP client registration, read [Register your MCP Client Application](https://auth0.com/ai/docs/mcp/guides/registering-your-mcp-client-application).

<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
Some customers may configure different security settings for DCR clients. To learn more, read [Permissive Mode for Third-Party Applications](/docs/get-started/applications/third-party-applications/permissive-mode).
</Callout>
