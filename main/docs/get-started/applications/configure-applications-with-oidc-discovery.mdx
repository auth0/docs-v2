...
redirects:
  - "/docs/configure/applications/configure-applications-with-oidc-discovery"
  - "/docs/protocols/configure-applications-with-oidc-discovery"
  - "/docs/oidc-rs256-owin"
  - "/docs/protocols/oidc/openid-connect-discovery"
  - "/docs/tutorials/openid-connect-discovery"
---
...
<Note>
When [Agents as Principal](/docs/ai-agents-mcp/agents-as-principal) is enabled for your tenant, the discovery document includes the `entity_profiles_supported` metadata field, which lists the client and subject [entity profiles](/docs/secure/tokens/access-tokens/access-token-profiles#claims) the tenant can issue. This field appears on both `/.well-known/openid-configuration` and its `/.well-known/oauth-authorization-server` alias.
</Note>
