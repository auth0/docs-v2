...
description: "Learn how Auth0 handles user consent when applications request access to APIs on behalf of users."
validatedOn: 2026-03-31
---
import {AuthCodeBlock} from "/snippets/AuthCodeBlock.jsx";
import {AuthCodeGroup} from "/snippets/AuthCodeGroup.jsx";

OAuth allows applications to access APIs on behalf of users. Before an application can act on a user's behalf, the user must explicitly approve the requested permissions. This approval step is called **consent**.

For [third-party applications](/docs/get-started/applications/third-party-applications), consent is always required — the user must approve every authorization request. For [first-party applications](/docs/get-started/applications/first-party-and-third-party-applications), consent can be skipped when configured, because you control the application and trust it to act appropriately.
...
For example, this authorization request:
...
Displays a consent dialog asking the user to approve the `read:posts` and `write:posts` permissions for the API.
...
By default, the consent page uses scope names to prompt for the user's consent. Define scopes using the `action:resource_name` format for clear display.
...
If a user rejects consent, Auth0 redirects to the `redirect_uri` with an `access_denied` error:
...
First-party applications can skip the consent dialog when the API has the **Allow Skipping User Consent** option enabled. Navigate to **Auth0 Dashboard > Applications > APIs > (select the API) > Settings > Access Settings** to configure this option.
