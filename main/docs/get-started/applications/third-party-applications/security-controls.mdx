...
Third-party applications must have an explicit grant, even when an API is configured with an **Allow All** policy. You can configure a per-application grant or a [default third-party permission](/docs/get-started/applications/application-access-to-apis-client-grants#default-permissions-for-third-party-applications).
