...
Third-party applications must have an explicit grant, even when an API is configured with an **Allow All** policy. You can configure per-application permissions or [default permissions for third-party applications](/docs/get-started/applications/application-access-to-apis-client-grants#default-permissions-for-third-party-applications).
...
Third-party applications support the `client_credentials` grant type for machine-to-machine access. This enables backend partner integrations and server-to-server API access without user involvement.

**Requirements and constraints:**

- Client type: The application must be a confidential client (`token_endpoint_auth_method` must not be `none`).
- [Organizations](/docs/manage-users/organizations): Supported. To learn more, read [Organizations](#organizations).
- Not available for applications created through [Dynamic Client Registration](/docs/get-started/applications/dynamic-client-registration) or [CIMD](/docs/get-started/auth0-overview/create-applications/register-applications-with-cimd).

**Extensibility:**

- [Actions](/docs/customize/actions) with the `credentials-exchange` trigger execute as usual for machine-to-machine access flows.

## Organizations

Third-party applications support [Organizations](/docs/manage-users/organizations) for both user flows and machine-to-machine access. Access is opt-in:

- **User flows (Authorization Code)**: The Organization must opt in via `third_party_client_access: allow`, and the connection used for login must be promoted to the [domain level](/docs/authenticate/identity-providers/promote-connections-to-domain-level). If either is missing, Auth0 returns `invalid_request`.
- **Machine-to-machine (Client Credentials)**: `allow_any_organization` is not available. Each Organization must be authorized through an explicit `organization_client_grant`.

To set this up, read [Configure Organizations access](/docs/get-started/applications/third-party-applications/configure-third-party-applications#configure-organizations-access).
...
| `grant_types` | Must be `authorization_code`, `refresh_token`, or `client_credentials` |
...
| **Hooks** (credentials-exchange) | Not supported. Tenants with an active `credentials-exchange` Hook will receive an error. Migrate to [Actions](/docs/customize/actions) for `credentials-exchange` extensibility. |
