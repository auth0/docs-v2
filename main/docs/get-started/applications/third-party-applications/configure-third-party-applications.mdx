...
Third-party applications can access a connection that isn't promoted to the domain level through [Cross App Access (XAA)](/docs/ai-agents-mcp/cross-app-access), as long as access is authorized in the enterprise IdP. Only promote a connection to the domain level if you want any third-party application to access it through both standard login flows (`/authorize` endpoint) and XAA. If you want XAA to be the only way third-party applications can get tokens through this connection, do not promote it to the domain level.
...
Organization Admins can also self-service configure third-party application access for their Organization by setting the `third_party_client_access` attribute through the My Organization API. To learn more, read [Configure client application attributes](/docs/manage-users/my-organization-api#configure-client-application-attributes).
