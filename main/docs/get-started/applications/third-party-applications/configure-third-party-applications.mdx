...
Third-party applications can also access a connection that isn't promoted to the domain level through [Cross App Access (XAA)](/docs/secure/call-apis-on-users-behalf/xaa), as long as that access is authorized in the corresponding Enterprise IdP. If you want all third-party access to a connection approved at the Enterprise IdP, don't promote it to the domain level.
