...
Create third-party applications to allow external developers, partners, or AI agents to access your APIs with enhanced security controls.
...
To create a third-party application using the Management API, make a `POST` request to the `/api/v2/clients` endpoint with the following request body: 
...
To configure the API access policy using the Management API, make a `POST` request to the `/api/v2/client-grants` endpoint with the following request body: 
...
Third-party applications can authenticate users through connections that are promoted to the domain level. Domain-level connections are available to all third-party applications in the tenant.
