...
When both exist for the same API, the per-application grant takes precedence over the default grant.

### Per-application grant

<Tabs><Tab title="Auth0 Dashboard">

To configure a per-application grant using the Auth0 Dashboard:
...
For per-application grants, you need to individually authorize API access for each application.
...
To configure a per-application grant, use the `/client-grants` endpoint.
