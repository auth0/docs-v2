...
### Auth0 acting as a SAML or WS-Fed IdP

When Auth0 acts as a <Tooltip tip="Security Assertion Markup Language (SAML): Standardized protocol allowing two parties to exchange authentication information without a password." cta="View Glossary" href="/docs/glossary?term=SAML">SAML</Tooltip> or <Tooltip tip="Web Service Federation (WS-Fed): Protocol for managing user identities across domains." cta="View Glossary" href="/docs/glossary?term=WS-Fed">WS-Fed</Tooltip> identity provider, rotating the signing key changes the certificate used to sign responses, and the change applies to every service provider on your tenant at once. You must update each service provider with the new certificate so it can keep validating Auth0's signed responses, otherwise logins break. This also changes the JWKS key used to validate tokens, so make sure your implementation does not assume signing keys never change. To prepare your service providers to trust the next certificate before you rotate, follow [Rotate the tenant signing key](/docs/authenticate/protocols/saml/saml-sso-integrations/sign-and-encrypt-saml-requests#rotate-the-tenant-signing-key).
