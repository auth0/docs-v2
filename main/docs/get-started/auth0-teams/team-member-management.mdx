...
Members with this team-level role can perform delegated tenant management tasks without requiring full Team Owner access. Tenant Managers can:

- View all tenants where they have access and the complete team member list.
- Create new tenants (they automatically become Tenant Admin on tenants they create).
- Add, update, or remove Contributors and Report Viewers on tenants where they hold the [Tenant Admin](/docs/get-started/auth0-teams/tenant-member-management) tenant member role.
- Delete tenants where they hold the Tenant Admin tenant member role.

Tenant Managers cannot access Team Settings, Security Policies, Activity Logs, Teams API, or modify access for Team Owners or other Tenant Managers.

<Callout icon="file-lines" color="#0EA5E9" iconType="regular">

The Tenant Manager is a **team-level** role assigned in the Teams Dashboard. Tenant Admin is a separate **tenant-level** role assigned per tenant. A Tenant Manager must also be a Tenant Admin on a given tenant to delete that tenant, view its members, or manage its member access. To learn more about tenant member roles, read [Tenant Member Management](/docs/get-started/auth0-teams/tenant-member-management).
...
¹ Tenant Managers can only perform this action on tenants where they also hold the **Tenant Admin** tenant member role.
