...
sequenceDiagram
    ...
    participant CustAuthServer as Customer Auth Server
    participant Auth0 as Auth0 Enterprise Connect
    participant IdP as Enterprise IdP

    User->>App: login
    App->>CustAuthServer: Existing Authentication request
    CustAuthServer->>CustAuthServer: Auth0 Enterprise Connect User
    CustAuthServer->>Auth0: Authentication request (SAML or OIDC)
    Auth0->>IdP: Authentication request <br/> (e.g. Okta, OIDC, SAML, Entra ID, ADFS, Google WorkSpace)
    User->>IdP: Authenticates with corporate credentials
    IdP-->>Auth0: Returns assertion or token
    Auth0-->>CustAuthServer: Returns assertion or token
    CustAuthServer-->>App: Returns existing Auth Server token
    Note over App: Validates token and <br/>bootstraps own session
    App-->>User: Access granted
```

1. The user initiates login in the Customer App.
2. The Customer App sends an authentication request to the Customer Auth Server using its existing authentication flow.
3. The Customer Auth Server identifies the user as an Auth0 Enterprise Connect user and routes the request to Auth0 Enterprise Connect.
4. Auth0 Enterprise Connect sends an authentication request to the enterprise identity provider (IdP) using SAML or OIDC (for example, Okta, Entra ID, ADFS, or Google Workspace).
5. The user authenticates with their corporate credentials at the enterprise IdP.
6. The enterprise IdP returns a SAML assertion or OIDC token to Auth0 Enterprise Connect.
7. Auth0 Enterprise Connect returns the assertion or token to the Customer Auth Server.
8. The Customer Auth Server returns its own token to the Customer App.
9. The Customer App validates the token and bootstraps its own session.
10. The user is granted access to the Customer App.
...
### Update an existing tenant with Auth0 Dashboard
