...
- `OrganizationMemberManagement`—Pre-configured container with built-in data fetching and modals.
- `OrganizationMemberManagementView`—Stateless view layer; bring your own data via `useOrganizationMemberManagement`.

When installed via shadcn, the subcomponent source files (`OrganizationMemberTable`, `OrganizationInvitationTable`, `OrganizationInvitationCreateModal`, etc.) are located in your local project directory and can be modified directly.

### Available hooks

- `useOrganizationMemberManagement`—Full data and interaction layer: tab state, member and invitation queries, modal state, and all event handlers.
