...
This component is the per-member drill-down for the [`OrganizationMemberManagement`](/docs/get-started/universal-components/web/components/organization-member-management) component. Wire that component's `viewMemberDetailsAction` to the route that renders `OrganizationMemberDetail`, and the same `userId` flows through to this component's required `userId` prop.
