...
<Callout icon="info" color="#0EA5E9" iconType="regular">
  In the current Early Access release, `hideHeader` does not hide the Member Detail header.
</Callout>
...
Fires when clicking the header back button and automatically after member removal. Wire this callback to your router to return to the member list.
...
- `OrganizationMemberDetail`—Pre-configured container with built-in data fetching and modals.
- `OrganizationMemberDetailView`—Stateless view layer; bring your own data via `useOrganizationMemberDetail`.

When installed via shadcn, the subcomponent source files (`OrganizationMemberEditDetailsTab`, `OrganizationMemberEditRolesTab`, etc.) are located in your local project directory and can be modified directly.

### Available hooks

- `useOrganizationMemberDetail`—Full data and interaction layer: member query, role queries, modal state, and all event handlers.
