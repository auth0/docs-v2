...
description: Describes how to use Universal Components to build self-service account security interfaces with Auth0's My Account API.
...
The [My Account API](/docs/manage-users/my-account-api) enforces low rate limits, especially on free-tier tenants. This may cause errors while using these components.
</Callout>
...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
The install commands also add the `@auth0/universal-components-core` dependency for shared utilities and Auth0 integration.
</Callout>
...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
The installs the component source into `src/components/auth0/my-account/` along with all UI dependencies and the `@auth0/universal-components-core` dependency for shared utilities and Auth0 integration.
</Callout>
