...
The `UserMFAMgmt` component lets users enroll, view, and delete their own multi-factor authentication factors in a single card-based interface. The component renders a list of all factor types enabled on the tenant (authenticator application (TOTP), SMS, email one-time password (OTP), push notification, WebAuthn platform/roaming authenticator, and recovery code), handles the multi-step enrollment flow for each, and shows a delete confirmation before removing an active factor. It loads its data from the My Account API and requires no props to render.
