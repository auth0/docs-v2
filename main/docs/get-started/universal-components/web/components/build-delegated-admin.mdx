...
| **OrganizationMemberManagement** — Manage organization members, invitations, and role assignments. | `/my-org/members`, `/my-org/invitations` |
| **OrganizationMemberDetail** — View and manage an individual member's profile and assigned roles. | `/my-org/members/{user_id}`, `/my-org/members/{user_id}/roles` |

## Permission-aware controls

My Organization components automatically use the signed-in administrator's My Organization permissions to adapt available controls. Depending on the component and action, unavailable controls are hidden or disabled with explanatory feedback. Mutation handlers also apply client-side permission checks before making requests.

In React SPA applications, the components read the `urn:auth0:my_org_current_user_permissions` claim from ID token claims. In Next.js applications that use proxy mode, they read permissions from the proxy profile response. Access tokens authorize requests to the My Organization API, which remains the authoritative authorization boundary.

The `readOnly` prop is a separate application-level control that disables mutations even when the administrator has the required permissions. Member mutations also require the target member to have a `limited` or `full` access level.

| Component area | Permissions used for gated operations |
| :--- | :--- |
| Organization details | `update:my_org:details` |
| Domains | `create:my_org:domains`, `update:my_org:domains`, `delete:my_org:domains`, `create:my_org:identity_providers_domains`, `delete:my_org:identity_providers_domains` |
| SSO providers | `create:my_org:identity_providers`, `update:my_org:identity_providers`, `delete:my_org:identity_providers`, `update:my_org:identity_providers_detach`, plus the domain, provisioning, and SCIM-token permissions required by the selected configuration |
| Members and invitations | `create:my_org:member_invitations`, `delete:my_org:member_invitations`, `create:my_org:member_roles`, `delete:my_org:member_roles`, `delete:my_org:memberships` |
...
## Configure application access
  ...
  3. Configure the following settings:
     - [Connection Profile](/docs/authenticate/enterprise-connections/connection-profile): Select or create a profile with connection attribute mappings.
     - [User Attribute Profile](/docs/authenticate/enterprise-connections/user-attribute-profile): Select or create a profile with user attribute mappings.
     - Supported [Identity Providers](/docs/authenticate/identity-providers): Enable providers your customers can use.
...
## Set up the role
  ...
  1. Create a role (for example, "Organization Admin").
...
My Organization components require an access token for the My Organization API audience. For React SPA configuration, see [Configure Auth0ComponentProvider](/docs/get-started/universal-components/web/auth0-component-provider#use-with-my-organization-components).
