...
<Callout icon="triangle-exclamation" color="#F59E0B" iconType="regular">
  Setting `--primary` or `--color-primary` has no effect on components. The compiled stylesheet reads `--auth0-*` variables directly.
</Callout>
...
Define your design tokens and expose them to Tailwind utilities with `@theme inline`. Then forward to the component namespace — one value, two names:
...
<Callout icon="triangle-exclamation" color="#F59E0B" iconType="regular">
  The `/tailwind` export (`@auth0/universal-components-react/tailwind`) is a raw source scan file used internally by the library's build. Always import `/styles` for component CSS.
</Callout>
...
:root {
  ...
  /* --auth0-primary: oklch(50% 0.2 265); */
}
```
