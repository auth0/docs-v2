...
<CodeGroup>
```bash npm  wrap lines
npm install @auth0/universal-components-react react-hook-form @tanstack/react-query @auth0/auth0-react
```

```bash pnpm wrap lines
pnpm add @auth0/universal-components-react react-hook-form @tanstack/react-query @auth0/auth0-react
```

<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
Installs the package alongside `react-hook-form` and `@tanstack/react-query` (peer dependencies), and `@auth0/auth0-react` (required for client-side SPA authentication). The Next.js tab uses proxy mode and does not need `@auth0/auth0-react`.
</Callout>
</CodeGroup>
...
```bash wrap lines
pnpm dlx shadcn@latest add https://auth0-universal-components.vercel.app/r/my-organization/organization-details-edit.json
```
