...
The default theme renders the Auth0 default theme, you can provide your own theme to match your branding.
...
If you do not configure a theme, Universal Components for iOS render the Auth0 default theme. The following example displays the account methods view without any customization:
...
You can provide your own branding theme that implements the three color category properties and wire them into an `Auth0ColorTokens` container:
...
You can access the `@Environment(\.auth0Theme)` property wrapper to apply the same tokens to any Swift view:
