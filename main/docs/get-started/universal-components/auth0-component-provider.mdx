...
## Import `Auth0ComponentProvider`

The `Auth0ComponentProvider` must be nested in your authentication provider (`Auth0Provider`) to allow access to authentication state and token acquisition methods required to call Auth0 APIs.
...
For detailed styling examples and customization patterns, read the [Customize Style and Themes with Universal Components](/docs/get-started/universal-components/universal-components-style).
</Callout>
    ...
    <table class="table">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>No</td>
          <td>Custom authentication context for SPAs not using `@auth0/auth0-react`. Provides auth functions (getAccessTokenSilently, etc.).</td>
        </tr>
        <tr>
          <td><code>previewMode</code></td>
          <td><code>boolean</code></td>
