---
'og:description': User provisioning functionality and considerations for your B2C
  IAM  implementation.
...
Auth0 Database Connection types can also be configured to proxy an existing (legacy) identity store. If you need to keep user identities defined in your own legacy store - for example, if you have one or more business critical applications that you can’t migrate to Auth0, but which still need access to these identities - then you can easily integrate with Auth0. See [Authenticate Users Using Your Database](/docs/authenticate/identity-providers/database/custom-db) for more information.

## Self sign up

Self sign up leverages Auth0 [Database Connections](/docs/authenticate/identity-providers/database) to store the user ID, password, and (optional) username identity information collected from new users during the sign up process. Database connection policies governing things such as minimum [username length](/docs/authenticate/identity-providers/database/require-username) or [password strength and complexity](/docs/authenticate/identity-providers/database/password-options) can be configured via the <Tooltip tip="Auth0 Dashboard: Auth0's main product to configure your services." cta="View Glossary" href="/docs/glossary?term=Auth0+Dashboard">Auth0 Dashboard</Tooltip>.
