...
For example, you could define the mappings section of a SAML enterprise connection:
...
In this example, each field is assigned a value in the `user` object in an Action, which maps to standard claims in an identity token or can be mapped using custom claims. To learn more customizing SAML mappings, read [Connect Your App to SAML Identity Providers: Set up mappings](/docs/authenticate/identity-providers/enterprise-identity-providers/saml#set-up-mappings).

When your application uses SAML and Auth0 acts as the IdP, you can include an organization in the SAML sign-in request using the organization query parameter:

```js wrap lines
https://{YOUR_DOMAIN}/samlp/{YOUR_APP_CLIENT_ID}?organization={YOUR_ORGANIZATION_ID}
```

Auth0 returns the organization in the SAML response as a custom attribute:

```js wrap lines
http://schemas.auth0.com/org_id/YOUR_ORGANIZATION_ID
```

Any non-standard claims that would be included in an OIDC token such as, custom claims set in an Action, are also included in the SAML response, prefixed with `http://schemas.auth0.com/`.

For IdP initiated flows, pass the same `organization` parameter directly to the `/samlp/{YOUR_APP_CLIENT_ID}` endpoint without a SAML request.
