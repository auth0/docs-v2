...
In this example, each field is assigned a value in the `user` object in an Action, which maps to standard claims in an identity token or can be mapped using custom claims. To learn more about customizing SAML mappings, read [Connect Your App to SAML Identity Providers: Set up mappings](/docs/authenticate/identity-providers/enterprise-identity-providers/saml#set-up-mappings).
