...
1. [Register `authorization_details` types](#register-authorization-details-types) for the resource server.
2. [Configure the API Access policies](#configure-api-access-policies) to allow your app to request those types.
2. [Set the customized consent prompt](#set-customized-consent-prompt) to render the `authorization_details`.
3. Optional: [Configure the consent policy for the resource server](#configure-consent-policy-for-the-resource-server).
...
If you’re using the Auth0 Guardian app, then the `authorization_details` parameter value must:

 * Have only one object in the array
 * That object must conform to our the following Auth0 schema:
...
<tr>
<th><strong>Which endpoint is being used?</strong></th>
<th><strong>Does the request include `authorization_details`?</strong></th>
<th><strong>Is a Guardian Push notification sent?</strong></th>
...
<td>No</td>
<td>No</td>
<td>Standard consent is shown.</td>
</tr>
<tr>
<td>Any</td>
<td>Yes</td>
...
<td>
  ...
  <p>
    If the Auth0 Guardian app is used, it will automatically display the `authorization_details` to the user, hence why the more restrictive Auth0 schema is required when using the Guardian app.
  </p>
  <p>
    If a custom mobile app is used, the `authorization_details` can be retreived using the Auth0 Guardian SDK and render them using their own UI.
  </p>
</td>
</tr>
<tr>
...
<td>
  <p>
    If the Auth0 Guardian App is being used to authorize the CIBA request, the `authorization_details` will be fetched automatically and displayed, hence why the more restrictive Auth0 schema is required when using the Guardian app.
  </p>
  <p>
  ...
  <p>
    Customers MAY also choose to trigger a push notificaiton as a second factor for the CIBA request, in which case the behavior will be the same as above, the Auth0 Guardian app will automatically display the `authorization_details` to the user again, while custom mobile apps can choose to retrieve the `authorization_details` using the Auth0 Guardian SDK and render them using their own UI.
  </p>
</td>
</tr>
</tbody>
...
Customers can also set the `consent_policy` to `transactional_authorization_with_mfa`, which has the following behavior (CIBA flow is not supported with `transactional_authorization_with_mfa` consent policy):
...
<td>Yes</td>
<td>No</td>
<td>The customized consent prompt is shown (after the use fulfills any non push MFA factors).</td>
</tr>
<tr>
<td>Yes</td>
<td>Yes</td>
