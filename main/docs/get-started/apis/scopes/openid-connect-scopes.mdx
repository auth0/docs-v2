...
Standard claims are intended to provide an application with user details, such as name, email, and picture, and are pre-defined for the OIDC protocol. These claims are returned in an ID Token and are also available through the [`/userinfo` endpoint](/docs/api/authentication/user-profile/get-user-info). Depending on how your API and application are configured, standard claims can also be included in Access Tokens.
...
Scope-based filtering applies regardless of how a claim ends up on the token. If the corresponding scope (for example, `email`) is not included in the request, the claim is removed from the issued token(s) even if your application or a custom Action explicitly sets it — for example, via `setUserById()`/`setUserByConnection()` in a [Custom Token Exchange](/docs/authenticate/custom-token-exchange) Action, or via a custom claims Action. This applies to ID Tokens, the `/userinfo` response, and Access Tokens alike.
</Callout>
