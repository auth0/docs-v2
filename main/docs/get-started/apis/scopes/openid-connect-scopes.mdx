...
Standard claims are intended to provide an application with user details, such as name, email, and picture, and are pre-defined for the OIDC protocol. These claims are returned in an ID Token and are also available through the [`/userinfo` endpoint](/docs/api/authentication/user-profile/get-user-info). Depending on how your API and application are configured, you can also standard claims in access tokens.
...
Scope-based filtering applies regardless of how a claim ends up on the token. If a scope is not included in the request, the corresponding claim is removed from the issued token(s) even if your application or a custom Action explicitly sets it. For example, if `email` is not requested, the `email` claim is removed even if set by `setUserById()` or `setUserByConnection()` in a [Custom Token Exchange](/docs/authenticate/custom-token-exchange) Action or a custom claims Action. This applies to ID tokens, access tokens, and the `/userinfo` response.
</Callout>
