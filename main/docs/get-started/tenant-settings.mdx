...
* **Resource Parameter Compatibility Profile (Early Access)**: When enabled, Auth0 uses the `resource` parameter to specify which resource server (API) a client application wants to access. When disabled, which is the default, Auth0 uses the `audience` parameter to specify the resource server. When enabled, Auth0 first checks the `audience` parameter, and if not available, it uses the `resource` parameter. The value of the `resource` parameter must be an absolute URI and can be forwarded to an upstream identity provider.
