---
description: Lock.Android - Native Social Authentication
title: 'Lock.Android: Native Social Authentication'
redirects:
  - "/docs/libraries/lock-android/v1/native-social-authentication"
  - "/docs/libraries/lock-android/v2/native-social-authentication"
---
...
The Lock-Google is available through [Maven Central](https://central.sonatype.com/). To install it, simply add the following line to your `build.gradle`:

`compile 'com.auth0.android:lock-google:1.+'`

You can check for the latest version on the repository [Readme](https://github.com/okta-graveyard/Lock-Google.Android#install) or in [Maven](https://central.sonatype.com/artifact/com.auth0.android/lock-google).
...
3. Complete the "Client ID" field with the `CLIENT ID` value obtained in the step 4 of the **Google Cloud Console** section above.
4. Complete the "Client Secret" field with the `CLIENT SECRET` value obtained in the step 4 of the **Google Cloud Console** section above.
5. Complete the "Allowed Mobile Client IDs" field with the `CLIENT ID` obtained in the step 5 of the **Google Cloud Console** section above.
...
1. In your android application, create a new String resource in the `res/strings.xml` file. Name it `google_server_client_id` and set as value the `CLIENT_ID` obtained in the step 5 of the **Google Cloud Console** setup section above.
...
If you need further help with the Google SDK setup, please check Google's [Sign-In for Android Guide](https://developers.google.com/identity/android-credential-manager).
...
This provider doesn't require any special Android Manifest Permissions to authenticate the user. But if your use case requires them, you can let the AuthProvider handle them for you. Use the `setRequiredPermissions` method.
...
If you need assistance, you can follow this [Keystores Guide](/docs/libraries/lock-android/android-development-keystores-hashes) to acquire those values.
...
[Lock-Facebook](https://github.com/okta-graveyard/Lock-Facebook.Android) requires Android API 15 or later & Facebook Android SDK 4.+

## Latest version

The Lock-Facebook is available through [Maven Central](https://central.sonatype.com/). To install it, simply add the following line to your `build.gradle`:

`compile 'com.auth0.android:lock-facebook:3.+'`

You can check for the latest version on the repository [Readme](https://github.com/okta-graveyard/Lock-Facebook.Android#install) or in [Maven](https://central.sonatype.com/artifact/com.auth0.android/lock-facebook).
