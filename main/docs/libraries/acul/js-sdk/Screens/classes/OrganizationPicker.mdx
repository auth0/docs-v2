---
title: "OrganizationPicker"
---

<Frame><img style={{maxHeight:"400px"}} src="/docs/images/cdy7uua7fh8z/3aFR1VujjNlIauzRAuMapf/12e80cb2e3f131d2eb45aa0ca13f1d8c/Screenshot_2025-03-25_at_20.49.09.png" alt=""/></Frame>

Class implementing the organization-picker screen functionality.
This screen allows users to select an organization from a list of available organizations.

```ts Example
import OrganizationPicker from '@auth0/auth0-acul-js/organization-picker';

const organizationPicker = new OrganizationPicker();
organizationPicker.selectOrganization({
  organization: 'org_XXXXXXXXXXXXXXX'
});
```

## Constructors

<ParamField body="OrganizationPicker" type="Constructor">

Creates an instance of OrganizationPicker screen manager.

</ParamField>
## Properties

<ParamField body='branding' type={<span><a href="/docs-v2/main/docs/libraries/acul/js-sdk/Screens/interfaces/BrandingMembers">BrandingMembers</a></span>}/>

<ParamField body='client' type={<span><a href="/docs-v2/main/docs/libraries/acul/js-sdk/Screens/interfaces/ClientMembers">ClientMembers</a></span>}/>

<ParamField body='organization' type={<span><a href="/docs-v2/main/docs/libraries/acul/js-sdk/Screens/interfaces/OrganizationMembers">OrganizationMembers</a></span>}/>

<ParamField body='prompt' type={<span><a href="/docs-v2/main/docs/libraries/acul/js-sdk/Screens/interfaces/PromptMembers">PromptMembers</a></span>}/>

<ParamField body='screen' type={<span><a href="/docs-v2/main/docs/libraries/acul/js-sdk/Screens/interfaces/ScreenMembers">ScreenMembers</a></span>}/>

<ParamField body='tenant' type={<span><a href="/docs-v2/main/docs/libraries/acul/js-sdk/Screens/interfaces/TenantMembers">TenantMembers</a></span>}/>

<ParamField body='transaction' type={<span><a href="/docs-v2/main/docs/libraries/acul/js-sdk/Screens/interfaces/TransactionMembers">TransactionMembers</a></span>}/>

<ParamField body='untrustedData' type={<span><a href="/docs-v2/main/docs/libraries/acul/js-sdk/Screens/interfaces/UntrustedDataMembers">UntrustedDataMembers</a></span>}/>

<ParamField body='user' type={<span><a href="/docs-v2/main/docs/libraries/acul/js-sdk/Screens/interfaces/UserMembers">UserMembers</a></span>}/>

<ParamField body='screenIdentifier' type='string'>

Screen identifier for validation and telemetry

</ParamField>

## Methods

<ParamField body='getErrors' type={<span><a href="/docs-v2/main/docs/libraries/acul/js-sdk/Screens/interfaces/Error">Error</a>[]</span>}>

Retrieves the array of transaction errors from the context, or an empty array if none exist.

An array of error objects from the transaction context.

</ParamField>

<ParamField body='selectOrganization' type='Promise<void>'>

Submits the selected organization ID.

```typescript Example
import OrganizationPicker from '@auth0/auth0-acul-js/organization-picker';

const organizationPicker = new OrganizationPicker();
organizationPicker.selectOrganization({
  organization: 'org_XXXXXXXXXXXXXXX'
});
```

<Expandable title="Parameters">
<ParamField body='payload' type={<span><a href="/docs-v2/main/docs/libraries/acul/js-sdk/Screens/interfaces/SelectOrganizationOptions">SelectOrganizationOptions</a></span>}>
**Properties**

<ParamField body='organization' type='string'/>

<ParamField body='state' type='string'/>
</ParamField>

</Expandable>
</ParamField>

<ParamField body='skipOrganizationSelection' type='Promise<void>'>

Submits the action to skip the organization selection.

```typescript Example
import OrganizationPicker from '@auth0/auth0-acul-js/organization-picker';

const organizationPicker = new OrganizationPicker();
organizationPicker.skipOrganizationSelection();
```

<Expandable title="Parameters">
<ParamField body='payload?' type={<span><a href="/docs-v2/main/docs/libraries/acul/js-sdk/Screens/interfaces/CustomOptions">CustomOptions</a></span>}>

Optional custom options to include with the request.

</ParamField>

</Expandable>
</ParamField>