...
description: "Describes all the properties and methods available to customize the Universal Login `reset-password-error` screen."
---

The ResetPasswordError class implements the `reset-password-error` screen functionality. This screen is displayed when an error occurs during the password reset flow, such as an expired or invalid reset link.
...
Create an instance of ResetPasswordError screen manager:
...
Contains details specific to the `reset-password-error` screen, including its configuration and context.
</ParamField>
...
Provides transaction-specific data for the `reset-password-error` screen, such as active identifiers and flow states.
</ParamField>
