...
description: "Describes all the properties and methods available to customize the Universal Login `device-code-activation-denied` screen."
---

The DeviceCodeActivationDenied class implements the `device-code-activation-denied` screen functionality. This screen is displayed when the device code activation has been denied.
...
Create an instance of DeviceCodeActivationDenied screen manager:
...
Contains details specific to the `device-code-activation-denied` screen, including its configuration and context.
</ParamField>
...
Provides transaction-specific data for the `device-code-activation-denied` screen, such as active identifiers and flow states.
</ParamField>
