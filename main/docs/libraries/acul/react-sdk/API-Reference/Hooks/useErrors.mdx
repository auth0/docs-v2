...
<pre>
<code>
{`useErrors(options?: UseErrorOptions): `}<a href="/docs/libraries/acul/react-sdk/API-Reference/Types/interfaces/UseErrorsResult">UseErrorsResult</a>
</code>
</pre>

<ParamField body="useErrors" type={<span><a href="/docs/libraries/acul/react-sdk/API-Reference/Types/interfaces/UseErrorsResult">UseErrorsResult</a></span>}>
...
### Key Features
- **Unified error store** — surfaces server, client, and developer errors from a single hook.
...
- `errors` — the full error list of type [`ErrorsResult`](/docs/libraries/acul/react-sdk/API-Reference/Types/interfaces/ErrorsResult), with helpers:
  - `errors.byType(kind, filter?)` — filter by error kind and optionally by field.
...
```ts
console.log(errors.byType('client')); // all client errors
console.log(errors.byType('client', { field: 'username' })); // client errors for field 'username'
