...
Provides branding configurations, such as logo, colors, and theme settings displayed on the signup-id screen.
...
Provides transaction-specific data for the `signup-id` screen, such as active connections and current flow state.
...
Submits the user's identifier to proceed to the next step of the signup flow.
