...
Provides branding configurations, such as logo, colors, and theme settings displayed on the login-passwordless-email-code screen.
...
Provides transaction-specific data for the `login-passwordless-email-code` screen, such as active connections and current flow state.
...
Handles untrusted data passed to the screen, such as a prefilled email address from URL parameters.
...
Resends the one-time code to the user's email address.
...
Submits the one-time code to complete passwordless email authentication.
