...
description: "Describes all the hooks and methods available to customize the Universal Login `reset-password-mfa-otp-challenge` screen."
---

The `reset-password-mfa-otp-challenge` screen is displayed during the password reset flow when multi-factor authentication is required via a one-time password (OTP). It prompts the user to enter the OTP from their authenticator app to continue the reset process.

<Frame><img style={{maxHeight:"400px"}} src="/docs/images/cdy7uua7fh8z/4llG8aTJRKxkgGSgZBhFOa/327e242fda0888af45f56286fc6d6098/Screenshot_2025-03-26_at_09.15.22.png" alt="ACUL Reset Password MFA OTP Challenge"/></Frame>
...
Screen-scoped hooks that provide read-only access to Auth0 context data on the `reset-password-mfa-otp-challenge` screen. Import them from `@auth0/auth0-acul-react/reset-password-mfa-otp-challenge`.
...
This hook provides branding configurations, such as logo, colors, and theme settings displayed on the `reset-password-mfa-otp-challenge` screen.
...
This hook provides client-related configurations, such as `id`, `name`, and `logoUrl`, for the `reset-password-mfa-otp-challenge` screen.
...
This hook contains details specific to the `reset-password-mfa-otp-challenge` screen, including its configuration and context.
...
This hook provides transaction-specific data for the `reset-password-mfa-otp-challenge` screen, such as active connections and current flow state.
...
This hook returns all methods and context available on the `reset-password-mfa-otp-challenge` screen.
</ParamField>
...
This method submits the OTP code to continue the MFA step in the password reset flow.
