---
title: "MfaEmailList"
description: "Describes all the hooks and methods available to customize the Universal Login `mfa-email-list` screen."
---

The `mfa-email-list` screen presents a list of email addresses available for MFA verification and allows the user to select one.

<Frame><img style={{maxHeight:"400px"}} src="/docs/images/ja-jp/cdy7uua7fh8z/7d8tt9q970YJ4gT5C0kNhx/dfbaefcd9d913b41addc7b652d3bf63d/Screenshot_2025-02-20_at_15.43.19.png" alt="MfaEmailList" /></Frame>
...
Screen-scoped hooks that provide read-only access to Auth0 context data on the `mfa-email-list` screen. Import them from `@auth0/auth0-acul-react/mfa-email-list`.
...
This hook provides branding configurations, such as logo, colors, and theme settings displayed on the `mfa-email-list` screen.
...
This hook provides client-related configurations, such as `id`, `name`, and `logoUrl`, for the `mfa-email-list` screen.
...
This hook contains details specific to the `mfa-email-list` screen, including its configuration and context.
...
This hook provides transaction-specific data for the `mfa-email-list` screen, such as the current MFA flow state.
...
This hook returns all methods and context available on the `mfa-email-list` screen.
</ParamField>
...
This method selects an email address from the list to use for the MFA challenge.
...
<ParamField body="options">
  [SelectMfaEmailOptions](/docs/libraries/acul/react-sdk/API-Reference/Types/interfaces/SelectMfaEmailOptions).
</ParamField>

<ParamField body='index' type='number' required>
  The index of the email address to select.
</ParamField>
