...
description: "Describes all the hooks and methods available to customize the Universal Login `mfa-webauthn-error` screen."
---

The `mfa-webauthn-error` screen displays an error that occurred during a WebAuthn authentication attempt and provides options for the user to recover or try a different method.
...
Screen-scoped hooks that provide read-only access to Auth0 context data on the `mfa-webauthn-error` screen. Import them from `@auth0/auth0-acul-react/mfa-webauthn-error`.
...
This hook provides branding configurations, such as logo, colors, and theme settings displayed on the `mfa-webauthn-error` screen.
...
This hook provides client-related configurations, such as `id`, `name`, and `logoUrl`, for the `mfa-webauthn-error` screen.
...
This hook contains details specific to the `mfa-webauthn-error` screen, including its configuration and context.
...
This hook provides transaction-specific data for the `mfa-webauthn-error` screen, such as the current MFA flow state.
...
This hook returns all methods and context available on the `mfa-webauthn-error` screen.
</ParamField>
...
This method dismisses the WebAuthn error and cancels the current MFA flow.
...
This method retries the WebAuthn authentication after an error.
...
This method switches the authentication flow to use a password instead of WebAuthn.
