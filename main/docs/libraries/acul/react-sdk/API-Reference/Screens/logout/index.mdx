...
Provides information about the user's organization if the logout is organization-scoped. Returns `null` when no organization context is present.
...
Provides transaction-specific data for the `logout` screen, such as active connections and current flow state.
