...
Provides branding configurations, such as logo, colors, and theme settings displayed on the signup screen.
...
Provides transaction-specific data for the `signup` screen, such as active connections and current flow state.
...
Opens the country code picker for phone number input during signup.
...
Submits the user's registration details to create a new account.
...
Returns the available signup identifier types (email, username, phone) configured for the connection.
</ParamField>
...
Validates the user's username against the tenant's username policy requirements.
</ParamField>
