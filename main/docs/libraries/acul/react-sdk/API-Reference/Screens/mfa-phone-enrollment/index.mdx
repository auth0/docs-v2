...
description: "Describes all the hooks and methods available to customize the Universal Login `mfa-phone-enrollment` screen."
---
...
This hook provides branding configurations, such as logo, colors, and theme settings displayed on the `mfa-phone-enrollment` screen.
...
This hook provides client-related configurations, such as `id`, `name`, and `logoUrl`, for the `mfa-phone-enrollment` screen.
...
This hook contains details specific to the `mfa-phone-enrollment` screen, including its configuration and context.
...
This hook provides transaction-specific data for the `mfa-phone-enrollment` screen, such as the current MFA flow state.
...
This hook returns all methods and context available on the `mfa-phone-enrollment` screen.
</ParamField>
...
function PhoneEnrollmentForm() {
  const { continueEnrollment } = useMfaPhoneEnrollment();
  return (
    <button onClick={() => continueEnrollment({ phone: '5551234567', type: 'sms' })}>
      Enroll Phone
    </button>
  );
}
...
<ParamField body="options">
  [MfaPhoneEnrollmentContinueOptions](/docs/libraries/acul/react-sdk/API-Reference/Types/interfaces/MfaPhoneEnrollmentContinueOptions).
</ParamField>

<ParamField body='phone' type='string' required>
  The phone number to enroll for MFA.
</ParamField>
