...
description: "Describes all the hooks and methods available to customize the Universal Login `mfa-sms-enrollment` screen."
---

The `mfa-sms-enrollment` screen allows the user to enroll a phone number for SMS-based MFA by entering their number and selecting a delivery method.

<Frame><img style={{maxHeight:"400px"}} src="/docs/images/ja-jp/cdy7uua7fh8z/5DG03Sm0FFKKt9JOxOl5WN/5df6ce785b080baa951b749b4c472793/Screenshot_2025-02-21_at_11.55.42.png" alt="MfaSmsEnrollment" /></Frame>
...
Screen-scoped hooks that provide read-only access to Auth0 context data on the `mfa-sms-enrollment` screen. Import them from `@auth0/auth0-acul-react/mfa-sms-enrollment`.
...
This hook provides branding configurations, such as logo, colors, and theme settings displayed on the `mfa-sms-enrollment` screen.
...
This hook provides client-related configurations, such as `id`, `name`, and `logoUrl`, for the `mfa-sms-enrollment` screen.
...
This hook contains details specific to the `mfa-sms-enrollment` screen, including its configuration and context.
...
This hook provides transaction-specific data for the `mfa-sms-enrollment` screen, such as the current MFA flow state.
...
This hook returns all methods and context available on the `mfa-sms-enrollment` screen.
</ParamField>
...
This method submits the user's phone number to complete SMS MFA enrollment.
...
<ParamField body='payload' type={<span><a href="/docs/libraries/acul/react-sdk/API-Reference/Types/interfaces/MfaSmsEnrollmentContinueOptions">MfaSmsEnrollmentContinueOptions</a></span>}>
