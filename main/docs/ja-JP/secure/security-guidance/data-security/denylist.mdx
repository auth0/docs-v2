...
プライバシー上の理由により Auth0 データベースに保存すべきでないユーザーフィールドがある場合は、それらを拒否リストに追加できます。拒否リストに属性を追加するには、<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" href="/docs/ja-JP/ja-jp/glossary?term=management-api" tip="Management API: 顧客が管理タスクを実行できるようにするための製品。" cta="用語集の表示">Management API</Tooltip>の更新接続エンドポイントに対して`PATCH`呼び出しを行います。

1. `/patch_connections_by_id`[エンドポイント](/docs/ja-JP/ja-jp/api/management/v2#!/Connections/patch_connections_by_id)にアクセスするための有効なアクセストークンを取得します。トークンには、`update:connections`スコープが含まれている必要があります。詳細については、[［Management API Access Tokens（Management APIのアクセストークン）］](/docs/ja-JP/ja-jp/secure/tokens/access-tokens/management-api-access-tokens)を参照してください。
...
1. `{yourConnectionId}` は、これらの属性が拒否される[接続 ID](/docs/ja-JP/ja-jp/authenticate/identity-providers/locate-the-connection-id)です。
...
* 拒否できるのは、[ルート フィールド](/docs/ja-JP/ja-jp/manage-users/user-accounts/user-profiles/user-profile-structure)（`user.name`や`user.phone_number` など）のみです。
...
これらの制限のいずれかが受け入れられない場合は、データを暗号化する[ルール](/docs/ja-JP/ja-jp/customize/rules)を記述し、データを`user.app_metadata`オブジェクトに保持することができます。

<div id="learn-more">
  ## もっと詳しく
</div>

* [許可リスト用のAuth0 IPアドレス](/docs/ja-JP/ja-jp/secure/security-guidance/data-security/allowlist)