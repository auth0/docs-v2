...
通常、（サービスの中断を防ぐため）以前のアクセストークンが期限切れになる前に新しいアクセストークンを要求すべきですが、トークン交換には当社の[レート制限ポリシー](/docs/ja-JP/ja-jp/troubleshoot/customer-support/operational-policies/rate-limit-policy)が適用されるため、API呼び出しのたびには行いません。
...
認証中に受け取ったリフレッシュトークンを新しいアクセストークンと交換するには、Auth0 Authentication APIでその[トークン取得API](/docs/ja-JP/ja-jp/api/authentication#refresh-token)を呼び出します。
...
[多要素認証（MFA）](/docs/ja-JP/ja-jp/secure/multi-factor-authentication)が有効で、リフレッシュトークンに失敗した場合は、以下の回避策があります。
...
* [Lock.Android:JSON Webトークンのリフレッシュ](/docs/ja-JP/ja-jp/libraries/lock-android/lock-android-refresh-jwt)