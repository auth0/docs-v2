...
リフレッシュトークンを取得するには、`/authorize`エンドポイントを通じて認証要求を開始する際に、`offline_access`[スコープ](/docs/ja-JP/ja-jp/get-started/apis/scopes)を含める必要があります。必ずAPIでオフラインアクセスを開始してください。詳細については、「[API設定](/docs/ja-JP/ja-jp/get-started/apis/api-settings)」をお読みください。

たとえば、[認可コードフロー](/docs/ja-JP/ja-jp/get-started/authentication-and-authorization-flow/authorization-code-flow)を使用している場合、認証要求は以下のようになります。
...
認可コードフローを使用してこれを実装する方法の詳細については、「[認可コードフローを使用してAPIを呼び出す](/docs/ja-JP/ja-jp/get-started/authentication-and-authorization-flow/authorization-code-flow/call-your-api-using-the-authorization-code-flow)」のチュートリアルを参照してください。その他の付与については、「[認証フローと認可フロー](/docs/ja-JP/ja-jp/get-started/authentication-and-authorization-flow)」を参照してください。
