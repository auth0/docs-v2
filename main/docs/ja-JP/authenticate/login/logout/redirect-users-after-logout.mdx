...
2. `*`を[サブドメインのワイルドカードとして使用する](/docs/ja-JP/ja-jp/get-started/applications/wildcards-for-subdomains)（例：`http://*.example.com`）
...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  Auth0の`oidc/logout`エンドポイントを使ってリダイレクトする方法については、「[OIDCのエンドポイントを使ってユーザーをAuth0からログアウトさせる](/docs/ja-JP/ja-jp/authenticate/login/logout/log-users-out-of-auth0)」をお読みください。
</Callout>
...
すべての<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" href="/docs/ja-JP/ja-jp/glossary?term=idp" tip="IDプロバイダー（IdP）: デジタルIDを保存および管理するサービス。" cta="用語集の表示">IdPs</Tooltip>が`returnTo`をサポートしているとは限りません。
...
* [ログインとログアウトの問題を確認する](/docs/ja-JP/ja-jp/troubleshoot/authentication-issues/check-login-and-logout-issues)