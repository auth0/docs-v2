...
推奨はされませんが、信頼性の高いアプリケーションでは、[リソース所有者のパスワードフロー](/docs/ja-JP/ja-jp/get-started/authentication-and-authorization-flow/resource-owner-password-flow)（リソース所有者のパスワード付与（ROPG）とも呼ばれる）を使ってサーバー側APIを呼び出すことができます。このフローは、ユーザーに資格情報（ユーザー名とパスワード）の提供を要求するもので、通常は対話型フォームを使用します。Auth0でこの資格情報を検証中に[総当たり攻撃防御が有効](/docs/ja-JP/ja-jp/secure/attack-protection/brute-force-protection)になっている場合、[攻撃の有無も確認](/docs/ja-JP/ja-jp/secure/attack-protection)され、攻撃が検出されたら適切な措置が講じられます。
...
[通常のWebアプリケーション](/docs/ja-JP/ja-jp/get-started/auth0-overview/create-applications/regular-web-apps)または[マシンツーマシンアプリケーション](/docs/ja-JP/ja-jp/get-started/auth0-overview/create-applications/machine-to-machine-apps)を登録します。アプリケーションの構成中に、以下の操作を行います。
...
1. [リソース所有者のパスワードフロー](/docs/ja-JP/ja-jp/get-started/authentication-and-authorization-flow/resource-owner-password-flow/call-your-api-using-resource-owner-password-flow)を使用してトークンを要求する場合には、ユーザーのIPアドレスの値が入った`auth0-forwarded-for`ヘッダーを含めてください。入力したIPアドレスがユーザーのもので間違いないことを確認します。
...
テナントの[［Breached Password Detection（侵害されたパスワードの検出）］](/docs/ja-JP/ja-jp/secure/attack-protection/breached-password-detection)が有効な場合は、[Auth0 Authentication API](/docs/ja-JP/ja-jp/api/authentication)からの応答に適宜対処するようアプリケーションを構成する必要があります。
...
アプリケーションはこのエラーを処理し、メッセージをユーザーに表示し、[インタラクティブなパスワードリセットフローをトリガー](/docs/ja-JP/ja-jp/authenticate/database-connections/password-change#authentication-api)します。
...
* [リソース所有者のパスワードフロー](/docs/ja-JP/ja-jp/get-started/authentication-and-authorization-flow/resource-owner-password-flow)
* [リソース所有者のパスワードフローを使ってAPIを呼び出す](/docs/ja-JP/ja-jp/get-started/authentication-and-authorization-flow/resource-owner-password-flow/call-your-api-using-resource-owner-password-flow)
