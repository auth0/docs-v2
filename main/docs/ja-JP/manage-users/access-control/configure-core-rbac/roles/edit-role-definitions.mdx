...
[ロール更新エンドポイント](/docs/ja-JP/ja-jp/api/management/v2#!/Roles/patch_roles_by_id)に`PATCH`呼び出しを行います。`ROLE_ID`、`MGMT_API_ACCESS_TOKEN`、`ROLE_NAME`、`ROLE_DESC`のプレースホルダーの値をそれぞれロールID、Management APIのアクセストークン、ロール名、ロールの説明に置き換えます。
