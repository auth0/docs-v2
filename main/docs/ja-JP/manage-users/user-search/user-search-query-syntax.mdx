...
* フィールド名のないタームは、[ユーザーメタデータ](/docs/ja-JP/ja-jp/manage-users/user-accounts/metadata)フィールドのテキストと一致しません。
...
すべての[正規化ユーザープロファイルフィールド](/docs/ja-JP/ja-jp/manage-users/user-accounts/user-profiles/normalized-user-profile-schema)および以下のフィールドを使用して、ユーザーを検索できます。
...
<table class="table">
  ...
  <tbody>
    ...
    <tr>
      <td>`last_login`</td>
      <td>日時</td>
      <td>ユーザーが最後にログインした日時を示すタイムスタンプです。このプロパティが<a href="/docs/ja-JP/ja-jp/rules">ルール</a>内から`user`オブジェクトを伴って実行される場合、値はルールを起動したログインと関連付けられます（ルールは実際のログインの後に実行されるからです）。</td>
    </tr>
...
* [ユーザー取得エンドポイントを使用してユーザーを取得する](/docs/ja-JP/ja-jp/manage-users/user-search/retrieve-users-with-get-users-endpoint)