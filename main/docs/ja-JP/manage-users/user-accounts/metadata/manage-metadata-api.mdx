...
Management API [`/patch_users_by_id`](/docs/ja-JP/ja-jp/api/management/v2#!/Users/patch_users_by_id)エンドポイントに対して`PATCH`呼び出しを実行することで、ユーザーのメタデータを更新できます。
...
クライアントメタデータは、[`PATCH /api/v2/clients/{id}`](/docs/ja-JP/ja-jp/api/management/v2#!/Users/patch_users_by_id)エンドポイントを使って更新し、アプリケーションオブジェクトに`clientMetadata property`を提供することができます。このプロパティは、変更したメタデータを含んだオブジェクトで構成される値を持っています。
...
クライアントメタデータキーを削除するには、上記の「app&#95;metadataを更新する」で説明されたように、PATCHを発行しますが、キー値にには`null`を指定します。この動作は、`PATCH`[/api/v2/users/{id}](/docs/ja-JP/ja-jp/api/management/v2#!/Users/patch_users_by_id)エンドポイントの`user_metadata`プロパティと`app_metadata`プロパティの動作に一致します。
...
* [アプリケーションメタデータを構成する](/docs/ja-JP/ja-jp/get-started/applications/configure-application-metadata)