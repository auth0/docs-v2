...
[ルート属性を個別に](/docs/ja-JP/ja-jp/manage-users/user-accounts/user-profiles/root-attributes/update-root-attributes-for-users)編集するか、Management APIを使用して[一括インポートする](/docs/ja-JP/ja-jp/manage-users/user-accounts/user-profiles/root-attributes/set-root-attributes-during-user-import)ことができます。
...
この手順を完了させる前に、必ず[接続の`options`オブジェクトに既存の値を取得して](/docs/ja-JP/ja-jp/authenticate/identity-providers/retrieve-connection-options)、現在の値がオーバーライドされないようにします。これを行わないと、更新後、元のオブジェクトに欠けているパラメーターが失われてしまいます。
...
* [接続オプションを取得する](/docs/ja-JP/ja-jp/authenticate/identity-providers/retrieve-connection-options)
