...
* アカウントリンク拡張機能の使用
* <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" href="/docs/ja-JP/ja-jp/glossary?term=management-api" tip="Management API: 顧客が管理タスクを実行できるようにするための製品。" cta="用語集の表示">Management API</Tooltip>の使用
* Auth0.jsの使用
...
Dashboardの[アカウントリンク拡張機能](/docs/ja-JP/ja-jp/customize/extensions/account-link)をインストールして構成すると、2つ目のアカウントを作成したユーザーの初回ログイン時に、古いアカウントと新しいアカウントをリンクするよう求めることができます。ユーザーは、2つのアカウントをリンクするか、別々にしておくかを選択できます。

## Management APIエンドポイント

Auth0 Management APIが提供する[ユーザーアカウントのリンク](/docs/ja-JP/ja-jp/api/v2#!/Users/post_identities)エンドポイントは、次の2つの方法で呼び出すことができます：
...
[Auth0.js](/docs/ja-JP/ja-jp/libraries/auth0js)ライブラリーを使用できます。まず、Management APIの呼び出しに使用できるアクセストークンを取得しなければなりません。それには、Auth0.jsを初期化する際に`https://{yourDomain}/api/v2/`オーディエンスを指定します。認証フローの一部としてアクセストークンを取得します。代わりに、`checkSession`メソッドを使用することもできます。アクセストークンを取得したら、そのアカウントのAuth0ドメインとアクセストークンを渡して新しい`auth0.Management`インスタンスを作成することができます。詳細については、[Auth0.js &gt; ユーザー管理](/docs/ja-JP/ja-jp/libraries/auth0js/v9#user-management)をご覧ください。
...
2. [アクションのリダイレクト機能](/docs/ja-JP/ja-jp/customize/actions/explore-triggers/signup-and-login-triggers/login-trigger/redirect-with-actions)とトークンを使用して、外部リンクアプリにリダイレクトします。
...
* [ユーザーアカウントのリンクを解除する](/docs/ja-JP/ja-jp/manage-users/user-accounts/user-account-linking/unlink-user-accounts)