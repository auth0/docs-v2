...
詳細については、「[Understand How Metadata Works in User Profiles（ユーザープロファイルでのメタデータの仕組みを理解する）](/docs/ja-JP/ja-jp/manage-users/user-accounts/metadata)」をお読みください。
...
シングルページアプリケーションでユーザー主導のアカウントリンクを実装する方法について詳しくは、[「Client-Side Account Linking（クライアント側のアカウントリンク）」](/docs/ja-JP/ja-jp/users/references/link-accounts-client-side-scenario)をご一読ください。通常のウェブアプリケーションでユーザー主導のアカウントリンクを実装する方法について詳しくは、[「Server-Side Account Linking（サーバー側のアカウントリンク）」](/docs/ja-JP/ja-jp/users/references/link-accounts-server-side-scenario)をご一読ください。
...
また、[Account Link拡張機能](/docs/ja-JP/ja-jp/customize/extensions/account-link)を使用しても、同じ結果を得ることができます。
