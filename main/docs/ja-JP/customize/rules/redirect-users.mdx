...
認証トランザクションの完了前に、 [Auth0ルール](/docs/ja-JP/ja-jp/customize/rules)を使用してユーザーをリダイレクトすることができます。これによって、カスタム認証フローを実装して、標準のログインフォームにはない追加のユーザー操作に対応できるようになります。リダイレクトルールは一般的に、Auth0でカスタムの多要素認証（<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" href="/docs/ja-JP/ja-jp/glossary?term=multifactor-authentication" tip="多要素認証（MFA）: ユーザー名とパスワードに加えて、SMS経由のコードなどの要素を使用するユーザー認証プロセス。" cta="用語集の表示">MFA</Tooltip>）を実行するために使用されますが、以下にも使用できます。
...
詳細については、「[Auth0での多要素認証](/docs/ja-JP/ja-jp/secure/multi-factor-authentication)」をご覧ください。
...
認証トランザクションを再開するには、リダイレクトURLが`state`パラメーターを抽出し、それをAuth0に送り返す必要があります。状態は不透明な値で、[クロスサイトリクエストフォージェリ（CSRF）攻撃](/docs/ja-JP/ja-jp/security/common-threats#cross-site-request-forgery)を防ぐために使用されます。
...
* ユーザーオブジェクトへの変更は、`/continue` エンドポイントを呼び出す前のリダイレクト中に行われます。たとえば、Auth0 <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" href="/docs/ja-JP/ja-jp/glossary?term=management-api" tip="Management API: 顧客が管理タスクを実行できるようにするための製品。" cta="用語集の表示">Management API</Tooltip>を介した更新は、トランザクションの続行後に使用できます。
...
3. アプリケーションが[Auth0 Management API](/docs/ja-JP/ja-jp/api/v2#!/Users/patch_users_by_id)
...
* [リソース所有者 のエンドポイント](/docs/ja-JP/ja-jp/api/authentication/reference#resource-owner)
* [パスワードの交換](/docs/ja-JP/ja-jp/get-started/authentication-and-authorization-flow/resource-owner-password-flow)
* [リフレッシュトークンの交換](/docs/ja-JP/ja-jp/secure/tokens/refresh-tokens)
...
* [ユーザーをリダイレクトする](/docs/ja-JP/ja-jp/authenticate/login/redirect-users-after-login)
* [プログレッシブプロファイリングの仕組みを理解する](/docs/ja-JP/ja-jp/manage-users/user-accounts/user-profiles/progressive-profiling)
