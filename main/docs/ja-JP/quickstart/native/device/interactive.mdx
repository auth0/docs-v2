...
<Recipe>
  <Content>
    ...
    <Section id={sections[6].id} title={sections[6].title} stepNumber="7">
      ...
      IDトークンには、[デコードと抽出](/docs/ja-JP/ja-jp/tokens/id-tokens#id-token-payload)が必要なユーザー情報が含まれています。デバイスアプリケーションが`openid`スコープを要求した場合には、Authentication APIは`id_token`のみを返します。
    ...
    <Section id={sections[8].id} title={sections[8].title} stepNumber="9">
      ...
      リフレッシュトークンの詳細については、「[リフレッシュトークン](/docs/ja-JP/ja-jp/secure/tokens/refresh-tokens)」をお読みください。
    </Section>
    ...
    <Section id={sections[11].id} title={sections[11].title} stepNumber="12">
      ...
      * ネイティブアプリケーション
      * [認証方法](/docs/ja-JP/ja-jp/secure/application-credentials#application-authentication-methods)が**［None（なし）］** に設定されている
      * [OIDCに準拠](/docs/ja-JP/ja-jp/dashboard/reference/settings-application#oauth)
