...
<SsoProviderCreate
  ...
  onNext={(stepId, values) => {
    ...
    // Validate provider name on step 2
    if (stepId === "provider-details" && !values.name) {
  ...
  onPrevious={(stepId) => {
    analytics.track("SSO Wizard Step Back", { step: stepId });
    return true;
  }}
/>
```
...
| Prop | Type | Description |
|------|------|-------------|
| `schema` | `SsoProviderSchema` | Field validation rules by strategy. [Details](#schema-validation) |
| `customMessages` | `Partial<SsoProviderCreateMessages>` | i18n text overrides. [Details](#custom-messages) |
| `styling` | `ComponentStyling<SsoProviderCreateClasses>` | CSS variables and class overrides. [Details](#custom-styling) |

#### Schema Fields by Strategy

Each SSO provider type (strategy) has different configuration fields. Use this table to see which fields you can validate for each provider type. All fields support `regex`, `errorMessage`, `minLength`, `maxLength`, and `required` options.
...
Set custom validation rules for provider fields. Rules are organized by provider strategy.
