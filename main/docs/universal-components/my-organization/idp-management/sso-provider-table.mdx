...
<div data-uc-component="sso-provider-table" data-uc-props="{}" style={{ minHeight: "300px", marginTop: "50px" }}>
  <img src="https://auth0-universal-components.vercel.app/img/my-organization/idp-management/sso-provider-table.png" alt="SSO Provider Table Component Loading Preview" style={{ width: "100%", height: "auto", borderRadius: "8px" }} />
</div>
...
<SsoProviderTable
  ...
  enableProviderAction={{
    onBefore: (provider) => {
      ...
      return confirm(`Disable "${provider.display_name}"? Users won't be able to authenticate.`);
    },
    onAfter: (provider) => {
      analytics.track(provider.is_enabled ? "Provider Enabled" : "Provider Disabled", {
        name: provider.name,
      });
    },
  }}
...
| Prop | Type | Description |
|------|------|-------------|
| `schema` | `SsoProviderTableSchema` | Confirmation field validation. [Details](#schema-validation) |
| `customMessages` | `Partial<SsoProviderTableMessages>` | i18n text overrides. [Details](#custom-messages) |
| `styling` | `ComponentStyling<SsoProviderTableClasses>` | CSS variables and class overrides. [Details](#custom-styling) |
...
<SsoProviderTable
  ...
  customMessages={{
    ...
    table: {
      empty_message: "No SSO providers configured. Add one to enable single sign-on.",
    },
    notifications: {
    ...
    <div className="max-w-6xl mx-auto p-6">
      <SsoProviderTable
        ...
        customMessages={{
          ...
          table: {
            empty_message: "No providers configured yet. Add one to enable SSO.",
          },
        }}
        styling={{
          variables: {
