...
Once [associated with a client](/docs/ai-agents-mcp/agent-as-principal/associate-agent-client), tokens issued to that client carry the agent's identity for attribution and traceability. How the agent's identity appears in the token depends on the grant type:
...
- Add [agent context to access tokens using Actions](/docs/ai-agents-mcp/agent-as-principal/actions-context)
- Query the [agent ID in tenant logs](/docs/ai-agents-mcp/agent-as-principal/tenant-logs) for agent identity attribution and traceability