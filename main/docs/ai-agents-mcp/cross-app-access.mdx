...
Both the Auth0 Resource App Authorization Server and the enterprise IdP must be configured for XAA to work. Start with [Environment Setup](/docs/ai-agents-mcp/cross-app-access/set-up-xaa-test-environment) to configure the Auth0 side, then read the applicable guide in IdP Integration — [Okta as OIDC IdP](/docs/ai-agents-mcp/cross-app-access/idp/okta-as-oidc-idp) and [Okta as SAML IdP](/docs/ai-agents-mcp/cross-app-access/idp/okta-as-saml-idp) — depending on the IdP and protocol you want to use to federate with it.
