---
description: Create and manage AI agent-to-app and app-to-app connections using centralized enterprise admin controls, with out-of-the-box support for MCP's Enterprise-Managed Authorization.
...
XAA implements the [Identity Assertion Authorization Grant](https://datatracker.ietf.org/doc/draft-ietf-oauth-identity-assertion-authz-grant/), an in-progress OAuth extension that lets a Requesting App, such as an application or AI agent, obtain a secure token from the enterprise IdP to call another application's API (Resource App) on the end-user's behalf. This covers both app-to-app connections and agent-to-app connections. XAA is the protocol solution behind MCP's [Enterprise-Managed Authorization](https://blog.modelcontextprotocol.io/posts/enterprise-managed-auth/) extension, which enables an AI agent acting as an [MCP client](https://modelcontextprotocol.io/) to seamlessly connect to an MCP server exposed by the Resource App. To learn more, read [How it works](#how-it-works).
...
The Requesting App and Resource App can each use OIDC or SAML to federate with the enterprise IdP for this SSO step. Read [End-to-end Testing](/docs/ai-agents-mcp/cross-app-access/end-to-end-testing) to understand the how the XAA flow works in each case.
</Callout>
...
| Read… | To… |
| --- | --- |
| [Set up Auth0 as Resource App](/docs/ai-agents-mcp/cross-app-access/resource-app) | Understand what’s involved and where to start. |
| [Environment Setup](/docs/ai-agents-mcp/cross-app-access/set-up-xaa-test-environment) | Configure your API and register a test Requesting App in your Auth0 tenant. |
| [Okta as OIDC IdP](/docs/ai-agents-mcp/cross-app-access/idp/okta-as-oidc-idp) | Configure Okta as the OIDC enterprise IdP for your Resource App. |
| [Okta as SAML IdP](/docs/ai-agents-mcp/cross-app-access/idp/okta-as-saml-idp) | Configure Okta as the SAML enterprise IdP for your Resource App. |
| [End-to-end Testing](/docs/ai-agents-mcp/cross-app-access/end-to-end-testing) | Test the full XAA flow once both Auth0 and the enterprise IdP are configured. |

### Set up Auth0 as Requesting App

Configure your Auth0 tenant as the Requesting App so that your AI agent or SaaS application can call third-party APIs on a user’s behalf without requiring OAuth consent flows. Access is governed by the enterprise IT admin’s policy in their IdP. Auth0 stores and reuses the tokens your application retrieves via XAA in [Token Vault](/docs/secure/tokens/token-vault).

| Read… | To… |
| --- | --- |
| [Set up Auth0 as Requesting App](/docs/ai-agents-mcp/cross-app-access/requesting-app) | Understand what’s involved and where to start. |
| [Environment Setup](/docs/ai-agents-mcp/cross-app-access/requesting-app/set-up-xaa-test-environment) | Configure a test Resource App and the OIDC connection between your tenant and the Resource App tenant. |
| [Okta as OIDC IdP](/docs/ai-agents-mcp/cross-app-access/requesting-app/idp/okta-as-oidc-idp) | Configure Okta as the OIDC enterprise IdP for your Requesting App. |
| [Cross App Access with Token Vault](/docs/secure/call-apis-on-users-behalf/token-vault/xaa-with-token-vault) | Configure Token Vault to store and reuse the access tokens your application retrieves via XAA. |
...
- Token Vault requires exactly one valid XAA-enabled identity linked to the user's profile at token exchange time. If a user has multiple linked identities from XAA-enabled connections, the exchange will fail with a `Multiple enterprise connections with XAA support enabled error`. To learn more, read [Cross App Access with Token Vault](/docs/secure/call-apis-on-users-behalf/token-vault/xaa-with-token-vault).
