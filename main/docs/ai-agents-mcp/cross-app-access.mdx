...
XAA implements the [Identity Assertion Authorization Grant](https://datatracker.ietf.org/doc/draft-ietf-oauth-identity-assertion-authz-grant/), an in-progress OAuth extension that lets a Requesting App, such as an application or AI agent, obtain a secure token from the enterprise IdP to call another application's API (Resource App) on the end-user's behalf. This covers both app-to-app connections and agent-to-app connections — in fact, XAA is the protocol solution behind MCP's [Enterprise-Managed Authorization](https://blog.modelcontextprotocol.io/posts/enterprise-managed-auth/) extension, which enables an AI agent acting as an [MCP Client](https://modelcontextprotocol.io/) to seamlessly connect to an MCP server exposed by the Resource App. To learn more, read [How it works](#how-it-works).
