...
| [Cross App Access with Token Vault](/ai/docs/call-apis-on-users-behalf/token-vault/xaa-with-token-vault) | Configure Token Vault to store and reuse the access tokens your application retrieves via XAA. |
...
- Token Vault with XAA Requesting App support requires exactly one valid XAA-enabled identity linked to the user's profile at token exchange time. To learn more, read [Cross App Access with Token Vault](/ai/docs/call-apis-on-users-behalf/token-vault/xaa-with-token-vault).
