...
- Token Vault with XAA Requesting App support requires exactly one valid XAA-enabled identity linked to the user's profile at token exchange time. To learn more, read [Cross App Access with Token Vault](/docs/secure/call-apis-on-users-behalf/token-vault/xaa-with-token-vault).
