...
You can test the XAA flow [manually](#test-manually) or using our [online sandbox environment](#recommended-test-using-our-online-sandbox-environment) (recommended). If you use Okta as your enterprise IdP, once you've tested the XAA flow, [publish your Resource App to the Okta Integration Network (OIN)](#publish-your-resource-app-in-oin) to increase app discoverability for your enterprise customers.

## (Recommended) Test using our online sandbox environment

If you use Okta as your enterprise identity provider, we recommend using [XAA.dev](https://xaa.dev/) to test Cross App Access in your browser. Follow the [instructions](https://developer.okta.com/blog/2026/07/03/cross-app-access-saml#configure-your-xaa-saml-app-in-okta) to test your Resource App against xaa.dev, which serves as a hosted Requesting App that you can use to obtain an ID-JAG via OIDC or SAML, depending on your IdP connection, to exchange with your Resource App.
...
If you use Okta as your enterprise identity provider, B2B and SaaS providers can amplify their reach to enterprise customers by publishing their Resource App to the [Okta Integration Network (OIN)](https://www.okta.com/integrations/). To learn how, follow the instructions in [How to Build and List Secure Cross App Access Connections on OIN](https://developer.okta.com/blog/2026/07/06/submit-oin-xaa).

For a list of fields needed to publish your Resource App to the OIN, review your Auth0 logs for [idjag-exchange-succeeded](https://auth0.com/docs/tenant-logs/auth-token-exchange-success/idjag-exchange-succeeded).
