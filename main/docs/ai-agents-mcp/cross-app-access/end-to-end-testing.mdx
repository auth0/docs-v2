...
You can test the XAA flow [manually](#test-manually) or using our [online sandbox environment](#recommended-test-using-our-online-sandbox-environment) (recommended). Once you've tested the XAA flow, [publish your Resource App to the Okta Integration Network (OIN)](#publish-your-resource-app-in-oin) to increase app discoverability for your enterprise customers.

## (Recommended) Test using our online sandbox environment

We recommend using [XAA.dev](https://xaa.dev/) to test Cross App Access in your browser. Follow the [instructions](https://developer.okta.com/blog/2026/07/03/cross-app-access-saml#configure-your-xaa-saml-app-in-okta) to test your Resource App against xaa.dev, which serves as a hosted Requesting App that you can use to obtain an ID-JAG via OIDC or SAML, depending on your IdP connection, to exchange with your Resource App.
