...
1. Under **Scopes**, add `offline_access` to the list of scopes. This enables your application to obtain a refresh token, which is required to perform the [refresh token exchange with Token Vault](/ai/docs/call-apis-on-users-behalf/token-vault/refresh-token-exchange-with-token-vault).
