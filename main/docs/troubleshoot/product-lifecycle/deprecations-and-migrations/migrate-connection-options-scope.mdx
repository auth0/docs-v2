...
​Auth0 is introducing new Management API scopes for connection `options` and deprecating the ability to access or update this field without these [scopes](/docs/get-started/apis/scopes). Customers must update their application grants to include the new scopes in order to continue accessing or updating the options field. To continue using the `options` field, you must update your application grants to include the new scopes.

**Breaking changes**

*   Responses from [Get All Connections](/docs/api/management/v2/connections/get-connections) or [Get Connection by Id ](/docs/api/management/v2/connections/get-connections-by-id) do not include the `options` field unless the caller has the `read:connections_options` scope.
*   Requests to [Update a Connection](/docs/api/management/v2/connections/patch-connections-by-id) fail if the request body contains the options field and the caller does not have the `update:connections_options` scope. Successful responses do not include the `options` field unless the caller has the `read:connections_options` scope.

To prepare for this change and ensure your integrations continue to function without interruption, review whether your tenant and integrations are affected, then follow the steps below to migrate to the options-specific scopes.
...
Once your integrations no longer depend on the deprecated behavior, you should configure the tenant to explicitly opt out of it.

**Why Opt-Out Early?** Opting out early lets you choose exactly when your tenant adopts the new behavior, providing more control over your migration process. If you face any issues, you can quickly revert the toggle to temporarily restore the previous behavior while you address the problem.

**How to Opt-Out**: Navigate to [Auth0 Dashboard > Tenant Settings > Advanced](https://manage.auth0.com/dashboard/#/tenant/advanced). Under the Migrations section, toggle off the **Allow Connections Management without Options Scopes** option.
