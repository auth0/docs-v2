...
<Warning>
Private cloud tenant administrators can follow these instructions if their Auth0 private cloud deployment is version `v202608` or later. To learn more about the private cloud deployments versions, read [How to Check the Auth0 Private Cloud's Release Number and Deployment Date](https://support.auth0.com/center/s/article/How-to-check-the-Auth0-private-cloud-s-release-number-and-deployment-date).
</Warning>
...
<Warning>
Public cloud tenants received these notifications in the first half of February 2026. 
Private cloud tenants received these notifications when their Auth0 private cloud deployment versions reached `v202608`.
</Warning>
...
* If the `client_id` field is empty, the notification originated from an extension. 
* If the value does not match any of the tenant’s client applications, it originated from a platform service, and it can be ignored.
