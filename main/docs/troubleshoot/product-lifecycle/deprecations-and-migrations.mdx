...
**Deprecated**: August 3, 2026

Auth0 is deprecating unbounded [user profile](/manage-users/user-accounts/user-profiles) sizes. All user profiles will have a size limit of 1 KB.

Writes that cause profiles to exceed this size limit succeed for now, but may start to fail. A tenant log flags profiles as oversized, and we strongly recommend bringing user profiles within the size limit to avoid errors.
