...
After the end-of-life date, we will require the use of modern ciphers to connect to the Auth0 service endpoints and web applications. We will remove support for TLS 1.2 cipher suites that no longer provide sufficient security to protect network communications. More specifically, the change to supported cipher suites applies to:
...
If you received a notification via email, one or more of your tenants one or more tenants may have a Microsoft Azure AD connection targeting the **Azure Active Directory (v1)** identity API and configured to obtain extended attributes and could potentially be impacted.
