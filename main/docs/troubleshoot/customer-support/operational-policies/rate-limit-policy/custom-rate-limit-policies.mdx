...
Although applications registered using [Client ID Metadata Document (CIMD)](/docs/get-started/auth0-overview/create-applications/register-applications-with-cimd#register-applications-with-cimd) are technically third-party applications, Auth0 places them in a separate group from traditional third-party apps. This means two distinct group buckets exist:
...
- **CIMD clients** (Client ID prefix: `https://`): Applications created through Client ID Metadata Document.
