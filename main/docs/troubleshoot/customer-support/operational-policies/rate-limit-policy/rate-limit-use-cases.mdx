...
The `api_limit` event is triggered immediately after a rate limit is exceeded for the global rate limit bucket for the Authentication or <Tooltip tip="Management API: A product to allow customers to perform administrative tasks." cta="View Glossary" href="/docs/glossary?term=Management+API">Management API</Tooltip>. If a rate limit is exceeded for a different rate limit bucket, a new `api_limit` event is generated. This helps customers identify which rate limit configuration their API calls are triggering, which is a critical first step in diagnosing the root cause.
...
The `appi` log is triggered when a customer tenant with a Public Performance Burst add-on exceeds their sustained Authentication API request rate limit of 100 RPS, consuming a one-minute block of their 48-hour burst allocation. If, after 1 minute, the request rate exceeds the 100 RPS sustained request limit again, a second `appi` log triggers.
...
| Authentication flow | Flow type | Requests to Authentication API endpoints |
| --- | --- | --- |
