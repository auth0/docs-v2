...
Vous pouvez tester le flux XAA [manuellement](#test-manually) ou à l’aide de notre [environnement isolé en ligne](#recommended-test-using-our-online-sandbox-environment) (recommandé). Une fois le flux XAA testé, [publiez votre application de ressources dans l’Okta Integration Network (OIN)](#publish-your-resource-app-in-oin) afin d’améliorer la découvrabilité de votre application auprès de vos clients d’entreprise.

## (Recommandé) Tester avec notre environnement isolé en ligne {#recommended-test-using-our-online-sandbox-environment}

Nous vous recommandons d&#39;utiliser [XAA.dev](https://xaa.dev/) pour tester Cross App Access dans votre navigateur. Suivez les [instructions](https://developer.okta.com/blog/2026/07/03/cross-app-access-saml#configure-your-xaa-saml-app-in-okta) pour tester votre application de ressources avec XAA.dev, qui fait office d&#39;application demandeuse hébergée et qui vous permet d&#39;obtenir un ID-JAG au moyen d&#39;OIDC ou de SAML, selon votre connexion IdP, afin de l&#39;échanger avec votre application de ressources.

## Tester manuellement {#test-manually}

### Obtenir l’ID-JAG {#obtain-the-id-jag}
...
#### Application demandeuse SAML {#saml-requesting-app}
...
#### Application demandeuse OIDC {#oidc-requesting-app}
...
### Envoyer l’ID-JAG au point de terminaison `/token` d’Auth0 {#send-id-jag-to-auth0s-token-endpoint}
...
| **Paramètre**           | **Description**                                                                                                                                                                                                                                                                                                                                                                                                         |
| ----------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `grant_type`            | Le type d’autorisation. Il indique au serveur d’autorisation qu’il doit s’attendre à recevoir un JSON Web Token (JWT) comme principal identifiant d’authentification dans la requête.                                                                                                                                                                                                                                    |
| `client_id`             | L’ID client de l’application demandeuse auprès du serveur d’autorisation de l’application de ressources qui effectue l’appel d’API.                                                                                                                                                                                                                                                                                     |
...
| `client_secret`         | (Facultatif) Le secret client de l’application demandeuse auprès du serveur d’autorisation de l’application de ressources qui effectue l’appel d’API.                                                                                                                                                                                                                                                                   |
| `client_assertion`      | (Facultatif) L’assertion client, qui peut être utilisée pour authentifier l’application demandeuse au moyen de Private Key JWT.                                                                                                                                                                                                                                                                                         |
...
Après avoir validé l’ID-JAG pour vérifier l’identité de l’utilisateur, le serveur d’autorisation Auth0 émet un jeton d’accès pour l’API de votre tenant Auth0. Le jeton d’accès comprend également les scopes demandés, autorisés par RBAC et les autres politiques définies dans votre tenant Auth0.

Le serveur d’autorisation Auth0 n’émet pas de jetons d’actualisation en réponse aux échanges de jetons ID-JAG. L’application demandeuse doit donc obtenir un nouvel ID-JAG auprès de l’IdP d’entreprise et se soumettre aux contrôles d’accès applicables pour obtenir un nouveau jeton d’accès par XAA.

## Publiez votre application de ressources dans l&#39;OIN {#publish-your-resource-app-in-oin}

Les fournisseurs B2B et SaaS peuvent élargir leur portée auprès des clients d&#39;entreprise en publiant leur application de ressources dans l&#39;[Okta Integration Network (OIN)](https://www.okta.com/integrations/). Pour savoir comment procéder, suivez les instructions de la page [How to Build and List Secure Cross App Access Connections on OIN](https://developer.okta.com/blog/2026/07/06/submit-oin-xaa).

Pour connaître la liste des champs nécessaires à la publication de votre application de ressources dans l&#39;OIN, consultez vos journaux Auth0 relatifs à [idjag-exchange-succeeded](https://auth0.com/docs/tenant-logs/auth-token-exchange-success/idjag-exchange-succeeded).
