---
description: Découvrez quels scopes de gestion sont requis pour chaque outil Auth0 MCP Server et comment les configurer à l’aide de motifs glob lors de l’initialisation.
...
<Accordion title="Scopes des opérations de gestion">
  ...
  | Créer une autorisation de client pour une application                           | `auth0_create_application_grant` | `create:client_grants`    | Autorise une application à accéder à une API, permettant des flux d’authentification entre machines                                     |
  | Enregistrer les informations d’identification d’une application dans un fichier | `auth0_save_credentials_to_file` | `read:client_credentials` | Permet de lire les informations d’identification du client d’une application pour les enregistrer dans un fichier d’environnement local |
