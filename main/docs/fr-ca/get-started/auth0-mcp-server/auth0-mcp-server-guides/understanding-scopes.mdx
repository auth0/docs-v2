...
<Accordion title="Scopes des opérations de gestion">
  ...
  | Créer un grant client pour une application                        | `auth0_create_application_grant` | `create:client_grants`  | Autorise une application à accéder à une API, permettant les flux d’authentification machine à machine                            |
  | Enregistrer les identifiants d’une application dans un fichier   | `auth0_save_credentials_to_file` | `read:client_credentials` | Permet de lire les identifiants client d’une application pour les enregistrer dans un fichier d’environnement local               |
