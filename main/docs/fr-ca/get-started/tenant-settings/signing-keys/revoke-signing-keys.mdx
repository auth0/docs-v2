---
description: Découvrez comment révoquer la clé de signature d’application de votre tenant à l’aide de l’Auth0 Dashboard ou de la Management API.
...
### Révoquer la clé de signature utilisée auparavant {#revoke-previously-used-signing-key}
...
### Rotation et révocation de la clé de signature {#rotate-and-revoke-signing-key}
...
## Utiliser Management API {#use-the-management-api}
...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">Vous utilisez Auth0 CLI ? Si ce n’est pas déjà fait, [configurez votre session CLI et authentifiez-vous](/docs/fr-ca/deploy-monitor/auth0-cli) avant d’exécuter cette commande.</Callout>
...
| `MGMT_API_ACCESS_TOKEN` | [Jeton d’accès pour Management API](https://auth0.com/docs/api/management/v2/tokens) avec la portée `update:signing_keys`.                                                                                            |
