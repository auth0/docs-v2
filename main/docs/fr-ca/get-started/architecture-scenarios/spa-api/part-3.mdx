...
* **audience** : Valeur de l’identifiant de votre API. Vous pouvez la récupérer à partir des [Paramètres de votre API](https://manage.auth0.com/#/apis%7D) dans le Tableau de bord.
...
Par mesure de sécurité, il est recommandé de limiter la durée de vie du jeton d’accès d’un utilisateur. Lorsque vous créez une API dans le Auth0 Dashboard, la durée de vie par défaut est de `7200` secondes (2 heures), mais elle peut être contrôlée par API.
