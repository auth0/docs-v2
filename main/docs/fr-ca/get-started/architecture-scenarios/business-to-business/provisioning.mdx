---
title: Provisionnement interentreprises
sidebarTitle: Provisionnement
...
### Portail d’administration des organisations {#organization-admin-portal}
...
## Migration des utilisateurs {#user-migration}

En plus d’héberger le [profil utilisateur](/docs/fr-ca/get-started/architecture-scenarios/business-to-business/profile-management), Auth0 peut aussi servir de [proxy](#identity-store-proxy) pour votre stockage d’identités hérité et offrir une solution de remplacement sécurisée hébergée par Auth0. Ces deux capacités sont prises en charge au moyen des [Database Connections](/docs/fr-ca/authenticate/database-connections) d’Auth0. Si vous décidez d’utiliser Auth0 pour remplacer votre stockage d’identités hérité, vous pouvez [migrer les utilisateurs](/docs/fr-ca/manage-users/user-migration) soit en une seule fois au moyen d’une migration en bloc, soit progressivement avec une migration automatique.
...
### Proxy de référentiel d’identité {#identity-store-proxy}
...
## Provisionnement des utilisateurs de l’organisation {#provisioning-organization-users}
...
### Provisionnement d’utilisateurs isolés dans l’organisation {#provisioning-users-isolated-to-the-organization}
...
### Limites du déprovisionnement {#deprovisioning-limitations}
...
## Inscription d’entreprise {#enterprise-sign-up}
...
## Architecture à organisations multiples (multitenance) {#multiple-organization-architecture-multitenancy}
