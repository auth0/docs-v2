...
Auth0 prend en charge l’[internationalisation (I18N)](/brand-and-customize/internationalization-and-localization) et la localisation (L10N), deux aspects importants si vous travaillez sur l’image de marque pour une clientèle internationale. Les éléments collatéraux prêts à l’emploi, tels que le gadget logiciel Auth0 Lock UI sont déjà activés pour la prise en charge de plusieurs langues, avec une extensibilité intégrée permettant d’ajouter d’autres langues si ce dont vous avez besoin [n’existe pas encore](/libraries/lock/lock-internationalization).
...
Les pages d’authentification multifacteur (MFA) peuvent être personnalisées en ajustant les options d’image de marque de la connexion universelle dans la section [Paramètres de la connexion universelle](https://manage.auth0.com/#/login_settings).

Si vous avez besoin d’une personnalisation plus complète, vous pouvez également personnaliser le [contenu HTML complet](/mfa/customize-mfa-user-pages/customize-mfa-classic-login) pour refléter les exigences particulières de votre organisation en matière d’expérience utilisateur.