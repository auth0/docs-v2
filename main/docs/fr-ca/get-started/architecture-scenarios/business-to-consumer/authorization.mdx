...
Il existe de nombreux scénarios qui nécessitent qu’une application sans session utilisateur interactive obtienne un jeton d’accès afin d’appeler une API. Dans de tels scénarios, vous devez authentifier le client au lieu de l’utilisateur, et <Tooltip href="/docs/fr-ca/glossary?term=oath2" tip="OAuth 2.0
Cadre d’applications d’autorisation qui définit les protocoles d’autorisation et les flux de production." cta="Voir le glossaire">OAuth</Tooltip> 2 fournit le type d’octroi des [identifiants client](/docs/fr-ca/get-started/authentication-and-authorization-flow/client-credentials-flow) pour faciliter le processus. Quelques exemples courants où cela est nécessaire :
