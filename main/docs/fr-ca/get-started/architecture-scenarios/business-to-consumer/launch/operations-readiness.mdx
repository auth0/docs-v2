...
Si le code personnalisé exécuté dans Auth0 (par exemple dans une Action, une Règle, un Hook ou des scripts personnalisés de base de données) appelle un service à l’intérieur de votre réseau, ou si vous configurez un fournisseur SMTP sur site dans Auth0, il se peut que vous deviez configurer votre pare-feu pour autoriser [le trafic entrant en provenance d’Auth0](/security/data-security/allowlist). Les adresses IP à autoriser dans le pare-feu sont spécifiques à chaque région et sont répertoriées dans les fenêtres de configuration des règles, des crochets, des scripts de base de données personnalisées et du fournisseur de messagerie électronique dans votre Auth0 Dashboard.
...
Si vous utilisez le connecteur AD/LDAP, vous devez vérifier les paramètres de l’équilibreur de charge dans votre environnement pour voir s’il met fin aux connexions inactives de longue durée. Si c’est le cas, vous pouvez modifier les [paramètres de connexion AD/LDAP Auth0](/extensions/ad-ldap-connector/ad-ldap-connector-to-auth0) pour utiliser le paramètre `LDAP_HEARTBEAT_SECONDS` afin d’envoyer des messages périodiques de synchronisation pour maintenir la connexion ouverte.
...
En outre, vous devriez consulter régulièrement la page des [migrations Auth0](/product-lifecycle/deprecations-and-migrations) pour obtenir des informations sur les dépréciations à venir qui pourraient obliger votre équipe à effectuer des changements.
...
* Utiliser [le remplacement des mots de passe](/configure-the-deploy-cli) si vous utilisez l’outil Auth0 Deploy CLI
...
* Vérifiez que vous utilisez la dernière version supportée de node.js dans [Paramètres Auth0 Dashboard](/tenant-settings).
* Vérifiez que vous utilisez une version de la trousse SDK/des bibliothèques prise en charge par Auth0 conformément à la [Matrice d’assistance d’Auth0](/support/product-support-matrix).
