...
L’auto-inscription tire parti des [Connexions de base de données](/docs/fr-ca/authenticate/database-connections) d’Auth0 pour stocker l’identifiant utilisateur, le mot de passe et les informations d’identité du nom d’utilisateur (facultatives) recueillies auprès des nouveaux utilisateurs au cours du processus d’inscription. Les politiques de connexion de base de données régissant des éléments tels que la [longueur minimale du nom d’utilisateur](/docs/fr-ca/authenticate/database-connections/require-username) ou [force et la complexité du mot de passe](/docs/fr-ca/authenticate/database-connections/password-options) peuvent être configurées à partir de <Tooltip href="/docs/fr-ca/glossary?term=auth0-dashboard" tip="Auth0 Dashboard
