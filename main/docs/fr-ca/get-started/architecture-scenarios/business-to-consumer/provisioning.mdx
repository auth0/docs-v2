---
title: Provisioning Business-to-Consumer
...
Auth0 offre un stockage des identités prêt à l’emploi qui peut être utilisé pour stocker les informations d’authentification des utilisateurs de façon sûre et sécurisée. Consultez [inscription libre-service](#self-sign-up) pour en savoir plus. Si vous disposez déjà d’un magasin d’identités hérité et que vous souhaitez en transférer la gestion, les capacités de [migration des utilisateurs](#user-migration) vous offrent plusieurs options pour le faire.

Autrement, si vous devez conserver votre magasin d’identités hérité — peut-être parce que vous avez des applications que vous n’êtes pas encore prêt à migrer ou qui ne peuvent pas l’être —, vous pouvez utiliser la capacité de [proxy du magasin d’identités](#identity-store-proxy). Permettre à vos clients d’utiliser leur propre identité est aussi une option intéressante et, bien que nous constations que nos clients ne le font généralement pas au départ, vous pouvez utiliser la capacité de [inscription sociale](#social-sign-up) pour l’offrir.

## Migration des utilisateurs {#user-migration}

En plus d’héberger le [profil utilisateur](/docs/fr-ca/get-started/architecture-scenarios/business-to-consumer/profile-management), Auth0 peut aussi servir de [proxy](#identity-store-proxy) pour votre propre magasin d’identités hérité et offrir une solution de remplacement sécurisée hébergée par Auth0. Ces deux capacités sont prises en charge au moyen des [Database Connections](/docs/fr-ca/authenticate/database-connections) d’Auth0. Si vous décidez d’utiliser Auth0 pour remplacer votre magasin d’identités hérité, vous pouvez [migrer les utilisateurs](/docs/fr-ca/manage-users/user-migration) soit en une seule opération avec une migration en bloc, soit progressivement avec une migration automatique.
...
### Proxy du magasin d’identités {#identity-store-proxy}
...
## Inscription libre-service {#self-sign-up}
...
## Inscription sociale {#social-sign-up}
