...
### Automatiser Home Realm Discovery (HRD) {#automate-home-realm-discovery-hrd}
...
<Info>
  ### Comment contrôler la durée de la session locale de l’utilisateur dans l’application ? Puis-je la gérer à partir d’Auth0 ? {#how-do-i-control-the-duration-of-the-users-local-application-session-can-i-drive-that-from-auth0}
...
<Info>
  ### Comment la session SSO d’Auth0 a-t-elle une incidence sur la session de l’application ? {#how-does-auth0s-sso-session-impact-the-applications-session}
...
### Installer l’Authorization Extension {#install-the-authorization-extension}
...
### Appliquer les permissions dans votre application {#enforce-permissions-in-your-application}
