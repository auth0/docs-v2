---
description: Configurez les politiques de sécurité d’une Auth0 Team afin d’appliquer des exigences relatives à MFA, aux sessions et à la liste d’autorisation IP pour tous les membres de l’équipe.
...
## Connexions d’IdP d’entreprise disponibles {#available-enterprise-idp-connections}
...
### Ajouter une connexion SSO (Beta) {#add-sso-connection-beta}
...
### Configurer le provisionnement juste-à-temps (JIT) {#configure-just-in-time-jit-provisioning}
...
## Appliquer l’authentification unique {#enforce-single-sign-on}
...
### Utiliser le provisionnement JIT {#use-jit-provisioning}
...
### Gérer manuellement les membres de l’équipe et du tenant {#manage-team-and-tenant-membership-manually}
...
### Configurer la découverte du realm d’origine (HRD) {#configure-home-realm-discovery-hrd}
