---
description: Activez mTLS au niveau du tenant en configurant un domaine personnalisé et des alias de point de terminaison mTLS afin d’accepter les connexions TLS mutuelles.
...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  Les exemples suivants indiquent **$management&#95;access&#95;token**, ou un [jeton d’accès à la Management API](/docs/fr-ca/secure/tokens/access-tokens/management-api-access-tokens). Cette valeur doit être remplacée par un jeton d’accès qui contient au moins les scopes suivants :
...
### Mettre à jour un domaine personnalisé existant {#patch-an-existing-custom-domain}
...
<Tabs>
  <Tab title="Auth0 CLI">
    ```bash
    auth0 api post "custom-domains/:id" \
      --data '{
        "tls_policy":"recommended",
        "custom_client_ip_header":"true-client-ip"
      }'
...
### Vérifier le domaine personnalisé {#verify-the-custom-domain}
...
<Tabs>
  <Tab title="Auth0 CLI">
    ```bash
    auth0 api post "custom-domains/:id/verify"
    ```
  </Tab>
...
### Activer les alias de point de terminaison mTLS {#enable-mtls-endpoint-aliases}
