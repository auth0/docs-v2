...
Les algorithmes de signature sont des algorithmes utilisés pour signer les jetons émis pour votre application ou votre API. Une signature est incluse dans un [Jeton Web JSON (JWT)](/security/tokens/json-web-tokens) et sert à vérifier que l’expéditeur du jeton est bien celui qu’il prétend être et à s’assurer que le message n’a pas été modifié en cours de route.
...
Pour obtenir de l’aide en cas de problème, consultez [Résolution des erreurs de jeton non valide](/troubleshoot/troubleshoot-basic/invalid-token-errors).