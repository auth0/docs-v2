...
Vous pouvez utiliser l’Auth0 <Tooltip href="/docs/fr-ca/glossary?term=management-api" tip="Management API
...
| `MGMT_API_ACCESS_TOKEN` | [Jetons d’accès à Management API](/docs/fr-ca/api/management/v2/tokens) avec la <dfn>scope</dfn> `update:clients`. |
| `OWNERSHIP_BOOLEAN` | La propriété que vous souhaitez spécifier pour l’application. Si l’application est de première partie, `is_first_party` elle doit avoir une valeur de `true`. Si l’application est tierce, `is_third_party` doit avoir une valeur de `false`. |
