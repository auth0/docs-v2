---
description: Effectuez la rotation des informations d’identification Private Key JWT de votre application sans interruption de service en maintenant deux clés actives pendant la période de transition.
  ...
  <Tab title="Utiliser Management API">
    ...
    6. Supprimez la clé inutilisée de votre application. Cela supprimera définitivement l’information d’identification du stockage. Vous devez d’abord dissocier l’information d’identification de votre application, sinon vous ne pourrez pas la supprimer.
       ...
       <Tabs>
         <Tab title="Auth0 CLI">
           ```bash
           auth0 api delete "clients/{clientId}/credentials/{credentialId}"
           ```
         </Tab>
...
### Informations d’identification actives {#active-credentials}
