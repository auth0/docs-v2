---
title: Applications de première partie et applications tierces
description: "Découvrez la différence entre les applications de première partie et les applications tierces dans Auth0."
---
...
## Applications de première partie {#first-party-applications}
...
## de première partie et tierce dans Auth0 {#first-party-vs-third-party-in-auth0}
...
## Propriété de l’application {#application-ownership}
...
## Vérifier à qui appartient l’application {#check-application-ownership}
...
<Tabs>
  ...
  <Tab title="Management API">
    Effectuez un appel `GET` vers le [point de terminaison Get a Client](https://auth0.com/docs/api/management/v2#!/Clients/get_clients_by_id). Assurez-vous de remplacer les valeurs des espaces réservés `{YOUR_CLIENT_ID}` et `{YOUR_MANAGEMENT_API_ACCESS_TOKEN}` par votre <Tooltip tip="ID client : valeur d’identification attribuée à votre ressource enregistrée par Auth0." cta="Voir le glossaire" href="/docs/fr-ca/glossary?term=client+ID">ID client</Tooltip> et votre jeton d’accès à l’API Management <Tooltip tip="Jeton d'accès : identifiant d'autorisation, sous forme de chaîne opaque ou de JWT, utilisé pour accéder à une API." cta="Voir le glossaire" href="/docs/fr-ca/glossary?term=Access+Token">jeton d’accès</Tooltip>, respectivement.
