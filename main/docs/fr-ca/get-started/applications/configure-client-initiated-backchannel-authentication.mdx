...
## Activez les canaux de notification pour votre application {#enable-notification-channels-for-your-application}
...
## Configurer le canal de notification {#configure-notification-channel}
...
#### Inscrire l’utilisateur autorisant à l’authentification multifacteur au moyen des notifications push {#enroll-the-authorizing-user-in-mfa-using-push-notifications}
...
#### Configurez votre fournisseur de courriel {#configure-your-email-provider}
...
#### Configurez votre modèle de courriel {#configure-your-email-template}
...
<Tabs>
  ...
  <Tab title="Management API">
    ...
    <Tabs>
      <Tab title="Auth0 CLI">
        ```bash
        auth0 api post "email-templates" \
          --data '{"template":"async_approval","body":"{{application.name}} says click on {{url}}, binding message: {{binding_message}}","subject":"Action Required","syntax":"liquid","enabled":true,"from":""}'
        ```
      </Tab>
...
#### S’assurer que l’utilisateur qui autorise dispose d’une adresse courriel vérifiée {#ensure-the-authorizing-user-has-a-verified-email-address}
