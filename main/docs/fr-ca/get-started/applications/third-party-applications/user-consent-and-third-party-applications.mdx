...
description: "Découvrez comment Auth0 gère le consentement de l’utilisateur lorsque des applications demandent à accéder aux API en son nom."
...
## Boîte de dialogue de consentement {#consent-dialog}
...
## Descriptions des portées {#scope-descriptions}

Par défaut, la page de consentement utilise les noms de portée pour obtenir le consentement de l’utilisateur. Comme illustré ci-dessous, définissez les portées au format `action:resource_name` pour un affichage clair :
...
La page de consentement regroupe les portées d’une même API et affiche toutes les actions sur une seule ligne. Par exemple, la configuration ci-dessus produit **Posts: lire et écrire vos publications**.

Pour afficher le champ **Description** au lieu du nom de la portée, définissez l’indicateur `use_scope_descriptions_for_consent` du tenant sur `true` :
...
## Gérer les permissions refusées {#handle-rejected-permissions}
...
## Ignorer le consentement pour les applications de première partie {#skip-consent-for-first-party-applications}
...
## Consentement avec les Organizations {#consent-with-organizations}
...
<Frame>![Boîte de dialogue de consentement pour l'Organization Acme — application tierce demandant l'accès au compte Acme de l'utilisateur](/docs/images/third-party-applications/org-scoped-consent-acme.png)</Frame>
...
<Frame>![Boîte de dialogue de consentement pour l'Organization Contoso — le même utilisateur doit consentir de nouveau pour une autre Organization](/docs/images/third-party-applications/org-scoped-consent-contoso.png)</Frame>
...
## Révoquer le consentement {#revoke-consent}
...
## Flux avec mot de passe {#password-based-flows}
...
## Forcer le consentement {#force-consent}
