...
## Identifier les problèmes liés aux applications tierces {#identify-third-party-application-issues}
...
### `unauthorized_client` lors d’une demande de jetons {#unauthorized_client-when-requesting-tokens}
...
### `unauthorized_client` même avec la politique d’accès à l’API « Allow All » {#unauthorized_client-even-with-allow-all-api-policy}
...
### `invalid_request` sur `/authorize` avec des paramètres non pris en charge {#invalid_request-on-authorize-with-unsupported-parameters}
...
### `unsupported_response_type` pour `id_token` ou `token` {#unsupported_response_type-for-id_token-or-token}
...
### Aucun ID token renvoyé par `/oauth/token` {#no-id-token-returned-from-oauthtoken}
...
### Type d’octroi non pris en charge {#grant-type-not-supported}
...
### Classic Login ne fonctionne pas {#classic-login-not-working}
...
### Le Client ID commence par `tpc_` {#client-id-starts-with-tpc_}
...
### Impossible de modifier `is_first_party` ou le mode de sécurité {#cannot-change-is_first_party-or-security-mode}
...
### La vérification de l’e-mail ou la réinitialisation du mot de passe affiche une page d’erreur {#email-verification-or-password-reset-shows-an-error-page}
...
### Le client DCR n’a accès à aucune API {#dcr-client-cannot-access-any-api}
...
### `/userinfo` renvoie une erreur {#userinfo-returns-error}
...
### `/oauth/revoke` fonctionne, mais pas les points de terminaison de déconnexion {#oauthrevoke-works-but-logout-endpoints-do-not}
...
### Connexion non disponible pour une application tierce {#connection-not-available-for-a-third-party-application}
...
### `invalid_request` lors de l’authentification via une Organization {#invalid_request-when-authenticating-through-an-organization}
...
### La rotation des jetons d’actualisation cause des problèmes {#refresh-token-rotation-causing-issues}
