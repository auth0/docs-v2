...
## Créer une application tierce {#create-a-third-party-application}
...
<Tabs>
  ...
  <Tab title="Management API">
    ...
    | `callbacks`                  | Array    | Une liste des URL de rappel autorisées pour la redirection après l’authentification.                                                                                                                                                                                                                                                           |
    ...
    En cas de succès, Auth0 crée l’application avec un préfixe de Client ID `tpc_`, les grant types `authorization_code` et `refresh_token`, et définit `third_party_security_mode` à `strict`, ce qui indique que des mesures de sécurité renforcées sont activées.
...
## Configurer les politiques d’accès aux API {#configure-api-access-policies}
...
### Permissions par application {#per-application-permissions}
...
### Permissions par défaut pour toutes les applications tierces {#default-permissions-for-all-third-party-applications}
...
## Configurer l’accès aux Organizations {#configure-organizations-access}
...
Les administrateurs d’Organization peuvent aussi configurer eux-mêmes l’accès des applications tierces pour leur Organization en définissant l’attribut `third_party_client_access` au moyen de l’API My Organization. Pour en savoir plus, consultez [Configure client application attributes](/docs/fr-ca/manage-users/my-organization-api#configure-client-application-attributes).

{/* TODO: AuthCodeGroup — exemple de /authorize avec le paramètre `organization` */}
...
### Résumé du comportement {#behavior-summary}
...
## Configurer la protection contre les redirections ouvertes {#configure-open-redirect-protection}

Les applications tierces dotées de mesures de sécurité renforcées ont la **Protection contre les redirections ouvertes** activée par défaut. Lorsqu’elle est activée, Auth0 ne :
...
## Enregistrement dynamique des clients {#dynamic-client-registration}
