...
| Permission | Demandes |
| --- | --- |
| `openid` | (obligatoire) Renvoie la demande `sub`, qui identifie de manière unique l’utilisateur. Dans un jeton d’ID, les demandes `iss`, `aud`, `exp`, `iat`, et `at_hash` seront également présentes. Pour en savoir plus sur les demandes du jeton d’ID, veuillez consulter [Structure du jeton d’ID](/docs/fr-ca/tokens/id-tokens/id-token-structure). |
