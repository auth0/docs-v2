---
description: Découvrez comment utiliser une seule API logique dans Auth0 pour représenter plusieurs API et gérer l’accès à celles-ci.
...
## Activer une connexion pour votre application {#enable-a-connection-for-your-application}
...
## Créer un utilisateur de test {#create-a-test-user}
...
## Enregistrer une API logique dans Auth0 {#register-a-logical-api-in-auth0}
...
## Configurer les permissions pour l’API logique {#configure-permissions-for-the-logical-api}
...
## Accorder l’accès à l’API logique {#grant-access-to-the-logical-api}
...
3. S’il s’agit de la première fois que l’utilisateur passe par ce flux, il voit un écran de consentement qui présente les permissions qu’Auth0 accordera à la SPA. Dans ce cas, on lui demande d’autoriser l’application à lire ses contacts et son calendrier.
   ...
   <Frame>![Écran de consentement Lock de l’application exemple](/docs/images/cdy7uua7fh8z/1te4FYRbu0aFcdohdXY2Rv/116bed5515eb2114c39374fb0a258912/consent-screen.png)</Frame>
4. Si l’utilisateur donne son consentement, Auth0 le redirige vers la SPA avec des jetons dans le fragment de hachage de l’URI. La SPA peut maintenant extraire les jetons du fragment de hachage à l’aide de JavaScript et utiliser le jeton d’accès pour appeler vos API au nom de l’utilisateur.
   ...
   <Frame>![Écran d’autorisation de l’utilisateur de l’application exemple](/docs/images/cdy7uua7fh8z/2pGWG5Wa7U1tBPhAJZ7Bat/7e194066560605cc842f20624b80d958/apis.png)</Frame>

### Mettre en œuvre la déconnexion unique (SLO) ou l’authentification unique (SSO) {#implement-single-logout-slo-or-single-sign-on-sso}
