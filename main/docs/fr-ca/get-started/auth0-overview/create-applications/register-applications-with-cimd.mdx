...
### Phase 1 : Enregistrement {#phase-1-registration}
...
### Phase 2 : Autorisation {#phase-2-authorization}
...
### Configuration du tenant {#tenant-configuration}
...
## Enregistrer des applications avec un CIMD manuel {#register-applications-with-manual-cimd}
...
### Prévisualiser le CIMD {#preview-cimd}
...
### Enregistrer le client CIMD {#register-cimd-client}
...
## Configurer le client CIMD {#set-up-cimd-client}
...
## Actualiser les métadonnées du client {#refresh-client-metadata}
...
## Obtenir le client CIMD {#get-cimd-client}

Pour obtenir un client CIMD, faites une requête `GET` vers le point de terminaison `/v2/clients/{clientId}`, où `{clientID}` correspond à l’ID client généré par Auth0 attribué au client CIMD :
...
## Mettre à jour le client CIMD {#update-cimd-client}
...
Pour mettre à jour un client CIMD, envoyez une requête `PATCH` au point de terminaison `/v2/clients/{clientId}`, où `{clientID}` est l’ID client généré par Auth0 et attribué au client CIMD :
...
## Règles de validation des URL CIMD {#cimd-url-validation-rules}
...
## Règles de validation JSON CIMD {#cimd-json-validation-rules}
...
### Rotation des clés pour les clients CIMD avec l’authentification private_key_jwt {#cimd-client-key-rotation-for-private_key_jwt-authentication}
