...
Dans Auth0, la meilleure méthode pour mettre en œuvre la multilocation est [Auth0 Organizations](/organizations/organizations-overview). Si nécessaire, d’autres solutions héritées peuvent être utilisées pour répondre à des cas d’utilisation distincts. Si vous proposez un produit ou un service [commerce interentreprises (B2B)](/architecture-scenarios/b2b), la mise en place d’une multilocation pour vos utilisateurs professionnels peut s’avérer avantageuse pour votre cas d’utilisation.
...
Avec Auth0 Organizations, vous pouvez créer des groupes uniques d’utilisateurs et personnaliser leurs expériences avec un [contrôle d’accès basé sur les rôles (RBAC)](/rbac), [des pages de connexion et des modèles de courriel personnalisés](/manage-users/organizations/create-first-organization#customize-prompts-and-email-templates), et plus encore.

Pour en savoir plus sur l’utilisation d’Auth0 Organizations pour mettre en œuvre la multilocation, consultez l’article [Architecture d’organizations multiples](/architecture-scenarios/multiple-organization-architecture).

## Solutions héritées

Si Auth0 Organizations ne répond pas aux exigences de votre cas d’utilisation, veuillez contacter notre équipe de [services professionnels](/professional-services) pour trouver une solution qui assure votre succès.
