...
L’extension Real-time Webtask Logs affiche tous les journaux du code personnalisé en temps réel, y compris la sortie `console.log` et autres exceptions. Si vous utilisez Auth0 Actions ou une autre logique personnalisée, vous pouvez utiliser cette extension pour déboguer et dépanner les problèmes. Pour en savoir plus sur l’installation et la configuration, veuillez consulter [Journaux d'actions en temps réel](/docs/fr-ca/customize/actions/actions-real-time-logs).
