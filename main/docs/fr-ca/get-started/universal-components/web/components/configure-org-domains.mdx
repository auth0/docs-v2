...
## Installer le composant {#install-the-component}
      ...
      export default function App() {
        ...
        return (
          <Auth0Provider
            ...
            interactiveErrorHandler="popup" // Requis pour gérer les défis d’authentification renforcée au moyen d’une fenêtre contextuelle de connexion universelle
          >
...
### Props d’action {#action-props}
    ...
    | `associateToProviderAction` | `ComponentAction<Domain, IdentityProvider>` | Associe le domaine au fournisseur.                               |
    | `deleteFromProviderAction`  | `ComponentAction<Domain, IdentityProvider>` | Retire le domaine du fournisseur.                                |
    | `onOpenProvider`            | `(provider: IdentityProvider) => void`      | Accède au fournisseur depuis la fenêtre modale de configuration. |
    ...
    **Type :** `ComponentAction<Domain>`
    ...
    Contrôle le flux de création de domaine. Utilisez `onAfter` pour faire le suivi de l&#39;ajout de nouveaux domaines.
...
### Propriétés de personnalisation {#customization-props}
...
## Installer le composant {#install-component}
...
### Props de l’action {#action-props-2}
...
## Installer le composant {#install-the-component-2}
    ...
    <Accordion title="Registre Vercel Legacy (déconseillé)">
...
### Props de l’action {#action-props-3}
    ...
    Les props d&#39;action gèrent les interactions des utilisateurs et définissent ce qui se passe lorsque ceux-ci effectuent des opérations de domaine. Utilisez les hooks du cycle de vie (`onBefore`, `onAfter`) pour vous intégrer au routage et à l&#39;analyse de votre application.
...
### Props de personnalisation {#customization-props-3}
