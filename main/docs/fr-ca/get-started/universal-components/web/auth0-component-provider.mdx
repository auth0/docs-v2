...
## Architecture d’intégration {#integration-architecture}
...
## Configurer `Auth0ComponentProvider` {#configure-auth0componentprovider}
    ...
    function App() {
      return (
        <Auth0Provider
          ...
          interactiveErrorHandler='popup' // Requis pour gérer les défis d'authentification renforcée via la fenêtre contextuelle d'Universal Login
        >
...
### Renouvellement des jetons {#token-renewal}

    Par défaut, le SDK Auth0 React utilise l’authentification silencieuse pour obtenir de nouveaux jetons d’accès. Si votre application active le renouvellement par jeton d’actualisation, configurez les jetons d’actualisation pour l’application Auth0 et définissez `useRefreshTokens={true}` sur `Auth0Provider` :
...
### Utilisation avec les composants My Organization {#use-with-my-organization-components}

    Les composants My Organization demandent des jetons d’accès limités à l’utilisateur pour l’API My Organization. Ils fournissent le public et les permissions requis lors des requêtes d’API.
...
## État et rendement {#state-and-performance}

    Ajustez finement la mise en cache de TanStack Query pour chaque composant Auth0. Par défaut, les données restent à jour pendant deux minutes, sont supprimées après cinq minutes et ne sont pas récupérées de nouveau lorsque la fenêtre reprend le focus.
    ...
    <table class="table">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            Nombre de millisecondes avant que les données ne deviennent périmées. La valeur par défaut est de deux minutes. Augmentez cette valeur
        ...
        <tr>
          ...
          <td>
            Nombre de millisecondes avant que les requêtes inactives ne soient supprimées par le ramasse-miettes. La valeur par défaut est de cinq minutes.
          </td>
        </tr>
    ...
    <Callout icon="file-lines" color="#0EA5E9" iconType="regular">
      **Désactiver la mise en cache :** Transmettez `{ enabled: false }`. Cela règle automatiquement `staleTime` à 0 et réduit le délai avant récupération de mémoire à cinq secondes afin que chaque rendu récupère des données à jour.

      **Conseil de pro :** Gardez la mise en cache activée, mais réduisez `staleTime` lors de l’intégration à des panneaux d’administration qui exigent des mises à jour quasi instantanées.
    </Callout>
...
## Configurer le mode proxy {#configure-proxy-mode}
...
### Propriétés du mode proxy {#proxy-mode-properties}
...
## Expérience utilisateur {#user-experience-2}
...
## État et rendement {#state-and-performance-2}
    ...
    <table class="table">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            Nombre de millisecondes avant que les requêtes inactives ne soient supprimées par le ramasse-miettes (cinq minutes par
            défaut).
          </td>
        </tr>
    ...
    <Callout icon="file-lines" color="#0EA5E9" iconType="regular">
      **Désactiver la mise en cache :** Transmettez `{ enabled: false }`. Cela règle automatiquement `staleTime` à 0 et réduit le délai avant récupération de mémoire à 5 secondes afin que chaque rendu récupère des données à jour.
...
## Expérience utilisateur {#user-experience-3}
...
## État et rendement {#state-and-performance-3}
