---
description: Découvrez comment déterminer le flux OAuth 2.0 approprié pour votre type d’application, des applications Web côté serveur et des SPA aux flux machine à machine et d’appareil.
...
## Terminologie OAuth 2.0 {#oauth-20-terminology}
...
## Le Client est-il le propriétaire de la ressource ? {#is-the-client-the-resource-owner}
...
## Le Client est-il une application web qui s’exécute sur le serveur ? {#is-the-client-a-web-app-executing-on-the-server}
...
## Peut-on faire absolument confiance au client avec les identifiants de l’utilisateur? {#is-the-client-absolutely-trusted-with-user-credentials}
...
## Le client est-il une application monopage ? {#is-the-client-a-single-page-app}
...
## Le client est-il une application native/mobile ? {#is-the-client-a-nativemobile-app}
...
## J’ai une application qui doit communiquer avec différents serveurs de ressources {#i-have-an-application-that-needs-to-talk-to-different-resource-servers}
...
## Puis-je tester les endpoints avant d’implémenter mon application? {#can-i-try-the-endpoints-before-i-implement-my-application}
...
## L’Application cliente doit-elle demander aux utilisateurs de s’authentifier sans passer par un navigateur? {#does-the-client-application-need-to-challenge-users-for-authentication-without-browser-interaction}
