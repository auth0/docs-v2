...
og:title: Éviter les problèmes courants liés au flux de mot de passe du propriétaire
  de ressource et à la protection contre les attaques
og:url: https://auth0.com/docs/
permalink: avoid-common-issues-with-resource-owner-password-flow-and-attack-protection
title: Éviter les problèmes courants liés au flux de mot de passe du propriétaire
  de ressource et à la protection contre les attaques
twitter:description: Learn how to avoid common issues encountered when using the Resource
  Owner Password Flow to call server-side APIs with attack protection enabled.
twitter:title: Éviter les problèmes courants liés au flux de mot de passe du propriétaire
  de ressource et à la protection contre les attaques
---



Bien que nous ne le recommandons pas, les applications hautement fiables peuvent utiliser le [Flux de mot de passe du propriétaire de ressource](/login/flows/resource-owner-password-flow), (parfois appelé consentement de mot de passe au propriétaire de la ressource ou ROPG), qui demande aux utilisateurs de fournir des informations d’identification (nom d’utilisateur et mot de passe), généralement à l’aide d’un formulaire interactif. Si la [protection contre la force brute est activée](/attack-protection/brute-force-protection), lorsque Auth0 valide les informations d’identification, nous pouvons également [vérifier les attaques](/attack-protection) et effectuer les actions appropriées si une attaque est détectée.
...
Enregistrez soit une [application Web standard](/get-started/create-apps/regular-web-apps), soit une [application de communication entre machines](/get-started/create-apps/machine-to-machine-apps). Lors de la configuration de l’application :
...
1. Lorsque vous demandez des jetons à l’aide du [flux de mot de passe du propriétaire de ressource](/login/flows/call-your-api-using-resource-owner-password-flow), incluez un en-tête `auth0-forwarded-for` qui contient la valeur de l’adresse IP de l’utilisateur. Assurez-vous que l’adresse IP que vous fournissez appartient réellement à votre utilisateur.
...
Si vous avez activé [Détection de mot de passe violé](/attack-protection/breached-password-detection) pour votre locataire, vous devez configurer votre application pour gérer les réponses de l’[Authentication API Auth0](/docs/api/authentication) en conséquence.
