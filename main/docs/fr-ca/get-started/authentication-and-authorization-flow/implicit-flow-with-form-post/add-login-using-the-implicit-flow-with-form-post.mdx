...
| `nonce` | (obligatoire pour le paramètre `response_type` contenant `id_token token`. Une chaîne cryptographiquement aléatoire que votre application ajoute à la requête initiale et qu’Auth0 inclut dans le jeton d’ID, [utilisée pour prévenir les attaques par réinsertion de jeton](/docs/fr-ca/api-auth/tutorials/nonce). |
