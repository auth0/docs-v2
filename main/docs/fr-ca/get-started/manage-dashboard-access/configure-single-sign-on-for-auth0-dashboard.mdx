...
description: Configurez l’authentification unique pour Auth0 Dashboard afin de permettre aux membres du tenant de s’authentifier par l’intermédiaire de votre fournisseur d’identité d’entreprise, en plus des méthodes de connexion existantes.
---
...
Vous pouvez configurer Auth0 pour permettre aux [Tenant Member](/docs/fr-ca/get-started/manage-dashboard-access) d’utiliser votre propre [fournisseur d’identité d’entreprise](/docs/fr-ca/authenticate/identity-providers/enterprise-identity-providers) (<Tooltip tip="Identity Provider (IdP) : service qui stocke et gère les identités numériques." cta="Voir le glossaire" href="/docs/fr-ca/glossary?term=IdP">IdP</Tooltip>) pour s’authentifier dans l’<Tooltip tip="Auth0 Dashboard : principal produit d’Auth0 pour configurer vos services." cta="Voir le glossaire" href="/docs/fr-ca/glossary?term=Auth0+Dashboard">Auth0 Dashboard</Tooltip> au moyen de l’<Tooltip tip="Authentification unique (SSO) : méthode d’authentification qui permet à un utilisateur de se connecter une seule fois et d’accéder à plusieurs applications sans se réauthentifier." cta="Voir le glossaire" href="/docs/fr-ca/glossary?term=single+sign-on">authentification unique</Tooltip> (SSO).

## Fonctionnement {#how-it-works}

La configuration du SSO pour le Dashboard exige de travailler avec soutien Auth0 afin d’ajouter une Enterprise connexion à la root tenant authority (RTA), qui gère les méthodes d’authentification qu’un membre du tenant peut utiliser pour se connecter au Dashboard.
...
#### Accès complet à l’annuaire dans le Dashboard {#full-directory-access-to-dashboard}

Lorsque vous ajoutez votre IdP comme connexion disponible pour permettre aux Tenant Members de se connecter, tous les utilisateurs de l’annuaire de votre IdP peuvent accéder au Dashboard, mais seuls les Tenant Members invités à un tenant donné peuvent accéder à ce tenant.

Lorsque des utilisateurs tentent d’accéder à un tenant dans le Dashboard sans y avoir été invités, le système refuse l’opération. Si les utilisateurs n’appartiennent à aucun tenant, le système les invite à [compléter leur profil utilisateur](#complete-user-profile) et à créer un nouveau tenant d’essai. La création d’un nouveau tenant d’essai ne l’associe pas à votre forfait Enterprise.

#### Identités résiduelles des membres du tenant {#residual-tenant-member-identities}
...
## Configurer le SSO pour le Dashboard {#configure-sso-for-the-dashboard}

La configuration du SSO pour le Dashboard exige une série d’étapes à réaliser conjointement avec soutien Auth0.

### Partager les données de configuration de l’IdP {#share-idp-configuration-data}
...
5. [Ajoutez la claim d’adresse courriel](https://learn.microsoft.com/en-us/azure/active-directory/develop/active-directory-optional-claims) à l’ID Token.
...
4. Transmettez au soutien Auth0 les données XML des métadonnées SAML. Vous pouvez soit :
...
##### Flux d’authentification initiés par l’IdP {#idp-initiated-authentication-flows}
...
#### IdP générique (OIDC) {#generic-idp-oidc}
...
#### IdP générique (SAML) {#generic-idp-saml}
...
### Configurer la connexion SSO {#set-up-sso-connection}
...
### Tester la connexion SSO {#test-sso-connection}
...
## Appliquer l’authentification unique avec Auth0 Teams {#enforce-sso-with-auth0-teams}
...
### Activer Home Realm Discovery (facultatif) {#enable-home-realm-discovery-optional}
...
#### Quand activer HRD {#when-to-enable-hrd}

Si HRD est activé, les Tenant Members qui utilisaient auparavant une identité par courriel et mot de passe (avec une adresse courriel correspondant au domaine HRD configuré) ne pourront plus se connecter à partir de la page de connexion et seront plutôt redirigés vers la nouvelle connexion SSO. HRD n’est compatible qu’avec les identifiants de courriel.
...
#### Comment contourner HRD {#how-to-bypass-hrd}
...
#### Exemple de comportement de connexion HRD {#example-of-hrd-login-behavior}
...
## Migrer les Tenant Members existants {#migrate-existing-tenant-members}

Le processus de migration des Tenant Members existants dépend de l’activation ou non de HRD.

### Comment migrer avec HRD désactivé {#how-to-migrate-with-hrd-disabled}
...
2. Demandez au membre du tenant de :
   ...
   1. se connecter à la nouvelle connexion SSO à l’aide de l’URL de connexion directe avant d’accepter l’invitation.
      ...
      <Callout icon="file-lines" color="#0EA5E9" iconType="regular">
        S’il s’agit de sa première connexion à la nouvelle connexion SSO, il se peut qu’il voie une page d’enrichissement du profil (à l’adresse https://auth0.com/profile). Ne remplissez aucun champ et ne sélectionnez pas **Next**. Suivez plutôt l’étape suivante.
      </Callout>
...
### Comment migrer avec HRD activé {#how-to-migrate-with-hrd-enabled}
