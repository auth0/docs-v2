...
La documentation de l’Auth0 Management API suit le [schéma OpenAPI v3.1 de l’Auth0 Management API](https://auth0.com/docs/oas/management/v2/management-api-oas.json). Veuillez noter que la prise en charge du schéma OpenAPI v3.1 est actuellement en version Beta.

## Authentification {#authentication}

L’utilisation de l’Auth0 Management API nécessite un jeton d’accès à la Management API. Pour savoir comment demander ce jeton, consultez [Jetons d’accès à la Management API](https://auth0.com/docs/secure/tokens/access-tokens/management-api-access-tokens).

L’Auth0 Management API utilise des JSON Web Tokens (JWT) pour authentifier les requêtes. La claim `scopes` du jeton d’accès à la Management API indique quelles méthodes de requête peuvent être exécutées lors d’un appel à cette API. L’exemple de jeton désérialisé sur cette page accorde un accès en lecture seule aux utilisateurs et un accès en lecture/écriture aux connexions. Toute tentative d’exécuter une méthode de requête non autorisée par les scopes définis entraînera une réponse **403 Forbidden**.
...
## Corrélation des requêtes {#request-correlation}
...
### Pagination par point de contrôle {#checkpoint-based-pagination}
...
#### Expiration de l’ID de point de contrôle {#checkpoint-id-expiry}
...
#### Contraintes de progression vers l’avant {#forward-only-constraints}
...
### Choisir entre la pagination par décalage et la pagination par point de contrôle {#choosing-between-offset-and-checkpoint-pagination}
...
### Bonnes pratiques pour gérer la pagination {#best-practices-for-handling-pagination}
...
### Tester avec un jeton d’accès {#test-with-an-access-token}
