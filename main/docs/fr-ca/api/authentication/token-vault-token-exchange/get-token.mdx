...
<ParamField body="subject_token_type" type="string" required>
  Le type du jeton du sujet. Pour l’échange de jeton d’actualisation, utilisez `urn:ietf:params:oauth:token-type:refresh_token`. Pour l’échange de jeton d’accès, utilisez `urn:ietf:params:oauth:token-type:access_token`.
</ParamField>
...
<ParamField body="scope" type="string">
  <Badge color="yellow">Accès anticipé</Badge> (Facultatif) Une liste de portées séparées par des espaces à demander pour le jeton d’accès du fournisseur externe. Utilisez-la pour demander un sous-ensemble des portées accordées initialement. Si elle n’est pas fournie, le jeton est émis avec toutes les portées accordées initialement. Pour l’échange de jeton de travailleur privilégié (`subject_token_type` est `urn:ietf:params:oauth:token-type:jwt`), les portées demandées sont croisées avec les portées configurées dans `token_vault_privileged_access.grants` pour la connexion spécifiée.
</ParamField>
