...
# Metric Streams (bêta) {#metric-streams-beta}
...
## Ce que vous pouvez surveiller {#what-you-can-monitor}
...
### Repérer les problèmes de limites de débit {#identify-rate-limit-issues}
...
### Diagnostiquer les échecs de l’API {#troubleshoot-api-failures}
...
### Capacité du forfait {#plan-capacity}
...
## Référence des métriques {#metric-reference}

### auth0.api_request.count {#auth0api_requestcount}
...
#### Balises disponibles {#available-tags}
...
#### Valeurs d’énumération {#enum-values}
...
### Créer un flux de métriques {#create-metric-stream}
...
<Tabs>
  <Tab title="Dashboard">
    ...
    **Configurer le flux :**
    ...
    3. **clé d’API Datadog**: Collez votre clé API Datadog.
    4. Sélectionnez **Save**.
...
### Vérifiez dans Datadog {#verify-in-datadog}
...
## Configurer New Relic {#set-up-new-relic}

Transmettez les métriques Auth0 à New Relic à l’aide d’OpenTelemetry Protocol (OTLP).

### Obtenir les identifiants New Relic {#get-new-relic-credentials}
...
### Créer un flux de métriques {#create-metric-stream-2}
...
### Créer un flux de métriques {#create-metric-stream-3}
...
## Comprendre la cardinalité des métriques {#understand-metric-cardinality}

La cardinalité correspond au nombre de séries chronologiques uniques générées par vos métriques. Plus la cardinalité est élevée, plus les coûts de surveillance augmentent.
...
### Les métriques n’apparaissent pas {#metrics-not-appearing}
...
### Coûts de surveillance élevés {#high-monitoring-costs}
