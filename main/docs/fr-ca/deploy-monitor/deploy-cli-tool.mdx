...
La décision de choisir l’un ou l’autre ou les deux dépend de votre cas d’utilisation particulier et de vos préférences. Pour en savoir plus, consultez [Configurer l’outil Deploy CLI](/configure-the-deploy-cli).
...
Pour une liste complète des drapeaux et des options, veuillez lire [Utiliser comme interface de ligne de commande](/use-as-a-cli).