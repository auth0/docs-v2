...
<td>`state`</td>
<td>facultatif</td>
<td>(Chaîne)  Une valeur arbitraire qui devrait être maintenue dans les redirections. Il est utile pour atténuer les attaques CSRF et pour toute information contextuelle (par exemple, une URL de retour) dont vous pourriez avoir besoin après le processus d’authentification. Pour plus d’informations, consultez <a href="/docs/fr-ca/secure/attack-protection/state-parameters">Paramètres d’état</a>. auth0.js, lorsqu’il est utilisé dans les applications à page unique, gère la génération et la validation d’état automatiquement si elle n’est pas spécifiée.</td>
</tr>
<tr>
<td>`prompt`</td>
<td>facultatif</td>
...
<td>`user_metadata`</td>
<td>optionnel</td>
<td>(objet JSON) Attributs additionnels utilisés pour les informations de l'utilisateur. Ils seront stockés dans <a href="/docs/fr-ca/manage-users/user-accounts/metadata">user_metadata</a></td>
</tr>
</tbody>
</table>
