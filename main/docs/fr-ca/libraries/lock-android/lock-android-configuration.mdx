...
* **withAuthStyle(String, int)**: personnalise l’apparence d’une connexion donnée (nom) avec un style particulier. Consultez [ce document sur les connexions OAuth personnalisées](/libraries/lock-android/lock-android-custom-theming) pour en savoir plus.
* **withConnectionScope(String, String...)**: permet d’indiquer des permissions supplémentaires pour un nom de connexion donné, qui seront demandées en même temps que celles définies dans les paramètres de connexion dans [Auth0 Dashboard](%24%7Bmanage_url%7D). Les permissions ne sont en aucun cas validées et doivent être reconnues par le fournisseur d’authentification donné. Pour obtenir la liste, cochez [Auth0 Dashboard](%24%7Bmanage_url%7D) dans les paramètres de la connexion concernée.
