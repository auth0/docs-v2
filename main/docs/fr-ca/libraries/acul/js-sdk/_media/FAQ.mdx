...
Vous trouverez ci-dessous un certain nombre de questions ou de problèmes soulevés par l’utilisation du SDK.

## Comment puis-je importer différents écrans dans mon projet? {#how-do-i-import-different-screens-in-my-project}
...
## Où puis-je voir les erreurs liées à mon écran et comment les gérer {#where-can-i-see-errors-related-to-my-screen-and-how-to-handle-them}

Le SDK offre plusieurs façons de détecter et d’afficher les erreurs pendant le processus :

1. **Méthode `getErrors` pour une erreur propre à l’écran**
   Une fonction getter de premier niveau, `getErrors`, peut être utilisée pour récupérer des erreurs d’authentification détaillées lorsque la validation côté backend échoue.
   ```javascript
      import Login from "@auth0/auth0-acul-js/login";
   ...
   ```
2. **Erreurs propres à l’écran**
   Chaque écran comprend une propriété `transaction.errors`, qui renvoie une liste d’erreurs propres à cet écran. Ces erreurs sont générées après la validation finale de la requête, ce qui vous aide à repérer les problèmes nécessitant votre attention.

En utilisant ces méthodes, vous pouvez facilement suivre et corriger les erreurs tout au long du flux du SDK.

## Puis-je utiliser ce SDK depuis un CDN? {#can-i-use-this-sdk-from-a-cdn}

Pour le moment, ce SDK est offert uniquement à l’installation à partir du registre npm et ne peut pas être utilisé directement depuis un CDN.
...
## Puis-je créer une application ReactJS avec ce SDK ? {#can-i-build-a-reactjs-application-with-this-sdk}

Oui, ce SDK peut facilement être intégré à une application ReactJS.

Bien que le SDK soit écrit en JavaScript Vanilla, il est entièrement compatible avec tous les frameworks basés sur JavaScript, notamment React, Angular, Vue et d’autres. Vous pouvez l’utiliser sans problème dans vos composants React ou vos hooks.