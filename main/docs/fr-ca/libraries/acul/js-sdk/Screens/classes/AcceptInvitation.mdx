...
La classe AcceptInvitation implémente la fonctionnalité de l’écran `accept-invitation`. Cet écran s’affiche lorsqu’un utilisateur doit accepter une invitation pour se joindre à une organisation.
...
<ParamField body="countryCodes" type={<span><a href="/docs/fr-ca/libraries/acul/js-sdk/Screens/interfaces/CountryCodesMembers">CountryCodesMembers</a></span>}>
  Fournit des données sur les indicatifs de pays pour la saisie d’un numéro de téléphone, comme `available` et `recommended`, pour l’écran `accept-invitation`.
</ParamField>
