...
<ParamField body="countryCodes" type={<span><a href="/docs/fr-ca/libraries/acul/js-sdk/Screens/interfaces/CountryCodesMembers">CountryCodesMembers</a></span>}>
  Fournit des données sur les codes de pays pour la saisie d’un numéro de téléphone, comme `available` et `recommended`, pour l’écran `login-id`.
</ParamField>
...
<ParamField body="login" type="Promise<void>">
  This méthode invite l’utilisateur à saisir son nom d’utilisateur.
  ...
  **Paramètres de la méthode**
  ...
  <Expandable title="Paramètres">
    ...
    <ParamField body="identifier?" type="string">
      L’identifiant de l’utilisateur — courriel, nom d’utilisateur ou numéro de téléphone, selon `identifierType`. Fournissez celui-ci ou `username`; si les deux sont fournis, `identifier` prévaut.
    </ParamField>

    <ParamField body="username?" type="string">
      L’identifiant de l’utilisateur dans sa forme d’origine. Fournissez celui-ci ou `identifier`.
    </ParamField>
    ...
    <ParamField body="identifierType?" type={<span><a href="/docs/fr-ca/libraries/acul/js-sdk/Screens/type-aliases/IdentifierType">IdentifierType</a></span>}>
      Indique ce que contient `identifier` (ou `username`). Généralement lu à partir de `screen.data.activeIdentifierType`. Omettez-le pour soumettre l’identifiant sans préciser son type.
    </ParamField>

    <ParamField body="phoneCountryCode?" type="string">
      Le pays ISO 3166-1 alpha-2 associé à un identifiant téléphonique, à partir d’un `code` dans `countryCodes.available`. Requis lorsque `identifierType` est `phone`. Transmettez le numéro national — le code de numérotation est ajouté côté serveur.
    </ParamField>

    <ParamField body="captcha?" type="string">
      Le code CAPTCHA ou la réponse du fournisseur de CAPTCHA. Cette propriété est requise si votre locataire Auth0 a activé la fonctionnalité de [Bot Detection](/docs/fr-ca/secure/attack-protection/bot-detection).
    </ParamField>
...
<ParamField body="passkeyLogin" type="Promise<void>">
  This méthode authentifie l’utilisateur à l’aide de la clé d’accès fournie et, en cas de réussite, le redirige vers `redirect_url`.
...
<ParamField body="googleOneTap" type="Promise<void>">
  This méthode authentifie l’utilisateur à l’aide d’un identifiant Google One Tap, ce qui permet une connexion en un clic sans redirection vers une page de connexion distincte.
...
<ParamField body="pickCountryCode" type="Promise<void>">
  This méthode redirige l’utilisateur vers la liste de sélection des codes de pays, où il peut modifier le préfixe de pays de son numéro de téléphone.
