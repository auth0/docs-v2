---
title: "LoginPassword"
description: "Décrit toutes les propriétés et méthodes disponibles pour personnaliser l’écran `login-password` d’Universal Login."
---

La classe LoginPassword implémente la fonctionnalité associée à l’écran `login-password`. Cet écran recueille l’identifiant et le mot de passe de l’utilisateur.
...
<ParamField body="countryCodes" type={<span><a href="/docs/fr-ca/libraries/acul/js-sdk/Screens/interfaces/CountryCodesMembers">CountryCodesMembers</a></span>}>
  Fournit des données sur les indicatifs de pays pour la saisie d’un numéro de téléphone, comme `available` et `recommended`, pour l’écran `login-password`.
</ParamField>
...
<ParamField body="getErrors" type={<span><a href="/docs/fr-ca/libraries/acul/js-sdk/Screens/interfaces/Error">Error</a>[]</span>}>
  Cette méthode récupère, à partir du contexte, le tableau des erreurs de transaction, ou un tableau vide s’il n’y en a pas.
</ParamField>
...
<ParamField body="switchConnection" type="Promise<void>">
  Cette méthode permet de basculer entre une connexion de base de données (mot de passe) et une connexion Passwordless (Email/SMS OTP).
...
<ParamField body="switchToOtp" type="Promise<void>">
  Cette méthode permet de passer de la saisie du mot de passe à un défi par code à usage unique pour cette connexion.

  Vérifiez `screen.data.showSwitchToOtpButton` avant d’afficher l’option — cette valeur indique si le basculement est disponible pour cette transaction.
