...
<ParamField body="countryCodes" type={<span><a href="/docs/fr-ca/libraries/acul/js-sdk/Screens/interfaces/CountryCodesMembers">CountryCodesMembers</a></span>}>
  Fournit des données d’indicatifs de pays pour la saisie du numéro de téléphone, comme `available` et `recommended`, pour l’écran `login`.
</ParamField>
  ...
  **Paramètres de la méthode**
  ...
  <Expandable title="Paramètres">
    ...
    <ParamField body="identifier?" type="string">
      L’identifiant de l’utilisateur — courriel, nom d’utilisateur ou numéro de téléphone, selon `identifierType`. Indiquez ce champ ou `username`; si les deux sont fournis, `identifier` est prioritaire.
    </ParamField>

    <ParamField body="username?" type="string">
      L’identifiant de l’utilisateur, dans sa forme d’origine. Indiquez ce champ ou `identifier`.
    </ParamField>
    ...
    <ParamField body="identifierType?" type={<span><a href="/docs/fr-ca/libraries/acul/js-sdk/Screens/type-aliases/IdentifierType">IdentifierType</a></span>}>
      Le type de valeur contenu dans `identifier` (ou `username`). Il est généralement lu à partir de `screen.data.activeIdentifierType`. Omettez-le pour envoyer l’identifiant sans préciser son type.
    </ParamField>
    ...
    <ParamField body="phoneCountryCode?" type="string">
      Le code pays ISO 3166-1 alpha-2 d’un identifiant téléphonique, à partir d’un `code` dans `countryCodes.available`. Requis lorsque `identifierType` est `phone`. Transmettez le numéro national — l’indicatif téléphonique est ajouté côté serveur.
    </ParamField>
