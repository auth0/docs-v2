...
description: "Décrit toutes les propriétés et méthodes disponibles pour personnaliser l’écran `mfa-webauthn-change-key-nickname` d’Universal Login."
---

La classe MfaWebAuthnChangeKeyNickname implémente la fonctionnalité de l’écran `mfa-webauthn-change-key-nickname`. Cet écran s’affiche lorsque l’utilisateur doit modifier le surnom de sa clé de sécurité WebAuthn.
...
Créez une instance du gestionnaire de l’écran MFA WebAuthn de modification du surnom de la clé :
...
<ParamField body="countryCodes" type={<span><a href="/docs/fr-ca/libraries/acul/js-sdk/Screens/interfaces/CountryCodesMembers">CountryCodesMembers</a></span>}>
  Fournit des données sur les indicatifs de pays pour la saisie de numéros de téléphone, comme `available` et `recommended`, pour l’écran `mfa-webauthn-change-key-nickname`.
</ParamField>
...
<ParamField body="untrustedData" type={<span><a href="/docs/fr-ca/libraries/acul/js-sdk/Screens/interfaces/UntrustedDataMembers">UntrustedDataMembers</a></span>}>
  Gère les données non fiables transmises au SDK, comme les données saisies par l’utilisateur pendant la modification du surnom de la clé WebAuthn de la MFA.
</ParamField>
  ...
  **Paramètres de la méthode**
  ...
  <Expandable title="Paramètres">
    ...
    <ParamField body="persist?" type="&#x22;session&#x22;">
      Si cette valeur est définie sur `'session'`, la langue sélectionnée est enregistrée pendant toute la durée de la session.
    </ParamField>
  </Expandable>
</ParamField>

<ParamField body="continueWithNewNickname" type="Promise<void>">
  Cette méthode envoie le nouveau surnom de la clé de sécurité WebAuthn de l’utilisateur.
