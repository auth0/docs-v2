...
description: "Décrit toutes les propriétés et méthodes disponibles pour personnaliser l’écran `login-passwordless-sms-otp` d’Universal Login."
---

La classe LoginPasswordlessSmsOtp implémente les fonctionnalités de l’écran `login-passwordless-sms-otp`. Cet écran permet de saisir le numéro de téléphone de l’utilisateur et le code.
...
<ParamField body="countryCodes" type={<span><a href="/docs/fr-ca/libraries/acul/js-sdk/Screens/interfaces/CountryCodesMembers">CountryCodesMembers</a></span>}>
  Fournit des données de codes de pays pour la saisie de numéros de téléphone, comme `available` et `recommended`, pour l’écran `login-passwordless-sms-otp`.
</ParamField>
...
<ParamField body="getErrors" type={<span><a href="/docs/fr-ca/libraries/acul/js-sdk/Screens/interfaces/Error">Error</a>[]</span>}>
  Cette méthode récupère le tableau des erreurs de transaction à partir du contexte, ou un tableau vide s’il n’en existe aucune. Elle renvoie un tableau d’objets d’erreur provenant du contexte de transaction.
</ParamField>
  ...
  **Paramètres de la méthode**
  ...
  <Expandable title="Paramètres">
    ...
    <ParamField body="onTimeout?" type="() => void">
      Fonction de rappel invoquée lorsque la période d’attente expire et que le renvoi redevient disponible.
    </ParamField>
  </Expandable>
</ParamField>
  ...
  **Paramètres de la méthode**
  ...
  <Expandable title="Paramètres">
    ...
    <ParamField body="connection" type="string" required>
      Le nom de la connexion vers laquelle basculer. Utilisez `'email'` ou `'sms'` pour l’authentification sans mot de passe, ou un nom de connexion DB pour l’authentification par mot de passe.
    </ParamField>
  </Expandable>
</ParamField>