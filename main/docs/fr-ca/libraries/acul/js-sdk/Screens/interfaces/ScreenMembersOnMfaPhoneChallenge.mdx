---
title: "ScreenMembersOnMfaPhoneChallenge"
---

ScreenMembersOnMfaPhoneChallenge

```ts Example
export interface ScreenMembersOnMfaPhoneChallenge extends ScreenMembers {
  /**
   * @property {object | null} data - Données spécifiques à l'écran.
   * @property {string} data.phone_number - Le numéro de téléphone (souvent masqué) associé à ce défi d’AMF.
   */
  data: {
    /** Le numéro de téléphone (souvent masqué) associé à ce défi d’AMF. */
    phoneNumber: string;
  } | null;
}
```

<div id="properties">
  ## Propriétés
</div>

<ParamField body="captcha" type={<span><a href="/docs/fr-ca/libraries/acul/js-sdk/Screens/interfaces/CaptchaContext">CaptchaContext</a></span>} />

<ParamField body="captchaImage" type="string" />

<ParamField body="captchaProvider" type="string" />

<ParamField body="captchaSiteKey" type="string" />

<ParamField body="data" type="phoneNumber">
  <div id="type-declaration">
    #### Déclaration de type
  </div>

  &#123; `phoneNumber`: `string`; &#125;

  <div id="phonenumber">
    #### phoneNumber
  </div>

  Le numéro de téléphone (souvent masqué) associé à ce défi d’AMF.

  `null`
</ParamField>

<ParamField body="isCaptchaAvailable" type="boolean" />

<ParamField body="links" type="Record" />

<ParamField body="name" type="string" />

<ParamField body="texts" type="Record" />