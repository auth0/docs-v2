...
L’écran `mfa-webauthn-platform-enrollment` demande à l’utilisateur d’enregistrer un authentificateur de plateforme, comme Touch ID ou Windows Hello, en tant que facteur AMF.
...
function MfaWebAuthnPlatformEnrollmentScreen() {
  ...
  return (
    <button onClick={() => submitPasskeyCredential()}>
      inscrire l’authentificateur de plateforme
    </button>
  );
}
```

## hook de contexte {#context-hooks}

Hooks de contexte associés à l’écran qui fournissent un accès en lecture seule aux données de contexte d’Auth0 sur l’écran `mfa-webauthn-platform-enrollment`. Importez-les depuis `@auth0/auth0-acul-react/mfa-webauthn-platform-enrollment`.
...
<ParamField body="useBranding" type={<span>() =&gt; <a href="/docs/fr-ca/libraries/acul/react-sdk/API-Reference/Types/interfaces/BrandingMembers">BrandingMembers</a></span>}>
  Ce hook fournit les configurations de l’image de marque, comme le logo, les couleurs et les paramètres du thème affichés sur l’écran `mfa-webauthn-platform-enrollment`.
...
<ParamField body="useOrganization" type={<span>() =&gt; <a href="/docs/fr-ca/libraries/acul/react-sdk/API-Reference/Types/interfaces/OrganizationMembers">OrganizationMembers</a></span>}>
  Ce hook fournit des renseignements sur l’Organization de l’utilisateur si le flux AMF est associé à une Organization. Il retourne `null` lorsqu’aucun contexte d’Organization n’est présent.
...
<ParamField body="useTransaction" type={<span>() =&gt; <a href="/docs/fr-ca/libraries/acul/react-sdk/API-Reference/Types/interfaces/TransactionMembers">TransactionMembers</a></span>}>
  Ce hook fournit des données propres à la transaction pour l’écran `mfa-webauthn-platform-enrollment`, comme l’état actuel du flux AMF.
...
<ParamField body="useUntrustedData" type={<span>() =&gt; <a href="/docs/fr-ca/libraries/acul/react-sdk/API-Reference/Types/interfaces/UntrustedDataMembers">UntrustedDataMembers</a></span>}>
  Ce hook permet d’accéder aux données non fiables transmises à l’écran, comme les valeurs préremplies à partir des paramètres d’URL.
...
<ParamField body="useMfaWebAuthnPlatformEnrollment" type={<a href="/docs/fr-ca/libraries/acul/react-sdk/API-Reference/Types/interfaces/MfaWebAuthnPlatformEnrollmentMembers">MfaWebAuthnPlatformEnrollmentMembers</a>}>
  Ce hook renvoie toutes les méthodes et tout le contexte disponibles sur l’écran `mfa-webauthn-platform-enrollment`.
</ParamField>

## Méthodes {#methods}

<ParamField body="refuseEnrollmentOnThisDevice" type="Promise<void>">
  Cette méthode refuse l’inscription de l’authentificateur de plateforme sur l’appareil actuel.
...
<ParamField body="reportBrowserError" type="Promise<void>">
  Cette méthode signale une erreur WebAuthn au niveau du navigateur survenue pendant l’inscription de l’authentificateur de plateforme.
...
<ParamField body="snoozeEnrollment" type="Promise<void>">
  Cette méthode reporte à plus tard l’inscription de l’authentificateur de plateforme.
...
<ParamField body="submitPasskeyCredential" type="Promise<void>">
  Cette méthode soumet les informations d’identification de la clé d’accès créées par l’authentificateur de plateforme pour terminer l’inscription.
...
<ParamField body="tryAnotherMethod" type="Promise<void>">
  Cette méthode permet à l’utilisateur de renoncer à l’inscription WebAuthn de plateforme et de choisir une autre méthode MFA.

  ```jsx Example
  import { useMfaWebAuthnPlatformEnrollment } from '@auth0/auth0-acul-react/mfa-webauthn-platform-enrollment';

  function TryAnotherMethodButton() {
    const { tryAnotherMethod } = useMfaWebAuthnPlatformEnrollment();
    return (
