...
<ParamField body="resendCode" type="Promise<void>">
  Cette méthode effectue le renvoi du code à usage unique au numéro de téléphone de l’utilisateur.
...
<ParamField body="switchToPassword" type="Promise<void>">
  Cette méthode bascule du Phone OTP à l’authentification par mot de passe.

  ```jsx Example
  import { usePhoneIdentifierChallenge } from '@auth0/auth0-acul-react/phone-identifier-challenge';

  function SwitchToPasswordButton() {
    const { switchToPassword } = usePhoneIdentifierChallenge();
    return (
