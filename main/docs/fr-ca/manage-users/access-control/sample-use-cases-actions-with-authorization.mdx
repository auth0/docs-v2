---
description: Utilisez les Actions Auth0 pour étendre le contrôle d’accès basé sur les rôles avec une logique personnalisée, évaluer les rôles des utilisateurs lors de la connexion et ajouter ou supprimer des permissions de jeton d’accès.
...
## Autoriser l’accès uniquement les jours de semaine pour une application donnée {#allow-access-only-on-weekdays-for-a-specific-application}
...
## Autoriser l’accès uniquement aux utilisateurs qui se trouvent sur le réseau d’entreprise {#allow-access-only-to-users-who-are-inside-the-corporate-network}
...
## Rejeter l’accès à toute personne qui fait une requête à une API {#deny-access-to-anyone-calling-an-api}
...
## Ajouter des rôles utilisateur aux jetons {#add-user-roles-to-tokens}
...
## Rejeter l’accès à des empreintes JA3/JA4 précises {#deny-access-to-specific-ja3ja4-fingerprints}
