---
description: Découvrez comment ajouter des permissions à des rôles dans l’Auth0 Dashboard ou à l’aide de la Management API.
...
1. Accédez à [Dashboard &gt; User Management &gt; Roles](https://manage.auth0.com/#/roles), puis cliquez sur le nom du rôle à afficher.
...
Envoyez une requête `POST` au [point de terminaison Add Role Permissions](https://auth0.com/docs/api/management/v2#!/Roles/post_role_permission_assignment). Assurez-vous de remplacer les valeurs de l’espace réservé `ROLE_ID`, `MGMT_API_ACCESS_TOKEN`, `API_IDENTIFIER` et `PERMISSION_NAME` par, respectivement, l’ID de votre rôle, le <Tooltip tip="Jeton d’accès : justificatif d’autorisation, sous la forme d’une chaîne opaque ou d’un JWT, utilisé pour accéder à une API." cta="Voir le glossaire" href="/docs/fr-ca/glossary?term=access+token">jeton d’accès</Tooltip> de la Management API, l’identifiant de l’API (<Tooltip tip="Jeton d’accès : justificatif d’autorisation, sous la forme d’une chaîne opaque ou d’un JWT, utilisé pour accéder à une API." cta="Voir le glossaire" href="/docs/fr-ca/glossary?term=audience">audience</Tooltip>) et le nom de la ou des permissions.
...
| `API_IDENTIFIER`        | Il s’agit de l’identifiant de l’API associée à la ou aux permissions que vous souhaitez ajouter pour le rôle indiqué, aussi appelé l’audience. Il ne s’agit pas de l’ID de l’API. |
| `PERMISSION_NAME`       | Nom de la ou des permissions que vous souhaitez ajouter pour le rôle indiqué.                                                                                                     |
