...
description: Découvrez comment attribuer et supprimer des rôles aux groupes d’entreprise à l’aide de l’Auth0 Dashboard ou de la Management API.
...
## Assigner des rôles {#assign-roles}

### Groupes d’organisation {#organization-groups}
...
Vous pouvez attribuer des rôles de tenant et des rôles d’organisation aux groupes d’entreprise au sein d’une organisation. Pour en savoir plus sur les rôles d’organisation, consultez [Rôles d’organisation](/docs/fr-ca/manage-users/organizations/organization-roles).
...
<Tabs>
  <Tab title="Dashboard">
    ...
    Les rôles attribués s’affichent dans les détails du groupe. Les rôles de tenant et les rôles d’organisation limités à cette organisation peuvent tous deux être attribués.
  </Tab>
  ...
  <Tab title="Management API">
    ...
    <Tabs>
      <Tab title="Auth0 CLI">
        <Callout icon="file-lines" color="#0EA5E9" iconType="regular">Vous utilisez le Auth0 CLI? Si ce n’est pas déjà fait, [configurez et authentifiez votre session CLI](/docs/fr-ca/deploy-monitor/auth0-cli) avant d’exécuter cette commande.</Callout>
...
### Groupes d’entreprise {#enterprise-groups}
...
## Supprimer des rôles {#remove-roles}

### Groupes d’organisation {#organization-groups-2}
...
### Groupes d’entreprise {#enterprise-groups-2}
...
<Tabs>
  ...
  <Tab title="Management API">
    Effectuez une requête `DELETE` vers le point de terminaison [Remove Roles from a Group](/docs/fr-ca/api/management/v2). Vous avez besoin des renseignements suivants :
