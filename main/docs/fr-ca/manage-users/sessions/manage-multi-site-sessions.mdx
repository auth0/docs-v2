...
Pour en savoir plus sur les minuteries d’inactivité et les modalités de délai d’expiration, lisez [URL de déconnexion spécifiques à l’application](/users/sessions/configure-session-lifetime-settings).
...
Les détails du flux de production changeraient dans le cas d’une session de longue durée où la session d’application serait très probablement plus courte que celle d’[authentification unique (SSO)](/single-sign-on).

Pour en savoir plus, lisez [Limites de durée de vie de la session](/users/sessions/configure-session-lifetime-settings) et [Obtenir des jetons d’actualisation](/security/tokens/access-tokens/update-access-token-lifetime).
