---
description: Découvrez comment configurer les métadonnées de session Auth0 à l’aide d’Actions et de la Management API.
...
### Récupérer les métadonnées de session existantes {#retrieve-existing-session-metadata}
...
### Ajouter ou mettre à jour les métadonnées de session existantes {#add-or-update-existing-session-metadata}
...
### Supprimer les métadonnées de session {#delete-session-metadata}
...
### Récupérer les métadonnées de session existantes {#retrieve-existing-session-metadata-2}
...
### Ajouter ou mettre à jour des métadonnées existantes {#add-or-update-existing-metadata}
...
### Supprimer les métadonnées de session {#delete-session-metadata-2}
...
### Auth0 Management API {#auth0-management-api-2}
