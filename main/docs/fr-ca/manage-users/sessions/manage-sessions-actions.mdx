---
description: Gérez les sessions utilisateur Auth0 avec Actions pour consulter les métadonnées de session, définir des durées de vie personnalisées et révoquer ou prolonger des sessions.
...
## Révoquer des sessions avec Actions {#revoke-sessions-with-actions}
...
## Modifier les dates d’expiration des sessions avec Actions {#change-sessions-expiry-dates-with-actions}
...
## Définir la persistance du témoin de session avec Actions {#set-session-cookie-persistence-with-actions}
...
## Cas d’utilisation : Révoquer une session {#use-cases-revoke-a-session}
...
### Révoquer une session en raison d’une liaison au réseau ASN {#revoke-a-session-due-to-asn-network-binding}
...
### Révoquer une session en raison d’une association à une adresse IP {#revoke-a-session-due-to-an-ip-binding}
...
## Cas d’utilisation : Personnaliser les dates d’expiration d’une session {#use-cases-customize-a-session-expiry-dates}
...
### Personnaliser le délai d’expiration absolu d’une session en fonction des connexions {#customize-absolute-session-expiration-time-based-on-connections}
...
### Personnaliser le délai d’inactivité de la session en fonction de l’organisation {#customize-session-inactivity-timeout-based-on-the-organization}
