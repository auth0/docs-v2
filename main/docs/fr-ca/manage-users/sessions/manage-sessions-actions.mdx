...
Les objets `event.session` et `api.session` prennent en charge les flux interactifs basés sur le Web, y compris le [flux de code d’autorisation](/not-found/5xWouuRIDiCUS310UW0wq3), le flux implicite, le flux de code d’appareil, ainsi que SAML et WS-Fed.
...
* [Objet Event](/signup-and-login-triggers/login-trigger/post-login-event-object) : En savoir plus sur l’objet de session Event et ses propriétés
* [Objet API](/signup-and-login-triggers/login-trigger/post-login-api-object) : En savoir plus sur l’objet de session API et ses méthodes.
...
En plus de révoquer la session, la méthode lancera également un [initiateur de déconnexion de canal d’appui OIDC](/oidc-back-channel-logout-initiators)`session-revoked` pour déconnecter les utilisateurs de toutes les applications liées à la session en cours et enregistrer un événement [session_revoked](/logs/log-event-type-codes) dans les journaux du locataire.
...
* révoquer tous les [jetons d’actualisation](/security/tokens/refresh-tokens) associés à la session existante avec une valeur `session_id` correspondante.
...
Vous pouvez modifier [les dates d’expiration](/users/sessions/session-lifetime-limits) de la session avec les méthodes de post-connexion suivantes :
...
Pour des raisons de sécurité, les délais d’inactivité et absolu ne peuvent pas être définis au-dessus des paramètres de la session définis dans les [limites de la durée de vie de la session](/users/sessions/configure-session-lifetime-settings) pour le locataire. Si vous tentez de définir une date au-dessus des limites de la durée de vie, les méthodes de l’API la mettront à jour en fonction des limites de durée de vie et enregistreront un événement d’avertissement (`w`) dans les journaux du locataire.

## Cas d’utilisation : Révoquer une session

Vous pouvez utiliser les [Actions](/actions) pour configurer les détections de risques et révoquer les sessions risquées et leurs jetons d’actualisation associés avec la méthode post-connexion `api.session.revoke(reason, options)` et l’objet `event.session`.
