---
description: Découvrez comment configurer Keep Me Signed avec des sessions Auth0 afin d’offrir aux utilisateurs des sessions persistantes ou éphémères.
...
## 1. Configurer une page Universal Login et ajouter une invite personnalisée {#1-configure-a-universal-login-page-and-add-a-custom-prompt}
...
### Facultatif : activer le modèle de page Universal Login {#optional-enable-the-universal-login-page-template}
...
## 2. Ajouter une case à cocher Keep Me Signed In à l’aide d’Auth0 CLI {#2-add-a-keep-me-signed-in-checkbox-using-auth0-cli}

Pour ajouter une case à cocher dans l’écran Universal Login, suivez ces étapes :

### Ouvrez l’éditeur de personnalisation {#launch-the-customization-editor}
...
### Configurer la case à cocher {#configure-the-checkbox}
...
### Déployez vos modifications {#deploy-your-changes}
...
## 3. Créer une action post-login avec Auth0 CLI {#3-create-a-post-login-action-using-auth0-cli}
...
### Définir la durée de la session pour les appareils de confiance {#set-session-duration-for-trusted-devices}
...
## 4. Tester la mise en œuvre {#4-test-implementation}
