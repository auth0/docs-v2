---
description: Découvrez comment retirer des rôles aux membres de l’organisation à l’aide de l’Auth0 Dashboard ou de la Management API.
title: Retirer des rôles aux membres de l’organisation
---
...
Pour retirer des rôles d’un membre d’une organisation au moyen de la Management API :
Effectuez une requête `DELETE` vers le endpoint `Delete Organization Member Roles`. Assurez-vous de remplacer les valeurs de l’espace réservé `ORG_ID`, `MGMT_API_ACCESS_TOKEN`, `USER_ID` et `ROLE_ID` par votre Organization ID, votre Management API <Tooltip tip="jeton d’accès : information d’identification d’autorisation, sous la forme d’une chaîne opaque ou d’un JWT, utilisée pour accéder à une API." cta="Voir le glossaire" href="/docs/fr-ca/glossary?term=Access+Token">jeton d’accès</Tooltip>, votre user ID et votre rôle ID, respectivement.
...
<AuthCodeGroup>
  ```bash Auth0 CLI
  auth0 api delete "organizations/ORG_ID/members/USER_ID/roles" \
...
| `USER_ID`               | ID de l’utilisateur auquel vous voulez retirer le ou les rôles indiqués.                                                                                  |
| `ROLE_ID`               | ID du rôle que vous voulez retirer à l’utilisateur indiqué pour l’organisation indiquée.                                                                  |
...
| `403`       | `insufficient_scope`   | Scope insuffisant; une des valeurs suivantes est attendue : `delete:organization_member_roles`.        | Tentative de lire ou de modifier un champ non autorisé avec les scopes du jeton Bearer fourni. |
