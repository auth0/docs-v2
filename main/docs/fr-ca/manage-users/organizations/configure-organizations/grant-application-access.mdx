...
## Activer l’accès par application {#enable-per-application-access}
...
## Accorder l’accès à une application {#grant-access-to-an-application}
...
<Tabs>
  ...
  <Tab title="Management API">
    ...
    <Tabs>
      <Tab title="Auth0 CLI">
        ```bash
        auth0 api post "organizations/ORG_ID/clients" \
          --data '{
            "clients": [
              {
...
## Révoquer l’accès à une application {#revoke-access-to-an-application}
...
<Tabs>
  ...
  <Tab title="Management API">
    ...
    <Tabs>
      <Tab title="Auth0 CLI">
        ```bash
        auth0 api delete "organizations/ORG_ID/clients" \
          --data '{
            "clients": [
              { "client_id": "YOUR_CLIENT_ID" }
            ]
...
## Répertorier les applications d’une organisation {#list-an-organizations-applications}

Utilisez l’Management API pour récupérer toutes les applications associées à une organisation, y compris le statut d’accès des membres pour chacune d’elles.

<Tabs>
  <Tab title="Auth0 CLI">
    ```bash
    auth0 api get "organizations/ORG_ID/clients"
    ```
  </Tab>
