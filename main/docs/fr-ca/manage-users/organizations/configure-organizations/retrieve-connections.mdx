...
| Valeur | Description |
| --- | --- |
| `ORG_ID` | Identifiant de l’organisation pour laquelle vous voulez récupérer les connexions activées. |
| `MGMT_API_ACCESS_TOKEN` | [Jeton d’accès à Management API](/docs/fr-ca/tokens/management-api-access-tokens) avec la permission `read:organization_connections`. |
...
| `400` | `invalid_body` | Corps de demande non valide. Le message variera en fonction de la cause. | La requête de charge utile n’est pas valide. |
| `401` |   | Jeton non valide. |   |
| `401` |   | Signature non valide reçue pour la validation du jeton Web JSON. |   |
| `401` |   | Le client n’est pas global. |   |
| `403` | `insufficient_scope` | Permission insuffisante; on s’attend à l’un des éléments suivants : `read:organization_connections`. | Tentative de lecture/écriture d’un champ qui n’est pas autorisé avec les permissions de jeton du porteur fourni. |
