...
| `MGMT_API_ACCESS_TOKEN` | [Jeton d’accès pour Management API](/docs/fr-ca/tokens/management-api-access-tokens) avec la permission `delete:organization_connections`. |
...
| --- | --- | --- | --- |
| `204` | La connexion a été supprimée de l’organisation avec succès. |   |   |
| `401` |   | Jeton invalide. |   |
| `401` |   | Signature non valide reçue pour la validation du jeton Web JSON. |   |
| `401` |   | Le client n’est pas global. |   |
| `403` | `insufficient_scope` | Permission insuffisante ; est attendu toute option suivante : `delete:organization_connections`. | Tentative de lecture/écriture d’un champ qui n’est pas autorisé avec les permissions de jeton du porteur fourni. |
