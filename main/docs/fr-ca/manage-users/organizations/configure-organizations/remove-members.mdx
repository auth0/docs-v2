...
| Valeur | Description |
| --- | --- |
| `ORG_ID` | ID de l’organisation pour laquelle vous souhaitez supprimer l’affiliation. |
| `MGMT_API_ACCESS_TOKEN` | [Jeton d’accès pour Management API](/docs/fr-ca/tokens/management-api-access-tokens) avec la permission `delete:organization_members`. |
| `USER_ID` | ID des utilisateurs que vous souhaitez supprimer de l’organisation indiquée. |
...
| --- | --- | --- | --- |
| `204` |   | Utilisateurs retirés avec succès de l’organisation. |   |
...
| `403` | `insufficient_scope` | Permission insuffisante; permission attendue : `delete:organization_members`. | Tentative de lecture/écriture d’un champ qui n’est pas autorisé avec les permissions de jeton du porteur fourni. |
