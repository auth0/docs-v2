...
La plupart des types d’applications (telles que les applications web, les applications à page unique et les applications natives) devraient utiliser le [Flux de code d’autorisation avec PKCE](/login/flows/authorization-code-flow-with-proof-key-for-code-exchange-pkce) pour faciliter l’authentification de la connexion. Ce flux implique l’échange d’un code d’autorisation contre des jetons.
...
Cependant, les sessions locales ne sont pas disponibles pour les applications sans système dorsal, telles que les applications à page unique. Au lieu de cela, ces applications utilisent une approche différente connue sous le nom d’[authentification silencieuse](/login/configure-silent-authentication) pour maintenir la connexion des utilisateurs.
