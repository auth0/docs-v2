...
(Facultatif) Recherchez le `connection_id` et le nom de domaine de votre locataire Auth0 dans <Tooltip href="/docs/fr-ca/glossary?term=auth0-dashboard" tip="Auth0 Dashboard
...
```bash cURL
curl --request GET \
  --url 'https://{yourDomain}/api/v2/jobs/{yourJobId}' \
  --header 'authorization: Bearer {yourMgmtAPIAccessToken}'
```
```csharp C#
var client = new RestClient("https://{yourDomain}/api/v2/jobs/{yourJobId}");
...
```
```java Java
HttpResponse<String> response = Unirest.get("https://{yourDomain}/api/v2/jobs/{yourJobId}")
  .header("authorization", "Bearer {yourMgmtAPIAccessToken}")
  .asString();
```
```javascript Node.JS
...
NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:[NSURL URLWithString:@"https://{yourDomain}/api/v2/jobs/{yourJobId}"]
                                                       cachePolicy:NSURLRequestUseProtocolCachePolicy
                                                   timeoutInterval:10.0];
...
curl_setopt_array($curl, [
  CURLOPT_URL => "https://{yourDomain}/api/v2/jobs/{yourJobId}",
  CURLOPT_RETURNTRANSFER => true,
  CURLOPT_ENCODING => "",
  CURLOPT_MAXREDIRS => 10,
  CURLOPT_TIMEOUT => 30,
...
conn.request("GET", "/{yourDomain}/api/v2/jobs/{yourJobId}", headers=headers)
...
url = URI("https://{yourDomain}/api/v2/jobs/{yourJobId}")
...
let request = NSMutableURLRequest(url: NSURL(string: "https://{yourDomain}/api/v2/jobs/{yourJobId}")! as URL,
                                        cachePolicy: .useProtocolCachePolicy,
                                    timeoutInterval: 10.0)
