...
To export `app_metadata` or `user_metadata` as single objects, use the Toutes les données relatives à votre tâche sont automatiquement supprimées au bout de 24 heures et ne sont plus accessibles par la suite. Pour cette raison, **nous vous recommandons vivement de stocker les résultats de vos tâches en utilisant le mécanisme de stockage de votre choix**. and include the desired field in the `fields` parameter of the request body. For example:
