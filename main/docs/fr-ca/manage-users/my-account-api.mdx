...
### Activer la Default Policy {#enable-default-policy}
...
### Hiérarchie des exigences d’authentification {#authentication-requirements-hierarchy}
...
### Comportement de Default Politique {#default-policy-behavior}
...
### Exemples de jetons d’accès {#access-token-examples}
...
## Gérer les méthodes d’authentification {#manage-authentication-methods}
...
### Exemples de gestion des méthodes d’authentification {#manage-authentication-method-examples}

#### S’inscrire à un authentificateur TOTP {#enroll-a-totp-authenticator}

L’inscription à un authentificateur TOTP se fait en deux étapes : démarrer l’inscription, puis la confirmer.

Commencez par lancer l’inscription pour obtenir un code QR et un secret à saisir manuellement que l’utilisateur pourra ajouter à son application d’authentification :
...
Confirmez ensuite l’inscription en envoyant le code à usage unique généré par l’application d’authentification de l’utilisateur, ainsi que les valeurs `auth_session` et `id` de la réponse précédente :
...
#### Lister les méthodes d’authentification {#list-authentication-methods}
...
#### Supprimer une méthode d’authentification {#delete-an-authentication-method}
...
## Requêtes inter-origines {#cross-origin-requests}
