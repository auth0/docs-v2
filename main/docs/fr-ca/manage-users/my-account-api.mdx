---
og:description: Learn how you can empower your users to manage their own account information
  with the My Account API.
...
You can activate the My Account API for your tenant in the <Tooltip cta="View Glossary" href="/docs/glossary?term=Auth0+Dashboard" tip="Auth0 Dashboard: Auth0's main product to configure your services.">Auth0 Dashboard</Tooltip>:
...
You can get an <Tooltip cta="View Glossary" href="/docs/glossary?term=access+token" tip="Access Token: Authorization credential, in the form of an opaque string or JWT, used to access an API.">access token</Tooltip> for the My Account API in the same way you’d get an access token for one of your own APIs.
...
If you’re using <Tooltip cta="View Glossary" href="/docs/glossary?term=Universal+Login" tip="Universal Login: Your application redirects to Universal Login, hosted on Auth0's Authorization Server, to verify a user's identity.">Universal Login</Tooltip>, read the following articles:
...
The <Tooltip cta="View Glossary" href="/docs/glossary?term=audience" tip="Audience: Unique identifier of the audience for an issued token. Named aud in a token, its value contains the ID of either an application (Client ID) for an ID Token or an API (API Identifier) for an Access Token.">audience</Tooltip> of the My Account API is `https://{yourDomain}/me/`.
