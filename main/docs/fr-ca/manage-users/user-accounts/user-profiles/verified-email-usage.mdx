...
Quand vous souhaitez [associer deux comptes d’utilisateurs](/users/user-account-linking), assurez-vous que l’utilisateur a toujours accès aux deux comptes. La seule façon d’y parvenir est de demander aux utilisateurs de s’authentifier auprès des deux comptes avant de les associer.
