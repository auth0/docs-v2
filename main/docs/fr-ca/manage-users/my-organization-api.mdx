...
description: Utilisez l’API My Organization pour permettre aux administrateurs d’Organization de gérer leurs propres fournisseurs d’identité et domaines vérifiés directement dans votre application.
validatedOn: 2026-09-20
---

<Warning>
  API My Organization et composants d’interface utilisateur intégrables est actuellement offerte en Early Access à tous les clients. En utilisant cette fonctionnalité, vous acceptez les conditions applicables de l’essai gratuit figurant dans le [Master Subscription Agreement d’Okta](https://www.okta.com/legal/). Pour en savoir plus sur le cycle de publication des produits d’Auth0, consultez [Product Release Stages](/docs/fr-ca/troubleshoot/product-lifecycle/product-release-stages).

  Les clients sont responsables de s’assurer que leur utilisation de l’API My Organization et de composants d’interface utilisateur intégrables est conforme à leurs politiques de sécurité et aux lois applicables, y compris toute permission accordée à leurs utilisateurs finaux.
</Warning>
...
* les paramètres d’application de ressource de [Cross App Access (XAA)](/docs/fr-ca/ai-agents-mcp/cross-app-access) pour les connexions
* les invitations de membres
* la gestion des membres
* l’attribution et le retrait des rôles des membres
* la découverte du magasin d’utilisateurs pour la création d’invitations
...
## Configurer My Organization API {#set-up-my-organization-api}

### Activer API My Organization dans Auth0 Dashboard {#activate-the-my-organization-api-in-auth0-dashboard}
...
### Configurer les attributs de l’application cliente {#configure-client-application-attributes}
...
| `third_party_client_access`           | **Objet.** Contrôle les valeurs que les administrateurs d’Organization peuvent définir pour l’accès des applications tierces à cette Organization au moyen de l’API My Organization. Structure : `{ "default_value": "block" \| "allow", "allowed_values": ["block", "allow"] }`. La valeur par défaut est `{ "default_value": "block", "allowed_values": ["block"] }`, ce qui signifie que les administrateurs d’Organization ne peuvent pas activer l’accès des applications tierces avant qu’il ne soit explicitement autorisé.                                                                                                                                                                                                                                            |
| `invitation_landing_client_id`        | **Chaîne de caractères.** ID client de l’application vers laquelle les utilisateurs sont redirigés lorsqu’ils acceptent une invitation à devenir membre. Obligatoire si l’application utilise la permission `create:my_org:member_invitations`. Le client indiqué doit être activé pour l’Organization lors de la création de l’invitation.                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `enforce_permission_ceiling`          | **Booléen.** Lorsqu’il est activé, empêche un administrateur d’Organization d’attribuer un rôle dont les permissions dépassent ses propres permissions effectives au moment de l’opération. La valeur par défaut est `false`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `enforce_self_assignment_restriction` | **Booléen.** Lorsqu’il est activé, empêche un administrateur d’Organization de s’attribuer un rôle. La valeur par défaut est `false`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |

Le [Connection Profile](/docs/fr-ca/authenticate/enterprise-connections/connection-profile), `connection_profile_id`, détermine si les administrateurs d’Organization peuvent configurer une connexion pour qu’elle agisse comme application de ressource [Cross App Access (XAA)](/docs/fr-ca/ai-agents-mcp/cross-app-access) au moyen de l’API My Organization.

#### Configurer les attributs de l’application cliente {#configure-client-application-attributes-2}
...
<Tabs>
  <Tab title="Auth0 Dashboard">
    ...
    4. Configurez les paramètres suivants :<br />
       ...
       1. Activez un ou plusieurs fournisseurs d’identité. Les administrateurs d’Organization peuvent sélectionner l’option de leur choix dans la liste des fournisseurs activés. <br />
       ...
       E. **Facultatif**. Configurez si vous voulez permettre à l’administrateur d’Organization de définir l’accès tiers. <br />

       F. Configurez l’autorisation d’accès des utilisateurs sur Unauthorized, Authorized ou All. <br />
       ...
       G. Configurez l’autorisation d’accès des informations d’identification du client sur Unauthorized, Authorized ou All. <br />

       1. **Unauthorized**. Aucune permission n’est accordée. <br />
       2. **Authorized**. Sélectionnez les permissions souhaitées. <br />
       3. **All**. Inclut toutes les permissions existantes et futures. <br />

       H. **Facultatif**. Configurez l’Invitation Landing Client. <br />

       1. Sélectionnez le client vers lequel les membres de l’Organization sont redirigés lorsqu’ils acceptent une invitation de membre. Requis si l’application utilise la permission `create:my_org:member_invitations`. <br />

       I. **Facultatif**. Configurez les garde-fous de permissions. <br />

       1. Enforce Permission Ceiling : lorsque cette option est activée, elle empêche un administrateur d’Organization d’attribuer un rôle dont les permissions dépassent ses propres permissions effectives. <br />
       2. Enforce Self-Assignment Restriction : lorsque cette option est activée, elle empêche un administrateur d’Organization de s’attribuer un rôle à lui-même. <br />
...
## Générer un jeton d’accès {#generate-an-access-token}

L’API My Organization peut uniquement être appelée à l’aide d’un jeton d’accès associé à un utilisateur, obtenu au moyen de l’un des [flux OAuth 2.0 pris en charge](/docs/fr-ca/get-started/authentication-and-authorization-flow).
...
### Exemple avec le flux de code d’autorisation {#example-with-authorization-code-flow}
...
### Exemple avec le flux de code d’autorisation avec PKCE {#example-with-authorization-code-flow-with-pkce}
...
### Permissions d&#39;administrateur d&#39;Organization dans le jeton d&#39;ID {#organization-admin-permissions-in-id-token}

Lorsque le public `/my-org` est présent dans une requête d&#39;autorisation basée sur l&#39;utilisateur, le jeton d&#39;ID inclut automatiquement la demande personnalisée `urn:auth0:my_org_current_user_permissions`. Cette demande personnalisée n&#39;est pas offerte dans les flux de communication entre machines. Elle contient toutes les permissions `my_org:*` que détient l&#39;utilisateur authentifié par l&#39;entremise des rôles qui lui sont attribués, et elle sert aux composants d&#39;interface utilisateur intégrables à déterminer quelles actions afficher.
...
| `read:my_org:domains`                           | Consulter les domaines d’une organisation                                                              |
| `delete:my_org:domains`                         | Supprimer un domaine d’une organisation                                                                |
| `create:my_org:domains`                         | Créer un domaine pour une organisation                                                                 |
| `update:my_org:domains`                         | Mettre à jour un domaine d’une organisation                                                            |
| `create:my_org:member_invitations`              | Créer des invitations de membres pour une organisation                                                 |
| `read:my_org:member_invitations`                | Consulter les invitations de membres d’une organisation                                                |
| `delete:my_org:member_invitations`              | Supprimer les invitations de membres d’une organisation                                                |
| `read:my_org:members`                           | Lister les membres d’une organisation                                                                  |
| `delete:my_org:memberships`                     | Retirer un membre d’une organisation (supprimer l’adhésion) sans supprimer l’utilisateur correspondant |
| `read:my_org:member_roles`                      | Lister les rôles attribués à un membre de l’organisation                                               |
| `create:my_org:member_roles`                    | Attribuer des rôles à un membre de l’organisation                                                      |
| `delete:my_org:member_roles`                    | Retirer des rôles à un membre de l’organisation                                                        |
| `read:my_org:user_stores`                       | Consulter les magasins d’utilisateurs d’une organisation                                               |

## Référence des points de terminaison {#endpoint-reference}

L’API My Organization prend en charge des points de terminaison pour la configuration, les détails de l’organisation, les fournisseurs d’identité, les domaines, les configurations de provisionnement et les jetons SCIM, les invitations de membres, les membres, les rôles des membres et les magasins d’utilisateurs. Pour consulter la référence complète de ces points de terminaison, y compris les schémas, les codes d’erreur, etc., reportez-vous à notre [API Explorer](https://auth0.com/docs/api/myorganization).
...
### Connection Profile (CP) {#connection-profile-cp}
...
### User Attribute Profile (UAP) {#user-attribute-profile-uap}
...
### Limites de débit par organisation {#per-organization-rate-limits}
...
## Requêtes interorigines {#cross-origin-requests}
...
## Événements de journalisation {#log-events}
...
| `my_organization_api_member_succeeded`      | Membre de My Organization API réussi                     | appel d’API réussi vers la ressource de membre du service My Organization API. Couvre toutes les opérations sur les membres, y compris l’attribution et le retrait de rôles. |
| `my_organization_api_member_failed`         | Membre de My Organization API Failed                     | appel d’API ayant échoué vers la ressource de membre du service My Organization API                                                                                          |
| `my_organization_api_invitation_succeeded`  | Invitation de My Organization API réussie                | appel d’API réussi vers la ressource d’invitation du service My Organization API                                                                                             |
| `my_organization_api_invitation_failed`     | Invitation de My Organization API Failed                 | appel d’API ayant échoué vers la ressource d’invitation du service My Organization API                                                                                       |
| `my_organization_api_role_failed`           | Rôle de My Organization API Failed                       | appel d’API ayant échoué vers la ressource de rôle du service My Organization API                                                                                            |
| `my_organization_api_user_stores_failed`    | Magasins d’utilisateurs de My Organization API Failed    | appel d’API ayant échoué vers la ressource des magasins d’utilisateurs du service My Organization API                                                                        |

## Propriété des connexions d’organisation {#organization-connection-ownership}
...
| `organization_access_level`        | **Énumération (null, none, readonly, limited, full)**. Détermine le type d’accès qu’un utilisateur aura lorsqu’il utilise la My Organization API : <p /> 1. `null` <p /> 2. `none` - Association attribuée par l’administrateur de tenant. Cette connexion ne peut pas être consultée ni modifiée au moyen de la My Organization API. <p /> 3. `readonly` - Association attribuée par l’administrateur de tenant. Cette connexion peut être consultée, mais ne peut pas être modifiée au moyen de la My Organization API. <p /> 4. `limited` - Association attribuée par l’administrateur de tenant. Des renseignements et des modifications limités sont permis au moyen de la My Organization API. <p /> 5. `full` - Association attribuée par l’administrateur de tenant. Les utilisateurs de la My Organization API pourront modifier la connexion sans restrictions supplémentaires lorsqu’ils utilisent la My Organization API. Remarque : Peu importe `organization_access_level`, les modifications effectuées par la My Organization API sont toujours soumises aux restrictions appliquées par le Connection Profile.                                                                                                                                                                                                                                  |
...
| `organization_member_access_level` | **Énumération (none, readonly, limited, full).** Contrôle les actions qu’un administrateur d’Organization peut effectuer sur les membres authentifiés par cette connexion. Configurable uniquement par l’administrateur de tenant, pour chaque organisation. La même connexion sous-jacente peut avoir des valeurs différentes selon les organisations. <p /> 1. `none` — Le membre n’est pas visible pour les administrateurs d’Organization. Toutes les opérations renvoient 404 Not Found. <p /> 2. `readonly` — L’administrateur d’Organization peut consulter et répertorier le membre, ainsi que consulter ses rôles. Aucune modification n’est permise. <p /> 3. `limited` — Toutes les opérations readonly, en plus du retrait du membre de l’organisation et de l’attribution ou du retrait de rôles. <p /> 4. `full` — Toutes les opérations limited, en plus de la suppression du compte utilisateur sous-jacent du membre et de la mise à jour des attributs du membre (régis par le User Attribute Profile). <p /> Par défaut : lorsque `organization_access_level` est `full`, `organization_member_access_level` est défini par défaut sur `full`. Sinon, il est défini par défaut sur `limited`. L’administrateur de tenant peut remplacer cette valeur à tout moment au moyen de `PATCH /api/v2/organizations/{id}/connections/{connectionId}`. |
...
* `organization_member_access_level` peut être configuré au moyen de la Management API ou à partir des paramètres de connexion dans [Auth0 Dashboard &gt; Organizations](https://manage.auth0.com/#/organizations).

## Auth0 Universal Components {#auth0-universal-components}
