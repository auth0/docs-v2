...
## Déterminez quand les requêtes vers un tenant sont soumises à une limitation de débit {#discover-when-requests-to-a-tenant-are-rate-limited}
...
#### appi (Public Performance Burst uniquement) {#appi-public-performance-burst-only}

Le journal `appi` est déclenché lorsqu’un tenant client doté de l’add-on Public Performance Burst dépasse la limite anti-attaques soutenue de 100 RPS pour le débit de requêtes à l’Authentication API, ce qui consomme un bloc d’une minute de son allocation de burst de 48 heures. Si, après 1 minute, le débit de requêtes dépasse de nouveau la limite anti-attaques soutenue de 100 RPS, un deuxième journal `appi` est déclenché.

### Réponses de l’API {#api-responses}
...
### Gestion des erreurs du SDK {#sdk-error-handling}
...
## Prévoir quand les requêtes adressées à un tenant seront limitées {#predict-when-requests-to-a-tenant-will-be-rate-limited}
...
## Exemples d’application des limites de débit {#examples-of-how-rate-limits-are-enforced}

### Exemple de requêtes par seconde {#requests-per-second-example}
...
### Exemple de requêtes par minute {#requests-per-minute-example}
...
## Utilisation de l’API de connexion et d’inscription pour les utilisateurs finaux {#end-user-login-and-signup-api-usage}
