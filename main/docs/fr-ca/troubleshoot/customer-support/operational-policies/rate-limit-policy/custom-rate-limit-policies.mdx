...
## Comment les limites de débit personnalisées s’articulent avec les limites de débit du tenant {#how-custom-rate-limits-relate-to-tenant-rate-limits}
...
## Fonctionnement de l’évaluation des politiques {#how-policy-evaluation-works}
...
### Compteurs communs (Politiques de groupe) {#pooled-counters-group-policies}
...
#### Applications tierces et clients CIMD {#third-party-apps-vs-cimd-clients}
...
### Compteurs indépendants (politiques globales/par défaut) {#independent-counters-global-default-policies}
...
## Requêtes prises en compte dans les limites de débit personnalisées {#requests-that-count-toward-custom-rate-limits}
...
## Exclusions des limites de débit personnalisées {#custom-rate-limit-exclusions}
...
## Mode journalisation uniquement {#log-only-mode}
...
## Choisir une valeur de limite de débit {#choose-a-rate-limit-value}

La limite appropriée dépend de votre contrôle sur les schémas de trafic de l’application.

### Applications de première partie (politiques d’ID client) {#first-party-applications-client-id-policies}
...
### Applications tierces (politiques de groupe ou globales) {#third-party-applications-group-or-global-policies}
...
### Bloquer entièrement une application {#block-an-application-entirely}
...
{/* ## Créer une politique de limite de débit personnalisée
  ...
  <Steps>
    ...
    Activez **Journalisation uniquement** pour observer la politique sans l’appliquer. Surveillez les [événements `api_limit`](/docs/deploy-monitor/logs/log-event-type-codes) pendant 3 à 7 jours avant d’activer son application.
  ...
  <Steps>
    ...
    Activez **Journalisation uniquement** pour observer la politique sans l’appliquer. Surveillez les [événements `api_limit`](/docs/deploy-monitor/logs/log-event-type-codes) pendant 3 à 7 jours.
...
## Créer une politique personnalisée de limite de débit {#create-a-custom-rate-limit-policy}
...
## Surveiller à l’aide des journaux {#monitor-via-logs}
...
## Surveiller au moyen des en-têtes de réponse HTTP {#monitor-via-http-response-headers}
