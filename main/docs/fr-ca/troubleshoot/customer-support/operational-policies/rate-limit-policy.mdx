...
Dans un Nuage privé, les limites de requête sont fondées sur le niveau de la performance du Nuage privé.  Pour en savoir plus, veuillez consulter [Nuage privé pour AWS](/private-cloud-on-aws) ou [Nuage privé pour Azure](/deploy/private-cloud-on-azure).
...
Les clients qui utilisent l’extensibilité peuvent ajouter d’autres requêtes, non seulement à l’Authentication API, mais aussi à Management API, en fonction de la configuration de l’extensibilité. 
...
Le suivi des événements de déduction d’intervalle est possible à partir des [journaux des locataires](/logs/log-event-type-codes). En effet, chaque déduction entraine la création d’un type particulier d’événement dans le journal du locataire `appi`. Ce dernier contient des informations sur l’allocation déjà consommée et celle qui reste.

Pour plus d'informations, voir la section Authentication API dans [Limites anti-attaques – Entreprise](/enterprise-public)
