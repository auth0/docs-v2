...
## Demande du nom de l&#39;Organization sans SSO {#prompt-for-organization-name-without-sso}

**Déconseillé** : 31 octobre 2025

**Fin de vie** : 1er mai 2026

Les flux de connexion amorcés dans le contexte d&#39;applications clientes associées à des utilisateurs d&#39;affaires (`organization_usage=require`) et configurées pour demander l&#39;organization au début du flux de connexion (`organization_require_behavior=pre_login_prompt`) tiendront compte d&#39;une session authentifiée existante.

Auparavant, le service demandait le nom de l&#39;organization à l&#39;utilisateur, qui devait ensuite se connecter. Par exemple, un utilisateur possédant un compte protégé par mot de passe devait saisir de nouveau ses informations d&#39;identification même si une session authentifiée était valide pour l&#39;organization sélectionnée.

## Connexion non confirmée avec redirections vers des URI de rappel non vérifiables {#unconfirmed-login-with-non-verifiable-callback-uri-redirects}

**Déconseillé** : 28 octobre 2025

**Fin de vie** : 28 avril 2026

Auth0 recommande de passer à des rappels basés sur HTTPS à l&#39;aide des [Android App Links](https://developer.android.com/training/app-links#android-app-link) et des [Apple Universal Links](https://developer.apple.com/documentation/xcode/allowing-apps-and-websites-to-link-to-your-content), dans la mesure du possible, pour toutes les applications natives qui utilisent le [flux de code d&#39;autorisation](/docs/fr-ca/get-started/authentication-and-authorization-flow/authorization-code-flow-with-pkce), afin de renforcer la sécurité et de réduire les risques d&#39;usurpation d&#39;application et d&#39;attaques par hameçonnage.

De plus, Auth0 met en place une nouvelle invite de confirmation de connexion pour les demandes d&#39;authentification qui utilisent des schémas d&#39;URI personnalisés ou des URI de bouclage comme rappels. Cette invite s&#39;affichera dans les cas où une réponse était auparavant renvoyée sans aucune interaction de l&#39;utilisateur.

Consultez [Migrer vers la confirmation par l&#39;utilisateur final des URI de rappel non vérifiables](/docs/fr-ca/troubleshoot/product-lifecycle/past-migrations/migrate-to-non-verifiable-callback-uri-end-user-confirmation) pour en apprendre davantage.

## Attributs étendus dans les connexions à l&#39;API d&#39;identité Azure Active Directory (v1) {#extended-attributes-in-azure-active-directory-v1-identity-api-connections}

**Déconseillé** : 18 juin 2025

**Fin de vie** : 1er septembre 2025

En raison de l&#39;[abandon d&#39;Azure AD Graph](https://learn.microsoft.com/en-us/graph/migrate-azure-ad-graph-overview) et de son [retrait](https://techcommunity.microsoft.com/blog/microsoft-entra-blog/what%E2%80%99s-new-in-microsoft-entra-%E2%80%93-june-2025/4352579#community-4352579-toc-hId--1605859169) prévu, Auth0 ne prendra plus en charge l&#39;activation des options liées aux attributs étendus dans les connexions Microsoft Azure AD (`strategy=waad`) configurées pour utiliser l&#39;API d&#39;identité Azure Active Directory (v1).

Si vous avez reçu une notification par courriel, il se peut qu&#39;un ou plusieurs de vos locataires disposent d&#39;une connexion Microsoft Azure AD qui cible l&#39;API d&#39;identité **Azure Active Directory (v1)** et qui est configurée pour obtenir des attributs étendus, ce qui pourrait avoir des répercussions.

Vous devez examiner les locataires concernés. Pour les connexions qui dépendent de la fonctionnalité déconseillée, vous devez soit :

* Mettre à jour les connexions pour cibler **Microsoft Identity Platform (v2)**, afin que les terminaux Microsoft Graph soient utilisés plutôt qu&#39;Azure AD Graph, désormais déconseillé, lors de la récupération des informations sur les attributs étendus.

* Désactiver toutes les options d&#39;attributs étendus.

Même si la deuxième option vous permet de conserver des connexions ciblant l&#39;API d&#39;identité **Azure Active Directory (v1)** lorsque les informations étendues ne sont pas nécessaires, il est généralement recommandé de cibler **Microsoft Identity Platform (v2)**. Pour en savoir plus, consultez [Connecter votre application à Microsoft Azure Active Directory](/docs/fr-ca/authenticate/identity-providers/enterprise-identity-providers/azure-active-directory/v2).

## Extension Real-Time Webtask Logs {#real-time-webtask-logs-extension}
...
En remplacement, la fonctionnalité [Journaux en temps réel des Actions](/docs/fr-ca/customize/actions/actions-real-time-logs) est offerte directement dans le Auth0 Dashboard. L&#39;extension ne pourra plus être installée, mais les locataires qui l&#39;ont déjà installée y conserveront l&#39;accès jusqu&#39;à l&#39;EOL prévue.

## Actions multiples pour les triggers de fournisseurs personnalisés de téléphone et de courriel {#multiple-actions-for-custom-phone-and-email-provider-triggers}

**Déconseillé :** 16 juin 2025

**Fin de vie :** 16 décembre 2025

Auth0 instaure une limite d&#39;une seule Action pour les Actions associées aux triggers suivants :

* `custom-phone-provider`
* `custom-email-provider`

Cette limitation s&#39;applique au terminal [Create an Action](/docs/fr-ca/api/management/v2/actions/post-action) de la Management API (`POST` - `/api/v2/actions/actions`). Dès que cette nouvelle limite entre en vigueur pour un locataire donné, toute tentative de création de plusieurs actions pour ces triggers échoue.

## Flux de courriel de déblocage non personnalisable de la protection contre les attaques par force brute {#uncustomizable-brute-force-protection-unblock-email-flow}

**Déconseillé :** 9 juin 2025

**Fin de vie :** 9 décembre 2025

Une version mise à jour du flux de déblocage par courriel de la [protection contre les attaques par force brute](/docs/fr-ca/secure/attack-protection/brute-force-protection) permet la personnalisation et la localisation au moyen de la connexion universelle et améliore l&#39;expérience lorsque des analyseurs de sécurité du courriel traitent le courriel de déblocage.

## Champ `fromSandbox` dans les réponses d&#39;erreur de l&#39;API d&#39;authentification {#field-fromsandbox-in-authentication-api-error-responses}

**Déconseillé :** 11 juin 2025

**Fin de vie :** 11 décembre 2025

Les réponses d&#39;erreur de l&#39;API d&#39;authentification n&#39;incluront plus le champ `fromSandbox` pour les flux qui exigent l&#39;appel d&#39;un script de base de données personnalisée. Par exemple, dans un flux d&#39;inscription d&#39;un utilisateur final à une connexion de base de données personnalisée, la réponse d&#39;erreur de l&#39;API ne renverra plus ce champ.

## Autorisation d’omettre le mot de passe lors des changements liés à l’hôte du fournisseur de courriel SMTP {#allow-omitting-password-on-smtp-email-provider-host-related-changes}

**Déconseillé :** 13 mai 2025

**Fin de vie :** 13 novembre 2025

Lorsque vous mettez à jour l’hôte, le port ou le nom d’utilisateur d’un fournisseur de courriel SMTP au moyen d’une requête `PATCH` vers le [`terminal /api/v2/emails/provider`](/docs/fr-ca/api/management/v2/emails/patch-provider), vous devrez peut-être préciser un mot de passe dans le champ `credentials.smtp_pass`.
...
* Lorsque vous attribuez aux champs `credentials.smtp_host`, `credentials.smtp_port` ou `credentials.smtp_user` d’un fournisseur de courriel SMTP une valeur différente de la valeur existante, ou lorsque vous ne mettez à jour qu’une partie de ces trois champs.

Auth0 n’exige pas de valeur explicite pour le champ `credentials.smtp_pass` dans les cas suivants :

* Lorsque vous mettez à jour un fournisseur de courriel SMTP et que le corps de la requête contient, pour les champs `credentials.smtp_host`, `credentials.smtp_port` et `credentials.smtp_user`, les mêmes valeurs que celles déjà en place.

## Pagination par décalage sans restriction dans la Management API des connexions {#unrestricted-offset-pagination-in-connections-management-api}

**Déconseillé** : 29 avril 2025

**Fin de vie** : 27 octobre 2025

La pagination par décalage offerte par le terminal [get all connections](/docs/fr-ca/api/management/v2/connections/get-connections) de la Management API ne permettra plus de récupérer un résultat paginé au-delà des 1000 premières connexions.

Par exemple, le service renverra une réponse d&#39;erreur si `page=30&per_page=50` ou `page=15&per_page=100` est utilisé. Dans les deux cas, le nombre d&#39;enregistrements demandés par page multiplié par l&#39;index de page demandé plus un (puisque l&#39;index de page commence à zéro) fait dépasser à la requête les 1000 connexions initiales.

Ainsi, avec une taille de page de 50, le dernier index de page que vous pouvez demander sans erreur est 19 (`page=19&per_page=50`), et avec la taille de page maximale de 100, vous pouvez aller jusqu&#39;à l&#39;index de page 9 (`page=9&per_page=100`).

Les conditions qui dépassent la limite déclenchent l&#39;erreur même si le locataire associé à la requête compte moins de 1000 connexions.

## Propriétés protégées dans les connexions sociales non personnalisées {#protected-properties-in-non-custom-social-connections}
...
## Suppression injustifiée de sessions après des mises à jour d’utilisateurs par la Management API {#unwarranted-session-removal-after-management-api-user-updates}
...
## Utilisation obligatoire du SNI pour les requêtes HTTPS {#mandatory-use-of-sni-for-https-requests}
...
## Utilisez toujours HTTPS pour communiquer avec Auth0 {#always-use-https-for-communication-with-auth0}
...
## Transition de la Management API : mise à jour de l’attribution des rôles pour exiger le scope Create {#management-api-transition-updating-roles-assignment-to-require-create-scope}
...
## Mettre à jour les applications qui utilisent l’authentification inter-origines {#update-applications-that-use-cross-origin-authentication}
...
## Retirer l’accès aux Subscription Tickets pour les administrateurs de tenant {#remove-subscription-tickets-access-for-tenant-admins}
...
## Dépréciation des liens de réinitialisation du mot de passe et de vérification du courriel dans les journaux du tenant {#password-reset-and-email-verification-links-in-tenant-logs-deprecation}
...
## Réduction de la durée de vie maximale des transactions de connexion {#reducing-maximum-expiration-time-for-login-transactions}
...
## Dépréciation des portées non enregistrées dans les jetons d’actualisation {#unregistered-scopes-in-refresh-tokens-deprecation}
...
## Dépréciation des dépôts auth0-cordova, angular-auth0 et express-oauth2-bearer {#auth0-cordova-angular-auth0-and-express-oauth2-bearer-repo-deprecations}
...
## Migration des Actions de Node.js 16 vers Node.js 18 {#actions-migration-from-nodejs-16-to-nodejs-18}
...
## Prise en charge d’edge.js dans les fonctionnalités d’extensibilité {#support-for-edgejs-in-extensibility-features}
...
## Prise en charge de oracledb dans les fonctionnalités d’extensibilité {#support-for-oracledb-in-extensibility-features}
...
## Fournisseur de revendications Auth0 pour SharePoint 2010 / 2013 {#auth0-claims-provider-for-sharepoint-2010-2013}
...
## Pagination par checkpoint pour le point de terminaison Get Role Users {#checkpoint-pagination-on-get-role-users-endpoint}
...
## Revendications personnalisées héritées {#legacy-custom-claims}
...
## Ancienne plateforme Private Cloud {#legacy-private-cloud-platform}
...
## Extensions de logs {#log-extensions}
...
## Validation du nom d’hôte du tenant {#tenant-hostname-validation}
...
## Migration vers Node.js 16 {#nodejs-16-migration}
...
## Longueur fixe des jetons d’accès opaques et des codes d’autorisation {#opaque-access-token-and-authorization-code-fixed-length}
...
## Fin de vie du runtime d’extensibilité Node.js v8 {#nodejs-v8-extensibility-runtime-end-of-life}
...
## Dépréciation de l’ancienne périphérie réseau {#legacy-network-edge-deprecation}
...
## Dépréciation des requêtes non paginées vers Management API v2 {#unpaginated-management-api-v2-request-deprecation}
...
## Dépréciation de Custom Domain dans Private Cloud {#private-cloud-custom-domain-deprecation}
...
## Validation de la redirection à la déconnexion {#logout-redirect-validation}
...
## Dépréciation du rôle Dashboard Application Admin {#application-admin-dashboard-role-deprecation}
...
## Dépréciation des anciennes versions de TLS {#legacy-tls-deprecation}
...
## Dépréciation d’Auth0-analytics.js {#auth0-analyticsjs-deprecation}
...
## Dépréciation des métadonnées des identifiants d’appareil sans `user_id` {#device-credential-metadata-without-user_id-deprecation}
...
## Dépréciation de la vérification des courriels pour Azure AD/ADFS {#azure-adadfs-email-verification-deprecation}
...
## changements apportés à l’attribut sameSite des cookies {#samesite-cookie-attribute-changes}
...
## Dépréciation de User Search v2 {#user-search-v2-deprecation}
...
## Dépréciation du point de terminaison Passwordless pour les applications confidentielles {#passwordless-endpoint-from-confidential-applications-deprecation}
...
## Dépréciation des points de terminaison de la Management API qui utilisent des jetons d’identification comme identifiants {#management-api-endpoints-using-id-token-credentials-deprecation}
...
## Dépréciation de Resource Owner Password /oauth/ro {#resource-owner-password-oauthro-deprecation}
...
## Dépréciation de Management API v1 {#management-api-v1-deprecation}
...
## Dépréciation de la connexion Instagram {#instagram-connection-deprecation}
...
## Modifications de l’API Yahoo {#yahoo-api-changes}
...
## Abandon de Google Cloud Messaging {#google-cloud-messaging-deprecation}
...
## Dépréciation du champ Social Context de Facebook {#facebook-social-context-field-deprecation}
...
## Changements apportés à Facebook Graph API {#facebook-graph-api-changes}
...
## Lock v11 et Auth0.js v9 {#lock-v11-and-auth0js-v9}
...
### Fonctionnalités touchées {#features-affected}
...
## Dépréciation de Tenant Log Search v2 {#tenant-log-search-v2-deprecation}
...
## Nouvelles adresses IP à mettre sur la liste d’autorisation en Australie {#new-ip-addresses-for-allowlisting-in-australia}
...
### Fonctionnalités touchées {#features-affected-2}
...
## Nouvelles adresses IP à ajouter à la liste d’autorisation en Europe {#new-ip-addresses-for-allowlisting-in-europe}
...
### Fonctionnalités concernées {#features-affected-3}
...
## Migration du fournisseur de CDN dans les environnements d’Europe et d’Australie {#cdn-provider-migration-in-the-europe-and-australia-environments}
...
### Fonctionnalités concernées {#features-affected-4}
...
## Migration des Rules pour les échanges Password et refresh token {#password-and-refresh-token-exchange-rules-migration}
...
### Fonctionnalités concernées {#features-affected-5}
...
## Suppression de la liaison de comptes {#account-linking-removal}
...
### Fonctionnalités concernées {#features-affected-6}
...
## Plages d’adresses IP de la liste d’autorisation {#allowlist-ip-address-ranges}
...
### Fonctionnalités concernées {#features-affected-7}
...
## Processus de réinitialisation du mot de passe vulnérable {#vulnerable-password-flow}
...
### Fonctionnalités touchées {#features-affected-8}
...
## Paramètre state requis lors d’une redirection depuis une rule {#state-parameter-required-on-redirect-from-rule}
...
### Fonctionnalités concernées {#features-affected-9}
...
## Changement du point de terminaison de suppression de tous les utilisateurs {#delete-all-users-endpoint-change}
...
### Fonctionnalités concernées {#features-affected-10}
...
## Changements apportés à la personnalisation des modèles d’envoi de courriels {#email-delivery-template-customization-changes}
...
## Jetons d’accès du fournisseur d’identité supprimés du profil utilisateur et du jeton d’identité {#identity-provider-access-tokens-removed-from-user-profile-and-id-token}
...
### Fonctionnalités touchées {#features-affected-11}
...
## Validation du point de terminaison Tokeninfo {#tokeninfo-endpoint-validation}
...
### Fonctionnalités touchées {#features-affected-12}
...
## Changements à l’adresse d’expéditeur des courriels {#email-delivery-from-address-changes}
...
## Les points de terminaison PATCH et POST n’acceptent plus l’indicateur secret_encoded {#patch-and-post-endpoints-no-longer-accept-secret_encoded-flag}
...
### Fonctionnalités concernées {#features-affected-13}
