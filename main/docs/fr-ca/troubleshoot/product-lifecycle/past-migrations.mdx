...
Ce sont des migrations qui ont déjà été activées pour tous les clients. Si vous avez des questions, créez un ticket dans notre [Centre d’assistance](%24%7Benv.DOMAIN_URL_SUPPORT%7D).
...
Pour en savoir plus, consultez [Migration de la validation de nom d’hôte de locataire](/product-lifecycle/deprecations-and-migrations/tenant-hostname-migration).
...
Depuis le 1er février 2021, Auth0 affiche un bouton de migration pour vous aider à vous préparer à ce changement. Pour en savoir plus, consultez [Migrer vers la gestion des nouveaux rôles du Dashboard](/product-lifecycle/deprecations-and-migrations/migrate-tenant-member-roles).
...
Le 18 mai 2021, dans le nuage public et dans la version de juin du nuage privé (v2106), Auth0 commencera à utiliser la propriété de connexion pour toutes les connexions Azure AD et ADFS. Vous devez vous assurer que toutes vos connexions sont correctement configurées avant cette date. Pour en savoir plus, consultez [Vérification des courriels pour Azure AD et ADFS](/connections/azuread-adfs-email-verification).
...
L’objectif de ces changements est d’améliorer la sécurité et d’aider à atténuer les attaques CSRF. Pour plus de détails, consultez [Changements de l’attribut des témoins sameSite](/users/cookies/samesite-cookie-attribute-changes).
...
Auth0 déconseille l’utilisation de jetons d’ID comme identifiants pour appeler certains des utilisateurs et des points de terminaison de l’appareil et les remplace par l’utilisation de jetons d’accès à la place. Pour plus de détails, consultez [Migrer vers les points de terminaison du Management API avec des jetons d’accès](/product-lifecycle/deprecations-and-migrations/past-migrations/migrate-to-calling-api-with-access-tokens) et [Migrer vers Associer des comptes d’utilisateurs liés avec des jetons d’accès](/product-lifecycle/deprecations-and-migrations/past-migrations/link-user-accounts-with-access-tokens-migration).
...
À compter du 8 juillet 2017, Auth0 a déconseillé le point de terminaison `/oauth/ro` pour les connexions par mot de passe et sans mot de passe. Vous pouvez désormais mettre en œuvre la même fonctionnalité à l’aide du point de terminaison `/oauth/token`. Pour en savoir plus, consultez [Migration du flux de mot de passe du propriétaire de ressource](/product-lifecycle/deprecations-and-migrations/past-migrations/migration-oauthro-oauthtoken).
...
Facebook a annoncé qu’à partir du 31 mars 2020, il déconseillait les API Instagram héritées et qu’aucune autre connexion avec Instagram ne serait proposée. Pour plus de détails, reportez-vous à la section [Obsolescence des connexions Instagram](/product-lifecycle/deprecations-and-migrations/past-migrations/instagram-connection-deprecation).
...
Yahoo a modifié la façon de récupérer le profil utilisateur et les informations qu’il contient. Pour plus de détails, reportez-vous à la section [Changements apportés à l’API Yahoo](/product-lifecycle/deprecations-and-migrations/past-migrations/yahoo-api-changes).
...
Depuis le 11 avril 2019, [Google a déconseillé](https://firebase.googleblog.com/2018/04/time-to-upgrade-from-gcm-to-fcm.html) et remplacé Google Cloud Messaging (GCM) par Firebase Cloud Messaging (FCM). Pour plus de détails, reportez-vous à la section [Migration de Google vers Firebase Cloud Messaging](/product-lifecycle/deprecations-and-migrations/past-migrations/google-firebase-migration).
...
Le 30 avril 2019, Facebook a déconseillé l’utilisation du champ **Social Context (Contexte social)** pour les nouvelles applications. Pour plus de détails, reportez-vous à la section [Obsolescence du champ de contexte social‌ de Facebook](/product-lifecycle/deprecations-and-migrations/past-migrations/facebook-social-context-field-deprecation).
...
Si vous souhaitez activer le nouveau comportement sur ce locataire pour tester la date d’activation obligatoire, connectez-vous à [Dashboard](%24%7Bmanage_url%7D) et activez la bascule **Run Rules on Password and Refresh Token Exchanges (Exécuter des règles sur les échanges de mots de passe et de jetons d’actualisation)** dans [ Tenant Settings (Paramètres du locataire) > Advanced (Avancés)](%24%7Bmanage_url%7D/#/tenant/advanced).
...
Si vous appelez directement le point de terminaison Tokeninfo, assurez-vous que la valeur de l’attribut `iss` du jeton d’ID en cours de validation correspond à votre espace de noms de locataire Auth0 : `https://${account.namespace}/`. Vous pouvez utiliser [jwt.io](https://jwt.io/) pour décoder le jeton afin de confirmer la valeur de l’attribut `iss`.
