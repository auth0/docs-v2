...
3. Désactivez les requêtes non paginées de Management API pour votre locataire. Accédez à [Dashboard > Paramètres du locataire > Avancés > Migration](%24%7Bmanage_url%7D/#/tenant/advanced). Cela simulera le comportement attendu après la fermeture de la fenêtre de migration, ce qui permettra aux appels vers les points de terminaison concernés de renvoyer jusqu’à 50 résultats. Vous pourrez réactiver les demandes non paginées à tout moment avant cette date.

### Mettre à jour les extensions

Vous devrez peut-être mettre à jour les versions précédentes des [extensions Auth0](/extensions) et des extensions personnalisées vers les dernières versions pour vous assurer qu’elles effectuent uniquement des requêtes paginées.
...
2. Accédez à [Dashboard > Extensions](%24%7Bmanage_url%7D/#/extensions), sélectionnez **Installed Extensions (Extensions installées)** et cliquez sur le lien **Update (Mettre à jour)** de l’extension s’il est présent.

### Mettre à jour l’outil Deploy CLI

Si vous utilisez l’outil `auth0-deploy-cli` , vous devez mettre à jour vers la version **5.3.0 ou ultérieure**. Pour en savoir plus, consultez [Déployer l’outil Deploy CLI](/deploy/deploy-cli-tool).