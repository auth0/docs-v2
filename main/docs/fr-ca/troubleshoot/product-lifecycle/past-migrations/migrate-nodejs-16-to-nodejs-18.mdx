---
description: Décrit les éléments d’Auth0 Actions touchés par la migration de Node.js v12 et v16 vers Node.js v18.
...
### Migrer les Rules et Hooks vers Actions {#migrate-rules-and-hooks-to-actions}
...
### Mettre à niveau les Actions personnalisées existantes {#upgrade-existing-custom-actions}

Vous pouvez mettre à niveau individuellement les Actions personnalisées existantes basées sur Node 12 ou 16 vers Node 18 et [rétablir la version précédente](/docs/fr-ca/customize/actions/manage-versions#revert-actions) en utilisant l’ancien runtime. Pour faire passer les Actions à Node 18, créez et déployez une nouvelle version de l’implémentation existante avec les modifications requises, puis configurez-la pour utiliser Node 18 comme runtime.

### Choisissez Node 18 pour les autres produits d’extensibilité {#choose-node-18-for-other-extensibility-products}
...
Plus précisément pour les Custom DB scripts, vous pouvez [suivre les étapes expliquées sur cette page](/docs/fr-ca/authenticate/database-connections/custom-db/custom-database-connections-scripts/environment) pour vérifier individuellement un script avec une version précise du runtime avant de modifier la version globale du runtime.

## Incompatibilités connues {#known-breaking-changes}

### Modules npm magiques {#magic-npm-modules}
...
* cql
* ip
* <Tooltip tip="JSON Web Token (JWT) : format standard des ID Token (et souvent des access tokens) utilisé pour représenter des claims de façon sécurisée entre deux parties." cta="Voir le glossaire" href="/docs/fr-ca/glossary?term=jwt">jwt</Tooltip>
* Knex
* mongo
* mysql
* mysql&#95;pool
...
Dans le contexte de Rules, Custom Database Connections et connexions sociales personnalisées, vous devez inclure explicitement une version du module [indiquée comme disponible pour Node 18](https://auth0-extensions.github.io/canirequire/).

Dans Hooks et Actions, vous devez ajouter la version cible voulue comme dépendance explicite avant d’inclure le module.

### Versions de modules supprimées de Can I Require {#can-i-require-module-versions-removed}

Nous avons retiré de [Can I Require](https://auth0-extensions.github.io/canirequire/) la prise en charge des versions précisées des modules énumérés ci-dessous pour le runtime Node 18. Cette modification touche le code d’extensibilité associé à Rules, aux scripts de Custom Database Connection et aux scripts de connexion sociale personnalisée.

<Warning>
  Le script Fetch User Profile pour les connexions sociales personnalisées suivantes ([Indeed](https://marketplace.auth0.com/integrations/indeed-social-connection), [monday.com](https://marketplace.auth0.com/integrations/monday-com-monday-com-social-connection), [Snapchat](https://marketplace.auth0.com/integrations/snapchat-social-connection) et [Tumblr](https://marketplace.auth0.com/integrations/tumblr-social-connection)), offertes dans le Marketplace, utilisait la version `0.22.0` du module `axios`, qui n’est pas disponible dans Node 18. Si vous utilisez l’une de ces connections, vérifiez et mettez à jour les scripts correspondants au besoin au moyen de la [Management API](https://auth0.com/docs/api/management/v2).
</Warning>
...
### Renégociation sécurisée requise par défaut pour les connexions TLS {#secure-renegotiation-required-by-default-for-tls-connections}
...
Si votre code d’extensibilité effectue des appels réseau vers des services externes, les serveurs cibles doivent prendre en charge la renégociation sécurisée; sinon, les requêtes échoueront et vous recevrez une erreur semblable à :
