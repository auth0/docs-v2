...
## Points de terminaison touchés {#endpoints-affected}
...
### Modifications des portées {#scope-changes}
...
### Obtenir des jetons d’accès {#get-access-tokens}
...
Notez que `aud` correspond à l’URI de l’API de votre tenant, `scope` à `${scope}` et `sub` à l’ID de l’utilisateur connecté.
...
#### Embedded Lock ou auth0.js {#embedded-lock-or-auth0js}
...
### Modifications à la liaison de comptes {#account-linking-changes}
