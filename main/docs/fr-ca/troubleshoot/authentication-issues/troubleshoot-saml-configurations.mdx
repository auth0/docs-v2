...
1. Naviguez vers [Auth0 Dashboard; Authentification; Entreprise](%24%7Bmanage_url%7D/#/connections/enterprise), et sélectionnez un type de connexion.
...
1. Naviguez vers [Auth0 Dashboard > Authentification > Entreprise](%24%7Bmanage_url%7D/#/connections/enterprise), et sélectionnez **SAML**.
...
1. Vérifiez les pages [Logs (Journaux)](%24%7Bmanage_url%7D/#/logs) et [Users (Utilisateurs)](%24%7Bmanage_url%7D/#/users) dans Auth0 Dashboard pour voir si Auth0 affiche un événement de connexion réussi. Si les journaux d’Auth0 n’affichent pas d’événement de connexion réussi, il y a probablement un problème avec l’Assertion d’authentification SAML renvoyée par l’IdP ou Auth0 n’est pas en mesure de consommer l’assertion.
2. Vérifiez les informations qu’Auth0 envoie à l’application en [capturant un suivi HTTP de la séquence de connexion](/troubleshoot/tools/generate-and-analyze-har-files) et en analysant ce suivi HTTP.
...
2. Il y aura ensuite une redirection vers une URL Auth0 (comme par exemple `${account.namespace}`).
...
1. Allez à [Dashboard) > User Management (Gestion des utilisateurs) > Users (Utilisateurs)](%24%7Bmanage_url%7D/#/users).
...
1. Pour activer le débogage pour la connexion, accédez à [Authentification (Authentification) > Enterprise (Entreprise)](%24%7Bmanage_url%7D/#/connections/enterprise).
2. Ouvrez la liste des connexions de l’IdP **SAML** , cliquez sur **Settings (Paramètres)**, et activez **Debug Mode (Mode débogage)**.
3. Avec **Debug Mode (Mode débogage)** activé, les entrées de journal **Warning During Login (Avertissement lors de la connexion)**[dans le Dashboard](%24%7Bmanage_url%7D/#/logs) contiennent une propriété `original_profile` répertoriant chaque attribut inclus dans l’assertion SAML par le fournisseur d’identité. Vous pouvez utiliser cette liste pour voir les informations que l’IdP envoie et pour pouvoir créer les mappages. Si l’attribut manquant n’est pas du tout présent dans l’assertion, veuillez travailler avec l’IdP pour vous assurer qu’il est inclus.
...
1. Vous pouvez le faire en accédant à [Authentication (Authentification) > Enterprise (Entreprise)](%24%7Bmanage_url%7D/#/connections/enterprise).
...
2. Vérifiez les informations qu’Auth0 envoie à l’application en [capturant un suivi HTTP de la séquence de connexion](/troubleshoot/tools/generate-and-analyze-har-files) et en analysant ce suivi HTTP. Vous pouvez visualiser le suivi HTTP dans un analyseur de fichiers HAR, tel que [l’analyseur HAR de Google](https://toolbox.googleapps.com/apps/har_analyzer/).
...
1. L’URL du Service consommateur d’assertions (ACS) au niveau du fournisseur d’identité inclut le nom de la connexion (p. ex., `https://${account.namespace}/login/callback?connection=CONNECTION_NAME`)
...
1. Allez à [Dashboard (Tableau de bord) > User Management (Gestion des utilisateurs) > Users (Utilisateurs)](%24%7Bmanage_url%7D/#/users).
2. Trouvez et cliquez sur l’utilisateur spécifique pour ouvrir son profil. Si plusieurs lignes existent pour un utilisateur précis, assurez-vous d’ouvrir l’enregistrement liée à la connexion SAML.
...
3. Vérifiez les informations qu’Auth0 envoie à l’application en [capturant un suivi HTTP de la séquence de connexion](/troubleshoot/tools/generate-and-analyze-har-files) et en analysant ce suivi HTTP. Vous pouvez visualiser le suivi HTTP dans un analyseur de fichiers HAR, tel que [l’analyseur HAR de Google](https://toolbox.googleapps.com/apps/har_analyzer/).
...
10. Un suivi HTTP de l’interaction SSO dans un [fichier .har](/troubleshoot/tools/generate-and-analyze-har-files)
