...
Essayez d’éviter les appels à Management API Auth0. Management API Auth0 est limitée en termes de taux, un facteur à prendre en compte même lorsque l’on utilise l’objet `auth0` (veillez donc à l’utiliser avec parcimonie). Pour en savoir plus, consultez [Limites anti-attaques des points de terminaison de Management API.](/support/policies/rate-limit-policy/management-api-endpoint-rate-limits)
