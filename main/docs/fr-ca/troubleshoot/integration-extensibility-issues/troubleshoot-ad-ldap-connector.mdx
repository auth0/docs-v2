...
Si vous rencontrez des problèmes avec votre [ Connecteur AD/LDAP](/extensions/ad-ldap-connector), lisez ce qui suit pour savoir comment résoudre les problèmes les plus courants.
...
Pour détecter les problèmes liés aux certificats, définissez la variable `CONNECTIONS_API_V2_KEY` dans le fichier de configuration du [Connecteur AD/LDAP](/extensions/ad-ldap-connector/ad-ldap-connector-config-file-schema).
...
Si vous n’avez pas besoin que les groupes soient renvoyés sur le profil utilisateur, Auth0 vous recommande de désactiver explicitement la variable `GROUPS` dans [le fichier de configuration du Connecteur AD/LDAP](/extensions/ad-ldap-connector/ad-ldap-connector-config-file-schema).
...
Pour atténuer l’impact, corrigez toute affectation de groupe récursive ou problématique dans votre serveur AD/LDAP, et/ou augmentez la valeur de la variable `GROUPS_CACHE_SECONDS` dans le [fichier de configuration du Connecteur AD/LDAP](/extensions/ad-ldap-connector/ad-ldap-connector-config-file-schema).

#### Évaluer le volume d’utilisateurs actifs

Vérifiez si le nombre d’utilisateurs actifs utilisant le Connecteur AD/LDAP a augmenté depuis son déploiement initial. Vous pouvez évaluer cela en utilisant votre propre [solution de surveillance](/monitor-auth0/streams), ou vous pouvez obtenir une estimation à partir du [rapport des utilisateurs actifs d’Auth0](/monitor-subscription-usage).



Si le nombre d’utilisateurs actifs a augmenté et que toutes les étapes ci-dessus ont déjà été suivies, il se peut que vous deviez mettre à niveau les spécifications matérielles de votre machine hôte ou configurer un [environnement à haute disponibilité](/extensions/ad-ldap-connector/ad-ldap-high-availability).
...
Assurez-vous que votre serveur peut se connecter à votre locataire Auth0 (`https://${account.namespace}`).

Pour vérifier cela, ouvrez un navigateur et naviguez vers `https://${account.namespace}/test`.
...
Le Connecteur AD/LDAP met en cache les appartenances d’un utilisateur à un groupe. La durée de vie de ce cache est déterminée par la variable `GROUPS_CACHE_SECONDS` du fichier de configuration du [Connecteur AD/LDAP](/extensions/ad-ldap-connector/ad-ldap-connector-config-file-schema), avec une valeur par défaut de 600 (secondes).
...
1. Allez dans [Auth0 Dashboard > Authentification > Entreprise](%24%7Bmanage_url%7D/#/connections/enterprise), et sélectionnez le type de connexion **Active Directory/LDAP.**
...
Vous pourriez rencontrer cette erreur si vous n’avez pas terminé la configuration supplémentaire d’un [domaine personnalisé](/custom-domains).
...
1. Ouvrez le [fichier de configuration du Connecteur AD/LDAP](/extensions/ad-ldap-connector/ad-ldap-connector-config-file-schema).
2. Définissez `https://${account.namespace}/p/ad/jUG0dN0R` pour la variable de configuration `PROVISIONING_TICKET`.
...
Si vous configurez un [environnement à haute disponibilité](/extensions/ad-ldap-connector/ad-ldap-high-availability) et que vous rencontrez cette erreur sur une machine supplémentaire, vérifiez que les autorités de certification racine de confiance sur la machine supplémentaire correspondent aux autorités de certification racine de confiance sur la machine initiale.

## Contacter le support Auth0

Si vous ne parvenez pas à résoudre l’un de ces problèmes par vous-même, [contactez le Support Auth0](/support/open-and-manage-support-tickets).

Pour aider l’équipe de support à résoudre votre problème, incluez les éléments suivants dans votre ticket de support :

1. Description du problème.
2. Une [exportation de vos fichiers de configuration AD/LDAP](/extensions/ad-ldap-connector/import-export-ad-ldap-connector-configs).
