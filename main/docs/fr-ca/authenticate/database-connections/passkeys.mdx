...
description: Découvrez la mise en œuvre des clés d’accès dans Auth0, un identifiant d’appareil WebAuthn résistant à l’hameçonnage qui offre des avantages par rapport aux méthodes d’authentification traditionnelles.
...
## À propos des clés d’accès dans Auth0 {#about-passkeys-on-auth0}
...
### Proposer les mots de passe et les clés d’accès à la connexion {#offer-passwords-and-passkeys-at-login}

Les clés d’accès et les mots de passe peuvent coexister dans la même expérience de connexion; vous pouvez permettre aux utilisateurs finaux de choisir l’une ou l’autre méthode. Lorsque vous activez les clés d’accès pour une connexion de base de données, la connexion par mot de passe reste disponible tant que cette connexion autorise l’authentification par mot de passe.

La politique de clé d’accès de votre connexion de base de données détermine comment les clés d’accès s’affichent à la connexion :

* Remplissage automatique (UI conditionnelle) : lorsque l’utilisateur final place le curseur dans le champ d’adresse courriel ou de nom d’utilisateur, son gestionnaire d’identifiants suggère les clés d’accès enregistrées, parmi d’autres identifiants sauvegardés comme les mots de passe. Les utilisateurs qui préfèrent utiliser un mot de passe peuvent ignorer la suggestion et saisir leurs identifiants comme d’habitude.
...
Vous pouvez activer le remplissage automatique, le bouton de clé d’accès ou les deux. Pour conserver la connexion par mot de passe, assurez-vous que la connexion autorise l’authentification par mot de passe et configurez ces options dans la politique de clé d’accès de votre connexion. Pour en savoir plus, consultez [Configurer la politique de clé d’accès](/docs/fr-ca/authenticate/database-connections/passkeys/configure-passkey-policy).

### Clés d’accès avec MFA activée {#passkeys-with-mfa-enabled}
...
### Clés d’accès avec Multiple Custom Domains (MCD) {#passkeys-with-multiple-custom-domains-mcd}
...
### ID de partie de confiance pour les clés d’accès {#relying-party-id-for-passkeys}
