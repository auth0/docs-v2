...
## Configurer les paramètres de l’appareil {#configure-device-settings}
...
## Activez le type d’autorisation `Passkey` {#enable-the-passkey-grant}
...
## ID de la partie utilisatrice (`rpId`) {#relying-party-id-rpid}
...
## Implémenter des flux de clés d’accès {#implement-passkey-flows}
...
## ID de partie utilisatrice {#relying-party-id}
...
### Faire passer les utilisateurs d’une plateforme à l’autre {#move-users-between-platforms}
...
### Choisissez votre ID de partie de confiance {#choose-your-relying-party-id}
...
### Scénarios à plusieurs domaines et images de marque {#multi-domain-and-brand-scenarios}
...
### Liste de contrôle de la configuration {#configuration-checklist}
