...
6. Votre application appelle le point de terminaison [`POST /oauth/token`](/docs/fr-ca/api/authentication/passwordless/authenticate-user) avec `auth_session` et le code saisi par l’utilisateur.
...
<Card title="Avant de commencer">
  * Configurez `email_otp` et/ou `phone_otp` comme méthodes d’authentification dans votre connexion de base de données. Pour en savoir plus, consultez [Authentification sans mot de passe sur les connexions de base de données](/docs/fr-ca/authenticate/database-connections/passwordless-authentication-for-db-connect).
  * Activez la vérification OTP pour les inscriptions si vous souhaitez que les utilisateurs s’inscrivent au moyen du flux d’inscription implicite. Pour en savoir plus, consultez [Authentification sans mot de passe sur les connexions de base de données](/docs/fr-ca/authenticate/database-connections/passwordless-authentication-for-db-connect).
  ...
  * Pour l’OTP par appel vocal, activez [Unified Phone Experience](/docs/fr-ca/customize/phone-messages/unified-phone/configure-unified-phone) afin de pouvoir utiliser la voix comme canal de transmission.
</Card>

## Initier la demande d’OTP {#initiate-the-otp-challenge}
...
## Échangez le code contre des jetons {#exchange-the-code-for-tokens}
...
## Inscription implicite {#implicit-signup}
...
## Comment Auth0 distingue la connexion de l’inscription {#how-auth0-determines-login-vs-signup}
...
### POST /otp/challenge {#post-otpchallenge}
