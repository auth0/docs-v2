...
## Configurer les attributs (identifiants) {#configure-attributes-identifiers}
...
## Créer une nouvelle connexion de base de données {#create-a-new-database-connection}
...
## Mettre à jour les connexions existantes {#update-existing-connections}
...
<Tabs>
  <Tab title="Auth0 Dashboard" icon="browser">
    ...
    4. **Activer les méthodes d’authentification OTP** : Sous l’onglet **Authentication Methods** :
       * Configurez Phone pour **Allow** OTP par téléphone et enregistrez vos modifications.
...
## Utiliser les Actions Auth0 {#use-auth0-actions}
...
### Déclencheur post-login {#post-login-trigger}

Le déclencheur `post-login` se déclenche après l’authentification d’un utilisateur, mais avant que le serveur d’autorisation renvoie un jeton. L’objet `event.authentication` dans [Auth0 Dashboard &gt; Actions &gt; Triggers &gt; Post Login](https://manage.auth0.com/#/actions/triggers) expose les méthodes suivantes :
...
| OTP par téléphone par texto       | `sms`     | OTP par téléphone (SMS) utilisé pour authentifier l’utilisateur comme premier facteur.           |
| OTP par téléphone par appel vocal | `tel`     | OTP par téléphone par appel vocal utilisé pour authentifier l’utilisateur comme premier facteur. |
| Mot de passe              | `pwd`     | Mot de passe utilisé pour authentifier l’utilisateur comme premier facteur.              |

<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  `email_verified` et `phone_verified` sont définis automatiquement dans le profil utilisateur lorsqu’un utilisateur s’authentifie au moyen d’un OTP par courriel ou d’un OTP par téléphone. Si vous utilisiez auparavant une Action post-login pour définir manuellement ces indicateurs, vous pouvez supprimer cette solution de contournement.
</Callout>

Utilisez `event.authentication` pour :

* Détecter quel facteur sans mot de passe l’utilisateur a utilisé (`email`, `sms`, `tel` ou `pwd`)
* Ajouter des demandes personnalisées aux jetons selon la méthode d’authentification
