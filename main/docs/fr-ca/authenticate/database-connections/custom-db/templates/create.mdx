...
| --- | --- | --- |
| `user` | Objet | Contient les [données de profil utilisateur](#user-object-parameter) tirées du processus de création de l’utilisateur. |
...
| --- | --- |
| `errorCode` | (Requis) Chaîne. Indique le type d’erreur. |
...
| --- | --- |
| `client_id` | L’ID client de l’application Auth0 si l’utilisateur s’est inscrit par la connexion universelle, ou la clé API si l’utilisateur a été créé par l’Auth0 Dashboard ou par Management API. |
...
| `username` | Le nom d’utilisateur de l’utilisateur. Requis uniquement si le paramètre [**Nécessite le nom d’utilisateur**](/docs/fr-ca/connections/database/require-username) est activé pour la connexion à la base de données personnalisée. |
| `connection` | Le nom de la connexion Auth0. |
| `user_metadata` | Contient les propriétés de l’objet `user_metadata` sur le profil Auth0 de l’utilisateur, si l’objet existe. |
| `app_metadata` | Contient les propriétés de l’objet `app_metadata` sur le profil Auth0 de l’utilisateur, si l’objet existe. |
...
1. Vérifiez les instructions `console.log()` avec l’[Journaux d'actions en temps réel](/docs/fr-ca/customize/actions/actions-real-time-logs) d’Auth0.
