...
| `newPassword` | Chaîne | Valeur à définir comme nouveau mot de passe de l’utilisateur dans la base de données externe. Cette valeur est envoyée en clair à la fonction et [doit être cryptée](#Encryption) avant d’être envoyée à la base de données externe. |
