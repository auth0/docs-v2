...
* [Règles](/rules) : Exécutées lorsque des artefacts pour l’authentification de l’utilisateur sont générés. Par exemple :

* Jeton d’ID dans OpenID Connect (OIDC)
* Jeton d’accès dans OAuth 2.0
* Une affirmation dans SAML
* [Hooks](/hooks) : Fournissez une extensibilité supplémentaire lorsqu’il y a un échange d’artefacts non liés à l’utilisateur et lorsque des identités d’utilisateur sont créées, telles que l’enregistrement pré-utilisateur et l’enregistrement post-utilisateur.
...
* Veillez à restreindre l’accès à l'audience à l’aide d’une [Action](/actions).
