...
## Accès sécurisé à votre répertoire d’utilisateurs externe {#secure-access-to-your-external-user-store}
...
### Fournir un accès au moyen d’une API protégée {#provide-access-with-a-protected-api}
...
### Restreindre l’accès au réseau {#restrict-network-access}
...
## Délai d’expiration des requêtes asynchrones et externes {#time-out-async-and-external-calls}
...
## Évitez les fonctions anonymes {#avoid-anonymous-functions}
...
## Récupérer les jetons du fournisseur d’identité {#retrieve-identity-provider-tokens}
