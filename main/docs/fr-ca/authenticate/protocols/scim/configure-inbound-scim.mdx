...
description: "Configurer Inbound SCIM sur une connexion d’entreprise Auth0 afin qu’un fournisseur d’identité en amont puisse provisionner et déprovisionner des utilisateurs dans Auth0 au moyen du protocole SCIM 2.0."
---
...
## Activer Inbound SCIM pour une connexion Enterprise {#enable-inbound-scim-for-an-enterprise-connection}
...
### Tester avec Postman {#test-with-postman}
...
## Opérations SCIM prises en charge {#supported-scim-operations}
...
### Points de terminaison SCIM et jetons {#scim-endpoints-and-tokens}
...
### Révocation de session et déconnexion back-channel {#session-revocation-and-backchannel-logout}
...
## Options de provisionnement des utilisateurs {#user-provisioning-options}
...
### Mappage des attributs utilisateur {#user-attribute-mapping}
...
### Synchroniser des attributs supplémentaires à la connexion {#sync-additional-attributes-at-login}
...
### Synchroniser les rôles en tant qu’attribut utilisateur {#sync-roles-as-a-user-attribute}
...
### Appartenance à l’organisation {#organization-membership}
...
### Liaison de comptes d’utilisateur {#user-account-linking}
...
## Options de provisionnement des groupes {#group-provisioning-options}
...
### Utiliser des groupes avec des rôles {#use-groups-with-roles}
...
### Utiliser les groupes avec les rôles d’organisation {#use-groups-with-organizational-roles}
...
### Utiliser des groupes dans une Action post-login {#use-groups-in-a-post-login-action}
...
## Synchroniser les utilisateurs et les groupes vers des systèmes externes {#sync-users-and-groups-to-external-systems}
...
## Consignes de déploiement {#deployment-guidelines}

### Tirez parti des galeries d’intégration pour une configuration simplifiée {#leverage-integration-galleries-for-streamlined-setup}
...
### Utiliser Auth0 Self-Service SSO et le provisionnement des utilisateurs {#use-auth0-self-service-sso-and-user-provisioning}
...
### Testez d’abord SCIM dans des environnements hors production {#test-scim-in-lower-level-environments-first}
...
### Envoyez les jetons SCIM en toute sécurité {#send-scim-tokens-securely}
