...
1. Lancez [Auth0 Dashboard](%24%7Bmanage_url%7D/#/) et rendez-vous dans **Authentification > Entreprise > SAML > [Connexion] > Fourniture.**
2. Désactivez **Synchroniser les attributs du profil utilisateur à chaque connexion** sauf si vous souhaitez [synchroniser des attributs supplémentaires à la connexion](/protocols/scim/configure-inbound-scim).
...
5. Passez en revue les **mappages supplémentaires** pour vous assurer que les attributs SCIM étendus sont mappés avec vos attributs Auth0 préférés, en examinant [mappage des attributs](/authenticate/protocols/scim/configure-inbound-scim#attribute-mapping) pour plus de détails. Si vous recevez des numéros de téléphone et des adresses physiques, notez qu’Okta Workflow les envoie en tant que valeur`phoneNumbers[primary eq true].value` et `adresses[primary eq true].*` par défaut.
