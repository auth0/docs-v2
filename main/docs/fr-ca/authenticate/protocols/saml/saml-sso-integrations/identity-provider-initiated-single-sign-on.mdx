...
| --- | --- |
| `redirect_uri` | Lorsque la connexion initiée par l'IdP est terminée, la demande est ensuite redirigée vers la première URL répertoriée dans les **URL de rappel autorisées** pour l'application. Cependant, si vous définissez un `redirect_uri`, l'IdP redirigera vers cette URL. Cela ajoute de la flexibilité au cas où, par exemple, vous avez un schéma de sous-domaine défini avec un caractère générique et que vous souhaitez uniquement rediriger vers un sous-domaine spécifique. |
