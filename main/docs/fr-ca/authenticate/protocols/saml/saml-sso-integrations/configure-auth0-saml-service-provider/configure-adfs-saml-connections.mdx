...
7. Sur la page **Configurer les identifiants**, indiquez que l’**Identifiant de la partie de confiance** est `urn:auth0:${account.tenant}:{yourConnectionName}` (ou la valeur que vous avez utilisée comme nom d’affichage lorsque vous avez commencé à utiliser l’assistant). Cliquez sur **Suivant**.
...
1. Suivez le tutoriel sur la création d’une connexion SAML où [Auth0 joue le rôle de fournisseur de services](/saml-configuration/configure-auth0-saml-service-provider). Lorsque vous y êtes invité, téléversez le certificat de signature que vous avez exporté depuis ADFS.
...
1. Allez à [Authentification > Entreprise > SAML](%24%7Bmanage_url%7D/#/connections/enterprise/samlp).
