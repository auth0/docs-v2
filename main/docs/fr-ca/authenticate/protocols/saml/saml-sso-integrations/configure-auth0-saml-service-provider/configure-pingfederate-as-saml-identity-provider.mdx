...
* **EntityID :**`urn:auth0:${account.tenant}:{yourConnectionName}`
...
Lorsque vous utilisez l’authentification unique initiée par IdP, assurez-vous d’inclure le paramètre de `connection` dans l’URL du Service consommateur d'assertions (ACS) : `https://${account.namespace}/login/callback?connection={yourConnectionName}`.

Pour savoir comment configurer Auth0 pour acheminer des assertions SAML entrantes, lisez [Configurer l’authentification unique initiée par le fournisseur d’identité](/saml-configuration/saml-sso-integrations/identity-provider-initiated-single-sign-on).