---
description: Référence sur les métadonnées du fournisseur d’identité SAML exposées par Auth0, y compris l’URL de retour POST, l’ID d’entité et le certificat de signature nécessaires pour configurer votre IdP.
...
### URL de retour POST {#post-back-url}
...
### Liaison réponse SAML {#saml-response-binding}
...
### Assertion et réponse SAML {#saml-assertion-and-response}

L’assertion SAML et la réponse SAML peuvent être signées séparément ou en même temps.

### URL du service SingleLogout {#singlelogout-service-url}
...
## Assertions signées {#signed-assertions}
...
| PEM brut  | `https://{yourDomain}/rawpem?cert={yourConnectionName}`      |
| PKCS#7    | `https://{yourDomain}/pb7?cert={yourConnectionName}`         |
| Empreinte | `https://{yourDomain}/fingerprint?cert={yourConnectionName}` |

Téléchargez le certificat dans le format demandé par l’IdP.

### Authentification unique initiée par l’IdP {#idp-initiated-single-sign-on}
