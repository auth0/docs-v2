...
L’authentification unique et la déconnexion unique sont possibles grâce à l’utilisation de [sessions](/users/sessions). Il peut y avoir jusqu’à trois sessions différentes pour un utilisateur avec la SSO :

* Session locale maintenue par l’application,
* , si l’authentification unique (SSO) est activée
* , si l’utilisateur choisit de se connecter via un fournisseur d’identité (tel que Google, Facebook ou un fournisseur d’identité SAML d’entreprise)
...
Vous pouvez vérifier l’état SSO d’un utilisateur depuis une application faisant appel à la méthode `checkSession` de la trousse SDK `auth0.js` qui tentera d’[authentifier silencieusement](/login/configure-silent-authentication) l’utilisateur dans un iframe. Que l’authentification soit réussie ou non indique si l’utilisateur a un témoin SSO actif.

## Protocoles

### SAML et WS-Federation

Le Security Assertion Markup Language (SAML) et le Web Services Federation (WS-Fed) sont deux [protocoles](/protocols) largement utilisés dans les implémentations de SSO. SAML et WS-Fed échangent tous deux des données d’autorisation et d’authentification au format XML. Les principales parties de cet échange sont l’utilisateur, le fournisseur d’identité et le fournisseur de services.
...
En ce qui concerne la [SSO initiée par le fournisseur de services](/single-sign-on/inbound-single-sign-on), Auth0 est le fournisseur de services de la SSO.
...
En ce qui concerne la [SSO initiée par le fournisseur d’identité](/single-sign-on/outbound-single-sign-on), un fournisseur d’identité tiers (IdP) est le fournisseur de la SSO.
