...
description: Auth0 Single Sign-On (SSO) permet aux utilisateurs de s’authentifier une seule fois et d’accéder à toutes les applications du même locataire sans avoir à saisir de nouveau leur information d’authentification.
validatedOn: 2026-08-07
---

<Tooltip tip="authentification unique (SSO) : service qui, après qu’un utilisateur s’est connecté à une application, le connecte automatiquement à d’autres applications." cta="Voir le glossaire" href="/docs/fr-ca/glossary?term=Single+Sign-on">authentification unique</Tooltip> (SSO) se produit lorsqu’un utilisateur se connecte à une application, puis est automatiquement connecté à d’autres applications, peu importe la plateforme, la technologie ou le domaine utilisé. L’utilisateur ne se connecte qu’une seule fois, d’où le nom de cette fonctionnalité [(authentification unique)](https://auth0.com/features/single-sign-on).
...
Le SSO offre une expérience fluide aux utilisateurs de vos applications et services. Au lieu d’avoir à mémoriser des information d’authentification distincts pour chaque application ou service, les utilisateurs peuvent simplement se connecter une seule fois et accéder à l’ensemble de votre suite d’applications.
...
Pourquoi utiliser le SSO d’Auth0 ?

* **Accès unifié** : Les utilisateurs s’authentifient une seule fois pour accéder à toutes les applications indépendantes et connexes sans avoir à saisir de nouveau leur information d’authentification.
...
* **[Universal Login](/docs/fr-ca/authenticate/single-sign-on#sso-with-universal-login)** : L’utilisation d’Universal Login d’Auth0 est une façon sécurisée de mettre en œuvre le SSO sur toutes les plateformes.
...
## SSO avec Universal Login {#sso-with-universal-login}
...
### SSO lors de la première connexion {#sso-on-first-login}
...
### SSO lors des connexions ultérieures {#sso-on-subsequent-logins}
...
## SSO initié par le fournisseur de services {#service-provider-initiated-sso}
...
## SSO initié par le fournisseur d’identité {#identity-provider-initiated-sso}
...
## SSO initié par le fournisseur de services ou par le fournisseur d’identité {#service-provider-initiated-vs-identity-provider-initiated-sso}

Le choix entre le SSO initié par le fournisseur de services et celui initié par le fournisseur d’identité dépend principalement du point de départ souhaité pour le parcours utilisateur et du niveau de sécurité requis.

Nous recommandons le SSO initié par le fournisseur de services lorsque vous avez besoin d’authentification dans les cas suivants :

* **Accès direct** : Utilisez cette option lorsque les utilisateurs ajoutent généralement la page de connexion de votre application à leurs favoris ou y accèdent directement.
...
Le SSO initié par le fournisseur d’identité est courant dans de nombreuses entreprises et est souvent utilisé dans les cas suivants :
...
### Business to Business SSO {#business-to-business-sso}
