...
## Activer l’application de l’expiration des sessions {#enable-session-expiry-enforcement}
...
## Transmettre l’expiration de session à votre application {#send-session-expiration-to-your-application}
...
### Créer l’Action {#create-the-action}
...
### Associer l’Action au déclencheur Login {#bind-the-action-to-the-login-trigger}
...
## Utiliser l’expiration de session avec les SDK Auth0 {#use-session-expiry-with-auth0-sdks}
...
## Ajoutez des valeurs d’expiration de session à votre application {#add-session-expiry-values-to-your-application}
...
## Vérifier dans les journaux du tenant {#verify-in-tenant-logs}
...
## Désactiver l’application de l’expiration de la session {#disable-session-expiry-enforcement}
