...
You can set up Private Key JWT Client Authentication for your OIDC and Okta Workforce enterprise connections through either the <Tooltip href="/docs/fr-CA/glossary?term=auth0-dashboard" tip="Auth0 Dashboard
...
The OIDC Connection flow uses authenticated endpoints such as `/oauth/token` or `/oauth/par` to verify a client’s identity against the <Tooltip href="/docs/fr-CA/glossary?term=authorization-server" tip="Serveur d’autorisations :
...
* An `aud` (<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-6" tip="">audience</Tooltip>) identifying the OpenID provider’s <Tooltip href="/docs/fr-CA/glossary?term=token-endpoint" tip="Point de terminaison de jeton
Point de terminaison sur le serveur d’autorisations utilisé pour demander des jetons de manière programmatique." cta="Voir le glossaire">token endpoint</Tooltip>.
* A `jti` (JWT ID) to enable one-time use or replay protection.
* An `exp` (expiration time) that limits the token’s validity window.
* A `sub` and `iss` identifying the <Tooltip href="/docs/fr-CA/glossary?term=client-id" tip="ID client
Identifiant attribué par Auth0 à votre ressource enregistrée." cta="Voir le glossaire">client ID</Tooltip>.
...
After a user completes authentication with an upstream <Tooltip href="/docs/fr-CA/glossary?term=idp" tip="Fournisseur d’identité (IdP)
Service de stockage et de gestion des identités numériques." cta="Voir le glossaire">identity provider</Tooltip> (IdP), the user is redirected to Auth0 with an authorization code that is exchanged for tokens at the OpenID provider’s token endpoint. When Private Key JWT is enabled for a connection, the call to the OpenID provider’s token endpoint uses a client assertion instead of a client secret for a more secure authentication.

The following steps demonstrate a typical Private Key JWT Client Authentication flow.

<Frame>![A diagram mapping the Private Key JWT Client Authentication flow. ](/docs/images/fr-CA/cdy7uua7fh8z/4JOxbdG7aweqpDHCJ8VOZq/aea18936c0cbdbfbb39d0cd79af3987e/Client_Assertion_JWT_-_Diagram.png)</Frame>
