...
## Vérifier la prise en charge par l’IdP en amont {#verify-upstream-idp-support}
...
## Choisissez un algorithme de signature {#choose-a-signing-algorithm}
...
### Vérifier la configuration Auth0 {#check-auth0-configuration}
...
#### Champs DPoP manquants dans les logs du tenant {#dpop-fields-missing-in-tenant-logs}
...
### L’authentification échoue après l’activation de DPoP {#authentication-fails-after-enabling-dpop}
...
#### Le fournisseur d’identité rejette la preuve DPoP {#identity-provider-rejects-dpop-proof}
...
#### L’échange de jetons échoue pour une raison autre que DPoP {#token-exchange-fails-for-a-non-dpop-reason}
...
#### Échec de la génération de clé DPoP {#dpop-key-generation-fails}
...
#### Liaison des jetons de l’IdP {#idp-token-binding}
...
#### Gestion du nonce DPoP {#dpop-nonce-handling}
