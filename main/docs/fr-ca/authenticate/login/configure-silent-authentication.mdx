...
<td>L’utilisateur n’était pas connecté à Auth0, donc l’authentification silencieuse n’est pas possible. Cette erreur peut se produire en fonction de la configuration des paramètres <strong>Log In Session Management (Gestion de session de connexion)</strong> au niveau du locataire; en particulier, elle peut se produire après la période définie dans le paramètre <strong>Require log in after (Exiger la connexion après)</strong>. Consultez <a href="/docs/fr-ca/manage-users/sessions/configure-session-lifetime-settings">Configuration des paramètres de durée de vie des sessions</a> pour plus de détails.</td>
</tr>
<tr>
