...
| Réponse | Description |
| --- | --- |
| `login_required` | L’utilisateur n’était pas connecté à Auth0, donc l’authentification silencieuse n’est pas possible. Cette erreur peut se produire en fonction de la configuration des paramètres **Log In Session Management (Gestion de session de connexion)** au niveau du locataire; en particulier, elle peut se produire après la période définie dans le paramètre **Require log in after (Exiger la connexion après)**. Consultez [Configuration des paramètres de durée de vie des sessions](/docs/fr-ca/dashboard/guides/tenants/configure-session-Lifetime-settings) pour plus de détails. |
| `consent_required` | L’utilisateur a été connecté à Auth0, mais doit donner son consentement pour autoriser l’application. |
| `interaction_required` | L’utilisateur était connecté à Auth0 et a autorisé l’application, mais doit être redirigé ailleurs avant que l’authentification puisse être terminée; par exemple, lors de l’utilisation d’une [règle de redirection](/docs/fr-ca/rules/redirect). |
