...
Auth0 prend en charge la déconnexion unique lorsque vous [connectez votre application à un fournisseur d’identité (IdP) SAML ](/saml) et, de manière plus limitée lorsque vous [configurez Auth0 en tant qu’IdP SAML](/saml-configuration/configure-auth0-saml-identity-provider).

## Configurer la déconnexion unique lorsque l’application est connectée à un IdP SAML

Si vous souhaitez qu’Auth0 [déconnecte un utilisateur de son fournisseur d’identité](/logout/log-users-out-of-idps), ajoutez le paramètre `federated` lorsque vous appelez le [point de terminaison Déconnexion de l’Authentication API Auth0](https://auth0.com/docs/api/authentication#logout).
...
Dans les deux scénarios, vous devez [configurer le module supplémentaire SAML2 Web App](/saml-configuration/saml-sso-integrations/enable-saml2-web-app-addon) pour savoir où envoyer les réponses de déconnexion :
