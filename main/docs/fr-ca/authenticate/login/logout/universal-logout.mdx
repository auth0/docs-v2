...
You can use these attributes in Vous pouvez également intégrer des [exportations de journaux personnalisées](/monitor-auth0/streams) pour notifier les systèmes externes lorsque des événements Universal Logout se produisent. calls to [update](https://auth0.com/docs/api/management/v2#!/Connections/patch_connections_by_id) or [create](https://auth0.com/docs/api/management/v2/connections/post-connections)  SAML connections. Review the following JSON example to create a request:
