...
Pour en savoir plus sur les différences entre la connexion universelle et la connexion intégrée, consultez la section [Connexion universelle centralisée par rapport à la Connexion intégrée](/login/universal-vs-embedded-login).

## Configuration de la connexion universelle

Sur votre Auth0 Dashboard, vous pouvez choisir l’expérience à utiliser pour les pages de connexion par défaut sous [Image de marque >Connexion universelle >Options avancées](%24%7Bmanage_url%7D/#/universal-login/customizations-new).
...
Pour en savoir plus sur la façon dont ces deux expériences se comparent, consultez [Connexion universelle par rapport à la Connexion classique](/login/universal-login/universal-login-vs-classic-login).
...
Vous avez la possibilité d’activer ces normes à l’aide du bouton bascule situé dans votre Auth0 Dashboard en accédant à [**Image de marque > Connexion universelle > Options avancées**](%24%7Bmanage_url%7D/#/universal-login/customizations-new).

Vous pouvez également activer cette fonctionnalité dans Management API en mettant à jour l’indicateur `enable_ulp_wcag_compliance` dans le [point de terminaison Mise à jour des paramètres de l’invite](api/management/v2/prompts/patch-prompts) :
...
2. Configurez votre application pour qu’elle appelle le [point de terminaison de connexion](/api/authentication#login) de l’Authentication API afin de déclencher le flux de connexion et de gérer la réponse. Vous pouvez configurer cela directement ou utiliser l’une des trousses [SDK](https://auth0.com/docs/libraries) d’Auth0.
3. Créez une connexion par [base de données](/applications/set-up-database-connections), d’[entreprise](/enterprise-connections) ou via [réseau social](/social) et activez-la pour votre application.

### Rendez-vous à la page de connexion

Vous pouvez appeler directement le [point de terminaison de la connexion](/api/authentication#login) à partir de n’importe quel navigateur.
...
* Pour mieux comprendre pourquoi cette valeur est nécessaire, consultez la section [Prévenir les attaques et rediriger les utilisateurs avec les paramètres d’état d’Oauth 2.0](/attack-protection/state-parameters).
...
Pour plus d’informations sur la façon de configurer la Connexion universelle pour votre application, consultez les [Guides de démarrage rapide](/quickstarts). Choisissez l’approche qui correspond le mieux à vos technologies et suivez le guide de démarrage rapide pour un aperçu de la mise en œuvre.