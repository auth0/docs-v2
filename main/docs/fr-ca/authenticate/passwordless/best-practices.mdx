...
* [Connexion universelle sans mot de passe](/passwordless-with-universal-login)
* [Passwordless Authentication with Universal Login (Authentification sans mot de passe avec la connexion universelle)](/universal-login)
* [Passwordless Authentication with Embedded Login (Authentification sans mot de passe avec connexion intégrée)](/embedded-login)
...
Pour ces raisons, nous vous recommandons, si vous utilisez l’authentification sans mot de passe, de mettre également en œuvre [Authentification multifacteur (MFA)](/mfa) avec un facteur différent lorsque l’utilisateur effectue une opération sensible pour la sécurité.
...
* Si un utilisateur de connexion sans mot de passe a été bloqué [administrativement](/users/block-and-unblock-users), Auth0 n’enverra aucun code OTP par SMS ou courriel jusqu’à ce que l’utilisateur ait été débloqué. Ce comportement protège vos fournisseurs de SMS et de courriel en leur évitant de recevoir des demandes inutiles.

Le délai d’expiration du code à usage unique peut être modifié dans [Auth0 Dashboard > Authentication (Authentification) > Passwordless (Sans mot de passe)](%24%7Bmanage_url%7D/#/connections/passwordless).
