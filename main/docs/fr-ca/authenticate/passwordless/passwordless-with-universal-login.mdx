...
To specify a Passwordless connection during login, you must pass the `connection` parameter and its value (either `sms` or `email`) to the [Auth0 Authentication API Connexion endpoint](https://auth0.com/docs/api/authentication#login):
...
If you are using Passwordless with Universal Connexion and [Single Sign-On](/docs/authenticate/single-sign-on), connection parameters `sms` and `email` do not utilize the existing Auth0 session, and the user will be prompted to log in.
