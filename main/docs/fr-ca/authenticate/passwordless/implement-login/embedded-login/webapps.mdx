---
description: Décrit la mise en place de l’authentification sans mot de passe avec connexion intégrée dans les applications Web régulières.
title: Connexion intégrée sans mot de passe dans les applications Web régulières
---
...
Pour utiliser les API <Tooltip tip="Passwordless : forme d’authentification qui ne repose pas sur un mot de passe comme premier facteur." cta="Voir le glossaire" href="/docs/fr-ca/glossary?term=Passwordless">Passwordless</Tooltip> intégrées dans les Regular Web Applications, assurez-vous d’activer l’autorisation **Passwordless OTP** dans [Auth0 Dashboard &gt; Applications &gt; Applications](https://manage.auth0.com/#/applications), dans les paramètres de votre application, sous **Advanced Settings** &gt; **Grant Types**.

L’authentification Passwordless pour les Regular Web Applications se déroule en deux étapes :
...
Notez que si vous utilisez des liens magiques, vous n’avez pas besoin d’appeler `/oauth/token`. L’utilisateur cliquera sur le lien magique et sera redirigé vers le URL de rappel de l’application.
...
## Définir l’en-tête auth0-forwarded-for pour la limitation de débit {#setting-the-auth0-forwarded-for-header-for-rate-limit-purposes}

Le point de terminaison `/passwordless/start` a une [limite de débit](/docs/fr-ca/troubleshoot/customer-support/operational-policies/rate-limit-policy) de 50 requêtes par heure et par adresse IP. Si vous appelez l’API côté serveur, l’adresse IP de votre backend peut facilement atteindre ces limites de débit. Pour savoir comment résoudre ce problème, consultez la section Limitation de débit dans les points de terminaison Passwordless de [Utiliser les API Passwordless](/docs/fr-ca/authenticate/passwordless/implement-login/embedded-login/relevant-api-endpoints).

## Personnaliser l’authentification multifacteur {#customize-mfa}
...
Pour exiger la vérification MFA, appelez le point de terminaison [`request/mfa/challenge`](https://auth0.com/docs/api/authentication/muti-factor-authentication/request-mfa-challenge).

Personnalisez encore davantage votre flux MFA avec Auth0 Actions. Pour en savoir plus, consultez [Déclencheurs d’Actions : post-challenge - Objet API](/docs/fr-ca/customize/actions/explore-triggers/password-reset-triggers/post-challenge-trigger/post-challenge-api-object).