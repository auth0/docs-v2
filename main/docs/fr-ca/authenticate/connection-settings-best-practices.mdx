...
## Utilisez vos identifiants pour les connexions sociales {#use-your-credentials-for-social-connections}
...
## Passez en revue les données demandées {#review-requested-data}
...
## Définir la politique de mot de passe pour les connexions de base de données {#set-password-policy-for-database-connections}
...
## Désactivez l’inscription des utilisateurs si elle n’est pas appropriée pour chaque connexion de base de données {#disable-user-signup-if-its-not-appropriate-for-each-database-connection}
...
## Vérifiez les applications activées pour chaque connexion {#review-applications-enabled-for-each-connection}
...
## Utiliser RSA-SHA256 pour les connexions SAML {#use-rsa-sha256-for-saml-connections}
