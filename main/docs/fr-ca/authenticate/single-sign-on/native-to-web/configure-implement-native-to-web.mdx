---
description: Découvrez comment configurer vos applications pour utiliser l’authentification unique Native to Web.
...
## Configurer Native to Web SSO {#configure-native-to-web-sso}
...
Pour configurer Native to Web SSO, vous devez créer et gérer des session&#95;transfer&#95;tokens, puis configurer vos applications natives et Web.
...
### Créer et gérer des jetons de transfert de session {#create-and-manage-session-transfer-tokens}
...
### Configurer les applications natives {#configure-native-applications}
...
<Frame>
  <img src="/docs/images/sessions/session-transfer-native-app.png" alt="Session Transfer application native" />
</Frame>
...
### Configurer les applications Web {#configure-web-applications}
...
<Frame>
  <img src="/docs/images/sessions/session-transfer-web-application.png" alt="Transfert de session dans l’application Web" />
</Frame>
...
## Implement Native to Web SSO {#implement-native-to-web-sso}
...
### Dans votre application native {#in-your-native-application}

#### Étape 1 : Échanger un jeton d’actualisation contre un jeton de transfert de session {#step-1-exchanging-a-refresh-token-for-a-session-transfer-token}
...
#### Étape 2 : Envoyer le jeton de transfert de session au moyen d’une URL ou d’un cookie {#step-2-send-the-session-transfer-token-through-a-url-or-cookie}
...
##### Option 1 : Envoyer le session_transfer_token dans un cookie {#option-1-send-the-session_transfer_token-as-a-cookie}
...
##### Option 2 : Envoyer le session_transfer_token comme paramètre d’URL {#option-2-send-the-session_transfer_token-as-a-url-parameter}
...
### Dans votre application web {#in-your-web-application}
...
#### Option 1 : Ajouter le jeton de transfert de session à la requête de votre application web {#option-1-add-the-session-transfer-token-in-your-web-application-request}
...
#### Option 2 : Ajouter le jeton de transfert de session aux applications Web à l’aide des Auth0 SDKs {#option-2-add-the-session-transfer-token-to-web-applications-using-auth0-sdks}
...
##### Node (Express.js) {#node-expressjs}
...
##### Auth0 SPA SDK (@auth0/auth0-spa-js) {#auth0-spa-sdk-auth0auth0-spa-js}
...
##### Auth0 React SDK (@auth0/auth0-react) {#auth0-react-sdk-auth0auth0-react}
...
## Native to Web SSO avec Organizations {#native-to-web-sso-with-organizations}
...
## Session Transfer Token avec Actions {#session-transfer-token-with-actions}
...
### Accéder aux métadonnées du jeton d’actualisation parent {#access-parent-refresh-token-metadata}
...
### Journal de Token Exchange {#token-exchange-logs}
...
### Journaux d’avertissement de validation du transfert de session {#session-transfer-validation-warning-logs}
