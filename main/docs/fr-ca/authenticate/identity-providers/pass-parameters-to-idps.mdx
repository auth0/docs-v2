---
description: Découvrez comment contrôler le comportement de connexion avec un fournisseur d’identité social ou un fournisseur d’identité d’entreprise en transmettant des paramètres de requête depuis votre application Auth0 lors de la requête d’autorisation.
...
* Les <Tooltip tip="Security Assertion Markup Language (SAML) : protocole normalisé permettant à deux parties d’échanger des renseignements d’authentification sans mot de passe." cta="Voir le glossaire" href="/docs/fr-ca/glossary?term=SAML">SAML</Tooltip> IdPs ne prennent pas en charge les paramètres en amont.

## Paramètres statiques {#static-parameters}

Utilisez des paramètres statiques pour configurer votre connexion afin d’envoyer un ensemble standard de paramètres à l’IdP lorsqu’un utilisateur se connecte.

Pour configurer des paramètres statiques, appelez le point de terminaison <Tooltip tip="Management API : un produit qui permet aux clients d’effectuer des tâches administratives." cta="Consulter le glossaire" href="/docs/fr-ca/glossary?term=Management+API">Management API</Tooltip> [Créer une connexion](https://auth0.com/docs/api/management/v2/#!/Connections/post_connections) ou [Mettre à jour une connexion](https://auth0.com/docs/api/management/v2/#!/Connections/patch_connections_by_id), puis transmettez l’objet `upstream_params` dans l’objet `options` avec les paramètres que vous souhaitez envoyer à l’IdP.

### Exemple : WordPress {#example-wordpress}

WordPress vous permet de transmettre un paramètre facultatif `blog` à son <Tooltip tip="OAuth 2.0 : cadre d’autorisation qui définit les protocoles et les flux d’autorisation." cta="Voir le glossaire" href="/docs/fr-ca/glossary?term=OAuth+2.0">OAuth 2.0</Tooltip> point de terminaison d’autorisation et de demander automatiquement l’accès à un blogue précis pour les utilisateurs lorsqu’ils se connectent. Pour en savoir plus, consultez [la documentation OAuth 2.0 de WordPress](https://developer.wordpress.com/docs/oauth2/).
...
#### Récupérer la connexion {#get-the-connection}

Envoyez une requête au point de terminaison [Get a connection](https://auth0.com/docs/api/management/v2#!/Connections/get_connections_by_id) de la Management API pour récupérer les valeurs existantes de l’objet `options` :
...
<AuthCodeGroup>
  ```bash Auth0 CLI
  auth0 api get "connections/{YOUR_WORDPRESS_CONNECTION_ID}    --header "
  ```
...
#### Mettre à jour la connexion (statique) {#update-the-connection-static}
...
Appelez le point de terminaison [mettre à jour une connexion](https://auth0.com/docs/api/management/v2/#!/Connections/patch_connections_by_id) de la Management API avec l’objet `options` dans le corps de la requête :

<AuthCodeGroup>
  ```bash Auth0 CLI
  auth0 api patch "connections/%7ByourWordpressConnectionId%7D" \
...
Utilisez des paramètres dynamiques pour configurer votre connexion afin d’envoyer à l’IdP un ensemble de paramètres contenant des valeurs propres à l’utilisateur lorsqu’il se connecte.

Pour configurer des paramètres dynamiques, appelez le endpoint [Create a connection](https://auth0.com/docs/api/management/v2/#!/Connections/post_connections) ou [mettre à jour une connexion](https://auth0.com/docs/api/management/v2/#!/Connections/patch_connections_by_id) de l’Auth0 Management API, transmettez l’objet `upstream_params` dans l’objet `options` avec les paramètres que vous souhaitez envoyer à l’IdP, puis indiquez le champ auquel le paramètre est mappé à l’aide de l’attribut `alias`.
...
Voici les champs offerts pour l’attribut `alias` :
...
X vous permet de transmettre un paramètre facultatif `screen_name` à son point de terminaison d’autorisation OAuth. Le paramètre `screen_name` préremplit le champ de saisie du nom d’utilisateur sur l’écran de connexion avec la valeur fournie. Pour en savoir plus, consultez [la référence de l’API de X](https://docs.x.com/fundamentals/authentication/api-reference#get-oauth%2Fauthorize).

Pour suivre cet exemple, vous aurez besoin d’une [connexion sociale X opérationnelle](https://marketplace.auth0.com/integrations/twitter-social-connection).

#### Récupérer la connexion {#get-the-connection-2}

Envoyez une requête au point de terminaison [Get a connection](https://auth0.com/docs/api/management/v2#!/Connections/get_connections_by_id) de la Management API pour récupérer les valeurs actuelles de l’objet `options` :

<AuthCodeGroup>
  ```bash Auth0 CLI
  auth0 api get "connections/{YOUR_X_CONNECTION_ID}"
  ```
...
#### Mettre à jour la connexion (dynamique) {#update-the-connection-dynamic}

Copiez l’objet `options` existant, ajoutez l’objet `upstream_params` avec le champ `screen_name` comme attribut, puis définissez l’attribut `alias` à `login_hint` :
...
Envoyez une requête au point de terminaison [Mettre à jour une connexion](https://auth0.com/docs/api/management/v2/#!/Connections/patch_connections_by_id) de la Management API en incluant l’objet `options` dans le corps :

<AuthCodeGroup>
  ```bash Auth0 CLI
  auth0 api patch "connections/{YOUR_X_CONNECTION_ID}" \
...
#### Appeler le point de terminaison de connexion {#call-the-login-endpoint}

Lorsque vous effectuez une requête au [point de terminaison de connexion](https://auth0.com/docs/api/authentication#login) de l’Authentication API pour un utilisateur, vous pouvez transmettre son adresse courriel dans le paramètre `login_hint` :
