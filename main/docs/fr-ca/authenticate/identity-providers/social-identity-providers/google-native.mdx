...
## Configurer Sign in with Google pour les applications Android {#configuring-sign-in-with-google-for-android-applications}
...
### Créer des identifiants dans la Google Cloud Console {#create-credentials-in-the-google-cloud-console}
...
### Mettre à jour le code dans l’application Android {#update-code-in-the-android-application}
...
```go lines expandable
// Générer un nonce cryptographiquement sûr pour la protection contre la réutilisation de jetons
val nonceBytes = ByteArray(16) // 128 bits d'entropie
java.security.SecureRandom().nextBytes(nonceBytes)
val randomNonce = android.util.Base64.encodeToString(
    nonceBytes,
...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  Passez en revue les recommandations suivantes concernant l’utilisation de `nonce` :

  * Utilisez un CSPRNG pour générer le nonce. Appelez directement `java.security.SecureRandom` plutôt que des sources aléatoires à usage général. Un nonce exige au moins 128 bits d’entropie. Pour en savoir plus, consultez l’[aide-mémoire OWASP sur le stockage cryptographique](https://cheatsheetseries.owasp.org/cheatsheets/Cryptographic_Storage_Cheat_Sheet.html#secure-random-number-generation).
  * Générez un nouveau nonce pour chaque requête d’authentification. Un nonce réutilisé annule la protection contre les attaques par rejeu qu’il procure.
  * Ne consignez pas le nonce dans les journaux. Le consigner expose sa valeur à quiconque a accès aux journaux et annule sa protection contre les attaques par rejeu.
</Callout>

## Scénarios d’authentification supplémentaires {#additional-authentication-scenarios}
...
### Fédération d’entreprise {#enterprise-federation}
...
### Considérations relatives à la MFA lors de l’échange de jetons {#mfa-considerations-during-token-exchange}
