...
* [Enregistrer votre application sur le portail des développeurs Apple](https://marketplace.auth0.com/integrations/apple-social-connection) si vous ne l’avez pas déjà fait. Notez les ID et la clé suivants pour les réglages de connexion de l’application dans le Auth0 Dashboard :
...
1. Accédez à [Auth0 Dashboard > Applications > Applications](%24%7Bmanage_url%7D/#/applications), sélectionnez votre application, puis l’icône d’engrenage pour afficher la page des réglages.
...
4. Accédez à [Auth0 Dashboard >Authentification > Social (Sociale)](%24%7Bmanage_url%7D/#/connections/social) et sélectionnez **Create Connection (Créer une connexion)**.
...
* [Révoquer le jeton d’actualisation Auth0](/api/authentication#revoke-refresh-token)
