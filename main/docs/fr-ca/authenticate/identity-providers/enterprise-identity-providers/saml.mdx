...
## Obtenir le certificat de signature de l’IdP {#get-the-signing-certificate-from-the-idp}
...
### Convertir le certificat de signature en Base64 {#convert-signing-certificate-to-base64}
...
## Chiffrement des assertions {#assertion-encryption}
...
## Créer une connexion d’entreprise dans Auth0 {#create-an-enterprise-connection-in-auth0}
...
### Créer une connexion d’entreprise à l’aide du Dashboard {#create-an-enterprise-connection-using-the-dashboard}
...
#### Créer une connexion à l’aide d’un document de métadonnées SAML {#create-a-connection-using-saml-metadata}
...
##### Fournir le contenu du document de métadonnées {#provide-metadata-document-content}
...
##### Fournissez l’URL du document de métadonnées {#provide-a-metadata-document-url}
...
##### Actualiser les informations d’une connexion existante avec l’URL de métadonnées {#refresh-existing-connection-information-with-metadata-url}
...
## Spécifier un ID d’entité personnalisé {#specify-a-custom-entity-id}
...
## Configurer les mappages {#set-up-mappings}
...
## Configurer la révocation globale des jetons {#configure-global-token-revocation}
