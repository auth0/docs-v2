---
title: Comment synchroniser les utilisateurs et les groupes de Google Workspace dans Auth0 avec Directory Sync
sidebarTitle: Utiliser Directory Sync
description: Mettez automatiquement à jour dans Auth0 les profils utilisateur, les structures de groupes et les appartenances aux groupes à partir de Google Workspace à l’aide de Directory Sync.
...
## Activer Directory Sync {#enable-directory-sync}
  ...
  <Tab title="Auth0 Dashboard">
    <Steps>
      ...
      <Step title="Personnaliser les groupes synchronisés (facultatif)">
        Lorsque vous activez Directory Sync pour synchroniser à la fois les utilisateurs et les groupes à partir de Google Workspace, vous pouvez choisir de synchroniser tous les groupes de votre répertoire ou seulement un sous-ensemble afin d’éviter l’augmentation excessive de la base de données et de préserver les limites de débit de l’API.

        Vous pouvez personnaliser les groupes que vous synchronisez en les sélectionnant dans la liste paginée d’Auth0 Dashboard ou en téléversant un fichier JSON contenant les ID des groupes :

        <Tabs>
          <Tab title="Sélectionner dans la liste">
            ...
            2. Dans la fenêtre **Select Groups** qui s’ouvre, sélectionnez l’onglet **Select** pour afficher la liste des groupes disponibles.

               Ces informations sont synchronisées en temps réel avec vos groupes Google Workspace. Vous pouvez utiliser la barre de recherche pour filtrer par nom de groupe ou par courriel.

            3. Cochez les cases à côté des groupes que vous souhaitez synchroniser, puis sélectionnez **Save Groups** au bas de la fenêtre.
          </Tab>

          <Tab title="Téléverser un fichier JSON">
            Commencez par créer le fichier JSON. Vous pouvez [obtenir la liste de tous les ID de groupe à l’aide de l’API Directory de Google Workspace](https://developers.google.com/workspace/admin/directory/reference/rest/v1/groups/list).

            Chaque objet de groupe de la liste possède les propriétés suivantes :
            ...
            Créez le fichier au format suivant en remplaçant les valeurs d’exemple par les ID de groupe réels et les autres valeurs :
            ...
            2. Dans la fenêtre **Select Groups** qui s’ouvre, sélectionnez l’onglet **Upload**.

            3. Sélectionnez **+ Choose file** et téléversez le fichier JSON.

            4. Une fois le fichier téléversé, sélectionnez **Save Groups** au bas de la fenêtre.
          </Tab>
        </Tabs>

        La section **Resources** affiche **Syncing specific groups**. Pour mettre à jour les groupes que vous synchronisez, revenez à la même fenêtre **Select Groups** et mettez à jour vos choix.
...
## Surveiller l’activité de Directory Sync {#monitor-directory-sync-activity}
