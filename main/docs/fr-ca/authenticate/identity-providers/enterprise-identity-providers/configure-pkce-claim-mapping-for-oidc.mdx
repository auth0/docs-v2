...
## Configurer PKCE pour les connexions OIDC {#configure-pkce-for-oidc-connections}
...
### Afficher la configuration PKCE d’une connexion {#view-pkce-configuration-for-a-connection}
...
#### Utiliser le point de terminaison de découverte {#use-discovery-endpoint}
...
#### Sans point de terminaison de découverte {#without-discovery-endpoint}
...
### Valeurs de configuration PKCE prises en charge {#supported-pkce-configuration-values}
...
## Mapper les claims pour les connexions OIDC {#map-claims-for-oidc-connections}
...
### Propriétés des modèles de mappage {#mapping-template-properties}
...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  Okta Workforce n’utilise pas le mappage `bind_all`. Vous devez plutôt utiliser une connexion Enterprise OIDC. Pour en savoir plus, lisez [Arrays Mapped to String in OIDC or Okta Workforce Connection](https://support.auth0.com/center/s/article/Arrays-mapped-to-string-in-OIDC-or-Okta-Workforce-connection).
</Callout>

#### Claims restreints {#restricted-claims}
...
#### Mappage simple des claims utilisateur {#simple-user-claim-mapping}
...
#### Mappage des claims de groupe {#group-claim-mapping}
...
#### Combiner des valeurs littérales et des objets de contexte {#combining-literal-values-and-context-objects}
