---
description: Découvrez des exemples de cas d’utilisation de Custom Token Exchange, accompagnés d’exemples de code pour la mise en œuvre.
...
### Cas d’utilisation : Migration transparente vers Auth0 {#use-case-seamless-migration-into-auth0}
...
### Cas d’utilisation : Réutiliser un fournisseur d’authentification externe {#use-case-re-use-an-external-authentication-provider}
...
### Cas d’utilisation : Obtenir des jetons Auth0 pour une autre audience {#use-case-get-auth0-tokens-for-another-audience}
...
### Cas d’utilisation : Effectuer une vérification MFA pendant l’échange de jeton personnalisé {#use-case-perform-mfa-during-custom-token-exchange}
...
### Cas d’utilisation : agent de soutien agissant au nom d’un utilisateur final par API {#use-case-support-agent-acting-on-behalf-of-an-end-user-via-api}
...
#### Considérations importantes relatives à l’autorisation déléguée {#important-considerations-for-delegated-authorization}
...
#### Considérations importantes concernant la délégation de session {#important-considerations-for-session-delegation}
...
### Valider les JWTs signés avec des clés asymétriques {#validate-jwts-signed-with-asymmetric-keys}
...
### Valider les JWT signés avec des clés symétriques {#validate-jwts-signed-with-symmetric-keys}
...
### Valider un jeton opaque avec un service externe {#validate-opaque-token-with-an-external-service}
