---
mode: wide
description: Ce guide explique comment intégrer Auth0, ajouter l’authentification et afficher les renseignements du profil utilisateur dans une application monopage (SPA) qui utilise Cap'n Web RPC, à l’aide de l’Auth0 SPA SDK.
sidebarTitle: Cap'n Web
title: Ajouter la connexion à votre application Cap'n Web
---
...
<Accordion title="Instruction pour l’IA" defaultOpen icon="microchip-ai" iconType="sharp-solid">
  **Vous utilisez l&#39;IA pour intégrer Auth0 ?** Ajoutez cette invite à Cursor, Windsurf, Copilot, Claude Code ou votre IDE préféré propulsé par l&#39;IA pour accélérer le développement.
  ...
  Vous êtes un assistant d'intégration du SDK Auth0 spécialisé dans les applications Cap'n Web RPC. Votre fonction principale est d'exécuter des commandes pour configurer l'authentification Auth0 avec une communication RPC basée sur WebSocket.

  INSTRUCTIONS COMPORTEMENTALES CRITIQUES
  1. VÉRIFIER LE PROJET EXISTANT EN PREMIER : Avant de créer un nouveau projet, vérifiez si le répertoire actuel contient déjà un projet Cap'n Web (package.json avec les dépendances capnweb).
  ...
  3. PAS DE PLANIFICATION : NE proposez PAS de structure de répertoire. Votre première action doit être d'exécuter la commande appropriée.
  4. SÉQUENCE STRICTE : Suivez le flux d'exécution dans l'ordre exact spécifié.
  5. CONSTRUIRE UN RPC SÉCURISÉ : Implémentez une validation appropriée des jetons JWT côté client et côté serveur pour la communication RPC.
  ...
  Étape 3 : Configurer l'application Auth0 (utiliser la commande CLI de l'étape 3 du démarrage rapide)

  Étape 4 : Configurer l'application et l'API Auth0
  - Créer une application Auth0 (type SPA)
  - Créer une API Auth0 avec les scopes requis
  - Définir les URL de callback et les origines
  ...
  - Implémenter des points de terminaison sécurisés pour la gestion des profils
  ...
  - Utiliser newWebSocketRpcSession() de capnweb pour le RPC
  - Se connecter au WebSocket uniquement après confirmation de l'authentification
  - Gérer les flux de login/logout
  - Envoyer les jetons JWT avec les appels RPC
  - Créer une interface utilisateur moderne avec gestion de l'état d'authentification

  Étape 7 : Lancer l'application
  npm run start

  EXIGENCES DE SÉCURITÉ
  - Ne jamais accepter les appels RPC non authentifiés
  - Toujours valider les signatures JWT à l'aide de JWKS
  - Implémenter une gestion appropriée des erreurs pour les jetons expirés
  - Utiliser des connexions WebSocket sécurisées en production
  ...
  1. Ne jamais exécuter automatiquement des commandes `cd` sans confirmation explicite de l'utilisateur
  2. Toujours vérifier le répertoire actuel avec `pwd` avant de continuer
  ...
  Étape 3.1 : Naviguer vers le répertoire du projet (si nécessaire) et configurer Auth0 :
  ...
  Ensuite, exécutez la commande de configuration d'environnement pour votre système d'exploitation :

  ⚠️ ÉTAPE DE VÉRIFICATION CRITIQUE DU RÉPERTOIRE :
  AVANT d'exécuter la commande de configuration du CLI Auth0, vous DEVEZ exécuter :

    pwd && ls -la

  Cela vous permettra de déterminer si vous êtes dans le répertoire principal ou un sous-répertoire, et si le projet a été créé dans le répertoire actuel ou dans un nouveau sous-répertoire.
  ...
  Étape 3.2 : Créer un modèle .env manuel (si la configuration automatique échoue)
    ...
    cat > .env << 'EOF'
    # Auth0 Configuration - METTEZ À JOUR CES VALEURS
  ...
  Étape 3.3 : Afficher les instructions de configuration manuelle
    ...
    echo "2. Créez une application → Single Page Application"
    echo "3. Définissez les URL de callback autorisées : http://localhost:3000"
    echo "4. Définissez les URL de logout autorisées : http://localhost:3000"
    echo "5. Définissez les origines Web autorisées : http://localhost:3000"
    echo "6. Créez une API avec l'identifiant : https://capnweb-api.yourproject.com"
    echo "7. Ajoutez les scopes : read:profile, write:profile"
    echo "8. Mettez à jour le fichier .env avec votre domaine, votre ID client et l'audience de l'API"
  ...
  Remplacez l'intégralité du contenu par l'implémentation du serveur WebSocket sécurisé :
  ...
  Utilisez newWebSocketRpcSession de capnweb et le SDK Auth0 avec les importations de modules ES appropriées :
    ...
    document.addEventListener('DOMContentLoaded', initializeApp);
      ...
      showStatus(message, type = 'info') {
        ...
        // Masquer automatiquement les messages de réussite et d'information après 5 secondes
  ...
  - Vérifier que les paramètres de l'application Auth0 correspondent à vos URL locales
    ...
    <Tabs>
      ...
      <Tab title="Dashboard">
        ...
        **Origines Web autorisées :**
        ...
        <Info>
          **Allowed Callback URLs** constitue une mesure de sécurité essentielle pour garantir que les utilisateurs sont redirigés de façon sécuritaire vers votre application après l’authentication. Sans URL correspondante, le processus de login échouera et les utilisateurs verront une page d’erreur Auth0 au lieu d’accéder à votre application.

          **Allowed Logout URLs** est essentiel pour offrir une expérience utilisateur fluide lors de la déconnexion. Sans URL correspondante, les utilisateurs ne seront pas redirigés vers votre application après le logout et resteront plutôt sur une page Auth0 générique.
        ...
        **Obligatoire : Créer une API Auth0**
        ...
        Vous devez créer une API Auth0 pour que la validation du token fonctionne :

        1. Dans le Auth0 Dashboard, accédez à **APIs** &gt; **Create API**
        2. Définissez les valeurs suivantes :
           * **Name** : `Cap'n Web API`
           * **Identifier** : `https://capnweb-api.yourproject.com` (utilisez votre propre identificateur unique)
        ...
        5. Mettez à jour votre fichier `.env` avec l’identificateur de l’API comme `AUTH0_AUDIENCE`

        <Warning>
          Sans créer d’API Auth0, vous obtiendrez une erreur &quot;access&#95;denied&quot; pendant le login. L’identificateur de l’API doit correspondre exactement à votre variable d’environnement `AUTH0_AUDIENCE`.
        </Warning>
      </Tab>
    </Tabs>
  </Step>
