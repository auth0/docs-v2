...
<Steps>
  ...
  <Step title="Configurer les identifiants de l’application" stepNumber={4}>
    ...
    <Warning>
      Ne codez jamais en dur les valeurs `clientId` ou `domain` directement dans les fichiers source Swift — elles se retrouveraient dans le contrôle de version. Lisez-les plutôt à l’exécution à partir de variables d’environnement ou d’un fichier de configuration généré au moment de la compilation. Pour la plupart des applications, privilégiez `Auth0.plist` (l’approche par défaut dans ce Quickstart).
    </Warning>
...
## Dépannage et avancé {#troubleshooting-advanced}

<Accordion title="Problèmes courants et solutions">
  ### Erreurs de build : module « Auth0 » introuvable {#build-errors-auth0-module-not-found}
  ...
  **Solutions** :
  ...
  ### Plantage de l’application : « Auth0.plist not found » {#app-crashes-auth0plist-not-found}
  ...
  **Correctif** :
  ...
  ### Le navigateur s’ouvre, mais ne revient jamais à l’application {#browser-opens-but-never-returns-to-app}
  ...
  **Correctif** :
  ...
  ### L’alerte d’autorisation apparaît chaque fois {#permission-alert-appears-every-time}
...
<Accordion title="Intégration avancée">
  ### Sécurité renforcée du Keychain avec la biométrie {#enhanced-keychain-security-with-biometrics}
  ...
  ### Actualisation automatique des jetons {#automatic-token-refresh}
  ...
  ### Informations d’identification partagées entre les extensions d’app {#shared-credentials-across-app-extensions}
  ...
  **Exigences :**
  ...
  ### Comparaison des options de flux d’authentification {#authentication-flow-options-comparison}
