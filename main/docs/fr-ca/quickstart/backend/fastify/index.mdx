---
mode: wide
description: Ce guide explique comment protéger des points de terminaison d’API Fastify à l’aide de jetons d’accès JWT avec le SDK d’API Fastify d’Auth0.
...
<Accordion title="Utilisez l’IA pour intégrer Auth0" icon="microchip-ai" iconType="solid" defaultOpen>
  Si vous utilisez un assistant de codage IA comme Claude Code, Cursor ou GitHub Copilot, vous pouvez ajouter automatiquement l’authentification API avec Auth0 en quelques minutes à l’aide d’[Agent Skills](https://agentskills.io/home).
  ...
  **Demandez ensuite à votre assistant IA :**
  ...
  Votre assistant IA créera automatiquement votre API Auth0, récupérera les informations d’identification, installera `@auth0/auth0-fastify-api`, configurera le plugin et protégera vos points de terminaison d’API à l’aide de la validation des JWT. [Documentation complète sur Agent Skills →](/docs/fr-ca/quickstart/agent-skills)
</Accordion>
...
<Note>
  ...
  **Compatibilité avec les versions de Fastify :** Ce guide de démarrage rapide fonctionne avec **Fastify 5.x** et les versions ultérieures.
</Note>

## Pour commencer {#get-started}

Ce guide de démarrage rapide montre comment protéger des points de terminaison d’API Fastify à l’aide de jetons d’accès JWT. Vous créerez une API sécurisée qui valide les jetons d’accès Auth0 et donne accès à des ressources protégées.
...
<Steps>
    ...
    <Tabs>
      ...
      <Tab title="Dashboard">
        ...
        4. Définissez l’**Identifier** (p. ex., `https://my-fastify-api.example.com`)
           ...
           * Il n’est pas nécessaire que ce soit une vraie URL — c’est simplement un identifiant
  ...
  <Step title="Configurer le plugin de l’API Auth0" stepNumber={4}>
    Créez votre serveur Fastify et enregistrez le plugin de l’Auth0 API :
    ...
    * Met le `preHandler` `requireAuth()` à disposition pour protéger les routes
  </Step>
  ...
  <Step title="Créer des routes d’API" stepNumber={5}>
    ...
    * `request.user` contient les claims décodés du JWT pour les requêtes authentifiées
    * Le champ `sub` contient l’identifiant unique de l’utilisateur
  </Step>
  ...
  <Step title="Testez votre API" stepNumber={7}>
    ...
    1. Créer une application cliente (application Web ou mobile) qui permet aux utilisateurs de s’authentifier
...
<Check>
  **Vérification**
  ...
  4. Fournit des renseignements sur l’utilisateur à partir des claims du jeton via `request.user`
</Check>
...
<Accordion title="Claims de jeton personnalisés avec TypeScript">
  Étendez l’interface Token pour ajouter un typage sûr à vos claims personnalisés dans vos jetons d’accès :

  ```typescript server.ts
  import '@auth0/auth0-fastify-api';

  // Étendez l’interface Token avec vos claims personnalisés
  ...
  TypeScript reconnaîtra maintenant vos claims personnalisés :
  ...
  <Note>
    Les claims personnalisés doivent utiliser des URL avec espace de noms (par ex., `https://myapp.com/roles`), sauf s’il s’agit de claims OIDC standards. [En savoir plus sur les claims personnalisés](/docs/fr-ca/secure/tokens/json-web-tokens/create-custom-claims).
  </Note>
</Accordion>
...
<Accordion title="Autorisation basée sur les permissions">
  ...
  <Note>
    Les permissions doivent être configurées dans les paramètres de votre Auth0 API et accordées aux clients. [En savoir plus sur les permissions d’API](/docs/fr-ca/manage-users/access-control/configure-core-rbac/rbac-users/assign-permissions-to-users).
  </Note>
</Accordion>
...
<Accordion title="Autorisation basée sur les rôles">
  ...
  // Route accessible au gestionnaire ou à l’admin
  ...
  <Note>
    Les rôles doivent être ajoutés aux jetons à l’aide d’Auth0 Actions. [Découvrez comment ajouter des rôles aux jetons](/docs/fr-ca/manage-users/access-control/sample-use-cases-actions-with-authorization#add-user-roles-to-tokens).
  </Note>
</Accordion>
...
<Accordion title="Gestion des erreurs">
  ...
  fastify.setErrorHandler((error, request, reply) => {
    ...
    // Gérer les erreurs de permission ou de rôle
  ...
  // Gestionnaire pour les routes introuvables
...
### &quot;Aucun jeton d’autorisation n’a été trouvé&quot; {#no-authorization-token-was-found}
...
### &quot;Jeton invalide&quot; ou &quot;jwt malformed&quot; {#invalid-token-or-jwt-malformed}
...
### &quot;Audience invalide&quot; {#invalid-audience}
    ...
    **Solution :** L’application cliente doit demander un jeton avec la bonne audience :
    ...
    const token = await getAccessTokenSilently({
      authorizationParams: {
        audience: 'https://my-fastify-api.example.com' // Doit correspondre à votre identifiant d’API
      }
    });
    ```

### Erreurs CORS dans le navigateur {#cors-errors-in-browser}

    **Problème :** Le navigateur bloque les requêtes API à cause de la politique CORS.
...
Maintenant que vous disposez d’une API protégée, vous pourriez explorer :

* **[Quickstart d’application web Fastify](/docs/fr-ca/quickstart/webapp/fastify)** - Créez une application web qui effectue des requêtes à votre API
* **[Contrôle d’accès basé sur les rôles](/docs/fr-ca/manage-users/access-control/rbac)** - Implémentez des permissions granulaires
* **[Meilleures pratiques en matière d’autorisation des API](/docs/fr-ca/secure/tokens/access-tokens)** - Découvrez les meilleures pratiques relatives aux jetons d’accès
* **[Surveillez votre API](/docs/fr-ca/deploy-monitor/logs)** - Configurez la journalisation et la surveillance
...
* **[Authentification de l’API Auth0](/docs/fr-ca/secure/tokens/access-tokens)** - Comprendre les jetons d’accès
