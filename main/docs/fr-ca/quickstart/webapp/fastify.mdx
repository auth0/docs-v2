---
title: Ajouter le login à votre application Fastify
sidebarTitle: Fastify
description: Ce guide montre comment intégrer Auth0, ajouter l’authentification et afficher les renseignements du profil utilisateur dans une application web Fastify à l’aide de l’Auth0 Fastify SDK.
mode: wide
---
...
<Accordion title="Utilisez l’IA pour intégrer Auth0" icon="microchip-ai" iconType="solid" defaultOpen>
  ...
  **Demandez ensuite à votre assistant IA :**
  ...
  Votre assistant IA créera automatiquement votre application Auth0, récupérera les identifiants, installera `@auth0/auth0-fastify`, configurera le plugin et créera toutes les routes et vues nécessaires. [Documentation complète sur Agent Skills →](/docs/fr-ca/quickstart/agent-skills)
</Accordion>
...
<Steps>
  ...
  <Step title="Installer le SDK Auth0 Fastify" stepNumber={2}>
  ...
  <Step title="Configurez votre Auth0 App" stepNumber={3}>
    ...
    <Tabs>
      ...
      <Tab title="Dashboard">
        ...
        3. Saisissez un nom pour votre application (p. ex., &quot;My Fastify App&quot;)
        ...
        7. Copiez les valeurs **Domain**, **Client ID** et **Client Secret** de la section **Basic Information**
        ...
        <Warning>
          Remplacez `YOUR_AUTH0_DOMAIN` par le domaine de votre tenant Auth0 (p. ex., `dev-abc123.us.auth0.com`), `YOUR_CLIENT_ID` par le Client ID de votre application et `YOUR_CLIENT_SECRET` par le Client Secret de votre application dans le dashboard.
        </Warning>
        ...
        Copiez la valeur générée et utilisez-la comme valeur de `SESSION_SECRET` dans votre fichier `.env`.
      </Tab>
    </Tabs>
  ...
  <Step title="Créer des modèles d’affichage" stepNumber={5}>
    ...
    <head>
      <meta charset="UTF-8">
      <meta name="viewport" content="width=device-width, initial-scale=1.0">
      <title>Profil - Auth0 Fastify</title>
      <style>
        body {
          font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;
          background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);
    ...
    <body>
      <div class="container">
        ...
        <div class="profile-card">
          <img src="<%= user.picture || 'https://via.placeholder.com/100' %>" alt="Profil" class="profile-picture">
          <div class="profile-info">
            <h2><%= user.name || user.nickname || 'User' %></h2>
            <p><strong>Email:</strong> <%= user.email || 'N/A' %></p>
          </div>
        </div>
        <h3>Objet `user` complet</h3>
        <div class="user-data">
          <pre><%= JSON.stringify(user, null, 2) %></pre>
        </div>
        <div style="margin-top: 2rem;">
          <a href="/" class="button button-primary">← Retour à l'accueil</a>
          <a href="/auth/logout" class="button button-secondary">Déconnexion</a>
        </div>
      </div>
    </body>
    </html>
...
<Accordion title="Routes personnalisées">
...
<Accordion title="Liaison de comptes">
  ...
  * `/auth/unconnect/callback` - Gérer le rappel de dissociation
  ...
  <div>
    <a href="/auth/connect?connection=google-oauth2">Lier un compte Google</a>
    <a href="/auth/unconnect?connection=google-oauth2">Dissocier un compte Google</a>
  </div>
  ```
</Accordion>
...
### Erreur « invalid state » après la connexion {#invalid-state-error-after-login}
    ...
    **Problème :** Le paramètre `state` de la requête d’authentification ne correspond pas à celui du callback.
...
### Erreur « session is undefined » {#session-is-undefined-error}
...
### Les variables d’environnement ne se chargent pas {#environment-variables-not-loading}
    ...
    **Solution :**
    ...
    ```javascript
    // Débogage : consigner les valeurs de configuration (à supprimer en production !)
...
* **[Authentification API Fastify](/docs/fr-ca/quickstart/backend/fastify)** - Protégez vos endpoints d’API grâce à la validation JWT
...
* **[Mettre en place la MFA](https://auth0.com/docs/secure/multi-factor-authentication)** - Ajoutez l’authentification multifacteur
...
* **[Fastify Documentation](https://fastify.dev/)** - En savoir plus sur Fastify
