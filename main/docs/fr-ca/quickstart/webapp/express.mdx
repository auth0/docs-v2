---
mode: wide
description: Ce guide montre comment intégrer Auth0, ajouter l’authentification et afficher les renseignements du profil utilisateur dans une application web Express.js à l’aide du SDK express-openid-connect.
sidebarTitle: Express
title: Ajouter l’authentification à votre application Express
---
...
<Callout icon="pencil" color="#FFC107" iconType="solid">
  Une nouvelle version **bêta** de ce Quickstart est disponible et utilise le SDK `@auth0/auth0-express`, qui remplacera bientôt ce guide. [Essayer le Quickstart bêta →](/docs/fr-ca/quickstart/webapp/express-beta)
</Callout>
...
<Note>
  ...
  * [jq](https://jqlang.github.io/jq/) - Requis pour la configuration d’Auth0 CLI (facultatif)
...
## 1. Créez un nouveau projet {#1-create-a-new-project}
...
## 2. Installer le SDK Auth0 Express {#2-install-the-auth0-express-sdk}
...
## 3. Configurez votre Auth0 App {#3-setup-your-auth0-app}
...
<Tabs>
  <Tab title="CLI">
    ...
    <CodeGroup>
      ```shellscript Mac
      # Installez l’Auth0 CLI (si ce n’est pas déjà fait)
  ...
  <Tab title="Dashboard">
    ...
    <Warning>
      Remplacez `YOUR_AUTH0_DOMAIN` par le domaine de votre tenant Auth0 (par exemple, `dev-abc123.us.auth0.com`) et `YOUR_CLIENT_ID` par le Client ID de votre application dans le Dashboard.
    </Warning>
...
## 4. Configurer le middleware {#4-configure-the-middleware}

Ajoutez le middleware Auth0 à votre application Express. Le middleware `auth()` gère la session et crée automatiquement les routes `/login`, `/logout` et `/callback`.
...
## 5. Créez des routes de login, de logout et de profil {#5-create-login-logout-and-profile-routes}

Ajoutez maintenant des routes pour afficher des liens de login et de logout, ainsi qu’une page de profil protégée.
...
## 6. Lancez votre application {#6-run-your-app}
...
<Check>
  **Checkpoint**
  ...
  Vous devriez maintenant avoir une page de connexion d’Auth0 entièrement fonctionnelle. Lorsque vous :
...
<AccordionGroup>
  <Accordion title="Protéger des routes précises avec requiresAuth()">
  ...
  <Accordion title="Appel d’API protégées avec des jetons d’accès">
  ...
  <Accordion title="Utilisation de l’autorisation fondée sur les revendications">
    ...
    // Vérification de revendication personnalisée avec logique conditionnelle
    ...
    <Note>
      Des claims comme `role` doivent être ajoutées à vos jetons au moyen d’Auth0 Rules ou d’Actions. [En savoir plus sur l’ajout de claims personnalisés](/docs/fr-ca/manage-users/access-control/sample-use-cases-actions-with-authorization#add-user-roles-to-tokens).
    </Note>
  </Accordion>
...
### Erreur « invalid state » après la connexion {#invalid-state-error-after-login}

    **Problème :** La valeur de `state` ne correspond pas entre la requête d&#39;authentification et le rappel.
...
### `req.oidc` est `undefined` {#reqoidc-is-undefined}
...
### Session trop volumineuse / erreurs liées aux cookies {#session-too-large-cookie-errors}

    **Problème :** Les données de session de l&#39;utilisateur dépassent la taille maximale des cookies.

    **Solution :** Utilisez un magasin de sessions personnalisé comme Redis :
...
### Incompatibilité de l&#39;URL de rappel {#callback-url-mismatch}
...
### Les variables d&#39;environnement ne se chargent pas {#environment-variables-not-loading}
    ...
    **Solution :**
    ...
    ```javascript
    // Débogage : affichez les valeurs de configuration (à supprimer en production !)
...
* **[Ajouter l’autorisation](/docs/fr-ca/manage-users/access-control/rbac)** - Implémentez le contrôle d’accès basé sur les rôles
* **[Appeler des API protégées](/docs/fr-ca/secure/tokens/access-tokens/get-access-tokens)** - Utilisez des jetons d’accès pour appeler vos API côté serveur
* **[Personnaliser Universal Login](/docs/fr-ca/customize/universal-login-pages)** - Adaptez l’image de marque de votre expérience de connexion
* **[Ajouter des connexions sociales](/docs/fr-ca/connections/social)** - Activez Google, GitHub et d’autres options de connexion sociale
* **[Implémenter MFA](/docs/fr-ca/secure/multi-factor-authentication)** - Ajoutez l’authentification multifacteur
