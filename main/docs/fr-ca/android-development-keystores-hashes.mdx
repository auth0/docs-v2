...
Lors de la création d’un nouveau justificatif OAuth pour plusieurs connexions, vous devrez fournir le SHA-1 du certificat que vous utilisez pour signer votre application. Lorsque vous complétez la configuration de votre application dans le [Auth0 Dashboard](%24%7Bmanage_url%7D), vous devez également fournir la valeur SHA-256.
...
Une fois que vous avez obtenu vos empreintes de clés, copiez la valeur SHA256 résultante et allez dans les paramètres de votre application dans le [Auth0 Dashboard](%24%7Bmanage_url%7D/#/applications). Cliquez sur « Afficher les paramètres avancés », et dans l’onglet « Configurations de l’appareil », sous « Android », remplissez le champ « Nom du package de l’application » avec le nom du paquet de votre application, et le champ « Empreintes de clés » avec la valeur SHA256 que vous avez copiée. N’oubliez pas d’enregistrer les modifications.
