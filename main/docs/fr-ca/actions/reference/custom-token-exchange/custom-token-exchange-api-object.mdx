...
### `api.access.rejectInvalidSubjectToken(reason)` {#apiaccessrejectinvalidsubjecttokenreason}
...
### `api.authentication.setUserById(user_id)` {#apiauthenticationsetuserbyiduser_id}
...
### `api.authentication.setUserByConnection(connection_name, user_attributes, options)` {#apiauthenticationsetuserbyconnectionconnection_name-user_attributes-options}
...
### `api.authentication.setOrganization(organization_id_or_name)` {#apiauthenticationsetorganizationorganization_id_or_name}
...
### `api.authentication.setActor(actor)` {#apiauthenticationsetactoractor}
...
```js
exports.onExecuteCustomTokenExchange = async (event, api) => {
  // Stocker des données à partager entre les Actions pendant toute la durée de la transaction.
  api.transaction.setMetadata('subject_verified', true);
  api.transaction.setMetadata('risk_score', 42);

  // Les relire depuis l'événement en temps réel.
  console.log(event.transaction.metadata.risk_score); // 42

  // Supprimer une propriété définie précédemment en passant `null`.
...
<Expandable title="Paramètres" defaultOpen>
  ...
  <ParamField body="value" type="object">
    La valeur de la propriété. Elle peut être réglée à `null` pour retirer la
