...
#
description: Découvrez l’objet API du déclencheur event-stream des Actions.
...
L’objet API du déclencheur event-stream des Actions comprend :

## `api.cache` {#apicache}

Stockez et récupérez des données conservées entre les exécutions.
...
`ttl` ou `expires_at` spécifiées. Si aucune durée de vie n’est précisée, une durée de vie
