...
### Autoriser l’accès à une application précise uniquement les jours de semaine {#allow-access-only-on-weekdays-for-a-specific-application}
...
### Rejeter l’accès à toute personne qui appelle une API {#deny-access-to-anyone-calling-an-api}
...
### Ajouter les rôles utilisateur aux jetons d’ID et d’accès {#add-user-roles-to-id-and-access-tokens}
...
### Enrichir le profil utilisateur {#enrich-the-user-profile}
...
### Appliquer une politique MFA personnalisée {#enforce-custom-mfa-policy}
...
### Réduire les frictions grâce aux passkeys {#reduce-friction-with-passkeys}
...
### Rediriger l’utilisateur vers un site externe {#redirect-the-user-to-an-external-site}
...
### Modifier les portées d’un jeton d’accès {#modify-scopes-on-an-access-token}
...
### Rejeter l’accès en fonction d’empreintes JA3/JA4 précises {#deny-access-to-specific-ja3ja4-fingerprints}
