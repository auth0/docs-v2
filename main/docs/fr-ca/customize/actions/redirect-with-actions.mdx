...
## Lancer une redirection {#start-a-redirect}
...
## Reprendre le flux d’authentification {#resume-the-authentication-flow}
...
## Transmettre des données au site externe {#pass-data-to-the-external-site}
...
### Assurez-vous que le jeton n’a pas été altéré {#ensure-the-token-has-not-been-tampered-with}
...
## Transmettre des données à Auth0 {#pass-data-back-to-auth0}
...
### Utilisez les métadonnées d’application dans la mesure du possible {#use-app-metadata-where-possible}
...
### Faites preuve de discernement lorsque vous stockez des données dans le profil utilisateur Auth0 {#be-selective-when-storing-data-on-the-auth0-user-profile}
...
## Envoyer des données sur le canal frontal {#send-data-on-the-front-channel}
...
### Transmettre des informations à l’Action {#pass-information-back-to-the-action}
...
## Méthodes d’authentification personnalisées {#custom-authentication-methods}
...
## Restrictions et limites {#restrictions-and-limitations}
...
### Point de terminaison Resource Owner {#resource-owner-endpoint}
...
### Flux où `prompt=none` {#flows-where-promptnone}
