...
### Copier le code d’une Rule dans une nouvelle Action {#copy-rule-code-to-a-new-action}
...
### Modifier la façon d’accéder aux données utilisateur {#change-how-user-data-is-accessed}
...
### Modifier la façon d’accéder aux données de contexte {#change-how-context-data-is-accessed}
...
### Modifier la gestion des secrets {#change-handling-of-secrets}
...
### Convertir les claims personnalisés dans les jetons {#convert-custom-claims-in-tokens}
...
### Convertir le déclenchement de l’authentification multifacteur {#convert-multi-factor-triggering}
...
### Convertir les mises à jour des métadonnées de l’utilisateur {#convert-user-metadata-updates}
...
### Convertir d’autres appels à la Management API {#convert-other-management-api-calls}
...
### Convertir les redirections {#convert-redirects}
...
### Convertir les références aux clients SSO actuels {#convert-current-sso-clients-references}
...
## Terminer la migration {#complete-the-migration}
