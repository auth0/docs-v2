...
* Chaque exécution d’un flux doit se terminer en 20 secondes ou moins, sinon le traitement se terminera par une erreur. Limiter les processus de longue durée, comme les requêtes HTTP sortantes sans délai d’attente, est nécessaire pour respecter ce délai. Une Action qui [redirige les utilisateurs vers une page externe](/signup-and-login-triggers/login-trigger/redirect-with-actions) a un délai d’attente distinct avant et après la redirection.
...
* Un maximum de 100 attributs SAML peut être modifié ou ajouté par des Actions.
