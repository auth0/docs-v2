---
description: Examinez les contraintes qui s’appliquent à Auth0 Actions et Auth0 Modules afin de pouvoir concevoir du code personnalisé qui respecte les limites imposées.
...
## Modules Actions {#actions-modules}
...
## Liaison de comptes (setPrimaryUser) {#account-linking-setprimaryuser}
...
## Données mises en cache {#cached-data}
...
## Attributs SAML {#saml-attributes}
...
## Configuration de SAML {#saml-configuration}
...
## Requêtes de service {#service-requests}
...
## Métadonnées de transaction {#transaction-metadata}
...
## Jetons provenant d’IdP externes {#upstream-tokens-from-external-idps}
...
## Métadonnées utilisateur et métadonnées d’application {#user-metadata-and-app-metadata}
