---
description: Le package **Actions NPM (`@auth0/actions`)** permet d’utiliser TypeScript dans des projets externes, ce qui permet aux développeurs de suivre les pratiques exemplaires et d’améliorer leurs **tests unitaires** en s’appuyant sur les définitions TypeScript.
...
## Contrôle d’accès et configuration des métadonnées utilisateur pour Pre-User Registration {#pre-user-registration-access-control-and-user-metadata-setup}
...
## Fournisseur de courriel personnalisé et requête HTTP {#custom-email-provider-and-http-request}
