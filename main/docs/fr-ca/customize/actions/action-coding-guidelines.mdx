...
* Utilisez les [métadonnées d’application](/applications/configure-application-metadata) pour filtrer les applications spécifiques afin de déterminer si une Action doit être exécutée.
...
* Utilisez `event.request.hostname` pour le domaine utilisé dans les appels d’Authentication API; il peut s’agir du domaine de locataire Auth0 par défaut ou d’un [domaine personnalisé](/custom-domains).
