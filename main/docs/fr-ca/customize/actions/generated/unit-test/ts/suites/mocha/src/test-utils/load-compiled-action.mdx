 ...
 * l'exécute au moyen de vm.compileFunction : il ne passe donc jamais par la transformation
 * TS propre à ts-node/Vitest. Les sources des actions (et tout module enregistré avec
 * actions:) doivent d'abord être transpilées en JS pur sur le disque.
 */
