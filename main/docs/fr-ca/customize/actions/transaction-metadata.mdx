---
title: "Métadonnées de transaction d'actions"
'description': "Describes how to pass user or application metadata between login and post-login Actions."
---
Actions Transaction Metadata stores, accesses, and/or shares, custom metadata within a [post-login](/docs/fr-ca/customize/actions/explore-triggers/signup-and-login-triggers/login-trigger) Action for the duration of a transaction.
...
Before using transaction metadata in development, we recommend reviewing the limitations. To learn more, read [Actions Limitations](/docs/fr-ca/customize/actions/limitations).

</Note>

## How it works

Use the [post-login API object](/docs/fr-ca/customize/actions/explore-triggers/signup-and-login-triggers/login-trigger/post-login-api-object) `api.transaction.setMetadata` to set the key/value pair in order to store transaction metadata.

Use the [post-login Event object](/docs/fr-ca/customize/actions/explore-triggers/signup-and-login-triggers/login-trigger/post-login-event-object) `event.transaction.metadata` to access stored key/value pair in the same Action or subsequent Actions on the post-login trigger for a single execution.
...
To learn more about writing Actions, read [Write Your First Action](/docs/fr-ca/customize/actions/write-your-first-action).
...
Preserve transaction metadata during [redirects](/docs/fr-ca/customize/actions/explore-triggers/signup-and-login-triggers/login-trigger/redirect-with-actions). Values are available when users continue the authentication flow.
...
[Render Forms using Actions](/docs/fr-ca/customize/forms/render) and transaction metadata values would be preserved and available when users continue the authentication flow.

To learn more about using Forms with Actions, read [Render Forms using Actions](/docs/fr-ca/customize/forms/render).
