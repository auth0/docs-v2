...
* **locale**: Locale (tel que déduit de l'URL)-- `https://${account.tenant}.us.webtask.io/auth0-delegated-admin/en/user` définira **locale** à `en`.
...
Pour en savoir plus sur les hooks d’écriture, veuillez consulter [Administration déléguée : Appel d’écriture](/extensions/delegated-administration-extension/delegated-administration-write-hook).
...
Pour préciser les paramètres régionaux, vous pouvez utiliser le chemin. Par exemple : `https://${account.tenant}.us.webtask.io/auth0-delegated-admin/en/users` définira `context.locale` sur `en` dans la requête de paramètres.
