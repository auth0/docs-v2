---
description: Décrit comment personnaliser l’écran de consentement présenté aux utilisateurs lors du processus d’autorisation.
...
Lorsqu’une application tierce demande des scopes, les utilisateurs voient un écran de consentement. Par défaut, cet écran utilise le **nom** du scope pour générer le texte et regroupe tous les scopes d’une ressource, en affichant les actions de la ressource sur une seule ligne.

Si vous avez besoin d’un écran de consentement particulier, par exemple pour le consentement parental, vous devez créer votre propre formulaire de consentement personnalisé. Gardez à l’esprit que les lois varient d’un pays à l’autre.

Par exemple, supposons que vous ayez une API enregistrée dans Auth0 avec les scopes définis suivants :
...
Vous pouvez plutôt utiliser la **description** du scope que vous avez défini pour générer ce texte. Dans ce cas, la boîte de dialogue de consentement afficherait : **Pouvoir lire vos courriels**, **Écrire des messages**.

Cette modification se fait au niveau du tenant, donc elle touchera les écrans de consentement pour toutes les API du tenant.

## Utilisez les descriptions des scopes pour générer le texte de l’écran de consentement {#use-scope-descriptions-to-generate-consent-prompt-text}

Définissez l’indicateur **use&#95;scope&#95;descriptions&#95;for&#95;consent** de votre tenant à `true` en effectuant l’appel d’API suivant :
...
## Utiliser les noms de scope pour générer le texte de l’écran de consentement {#use-scope-names-to-generate-consent-prompt-text}

Définissez l’indicateur **use&#95;scope&#95;descriptions&#95;for&#95;consent** de votre tenant à `false` en effectuant l’appel d’API suivant :
...
* [Scopes d’OpenID Connect](/docs/fr-ca/get-started/apis/scopes/openid-connect-scopes)
