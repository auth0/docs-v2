...
Vous pouvez éviter l’inclusion des adresses courriel dans les liens de redirection en mettant à jour les modèles de courriel via Management API. Par exemple :
...
* Si l’utilisateur s’est connecté via une [Organization](/organizations/organizations-overview) :
...
Pour localiser les courriels, vous pouvez définir la variable `locale` à envoyer dans `user_metadata` lors de la création d’une invitation. Pour en savoir plus, consultez [Personnaliser les modèles de courriel : Modèles de courriels multilingues](/brand-and-customize/email/customize-email-templates).
