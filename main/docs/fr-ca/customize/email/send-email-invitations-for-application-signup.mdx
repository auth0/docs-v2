...
* [Personnaliser un modèle de courriel](/brand-and-customize/email/customize-email-templates) et l’utiliser pour [envoyer un courriel de modification du mot de passe](/brand-and-customize/email/manage-email-flow).
* Créer un ticket de modification de mot de passe.

Vous pouvez autoriser un utilisateur à accéder à un compte existant que vous avez créé en son nom. Ensuite, envoyez à l’utilisateur un lien unique pour qu’il puisse définir son mot de passe. Vous générez le lien unique en créant un ticket de modification de mot de passe dans lequel votre application d’invitation appelle le point de terminaison `/password-change`[Management API](/api/management/v2/#!/Tickets/post_password_change). Vous aurez besoin de :

* [Créer un utilisateur de la base de données Auth0](/connections/database) avec le paramètre `user.email_verified` défini sur `false`. Vous pouvez utiliser le point de terminaison [Créer un utilisateur](/api/management/v2/#!/Users/post_users).
* Avoir accès à un [service de courriel externe et le configurer](/brand-and-customize/email/smtp-email-providers).
* [Obtenir un jeton d’accès à Management API](/security/tokens/access-tokens/management-api-access-tokens/get-management-api-access-tokens-for-testing).
...
Pour en savoir plus, consultez [Configuration des itinéraires de connexion par défaut](/login/universal-login/configure-default-login-routes).
