...
## Fonctionnement de l’Events API {#how-the-events-api-works}
...
### Exemple de flux SSE {#example-sse-stream}
...
## Se connecter à l’API Events {#connect-to-the-events-api}
...
## Reprendre après une déconnexion {#resume-after-a-disconnection}
...
## Gérer les types de messages {#handle-message-types}

### Événements réels {#real-events}
...
### Messages `offset-only` {#offset-only-messages}
...
## Rotation des connexions côté serveur {#server-side-connection-cycling}
...
## Mettre en place un consommateur {#implement-a-consumer}
