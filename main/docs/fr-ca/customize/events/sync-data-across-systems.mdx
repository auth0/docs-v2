---
description: Gardez une base de données externe ou un entrepôt de données synchronisé avec les changements d’identité dans Auth0 à l’aide d’Event Streams.
...
## Pourquoi synchroniser les données d’identité {#why-synchronize-identity-data}
...
* Un tenant Auth0 où Events est activé. Pour en savoir plus sur la disponibilité selon le forfait, consultez [Créer un Event Stream](/docs/fr-ca/customize/events/create-an-event-stream).
* Un Event Stream actif abonné à `user.created`, `user.updated` et `user.deleted`. Pour en savoir plus, consultez [Créer un Event Stream](/docs/fr-ca/customize/events/create-an-event-stream).
...
## Synchronisation des données avec Implement {#implement-data-synchronization}

Les sections ci-dessous montrent comment traiter chaque type d’événement. Les fonctions handler sont les mêmes, quelle que soit la destination de votre Event Stream. La section [Acheminer les événements par type](#route-events-by-type) montre comment acheminer les événements pour les destinations webhook et Auth0 Action.

### Traiter user.created {#handle-usercreated}
...
### Traiter user.updated {#handle-userupdated}
...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  Les Events peuvent ne pas arriver dans l’ordre. Comparez toujours les horodatages avant d’appliquer les mises à jour afin d’éviter que des données périmées n’écrasent des enregistrements plus récents. Pour en savoir plus, consultez [Events Best Practices](/docs/fr-ca/customize/events/events-best-practices).
</Callout>

### Traiter user.deleted {#handle-userdeleted}
...
### Acheminer les événements par type {#route-events-by-type}
...
<Tabs>
  <Tab title="Webhook">
    ...
    <Callout icon="file-lines" color="#0EA5E9" iconType="regular">
      Retournez une réponse HTTP `2XX` le plus rapidement possible. Si votre handler doit effectuer des opérations lentes, placez l’événement dans une file d’attente interne et traitez-le de façon asynchrone. Pour en savoir plus, consultez [Events Best Practices](/docs/fr-ca/customize/events/events-best-practices).
    </Callout>
  </Tab>
...
## Se prémunir contre les doublons et les problèmes d’ordre {#guard-against-duplicates-and-ordering-issues}
...
## Vérifiez la synchronisation {#verify-synchronization}
...
Pour en savoir plus sur la mise à l’essai des Event Streams, consultez [Tests des événements, observabilité et reprise après échec](/docs/fr-ca/customize/events/event-testing-observability-and-failure-recovery).

## En savoir plus {#learn-more}

* [Catalogue des événements](/docs/fr-ca/events)
