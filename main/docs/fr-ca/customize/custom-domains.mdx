...
| [Lock (Verrouiller)](/docs/glossary?term=lock) | Version 11 avec authentification inter-origines |
| [Sans mot de passe](/docs/glossary?term=passwordless) | Avec connexion universelle (le lien est envoyé par courriel à l’aide du domaine personnalisé si l’option est activée dans **Dashboard (Tableau de bord) &gt; Tenant Settings (Paramètres du locataire) &gt; Domaines personnalisés**.) |
| [SAML](/docs/glossary?term=security-assertion-markup-language) | Connexions et applications |
| WS-Federation | Auth0 en tant que fournisseur d’identité à l’aide du module complémentaire WS-Fed |
| Flux conformes OAuth 2.0/à l’OIDC | Utilisation des points de terminaison [`/authorize`](/docs/fr-ca/api/authentication#authorize-application) et [`/oauth/token`](/docs/fr-ca/api/authentication#get-token) |
