...
Vous configurez un domaine personnalisé dans l’onglet [Auth0 Dashboard > Branding (Image de marque) > Custom Domains (Domaines personnalisés)](%24%7Bmanage_url%7D/#/custom_domains) dans l’Auth0 Dashboard. Ajoutez votre domaine personnalisé, choisissez votre type de certification et suivez les instructions. Vous effectuerez une procédure de vérification pour votre domaine qui varie selon que vous utilisez un certificat géré par Auth0 ou un certificat autogéré. Lorsque vous créez un CNAME, vous devez le déclarer à Auth0 afin que ce dernier puisse le vérifier et utiliser le domaine personnalisé. Après avoir configuré et vérifié le domaine personnalisé, vous devez [configurer les fonctionnalités d’Auth0](/custom-domains/configure-features-to-use-custom-domains) pour utiliser le nouveau domaine personnalisé.
...
Pour en savoir plus, consultez [Rediriger les utilisateurs après la connexion](/login/redirect-users-after-login).

Cette flexibilité s’applique aux scénarios d’authentification suivants :

* [Configurer les applications avec OpenID Connect Discovery](/applications/configure-applications-with-oidc-discovery)
* [Configurer Auth0 en tant que fournisseur de services SAML](/saml-configuration/configure-auth0-saml-service-provider)
* [Configurer Auth0 en tant que fournisseur d'identité SAML](/saml-configuration/configure-auth0-saml-identity-provider)
