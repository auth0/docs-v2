...
| Valeur | Description |
| --- | --- |
| `MGMT_API_ACCESS_TOKEN` | [Jeton d’accès à Management API] (/api/management/v2/tokens) avec la <dfn>permission</dfn> `create:hooks`. |
| `HOOK_ID` | ID du crochet pour lequel vous souhaitez ajouter des secrets. |
| `HOOK_SECRET_KEY` | Nom du secret que vous souhaitez ajouter pour le crochet spécifié. Ce point de terminaison accepte un objet de paires clé-valeur. |
| `HOOK_SECRET_VALUE` | Valeur du secret que vous souhaitez ajouter au crochet spécifié. Ce point de terminaison accepte un objet de paires clé-valeur. |