...
| Valeur | Description |
| --- | --- |
| `MGMT_API_ACCESS_TOKEN` | Jeton d’accès à Management API avec la <dfn>permission</dfn> `update:hooks`. Pour en savoir plus, veuillez consulter [Jetons d’accès à Management API] (/api/gestion/v2/jetons). |
| `HOOK_ID` | ID du crochet pour lequel vous souhaitez mettre à jour des secrets. |
| `HOOK_SECRET_KEY` | Nom du secret que vous souhaitez mettre à jour pour le crochet spécifié. Ce point de terminaison accepte un objet de paires clé-valeur. |
| `HOOK_SECRET_VALUE` | Valeur du secret que vous souhaitez mettre à jour pour le crochet spécifié. Ce point de terminaison accepte un objet de paires clé-valeur. |
