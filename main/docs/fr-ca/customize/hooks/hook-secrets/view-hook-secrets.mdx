...
| Valeur | Description |
| --- | --- |
| `HOOK_ID` | ID du crochet pour lequel vous souhaitez récupérer des secrets. |
| `MGMT_API_ACCESS_TOKEN` | Jetons d’accès pour Management API avec la <dfn>permission</dfn> `update:hooks`. Pour en savoir plus, consultez [Jetons d’accès à Management API](/docs/fr-ca/api/management/v2/tokens). |
