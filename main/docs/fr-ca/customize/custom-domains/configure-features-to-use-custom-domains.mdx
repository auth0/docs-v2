...
Si vous utilisez [Connexion universelle Auth0.](/login/universal-login) et que vous avez personnalisé la page de connexion, vous devez mettre à jour le code pour utiliser votre domaine personnalisé. Si vous utilisez la page de connexion **par défaut** sans personnalisation, vous n’avez aucune modification à effectuer. Pour en savoir plus, consultez Connexion universelle Auth0.

Si vous utilisez[Lock pour le Web](/libraries/lock) vous devez définir les options `configurationBaseUrl` et `overrides` comme indiqué dans l’exemple de script suivant :



Si vous utilisez [Auth0.js](/libraries/auth0js) sur la page de Connexion universelle, vous devez définir l’option `overrides`.
...
Si vous utilisez [Lock pour le Web](/libraries/lock) intégré dans votre application, vous devez mettre à jour le code pour utiliser votre domaine personnalisé lors de l’initialisation de Lock. Vous devrez également définir la valeur `configurationBaseUrl` sur le CDN approprié URL.
...
Si vous utilisez la [Trousse SDK Auth0 pour les applications à page unique (SPA)](/libraries/auth0-spa-js), [Auth0.js](/libraries/auth0js) ou [d’autres trousses SDK](/support/matrix#auth0-sdks), vous devrez initialiser la trousse SDK en utilisant votre domaine personnalisé. Par exemple, si vous utilisez la trousse SDK Auth0.js, vous devez définir ce qui suit :
...
Consultez la section API ci-dessous si vous utilisez un domaine personnalisé et avez également l’intention d’effectuer [des actions avec Management API via Auth0.js](/libraries/auth0js/v9#user-management).
...
1. Affichez [Auth0 Dashboard > Image de marque > Domaines personnalisés](%24%7Bmanage_url%7D/#/branding/custom_domains).
...
Vous ne pouvez pas utiliser [Clés de développeur Auth0](/devkeys) avec des domaines personnalisés.
...
Continuez à utiliser votre nom de domaine de locataire par défaut (tel que `https://${account.namespace}/userinfo` et `https://${account.namespace}/api/v2/`) au lieu de votre domaine personnalisé lorsque vous spécifiez une `audience`. C’est le seul endroit où vous pouvez utiliser votre domaine de locataire par défaut.
...
Pour utiliser les connexions AD/LDAP avec la prise en charge de Kerberos, vous devrez mettre à jour le point de terminaison Ticket pour qu’il fonctionne avec le domaine personnalisé. Comme indiqué dans la documentation du [connecteur AD/LDAP d’Auth0](/connector/modify#point-an-ad-ldap-connector-to-a-new-connection), le fichier `config.json` doit être modifié, avec la valeur `PROVISIONING_TICKET` changée pour utiliser votre domaine personnalisé au format `https://<YOUR-CUSTOM-DOMAIN>/p/ad/jUG0dN0R`.
