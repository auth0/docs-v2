...
## Comment les clés d’accès fonctionnent avec les domaines personnalisés {#how-passkeys-work-with-custom-domains}

### ID de partie de confiance WebAuthn (RP ID) {#webauthn-relying-party-id-rp-id}
...
### Inscription par domaine {#per-domain-enrollment}
...
## Comprendre l’expérience utilisateur liée aux clés d’accès {#understanding-the-passkey-user-experience}

### Une seule image de marque, un seul domaine {#single-brand-single-domain}
...
### Plusieurs images de marque, domaines distincts {#multi-brand-separate-domains}
...
### Multilocataire avec domaine commun {#multi-tenant-with-common-domain}
...
### Activer les clés d’accès pour votre tenant {#enable-passkeys-for-your-tenant}
...
### Configurer des domaines personnalisés pour les clés d’accès {#configure-custom-domains-for-passkeys}
...
### Vérifier la configuration du RP ID {#verify-rp-id-configuration}
...
## Modèles de mise en œuvre {#implementation-patterns}

### Inciter à l’enrôlement de clés d’accès par domaine {#prompt-for-passkey-enrollment-per-domain}
...
### Suivre l’enrôlement des clés d’accès par domaine {#track-passkey-enrollment-by-domain}
...
### Pages d’enrôlement par domaine {#domain-specific-enrollment-pages}
...
### Invites contextuelles d’enrôlement de clé d’accès {#contextual-enrollment-prompts}
...
## Communication aux utilisateurs {#user-communication}

### Informez les utilisateurs de l’enrôlement distincte pour chaque domaine {#inform-users-about-per-domain-enrollment}
...
### Documentation d’aide {#help-documentation}
...
### Origines associées (fonctionnalité à venir) {#related-origins-future-feature}
...
### Migration d’un domaine personnalisé unique vers plusieurs {#migrating-from-single-custom-domain-to-multiple}
...
### Migration entre des domaines personnalisés {#migrating-between-custom-domains}
...
### Tester l’enrôlement d’une clé d’accès par domaine {#test-passkey-enrollment-per-domain}
...
### La clé d’accès ne fonctionne pas sur un domaine personnalisé {#passkey-not-working-on-custom-domain}
...
### L’utilisateur est confus face à plusieurs enrôlements {#user-confused-about-multiple-enrollments}
