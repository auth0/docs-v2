---
description: Apprenez à configurer un domaine personnalisé par défaut pour votre tenant afin d’offrir une expérience utilisateur uniforme.
...
## Qu’est-ce qu’un domaine personnalisé par défaut? {#what-is-a-default-custom-domain}
...
## Avantages de définir un domaine par défaut {#benefits-of-setting-a-default-domain}
...
## Configurer un domaine par défaut {#configure-a-default-domain}

Vous pouvez définir un domaine personnalisé par défaut à partir de l’Auth0 Dashboard ou de la Management API.

### Utilisation d’Auth0 Dashboard {#using-the-auth0-dashboard}
...
<AuthCodeGroup>
  ```bash Auth0 CLI
  auth0 api patch "custom-domains/default" \
    --data '{
...
## Retirer le statut de domaine par défaut {#remove-default-domain-designation}
...
## Utilisation du domaine par défaut {#how-the-default-domain-is-used}
...
### Points de terminaison de la Management API qui déclenchent des notifications {#management-api-endpoints-that-trigger-notifications}
...
<AuthCodeGroup>
  ```bash Auth0 CLI Sans l’en-tête de domaine personnalisé (utilise le domaine par défaut)
  auth0 api post "tickets/password-change" \
    --data '{
  ...
  ```bash Auth0 CLI Avec l’en-tête de domaine personnalisé (remplace la valeur par défaut)
  auth0 api post "tickets/password-change" \
    --data '{
      "user_id": "auth0|123456",
      "result_url": "https://example.com/password-changed"
...
## Utiliser le domaine par défaut avec Actions {#using-the-default-domain-with-actions}
