...
## Liste de contrôle pré-migration {#pre-migration-checklist}
...
### Ajoutez vos nouveaux domaines personnalisés {#add-your-new-custom-domains}
...
### Vérifier la propriété du domaine {#verify-domain-ownership}

Effectuez le processus de vérification du domaine pour chaque nouveau domaine personnalisé :

#### Pour les certificats gérés par Auth0 {#for-auth0-managed-certificates}
...
<Tabs>
  <Tab title="Auth0 CLI">
    ```bash
    auth0 api get "custom-domains/{customDomainId}"
    ```
  </Tab>
...
#### Pour les certificats gérés par le client {#for-self-managed-certificates}
...
### Configurer le domaine par défaut (facultatif) {#configure-default-domain-optional}
...
<Tabs>
  <Tab title="Auth0 CLI">
    ```bash
    auth0 api patch "custom-domains/{customDomainId}" \
      --data '{
        "is_default": true
      }'
    ```
...
### Mettre à jour la configuration des applications {#update-application-configurations}
...
### Mettre à jour les modèles de courriel {#update-email-templates}
...
### Mettre à jour les fournisseurs d’identité sociale (IdP) {#update-social-identity-providers-idp}
...
#### Autres fournisseurs {#other-providers}
...
### Mettre à jour les connexions d’entreprise (le cas échéant) {#update-enterprise-connections-if-applicable}
...
#### Connexions Azure AD {#azure-ad-connections}
...
### Surveiller et vérifier {#monitor-and-verify}
...
### Mettre l’ancien domaine hors service (le cas échéant) {#decommission-old-domain-if-applicable}
...
<Tabs>
  <Tab title="Auth0 CLI">
    ```bash
    auth0 api delete "custom-domains/{oldCustomDomainId}"
    ```
  </Tab>
...
## Stratégies de migration {#migration-patterns}
...
## Gestion des sessions utilisateur existantes {#handling-existing-user-sessions}

Lors de la migration des domaines personnalisés, les sessions utilisateur existantes peuvent être affectées :

### Considérations relatives aux sessions {#session-considerations}
...
## Plan de retour en arrière {#rollback-plan}

Si vous rencontrez des problèmes lors de la migration :

### Retour en arrière immédiat {#immediate-rollback}
...
### Retour en arrière partiel {#partial-rollback}
...
### Retour en arrière complète {#complete-rollback}
...
## Bonnes pratiques après la migration {#post-migration-best-practices}
