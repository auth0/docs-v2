...
## Configurer Domaines personnalisés multiples {#configure-multiple-custom-domains}
...
#### Afficher et gérer les domaines {#view-and-manage-domains}
...
#### Créer un domaine personnalisé {#create-a-custom-domain}
    ...
    <Callout icon="file-lines" color="#0EA5E9" iconType="regular">Vous utilisez l’Auth0 CLI ? Si ce n’est pas déjà fait, [configurez votre session CLI et authentifiez-vous](/docs/fr-ca/deploy-monitor/auth0-cli) avant d’exécuter cette commande.</Callout>
    ...
    Si l’opération réussit, les renseignements suivants sur votre nouveau domaine personnalisé, y compris son état de vérification, devraient être retournés. Le `status` est initialement retourné comme `pending`.
...
## Fonctionnalités de MCD {#mcd-features}
...
### Domaine par défaut {#default-domain}
...
### Vérification du domaine {#domain-verification}
...
### Métadonnées pour une gestion simplifiée {#metadata-for-enhanced-management}
...
### Personnaliser les modèles de courriel {#customize-email-templates}
...
#### Personnaliser la gestion des courriels à l’aide de la Management API {#customize-email-handling-using-the-management-api}
...
* [Envoyer un courriel de vérification d’adresse courriel](https://auth0.com/docs/api/management/v2/jobs/post-verification-email)
...
### Variables de remplacement d’URL d’application {#application-url-placeholders}
...
### Domaines personnalisés multiples avec Actions {#multiple-custom-domains-with-actions}
...
#### Cas d’utilisation : restreindre l’accès d’un utilisateur à une Organisation en fonction du custom domain {#use-case-restrict-user-access-to-an-organization-based-on-custom-domain}
...
### Attributs des domaines personnalisés {#custom-domain-attributes}
...
#### Attributs mis à jour {#updated-attributes}
...
#### Nouveaux attributs {#new-attributes}
