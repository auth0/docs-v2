---
og:description: Learn how to set up Google Cloud Platform (GCP) with load balancing
  for use as the custom domain proxy for Auth0.
og:image: https://cdn2.auth0.com/docs/1.14553.0/img/share-image.png
og:title: Configurer Google Cloud Platform avec un équilibrage de charge en tant que
  proxy inverse
og:url: https://auth0.com/docs/
permalink: configure-gcp-as-reverse-proxy
title: Configurer Google Cloud Platform avec un équilibrage de charge en tant que
  proxy inverse
twitter:description: Learn how to set up Google Cloud Platform (GCP) with load balancing
  for use as the custom domain proxy for Auth0.
twitter:title: Configurer Google Cloud Platform avec un équilibrage de charge en tant
  que proxy inverse
---
...
1. [Configurer des domaines personnalisés avec des certificats autogérés](/custom-domains/self-managed-certificates) si ce n’est déjà fait. Prenez note des valeurs **Nom du domaine d’origine** et `cname-api-key` pour utilisation ultérieure.
2. Vérifiez la propriété du domaine en ajoutant un enregistrement TXT sur votre serveur DNS dans [Auth0 Dashboard > Tenant Settings (Paramètres du locataire) > Custom Domains (Domaines personnalisés)](%24%7Bmanage_url%7D/#/tenant/custom_domains), puis cliquez sur **Verify (Vérifier)**.
