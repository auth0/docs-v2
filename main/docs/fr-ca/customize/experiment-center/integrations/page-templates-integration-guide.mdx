...
description: "Comment le contexte de l’expérimentation se transmet dans les gabarits de page et les partials d’Auth0 Universal Login, et comment adapter le rendu du gabarit selon la variation."
...
Lorsqu’une expérimentation est active et qu’une variation est attribuée, Experiment Center injecte automatiquement le contexte de l’expérimentation dans le contexte de rendu du [gabarit de page](/docs/fr-ca/customize/login-pages/universal-login/customize-templates#customize-universal-login-page-templates). Les gabarits de page ne nécessitent aucune étape d’activation : le contexte de l’expérimentation est transmis à tous les gabarits sans condition lorsqu’une expérimentation est active. Les gabarits reçoivent ce contexte sous la forme d’une variable locale nommée `experiment`.

## Champs de contexte disponibles {#available-context-fields}
...
### Structure de `experiment.config` {#experimentconfig-structure}
...
## Lire un paramètre dans un modèle {#read-a-parameter-in-a-template}
...
## Personnaliser l’inscription et la connexion avec des Partials {#customize-signup-and-login-with-partials}
...
## Exemple : variante du texte du titre de la page d’inscription {#example-signup-headline-copy-variant}
...
## Exemple : élément d’UI conditionnel {#example-conditional-ui-element}
...
## Valeurs par défaut en l’absence d’expérience active {#safe-defaults-when-no-experiment-is-active}
