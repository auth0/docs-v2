...
description: "Comment lire le contexte de l’expérimentation dans les écrans d’Auth0 Custom Universal Login (ACUL) et ajuster le rendu selon la variation."
...
Lorsqu’une expérimentation est active et qu’une variation est attribuée, Experiment Center injecte un objet `ExperimentContext` dans votre composant [ACUL](/docs/fr-ca/customize/login-pages/advanced-customizations) sous le nom d’`experiment`.
...
Pour qu’un écran reçoive le contexte de l’expérimentation, effectuez une requête `PATCH` vers le point de terminaison [`/api/v2/prompts/{prompt}/screen/{screen}/rendering`](/docs/fr-ca/api/management/v2/prompts/patch-rendering).
...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  La résolution de l’expérimentation et l’enrichissement du journal du tenant s’exécutent toujours, qu’un écran ait fait l’objet d’une activation explicite ou non. L’activation explicite détermine uniquement si les propriétés `experiment` sont transmises à votre composant ACUL. Il s’agit d’une mesure de minimisation des données : n’activez pas explicitement les écrans qui n’ont pas besoin du contexte de l’expérimentation.
</Callout>

## La structure du contexte de l’expérimentation {#the-experiment-context-shape}

Lorsqu’un écran a été explicitement activé et qu’une expérimentation est en cours, vous pouvez accéder au contexte de l’expérimentation dans `window.universal_login_context.experiment`.

```typescript
const experiment = window.universal_login_context.experiment;
```

La structure du contexte de l’expérimentation est :
...
Lorsqu’aucune expérimentation n’est active (ou lorsque la fonctionnalité n’est pas activée pour le tenant), `experiment` est `null`.
...
## Lire la valeur d’un paramètre {#read-a-parameter-value}
...
## Exemple : expérimentation sur les variantes de libellé {#example-copy-variant-experiment}
...
## Exemple : déploiement progressif d’une fonctionnalité booléenne {#example-boolean-feature-rollout}
