---
description: Découvrez comment utiliser des composants personnalisés pour créer des champs qui nécessitent une logique ou une interface utilisateur particulières à l’aide de JavaScript, HTML et CSS.
...
## Gestionnaires de champ personnalisé {#custom-field-handlers}
...
## Objet de contexte {#context-object}
...
### Méthodes personnalisées {#custom-methods}

#### context.custom.getValue() {#contextcustomgetvalue}

Retourne la valeur du champ personnalisé actuel.

#### context.custom.setValue() {#contextcustomsetvalue}
...
#### context.custom.createUid() {#contextcustomcreateuid}
...
#### context.custom.getParams() {#contextcustomgetparams}
...
### Méthodes du formulaire {#form-methods}
...
#### context.form.getAllHiddenFields() {#contextformgetallhiddenfields}

Renvoie un objet contenant toutes les valeurs des champs masqués.

#### context.form.setHiddenField(id, value) {#contextformsethiddenfieldid-value}
...
#### context.form.getValues() {#contextformgetvalues}

Retourne un objet contenant toutes les valeurs des champs, y compris les champs masqués.

#### context.form.getField(id) {#contextformgetfieldid}
...
### Champ personnalisé de saisie de couleur {#color-input-custom-field}
...
### Champ personnalisé de saisie semi-automatique utilisant les valeurs d’une API {#autocomplete-input-custom-field-using-values-from-api}
...
### Champ personnalisé de liste déroulante dynamique avec des valeurs provenant d’une API {#dynamic-dropdown-custom-field-using-values-from-api}
...
### Champ personnalisé à saisie dynamique avec un bouton (+) pour ajouter d’autres champs {#dynamic-input-custom-field-with-a-button-to-add-more-fields}
