---
description: Découvrez comment afficher un formulaire pendant un flux de connexion ou d’inscription à l’aide d’une Action post-login ou pre-user-registration.
...
Remplacez form&#95;id par l’ID du formulaire. Vous pouvez trouver l’ID dans l’URL du formulaire, par exemple : `ap_pUMG...`, ou le sélectionner dans l’onglet **Rendu** de l’éditeur de formulaires.
...
## Attribuer des valeurs aux champs existants et aux champs masqués (côté client) {#populate-values-for-existing-fields-and-hidden-fields-client-side}
...
## Injecter des données personnalisées avec des variables partagées (côté serveur) {#inject-custom-data-with-shared-variables-server-side}
...
## Données des champs et des variables partagées dans les Actions {#fields-and-shared-variables-data-in-actions}
