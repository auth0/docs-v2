...
## Configurer Azure AD {#configure-azure-ad}
...
## Créez la règle de provisionnement Azure AD {#create-the-azure-ad-provisioning-rule}
...
function (user, context, callback) {
  ...
  // le provisionnement des nouveaux utilisateurs (même si elle retourne immédiatement un statut ok)
...
Le moyen le plus simple pour vos utilisateurs externes de s’authentifier consiste à utiliser la connexion initiée par le fournisseur d’identité.
...
Le premier paramètre, `{yourCustomDomain}`, doit correspondre au domaine que vous avez configuré dans Azure AD pour <Tooltip tip="Service qui, après qu’un utilisateur a ouvert une session dans une application, ouvre automatiquement une session dans d’autres applications." cta="Voir le glossaire" href="/docs/fr-ca/glossary?term=Single+Sign-on+%28SSO%29">l’authentification unique (SSO)</Tooltip> (p. ex., `travel0.com`). En le spécifiant comme `whr`, Azure AD saura qu’il doit rediriger vers Auth0 au lieu d’afficher la page de connexion.
