...
## Créer un fournisseur d’identité externe dans AWS {#create-an-external-identity-provider-in-aws}
...
## Configurer SAML SSO pour Auth0 {#configure-saml-sso-for-auth0}
...
## Terminez la configuration du fournisseur d’identité AWS {#complete-aws-identity-provider-configuration}
...
## Créer un rôle AWS IAM {#create-aws-iam-role}
...
## Mapper un rôle AWS à un utilisateur {#map-aws-role-to-a-user}
...
### Mapper plusieurs rôles {#map-multiple-roles}
...
## Configurer l’expiration de la session {#configure-session-expiration}
