---
og:description: How to integrate Auth0 with Amazon Cognito using an OpenID Connect
  (OIDC) Provider.
...
twitter:description: How to integrate Auth0 with Amazon Cognito using an OpenID Connect
  (OIDC) Provider.
...
1. Rendez-vous dans [Auth0 Dashboard> Applications > Applications](%24%7Bmanage_url%7D/#/applications).
...
Cognito utilise la clé de connexion publique [Métadonnées du fournisseur OpenID](https://subscription.auth0.com/.well-known/jwks.json) pour valider la signature du jeton Web JSON (JWT).

Confirmez que votre application Auth0 est configurée pour utiliser l’algorithme de signature RS256 :

1. Allez à [Auth0 Dashboard > Applications > Applications](%24%7Bmanage_url%7D/#/applications), sélectionnez votre application, puis affichez les **Paramètres**.
