...
<DomainTable
  deleteAction={{
    onBefore: (domain) => {
      return confirm(
        `Supprimer le domaine ${domain.domain}? Cette action est irréversible.`,
      );
    },
    onAfter: (domain) => {
...
<Accordion title="Champs de schéma disponibles">
  **create.domainUrl** – Validation de l’URL du domaine – `regex` – Modèle de
  regex personnalisé – `errorMessage` – Message d’erreur personnalisé
</Accordion>
