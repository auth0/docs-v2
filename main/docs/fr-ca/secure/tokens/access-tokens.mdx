...
Dans le cas d’Auth0, les jetons opaques peuvent être utilisés avec le point de terminaison `/userinfo` pour renvoyer un profil utilisateur. Si vous recevez un jeton d’accès opaque, vous n’avez pas besoin de le valider. Vous pouvez l’utiliser avec le point de terminaison `/userinfo`, et Auth0 s’occupe du reste. Pour en savoir plus, consultez [Obtenir des jetons d’accès](/security/tokens/access-tokens/get-access-tokens).

## Jetons d’accès JWT

Les jetons d’accès Web JSON (JWT) sont conformes à la [norme JWT](https://tools.ietf.org/html/rfc7519) et contiennent des informations sur une entité sous la forme de demandes. Ils sont auto-suffisants, il n’est donc pas nécessaire pour le destinataire d’appeler un serveur pour valider le jeton

Les jetons d’accès émis pour Management API et les jetons d’accès émis pour toute API personnalisée que vous avez enregistrée auprès d’Auth0 respectent la norme JWT, ce qui signifie que leur structure de base est conforme à la [structure JWT](/security/tokens/json-web-tokens/json-web-token-structure) typique et qu’ils contiennent des [demandes JWT](/security/tokens/json-web-tokens/json-web-token-claims) concernant le jeton lui-même.

### Jetons d’accès à Management API

Un jeton d’accès émis pour Management API Auth0 devrait être traité comme opaque (peu importe s’il l’est réellement) : vous n’avez donc pas besoin de le valider. Vous pouvez l’utiliser avec Management API Auth0 et Auth0 s’occupe du reste. Pour en savoir plus, consutez [Jetons de Management API Auth0](/security/tokens/access-tokens/management-api-access-tokens).

### Jetons d’accès d’API personnalisée

Si la validation de votre jeton d’accès personnalisé à l’API échoue, assurez-vous qu’il a été émis avec votre API personnalisée comme `audience`. Pour en savoir plus, consultez [Obtenir des jetons d’accès](/security/tokens/access-tokens/get-access-tokens).
