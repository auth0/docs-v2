...
Tant que l’action est en place, les demandes personnalisées qu’elle ajoute apparaîtront dans des nouveaux jetons émis lors de l’utilisation d’un [jeton d’actualisation](/security/tokens/refresh-tokens).
...
Dans le [Registre des demandes de jetons Web JSON de l’IANA](https://www.iana.org/assignments/jwt/jwt.xhtml#claims), vous pouvez voir quelques exemples de demandes publiques enregistrées par OpenID Connect (OIDC) :
...
Pour en savoir plus sur les demandes personnalisées, lisez [Créer des demandes personnalisées](/security/tokens/json-web-tokens/create-custom-claims).