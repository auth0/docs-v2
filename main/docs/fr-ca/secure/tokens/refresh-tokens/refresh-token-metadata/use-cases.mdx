...
## Créer des revendications personnalisées persistantes {#create-persistent-custom-claims}
...
## Créer un ID de session unique {#create-a-unique-session-id}
...
## Créer un identifiant de tenant {#create-a-tenant-identifier}
...
## Gérer les données transitoires provenant des fournisseurs d’identité en amont (IDPs) {#manage-transient-data-from-upstream-identity-providers-idps}
...
## Renforcer la sécurité et la détection de fraude {#enhance-security-and-fraud-detection}
...
## Accéder aux métadonnées avec la Management API {#access-metadata-with-the-management-api}
