...
| --- | --- |
| **grant_type** | Indique le [flux OAuth 2.0](/docs/fr-ca/protocols/oauth2#grant-types) que vous souhaitez exécuter. Pour la communication entre machines, utilisez la valeur `client_credentials`. |
...
| **client_secret** | Il s’agit de la valeur du champ **Client Secret** de l’application de communication entre machines que vous avez créée. Vous pouvez la trouver dans l’onglet [Paramètres de votre application](https://manage.auth0.com/#/applications/{yourClientId}/settings). |
| **audience** | Il s’agit de la valeur du champ **Identifier** de l’`Auth0 Management API`. Vous pouvez le trouver dans l’onglet [Paramètres de l’API](https://manage.auth0.com/#/api). |
