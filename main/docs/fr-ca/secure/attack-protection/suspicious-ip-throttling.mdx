...
Vous pouvez configurer la limitation des adresses IP suspectes dans [Auth0 Dashboard](%24%7Bmanage_url%7D) ou avec [Auth0 Management API](https://auth0.com/docs/api/management/v2/attack-protection/patch-suspicious-ip-throttling).



1. Rendez-vous à [Dashboard > Sécurité > Protection contre les attaques](%24%7Bmanage_url%7D/#/security/attack-protection) et sélectionnez **Limitation des adresses IP suspectes**.
...
* **Authentification d’un grand nombre d’utilisateurs à partir de la même adresse IP** : les utilisateurs qui se trouvent derrière un proxy sont plus susceptibles d’atteindre les limites fixées et de déclencher une limitation. Vous pouvez éviter de déclencher par erreur une limitation en configurant une liste d’autorisations pour l’IP et la plage CIDR du proxy. Consulter [Éviter les problèmes courants liés au flux de mot de passe du propriétaire de ressource et à la protection contre les attaques configurer votre application pour faire confiance à l’adresse IP](/login/flows/avoid-common-issues-with-resource-owner-password-flow-and-attack-protection) :.