...
## Base de données externe vs. stockage de données Auth0 {#external-database-vs-auth0-data-store}
...
## Exemple de scénario de stockage des données utilisateur {#user-data-storage-example-scenario}
...
### Règles d’autorisation des données utilisateur {#user-data-permission-rules}
...
#### Attribuer le rôle Playlist Editor {#assign-playlist-editor-role}
...
#### Le paramètre `scope` indique le rôle {#scope-parameter-specifies-role}
...
#### Associer les chansons d’un utilisateur à cet utilisateur {#associate-a-users-music-with-the-user}
