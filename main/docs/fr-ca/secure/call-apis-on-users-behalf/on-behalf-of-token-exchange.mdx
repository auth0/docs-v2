...
Les échanges de jetons OBO déclenchent le déclencheur d’Action [`post-login`](/docs/fr-ca/customize/actions/explore-triggers/post-login), où :

* [`event.transaction.protocol`](/docs/fr-ca/actions/reference/post-login/post-login-event-object#param-protocol) est défini sur `oauth2-token-exchange`.
* [`event.transaction.actor`](/docs/fr-ca/actions/reference/post-login/post-login-event-object#param-actor) suit l’ensemble de la chaîne de délégation.
...
### Exemple : appels du serveur MCP à une API de première partie {#example-mcp-server-calls-first-party-api}
...
#### Étape 1 : Authentification de l’utilisateur {#step-1-user-authentication}
...
#### Étape 2 : échange OBO {#step-2-obo-exchange}
...
### Utilisateur &gt; serveur MCP &gt; flux API {#user-mcp-server-api-flow}
...
### Utilisateur &gt; API1 &gt; API2 &gt; API3 {#user-api1-api2-api3}
...
* Définissez `resource_server_identifier` sur le public du jeton entrant (c.-à-d. `https://incoming-token-api.example.com`), et non sur l’API en aval. Auth0 utilise l’identifiant du serveur de ressources pour valider le jeton entrant avant d’effectuer l’échange de jetons OBO.

Comme les clients d’API personnalisée sont des clients de première partie, assurez-vous de [ne pas demander le consentement de l’utilisateur](/docs/fr-ca/get-started/applications/third-party-applications/user-consent-and-third-party-applications#skip-consent-for-first-party-applications) pour les API auxquelles votre client de première partie doit accéder.

### Créer un client d’API personnalisé {#create-custom-api-client}
...
<Tabs>
  ...
  <Tab title="Management API">
    Pour créer un client d’API personnalisé avec la même valeur que la demande personnalisée `aud` du jeton entrant, envoyez une requête `POST` au terminal [`/api/v2/clients`](https://auth0.com/docs/api/management/v2/clients/post-clients) avec le corps de la requête suivant :
    ...
    | `resource_server_identifier` | Identifiant unique de votre client d’API personnalisé. Définissez-le sur le public du jeton entrant, c.-à-d. `https://incoming-token-api.example.com`. |
  </Tab>
</Tabs>

### Créer un client grant {#create-client-grant}
...
<Tabs>
  <Tab title="Auth0 Dashboard">
    ...
    2. Sous **API Access**, repérez l’API en aval (c.-à-d. `https://downstream-api.example.com`) et sélectionnez **Edit**.
  ...
  <Tab title="Management API">
    Faites une requête `POST` vers le terminal [`/api/v2/client-grants`](https://auth0.com/docs/api/management/v2/client-grants/post-client-grants) avec le corps de la requête suivant :
...
### Configurer l’échange de jetons OBO {#configure-the-obo-token-exchange}
...
<Tabs>
  ...
  <Tab title="Management API">
    Effectuez une requête `PATCH` au point de terminaison [`/api/v2/clients/{clientId}`](https://auth0.com/docs/api/management/v2/clients/patch-clients-by-id) avec le corps de la requête suivant :
...
## Effectuer un échange de jetons OBO {#perform-obo-token-exchange}
...
<Tabs>
  ...
  <Tab title="cURL">
    ...
    | `audience`             | `https://downstream-api.example.com`              | Obligatoire. L’identifiant du service en aval qui recevra et validera le nouveau jeton.                                                                                                                                                                        |
...
## Prise en charge d’Organizations {#organizations-support}

Lorsqu’un utilisateur s’authentifie via une organisation, le jeton d’accès comprend une demande personnalisée `org_id`. L’échange de jeton OBO préserve ce contexte de l’organisation tout au long de la chaîne de délégation.
...
* contient la même demande personnalisée `org_id` que le jeton d’origine
* applique les mêmes politiques RBAC propres à l’organisation
* rend le contexte de l’organisation accessible dans le [déclencheur `post-login` d’Actions](/docs/fr-ca/actions/reference/post-login/post-login-event-object#event-organization) via la propriété `event.organization`