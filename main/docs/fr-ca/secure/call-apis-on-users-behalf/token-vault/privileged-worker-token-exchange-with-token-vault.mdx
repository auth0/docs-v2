...
## Configurer l’application cliente {#configure-client-application}
...
## Créer un jeton de sujet JWT signé {#create-signed-jwt-subject-token}
...
## Demander un jeton pour l’API externe {#request-token-for-external-api}
...
| `scope`                | <Badge color="yellow">Early Access</Badge> (Facultatif) Une liste de scopes séparés par des espaces à demander pour le jeton d’accès du fournisseur externe. Les scopes demandés correspondent à l’intersection des scopes configurés dans les octrois `token_vault_privileged_access.grants` pour cette connexion sur le client. Si elles ne sont pas fournies, le jeton est émis avec tous les scopes configurés dans les octrois pour cette connexion. |