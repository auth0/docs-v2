...
## Authentification de l’utilisateur vs comptes connectés {#user-authentication-vs-connected-accounts}
...
### Configurer My Account API {#configure-my-account-api}
...
### Configurer le Multi-Resource Refresh Token {#configure-multi-resource-refresh-token}
...
<Tabs>
  ...
  <Tab title="Management API">
    Pour configurer le MRRT pour une application cliente, envoyez une requête `PATCH` au point de terminaison `/api/v2/clients/{clientId}` et ajoutez l’identifiant de My Account API ainsi que les scopes de comptes connectés aux politiques du jeton d’actualisation :
...
## Configurer les comptes connectés {#configure-connected-accounts}
...
2. Sélectionnez **Applications**, puis activez la bascule de la connexion pour votre application cliente.
...
<Tabs>
  <Tab title="Auth0 Dashboard">
    ...
    3. Dans **Purpose**, activez la bascule **Connected Accounts for Token Vault**. Selon le réglage **Purpose**, vous devrez peut-être activer `offline_access` dans l’Auth0 Dashboard pour permettre à l’application cliente d’obtenir un jeton d’actualisation du fournisseur externe pendant le flux des comptes connectés. Pour en savoir plus, consultez [Authentification de l’utilisateur vs comptes connectés](#user-authentication-vs-connected-accounts).
    4. Cliquez sur **Save**.
  </Tab>

  <Tab title="Management API">
    Pour configurer les comptes connectés avec la Management API, envoyez une requête `PATCH` à l’point de terminaison `/connections/{connectionId}` pour définir `connected_accounts` sur `true` :

    <Tabs>
      <Tab title="Auth0 CLI">
        ```bash
        auth0 api patch "connections/{connectionId}" \
...
## Obtenir un jeton d’accès pour Connected Accounts {#get-access-token-for-connected-accounts}
...
### Obtenir un jeton d’actualisation {#fetch-a-refresh-token}
...
### Échanger un refresh token contre un jeton d’accès à l’API My Account {#exchange-refresh-token-for-my-account-api-access-token}
...
## Lancer une requête Connected Accounts {#initiate-connected-accounts-request}
...
## Terminer une requête Connected Accounts {#complete-connected-accounts-request}
...
## Gérer les comptes connectés {#manage-connected-accounts}
...
### Interroger les connexions de Connected Accounts {#query-connected-accounts-connections}
...
### Lister les comptes connectés {#query-connected-accounts}
...
<Tabs>
  <Tab title="Auth0 CLI">
    ```bash
    auth0 api get "users/{userId}/connected-accounts"
    ```
  </Tab>
...
### Interroger les comptes connectés pour une connexion donnée {#query-connected-accounts-for-a-given-connection}
...
### Supprimer un compte connecté {#delete-connected-account}
