...
## Étape 1 : Se connecter et autoriser l’accès {#step-1-connect-and-authorize-access}
...
## Étape 2 : le SPA appelle l’API backend avec le jeton d’accès Auth0 {#step-2-spa-calls-backend-api-with-auth0-access-token}
...
## Étape 3 : L’API backend effectue l’échange de jeton d’accès {#step-3-backend-api-performs-access-token-exchange}
...
Lorsque l’API backend effectue l’échange de jeton d’accès, elle s’authentifie en transmettant les informations d’authentification du Custom API Client au serveur d’autorisation Auth0, ce qui prouve qu’il s’agit bien de la même entité enregistrée dans l’Auth0 Dashboard.
...
* Transmet les informations d’authentification de l’API backend (celles du Custom API Client) au serveur d’autorisation Auth0 pour s’authentifier.
...
| `scope`                | <Badge color="yellow">Early Access</Badge> (Facultatif) Une liste de scopes séparés par des espaces à demander pour le jeton d’accès du fournisseur externe. Utilisez-la pour demander un sous-ensemble des scopes initialement accordés pendant le flux Connected Accounts. S’il n’est pas fourni, le jeton est émis avec tous les scopes initialement accordés.      |
...
## Étape 4 : Le serveur d’autorisation Auth0 valide le jeton d’accès {#step-4-auth0-authorization-server-validates-access-token}
