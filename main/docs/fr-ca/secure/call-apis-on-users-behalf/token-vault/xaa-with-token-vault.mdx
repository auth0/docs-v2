---
description: Configurez Token Vault pour utiliser Cross App Access (XAA) afin de stocker et de réutiliser les jetons d'accès que votre application obtient par XAA au nom d'un utilisateur.
title: Cross App Access (XAA) avec Token Vault
---
...
## Qu&#39;est-ce qui change avec XAA? {#whats-different-with-xaa}
...
## Ajouter XAA à une intégration Token Vault existante {#add-xaa-to-an-existing-token-vault-integration}
...
### Configurer la connection de la Requesting App {#configure-the-requesting-app-connection}
...
### Configurer la connexion de la Resource App {#configure-the-resource-app-connection}

La connexion de la Resource App à l&#39;API tierce doit être une connexion OIDC dont les fonctionnalités Connected Accounts for Token Vault et Cross App Access pour Token Vault sont activées.

<Tabs>
  <Tab title="Auth0 Dashboard">
    1. Allez à **Authentication &gt; Enterprise**, sélectionnez la connexion OIDC vers l&#39;API tierce, puis ouvrez ses paramètres.
...
Une fois les deux connexions configurées, passez à [Configurer votre application Token Vault](#configure-your-token-vault-application) et [testez le flow end-to-end](#test-the-end-to-end-flow).

## Configurer votre application Token Vault {#configure-your-token-vault-application}
...
<Tabs>
  <Tab title="Auth0 Dashboard">
    ...
    2. Sous **Advanced Settings &gt; Grant Types**, activez **Authorization Code**, **Refresh Token** et **Token Vault**.
    3. Sélectionnez **Save Changes**.
  </Tab>

  <Tab title="Management API">
    Faites une requête `PATCH` à l&#39;endpoint [Update a Client](https://auth0.com/docs/api/management/v2/clients/patch-clients-by-id) pour ajouter les grant types requis et activer Cross App Access :
...
### Activer les connexions Okta pour l&#39;application {#enable-okta-connections-for-the-application}
...
Si vous avez déjà configuré Token Vault et que vous ajoutez la prise en charge de XAA : vous devez activer la connexion de la Requesting App et celle de la Resource App pour l&#39;application que vous venez de créer. Pour en savoir plus, consultez [Ajouter XAA à une intégration Token Vault existante](#add-xaa-to-an-existing-token-vault-integration).

<Tabs>
  <Tab title="Auth0 Dashboard">
    Pour activer la connexion Okta Workforce ou la connexion de la Requesting App :

    1. Accédez à **Authentication &gt; Enterprise &gt; Okta Workforce**, sélectionnez la connexion Okta Workforce, puis sélectionnez l&#39;onglet **Applications**. Activez-la ensuite pour l&#39;application que vous venez de créer.

    Pour activer la connexion OIDC ou la connexion de la Resource App :

    1. Accédez à **Authentication &gt; Enterprise &gt; OpenID Connect (OIDC)**, sélectionnez la connexion OIDC, puis sélectionnez l&#39;onglet **Applications**. Activez-la ensuite pour l&#39;application que vous venez de créer.
  </Tab>
...
## Tester le flow de bout en bout {#test-the-end-to-end-flow}

Pour tester le flow XAA Token Vault, votre application doit :

1. [Obtenir un jeton d’actualisation Auth0](#step-1-obtain-an-auth0-refresh-token) en réalisant un flux de code d’autorisation avec la connexion Okta Workforce ou la connexion de la Requesting App.
2. [Échanger le jeton d’actualisation](#step-2-exchange-the-refresh-token-with-token-vault) contre un jeton d’accès de la Resource App avec la connexion OIDC, à l&#39;aide du Token Vault grant type ou de la connexion de la Resource App.

Utilisez l&#39;application que vous avez configurée à la section [Configurer votre application Token Vault](#configure-your-token-vault-application), ou une application existante dont le grant Token Vault est activé.

### Étape 1 : Obtenir un refresh token Auth0 {#step-1-obtain-an-auth0-refresh-token}
...
#### Lancer l&#39;authorization request {#initiate-the-authorization-request}
...
#### Échanger le code d&#39;autorisation contre un jeton d&#39;actualisation {#exchange-the-authorization-code-for-a-refresh-token}
...
### Étape 2 : Échanger le refresh token avec Token Vault {#step-2-exchange-the-refresh-token-with-token-vault}
...
## Gérer plusieurs IdPs de Requesting App {#handle-multiple-requesting-app-idps}
...
## Utiliser Auth0 Organizations avec XAA {#use-auth0-organizations-with-xaa}
