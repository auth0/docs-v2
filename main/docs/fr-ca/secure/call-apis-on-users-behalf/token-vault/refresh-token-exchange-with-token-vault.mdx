...
## Étape 2 : Effectuer l’échange de jeton d’actualisation {#step-2-perform-refresh-token-exchange}
...
L’application peut utiliser un jeton d’actualisation Auth0 valide pour demander un Google jeton d’accès à Token Vault avec les scopes accordés dans le Connected Accounts flow. Ce processus permet à l’application d’obtenir un nouveau jeton d’accès sans obliger l’utilisateur à autoriser de nouveau la connexion.
...
| `subject_token_type`   | Type du jeton du sujet. Pour Token Vault, définissez-le comme jeton d’actualisation : `urn:ietf:params:oauth:token-type:refresh_token`                                                                                                                                                                                                                                    |
...
| `scope`                | <Badge color="yellow">Accès anticipé</Badge> (Facultatif) Une liste de scopes séparés par des espaces à demander pour le jeton d’accès du fournisseur externe. Utilisez-la pour demander un sous-ensemble des scopes initialement accordés pendant le Connected Accounts flow. S’il n’est pas fourni, le jeton est émis avec tous les scopes initialement accordés.       |
...
## Étape 3 : Auth0 Authorization Server valide le jeton d’actualisation {#step-3-auth0-authorization-server-validates-refresh-token}
...
## Politique d’expiration du jeton d’actualisation du fournisseur externe {#external-provider-refresh-token-expiration-policy}
