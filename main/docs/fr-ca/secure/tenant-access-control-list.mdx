...
* Si vous adoptez les Model Context Protocols (MCP) dans votre locataire, vous pouvez utiliser le scope [Dynamic Client Registration (`dynamic_client_registration`)](./tenant-access-control-list/reference#param-dcr) pour éviter des risques comme l’enregistrement non autorisé d’applications ou des tentatives d’hameçonnage au moyen de noms d’application trompeurs.
