  ...
  <Tab title="Management API">
    ...
    <Tabs>
      <Tab title="Auth0 CLI">
        ...
        auth0 api patch "clients/{YOUR_CLIENT_ID}" \
          --data '{"require_proof_of_possession": true}'
        ```
      </Tab>
    ...
    <Tabs>
      <Tab title="Auth0 CLI">
        ```bash
        auth0 api patch "clients/{YOUR_CLIENT_ID}" \
          --data '{"require_proof_of_possession": false}'
        ```
      </Tab>
...
<Tabs>
  ...
  <Tab title="Management API">
    Pour activer la contrainte d’émetteur (Sender Constraining) avec l’API Management, envoyez une requête PATCH pour [mettre à jour le serveur de ressources](https://auth0.com/docs/api/management/v2/resource-servers/patch-resource-servers-by-id). Définissez les paramètres de l’objet `proof_of_possession` comme suit :
    ...
    L’exemple de code suivant montre un corps de requête qui configure un serveur de ressources pour la contrainte d’émetteur mTLS. Comme mTLS ne prend en charge que `all_clients` pour `required_for` (qui est la valeur par défaut), il n’est pas nécessaire de le préciser :
    ...
    L’exemple de code suivant présente un corps de requête qui configure un serveur de ressources pour la contrainte de l’émetteur DPoP, requis uniquement pour les applications publiques :
    ...
    L’exemple de code suivant configure la contrainte d’émetteur DPoP comme requise pour toutes les applications. Comme `all_clients` est la valeur par défaut, `required_for` peut être omis :
    ...
    L’exemple de code suivant désactive la contrainte d’émetteur pour un serveur de ressources (équivalent à sélectionner « Jamais » dans le tableau de bord) :
