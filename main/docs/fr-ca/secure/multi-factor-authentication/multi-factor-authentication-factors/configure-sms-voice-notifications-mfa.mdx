---
description: Configurez les notifications par SMS et par appel vocal comme facteurs MFA Auth0 avec Twilio, le fournisseur Auth0 intégré ou un fournisseur personnalisé pour envoyer des codes à usage unique.
title: Configurer les notifications par SMS et par appel vocal pour MFA
---
...
<Frame>![Configurer SMS and Voice pour l’expérience utilisateur MFA Voice](/docs/images/cdy7uua7fh8z/2Q4BViGl71sdrytUDgNJ10/693969bb00444c6bef030e90bb1015f2/2025-02-12_10-06-29.png)</Frame>

Lorsque vous activez seulement SMS, le processus est plus simple :

<Frame>![Configurer SMS and Voice pour l’expérience utilisateur MFA SMS](/docs/images/cdy7uua7fh8z/64PgR0CO1Wjfie2Hxy1Ptw/e6dcf80c0739f8235b473c0291a01e63/2025-02-12_10-07-15.png)</Frame>
...
1. Accédez à [**Auth0 Dashboard &gt; Security &gt; Multi-factor Auth**](https://manage.auth0.com/#/security/mfa).
2. Sélectionnez **Phone Message**, puis activez l’interrupteur situé en haut de la page.
3. Indiquez si le fournisseur de messagerie téléphonique configuré sous **[Branding &gt; Phone Provider](https://manage.auth0.com/#/phone/templates/phone/provider)** s’applique à votre locataire. Si c’est le cas, activez **Use Tenant-Level Messaging Provider**.
4. Choisissez votre méthode d’envoi :
...
5. Configurez la longueur et la durée de validité du code :
...
<Warning>
  Les clients sont responsables de toute réduction du niveau de sécurité découlant d’une modification visant à : (i) prolonger la durée d’expiration du code ou la durée du comportement de session « Remember Me » au-delà de la configuration standard recommandée par Okta; ou (ii) réduire la longueur de l’OTP sous le nombre de caractères prévu par la configuration standard recommandée par Okta.
</Warning>

6. Configurez votre fournisseur de messagerie téléphonique, puis accédez à **[Branding &gt; Phone Provider](https://manage.auth0.com/#/phone/templates/phone/provider)** :
   1. **Twilio** : Envoie les messages à l’aide de l’[API Twilio Programmable SMS](https://www.twilio.com/sms) pour les SMS ou de l’[API Twilio Programmable Voice](https://www.twilio.com/voice) pour les messages vocaux. Utilisez les identifiants Twilio Live, et non les identifiants de test, qui servent uniquement à envoyer des messages dans un environnement de production. Pour en savoir plus, consultez [Configurer Twilio comme fournisseur de messagerie téléphonique](/docs/fr-ca/customize/phone-messages/configure-phone-messaging-providers/configure-twilio-as-a-phone-messaging-provider).
   2. **Personnalisé** : Envoie les messages en appelant une Action dans le [Send Phone Message Flow](/docs/fr-ca/customize/actions/explore-triggers/mfa-notifications-trigger).
   * [Configurer un fournisseur téléphonique personnalisé avec Terraform](/docs/fr-ca/customize/phone-messages/configure-phone-messaging-providers/configure-a-custom-phone-provider/configure-a-custom-phone-provider-with-terraform)
   * [Configurer un fournisseur téléphonique personnalisé avec Twilio Verify](/docs/fr-ca/customize/phone-messages/configure-phone-messaging-providers/configure-a-custom-phone-provider/configure-a-custom-phone-provider-with-twilio-verify)

### Configuration de Twilio {#twilio-configuration}
...
### Personnaliser les modèles de messages SMS ou vocaux {#customize-sms-or-voice-message-templates}
...
## Fournisseurs de messagerie SMS intégrés {#integrated-sms-messaging-providers}
...
## Fournisseurs de téléphonie personnalisés {#custom-phone-providers}
