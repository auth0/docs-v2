...
`languageDictionary` peut être utilisé pour remplacer le texte dans de nombreux domaines du processus <Tooltip href="/docs/fr-ca/glossary?term=multifactor-authentication" tip="Authentification multifacteur (MFA)
Processus d’authentification de l’utilisateur qui utilise un facteur en plus du nom d’utilisateur et du mot de passe, tel qu’un code par SMS." cta="Voir le glossaire">MFA</Tooltip> avec l’expérience de connexion classique. Utilisez le format suivant :
