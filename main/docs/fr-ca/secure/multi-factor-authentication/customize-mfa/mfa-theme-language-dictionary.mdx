---
description: Décrit les options de configuration de la page hébergée de la MFA pour personnaliser les propriétés du thème des pages MFA.
...
Le `languageDictionary` peut être utilisé pour remplacer le texte dans de nombreuses parties du processus de <Tooltip tip="Authentification multifacteur (MFA) : processus d’authentification de l’utilisateur qui utilise un facteur en plus du nom d’utilisateur et mot de passe, comme un code par SMS." cta="Voir le glossaire" href="/docs/fr-ca/glossary?term=MFA">MFA</Tooltip> de l’expérience Classic Login. Pour ce faire, utilisez le format suivant :
...
Voici un exemple où l’on modifie l’attribut `headerText` dans `smsEnrollmentConfirm` :
...
Vous trouverez ci-dessous la liste des catégories et des options disponibles.

### options de languageDictionary {#languagedictionary-options}
...
smsEnrollmentAddPhoneNumber: {
  ...
  // countryCodes: { 'US': 'Traduction de États-Unis', ... '<country code>': '<translation>' }
},
