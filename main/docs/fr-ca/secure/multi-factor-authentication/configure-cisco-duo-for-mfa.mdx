---
description: Configurez Cisco Duo Security comme facteur d’AMF Auth0 à l’aide de vos identifiants Duo Web SDK, notamment la clé d’intégration, la clé secrète et le nom d’hôte de l’API.
title: Configurer Cisco Duo Security pour l’AMF
---
...
## Utiliser Actions pour activer Duo {#use-actions-to-enable-duo}
...
## Modèle d’Actions pour Duo {#actions-template-for-duo}
...
export const codeExample = `exports.onExecutePostLogin = async (event, api) => {
	...
	if (CLIENTS_WITH_MFA.includes(event.client.client_id)) {
		...
		// facultatif; la valeur par défaut est false. Définissez true pour autoriser le comportement standard de Duo consistant à mémoriser les sessions pendant 30 jours.
