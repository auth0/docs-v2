...
* Si vous utilisez [Connexion universelle](/login/universal-login), redirigez l'utilisateur vers le point de terminaison Authorize, en spécifiant l'audience ` avant d'appeler l'API MFA.
...
### Lors de la connexion, faites appel à l'audience ` et utilisez un [jeton d’actualisation](/security/tokens/refresh-tokens) pour l’actualiser plus tard.
...
## Si un `mfa_token` a été utilisé pour répertorier les authentifiants, les utilisateurs devront [compléter MFA pour obtenir un jeton d’accès]() avec une audience de ` afin de supprimer un authentifiant.
...
Pour supprimer un code de récupération et en générer un nouveau, obtenez un [Jeton d’accès à Management API]() Autho et utiliser Management API [Point de terminaison de régénération du code de récupération]().
