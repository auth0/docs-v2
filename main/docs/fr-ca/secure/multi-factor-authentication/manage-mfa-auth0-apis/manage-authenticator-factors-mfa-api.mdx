---
description: Décrit comment gérer les inscriptions des facteurs d’authentification MFA pour les applications à l’aide de l’Auth0 MFA API.
...
## Obtenir des jetons d’accès à l’API MFA {#get-mfa-api-access-tokens}
...
<Warning>
  ...
  Auth0 recommande aux administrateurs de tenant de [créer une action](/docs/fr-ca/customize/actions/write-your-first-action) qui définit `allowRememberBrowser` sur `false`. Cela masquera **Remember this browser** dans l’expérience de l’utilisateur final. Vous pouvez aussi configurer des limites de durée de vie des navigateurs approuvés à l’aide du Auth0 Dashboard ou des Actions. Pour en apprendre davantage, lisez [Modifier la fréquence des invites MFA](/docs/fr-ca/secure/multi-factor-authentication/customize-mfa#change-frequency-of-mfa-prompts).
</Warning>

### Octroi par mot de passe du propriétaire de la ressource {#resource-owner-password-grant}
...
## Lister les authentificateurs {#list-authenticators}
...
## Inscrire des authentificateurs {#enroll-authenticators}
...
## Supprimer des authentificateurs {#delete-authenticators}
...
## Régénérer les codes de récupération {#regenerate-recovery-codes}
