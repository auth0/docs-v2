...
title: Gérer les méthodes d’authentification avec l’Auth0 Management API
---
...
L’[Auth0 Management API](https://auth0.com/docs/api/management/v2) fournit plusieurs points de terminaison que vous pouvez utiliser pour gérer les méthodes d’authentification <Tooltip tip="Authentification multifacteur (MFA) : processus d’authentification de l’utilisateur qui exige un facteur en plus du nom d’utilisateur et du mot de passe, comme un code envoyé par SMS." cta="Voir le glossaire" href="/docs/fr-ca/glossary?term=MFA">MFA</Tooltip> de vos utilisateurs.

Cette méthode repose sur une authentification effectuée au moyen d’une application confidentielle. Pour en savoir plus sur les applications confidentielles et les applications publiques, consultez [Applications confidentielles et publiques](/docs/fr-ca/get-started/applications/confidential-and-public-applications).

## Obtenir toutes les méthodes d’authentification {#get-all-authentication-methods}

Utilisez le point de terminaison [Récupère la liste des méthodes d’authentification](https://auth0.com/docs/api/management/v2#!/Users/get_authentication_methods) pour obtenir la liste de toutes les méthodes d’authentification qu’un utilisateur a complètement ou partiellement configurées.

Ce point de terminaison nécessite le scope : `read:authentication_methods`.
...
<AuthCodeGroup>
  ...
  auth0 api get "users/%7BuserId%7D/authentication-methods"
  ```
...
## Obtenir une seule méthode d’authentification {#get-a-single-authentication-method}

Utilisez le point de terminaison [Obtenir une méthode d’authentification par ID](https://auth0.com/docs/api/management/v2#!/Users/get_authentication_methods_by_authentication_method_id) pour récupérer une méthode d’authentification d’un utilisateur en indiquant l’ID de la méthode d’authentification.

Ce point de terminaison nécessite le scope suivant : `read:authentication_methods`.
...
<AuthCodeGroup>
  ```bash Auth0 CLI lines
  auth0 api get "users/%7BuserId%7D/authentication-methods/%7BauthenticationMethodId%7D"
  ```
...
<AuthCodeGroup>
  ```bash Auth0 CLI lines
  auth0 api post "users/%7BuserId%7D/authentication-methods" \
...
### Mots de passe à usage unique (OTP) {#one-time-passwords-otp}
...
Permettez aux utilisateurs d’utiliser des clés de sécurité conformes à FIDO (par exemple, [Yubikey](https://www.yubico.com/) ou [Google Titan](https://cloud.google.com/titan-security-key)) pour l’authentification multifacteur.
...
## Remplacer toutes les méthodes d’authentification {#replace-all-authentication-methods}
...
Cet point de terminaison nécessite le scope `update:authentication_methods`.
...
<AuthCodeGroup>
  ```bash Auth0 CLI lines
  auth0 api put "users/%7BuserId%7D/authentication-methods" \
...
## Mettre à jour une seule méthode d’authentification {#update-a-single-authentication-method}

Utilisez le point de terminaison [Updates an authentication method](https://auth0.com/docs/api/management/v2#!/Users/patch_authentication_methods_by_authentication_method_id) pour mettre à jour une seule méthode d’authentification d’un utilisateur.

Ce point de terminaison nécessite le scope : `update:authentication_methods`.
...
<AuthCodeGroup>
  ```bash Auth0 CLI lines
  auth0 api patch "users/%7BuserId%7D/authentication-methods/%7BauthenticationMethodId%7D" \
...
## Supprimer toutes les méthodes d’authentification {#delete-all-authentication-methods}

Utilisez le point de terminaison [Supprimer toutes les méthodes d’authentification de l’utilisateur spécifié](https://auth0.com/docs/api/management/v2#!/Users/delete_authentication_methods) pour supprimer toutes les méthodes d’authentification d’un utilisateur.

Ce point de terminaison nécessite le scope suivant : `delete:authentication_methods`.
...
<AuthCodeGroup>
  ```bash Auth0 CLI lines
  auth0 api delete "users/%7BuserId%7D/authentication-methods"
  ```
...
### Réponses {#responses-9}

Pour chaque requête valide, la Management API renvoie une réponse avec un code d’état `204` et un corps de la réponse vide.

## Supprimer une seule méthode d’authentification {#delete-a-single-authentication-method}

Utilisez le point de terminaison [Deletes an authentication method by ID](https://auth0.com/docs/api/management/v2#!/Users/delete_authentication_methods_by_authentication_method_id) pour supprimer une méthode d’authentification d’un utilisateur.
...
<AuthCodeGroup>
  ```bash Auth0 CLI lines
  auth0 api delete "users/%7BuserId%7D/authentication-methods/%7BauthenticationMethodId%7D"
  ```
