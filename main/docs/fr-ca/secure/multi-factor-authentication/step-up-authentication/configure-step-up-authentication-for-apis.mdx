...
## Valider les jetons d’accès pour MFA {#validate-access-tokens-for-mfa}
...
## Scénario : Transactions bancaires avec des notifications push {#scenario-bank-transactions-with-push-notifications}
...
Créez une Action qui demande à l’utilisateur de s’authentifier avec la MFA lorsque la permission `transfer:funds` est demandée. Accédez à [Dashboard &gt; Actions &gt; Flows](https://manage.auth0.com/#/actions/flows) et créez une Action contenant le contenu suivant :
...
### Configurer l’API {#configure-api}
