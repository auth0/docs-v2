...
## Valider les jetons d’identité pour la MFA {#validate-id-tokens-for-mfa}
...
#### Exceptions concernant la claim `amr` {#amr-claim-exceptions}
...
### Exemple : valeurs avec MFA {#example-values-with-mfa}
...
### Exemple : valeurs sans MFA {#example-values-without-mfa}
...
## Scénario : Données salariales avec notifications push {#scenario-salary-data-with-push-notifications}
...
exports.onExecutePostLogin = async (event, api) => {
 ...
 // demander MFA uniquement si l’application web l’a indiqué dans la requête d’authentification
...
### Valider le jeton d’identité {#validate-id-token}
