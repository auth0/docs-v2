---
description: Décrit comment créer vos propres flux MFA en utilisant les SMS ou l’appel vocal comme facteur d’authentification.
title: Inscrire et vérifier les authentificateurs SMS et d’appel vocal
---

import {AuthCodeBlock} from "/snippets/fr-ca/AuthCodeBlock.jsx";
import {AuthCodeGroup} from "/snippets/fr-ca/AuthCodeGroup.jsx";

Auth0 offre un <Tooltip tip="Authentification multifacteur (MFA) : processus d’authentification de l’utilisateur qui utilise un facteur en plus du nom d’utilisateur et mot de passe, comme un code envoyé par SMS." cta="Voir le glossaire" href="/docs/fr-ca/glossary?term=MFA">MFA</Tooltip> intégré d’inscription et d’authentification à l’aide de [Universal Login](/docs/fr-ca/authenticate/login/auth0-universal-login). Cependant, si vous souhaitez créer votre propre interface utilisateur, vous pouvez utiliser la [MFA API](/docs/fr-ca/secure/multi-factor-authentication/multi-factor-authentication-developer-resources/mfa-api) pour le faire.
...
* [Configurer le téléphone comme facteur](/docs/fr-ca/secure/multi-factor-authentication/multi-factor-authentication-factors/configure-sms-voice-notifications-mfa) dans le Dashboard ou à l’aide de la [Management API](https://auth0.com/docs/api/management/v2#!/Guardian/put_factors_by_name).

## S’inscrire par SMS ou appel vocal {#enroll-with-sms-or-voice}
...
### Inscrire un authentificateur {#enroll-authenticator}

Faites une requête `POST` au terminal MFA Associate pour inscrire l’authentificateur de l’utilisateur. Le bearer token exigé par ce terminal est le jeton MFA obtenu à l’étape précédente.

Pour l’inscription par SMS ou appel vocal, l’utilisateur s’inscrit avec un numéro de téléphone qui peut être utilisé pour une demande de vérification par SMS ou par appel vocal. Indiquez les paramètres ci-dessous pour faire une requête au terminal. Le paramètre `oob_channels` indique comment vous voulez envoyer le code à usage unique à l’utilisateur (SMS ou appel vocal).
...
Si vous obtenez l’erreur `User is already enrolled`, c’est que l’utilisateur a déjà un facteur MFA configuré. Avant d’associer un autre facteur à l’utilisateur, vous devez d’abord le soumettre à une vérification à l’aide du facteur existant.
...
### Confirmer l’inscription par SMS ou appel vocal {#confirm-sms-or-voice-enrollment}

Les utilisateurs devraient recevoir un message contenant un code à usage unique qu’ils devront fournir à l’application.

Pour terminer l’inscription, envoyez une requête `POST` au <Tooltip tip="Terminal de jeton : terminal du serveur d’autorisation utilisé pour demander des jetons par programmation." cta="Voir le glossaire" href="/docs/fr-ca/glossary?term=OAuth">OAuth</Tooltip> <Tooltip tip="OAuth 2.0 : cadre d’autorisation qui définit les protocoles et les flux d’autorisation." cta="Voir le glossaire" href="/docs/fr-ca/glossary?term=Token+endpoint">terminal de jeton</Tooltip>. Vous devez inclure le `oob_code` renvoyé dans la réponse précédente, ainsi que le `binding_code` avec la valeur reçue dans le message.
...
## Vérification par SMS ou appel vocal {#challenge-with-sms-or-voice}
...
Pour lancer une demande de vérification à l’utilisateur, vous avez besoin de l’`authenticator_id` du facteur que vous voulez interroger. Vous pouvez afficher tous les authentificateurs inscrits à l’aide du point de terminaison MFA Authenticators :
...
### Demander une vérification par code à usage unique à l’utilisateur {#challenge-user-with-a-one-time-code}

Pour déclencher une demande de vérification, envoyez une requête `POST` au terminal MFA Challenge à l’aide de l’`authenticator_id` correspondant et du `mfa_token`.
...
### Terminez l’authentification avec le code reçu {#complete-authentication-using-received-code}
...
Votre application doit inviter l’utilisateur à saisir le code envoyé dans le message et définir ce code dans le paramètre `binding_code`. Vous pouvez vérifier le code et obtenir des jetons d’authentification à l’aide du terminal de jeton d’OAuth0, en précisant les valeurs `binding_code` et `oob_code` renvoyées par la requête précédente :
...
Utilisez le `mfa_token` pour envoyer une requête au point de terminaison [`mfa/authenticator`](/docs/fr-ca/api/authentication/muti-factor-authentication/list-authenticators) afin d’obtenir la liste de tous les facteurs auxquels l’utilisateur s’est inscrit et de repérer un type pris en charge par votre application.  Vous devez aussi obtenir l’`authenticator_type` correspondant pour lancer des demandes de vérification :
