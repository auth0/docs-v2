...
## Demander le consentement {#ask-for-consent}
...
### Utiliser une interface utilisateur personnalisée {#use-custom-ui}
...
### Renouvellement du consentement et migration des utilisateurs {#re-consent-and-user-migration}
...
## Suivi du consentement {#track-consent}
...
### Rechercher des utilisateurs par adresse courriel {#search-for-users-by-email-address}
...
Définissez le paramètre de requête **fields** sur `user_metadata` afin de limiter les champs retournés. Ainsi, seules les données user&#95;metadata seront retournées plutôt que le profil utilisateur complet.
...
<AuthCodeGroup>
  ...
  auth0 api get "users-by-email" \
    -q "email=USER_EMAIL_ADDRESS" \
    -q "fields=user_metadata"
  ```
...
### Rechercher des utilisateurs par ID {#search-for-users-by-id}
...
<AuthCodeGroup>
  ```bash Auth0 CLI
  auth0 api get "users/%7ByourUserID%7D" \
    -q "fields=user_metadata"
  ```
...
### Mettre à jour les informations sur le consentement {#update-consent-information}
...
#### Mettre à jour la propriété racine {#update-root-property}
...
#### Mettre à jour une propriété imbriquée {#update-inner-property}
...
### Exporter les renseignements sur le consentement {#export-consent-information}
...
## Retirer le consentement {#withdraw-consent}
...
<AuthCodeGroup>
  ```bash Auth0 CLI
  auth0 api delete "users/USER_ID"
  ```
...
### Marquer l’utilisateur comme supprimé {#flag-user-as-deleted}
...
#### Marquer le profil {#flag-the-profile}
...
#### Désactiver la connexion pour les utilisateurs marqués {#disable-login-for-flagged-users}
