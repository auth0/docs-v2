...
Au lieu de créer une application de zéro, nous allons [utiliser l’exemple Guide rapide JavaScript](/quickstart/spa/vanillajs) d’Auth0. Nous utiliserons d’autre part les pages Universal Login (Connexion universelle) d’Auth0 afin de pouvoir implémenter une expérience de connexion universelle au lieu d’intégrer la connexion dans notre application. Pour en savoir plus sur la connexion universelle, consultez [Connexion universelle Auth0](/login/universal-login). Pour en savoir plus sur les différences entre la connexion universelle et la connexion intégrée, consultez [Connexion universelle centralisée par rapport à la Connexion intégrée](/login/universal-vs-embedded-login).
...
1. Accédez à [Auth0 Dashboard > Applications > Applications](%24%7Bmanage_url%7D/#/applications) et créez une nouvelle application. Choisissez le type `Applications Web à page unique`.
...
4. Accédez à [Auth0 Dashboard > Authentification > Base de données](%24%7Bmanage_url%7D/#/connections/database) et créez à nouveau une connexion. Cliquez sur **Créer une connexion par base de données**, choisissez un nom pour la nouvelle connexion, puis cliquez sur **Enregistrer**. Vous pouvez aussi activer une connexion par les réseaux sociaux dans [Auth0 Dashboard > Authentification > Social](%24%7Bmanage_url%7D/#/connections/social) (nous activerons la connexion par Google pour les besoins de ce tutoriel).
5. Accédez à l’onglet **Applications** de la connexion et vérifiez que l’application que vous venez de créer est bien activée.
6. [Téléchargez l’exemple d’application JavaScript à page unique](/quickstart/spa/vanillajs).
...
1. Accédez à [Auth0 Dashboard > Image de marque > Connexion universelle](%24%7Bmanage_url%7D/#/login_page).
...
7. Dans la barre latérale de l’éditeur de code des actions, sélectionnez Test (icône de lecture), puis sélectionnez **Exécuter** pour  [tester votre code](/actions/test-actions).
8. Lorsque vous êtes prêt à lancer l’action, sélectionnez **Déployer**.

Enfin, ajoutez l’action que vous avez créée dans le [flux de connexion](%24%7Bmanage_url%7D/#/actions/flows/login/). Pour savoir comment attacher des actions à des flux, lisez la section Attacher l’action à un flux dans [Écrire votre première action](/actions/write-your-first-action).
...
Si vous utilisez des connexions à des réseaux sociaux, l’ajout de champs personnalisés n’est pas une option, mais vous pouvez rediriger l’utilisateur vers une autre page où vous demandez son consentement et des informations supplémentaires, puis vous le redirigez pour terminer la transaction d’authentification. Cela peut être fait avec des Actions de redirection. Pour en savoir plus, consultez [Rediriger avec des actions](/signup-and-login-triggers/login-trigger/redirect-with-actions). Nous allons utiliser cette même Action pour enregistrer les informations de consentement dans les métadonnées de l’utilisateur, afin de suivre ces informations et ne pas avoir à les demander lors de la prochaine connexion.
...
1. [Créer à nouveau une Action](/actions/write-your-first-action). Saisissez un **Nom** descriptif pour votre Action (par exemple, `Redirect to consent form`), sélectionnez le déclencheur `Login/Post Login` , car vous allez ajouter l’Action au flux de connexion, puis sélectionnez **Créer**.
...
``
4. Dans la barre latérale de l’éditeur de code des actions, sélectionnez Test (icône de lecture), puis sélectionnez **Exécuter** pour  [tester votre code](/actions/test-actions).
...
La page de connexion sera présentée par défaut sur `YOUR_DOMAIN/login`. Pour apprendre à utiliser votre propre domaine, consultez [Domaines personnalisés](/custom-domains).
...
6. Accédez à [Auth0 Dashboard > Gestion des utilisateurs > Utilisateurs](%24%7Bmanage_url%7D/#/users), et recherchez le nouvel utilisateur.
