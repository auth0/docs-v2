...
## Authentification forte du client (SCA) {#strong-customer-authentication-sca}
...
### Protéger les données sensibles dans le front channel {#protect-sensitive-data-in-the-front-channel}
...
#### Requêtes d’autorisation poussées (PAR) {#pushed-authorization-requests-par}
...
#### Requête d’autorisation sécurisée par JWT (JAR) {#jwt-secured-authorization-request-jar}
...
#### Protéger les données sensibles dans les jetons d’accès {#protect-sensitive-data-in-access-tokens}
