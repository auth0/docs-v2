...
<Steps>
  ...
  <Step title="Configure the Auth0 client" stepNumber={4}>
    ...
    <Warning>
      Replace `your-tenant.us.auth0.com` with your actual Auth0 domain and update the audience to match your API identifier.
    </Warning>
  ...
  <Step title="Create protected routes" stepNumber={5}>
    ...
    async def private():
        return jsonify({
            ...
            "user": g.user_claims["sub"]
        })


    # Protected endpoint with scope validation
    @app.route("/api/admin", methods=["GET"])
    @require_auth
    async def admin():
        scopes = g.user_claims.get("scope", "").split()
        
        if "read:admin" not in scopes:
...
<Check>
  **Checkpoint**
  ...
  - `/api/admin` - Requires authentication and the `read:admin` scope
</Check>
...
<Accordion title="Using Application Factory Pattern">
  For larger applications, use Flask's application factory pattern:
...
<Accordion title="DPoP Authentication (Early Access)">
  For enhanced security, enable DPoP (Demonstrating Proof-of-Possession):

  ```python
  # Configure API client with DPoP enabled
  api_client = ApiClient(ApiClientOptions(
      domain=os.getenv("AUTH0_DOMAIN"),
      audience=os.getenv("AUTH0_AUDIENCE"),
      dpop_enabled=True  # Default, accepts both Bearer and DPoP
  ))
...
<CardGroup cols={3}>
  ...
  <Card title="Python Best Practices" icon="python" href="https://auth0.com/docs/quickstart/backend/python">
    Additional Python integration patterns
  </Card>
...
A complete sample application is available in the SDK repository with additional features:
...
```bash
git clone https://github.com/auth0/auth0-api-python.git
cd auth0-api-python/examples/flask
pip install -r requirements.txt
