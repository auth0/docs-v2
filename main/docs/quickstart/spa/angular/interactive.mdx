...
<Steps>
  ...
  <Step title="Configure the Auth0 module" stepNumber={4}>
    ...
    <Note>
      Copy the `NG_APP_AUTH0_DOMAIN` and `NG_APP_AUTH0_CLIENT_ID` values from the `.env` file (created by the CLI script) into your environment file, or use the values from your Auth0 Dashboard if you set up manually.
    </Note>
  </Step>
