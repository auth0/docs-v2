...
<Steps>
  ...
  <Step title="Configure the Auth0 module" stepNumber={4}>
    ...
    <Tabs>
      <Tab title="Zone.js (default)">
        This example uses Zone.js for change detection, which is the default for Angular projects generated with the CLI.
      ...
      <Tab title="Zoneless (Angular 21+)">
        Angular 21 made zoneless change detection stable. Replace `provideZoneChangeDetection` with `provideZonelessChangeDetection` and remove `zone.js` from the `polyfills` array in `angular.json`.
        ...
        Also update `angular.json` to remove `zone.js` from polyfills:
