...
<Steps>
  <Step title="Create a new project" stepNumber={1}>
    ...
    <Info>
      If this is the first time you've run `@tanstack/cli` on your machine, npm will ask to confirm installing it: answer **y** to continue.
    </Info>
  ...
  <Step title="Add Tailwind CSS" stepNumber={3}>
    ...
    <Callout icon="file-lines" color="#0EA5E9" iconType="regular">
      `strictPort: true` makes Vite fail immediately instead of silently switching ports when 3000 is taken. Your Auth0 app's callback URL is locked to port 3000, so a silent port change would leave you stuck on a blank page after approving the login screen, since the browser gets redirected to a port nothing is listening on.
    </Callout>
  ...
  <Step title="Create project files" stepNumber={4}>
    Create the additional files you'll need for Auth0 integration:
  ...
  <Step title="Create the Auth0 server instance" stepNumber={6}>
    Add the Auth0 server instance to `src/auth.server.ts`. It reads its configuration from the environment variables you just set:
  ...
  <Step title="Update the root route" stepNumber={9}>
    ...
    <Callout icon="file-lines" color="#0EA5E9" iconType="regular">
      This replaces `createRootRoute` with `createRootRouteWithContext<RouterContext>()` so the route tree is typed with `context.auth0`.
    </Callout>
  </Step>
