...
<Steps>
  ...
  <Step title="Configure Auth0" stepNumber={3}>
    ...
    <Tabs>
      ...
      <Tab title="Dashboard">
        ...
        6. Copy the **Domain**, **Client ID**, and **Client Secret** from the Settings tab
        ...
        <Info>
          ...
          The **Issuer** must include `https://` and a trailing `/`. For example: `https://your-tenant.auth0.com/`.
        </Info>
      </Tab>
    </Tabs>
  ...
  <Step title="Create controllers and views" stepNumber={5}>
    Create the controllers, and the Thymeleaf templates they render.
...
After clicking login, Auth0 shows an error about a callback URL mismatch.
...
okta.oauth2.issuer: "https://your-tenant.auth0.com/"
...
After clicking logout, the user is immediately logged back in without seeing the Auth0 login page.
...
Open `http://localhost:3000` in your browser and click on the **Login** link to test the Auth0 login flow.
