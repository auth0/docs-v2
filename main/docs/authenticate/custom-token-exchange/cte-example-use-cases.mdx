...
This results in an `mfa_required` error that returns an MFA token:
...
With the `mfa_token` that is returned, the application can then call the MFA API to challenge and verify a factor:
...
The challenge generates the following response:
...
{/* TODO: insert <Frame>![](/docs/images/custom-token-exchange/<diagram-name>.png)</Frame> once the diagram and sample app are ready, matching the pattern used for the use case above. */}
...
3. The associated Custom Token Exchange Action validates the subject token, authorizes the delegation, and calls `api.authentication.setActor()` and `api.authentication.setUserById()` — the same pattern shown in [Support agent acting on behalf of an end user](#use-case-support-agent-acting-on-behalf-of-an-end-user) above. Calling `setActor()` is required when requesting a Session Transfer Token; omitting it returns a `400` error.
...
5. The support tool redirects the agent's browser to GearUp's web application, passing the Session Transfer Token through a cookie or URL parameter, the same way as with Native to Web SSO.
6. GearUp's web application establishes an ephemeral, time-limited session for the end-user, with the agent recorded as the actor for auditing.

<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
GearUp's web application must explicitly opt in to accept delegated sessions, and several session behaviors differ from a regular login (session lifetime, refresh tokens, MFA, and more). Read [Session Delegation](/docs/authenticate/single-sign-on/session-delegation) to configure your web application and understand these behaviors, limitations, and how to audit delegated sessions.
</Callout>

The following code exemplifies how to implement this in the Custom Token Exchange Action. In this case:

* We check the requested `audience` to tell a session delegation request apart from a plain API-access request, and apply a distinct authorization policy for each.
* We set the actor and the user using the same commands as [Support agent acting on behalf of an end user](#use-case-support-agent-acting-on-behalf-of-an-end-user) above — Auth0 decides whether to issue an access token or a Session Transfer Token based on the requested `audience`, not on anything the Action does differently.
...
exports.onExecuteCustomTokenExchange = async (event, api) => {
  ...
  // an access token, so a distinct authorization policy can be applied. See
  // "Important considerations for session delegation" below.
  const isSessionDelegation = event.resource_server?.identifier?.endsWith(":session_transfer");
  ...
  // 5. SET THE USER for the transaction (the end-user being acted upon)
  api.authentication.setUserById(payload.sub);
  ...
  async function checkSessionDelegationPolicy(agent, userId) {
    // Implement your session delegation policy check here. See "Important
    // considerations for session delegation" below for what to consider.
  }
};
```
...
* **Detect a session delegation request before applying your authorization policy.** `event.resource_server.identifier` reflects the requested `audience`, including the `urn:{yourDomain}:session_transfer` pseudo-audience — check whether it ends in `:session_transfer` to tell a session delegation request apart from a plain API-access request, and apply whatever policy differences your use case requires between the two.

* **Target application reachability depends on the specific connection your Action sets, not every connection the subject user belongs to.** When your Action calls `setUserByConnection()`, the resulting Session Transfer Token is scoped to that connection; when it calls `setUserById()` instead, the token is scoped to the user's primary connection. A target application can only redeem the token if that specific connection is enabled for it — not simply because the subject user happens to also belong to some other connection the target application allows. When selecting which target applications to offer for a given user, resolve the connection your Action will use, find the clients enabled for that connection, then filter to clients with `session_transfer.delegation.allow_delegated_access` set to `true`.

* **The application calling Custom Token Exchange needs access to the same connections as your target application(s).** Your Action's `setUserByConnection()`/`setUserById()` call fails if the calling application — not the target application — isn't itself permitted to use that connection. Easy to miss, since it's a setting on the actor's application rather than the target's.

* **Use separate Auth0 domains for the application initiating delegation and the target application.** If both call `/authorize` against the same Auth0 domain, the browser holds one shared session cookie for that domain — once the actor authenticates with the initiating application, that cookie already exists, so the browser is treated as having an active session when it's later sent to redeem the Session Transfer Token against the target application (see [Existing sessions block delegated access](/docs/authenticate/single-sign-on/session-delegation/session-delegation-behavior-and-monitoring#session-behavior)). Give the target application its own Auth0 domain — typically a [Custom Domain](/docs/customize/custom-domains) — distinct from the domain the initiating application uses, so each gets a separate browser cookie and neither interferes with the other. If you don't want to set up a Custom Domain, an alternative is to have the actor copy the target application's initiate-login URL, with the Session Transfer Token attached, and open it in a separate, manually-opened private/incognito browsing window — this avoids the shared-cookie problem without any additional domain configuration.

* **Agents must log out between delegated sessions for different users.** Because a delegated session occupies the same browser session slot on the target application's domain, an agent who starts a session for one user must log out of it before a delegated session for a *different* user can be established on the same target application. Build a visible logout affordance into your workflow if agents will delegate into the same target application for multiple users in sequence.
...
* Use Actions [`api.cache ()`](/docs/customize/actions/explore-triggers/signup-and-login-triggers/custom-token-exchange-trigger/custom-token-exchange-api-object) methods to avoid having to fetch the signing keys for each transaction.
