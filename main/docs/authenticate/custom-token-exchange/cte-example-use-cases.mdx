...
* **Use separate Auth0 domains for the initiating and target applications.** Sharing one Auth0 domain means both share one browser session cookie, so the browser already looks like it has an active session by the time it's sent to redeem the Session Transfer Token against the target application (see [Existing sessions block delegated access](/docs/authenticate/single-sign-on/session-delegation/session-delegation-behavior-and-monitoring#session-behavior)). Give the target application its own [Custom Domain](/docs/customize/custom-domains), distinct from the initiating application's domain, so each gets a separate cookie. Without a Custom Domain, an alternative is to have the support agent copy the target application's `initiate_login_url`, with the Session Transfer Token attached, into a separately-opened private/incognito browsing window.

* **Agents must log out between delegated sessions for different users.** Because a delegated session occupies the same browser session slot on the target application's domain, an agent who starts a session for one user must log out of it before a delegated session for a *different* user can be established.
