...
3. The CTE Action validates the subject token, verifies the actor has the right to act on behalf of the end-user, and calls `api.authentication.setActor()`.
...
4. The CTE Action executes to validate and control the exchange: it verifies the payment token, extracts the end-user identity, and applies authorization policies.
