...
* **Unified Access**: Users authenticate once to gain access to all independent, related applications without re-entering credentials.
* **Centralized Session**: Auth0 uses a central domain and browser cookies to share authentication state across multiple applications.
* **[Universal Login](/docs/authenticate/single-sign-on#sso-with-universal-login)**: Utilizing Auth0’s Universal Login is a secure method to implement SSO across all platforms.
...
Check a user's SSO status from an application by calling the `checkSession` method of the `auth0.js` SDK, which will attempt to [silently authenticate](/docs/authenticate/login/configure-silent-authentication) the user within an iframe. Whether the authentication is successful or not indicates whether the user has an active SSO cookie.
...
## Service-provider-initiated vs. identity-provider-initiated SSO

Deciding between service-provider-initiated and identity-provider-initiated SSO depends primarily on where you want the user journey to begin and your required level of security.

We recommend service-provider-initiated SSO when you need authentication for:
   * **Direct Access**: Use this when users typically bookmark your application’s login page or navigate to your page directly.
   * **Enhanced Security**: It is more secure because it uses a specific "Request ID" to correlate the login request with the response, protecting against Cross-Site Request Forgery (CSRF) and replay attacks.
   * **Deep Linking**: Use this if you need to redirect users to a specific page within your app after they log in.

Identity-provider-initiated SSO is common in many enterprises, and is often used for:
   * **Company Portals**: Use this if your users start their day at a centralized dashboard (like Okta, Microsoft 365, or a company intranet) and select an icon to launch your app.
   * **Simplifying User Experience**: It removes the need for the end user to visit your application's login page first; end users navigate to your application already authenticated.
   * **Legacy Systems**: Often used when integrating with older SAML-based enterprise systems that lack modern redirect capabilities.

## Use cases

### Business to Business SSO
