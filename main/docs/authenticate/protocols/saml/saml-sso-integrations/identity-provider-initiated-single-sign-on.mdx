...
redirects:
  - "/docs/configure/saml-configuration/saml-sso-integrations/identity-provider-initiated-single-sign-on"
  - "/docs/protocols/saml-protocol/saml-sso-integrations/identity-provider-initiated-single-sign-on"
  - "/docs/protocols/saml-protocol/saml-configuration-options/identity-provider-initiated-single-sign-on"
  - "/docs/protocols/saml/saml-configuration/special-configuration-scenarios/idp-initiated-sso"
  - "/docs/protocols/saml-configuration-options/identity-provider-initiated-single-sign-on"
  - "/docs/protocols/saml/idp-initiated-sso"
---
...
* **Response Protocol:** This is the protocol used to connect your selected **Default Application**. Most commonly, applications are configured with the OpenID Connect protocol (see above). However, if you have configured a SAML2 Web App addon for your application and want to route the SAML assertion you will need to select SAML. Once a valid SAML Assertion has been passed to the postback URL, Auth0 sends a login response to the configured default application’s first allowed callback URL using the chosen response protocol, which can be modified using the query string field to specify a `redirect_uri` if you use OIDC.
  * If the callback URL configured for the application includes a Multiple Custom Domains (MCD) placeholder, the system dynamically populates it using the metadata value that corresponds to the custom domain of the postback URL that received the inital request from the IdP. For more information, see [Multiple Custom Domains](/docs/customize/custom-domains/multiple-custom-domains).
