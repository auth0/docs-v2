...
Because Auth0 signs SAML assertions and responses with your tenant's signing key, rotating that key affects every SAML service provider on your tenant at once. Each service provider validates signatures against the certificate it trusts, so any service provider that does not trust the new certificate will reject Auth0's signed responses, and its users will not be able to log in.

For the general rotation procedure, see [Rotate Signing Keys](/docs/get-started/tenant-settings/signing-keys/rotate-signing-keys). The SAML-specific step that the general procedure does not cover is configuring each service provider to trust the *next* certificate **before** you rotate. How you do this, and whether rotation can be seamless, depends on whether each service provider can hold more than one identity provider signing certificate at a time.

Before rotating, get the next certificate: the key marked **Next in Queue** in [Dashboard > Settings > Signing Keys](https://manage.auth0.com/#/tenant/signing_keys), or the entry with `"next": true` returned by the [Get all Application Signing Keys](https://auth0.com/docs/api/management/v2#!/Keys/get_signing_keys) endpoint.

**Service providers that support multiple signing certificates**

Many service providers can trust several identity provider signing certificates at the same time. For these, add the next certificate alongside the current one so both are trusted simultaneously. This enables a **zero-downtime** rotation: the service provider keeps accepting signatures from the current key and automatically accepts the new key the moment Auth0 starts using it.
...
Some service providers accept only one identity provider signing certificate at a time. For these there is **no overlap window**: the service provider trusts either the old certificate or the new one, never both, so a brief disruption is unavoidable. Plan the change to keep it as short as possible.

1. Schedule the change during a low-traffic or maintenance window, and coordinate with the service provider administrator if it is managed by another team.
...
If you are unsure whether a service provider supports multiple signing certificates, check its SAML configuration for an option to add (rather than replace) a signing certificate. If it only allows a single certificate value, treat it as single-certificate above.

<Warning>

Refreshing SAML metadata is not sufficient to pre-establish trust. Auth0's metadata endpoint (`https://{yourDomain}/samlp/metadata/{clientId}`) publishes only the *currently active* certificate, never the next one, so you must add the next certificate to each service provider manually before rotating to achieve a seamless rotation. A service provider that refreshes metadata automatically can pick up the new certificate *after* you rotate, but it will reject Auth0's responses until that refresh happens, causing downtime you cannot control.
