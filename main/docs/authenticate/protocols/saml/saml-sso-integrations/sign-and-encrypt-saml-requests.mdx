...
Because Auth0 signs SAML assertions and responses with your tenant's signing key, rotating that key affects all of your service providers at once. Each service provider validates signatures against the certificate it trusts, so any service provider that does not trust the new certificate will reject Auth0's signed responses, and its users will not be able to log in.
