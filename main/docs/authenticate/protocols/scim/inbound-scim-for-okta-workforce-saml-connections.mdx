---
description: Walkthrough for Okta Connection Setup via SAML SCIM
...
| --- | --- | --- |
| **SAML application** | Uses the existing **Provisioning** tab for SAML applications in Okta. This method does not support the `PATCH` for groups, limiting the maximum group members to what can be sent in a single `POST` or `PUT` request. |  Apropriate for pushings users. Does not support pushing groups with more than 10,000 members. |
...
9. Use the **Assignments** tab to assign users and/or groups to your application. Assigning a group provisions its users into your Auth0 tenant.

10. To push groups, select the **Push Groups** tab, find and select the group to push, then select **Save**. For more information, read [Okta's App assignments and Group Push documentation](https://help.okta.com/en-us/content/topics/users-groups-profiles/usgp-group-push-main.htm).
