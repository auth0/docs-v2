...
description: Configure Inbound SCIM for older Azure AD connections that use pairwise subject identifiers, using update and delete-only provisioning mode.
---
...
For new connections, read [New Connections](/docs/authenticate/protocols/scim/inbound-scim-for-new-azure-ad-connections).
...
3. Select the **Provisioning** tab and enable both **Sync user profile attributes at each login** and **Sync user profiles using SCIM**.
4. On the **Mapping** tab, ensure the **SCIM attribute containing the User ID** setting is set to **externalId**.

   <Callout icon="file-lines" color="#0EA5E9" iconType="regular">
     Auth0 uses `externalId` to match incoming SCIM requests to existing users. In a later step, you will configure Azure to send `objectId` as the value of `externalId`. This means Auth0 matches users by their Azure `objectId` — a stable, unique identifier — rather than by the pairwise subject identifier used for SSO. This separation is what makes it possible to use SCIM with older connections where pairwise IDs are already in use.
   </Callout>
...
11. On the **Edit Attribute** screen, change **Match objects using this attribute** to **Yes,** then set **Matching precedence** to **2** and choose **OK.**
