...
You can assign and remove [roles](/docs/manage-users/access-control/rbac) for <Tooltip tip="System for Cross-domain Identity Management (SCIM): A standardized protocol for provisioning and managing user identities and groups across systems." cta="View Glossary" href="/docs/glossary?term=SCIM">SCIM</Tooltip>-provisioned enterprise groups using the Auth0 Dashboard or Management API. When roles are assigned to a group, all members inherit those roles when they log in. You can manage group roles at two scopes:

* **Organization groups** — Enterprise groups within a specific [organization](/docs/manage-users/organizations). Requires [Auto-Membership](/docs/manage-users/organizations/configure-organizations/grant-just-in-time-membership) to be enabled on the enterprise connection for role inheritance to take effect.
...
```bash wrap lines
curl --request POST \
  --url 'https://{yourDomain}/api/v2/organizations/<org_id>/groups/<group_id>/roles' \
...
```bash wrap lines
curl --request POST \
  --url 'https://{yourDomain}/api/v2/groups/<group_id>/roles' \
  --header 'authorization: Bearer YOUR_MGMT_API_ACCESS_TOKEN' \
  --header 'content-type: application/json' \
  --data '{ "roles": [ "<role_id>" ] }'
```
