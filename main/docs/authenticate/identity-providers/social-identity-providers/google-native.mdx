...
```go lines expandable
// Generate a cryptographically secure nonce for token replay protection
val nonceBytes = ByteArray(16) // 128 bits of entropy
...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
Review the following recommendations on using `nonce`:
- Use a CSPRNG to generate the nonce. Call `java.security.SecureRandom` directly rather than general-purpose random sources. A nonce requires at least 128 bits of entropy. To learn more, read [OWASP Cryptographic Storage Cheat Sheet](https://cheatsheetseries.owasp.org/cheatsheets/Cryptographic_Storage_Cheat_Sheet.html#secure-random-number-generation).
- Generate a fresh nonce for each authentication request. A reused nonce defeats the replay-attack protection it provides.
- Do not log the nonce. Logging it exposes the value to anyone with log access and defeats its replay-attack protection.
</Callout>
