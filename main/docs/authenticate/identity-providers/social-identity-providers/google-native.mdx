...
<Tip>
- **Use a CSPRNG to generate the nonce.** Call `java.security.SecureRandom` directly rather than general-purpose random sources. A nonce requires at least 128 bits of entropy. See the [OWASP Cryptographic Storage Cheat Sheet](https://cheatsheetseries.owasp.org/cheatsheets/Cryptographic_Storage_Cheat_Sheet.html#secure-random-number-generation).
- **Generate a fresh nonce for each authentication request.** Reusing a nonce defeats the replay-attack protection it provides.
