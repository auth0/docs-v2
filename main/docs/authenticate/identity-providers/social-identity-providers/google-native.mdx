...
- **Protect tokens in logs.** Do not write the Google ID token or the Auth0 access token to application logs, crash reports, or analytics. Either value in a log is a usable credential.
- **Use TLS 1.3.** Verify that your application uses TLS 1.3 or later for all outbound connections. Android enforces this by default on API level 29 and above. For earlier API levels, add a `network_security_config.xml` to restrict accepted TLS versions.
- **Restrict your Google OAuth credentials.** In Google Cloud Console, bind your Android OAuth client to the SHA-1 fingerprint of your release certificate, and restrict the web OAuth client (`client_id_web`) to the origins that need it. Do not share OAuth clients across unrelated applications. To learn more, read [Google's OAuth 2.0 security best practices](https://developers.google.com/identity/protocols/oauth2/security-best-practices).
</Callout>
