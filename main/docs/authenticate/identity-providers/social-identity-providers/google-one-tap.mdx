...
- Do not write Auth0 access tokens, refresh tokens, or ID tokens to application logs or analytics. Treat them as passwords.
- Confirm that all connections between your application and Auth0 use TLS 1.3 or later. Most modern hosting providers enforce this by default. Verify it in your platform settings.
- Set `X-Frame-Options: DENY` or a `frame-ancestors 'none'` Content Security Policy directive on your application's pages to prevent clickjacking. Auth0 Universal Login already applies these headers; apply them to your own application as well.
- Apply a strict Content Security Policy to your application and encode all user-supplied content before rendering it. A CSP limits what scripts can run and what resources load. To learn more, read [application session management best practices](https://auth0.com/blog/application-session-management-best-practices/).
- In Google Cloud Console, scope your OAuth client to the domains your application uses. Review [Google's OAuth 2.0 security best practices](https://developers.google.com/identity/protocols/oauth2/security-best-practices) before going to production.
</Callout>
