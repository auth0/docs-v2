...
Auth0's Google One Tap integration does not support [Token Vault](/docs/secure/call-apis-on-users-behalf/token-vault#token-vault) or [Connected Accounts for Token Vault](/docs/secure/call-apis-on-users-behalf/token-vault/connected-accounts-for-token-vault). If you enable Google One Tap with Token Vault enabled, the Google One Tap prompt does not appear.
</Callout>
...
Use the following examples to add Google One Tap to Universal Login with an Auth0 SDK. For the best experience, we advise using the most recent version of Auth0's SDKs.
...
Google One Tap requires browser support for the FedCM API. Auth0 passes `"itp_support: true"` to the One Tap library by default. It is supported in Chrome and Chromium-based browsers. When browsers, such as iOS and Firefox, do not support FedCM, [a different UX is provided](https://developers.google.com/identity/gsi/web/guides/features#upgraded_ux_on_itp_browsers).

## Security considerations

<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
Review the following security recommendations:
- **Protect tokens in logs.** Do not write Auth0 access tokens, refresh tokens, or ID tokens to application logs or analytics. Either value in a log is a usable credential.
- **Use TLS 1.3.** Confirm that all connections between your application and Auth0 use TLS 1.3 or later. Most modern hosting providers enforce this by default. Verify it in your platform settings.
- **Set frame protection headers on your application.** To prevent clickjacking, set `X-Frame-Options: DENY` or a `frame-ancestors 'none'` Content Security Policy directive on your application's pages. Auth0 Universal Login already applies these headers; apply them to your own application as well.
- **Apply a Content Security Policy and sanitize user input.** A strict CSP limits what scripts can run and what resources load in your application. Encode all user-supplied content before rendering it. To learn more, read [application session management best practices](https://auth0.com/blog/application-session-management-best-practices/).
- **Follow Google's configuration recommendations.** In Google Cloud Console, scope your OAuth client to the domains your application uses. Review [Google's OAuth 2.0 security best practices](https://developers.google.com/identity/protocols/oauth2/security-best-practices) before going to production.
</Callout>
