...
Auth0's Google One Tap integration does not support [Token Vault](/ai/docs/call-apis-on-users-behalf/token-vault#token-vault) or [Connected Accounts for Token Vault](/ai/docs/call-apis-on-users-behalf/token-vault/connected-accounts-for-token-vault). If you enable Google One Tap with Token Vault enabled, the Google One Tap prompt does not appear.
</Callout>
