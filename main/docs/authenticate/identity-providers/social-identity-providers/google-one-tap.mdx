...
sequenceDiagram
    ...
    User->>Auth0: 1. Arrives at Universal Login
    Auth0-->>User: 2. Displays One Tap overlay
    User->>Google: 3. Selects Google account
    Google-->>Auth0: 4. Returns signed ID token
    Auth0->>Auth0: 5. Validates ID token
    Auth0->>Auth0: 6. Creates or updates user profile, issues tokens
    Auth0-->>App: 7. Redirects user with tokens
```
