...
10. If **SCIM 2.0** is selected, set the following fields under **SCIM provisioning properties**:<p></p>
    ...
    C. Set **Link to configuration guide** to link to your customer-facing instructions on how to configure SSO between Okta and your app with Express Configuration.
...
12. Proceed to the [Request an express configuration integration]() section. You may close the browser window containing the Okta console.

### Request an Express Configuration integration

Send an email to the Okta Express Configuration team at `expressconfig@okta.com` with the information copied from the [Auth0 Dashboard > Applications > Okta Integration Network > Request Integration](https://manage.auth0.com/dashboard/#/applications/#/okta-integration-network/wizard) section of the Auth0 Dashboard.
...
Include the name of your Okta Free Integrator Org, ex: `trial-7513274`, as well as the Okta Display name of your OIN Integration in the email.
...
5. Set **Account URL** to the login page of the application instance for testing.
...
14. To test SCIM: select **Provisioning**, and then **Express Configure SCIM**.
15. Once you are redirected to Auth0's Universal Login, you are prompted to consent. When complete, the SCIM integration is configured automatically.
16. Next, select the **Assignments** tab and assign a user from your Okta Free Integrator organization. If SCIM is enabled, the user should be provisioned to your Auth0 tenant. Confirm using the Auth0 Dashboard.
17. To test SSO: Use the user account you just assigned and the test instance. Log in with the user account credential.
18. To test Universal Logout: follow the instructions on [Test Universal Logout](/docs/authenticate/login/logout/universal-logout#test-universal-logout).
