...
* A [Connection Profile](/docs/authenticate/enterprise-connections/connection-profile)
...
<Frame>![Configure Okta OIN](/docs/images/cdy7uua7fh8z/express-configuration-oin.png)</Frame>
 ...
 1. On the **Configure your integration** screen of the OIN Wizard, select **Enable Express Configuration**. This shows a dialog asking for information you retrieve from your Auth0 tenant.
...
1. In the **Express Configuration for Auth0 apps** dialog of the OIN Wizard, choose **Download Key (.pem)** to save the key to your local device.
...
1. Under **OIDC Properties**, set the following fields:<p></p>
    A. **Redirect URIs** should be automatically set to the callback URI for your Auth0 tenant. This can use your tenant’s custom domain or your `auth0.com` domain. <br></br>Example: `https://tenant.auth0.com/login/callback`<p></p>
    B. (Optional) Set **Initiate Login URI** should be automatically set to the same value you configured for your application in Auth0, as described in Auth0 application and initiate login URI. Okta uses this URL to launch your application from the end-user dashboard. <p></p>
...
2. If **Universal Logout** is selected, confirm the following values are set under Universal logout properties:<p></p>
    A. The **Global token revocation** endpoint should be automatically set. This value will be dynamically replaced when Express Configuration runs.<p></p>
    B. The **Subject format** should be automatically set to **Issuer and Subject identifier**.<p></p>
...
3. If **SCIM 2.0** is selected, confirm the following values are set under **SCIM provisioning properties**:<p></p>
    A. The **Base URL** should be set to an integration variable. This value will be dynamically replaced when Express Configuration runs.<p></p>
    B. The **User Operations** should be automatically set to **Create**, **Read** **Update**, and **Deactivate**.<p></p>
...
After the public key is uploaded to Auth0, you can test Express Configuration using your Okta Free Integrator organization:
