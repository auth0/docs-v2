...
| `LDAP_SEARCH_QUERY` | The LDAP query used to find users in the LDAP store. The `{0}` token is replaced at runtime with the search string. Filters are required for the search to work as expected. If not configured, Auth0 does not send blocked account notification emails. | `(&(objectCategory=person)(anr={0}))` |
