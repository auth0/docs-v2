...
| `LDAP_SEARCH_QUERY` | The LDAP query used to find users in the LDAP store. [Filters](https://github.com/auth0/ad-ldap-connector/blob/master/lib/users.js#L364) are required for the search to work as expected. If not configured, Auth0 does not send [blocked account](/docs/secure/attack-protection/brute-force-protection#email) notification emails. | `(&(objectCategory=person)(anr={0}))` |
