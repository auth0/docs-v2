...
| `plain` | Uses plaintext as described in the [PKCE specification](https://www.rfc-editor.org/rfc/rfc7636#section-4.2). |
...
If you are using an OpenID Connect connection for Microsoft Entra ID, you must set `pkce` to `s256` as the connection's metadata does not expose the hashing algorithm used. Currently, the [Microsoft Entra ID Enterprise connection](/docs/authenticate/identity-providers/enterprise-identity-providers/azure-active-directory/v2) does not support PKCE.
...
Mapped claims are not automatically added to an Auth0 ID token. To add claims to an ID token, read [Create custom claims](/docs/secure/tokens/json-web-tokens/create-custom-claims#create-custom-claims).
...
| `userinfo_scope` | Optional | Scopes to request from the IdP during authorization, which determine what claims are available from the UserInfo endpoint. |
...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
Okta Workforce does not use `bind_all` mapping. You need to use an OIDC Enterprise connection instead. To learn more, read [Arrays Mapped to String in OIDC or Okta Workforce Connection](https://support.auth0.com/center/s/article/Arrays-mapped-to-string-in-OIDC-or-Okta-Workforce-connection).
</Callout>
...
The `userinfo_scope` property defines the scopes that Auth0 includes in the authorization request to the IdP. These scopes determine what claims the IdP makes available from its `UserInfo` endpoint. When the attribute map references `context.userinfo properties`, Auth0 calls the IdP’s `UserInfo` endpoint using the access token granted with these scopes.

For example, if you want to request the standard OIDC scopes and the `groups` scope so that group claims are available from the `UserInfo` endpoint, you can configure it as follows:
...
| --- | --- |
| `context.connection` | Contains the following properties:<br/><br/><li> `id`: The connection’s unique identifier (for example, `con_4423423423432423`).</li><br/><li> `strategy`: The connection’s strategy (for example, `oidc`).</li> |
| `context.tokenset` | Contains the following properties:<br/><br/><li> `access_token`: The entire validated access token sent by the IdP.</li><br/><li>`<claim name>`: Any ID token claim sent by the IdP.</li> |
| `context.userinfo` | Contains the following properties:<br/><br/><li> `<claim name>`: Any available claim provided by the IdP’s UserInfo endpoint.</li> |
