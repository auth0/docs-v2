...
When you promote a connection to domain level, any [third-party application](/docs/get-started/applications/third-party-applications) in your tenant can use it to authenticate users. This is the primary mechanism for enabling third-party applications to access your identity providers — if a connection is not promoted to domain level, third-party applications cannot use it unless they have an explicit client-to-connection mapping.

Promoting connections to domain level is especially important when you use [Dynamic Client Registration](/docs/get-started/applications/dynamic-client-registration), because dynamically registered clients cannot be individually mapped to connections at creation time.
