...
For connections associated with an Organization, Organization Admins can self-service configure Cross App Access (XAA) for the connection by setting the `cross_app_access_resource_app` attribute through the [My Organization API](/docs/manage-users/my-organization-api). To learn more, read [Connection Profile](/docs/authenticate/enterprise-connections/connection-profile).
