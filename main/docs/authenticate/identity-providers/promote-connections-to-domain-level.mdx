...
When you promote a connection to the domain level, any [third-party application](/docs/get-started/applications/third-party-applications) in your tenant can use it to authenticate users. This enables third-party applications to access your identity providers. Third-party applications can only use domain-level connections, except when accessing via [Cross App Access (XAA)](/docs/secure/call-apis-on-users-behalf/xaa), where access is instead authorized in the corresponding Enterprise IdP.
