...
4. In the **Settings** section, configure additional options as needed for the new connection:
   ...
   * **Enable as a domain level connection**: Allow third-party applications to use the connection; requires [Dynamic Client Registration](/docs/get-started/applications/dynamic-client-registration).
   * **Allow IT admin to configure third-party application access**: When enabled, the IT admin sees an option in the setup wizard to allow third-party applications to use the connection. If you want to set this value yourself rather than delegating it, use the **Enable Third-Party Client Configuration** toggle instead. These two options are mutually exclusive.
...
7. Under **Provisioning**, optionally enable **Sync Users and Groups through Provisioning**. When enabled, additional configuration is available:
