...
* The connection must not be configured to use [Token Vault](/ai/docs/call-apis-on-users-behalf/token-vault/configure-token-vault).
...
| ES384 | ECDSA with P-384 curve and SHA-384 | Your identity provider requires ES384. |
| ES512 | ECDSA with P-521 curve and SHA-512 | Your identity provider requires ES512. |
| Ed25519 | EdDSA with Curve25519 | Your identity provider requires Ed25519 for compliance. |

Choose ES256 unless your identity provider specifically requires a different algorithm.

## Enable DPoP

To enable DPoP and choose an algorithm, use the Auth0 Dashboard or Management API.

<Tabs><Tab title="Auth0 Dashboard">

In Auth0 Dashboard:
1. Navigate to **[Authentication > Enterprise](https://manage.auth0.com/dashboard/#/connections/enterprise/)**. Select the connection you want to configure.
2. Select the **Credentials** tab.
3. Select the checkbox for **Enable Demonstrating Proof of Possession (DPoP)**.
4. Select the menu under Signing Algorithms for DPoP and choose your algorithm.
<Frame>![Enable DPoP and select an algorithm](/docs/images/cdy7uua7fh8z/enable-dpop-dashboard.png)</Frame>
5. Select **Save**.
...
To use the Management API, you need to get a [Management API access token](/docs/secure/tokens/access-tokens/management-api-access-tokens).
...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
 If you `PATCH` the `options` parameter, the entire `options` object is overridden. To avoid partial data or other issues, ensure all wanted properties are present when you PATCH `options`.
</Callout>
...
A successful transaction in a log entry should be similar to:
...
The values `dpop_signing_alg` and `idp_token_type: "dpop"` confirm Auth0 sent a DPoP proof using the configured algorithm and your IdP issued DPoP-bound tokens. The `upstream_userinfo_fetch` object is only present if the [User Information](https://auth0.com/docs/api/authentication/user-profile/get-user-info) endpoint is called. The `dpop_bound` field is only present if the `GET` request to the `/userinfo` endpoint is successfully DPoP-bound.

## Disable DPoP

You can disable DPoP using the Auth0 Dashboard or the Management API.

<Tabs><Tab title="Auth0 Dashboard">

1. Navigate to [**Authentication > Enterprise**](https://manage.auth0.com/dashboard/#/connections/enterprise/). Select the connection you want to configure.
2. Select the **Credentials** tab.
3. Clear the **Enable Demonstrating Proof of Possession (DPoP)** checkbox.
4. Select **Save**.
...
* Verify the algorithm is one of the supported values: **ES256**, **ES384**, **ES512**, or **Ed25519**.
...
* Unsupported algorithm. Auth0 supports ES256, ES384, ES512, and Ed25519. If `dpop_signing_alg` is set to an unsupported value (for example, RS256), DPoP is silently disabled. No error is logged. Update the connection to use ES256, ES384, ES512, or Ed25519.
...
Note the failure does not include the `idp_token_type` because Auth0 did not receive a token from your IdP.
...
Check the IdP supports DPoP and if the configured algorithm (ES256, ES384, ES512, or Ed25519) is in the supported list. You can find out by accessing the IdP's OpenID Connect discovery document:

```curl
curl https://YOUR_IDP_DOMAIN/.well-known/openid-configuration
```

In the IdP response, look for `dpop_signing_alg_values_supported`. If this field is missing, the IdP may not support DPoP. If the field lists only algorithms that Auth0 does not support (for example, only RS256), DPoP cannot be used with this connection. You should disable DPoP for this connection or contact your IdP to enable support for ES256, ES384, ES512, or Ed25519.
