...
<Note>
We recommend migrating to the Use Attribute Profile from the existing User Profile for a more robust and extensible experience.
</Note>
Auth0 offers a robust profile to standardize and map user attributes across identity protocols, such as OIDC, SAML, and SCIM. Use the User Attribute Profile (UAP) to define a set of specific attributes (email, family name, department, etc.) and standardize labels, requirements, and strategy-specific overrides for other IdPs, like Okta and Entra ID. The UAP ensures that attributes are correctly mapped to Auth0 once the connection is created. To learn more, read [User Attribute Profiles](/authenticate/enterprise-connections/user-attribute-profiles.mdx).
