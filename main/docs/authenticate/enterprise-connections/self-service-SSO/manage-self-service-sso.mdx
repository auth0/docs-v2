...
* **Self-service profile**: Defines key elements of customer SSO implementations, including the <Tooltip tip="Identity Provider (IdP): Service that stores and manages digital identities." cta="View Glossary" href="/docs/glossary?term=identity+providers">identity providers</Tooltip> (IdPs) they can use and which user attributes they must capture, such as email. You can create up to 20 profiles in your tenant for different customers or segments.
   * Include a **[User Attribute Profile (UAP)]**(/authenticate/enterprise-connections/user-attribute-profiles.mdx) as a more robust and extensible profile to standardize and map user attributes across protocols, such as OIDC, SAML, and SCIM. Use the User Attribute Profile (UAP) to define a set of specific attributes (email, family name, department, etc.) and standardize labels, requirements, and strategy-specific overrides for other IdPs, like Okta and Entra ID. The UAP ensures that attributes are correctly mapped to Auth0 once the connection is created.
...
<tr>
<td><code>user_attribute_profile_id</code>
</td>
<td>
No.
</td>
<td>
ID of the [User Attribute Profile](//authenticate/identity-providers/enterprise-identity-providers/user-attribute-profile) to associate with self-service accounts.
</td>
</tr>
</tbody>
</table>
...
<tr>
<td><code>provisioning_config</code>
</td>
<td> **Optional.** Object. <br><br> Determines whether or not customer admin is able to set up SCIM. If the connection is created without all provisioning `scopes`, `get:users`,`post:users`,`put:users`, `patch:users`,`delete:users`, SCIM will not be enabled.
</td>
</tr>
<tr>
<td><code>ttl_sec</code></td>
