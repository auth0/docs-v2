...
| `organization.assign_membership_on_login` | Specifies whether users should automatically be assigned membership in the organization on login. This maps to the `assign_membership_on_login` property of the `enabled_connections` sub-resource of the organization. <Note> For Express Configuration integrations with Okta, `optional` is treated as `required`</Note> <p></p>| Connection Profile Value | Enable Connections`assign_membership_on_login`Value |
| --- | --- |
| `none` | `false` |
| `optional` | `false` |
| `required` | `true` ||
|`organization`|  An object containing settings that are applied to the organization under which the connection is created. |
| `organization.show_as_button` | Specifies whether the connection should be shown as a button on the login screen once an organization has been chosen. This maps to the `show_as_button` value on the enabled connection. | Connection Profile Value | Connection `show_as_button` Value |
| --- | --- |
| `none` | `false` |
| `optional` | `true` |
| `required` | `true` ||
