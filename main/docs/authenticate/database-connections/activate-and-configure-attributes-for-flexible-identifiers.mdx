...
To revert a connection back to its original state, use the <Tooltip tip="Management API: A product to allow customers to perform administrative tasks." cta="View Glossary" href="/docs/glossary?term=Management+API">Management API</Tooltip> to pull all existing values in the `options` object then updating the connection with existing values with the exception of the `attributes` parameter. 

First, pull existing `object` values by calling the [Get a connection](https://auth0.com/docs/api/management/v2/connections/get-connections-by-id) endpoint. Then add the values in a call to the [update a connection](https://auth0.com/docs/api/management/v2/connections/patch-connections-by-id) endpoint with an `options` object that does not include the `attributes` parameter, as shown below.
...
{
"options": {
  //except for the `attributes` parameter, copy existing parameters for this object here
       }
}
```
