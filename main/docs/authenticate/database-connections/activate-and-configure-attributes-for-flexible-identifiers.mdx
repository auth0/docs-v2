...
You can configure a list of phone country codes, using a PATCH call to the [/api/v2/tenants/](/docs/api/management/v2/tenants/patch-settings#body-country-codes-one-of-0) endpoint.
...
To revert a connection back to its original state, use the <Tooltip tip="Management API: A product to allow customers to perform administrative tasks." cta="View Glossary" href="/docs/glossary?term=Management+API">Management API</Tooltip> to pull all existing values in the `options` object. Then update the connection with existing values. 

First, pull existing `options` values by calling the [Get a connection](/docs/api/management/v2/connections/get-connections-by-id) endpoint. Then add the values in a call to the [Update a connection](/docs/api/management/v2/connections/patch-connections-by-id) endpoint with an `options` object that does not include the `attributes` parameter, as shown below.
...
{
"options": {
  //copy existing parameters for the `options` object here, except for the `attributes` parameter
       }
}
```
