...
For end users to enroll and challenge with a single passkey across application types or across the same application type with different subdomains, you need to set the Relying Party ID (RPID) to the parent or root domain. Your native or mobile application served at `app.example.com` and your web application served at `login.example.com` have an root domain of `example.com`. If you set your RPID to `example.com`, your end users can authenticate on both applications with the same passkey tied to the `example.com` domain.
