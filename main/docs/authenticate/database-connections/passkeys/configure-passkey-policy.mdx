...
<Frame>![](/docs/images/cdy7uua7fh8z/51aI3GgLqE4rTw93V26DZ0/73a0fe3efc42e2963586a01718f36ab7/Passkey_policy_-_English.png)</Frame>
...
<Warning>
Relying Party ID for Passkeys is in Limited Early Access. By using this feature, you agree to the applicable Free Trial terms in Okta’s [Master Subscription Agreement](https://www.okta.com/legal).
</Warning>

For end users to enroll and challenge across application types, you need to set the Relying Party ID (RPID) to the parent or root domain. Your native or mobile application served at `app.example.com` and your web application served at `login.example.com` have an origin of `example.com`. If you set your RPID to `example.com`, your end users can authenticate on both applications with the same passkey tied to the `example.com` domain.

<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
When you customize the RPID in Auth0 Dashboard or Management API, all passkeys for other domains will be invalidated and your end users must re-enroll.
</Callout>
...
4. Enter the domain.
5. If this RPID is the origin domain, select the checkbox to set the domain as the default for your tenant.
6. Select **Save**.
...
<Frame>![](/docs/images/cdy7uua7fh8z/Q8xHHuKUvs4BMAJOKvq6M/9b21eedf85e43080cdbe673c97ffc378/login-passkey_default.png)</Frame>
