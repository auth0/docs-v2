...
* Disable custom login pages. In the Auth0 Dashboard, go to [Branding > Universal Login > Advanced Options > Login](https://manage.auth0.com/#/login_page), and disable the **Custom Login Page** toggle.
...
* Either disable [using your own database](/docs/authenticate/database-connections/custom-db/create-db-connection) or, alternatively, use your own database with [import mode enabled](/docs/manage-users/user-migration/user-migration-scenarios). To use your own database with import mode disabled, follow the [additional prerequisites below on updating your database action scripts](#early-access-use-your-own-database-with-import-mode-enabled).

Database connections with passkeys enabled must also still have passwords enabled. This ensures users can continue to access their accounts from browsers and older devices that may not yet support passkeys, and retains 
...
<Steps titleSize="h3">
    ...
    If the save button is disabled, use the **Passkey Authentication Prerequisites** section to confirm that your tenant and database connection are configured correctly.
    </Step>
</Steps>
