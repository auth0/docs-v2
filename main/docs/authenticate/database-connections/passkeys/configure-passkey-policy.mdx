...
5. _Optional_. Review Relying Party IDs (RPID) to ensure passkeys are bound to the correct origin: <br></br>
    A. Select the Relying Party ID link in the Passkey panel or navigate to [Auth0 Dashboard > Tenant Settings](https://manage.auth0.com/#/tenant/relying_party_ids). <br></br>
    B. If the Review Relying Party ID is inaccurate, navigate to re-verify your [custom domain](//docs/customize/custom-domains/configure-features-to-use-custom-domains) with your domain origin (e.g. `acme.com`) without suffix, schemas, or ports.
