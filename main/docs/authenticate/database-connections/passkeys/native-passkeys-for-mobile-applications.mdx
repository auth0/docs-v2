...
For end users to authenticate with a single passkey with across different application types or application types with different subdomains, set the Relying Party ID to the root or parent domain. To learn more, review [Passkeys](/docs/authenticate/database-connections/passkeys).
