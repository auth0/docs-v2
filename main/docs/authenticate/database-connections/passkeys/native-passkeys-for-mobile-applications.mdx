...
Native passkeys is currently available in limited Early Access. By using this feature, you agree to the applicable Free Trial terms in [Okta’s Master Subscription Agreement](https://www.okta.com/legal/). To learn more about Auth0 releases, review [Product Release Stages](/docs/troubleshoot/product-lifecycle/product-release-stages).
...
To enable passkey sharing across application types with Relying Party ID (RPID), make sure yourS native application's metadata matches the custom domain's origin. For example, if your native application is served at `app.example.com`, your custom domain origin should be `example.com`.
</Callout>
...
For end users to authenticate with a single passkey with your native application and web application, verify your native application is authorized to use the same RPID as your web domain. To learn more, review [Passkeys](/docs/authenticate/database-connections/passkeys).
...
3. In response, Auth0 returns [`PublicKeyCredentialCreationOptions`](https://www.w3.org/TR/webauthn-3/#dictdef-publickeycredentialcreationoptions) along with an `auth_session` ID and `rpId`:
   ...
   ```
   <Callout icon="file-lines" color="#0EA5E9" iconType="regular">
    The returned `rpId` in the response must match the domain used in the webflow for passkeys to work in associated domains. 
   </Callout>
