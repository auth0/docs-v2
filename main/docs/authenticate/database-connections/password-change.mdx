...
Auth0 allows different ways to reset the password for a user in your database. The following instructions work only if the user's email address is known.

| Connection type | Where to reset password |
| --- | --- |
| [Database](/docs/authenticate/database-connections) | Use the methods described on this page. |
| [Social](/docs/authenticate/identity-providers/social-identity-providers) | Must reset with the <Tooltip tip="Identity Provider (IdP): Service that stores and manages digital identities." cta="View Glossary" href="/docs/glossary?term=identity+provider">identity provider</Tooltip> (such as Google or Facebook). |
| [Enterprise](/docs/authenticate/identity-providers/enterprise-identity-providers) | Must reset with the <Tooltip tip="Identity Provider (IdP): Service that stores and manages digital identities." cta="View Glossary" href="/docs/glossary?term=identity+provider">identity provider</Tooltip>. |
...
The language of the password reset email is determined by the user's tenant language settings, not by the language selected in your application. To send the reset email in a specific language, you can:

* Pass a `client_id` for a tenant/application configured with the desired language, so Auth0 uses that application's language settings when rendering the email template.
* Localize the email template yourself using [Liquid syntax](/docs/customize/email/email-templates) in the Change Password email template, branching on a value such as the user's `user_metadata.language`. Store the user's selected language in `user_metadata` (for example, via the Management API) before triggering the reset so the template can reference it.

To learn more about customizing and localizing email templates, read [Customize Email Templates](/docs/customize/email/email-templates).
...
## Redirect end users after password resets
...
3. In the red **Change Password** box, select **Change**.
   ...
   <Frame>![Manually set password](/docs/images/cdy7uua7fh8z/5mrEG3UtlZW47AnTikwIGv/94d186b748a53c13145388fa281af8c4/dashboard-users-edit_view-details_danger-zone__1_.png)</Frame>
