...
3. The user is presented with the most suitable option based on the `default_method` and available authentication methods (e.g., receive an OTP by email or phone or provide a password).
...
4. Select **Save**.
5. Configure settings authentication methods that correspond to the chosen identifiers. You cannot disable password unless you have `phone_otp` and/or `email_otp` configured.
