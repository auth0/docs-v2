...
Auth0 allows you to configure email and phone-based passwordless authentication directly on a [database connection](/docs/authenticate/database-connections). Rather than creating a separate connection for one-time password (OTP) authentication, you can offer passwordless login directly from your database connection. This reduces implementation complexity and simplifies the login experience for end-users. For API-based authentication, read [Passwordless Authentication on Database Connections with Authentication API](/docs/authenticate/database-connections/auth-api-passwordless-db-connections).

<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  Passwordless for database connections is not supported for use with Classic Login. To use passwordless authentication on database connections with APIs, read [Passwordless Authentication on Database Connections with Authentication API](/docs/authenticate/database-connections/auth-api-passwordless-db-connections).
</Callout>
...
1. An end user enters an identifier to login. Passwordless authentication works with Auth0 Universal Login or via [Authentication API](/docs/authenticate/database-connections/auth-api-passwordless-db-connections).
...
* Configure your tenant to use the [Identifier-First Authentication Profile](/docs/authenticate/login/auth0-universal-login/identifier-first).
...
The `post-login` trigger fires after a user authenticates but before the authorization server returns a token. The `event.authentication` object in [Auth0 Dashboard > Actions > Triggers > Post Login](https://manage.auth0.com/#/actions/triggers) exposes the following methods:
