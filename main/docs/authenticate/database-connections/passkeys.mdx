...
WebAuthn binds credentials like passkeys to a specific origin identified by the [Relying Party ID attribute](https://www.w3.org/TR/webauthn-2/#relying-party-identifier) to make sure authentication is sucessful with the intended service provider. For example, if your web application is served at `accounts.acme.com` and your native application served at `app.acme.com`, you can configure the <Tooltip tip="Relying Party: Entity (such as a service or application) that depends on a third-party identity provider to authenticate a user." cta="View Glossary" href="/docs/glossary?term=Relying+Party">Relying Party</Tooltip> ID to `acme.com`. This lets end users authenticate to any `acme.com` domain with a single passkey.
