---
title: Login Database Action Script and Templates
sidebarTitle: Login
description: The Login script runs when a user tries to log in.
---
...
If the user authenticates successfully, their profile data must be returned in the `profile` object in [normalized form](/docs/manage-users/user-accounts/user-profiles/normalized-user-profile-schema). For example:
...
For example, you can return an error with a value for the `userNameOrEmail` field

```js wrap
return callback(new WrongUsernameOrPasswordError(userNameOrEmail, "My custom error message"));
```

In this case, Auth0 records an `fp` tenant log event.
...
You can also return an error with a `null` value for the `userNameOrEmail` field:
...
function login(identifierValue, password, callback) {
  ...
  connection.on('connect', function(err) {
    ...
    getMembershipUser(identifierValue, function(err, user) {
      if (err || !user || !user.profile || !user.password) return callback(err || new WrongUsernameOrPasswordError(identifierValue));
  ...
  connection.on('connect', function (err) {
    ...
    var request = new Request(query, function (err, rowCount, rows) {
      if (err) {
        ...
        callback(new WrongUsernameOrPasswordError(identifierValue));
      } else {
        bcrypt.compare(password, rows[0][2].value, function (err, isValid) {
          if (err) { callback(new Error(err)); }
          else if (!isValid) { callback(new WrongUsernameOrPasswordError(identifierValue)); }
          else {
            callback(null, {
              user_id: rows[0][0].value,
...
async function loginAsync(identifierValue, password, callback) {
  ...
  try {
    ...
    if (e.response.status === 404) {
      ...
      return callback(
        new WrongUsernameOrPasswordError(identifierValue, "Invalid credentials provided.")
      );
    }
...
function login (identifierValue, password, callback) {
  ...
  ibmdb.open(credentials, function(err, conn) {
    ...
    conn.query("select * from =<user_name>.USERS where email='" + identifierValue + "'", function(err, data) {
      if (err) callback(new Error(err));
      else {
