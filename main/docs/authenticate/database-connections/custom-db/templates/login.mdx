...
This script is required for both legacy authentication and for [automatic migration](/docs/manage-users/user-migration/configure-automatic-migration-from-your-database). If automatic migration is configured for the connection, the migration process will be triggered after the first time the user logs in successfully. To avoid creating duplicate users, set a consistent, unchanging `user_id` on the returned user profile for the same user.
