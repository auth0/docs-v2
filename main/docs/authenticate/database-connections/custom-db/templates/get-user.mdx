---
title: Get User Database Action Script and Templates
sidebarTitle: Get User
description: The Get User script runs when Auth0 needs to determine the current state of existence of a user.
---
...
This script is required for [automatic migration to Auth0](/docs/manage-users/user-migration/configure-automatic-migration-from-your-database), and conditionally required without migration enabled depending on the operations configured for the connection. Auth0 strongly recommends that the `user_id` you return on the user profile be permanent and consistent: for a given user, the Get User script should return the same `user_id` on every execution. See [Return a consistent user_id](#return-a-consistent-user-id) for details.
...
* Change their email address ([Change Email](/docs/authenticate/database-connections/custom-db/templates/change-email) script)
* Log in ([Login](/docs/authenticate/database-connections/custom-db/templates/login) script)
* Change their password ([Change Password](/docs/authenticate/database-connections/custom-db/templates/change-password) script)

When **Import Users to Auth0** is disabled, the Get User script runs when a user attempts to sign up to check if the user already exists in the external user store. If a user already exists in the external user store, it does not execute the Create script.

The Get User script also runs when a user attempts to:

* Sign up ([Create](/docs/authenticate/database-connections/custom-db/templates/create) script)
* Change their email address ([Change Email](/docs/authenticate/database-connections/custom-db/templates/change-email) script)
* Change or reset their password ([Change Password](/docs/authenticate/database-connections/custom-db/templates/change-password) script)
...
If the user is found, pass a `null` value to the `error` parameter, and pass the user's profile data to the `profile` parameter in [normalized form](/docs/manage-users/user-accounts/user-profiles/normalized-user-profile-schema):
...
Returning an inconsistent `user_id` can create duplicate or orphaned users, and can break flows that rely on the identifier remaining stable. For example, a password reset link embeds the `user_id` in the reset ticket; in a future release, if the Get User script returns a different `user_id` when the user opens the link, the reset will fail and the link will appear expired. Returning a consistent `user_id` today ensures your connection continues to work as this validation is enforced.
</Callout>

**Do:** derive the `user_id` from a stable property of the source record, so the same user always maps to the same value.

```js lines
return callback(null, {
    // `user.id` is the immutable primary key from your database
...
return callback(null, {
    // Anti-pattern: a new value is generated on every execution, which Auth0 treats as a new user each time
    user_id: uuidv4(),
    email: user.email,
    email_verified: user.email_verified
});
...
For more information, read [Troubleshoot Custom Databases](/docs/authenticate/database-connections/custom-db/error-handling).
