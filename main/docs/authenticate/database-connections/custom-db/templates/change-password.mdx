---
title: Change Password Database Action Script and Templates
sidebarTitle: Change Password
description: The Change Password script runs during when you or the user start a password change workflow.
---
...
```js lines
changePassword(identifierValue, newPassword, callback): function
```
...
function changePassword(identifierValue, newPassword, callback) {
  ...
  client.connect(function (err) {
    ...
    bcrypt.hash(newPassword, 10, function (err, hash) {
      ...
      users.update({ email: identifierValue }, { $set: { password: hash } }, function (err, result) {
        client.close();
        if (err) return callback(err);
