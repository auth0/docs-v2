...
* Integrating an external <Tooltip tip="Identity Provider (IdP): Service that stores and manages digital identities." cta="View Glossary" href="/docs/glossary?term=identity+provider">identity provider</Tooltip>
* Migrating to Auth0

To learn more, read [Example Use Cases and Code Samples](#example-use-cases-and-code-samples).

Each Custom Token Exchange request maps to a [Custom Token Exchange Profile](/docs/secure/custom-token-exchange/configure-custom-token-exchange-profile) that is governed by an [Action](/docs/customize/actions), where you can:
...
You can [configure multiple Custom Token Exchange Profiles](/docs/secure/custom-token-exchange/configure-custom-token-exchange-profile) for an application. After the Auth0 Authorization Server validates that the Custom Token Exchange request is valid and maps to an existing Custom Token Exchange Profile, the [Custom Token Exchange trigger](/docs/customize/actions/explore-triggers/custom-token-exchange-trigger) executes the single Action associated with that profile. The application can then leverage Custom Token Exchange to authenticate and get Auth0 access, ID, and refresh tokens for the user.
...
<Frame>![](/docs/images/cdy7uua7fh8z/61fgdh8VJEmfiYn0l7Tem0/3ceac5de5413373662e8b800d8f80210/Screenshot_2025-02-03_at_9.30.50_PM.png)</Frame>
...
<Frame>![](/docs/images/cdy7uua7fh8z/4eKWTJtCQwSHUZyKU81mWm/33c1966367d2b7c157b414fa1f04c026/Screenshot_2025-02-03_at_5.36.38_PM.png)</Frame>