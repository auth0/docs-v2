...
<Warning>
  Native passkey registration is not supported during signup on connections that require SMS or Email OTP verification. If your mobile application uses the Native Passkeys API, ensure that SMS or Email OTP verification is not required on the same connection.
</Warning>
