...
redirects:
  - "/docs/connections/passwordless/best-practices"
---
## Implement login

You can implement <Tooltip tip="Passwordless: Form of authentication that does not rely on a password as the first factor." cta="View Glossary" href="/docs/glossary?term=Passwordless">Passwordless</Tooltip> authentication by redirecting to Auth0's <Tooltip tip="Passwordless: Form of authentication that does not rely on a password as the first factor." cta="View Glossary" href="/docs/glossary?term=Universal+Login">Universal Login</Tooltip>, by embedding login in your application, or by combining the two within the same app.

Universal Login is a good default when you want Auth0 to host the login surface and deliver updates transparently. Embedded login is appropriate when you want full control over the UI/UX or are building a native experience with [Passkey APIs](/docs/authenticate/database-connections/passkeys) or [Native Social Login](/docs/authenticate/identity-providers/social-identity-providers). Many applications use Universal Login for primary sign-in and embedded flows for in-app passkey enrollment, factor management, or step-up authentication. To compare the trade-offs, read [Hosted Login vs. Embedded Login](/docs/authenticate/login/universal-vs-embedded-login).
