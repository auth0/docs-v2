...
* **Embedded Login** fits flows where you want full control over the UI or want to keep users inside your application—for example, [factor enrollment](/docs/authenticate/login/embedded-login/use-cases/enrollment), step-up authentication, passkey registration with [Passkey APIs](/docs/authenticate/database-connections/passkeys), [connected account linking](/docs/manage-users/user-accounts/user-account-linking), or sign-in using [Native Social Login](/docs/authenticate/identity-providers/social-identity-providers).
