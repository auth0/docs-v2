...
Universal Login supports [Correlation ID](https://auth0.com/docs/api/management/v2/introduction#request-correlation), a unique identifier to track events. 
...
When you pass `correlation_id` in a call to the `/authorize` endpoint, you can track the following events:
* Successful sign-up
* Login attempts
* MFA enrollment and challenges
* Password resets

If you are using Auth0 Actions, you can use the `correlation_id` parameter with the `event.transaction` object for the following triggers:
