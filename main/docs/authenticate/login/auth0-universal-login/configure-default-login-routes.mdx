...
If you use [Multiple Custom Domains](/docs/customize/custom-domains/multiple-custom-domains) or [Organizations](/docs/manage-users/organizations/organizations-overview), you can configure the application-level `initiate_login_uri` to include metadata placeholders that resolve dynamically at runtime.

### Supported placeholders

| Placeholder | Source | Example |
|---|---|---|
| `{custom_domain.metadata.public_KEY}` | Metadata from the custom domain used in the request | `https://{custom_domain.metadata.public_app_host}/login` |
| `{organization.metadata.public_KEY}` | Metadata from the organization associated with the request | `https://{organization.metadata.public_login_host}/login` |

Both placeholder types can be combined in the same URI if the request includes both a custom domain and an organization context.

For placeholder validation rules and restrictions, see [Custom domain URL placeholders](/docs/get-started/applications/wildcards-for-subdomains#custom-domain-url-placeholders).
...
If a placeholder cannot be resolved (for example, the organization is unknown or the metadata key does not exist), Auth0 falls back to the tenant-level `default_redirection_uri`. If that is also not configured, Auth0 renders an error page. Configure a tenant-level `default_redirection_uri` without placeholders as a reliable fallback.
