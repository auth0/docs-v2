...
1. `{yourConnectionId}` est [l’ID de connexion](/docs/fr-CA/fr-ca/authenticate/identity-providers/locate-the-connection-id) pour lequel ces attributs seront refusés.
...
* Seuls les [champs racine](/docs/fr-CA/fr-ca/manage-users/user-accounts/user-profiles/user-profile-structure) (comme `user.name` ou `user.phone_number`) peuvent être refusés.
...
Si l’une de ces limitations est inacceptable, vous pouvez créer une [règle](/docs/fr-CA/fr-ca/customize/rules) pour chiffrer les données et faire en sorte que les données persistent dans l’objet `user.app_metadata`.

<div id="learn-more">
  ## En savoir plus
</div>

* [Adresses IP Auth0 pour listes d’autorisations](/docs/fr-CA/fr-ca/secure/security-guidance/data-security/allowlist)