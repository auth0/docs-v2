...
Dans ce tutoriel, nous allons expliquer comment utiliser auth0.js ou les API Auth0 pour obtenir des informations sur le consentement, puis enregistrer ces informations dans les métadonnées de l’utilisateur. Pour en savoir plus, lisez [Comprendre le fonctionnement des métadonnées dans les profils utilisateurs](/docs/fr-CA/fr-ca/manage-users/user-accounts/metadata).
...
1. Celle qui affiche un drapeau, fonctionne pour les connexions de base de données et utilise la bibliothèque auth0.js pour créer l’utilisateur (utilisée par les applications monopages). Pour en apprendre davantage, consultez [Auth0.js v9 Reference](/docs/fr-CA/fr-ca/libraries/auth0js).
...
Dans cette section, nous utiliserons une simple application monopage et personnaliserons le gadget logiciel de connexion pour ajouter un drapeau que les utilisateurs pourront utiliser pour fournir des informations relatives au consentement. Au lieu de créer une application de zéro, nous allons [utiliser l’exemple Guide rapide JavaScript](/docs/fr-CA/fr-ca/quickstart/spa/vanillajs) d’Auth0. Nous utiliserons également les pages Connexion universelle Auth0 afin de pouvoir implémenter une expérience de connexion universelle au lieu d’intégrer la connexion dans notre application. Pour en savoir plus sur la connexion universelle, consultez [Connexion universelle Auth0](/docs/fr-CA/fr-ca/authenticate/login/auth0-universal-login). Pour en savoir plus sur les différences entre la connexion universelle et la connexion intégrée, consultez [Connexion universelle centralisée par rapport à la Connexion intégrée](/docs/fr-CA/fr-ca/authenticate/login/universal-vs-embedded-login).
...
4. [Téléchargez l’exemple d’application JavaScript à page unique](/docs/fr-CA/fr-ca/quickstart/spa/vanillajs).
...
<Frame>![Dashboard Branding Universal Login Classic Login Tab Custom Login Form](/docs/images/fr-ca/cdy7uua7fh8z/4m3WA0sKMoR0C1KVnVmZ1G/9eb874ba2619b71f57e7fff80f84cc01/2025-02-26_15-23-14.png)</Frame>
...
Si vous servez votre page de connexion à partir de votre propre serveur, alors vous pouvez appeler directement le point de terminaison [**Signup**](/docs/fr-CA/fr-ca/api/authentication#signup) d’Authentication API une fois l’utilisateur inscrit.
...
Si la définition de valeurs booléennes est une exigence pour vous, vous pouvez plutôt utiliser <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=management-api" tip="Management API
Un produit permettant aux clients d’effectuer des tâches administratives." cta="Voir le glossaire">Management API</Tooltip>. Dans ce scénario, vous inscrivez votre utilisateur comme d’habitude puis vous appelez le point de terminaison [**Update User (Mettre à jour l’utilisateur)**](/docs/fr-CA/fr-ca/api/management/v2#!/Users/patch_users_by_id) de Management API pour définir les métadonnées requises après création de l’utilisateur. Pour plus d’informations sur la façon de procéder, lisez le paragraphe suivant.
...
Avant d’appeler Management API, vous devez disposer d’un jeton d’accès. Pour en savoir plus, lisez [Obtenir des jetons d’accès à Management API pour la production](/docs/fr-CA/fr-ca/secure/tokens/access-tokens/management-api-access-tokens/get-management-api-access-tokens-for-production).

L’article cité utilise le Flux des identifiants client pour obtenir un jeton, que vous ne pouvez pas utiliser à partir d’une application fonctionnant sur le navigateur. Le flux Implicite est recommandé. Pour en savoir plus sur le Flux des identifiants client, consultez [Flux des identifiants client](/docs/fr-CA/fr-ca/get-started/authentication-and-authorization-flow/client-credentials-flow). Pour en savoir plus sur le flux Implicite, lisez [Flux implicite](/docs/fr-CA/fr-ca/get-started/authentication-and-authorization-flow/implicit-flow-with-form-post).

Définissez le paramètre de requête **<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-1" tip="">audience</Tooltip>** sur `https://YOUR_DOMAIN/api/v2/` et le paramètre **scope** sur la permission `create:current_user_metadata`. Vous pouvez utiliser le jeton d’accès que vous recevrez dans la réponse pour appeler le point de terminaison [**Mettre à jour l’utilisateur**](/docs/fr-CA/fr-ca/api/management/v2#!/Users/patch_users_by_id) de Management API.
