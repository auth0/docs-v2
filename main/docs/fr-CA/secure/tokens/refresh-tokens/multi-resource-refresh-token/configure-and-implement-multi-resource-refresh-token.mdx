...
To use Multi-Resource <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=refresh-token" tip="Jeton d’actualisation
Jeton utilisé pour obtenir un jeton d’accès renouvelé sans obliger les utilisateurs à se connecter à nouveau." cta="Voir le glossaire">Refresh Tokens</Tooltip> (MRRT), configure your application’s refresh token policies using the Auth0 [Management API](/docs/fr-CA/fr-ca/api/management/v2/clients/patch-clients-by-id). These policies will specify which API and scopes the application is allowed to request during a refresh token exchange.
...
For existing applications, make a PATCH call to the [Update a Client](/docs/fr-CA/fr-ca/api/management/v2/clients/patch-clients-by-id) endpoint. To create a new application, make a POST call to the [Create a Client](/docs/fr-CA/fr-ca/api/management/v2/clients/post-clients) endpoint:
...
Once you configure your application’s refresh token with MRRT policies, you can start to  exchange a single refresh token for <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=access-token" tip="Jeton d’accès
...
To facilitate this, your application needs to initiate a login flow using either the [Authorization Code Flow](/docs/fr-CA/fr-ca/get-started/authentication-and-authorization-flow/authorization-code-flow) or the [Resource Owner Password Grant](/docs/fr-CA/fr-ca/authenticate/login/oidc-conformant-authentication/oidc-adoption-rop-flow).

<div id="step-1-authenticate-and-request-a-refresh-token">
  ### Step 1: Authenticate and request a refresh token
</div>

To receive a refresh token, include the `offline_access` scope when initiating the authentication request. To learn more, read [Get Refresh Tokens](/docs/fr-CA/fr-ca/secure/tokens/refresh-tokens/get-refresh-tokens).

<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  If you do not receive a refresh token, confirm that:

  * The API has [Allow Offline Access](/docs/fr-CA/fr-ca/get-started/apis/api-settings#access-settings) enabled in its settings.
...
 To learn more, read [Use Refresh tokens](/docs/fr-CA/fr-ca/secure/tokens/refresh-tokens/use-refresh-tokens).
...
Use the access token to call the secured API using the [Bearer HTTP authorization scheme](https://datatracker.ietf.org/doc/html/rfc6750). To learn more, read [Use Access Tokens](/docs/fr-CA/fr-ca/secure/tokens/access-tokens/use-access-tokens).
