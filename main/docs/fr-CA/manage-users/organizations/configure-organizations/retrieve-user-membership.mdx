...
Lorsque vous travaillez avec des [organisations](/docs/fr-CA/fr-ca/manage-users/organizations/organizations-overview) de manière programmatique, vous devrez peut-être récupérer la liste des organisations auxquelles un utilisateur est affilié.
...
Effectuez un appel `GET` au point de terminaison `Obtenir les organisations d’utilisateurs`. Veillez à remplacer les valeurs d’espace réservé `USER_ID` et `MGMT_API_ACCESS_TOKEN` par l’ID utilisateur et votre jeton d’accès à <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=management-api" tip="Management API
...
| `MGMT_API_ACCESS_TOKEN` | [Jeton d’accès à Management API](/docs/fr-CA/fr-ca/tokens/management-api-access-tokens) avec la permission `read:organizations`. |
