...
Vous pouvez modifier un [rôle](/docs/fr-CA/fr-ca/manage-users/access-control/rbac) avec <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=auth0-dashboard" tip="Auth0 Dashboard
...
Faites un appel `PATCH` au point de terminaison [Mettre à jour le rôle](/docs/fr-CA/fr-ca/api/management/v2#!/Roles/patch_roles_by_id). Assurez-vous de remplacer les valeurs des placeholders `ROLE_ID`, `MGMT_API_ACCESS_TOKEN`, `ROLE_NAME`, et `ROLE_DESC` par votre identifiant de rôle, votre jeton d’accès à Management API, le nom et la description de votre rôle, respectivement.
