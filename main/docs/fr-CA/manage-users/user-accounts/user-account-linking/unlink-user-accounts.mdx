...
Utilisez le point de terminaison [Unlink a User Account (Dissocier un compte utilisateur)](/docs/fr-CA/fr-ca/api/management/v2#!/Users/delete_user_identity_by_user_id) de <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=management-api" tip="Management API
...
Si votre objectif est de supprimer complètement l’identité secondaire, vous devez d’abord dissocier les comptes, puis [supprimer le compte secondaire nouvellement créé](/docs/fr-CA/fr-ca/manage-users/user-accounts/delete-users).

Selon l’endroit d’où vous appelez le point de terminaison, utilisez l’une de ces deux permissions :

* `update:current_user_identities` à partir de [client-side code](/docs/fr-CA/fr-ca/manage-users/user-accounts/user-account-linking/user-initiated-account-linking-client-side-implementation)
* `update:users` à partir de [server-side code](/docs/fr-CA/fr-ca/manage-users/user-accounts/user-account-linking/suggested-account-linking-server-side-implementation)
...
Pour dissocier des comptes, appelez Management API [Dissocier un point de terminaison de compte utilisateur](/docs/fr-CA/fr-ca/api/v2#!/Users/delete_user_identity_by_user_id) en utilisant le <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=json-web-token" tip="Jeton Web JSON (JWT)
...
Si vous devez dissocier deux ou plusieurs comptes utilisateur, appelez Management API [Dissocier un point de terminaison de compte utilisateur](/docs/fr-CA/fr-ca/api/v2#!/Users/delete_user_identity_by_user_id) en utilisant un [Jeton d’accès à Management API](/docs/fr-CA/fr-ca/api/v2/tokens) avec la permission `update:users`.
...
1. Appelez [Dissocier un point de terminaison de compte utilisateur](/docs/fr-CA/fr-ca/api/v2#!/Users/delete_user_identity_by_user_id) de Management API v2 en utilisant [Jeton d’accès à Management API](/docs/fr-CA/fr-ca/api/v2/tokens) avec la permission `update:users`.
...
* [Associer des comptes d’utilisateurs](/docs/fr-CA/fr-ca/manage-users/user-accounts/user-account-linking/link-user-accounts)
* [Association de comptes d’utilisateur : mise en œuvre côté serveur](/docs/fr-CA/fr-ca/manage-users/user-accounts/user-account-linking/suggested-account-linking-server-side-implementation)
* [Liaison de compte initiée par l’utilisateur : Mise en œuvre côté client](/docs/fr-CA/fr-ca/manage-users/user-accounts/user-account-linking/user-initiated-account-linking-client-side-implementation)