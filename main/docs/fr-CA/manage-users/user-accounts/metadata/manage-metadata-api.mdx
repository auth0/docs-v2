...
Effectuez l’appel `POST` suivant au point de terminaison <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=management-api" tip="Management API
Un produit permettant aux clients d’effectuer des tâches administratives." cta="Voir le glossaire">Management API</Tooltip> [`/post_users`](/docs/fr-CA/fr-ca/api/management/v2#!/Users/post_users) pour créer l’utilisateur et définir les valeurs des propriétés :
...
Vous pouvez mettre à jour les métadonnées d’un utilisateur en effectuant un appel `PATCH` au point de terminaison [`/patch_users_by_id`](/docs/fr-CA/fr-ca/api/management/v2#!/Users/patch_users_by_id) de Management API.
...
* [Configurer les métadonnées de l’application](/docs/fr-CA/fr-ca/get-started/applications/configure-application-metadata)