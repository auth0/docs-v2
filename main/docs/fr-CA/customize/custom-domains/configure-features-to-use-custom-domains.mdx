...
<table class="table">
  ...
  <tbody>
    <tr>
      ...
      Votre application redirige l’utilisateur vers la connexion universelle, hébergée sur le serveur d’autorisation d’Auth0, pour vérifier son identité." cta="Voir le glossaire" href="/docs/fr-CA/glossary?term=universal-login">Connexion universelle</Tooltip> avec une page de connexion personnalisée</td>
    ...
    <tr>
      ...
      Protocole normalisé permettant à deux parties d’échanger des informations d’authentification sans mot de passe." cta="Voir le glossaire" href="/docs/fr-CA/glossary?term=security-assertion-markup-language">SAML</Tooltip></td>
...
Si vous utilisez [Connexion universelle Auth0.](/docs/fr-CA/fr-ca/authenticate/login/auth0-universal-login) et que vous avez personnalisé la page de connexion, vous devez mettre à jour le code pour utiliser votre domaine personnalisé. Si vous utilisez la page de connexion **par défaut** sans personnalisation, vous n’avez aucune modification à effectuer. Pour en savoir plus, consultez Connexion universelle Auth0.

Si vous utilisez[Lock pour le Web](/docs/fr-CA/fr-ca/libraries/lock) vous devez définir les options `configurationBaseUrl` et `overrides` comme indiqué dans l’exemple de script suivant :
...
Si vous utilisez [Auth0.js](/docs/fr-CA/fr-ca/libraries/auth0js) sur la page de Connexion universelle, vous devez définir l’option `overrides`.
...
Si vous utilisez [Lock pour le Web](/docs/fr-CA/fr-ca/libraries/lock) intégré dans votre application, vous devez mettre à jour le code pour utiliser votre domaine personnalisé lors de l’initialisation de Lock. Vous devrez également définir la valeur `configurationBaseUrl` sur le CDN approprié URL.
...
Si vous utilisez la [Trousse SDK Auth0 pour les applications à page unique (SPA)](/docs/fr-CA/fr-ca/libraries/auth0-spa-js), [Auth0.js](/docs/fr-CA/fr-ca/libraries/auth0js) ou [d’autres trousses SDK](/docs/fr-CA/fr-ca/support/matrix#auth0-sdks), vous devrez initialiser la trousse SDK en utilisant votre domaine personnalisé. Par exemple, si vous utilisez la trousse SDK Auth0.js, vous devez définir ce qui suit :
...
Consultez la section API ci-dessous si vous utilisez un domaine personnalisé et avez également l’intention d’effectuer [des actions avec Management API via Auth0.js](/docs/fr-CA/fr-ca/libraries/auth0js/v9#user-management).
...
Si vous souhaitez utiliser votre domaine personnalisé avec des fournisseurs d’identité sociale (<Tooltip href="/docs/fr-CA/fr-ca/glossary?term=idp" tip="Fournisseur d’identité (IdP)
...
Vous ne pouvez pas utiliser [Clés de développeur Auth0](/docs/fr-CA/fr-ca/authenticate/identity-providers/social-identity-providers/devkeys) avec des domaines personnalisés.

<div id="configure-google-workspace-connections">
  ## Configurer les connexions Google Workspace
</div>

Si vous souhaitez utiliser votre domaine personnalisé avec les connexions Google Workspace, vous devez mettre à jour l’URI de redirection autorisée dans les Client Settings (Paramètres du client) <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=oath2" tip="OAuth 2.0
...
Si vous utilisez un flux d’authentification avec votre domaine personnalisé pour demander des jetons d’accès afin d’accéder à <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=management-api" tip="Management API
...
Si vous utilisez Auth0 avec un domaine personnalisé pour émettre des jetons d’accès pour vos API, vous devez valider le ou les émetteurs de <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=json-web-token" tip="Jeton Web JSON (JWT)
...
Pour utiliser votre domaine personnalisé avec des <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=security-assertion-markup-language" tip="Security Assertion Markup Language (SAML)
...
Si vous souhaitez utiliser votre domaine personnalisé avec des applications <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=ws-fed" tip="Web Service Federation (WS-Fed)
...
Pour utiliser les connexions AD/LDAP avec la prise en charge de Kerberos, vous devrez mettre à jour le point de terminaison Ticket pour qu’il fonctionne avec le domaine personnalisé. Comme indiqué dans la documentation du [connecteur AD/LDAP d’Auth0](/docs/fr-CA/fr-ca/connector/modify#point-an-ad-ldap-connector-to-a-new-connection), le fichier `config.json` doit être modifié, avec la valeur `PROVISIONING_TICKET` changée pour utiliser votre domaine personnalisé au format `https://<YOUR-CUSTOM-DOMAIN>/p/ad/jUG0dN0R`.
...
* [Configurer des domaines personnalisés avec des certificats gérés par Auth0](/docs/fr-CA/fr-ca/customize/custom-domains/auth0-managed-certificates)
* [Configurer des domaines personnalisés avec des certificats autogérés](/docs/fr-CA/fr-ca/customize/custom-domains/self-managed-certificates)
* [Dépanner les domaines personnalisés](/docs/fr-CA/fr-ca/troubleshoot/integration-extensibility-issues/troubleshoot-custom-domains)
* [Vérifier les appels d’API](/docs/fr-CA/fr-ca/troubleshoot/authentication-issues/check-api-calls)
* [Dépanner des configurations SAML](/docs/fr-CA/fr-ca/troubleshoot/authentication-issues/troubleshoot-saml-configurations)