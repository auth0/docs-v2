...
You can use [Auth0 Rules](/docs/fr-CA/fr-ca/customize/rules) to redirect users before an authentication transaction is complete. This lets you implement custom authentication flows that require additional user interaction beyond the standard login form. Redirect rules are commonly used to do custom <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=multifactor-authentication" tip="Authentification multifacteur (MFA)
...
To learn more, see [Multi-Factor Authentication in Auth0](/docs/fr-CA/fr-ca/secure/multi-factor-authentication).
...
Your redirect URL needs to extract the `state` parameter and send it back to Auth0 to resume the authentication transaction. State is an opaque value, used to prevent [Cross-Site Request Forgery (CSRF) attacks](/docs/fr-CA/fr-ca/security/common-threats#cross-site-request-forgery).
...
If you&#39;re using a <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=custom-domain" tip="Domaine personnalisé
...
* any changes to the user object are made during the redirect, prior to calling the `/continue` endpoint. For example, updates through the Auth0 <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=management-api" tip="Management API
...
3. The user changes their password in the application-specific page by having the application call the [Auth0 Management API](/docs/fr-CA/fr-ca/api/v2#!/Users/patch_users_by_id)
...
Passing information back and forth in the front channel opens up surface area for <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=bad-actors" tip="Acteurs menaçants
...
The application is going to redirect the user back to the Auth0 tenant, so any data related to the user can be gathered through the <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=id-token" tip="Jeton d’ID
...
* [Resource Owner endpoint](/docs/fr-CA/fr-ca/api/authentication/reference#resource-owner)
* [Password exchange](/docs/fr-CA/fr-ca/get-started/authentication-and-authorization-flow/resource-owner-password-flow)
* [Refresh Token exchange](/docs/fr-CA/fr-ca/secure/tokens/refresh-tokens)

You can detect the above cases by checking `context.protocol`:

* For Password exchange: `context.protocol === 'oauth2-password'`
* For <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=refresh-token" tip="Jeton d’actualisation
...
* For <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=resource-owner" tip="Propriétaire de ressources
...
* [Rediriger les utilisateurs](/docs/fr-CA/fr-ca/authenticate/login/redirect-users-after-login)
* [Comment fonctionne le profilage progressif](/docs/fr-CA/fr-ca/manage-users/user-accounts/user-profiles/progressive-profiling)
