...
Lorsque vous n’avez plus besoin de hooks, vous pouvez les supprimer à l’aide du Dashboard ou de <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=management-api" tip="Management API
...
Effectuez un appel `DELETE` vers le point de terminaison [Suppression d’un hook](/docs/fr-CA/fr-ca/api/management/v2/#!/Hooks/delete_hooks_by_id). Assurez-vous de remplacer les valeurs des paramètres fictifs `HOOK_ID` et `MGMT_API_ACCESS_TOKEN` avec l’ID de votre appel et votre jeton d’accès de Management API, respectivement.
