...
Vous pouvez mettre à jour les secrets d’appels ajoutés à un appel donné à l’aide du Dashboard ou de <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=management-api" tip="Management API
Un produit permettant aux clients d’effectuer des tâches administratives." cta="Voir le glossaire">Management API</Tooltip>.

Des secrets d’appels peuvent également être importés et exportés à l’aide de l’outil Deploy en interface de ligne de commande. Pour en savoir plus, consultez [Outil Deploy CLI](/docs/fr-CA/fr-ca/deploy-monitor/deploy-cli-tool).
...
Effectuez un appel `PATCH` au point de terminaison[Update Hook Secrets](/docs/fr-CA/fr-ca/api/management/v2/#!/Hooks/patch_secrets). Veillez à remplacer les valeurs de remplacement `HOOK_ID` et `MGMT_API_ACCESS_TOKEN` par votre identifiant d’appel et votre jeton d’accès à Management API, respectivement.
