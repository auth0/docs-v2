...
Vous pouvez créer plusieurs Secrets d’appel pour n’importe quel [Hook](/docs/fr-CA/fr-ca/customize/hooks) à l’aide de Dashboard ou de <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=management-api" tip="Management API
Un produit permettant aux clients d’effectuer des tâches administratives." cta="Voir le glossaire">Management API</Tooltip>.

Les secrets de hooks peuvent également être importés et exportés à l’aide de l’outil [Deploy CLI](/docs/fr-CA/fr-ca/deploy-monitor/deploy-cli-tool).
...
Faites une requête `POST` au [point de terminaison Ajouter des Secrets d’appel](/docs/fr-CA/fr-ca/api/management/v2/#!/Hooks/post_secrets). Assurez-vous de remplacer les valeurs des paramètres fictifs `MGMT_API_ACCESS_TOKEN`, `HOOK_ID`, `HOOK_SECRET_KEY` et `HOOK_SECRET_VALUE` avec votre jeton d’accès de Management  API, votre identifiant de hookID d’appel et les paires clé-valeur de hook d’appel respectivement.
