...
Lorsque vous n’avez plus besoin de secrets de hook pour un hook donné, vous pouvez les supprimer à l’aide du Dashboard ou de <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=management-api" tip="Management API
Un produit permettant aux clients d’effectuer des tâches administratives." cta="Voir le glossaire">Management API</Tooltip>.

Les secrets de hook peuvent également être importés et exportés à l’aide de l’outil [Deploy CLI](/docs/fr-CA/fr-ca/deploy-monitor/deploy-cli-tool).
...
Effectuez un appel `DELETE` au point de terminaison[Delete Hook Secrets (Supprimer les secrets de crochets)](/docs/fr-CA/fr-ca/api/management/v2/#!/Hooks/delete_secrets). Veillez à remplacer les valeurs fictives de `HOOK_ID`, `HOOK_SECRET_NAME` et `MGMT_API_ACCESS_TOKEN` par votre identifiant de crochet, votre (vos) nom(s) de secret de crochet et votre jeton d’accès à Management API, respectivement.
