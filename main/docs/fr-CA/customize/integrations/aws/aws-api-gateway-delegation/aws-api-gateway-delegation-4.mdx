...
Dans les situations où vous souhaitez prendre des décisions au sein de votre code (par exemple, vérifier la solvabilité d’un utilisateur achetant un animal de compagnie), il peut être nécessaire de transmettre également l’identité de l’utilisateur. Ceci sera démontré ci-dessous dans l’[Étape 5 – Utilisation de jetons d’ID pour transmettre l’identité](/docs/fr-CA/fr-ca/customize/integrations/aws/aws-api-gateway-delegation/aws-api-gateway-delegation-5).
...
Ajoutez une méthode OPTIONS pour la ressource `purchase`, comme indiqué précédemment pour les `pets` dans la [Section Configurer CORS et déployer l’API de l’étape 2 - Sécuriser et déployer la passerelle API Amazon](/docs/fr-CA/fr-ca/customize/integrations/aws/aws-api-gateway-delegation/aws-api-gateway-delegation-2#set-up-cors-and-deploy-the-api).
...
Une fois que vous avez défini la fonction Lambda, [ajouter une méthode POST à la ressource `purchase`](/docs/fr-CA/fr-ca/customize/integrations/aws/aws-api-gateway-delegation/aws-api-gateway-delegation-1#method-post-pet-information) qui appelle le `PetPurchase` Lambda Assurez-vous d’ajouter également l’en-tête `Access-Control-Allow-Origin` avec une valeur de `*` à la méthode POST en utilisant la configuration de réponse de la méthode/réponse d’intégration que l’on trouve dans la [Section Configurer CORS et déployer l’API de l’étape 2 - Sécuriser et déployer la passerelle API Amazon](/docs/fr-CA/fr-ca/customize/integrations/aws/aws-api-gateway-delegation/aws-api-gateway-delegation-2#set-up-cors-and-deploy-the-api).
...
Pour sécuriser votre API, suivez le même processus que pour ajouter un nouveau rôle que vous avez [effectué dans la partie 2](/docs/fr-CA/fr-ca/customize/integrations/aws/aws-api-gateway-delegation/aws-api-gateway-delegation-2) de ce tutoriel. Appeler le nouveau rôle `auth0-api-social-role`.
...
En utilisant la [Console de passerelle Amazon API](https://console.aws.amazon.com/apigateway), vous allez à nouveau [déployer l’API et générer une nouvelle trousse SDK JavaScript](/docs/fr-CA/fr-ca/customize/integrations/aws/aws-api-gateway-delegation/aws-api-gateway-delegation-2#set-up-cors-and-deploy-the-api).
