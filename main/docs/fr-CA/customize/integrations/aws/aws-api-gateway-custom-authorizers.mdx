...
Pour en savoir plus sur les algorithmes de signature, lisez [Algorithmes de signature](/docs/fr-CA/fr-ca/get-started/applications/signing-algorithms). Pour plus de détails sur l’utilisation de JWKS, consultez [JSON Web Key Sets](/docs/fr-CA/fr-ca/secure/tokens/json-web-tokens/json-web-key-sets) (Ensembles de clés JSON Web).

<div id="how-api-gateway-custom-authorizers-work">
  ## Fonctionnement des autorisateurs personnalisés de la passerelle API
</div>

[Selon Amazon](http://docs.aws.amazon.com/apigateway/latest/developerguide/use-custom-authorizer.html), un autorisateur personnalisé de passerelle API est une « fonction Lambda que vous fournissez pour contrôler l’accès à votre API à l’aide de stratégies d’authentification par jeton du porteur, telles que <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=oath2" tip="OAuth 2.0
Cadre d’applications d’autorisation qui définit les protocoles d’autorisation et les flux de production." cta="Voir le glossaire">OAuth</Tooltip> ou <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=security-assertion-markup-language" tip="Security Assertion Markup Language (SAML)
...
Vous pouvez utiliser l&#39;autorisateur personnalisé pour mettre en œuvre différents types de stratégies d&#39;autorisation, y compris la vérification <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=json-web-token" tip="Jeton Web JSON (JWT)
...
Pour en savoir plus, consultez [What is Amazon API Gateway?](http://docs.aws.amazon.com/apigateway/latest/developerguide/use-custom-authorizer.html) (Qu’est-ce que la passerelle API Amazon) dans le Amazon Developer’s Guide. Pour plus d’informations sur la vérification des JWT, consultez notre article [Jeton Web JSON](/docs/fr-CA/fr-ca/secure/tokens/json-web-tokens) .
...
Obtenir un jeton d’accès JWT valide. Il y a plusieurs façons d’en obtenir un et la méthode que vous choisissez dépend du type de votre application, du niveau de confiance ou de l’expérience globale de l’utilisateur final. Pour plus d’informations, consultez [Obtenir des jetons d’accès](/docs/fr-CA/fr-ca/secure/tokens/access-tokens/get-access-tokens).
...
Pour plus de détails, voir [Obtenir des jetons d’accès](/docs/fr-CA/fr-ca/secure/tokens/access-tokens/get-access-tokens).
