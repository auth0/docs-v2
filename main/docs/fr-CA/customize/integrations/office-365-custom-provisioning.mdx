...
La configuration par défaut d’Office 365 inclut Active Directory et DirSync/Microsoft Entra ID Sync Services, qui synchronisent et provisionnent les utilisateurs AD dans votre Microsoft Entra ID pour <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=single-sign-on" tip="Authentification unique (SSO)
...
Mais que se passe-t-il si vous souhaitez permettre à des sous-traitants, des partenaires ou même des clients d’accéder à votre environnement Office 365 (par exemple, SharePoint)? Dans ce cas, l’approche par défaut n’est pas optimale, car ces utilisateurs devront être créés dans votre environnement Entra ID. Au lieu de cela, vous devez personnaliser la fourniture des utilisateurs Entra ID à l’aide de [Règles d&#39;Auth0](/docs/fr-CA/fr-ca/customize/rules).
