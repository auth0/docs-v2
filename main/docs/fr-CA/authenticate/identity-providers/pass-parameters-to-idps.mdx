...
Vous pouvez passer des paramètres propres au fournisseur à un fournisseur d’identité (<Tooltip href="/docs/fr-CA/fr-ca/glossary?term=idp" tip="Fournisseur d’identité (IdP)
...
* Les fournisseur d’identité (IdP) <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=security-assertion-markup-language" tip="Security Assertion Markup Language (SAML)
...
WordPress vous permet de passer un paramètre `blog` facultatif à son point de terminaison d’autorisation <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=oath2" tip="OAuth 2.0
...
Appeler le point de terminaison [Get a connection (Obtenir une connexion)](/docs/fr-CA/fr-ca/api/management/v2#!/Connections/get_connections_by_id) de Management API pour récupérer les valeurs existantes de l’objet `options` :
