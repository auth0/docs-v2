...
* Determines if the user is authenticated. If not, it will redirect the user to the <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=universal-login" tip="Connexion universelle
  Votre application redirige l’utilisateur vers la connexion universelle, hébergée sur le serveur d’autorisation d’Auth0, pour vérifier son identité." cta="Voir le glossaire">Universal Login</Tooltip> page.
...
* Explore more configuration options for Native to Web SSO:Dive deeper into session lifetime, <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=refresh-token" tip="Jeton d’actualisation
  Jeton utilisé pour obtenir un jeton d’accès renouvelé sans obliger les utilisateurs à se connecter à nouveau." cta="Voir le glossaire">refresh token</Tooltip> rotation, device binding, and cascade revocation in the [Native to Web SSO documentation](https://auth0.com/docs/authenticate/single-sign-on/native-to-web).
