...
De nombreuses instructions pour la mise en place d’une fédération <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=security-assertion-markup-language" tip="Security Assertion Markup Language (SAML)
Protocole normalisé permettant à deux parties d’échanger des informations d’authentification sans mot de passe." cta="Voir le glossaire">SAML</Tooltip> commencent par l’authentification unique (<Tooltip href="/docs/fr-CA/fr-ca/glossary?term=single-sign-on" tip="Authentification unique (SSO)
Service qui, après qu’un utilisateur se soit connecté à une application, le connecte automatiquement à d’autres applications." cta="Voir le glossaire">SSO</Tooltip>) initiée par le fournisseur de services. Le fournisseur de services redirige l’utilisateur vers le fournisseur d’identité (<Tooltip href="/docs/fr-CA/fr-ca/glossary?term=idp" tip="Fournisseur d’identité (IdP)
...
<Tooltip href="/docs/fr-CA/fr-ca/glossary?term=openid" tip="OpenID
...
2. Sous **Settings (Paramètres)**, vous pouvez voir la configuration de l’authentification unique initiée par un fournisseur d’identité.
   ...
   <Frame>![Protocoles Authentification unique (SSO) initiée par un fournisseur d’identité (IdP) Écran de configuration](/docs/images/fr-ca/cdy7uua7fh8z/1HORCAp4fZQg0BOoIopvoS/92e0dcbf8a1d48b9abd842eacaf1bf33/2025-02-27_14-12-13.png)</Frame>
...
Si vous souhaitez sélectionner une application comme **application par défaut** dans l’authentification unique (SSO) initiée par un fournisseur d’identité (IdP), et que l’application ne figure pas dans les 100 premières applications répertoriées dans la liste déroulante de votre locataire, vous devez utiliser <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=management-api" tip="Management API
...
* [Utilisation de certificats et de clés sous forme de chaînes de caractères](/docs/fr-CA/fr-ca/authenticate/protocols/saml/saml-sso-integrations/work-with-certificates-and-keys-as-strings)