...
Le [protocole OpenID Connect](/docs/fr-CA/fr-ca/authenticate/protocols/openid-connect-protocol) prend en charge un paramètre `prompt=none` sur la demande d’authentification qui permet aux applications d’indiquer que le serveur d’autorisation ne doit afficher aucune interaction utilisateur (telle que l’authentification, le consentement ou la <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=multifactor-authentication" tip="Authentification multifacteur (MFA)
...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  Toutes les [règles](/docs/fr-CA/fr-ca/customize/rules) applicables seront exécutées dans le cadre du processus d’authentification silencieuse.
</Callout>
...
Si l’utilisateur n’est pas connecté via l’authentification unique (<Tooltip href="/docs/fr-CA/fr-ca/glossary?term=single-sign-on" tip="Authentification unique (SSO)
...
<table class="table">
  ...
  <tbody>
    <tr>
      ...
      <td>L’utilisateur n’était pas connecté à Auth0, donc l’authentification silencieuse n’est pas possible. Cette erreur peut se produire en fonction de la configuration des paramètres <strong>Log In Session Management (Gestion de session de connexion)</strong> au niveau du locataire; en particulier, elle peut se produire après la période définie dans le paramètre <strong>Require log in after (Exiger la connexion après)</strong>. Consultez <a href="/docs/fr-CA/fr-ca/dashboard/guides/tenants/configure-session-Lifetime-settings">Configuration des paramètres de durée de vie des sessions</a> pour plus de détails.</td>
    </tr>
    ...
    <tr>
      ...
      <td>L’utilisateur était connecté à Auth0 et a autorisé l’application, mais doit être redirigé ailleurs avant que l’authentification puisse être terminée; par exemple, lors de l’utilisation d’une <a href="/docs/fr-CA/fr-ca/rules/redirect">règle de redirection</a>.</td>
    </tr>
  </tbody>
</table>
...
Dans le cas d’un [Flux implicite](/docs/fr-CA/fr-ca/get-started/authentication-and-authorization-flow/implicit-flow-with-form-post), le paramètre `expires_in` est renvoyé par Auth0 sous forme de paramètre de hachage suite à une authentification réussie. Dans le [Flux de code d’autorisation avec PKCE](/docs/fr-CA/fr-ca/get-started/authentication-and-authorization-flow/authorization-code-flow-with-pkce), il est renvoyé au serveur dorsal lors de l’exécution de l’échange de code d’autorisation.
...
Pour en savoir plus, veuillez consulter [Modifier la fréquence des demandes d’authentification](/docs/fr-CA/fr-ca/secure/multi-factor-authentication/customize-mfa).
...
* [Dépannage des erreurs de jeton non valide](/docs/fr-CA/fr-ca/troubleshoot/basic-issues/invalid-token-errors)
* [Vérifier les appels d’API](/docs/fr-CA/fr-ca/troubleshoot/authentication-issues/check-api-calls)
* [Dépannage des problèmes d’authentification multifacteur (MFA)](/docs/fr-CA/fr-ca/troubleshoot/authentication-issues/troubleshoot-mfa-issues)