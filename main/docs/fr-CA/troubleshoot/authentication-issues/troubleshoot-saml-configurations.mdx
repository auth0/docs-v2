...
* **Auth0 sert-il de fournisseur de services <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=security-assertion-markup-language" tip="Security Assertion Markup Language (SAML)
  Protocole normalisé permettant à deux parties d’échanger des informations d’authentification sans mot de passe." cta="Voir le glossaire">SAML</Tooltip> (SP), de fournisseur d’identité SAML (<Tooltip href="/docs/fr-CA/fr-ca/glossary?term=idp" tip="Fournisseur d’identité (IdP)
...
* Vérifiez si l’utilisateur peut accéder à certaines des URL clés de l’application, comme l&#39;authentification unique (<Tooltip href="/docs/fr-CA/fr-ca/glossary?term=single-sign-on" tip="Authentification unique (SSO)
...
Une erreur courante consiste à spécifier le protocole de réponse incorrect dans l’onglet initié par l’IdP. Le protocole de réponse est celui utilisé entre Auth0 et l’application (et non le fournisseur d’identité distant) Par exemple, si vous définissez cette valeur sur **SAML** alors que votre application attend **Openidentifiant Connect** ou **<Tooltip href="/docs/fr-CA/fr-ca/glossary?term=ws-fed" tip="Web Service Federation (WS-Fed)
...
3. À côté de la connexion SAML, cliquez sur **Settings (Paramètres)** (r](/docs/images/par l’icône d’engrenage).

   <Frame>![Dashboard Authentication Enterprise SAML Connection Settings](/docs/images/fr-ca/cdy7uua7fh8z/6s8p4aMck06YnPStr493sA/86cf45f919a2b75376a5ee6474b71cb1/2025-02-27_15-56-22.png)</Frame>
...
2. Vérifiez les informations qu’Auth0 envoie à l’application en [capturant un suivi HTTP de la séquence de connexion](/docs/fr-CA/fr-ca/troubleshoot/troubleshooting-tools/generate-and-analyze-har-files) et en analysant ce suivi HTTP.
...
4. Si l’attribut est manquant, vérifiez s’il a été inclus dans l’assertion. Vous pouvez le faire en décodant l’assertion SAML, ou vous pouvez activer le débogage pour la connexion.
   ...
   2. Ouvrez la liste des connexions de l’IdP **SAML** , cliquez sur ](/docs/images/ (Paramètres)**, et activez **Debug Mode (Mode débogage)**.

      <Frame>![Troubleshoot SAML Connections Enable Debug Mode screen](/docs/images/fr-ca/cdy7uua7fh8z/1F6npTHy1P0c0ygXfbndBD/a8f34635de6282df5062c2f951ee3177/2025-02-27_15-33-50.png)</Frame>
...
5. Si une valeur d’attribut existe dans le profil utilisateur Auth0, mais n’est pas mappée au bon attribut, vous pouvez corriger cela via la fonctionnalité de mappage de connexion.
   ...
   2. Ouvrez la liste des connexions de l’IdP **SAML** et allez ](/docs/images/ **Mappages**.

      <Frame>![Troubleshoot SAML Connections Mappings Tab Screen](/docs/images/fr-ca/cdy7uua7fh8z/BPMjuhqE17E5vWcHukcKf/2ddef4b5c2e30abde00e2f9504799c20/2025-02-27_14-14-14.png)</Frame>
...
2. Vérifiez les informations qu’Auth0 envoie à l’application en [capturant un suivi HTTP de la séquence de connexion](/docs/fr-CA/fr-ca/troubleshoot/troubleshooting-tools/generate-and-analyze-har-files) et en analysant ce suivi HTTP. Vous pouvez visualiser le suivi HTTP dans un analyseur de fichiers HAR, tel que [l’analyseur HAR de Google](https://toolbox.googleapps.com/apps/har_analyzer/).
...
* Les pages Journaux et Utilisateurs dans <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=auth0-dashboard" tip="Auth0 Dashboard
...
3. Vérifiez les informations qu’Auth0 envoie à l’application en [capturant un suivi HTTP de la séquence de connexion](/docs/fr-CA/fr-ca/troubleshoot/troubleshooting-tools/generate-and-analyze-har-files) et en analysant ce suivi HTTP. Vous pouvez visualiser le suivi HTTP dans un analyseur de fichiers HAR, tel que [l’analyseur HAR de Google](https://toolbox.googleapps.com/apps/har_analyzer/).
...
10. Un suivi HTTP de l’interaction SSO dans un [fichier .har](/docs/fr-CA/fr-ca/troubleshoot/troubleshooting-tools/generate-and-analyze-har-files)
...
* [Intégrations SAML d’authentification unique](/docs/fr-CA/fr-ca/authenticate/protocols/saml/saml-sso-integrations)
* [Tester SAML SSO avec Auth0 comme fournisseur de service et fournisseur d’identité](/docs/fr-CA/fr-ca/authenticate/protocols/saml/saml-configuration/configure-auth0-as-service-and-identity-provider)