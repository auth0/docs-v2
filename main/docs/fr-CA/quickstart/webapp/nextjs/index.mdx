...
mode: wide
'og:description': Ce guide explique comment intégrer Auth0 à une application Next.js, nouvelle ou existante, à l'aide de la trousse de développement logiciel (SDK) Auth0 Next.js v4 (bêta).
'og:image': https://cdn2.auth0.com/docs/1.14553.0/img/share-image.png
'og:title': 'Guides de démarrage rapide du SDK Auth0 Next.js : ajoutez la connexion à votre application Next.js'
'og:url': https://auth0.com/docs/
sidebarTitle: Next.js
title: Ajoutez la connexion à votre application Next.js
'twitter:description': Ce guide explique comment intégrer Auth0 à une application Next.js, nouvelle ou existante, à l'aide de la trousse de développement logiciel (SDK) Auth0 Next.js v4 (bêta).
'twitter:title': 'Guides de démarrage rapide du SDK Auth0 Next.js : ajoutez la connexion à votre application Next.js'
---
...
<Note>
  **Prérequis :** Avant de commencer, assurez-vous d’avoir installé les éléments suivants :
  ...
  - **[jq](https://jqlang.org/)** – requis pour la configuration de l’interface de ligne de commande (CLI) Auth0
...
<Steps>
  ...
  <Step title="Installer la trousse de développement logiciel (SDK) Next.js d'Auth0" stepNumber={2}>
    ```shellscript
    ...
    <Tabs>
      ...
      <Tab title="Tableau de bord">
        ...
        **Générez un AUTH0&#95;SECRET sécurisé :**
        ...
        Copiez la sortie et remplacez `YOUR_LONG_RANDOM_SECRET_HERE` dans `.env.local`. Celle-ci doit comporter exactement 64 caractères hexadécimaux.
        ...
        3. Dans la fenêtre contextuelle, entrez un nom pour votre application, sélectionnez `Regular Web Application` comme type d&#39;application et cliquez sur **Create**
        4. Passez à l’onglet **Settings** sur la page Application Details
        5. Remplacez `YOUR_AUTH0_APP_DOMAIN`, `YOUR_AUTH0_APP_CLIENT_ID` et `YOUR_AUTH0_APP_CLIENT_SECRET` dans le fichier `.env.local` par les valeurs **Domain**, **Client ID** et **Client Secret** à partir du Auth0 Dashboard

        <Warning>
          **Critique :** Si vous modifiez `AUTH0_SECRET` après le démarrage de l’application, vous devez supprimer les témoins (cookies) de votre navigateur pour localhost. Les anciens témoins de session chiffrés avec le secret précédent provoqueront une erreur `JWEDecryptionFailed`.
        </Warning>
        ...
        **Origines Web autorisées :**
        ...
        <Info>
          Les **URL de redirection autorisées** sont une mesure de sécurité essentielle pour garantir que les utilisateurs sont renvoyés en toute sécurité à votre application après l’authentification. Sans URL correspondante, le processus de connexion échouera et les utilisateurs seront bloqués sur une page d’erreur Auth0 au lieu d’accéder à votre application.
  ...
  <Step title="Mettre à jour le layout avec Auth0Provider (OPTIONNEL)" stepNumber={9}>
  ...
  <Step title="Lancez votre application" stepNumber={11}>
    ...
    <Info>
      Votre application sera accessible à l&#39;adresse [http://localhost:3000](http://localhost:3000). La trousse de développement logiciel (SDK) Auth0 v4 monte automatiquement les routes d&#39;authentification sous `/auth/*` (et non `/api/auth/*` comme dans la v3).
    </Info>
  </Step>
</Steps>
...
<Accordion title="Erreur JWEDecryptionFailed">
  Si vous voyez une erreur `JWEDecryptionFailed: decryption operation failed`, cela est causé soit par une valeur `AUTH0_SECRET` non valide, soit par un ancien cookie de session chiffré avec un secret différent.
