...
<AccordionGroup>
  ...
  <Accordion title="2 Sécuriser les points de terminaison de l’API">
    ...
    * Obtenir un jeton d’accès. Pour plus d’informations sur la procédure à suivre pour y parvenir, reportez-vous à : [Get an Access Token](/docs/fr-CA/fr-ca/architecture-scenarios/application/server-api#get-an-access-token) (Obtenir un jeton d’accès).
  ...
  <Accordion title="4 Déterminer l’identité de l’utilisateur">
    ...
    Premièrement, [créez une nouvelle Action](/docs/fr-CA/fr-ca/customize/actions/write-your-first-action) qui ajoutera l’adresse courriel de l’utilisateur au jeton d’accès.
    ...
    4. Dans la barre latérale de l’éditeur de code des actions, sélectionnez Test (icône de lecture), puis sélectionnez **Exécuter** pour  [tester votre code](/docs/fr-CA/fr-ca/customize/actions/test-actions).
    ...
    Ensuite, ajoutez l’action créée au [Flux de connexion](https://manage.auth0.com/#/actions/flows/login/). Pour savoir comment attacher des actions à des flux, lisez la section Attacher l’action à un flux dans [Écrire votre première action](/docs/fr-CA/fr-ca/customize/actions/write-your-first-action).
