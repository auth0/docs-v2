...
* Vérifier que le jeton <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=json-web-token" tip="Jeton Web JSON (JWT)
...
<table class="table">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>Le défi-réponse de code généré par le vérificateur de code. Vous trouverez des instructions sur la génération d’un défi-réponse de code <a href="/docs/fr-CA/fr-ca/flows/guides/auth-code-pkce/call-api-auth-code-pkce#authorize-the-user#create-a-code-verifier">ici</a>.</td>
    </tr>
...
Ensuite vous pouvez échanger le `authorization_code` de la réponse pour un jeton d’accès qui peut être utilisé pour appeler votre API. Réalisez une demande `POST` à l’[URL du jeton](/docs/fr-CA/fr-ca/api/authentication#authorization-code-pkce-) incluant les données suivantes :
...
<table class="table">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>Clé cryptographiquement aléatoire utilisée pour générer le `code_challenge` transmis à <a href="/docs/fr-CA/fr-ca/api/authentication#authorization-code-grant-pkce-">authorization URL</a> (`/authorize`).</td>
    </tr>
...
* **refresh&#95;token**: Un [jeton d’actualisation](/docs/fr-CA/fr-ca/tokens/concepts/refresh-tokens) ne sera présent que si vous avez inclus la permission `offline_access` ET activé **Autoriser l’accès hors ligne** pour votre API dans le Dashboard.
