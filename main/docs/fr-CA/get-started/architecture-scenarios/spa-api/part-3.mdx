...
* **scope** : Les [permissions](https://auth0.com/docs/scopes) qui déterminent les informations à renvoyer dans le jeton d’ID et dans le jeton d’accès. Une permission du `openid profile` (profil <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=openid" tip="OpenID
...
Par mesure de sécurité, il est recommandé de limiter la durée de vie du jeton d’accès d’un utilisateur. Lorsque vous créez une API dans le <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=auth0-dashboard" tip="Auth0 Dashboard
