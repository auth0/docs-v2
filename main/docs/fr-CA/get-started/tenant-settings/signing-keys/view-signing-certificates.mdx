...
Vous pouvez afficher les secrets client et les clés de connexion de l’application de votre locataire à l’aide de l’<Tooltip href="/docs/fr-CA/fr-ca/glossary?term=auth0-dashboard" tip="Auth0 Dashboard
...
Effectuez un appel `GET` vers le point de terminaison [`/signing_keys/get_signing_keys`](/docs/fr-CA/fr-ca/api/management/v2#!/signing_keys/get_signing_keys). Assurez-vous de remplacer la valeur de remplacement `{yourMgmtApiAccessToken}` par votre jeton d’accès de Management API.
...
Effectuez un appel `GET` vers le point de terminaison [`/signing_keys/get_signing_key`](/docs/fr-CA/fr-ca/api/management/v2#!/signing_keys/get_signing_key). Assurez-vous de remplacer les valeurs des espaces réservés `{yourKeyId}` et `{yourMgmtApiAccessToken}` par votre ID de clé de connexion et votre jeton d’accès de Management API, respectivement.
...
* [Algorithmes de signature](/docs/fr-CA/fr-ca/get-started/applications/signing-algorithms)
