...
Pour les [navigateurs pris en charge](#browser-testing-support), vous pouvez utiliser la méthode `crossOriginVerification` de la [trousse SDK Auth0.js](/docs/fr-CA/fr-ca/libraries/auth0js) dans votre application sur une page dédiée pour gérer les cas où les témoins de tiers sont désactivés.

Pour les navigateurs qui ne sont pas pris en charge, tels que Chrome, Opera et Safari, l’authentification cross-origin ne fonctionnera pas lorsque les témoins de tiers sont désactivés, sauf si vous active les [domaines personnalisés](/docs/fr-CA/fr-ca/customize/custom-domains).
...
1. Créez une page dans votre application qui instancie `WebAuth` depuis [Auth0.js](/docs/fr-CA/fr-ca/libraries/auth0js). Appelez immédiatement `crossOriginVerification`. Le titre de la page est à votre discrétion.
...
* [Connexion intégrée](/docs/fr-CA/fr-ca/authenticate/login/embedded-login)
* [Authentification inter-origine](/docs/fr-CA/fr-ca/authenticate/login/cross-origin-authentication)
* [Paramètres d’application](/docs/fr-CA/fr-ca/get-started/applications/application-settings)
* [Modifications des attributs du témoin SameSite](/docs/fr-CA/fr-ca/manage-users/cookies/samesite-cookie-attribute-changes)