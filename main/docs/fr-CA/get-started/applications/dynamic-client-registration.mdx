...
Vous pouvez également mettre à jour cet indicateur en utilisant le point de terminaison de <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=management-api" tip="Management API
Un produit permettant aux clients d’effectuer des tâches administratives." cta="Voir le glossaire">Management API</Tooltip> [`/Tenant/patch_settings`](/docs/fr-CA/fr-ca/api/management/v2#!/Tenants/patch_settings).
...
Vous devez mettre à jour `API2_ACCESS_TOKEN` avec un jeton valide avec la permission `update:tenant_settings`. Pour en savoir plus, lisez [Jetons d’accès à Management API](/docs/fr-CA/fr-ca/secure/tokens/access-tokens/management-api-access-tokens).

<div id="use-dynamic-client-registration">
...
<div id="register-your-application">
...
* **client&#95;secret** : secret client alphanumérique de 64 bits. Cette valeur est utilisée par les applications pour s’authentifier auprès de l’Authentication API [`/token`](/docs/fr-CA/fr-ca/api/authentication#get-token) et pour signer et valider les jetons d’ID.
...
Notez l’ID et le secret client, car ce sont les éléments les plus importants pour l’exécution des flux d’authentification et d’autorisation. Pour en savoir plus, lisez [Flux d’authentification et d’autorisation](/docs/fr-CA/fr-ca/get-started/authentication-and-authorization-flow).
...
Nous allons voir un exemple simple, qui montre comment appeler une API à partir d’une application web côté client, en utilisant le [Flux implicite](/docs/fr-CA/fr-ca/get-started/authentication-and-authorization-flow/implicit-flow-with-form-post).
...
<div id="use-tenant-access-control-list-acl">
  ## En savoir plus
</div>
