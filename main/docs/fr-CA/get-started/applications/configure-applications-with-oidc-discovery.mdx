...
Les documents [OpenID Connect (OIDC) Discovery (Découverte OpenID Connect)](https://openid.net/specs/openid-connect-discovery-1_0-final.html#RFC5785)  contiennent des métadonnées sur le fournisseur d’identité (<Tooltip href="/docs/fr-CA/fr-ca/glossary?term=idp" tip="Fournisseur d’identité (IdP)
...
* Les [Demandes et permissions soutenues par l’OIDC](/docs/fr-CA/fr-ca/get-started/apis/scopes/openid-connect-scopes) standard (cela exclut les [demandes personnalisées](/docs/fr-CA/fr-ca/secure/tokens/json-web-tokens/create-custom-claims) et les permissions définies dans votre locataire)
* Fonctionnalités prises en charge par l’IdP

Vous pouvez configurer des applications avec les documents de découverte <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=openid" tip="OpenID
...
Le logiciel médiateur OIDC ne prend pas en charge les <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=json-web-token" tip="Jeton Web JSON (JWT)
