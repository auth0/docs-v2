...
* Tout consommateur tiers de votre ressource peut demander un jeton d’accès pour chaque appel. Les appels excessifs pourraient affecter votre [limite anti-attaques](/docs/fr-CA/fr-ca/troubleshoot/customer-support/operational-policies/rate-limit-policy/rate-limit-configurations/self-service-public).

<div id="create-an-account">
  ## Créer un compte
</div>

Maintenant que vous avez un plan pour votre architecture, vous allez configurer votre compte Auth0 et vos locataires. Lorsque vous vous inscrivez aux services Auth0, vous créez votre premier [locataire](/docs/fr-CA/fr-ca/get-started/auth0-overview/create-tenants). Vous y configurez les actifs, les services et les ressources Auth0. [Inscrivez-vous](https://auth0.com/signup) pour commencer.
...
<Card title="Avant de commencer">
  ...
  * Déterminez si vous devez prendre en charge plusieurs environnements, notamment pour le développement, la simulation et la production. Pour en savoir plus, consultez [Configurer plusieurs environnements](/docs/fr-CA/fr-ca/get-started/auth0-overview/create-tenants/set-up-multiple-environments).
  * Si votre cas d’utilisation implique des applications tierces que vous souhaitez enregistrer dans un locataire, vous pouvez utiliser l’[Enregistrement dynamique d&#39;applications](/docs/fr-CA/fr-ca/get-started/applications/dynamic-client-registration) basé sur le [protocole OIDC pour l’enregistrement des clients](https://openid.net/specs/openid-connect-registration-1_0.html).
...
Maintenant que vous avez un plan pour votre architecture, vous allez configurer votre compte Auth0 et <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=tenant" tip="Locataire
...
<Tabs>
  <Tab title="Auth0 Dashboard">
    ...
    1. Suivez les instructions pour [enregistrer votre API](/docs/fr-CA/fr-ca/get-started/auth0-overview/set-up-apis).
  ...
  <Tab title="Management API">
    Pour enregistrer une API de manière programmatique, utilisez Management API. Vous aurez besoin d’un jeton d’accès pour utiliser Management API. Pour en savoir plus, lisez [Jetons Management API](/docs/fr-CA/fr-ca/secure/tokens/access-tokens/management-api-access-tokens).
    ...
    <table class="table">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>Le profil détermine le format des jetons d’accès émis pour l’API. Les valeurs possibles sont `Auth0` et `RFC 9068`. Pour en savoir plus, lisez la section <a href="/docs/fr-CA/fr-ca/secure/tokens/access-tokens/access-token-profiles">Profils de jeton d’accès</a>.</td>
          <td>`access_token`</td>
        </tr>
...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  L’API d’Auth0 a plusieurs paramètres que vous devriez passer en revue avant la configuration. Pour en savoir plus, consultez[Paramètres de l’API](/docs/fr-CA/fr-ca/get-started/apis/api-settings).
</Callout>
...
<Tabs>
  <Tab title="Auth0 Dashboard">
    ...
    2. Sélectionnez l’application de test de communication entre machines (M2M) créée lors de la création de votre API
       ...
       <Callout icon="file-lines" color="#0EA5E9" iconType="regular">
         Vous pouvez créer une autre application pour le développement ou la production ultérieurement en suivant les instructions pour [Enregistrer les applications de communication entre machines](/docs/fr-CA/fr-ca/get-started/auth0-overview/create-applications/machine-to-machine-apps).
       </Callout>
    ...
    5. Sélectionnez **Update (Mise à jour)**.
       ...
       <Frame>![Dashboard > Applications > APIs](/docs/images/fr-ca/cdy7uua7fh8z/6L2R46XVdYw1kifRfMifRz/edee29a7ba67ba10f2ced2543f6ef581/Timesheet_API_Config.png)</Frame>
  ...
  <Tab title="Management API">
    ...
    Pour en savoir plus sur les réglages de l’application et du locataire, lisez [Paramètres du locataire](/docs/fr-CA/fr-ca/get-started/tenant-settings).
  </Tab>
</Tabs>
...
Pour les applications ou services non interactifs à authentifier, vous devez sélectionner une autorisation client ou un flux d’authentification. Le [Flux des identifiants client](https://tools.ietf.org/html/rfc6749#section-4.4) <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=oath2" tip="OAuth 2.0
...
<Card title="Avant de commencer">
  ...
  * Examinez le [Flux des identifiants client](/docs/fr-CA/fr-ca/get-started/authentication-and-authorization-flow/client-credentials-flow) pour l’authentification machine à machine. Voici le flux de travail pour l’authentification et l’autorisation non interactive.
  * Déterminez le niveau d’accès pour vos API. Cela permet de déterminer quelles [permissions](/docs/fr-CA/fr-ca/get-started/apis/scopes/api-scopes) vous configurerez lors de la création de votre API.
</Card>
...
Vous pouvez utiliser <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=auth0-dashboard" tip="Auth0 Dashboard
...
Suivez les instructions de la section [Mettre à jour les types d’autorisation](/docs/fr-CA/fr-ca/get-started/applications/update-grant-types) pour utiliser Auth0 Dashboard ou Management API.

<div id="m2m-access-tokens">
  ### Jetons d’accès M2M
</div>

Dans l’authentification basée sur des jetons, les clients non interactifs fournissent `client_id` et `client_secret` dans un appel au [point de terminaison du jeton Authentication API](https://auth0.com/docs/customize/actions/flows-and-triggers/machine-to-machine-flow) pour obtenir un [jeton d’accès](/docs/fr-CA/fr-ca/secure/tokens/access-tokens). Ce jeton d’accès permet d’accéder à votre API protégée.

Le profil ou format par défaut est le profil de jeton Auth0 associé à deux profils de jeton. Vous pouvez choisir de modifier le profil du jeton en choisissant RFC 9068. Pour en savoir plus, lisez [Profils de jetons d’accès](/docs/fr-CA/fr-ca/secure/tokens/access-tokens/access-token-profiles). Pour vérifier que le jeton est valide, votre API vérifiera les [Algorithmes de signature](/docs/fr-CA/fr-ca/get-started/applications/signing-algorithms). L’algorithme de signature par défaut est RSA256, un algorithme basé sur une clé.

<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  Auth0 prend en charge d’autres méthodes d’authentification client en plus de fournir l’ID client et le secret client en tant qu’identifiants. Ces méthodes, y compris notre recommandation de [Clé privée JWT](/docs/fr-CA/fr-ca/get-started/authentication-and-authorization-flow/authenticate-with-private-key-jwt) pour les configurations M2M, sont disponibles avec un plan Entreprise. Pour en savoir plus, consultez [Identifiants d’application](/docs/fr-CA/fr-ca/secure/application-credentials).
</Callout>
...
Vos jetons d’accès ont une limite de durée de validité. Étant donné que vos communications se déroulent sur le canal d&#39;appui, l’utilisation de jetons d’actualisation pour prolonger les sessions n’est pas possible. Il est donc recommandé de configurer vos jetons d’accès avec un délai d’expiration d’une heure. Il vous faudra peut-être trouver le bon équilibre entre sécurité et performance en fonction des besoins spécifiques de votre environnement. Pour en savoir plus, veuillez consulter [Mettre à jour la durée de vie du jeton d’accès](/docs/fr-CA/fr-ca/secure/tokens/access-tokens/update-access-token-lifetime).

<div id="scopes">
  ### Permissions
</div>

Avant que tout client ou service non interactif n’appelle votre API, vous devez définir les autorisations ou les portées autorisées par votre API. Vous pouvez définir les permissions dans Auth0 Dashboard afin de les inclure dans votre demande à l’Authentication API. Pour lire plus d’exemples de façons d’utiliser les permissions des API, veuillez consulter [Permissions des API](/docs/fr-CA/fr-ca/get-started/apis/scopes/api-scopes).

Pour configurer les permissions, veuillez consulter les instructions [Ajouter des autorisations API](/docs/fr-CA/fr-ca/get-started/apis/add-api-permissions) pour Auth0 Dashboard ou utilisez l’exemple fourni pour Management API.
...
<Card title="Intégration M2M – domaines personnalisés">
  ...
  * Vérifiez que la version TLS (SSL) et le chiffre que vous souhaitez utiliser pour les certificats autogérés sont pris en charge par Auth0. Pour en savoir plus, lisez [Versions et chiffres TLS (SSL)](/docs/fr-CA/fr-ca/customize/custom-domains/self-managed-certificates/tls-ssl).
</Card>

1. Pour configurer votre domaine personnalisé avec des certificats gérés par Auth0, suivez les instructions fournies à la section [Configurer des domaines personnalisés avec des certificats gérés par Auth0](/docs/fr-CA/fr-ca/customize/custom-domains/auth0-managed-certificates).

   1. Si vous souhaitez gérer vos propres certificats, suivez les instructions fournies à la section [Configurer des domaines personnalisés avec des certificats autogérés](/docs/fr-CA/fr-ca/customize/custom-domains/self-managed-certificates).

      <Callout icon="file-lines" color="#0EA5E9" iconType="regular">
        Vous devez avoir un abonnement Enterprise pour gérer les certificats dans votre domaine personnalisé. Pour en savoir plus, lisez [Tarification](https://auth0.com/pricing/) et [connexion](/docs/fr-CA/fr-ca/authenticate/login) Auth0.
      </Callout>
...
Si vous rencontrez des problèmes avec votre domaine personnalisé, veuillez consulter la section [Dépannage des domaines personnalisés](/docs/fr-CA/fr-ca/troubleshoot/integration-extensibility-issues/troubleshoot-custom-domains).
...
<Frame>![Auth0 Dashboard > Readiness Checklist](/docs/images/fr-ca/cdy7uua7fh8z/7iu1CzH0NgaXJbLBNWXZli/259f149b1c3ecb06f17f0b875c330652/Readiness_Checks_-_French.png)</Frame>
...
Nous vous conseillons de lire [Bonnes pratiques de déploiement](/docs/fr-CA/fr-ca/deploy-monitor/deployment-best-practices) pour les dernières vérifications avant de passer en situation réelle et de tirer parti des [Logs](/docs/fr-CA/fr-ca/deploy-monitor/logs) (Journaux) pour surveiller vos services.

<div id="learn-more">
  ## En savoir plus
</div>

* [Présentation d’Auth0](/docs/fr-CA/fr-ca/get-started/auth0-overview)
* [Aperçu de solution (Applications serveur + ](/docs/fr-CA/images/s/fr-ca/get-started/architecture-scenarios/server-application-api/part-1)
* [Effectuer une requête à votre API à l’aide du Flux des identifiants client](/docs/fr-CA/fr-ca/get-started/authentication-and-authorization-flow/client-credentials-flow/call-your-api-using-the-client-credentials-flow)
* [Récupérer les événements de journal à l’aide de Management API](/docs/fr-CA/fr-ca/deploy-monitor/logs/retrieve-log-events-using-mgmt-api)
* [Obtenir des jetons d’accès à Management API pour la production](/docs/fr-CA/fr-ca/secure/tokens/access-tokens/management-api-access-tokens/get-management-api-access-tokens-for-production)