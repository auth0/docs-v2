...
Auth0.js est une bibliothèque du côté client d’Auth0. Il est recommandé de l’utiliser en conjonction avec la [connexion universelle](/docs/fr-CA/fr-ca/universal-login), qui devrait être utilisée chaque fois que possible. Utiliser auth0.js dans votre SPA rend plus facile l’authentification et l’autorisation avec Auth0.
...
<table class="table">
  ...
  <tbody>
    ...
    <tr>
      <td>`state`</td>
      <td>facultatif</td>
      <td>(Chaîne)  Une valeur arbitraire qui devrait être maintenue dans les redirections. Il est utile pour atténuer les attaques CSRF et pour toute information contextuelle (par exemple, une URL de retour) dont vous pourriez avoir besoin après le processus d’authentification. Pour plus d’informations, consultez <a href="/docs/fr-CA/fr-ca/protocols/oauth2/oauth-state">Paramètres d’état</a>. auth0.js, lorsqu’il est utilisé dans les applications à page unique, gère la génération et la validation d’état automatiquement si elle n’est pas spécifiée.</td>
    </tr>

    <tr>
      <td>`prompt`</td>
      <td>facultatif</td>
      <td>(Chaîne) Une valeur de `login` forcera l’affichage de la page de connexion, quelle que soit la session en cours. Une valeur de `none` tentera de contourner les invites de connexion si une session existe déjà (voir la documentation sur l’<a href="/docs/fr-CA/fr-ca/sso/current/single-page-apps#silent-authentication">authentification silencieuse</a> pour plus de détails).</td>
    </tr>
  </tbody>
</table>
...
La méthode `login` permet une [authentification cross-origin](/docs/fr-CA/fr-ca/authenticate/login/cross-origin-authentication) pour les connexions par base de données, en utilisant `/co/authenticate`.
...
Les applications avec connexion intégrée doivent remplir deux critères afin de bénéficier de l’Authentification unique (<Tooltip href="/docs/fr-CA/fr-ca/glossary?term=single-sign-on" tip="Authentification unique (SSO)
...
Nous recommandons d’utiliser une connexion universelle au lieu de configurer la SSO dans des scénarios de connexion intégrée. La connexion universelle est la manière la plus fiable et stable de mettre en place la SSO, et c’est la seule option si vous devez utiliser plusieurs domaines pour vos applications, ou si vous utilisez des [applications tiers](/docs/fr-CA/fr-ca/get-started/applications/confidential-and-public-applications/enable-third-party-applications).

<div id="passwordless-login">
  ## Connexion sans mot de passe
</div>

L’authentification sans mot de passe permet aux utilisateurs de se connecter en recevant un mot de passe à usage unique (OTP) par courriel ou SMS. Ce processus demande que vous lanciez le processus <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=passwordless" tip="Sans mot de passe
...
Vous pouvez maintenant faire quelque chose d’autre avec cette information, selon les besoins de votre application, tel que d’acquérir l’ensemble des informations de profil de l’utilisateur dans <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=management-api" tip="Management API
...
<table class="table">
  ...
  <tbody>
    ...
    <tr>
      <td>`user_metadata`</td>
      <td>optionnel</td>
      <td>(objet JSON) Attributs additionnels utilisés pour les informations de l&#39;utilisateur. Ils seront stockés dans <a href="/docs/fr-CA/fr-ca/users/concepts/overview-user-metadata">user&#95;metadata</a></td>
    </tr>
  </tbody>
</table>
...
Notez que `checkSession()` déclenche toutes les [règles](/docs/fr-CA/fr-ca/customize/rules) que vous avez définies, vous devez donc vérifier vos règles dans le [Dashboard](https://manage.auth0.com/#/rules) avant de l’utiliser.
...
Cependant, le navigateur **doit** avoir activé les témoins de tierce-partie. Sinon, **checkSession()** est incapable d’accéder à la session courante de l’utilisateur (rendant impossible d’obtenir un nouveau jeton sans afficher quelque chose à l’utilisateur). La même chose se produira si les utilisateurs ont [activé l’ITP de Safari](/docs/fr-CA/fr-ca/troubleshoot/authentication-issues/renew-tokens-when-using-safari).
...
Management API offre une fonctionnalité qui vous permet de lier et de dissocier des comptes utilisateurs distincts provenant de différents fournisseurs, en les liant à un profil unique (consultez [Association de comptes d’utilisateur](/docs/fr-CA/fr-ca/manage-users/user-accounts/user-account-linking) pour plus de renseignements). Ceci vous permet aussi de mettre à jour les métadonnées utilisateur.
