...
Lorsque vous cliquez sur le bouton <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=idp" tip="Fournisseur d’identité (IdP)
...
L’authentification multifacteur (<Tooltip href="/docs/fr-CA/fr-ca/glossary?term=multifactor-authentication" tip="Authentification multifacteur (MFA)
...
Certaines fonctionnalités d’Auth0, telles que l’[authentification unique (SSO)](/docs/fr-CA/fr-ca/authenticate/single-sign-on) entre plusieurs applications, dépendent du fait que les utilisateurs soient redirigés vers Auth0 pour définir un témoin sur `'{yourDomain}'`.
...
Si vous ne souhaitez pas afficher de fenêtre contextuelle et que vous n’avez pas besoin de la <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=single-sign-on" tip="Authentification unique (SSO)
