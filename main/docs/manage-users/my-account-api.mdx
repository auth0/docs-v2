...
Enrolling TOTP is a two-step process. Start enrollment to receive a QR code and manual secret for the user to add to their authenticator app, then confirm by submitting the one-time code the app generates.
...
##### Step 2: Confirm TOTP enrollment

Submit the OTP code from the user's authenticator app along with the `auth_session` from the previous response. Replace `{id}` with the `id` returned in Step 1.
