...
The maximum length is 64 characters for the user/local part. The total email address (local part, `@`, and domain combined) cannot exceed 254 characters. This value is validated by the [JSON validation schema](https://json-schema.org/understanding-json-schema/reference/type#email-addresses).
</ResponseField>
