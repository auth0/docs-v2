...
---
Update user profiles when [using your own database as an identity provider](/docs/authenticate/identity-providers/database/custom-db) by doing the following tasks:
...
If you have [enabled user migration](/docs/authenticate/identity-providers/database/migrating), and a user has already been migrated to the Auth0 database, then Auth0 will not query your database again for the user profile. Therefore, all changes made in the custom database for that user will never reflect in Auth0.
