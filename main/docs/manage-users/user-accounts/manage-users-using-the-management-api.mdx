...
| **Endpoint type** | **Use** |
|---|---|
| **[`/users`](https://auth0.com/docs/api/management/v2/users/get-users)** | [Retrieve information about all users](https://auth0.com/docs/api/management/v2/users/get-users). You can also include search criteria to find specific users. |
| **[`/user_id`](https://auth0.com/docs/api/management/v2/users/get-users-by-id)** | [Retrieve information about one user](https://auth0.com/docs/api/management/v2/users/get-users-by-id) based on the `user_id`. The `user_id` is an internal identifier that consists of a connection name and a unique identifier for the user. The `user_id` is different from the <Tooltip tip="ID Token: Credential meant for the client itself, rather than for accessing a resource." cta="View Glossary" href="/docs/glossary?term=ID+token">ID token</Tooltip>. |
| **[`/userinfo`](https://auth0.com/docs/api/authentication/reference#get-user-info)** | Takes the Auth0 access token and [returns user profile information](https://auth0.com/docs/api/authentication/reference#get-user-info). Includes the results of any rules that altered the user profile during the authentication transaction, but the resulting profile will not be filtered by any [Scoping](/docs/get-started/apis/scopes/api-scopes). |
| **[`/tokeninfo`](https://auth0.com/docs/api/authentication/reference#get-token-info)** | Takes the Auth0 ID token and [returns user profile information](https://auth0.com/docs/api/authentication/reference#get-token-info). Does not include the results of any rules that alter the user profile. |
