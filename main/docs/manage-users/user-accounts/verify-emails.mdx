...
You can choose from the following email verification methods in Auth0:

| **Verification Method** | **Description** | **Configuration Method** |
|---|---|--- |
| **Email Verification Links** | An email verification link is sent to the given email address. When the link is clicked, the email is verified. | Configure Auth0's verification flow or use Management API |
| **One-time passwords** | A temporary code is sent to the given email address. When the user inputs that code, the email is verified. | onfigure Auth0's verification flow or use Management API |
| **Bulk verification** | A list of users are manually verified at the same time. | Management API |
| **Custom verification** | The email address is verified through a custom workflow. | Special email verification flow or Management API |
...
* Use the [email verification job](https://auth0.com/docs/api/management/v2/jobs/post-verification-email). This triggers Auth0 to send the verification email using the verify email template
* Create an [email verification ticket](https://auth0.com/docs/api/management/v2/tickets/post-email-verification) and send the email yourself, including the ticket the user should use to verify their email
