...
|---|---|
| **Auth0's built-in verification flow** | Auth0 automatically sends a verification email upon signup. |
| **API endpoints** | Use the Auth0 Management API to set a user's email verification status programmatically for bulk or custom workflows. |
| **Special email verification flow** | Auth0 supports custom email verification workflows for users from Azure AD and ADFS enterprise connections. |
...
|---|---|
| **Email Verification Links** | An email verification link is sent to the given email address. When the link is clicked, the email is verified. |
| **One-time passwords** | A temporary code is sent to the given email address. When the user inputs that code, the email is verified. |
| **Bulk verification** | A list of users are manually verified at the same time. |
| **Custom verification** | The email address is verified through a custom workflow. |
