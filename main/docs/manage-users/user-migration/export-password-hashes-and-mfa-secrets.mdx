...
description: Request a PGP-encrypted export of password hashes and MFA secrets from your Auth0 tenant through a support case.
---
...
This section uses [GnuPG (`gpg`)](https://gnupg.org/), the standard command-line tool for OpenPGP. For other tools or advanced options, see the [GnuPG manual](https://www.gnupg.org/gph/en/manual.html).
...
- Use the exact email address registered to your tenant administrator account when generating the key. Aliases or different capitalization may cause validation to fail. The support request must also be submitted by the same tenant administrator who generated the key.
<br></br>
...
<Steps>
  ...
  [Open a support case](https://support.auth0.com/center/s/) requesting a password hash or MFA secrets export. Include the following in your request:
