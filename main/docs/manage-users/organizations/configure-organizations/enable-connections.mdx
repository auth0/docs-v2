...
| Organization Member Access Level | All connection types | This property determines what level of access Organization Admins have to manage members authenticated via this connection using the [My Organization API](/docs/manage-users/my-organization-api). The available values are: <ul><li>**None**: Organization Admins cannot view or manage members authenticated via this connection using the My Organization API.</li> <li>**Read-only**: Organization Admins can view members authenticated via this connection using the My Organization API, but cannot modify them.</li> <li>**Limited**: Organization Admins can perform selected member management actions for members authenticated via this connection.</li> <li>**Full**: Organization Admins can perform all allowed member management actions for members authenticated via this connection.</li></ul> |
...
4. In the Organization Connection section, configure both properties required to leverage a connection via the My Organization API:
  ...
  * Locate **Organization Member Access Level** and if not populated, choose the level of access that you'd like Organization Admins to have for managing members authenticated via this connection.
