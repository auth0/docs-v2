...
redirects:
  - "/docs/organizations/configure/send-membership-invitations"
  - "/docs/organizations/send-membership-invitations"
---
...
3. Select the Application to which you want to invite the member, enter the email address of the user you want to invite to the Organization, and select **Send Invite(s)**.

   <Callout icon="file-lines" color="#0EA5E9" iconType="regular">

       Optionally, you can specify the connection with which you want users to accept the invitation, and roles you want to assign to the Organization member when they accept. To pre-assign an Organization role, select a role scoped to this Organization from the role selector.
...
| `ROLE_ID` | ID of the role(s) you want to assign to the invited user for the specified Organization. Pass the ID of an Organization role to pre-assign an Organization-scoped role. The `roles` array is optional; omitting it results in the invitee joining with no roles assigned. Maximum of 50 roles per member. |
| `SEND_INVITATION_EMAIL_OPTION` | Indicates whether Auth0 should send the email. Values are `true` or `false`. When set to `false`, Auth0 generates an invitation URL that you can deliver to users through your own email service. |
