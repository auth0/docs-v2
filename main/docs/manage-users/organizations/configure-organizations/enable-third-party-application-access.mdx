...
When you enable [per-application access](/docs/manage-users/organizations/configure-organizations/grant-application-access) for an Organization, this policy only applies to third-party applications not explicitly listed in the Organization's Applications tab. Third-party application you add to the list follow the individual Member Access setting instead.
</Callout>

<Frame>![Organization Settings showing Third-Party Application Access options](/docs/images/third-party-applications/org-third-party-access-settings.png)</Frame>
...
<AuthCodeGroup>
```bash Auth0 CLI wrap lines
auth0 api get "organizations/ORG_ID"
```
```bash cURL wrap lines
curl --request GET \
  --url 'https://YOUR_DOMAIN/api/v2/organizations/ORG_ID' \
...
This domain-level connection requirement doesn't apply to third-party application access granted through [Cross App Access (XAA)](/docs/ai-agents-mcp/cross-app-access). For XAA, third-party application access is approved directly in the enterprise IdP's Cross App Access policy. To learn more, read [Environment setup](/docs/ai-agents-mcp/cross-app-access/set-up-xaa-test-environment).
