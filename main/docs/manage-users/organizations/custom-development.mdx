---
description: Customize Auth0 Organizations behavior with Actions and SDKs, including organization-aware login flows, role assignment, and member-context tokens.
title: Custom Development with Organizations
redirects:
  - "/docs/organizations/custom-development"
---
...
If you change token content using the `event` object within an Action, your changes are available in tokens after all Actions have finished running.
...
The following are example tasks you can perform with organizations using the SDKs.
...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
This approach assumes you control the application code. For third-party applications, you cannot guarantee the external developer passes the `organization` parameter. Use [Prompt for Credentials or Prompt for Organization](/docs/manage-users/organizations/login-flows-for-organizations) instead.
</Callout>
...
If the user was authenticated using an organization, the organization ID appears in the `org_id` claim in the ID token. Using the Auth0 SPA SDK, retrieve it as follows:
