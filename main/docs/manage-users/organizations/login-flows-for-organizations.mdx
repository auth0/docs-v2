---
title: How to Configure Login Flows for Auth0 Organizations
sidebarTitle: Configure Login Flows
description: Configure Auth0 Organizations login flows, including prompt settings, Identifier First Authentication, domain discovery, and auto-membership.
...
[Auth0 Organizations](/docs/manage-users/organizations) allow leaders of B2B products or SaaS applications to build multi-tenant architectures, store identification tokens appropriately, and minimize end user login friction.
...
Even when selecting Prompt for Credentials, you can still direct users to [log in with a specific organization’s login prompt](/docs/manage-users/organizations/custom-development#i-want-users-to-log-in-to-a-specified-organization) as needed.
</Callout>
...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
If your Organization allows third-party application access, third-party applications are controlled by external developers whose code you cannot modify. They may not send the `organization` parameter in authorization requests. Configure **Prompt for Credentials** or **Prompt for Organization** (optionally with **Organization Domain Discovery**) to ensure users are routed to the correct Organization context. To learn more, read [Enable Third-Party Application Access for an Organization](/docs/manage-users/organizations/configure-organizations/enable-third-party-application-access).
</Callout>
...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
If your Auth0 tenant has a [Default Directory](https://manage.auth0.com/#/tenant/general) configured in its settings, Auth0 uses it as the connection for this login flow, even if Default Directory not enabled on the application. The Default Directory takes priority over the application’s enabled connections.

This behavior applies to the **Prompt for Credentials** flow only. For the Prompt for Organization flow or no-prompt scenarios, you must enable the Default Directory on the application for Auth0 to select it.
</Callout>
...
The user begins by entering either their email address (for example, `alice@rockymountainadventures.com`) or their organization name (for example, `Rocky Mountain High Adventures`).

The prompt screen contents depend on the application’s **Login Experience > Type of Users** setting:

* **Business Users** shows only the Organization prompt and users must log in with an Organization.

* **Both** shows both the Organization and personal account prompts and users may log in with either.

### Organization Domain Discovery (optional)

Auth0 supports Organization Domain Discovery, which can optionally be used to detect the user’s Organization before Identifier First Authentication runs.

When configured, Organization Domain Discovery helps Auth0 detect a user's organization automatically or narrow organization options when users enter their email address. Organization Domain Discovery does not restrict who can sign up or log in.

Together, Identifier First Authentication and Organization Domain Discovery streamline enterprise login flows while maintaining clarity and control across multi-organization environments.

If a tenant has [configured verified Organization domains](./configure-organizations/create-org-domains) and no `organization` parameter is provided, Auth0 attempts to determine the user’s organization automatically based on verified (not pending) domains:
...
* If no match is found, Auth0 displays an error.
...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
There are use cases - such as multiple database configurations assigned to different Organizations - in which Auth0 cannot determine which IdP a user’s email is associated with. In these cases, select Prompt for Organization as your login initiation prompt or send the `organization` parameter to Auth0.
</Callout>

You can use the Management API to configure Identifier First Authentication. Specifically, use the `identifier_first` parameter of the [Update prompts settings](/docs/api/management/v2/prompts/patch-prompts) endpoint.
...
You can use the Management API to configure auto-membership. Specifically, use the `assign_membership_on_login` parameter of the [Modify an organization's connection](/docs/api/management/v2/organizations/patch-enabled-connections-by-connection-id) endpoint.
