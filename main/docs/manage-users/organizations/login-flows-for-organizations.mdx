...
If your tenant has a Default Directory configured (navigate to **Auth0 Dashboard > Tenant Settings > General**), Auth0 uses it as the connection for this login flow — even if it is not enabled on the application. The Default Directory takes priority over the application’s enabled connections.
