...
The final permissions granted to the application will be determined by the intersection of the scopes allowed by the application API access policy, Role-Based Access Control (RBAC) permissions assigned to the end user, and any given user consent.
...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
he My Organization API will always remain opt-in for security reasons. Disabling the API removes access for all connected applications until re-enabled.
</Callout>
...
#### Configure client application endpoints
...
| **Scope** | **Description** |
| --- | --- |
| `read:my_org:configuration` | Read configuration and strategy settings |
| `read:my_org:details` | Read Organization details |
| `update:my_org:details` | Modify Organization details |
| `read:my_org:identity_providers` | Read identity provider details |
| `create:my_org:identity_providers` | Create new identity providers |
| `update:my_org:identity_providers` | Update identity providers |
| `delete:my_org:identity_providers` | Delete identity providers |
| `update:my_org:identity_providers` | Refresh identity provider attributes mapping |
| `update:my_org:identity_providers_detach` | Remove an identity provider from an Organization |
| `create:my_org:identity_providers_domains` | Add a domain to an identity provider |
| `delete:my_org:identity_providers_domains` | Remove a domain from an identity provider |
| `create:my_org:domains` | Create a new domain for an Organization |
| `read:my_org:domains` | Read domain details |
| `update:my_org:domains` | Verify a domain |
| `delete:my_org:domains` | Delete a domain from an Organization |
| `create:my_org:identity_providers_scim_tokens` | Create SCIM tokens |
| `read:my_org:identity_providers_scim_tokens` | List SCIM tokens for an identity provider |
| `delete:my_org:identity_providers_scim_tokens` | Revoke SCIM tokens |
| `create:my_org:identity_providers_provisioning` | Create a provisioning configuration for an identity provider |
| `read:my_org:identity_providers_provisioning` | View provisioning configuration details |
| `delete:my_org:identity_providers_provisioning` | Delete provisioning configuration for an identity provider |
| `update:my_org:identity_providers_provisioning` | Refresh provisioning configuration for an identity provider |
