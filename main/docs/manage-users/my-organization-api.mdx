...
* Third-party application access settings for Organizations and their connections
* [Cross App Access (XAA)](/docs/ai-agents-mcp/cross-app-access) Resource Application settings for connections
* Member invitations
* Member management
* Member role assignment and removal
* User store discovery for invitation creation
...
| `third_party_client_access` | **Object.** Controls which values Organization Admins can set for third-party application access on this Organization via the My Organization API. Shape: `{ "default_value": "block" \| "allow", "allowed_values": ["block", "allow"] }`. Defaults to `{ "default_value": "block", "allowed_values": ["block"] }`, means Organization Admins cannot enable third-party application access until explicitly allowed.|
| `invitation_landing_client_id` | **String.** The Client ID of the application to which users are redirected when accepting a member invitation. Required if the application uses the `create:my_org:member_invitations` scope. The specified client must be enabled for the Organization when the invitation is created.|
| `enforce_permission_ceiling` | **Boolean.** When enabled, prevents an Organization Admin from assigning any role whose permissions exceed their own effective permissions at the time of the operation. Defaults to `false`.|
| `enforce_self_assignment_restriction` | **Boolean.** When enabled, prevents an Organization Admin from assigning any role to themselves. Defaults to `false`.|

The [Connection Profile](/docs/authenticate/enterprise-connections/connection-profile), `connection_profile_id`, controls whether Organization Admins can configure a connection to act as a [Cross App Access (XAA)](/docs/ai-agents-mcp/cross-app-access) Resource Application via the My Organization API.
...
4. Configure the following Settings:<br></br>
    ...
    C. Configure the Supported Identity Providers. <br></br>
       1. Enable one or more identity providers. Organization Admins can select their preferred option from the list of enabled providers. <br></br>
    ...
    F. Configure the User Access Authorization to Unauthorized, Authorized, or All. <br></br>
    ...
    G. Configure the Client Credential Access Authorization to Unauthorized, Authorized, or All. <br></br>
       1. **Unauthorized**. No permission allowed. <br></br>
       2. **Authorized**. Select desired permissions. <br></br>
       3. **All**. Includes all existing and future permissions. <br></br>

    H. **Optional**. Configure the Invitation Landing Client. <br></br>
       1. Select the client that Organization members are redirected to when accepting a member invitation. Required if the application uses the `create:my_org:member_invitations` scope. <br></br>

    I. **Optional**. Configure Permission Guardrails. <br></br>
       1. Enforce Permission Ceiling: When enabled, prevents an Organization Admin from assigning any role whose permissions exceed their own effective permissions. <br></br>
       2. Enforce Self-Assignment Restriction: When enabled, prevents an Organization Admin from assigning any role to themselves. <br></br>
...
The My Organization API can only be called with a user-bound access token obtained through one of the supported [OAuth 2.0 flows](/docs/get-started/authentication-and-authorization-flow).
...
#### Organization Admin permissions in ID token

When the `/my-org` audience is present in a user-based authorization request, the ID token automatically includes the `urn:auth0:my_org_current_user_permissions` claim. This claim is not available in machine-to-machine flows. It contains all `my_org:*` permissions the authenticated user holds via their assigned roles and is used by Embeddable UI Components to determine which actions to surface.
...
| `create:my_org:member_invitations` | Create member invitations for an organization |
...
| `read:my_org:members` | List members in an organization |
| `delete:my_org:memberships` | Remove a member (delete membership) from an organization without deleting the underlying user |
| `read:my_org:member_roles` | List roles assigned to an organization member |
| `create:my_org:member_roles` | Assign roles to an organization member |
| `delete:my_org:member_roles` | Unassign roles from an organization member |
| `read:my_org:user_stores` | Read user stores for an organization |

## Endpoint reference

The My Organization API supports endpoints for configuration, Organization details, identity providers, domains, provisioning configurations, and SCIM Tokens, member invitations, members, member roles, and user stores. For a complete reference guide on the endpoints, including the schemas, error codes, etc., refer to our [API Explorer](https://auth0.com/docs/api/myorganization).
...
These event codes allow you to track activities across all resources managed by the API, specifically: configuration, Organization details, IdPs, domains, members, member invitations, member roles, and user stores. If you want to learn more about log event schemas, you can reference our [GitHub repository](https://github.com/auth0/auth0-log-schemas).
...
| `my_organization_api_member_succeeded` | My Organization API Member Succeeded | Successful API call to the member resource of the My Organization API service. Covers all member operations including role assignment and removal. |
| `my_organization_api_member_failed` | My Organization API Member Failed | Failed API call to the member resource of the My Organization API service |
| `my_organization_api_invitation_succeeded` | My Organization API Invitation Succeeded | Successful API call to the invitation resource of the My Organization API service |
| `my_organization_api_invitation_failed` | My Organization API Invitation Failed | Failed API call to the invitation resource of the My Organization API service |
| `my_organization_api_role_succeeded` | My Organization API Role Succeeded | Successful API call to the role resource of the My Organization API service |
| `my_organization_api_role_failed` | My Organization API Role Failed | Failed API call to the role resource of the My Organization API service |
| `my_organization_api_user_stores_failed` | My Organization API User Stores Failed | Failed API call to the user stores resource of the My Organization API service |
...
| `organization_access_level` | **Enum (null, none, readonly, limited, full)**. Determines the type of access a user will have when using the My Organization API: <p></p> 1. `null` <p></p> 2. `none` - Association assigned by the Tenant Administrator. This connection cannot be seen or edited via the My Organization API. <p></p> 3. `readonly` - Association assigned by the Tenant Administrator. This connection can be seen, but cannot be edited via the My Organization API. <p></p> 4. `limited` - Association assigned by the Tenant Administrator. Limited information and modifications are permissible via the My Organization API. <p></p> 5. `full` - Association assigned by the Tenant Administrator. Users of the My Organization API will be able to modify the Connection without additional restrictions when using the My Organization API. Note: Regardless of organization_access_level, the My Organization API powered modifications are always subject to restrictions enforced by the Connection Profile. |
...
| `organization_member_access_level` | **Enum (none, readonly, limited, full).** Controls what an Organization Admin can do to members authenticated via this connection. Configurable by Tenant Admin only, per Organization. The same underlying connection can have different values across different Organizations. <p></p> 1. `none` — Member is not visible to Organization Admins. All operations return 404 Not Found. <p></p> 2. `readonly` — Organization Admin can view and list the member and view their roles. No modifications permitted. <p></p> 3. `limited` — All readonly operations, plus remove the member from the Organization and assign or remove roles. <p></p> 4. `full` — All limited operations, plus delete the member's underlying user account and patch member attributes (governed by the User Attribute Profile). <p></p> Default: when `organization_access_level` is `full`, `organization_member_access_level` defaults to `full`. Otherwise, it defaults to `limited`. Tenant Admin can override this value at any time via `PATCH /api/v2/organizations/{id}/connections/{connectionId}`.|
...
* `organization_member_access_level` can be configured via the Management API or through the [Auth0 Dashboard > Organizations](https://manage.auth0.com/#/organizations) connection settings.
