...
| `invitation_landing_client_id` | **String.** The Client ID of the application users are redirected to when accepting a member invitation. Required if the application uses the `create:my_org:member_invitations` scope. The specified client must be enabled for the Organization when the invitation is created.|
