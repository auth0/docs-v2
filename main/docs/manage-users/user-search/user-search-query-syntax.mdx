---
title: Lucine Query Syntax Reference for Auth0 User Search
sidebarTitle: Search Query Syntax
...
This content describes Auth0 user search version 3.
</Callout>
...
| `last_login` | date time | The timestamp of when the user last logged in. In case this property executes from inside a [Rule](/docs/customize/rules) with the `user` object, the value will be associated with the login that triggered the rule (since rules execute after the actual login). |
...
For example, consider the following `user_metadata` object:
...
| Filter on... | Example query |
|--|--|
| Attribute | `q: _exists_:user_metadata.favorite_color` |
| Nested object attribute | `q: _exists_:user_metadata.preferences.fontSize` |
| Nested object value | `q: user_metadata.preferences.fontSize:13` |
| Nested array value | `q: user_metadata.addresses.city:"Seattle"` |
