...
description: Use Auth0 Organizations to manage multiple B2B customers in a single tenant with per-organization branding, connections, members, and roles. (Describes how Auth0 Organizations work, use cases, a sample scenario, planning, and limitations.)
---
...
<Frame>![B2B Organizations Business Case diagram](/docs/images/cdy7uua7fh8z/4l47Xknr2LpuMSOX0T3yCP/800877a39f474faa0c6d83551d56c337/b2b-business-case.png)</Frame>
...
By default, Organization members can access any application with enabled Organizations and an active connection configured. This is the implicit access model.

You can switch individual organizations to an explicit access model using [per-application access](/docs/manage-users/organizations/configure-organizations/grant-application-access). When enabled for an organization, first-party applications without an explicit grant are blocked. Third-party applications without an explicit grant continue to follow the organization's Third-Party Application Access policy. Other organizations in your tenant are unaffected.

Use the explicit model when you need to:

* Restrict which products a specific customer organization has purchased access to.
* Prevent members from accessing applications their organization has not been provisioned for.
* Manage application entitlements without custom Actions or `app_metadata` workarounds.

To get started, read [Grant Application Access to an Organization](/docs/manage-users/organizations/configure-organizations/grant-application-access).
...
Third-party applications are supported with Organizations. To learn how to enable a third-party application for your Organization, read [Enable Third-Party Application Access for an Organization](/docs/manage-users/organizations/configure-organizations/enable-third-party-application-access).
