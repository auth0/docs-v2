---
title: Manage Sessions with Actions
description: Manage Auth0 user sessions with Actions to read session metadata, set custom session lifetimes, and revoke or extend sessions.
---
...
| --- | --- | --- |
| `persistent` | string | The session is stored in a persistent cookie and survives browser restarts, unless manually cleared. |
| `non-persistent` | string | The session is stored in memory only and cleared when the browser or application is closed. |
