...
Anonymous sessions is part of the [Auth0 Identity Conversion Suite](/docs/customize/identity-conversion-suite). 
...
Upon successful login, Auth0 makes the anonymous session data available in your [`pre-user-registration`](/docs/customize/actions/explore-triggers/pre-user-registration) and [`post-login`](/docs/customize/actions/explore-triggers/post-login) Actions triggers using the `event.anonymous_session` object.
...
If you want more control over anonymous sessions, you can [configure your tenant to not issue cookies for anonymous sessions](/docs/manage-users/sessions/anonymous-sessions/configure-anonymous-sessions#configure-anonymous-sessions) at all.
...
- [Password reset](/docs/customize/actions/explore-triggers#password-reset-triggers) flows are not supported by anonymous sessions.
