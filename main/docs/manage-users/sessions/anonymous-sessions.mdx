...
Anonymous Sessions let your application create and manage guest user sessions without requiring authentication. Users can browse, add items to carts or wishlists, complete purchases, and set preferences before creating an account — and carry that activity into their authenticated profile when they do sign up or log in.
...
```
SPA/App                    Auth0                    Resource Server
   |                          |                            |
   |-- POST /anonymous/token -->|                            |
   |<-- session token, access token, sub:anon|123 ---------|
   |                          |                            |
   |-- POST /anonymous/token {language=EN} -->|             |
   |<-- session token, access token, sub:anon|123 ---------|
   |                          |                            |
   |-- POST /purchase, access token ----------------------->|
   |<-- purchase complete for user anon|123 ---------------|
   |                          |                            |
   |-- POST /anonymous/token {purchase=PUR_987} -->|        |
   |<-- session token, access token, sub:anon|123 ---------|
```
...
- Do not store sensitive information (credit card numbers, Social Security numbers, and so on) in session metadata. To learn more, read [Auth0 General Data Protection Regulation Compliance](/docs/secure/data-privacy-and-compliance/gdpr).
