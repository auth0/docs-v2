...
<Frame>
  <img src="/docs/images/cdy7uua7fh8z/anonymous-sessions/anonymous-session-workflow.png" alt="Sequence diagram showing an anonymous session workflow: the SPA/App sends POST /anonymous/token to Auth0 and receives a session token, access token, and sub:anon|123. Subsequent calls update the session with metadata such as language and country. The app then calls POST /purchase on the Resource Server using the access token, and finally updates the anonymous session with the purchase ID." />
</Frame>
