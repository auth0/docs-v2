...
Core components of the RBAC model:

| Component | Description |
| --- | --- |
| Permissions | The discrete actions allowed (e.g., `read:reports`). |
| Roles | A collection of permissions that can be applied to users (e.g., `Manager`). |
| Users | Individuals that can be assigned one or more roles. |

### Roles in Auth0 Organizations
...
## Implement role-based access control in Auth0
...
You can provide more control by using [Actions](/docs/customize/actions) to restrict access based on a combination of attributes, such as user department, time of day, location of access, or any other user or API attribute (for example, username, security clearance, or API name).

For more info about using Actions with authorization policies, see [Sample Use Cases: Actions with Authorization](/docs/manage-users/access-control/sample-use-cases-actions-with-authorization).
