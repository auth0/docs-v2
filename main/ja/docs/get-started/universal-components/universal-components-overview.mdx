...
<Tabs>
  ...
  <Tab title="Next.js">
    ...
    <Callout icon="file-lines" color="#0EA5E9" iconType="regular">
      インポートしたスタイルシート (`@auth0/universal-components-react/styles`)
      を使用すると、すべてのコンポーネントのスタイルが有効になります。Tailwind v4 を使用している場合は、`@import
                      "@auth0/universal-components-react/tailwind"` を CSS ファイルに追加してください。
    </Callout>
    ...
    詳しくは、[Universal Components のスタイル設定](/ja/docs/get-started/universal-components/universal-components-style)を参照してください。
  </Tab>
