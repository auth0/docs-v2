---
description: Universal Login で柔軟な接続切り替えを実装する方法について説明します
...
柔軟な接続切り替えは、ユーザーがアプリケーションへのログイン時に認証方法を選択できるオプション機能です。実装すると、アプリケーションのログイン画面に、従来のデータベース認証情報または<Tooltip tip="" cta="用語集を表示" href="/ja/docs/glossary?term=passwordless">パスワードレス</Tooltip>接続のいずれかで認証する選択肢が表示されます。[パスワードレス接続](/ja/docs/authenticate/passwordless)を選択したユーザーには、メールアドレスまたは SMS でワンタイムパスワード (OTP) が送信され、そのパスワードを使用してアプリケーションにログインできます。

柔軟な接続切り替えでは、[カスタム Universal Login プロンプト](/ja/docs/customize/login-pages/universal-login/customize-signup-and-login-prompts)を使用して、ユーザーがより主体的に認証方法を選べる体験を提供します。
...
* アプリケーションで以下を有効にします。
  ...
  * [Identifier First 認証](/ja/docs/authenticate/login/auth0-universal-login/identifier-first)
  * [データベース接続](/ja/docs/authenticate/database-connections)
  * [メールアドレスまたは SMS によるパスワードレス認証](/ja/docs/authenticate/login/auth0-universal-login/passwordless-login/email-or-sms)

<div id="implement-flexible-connection-switching">
  ## 柔軟な接続切り替えを実装する
</div>

この機能を実装するには、[Auth0 Management API](https://auth0.com/docs/api/management/v2) を使用して、カスタムのサインアップおよびログイン プロンプト partials を設定します。partials は、ログイン画面などのプロンプト画面内のエントリ ポイントに挿入するカスタム コードです。詳細については、[Customize Signup and Login Prompts](/ja/docs/customize/login-pages/universal-login/customize-signup-and-login-prompts) を参照してください。

柔軟な接続切り替えを実装するには、以下のパラメーターを使用してカスタム プロンプト partials を設定します。

| パラメーター       | 説明                                                                                                                                                                                                                               | 例                                                       |
| ------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------- |
| `state`      | 現在のページの state 値をレンダリングします。これは不透明で、セキュリティ上の目的で使用されます。<br /><br />現在の画面情報の詳細については、[Customize Univeral Login Page Templates](/ja/docs/customize/login-pages/universal-login/customize-templates#current-screen-information) を参照してください。 | `<input type='hidden' name='state' value='{{state}}'>`  |
...
[`プロンプトの partials を設定する`](https://auth0.com/docs/api/management/v2/prompts/put-partials)エンドポイントを使用して、`signup-password`プロンプトを設定できます。
...
最適な結果を得るには、パスワード接続とパスワードレス接続の両方でログイン プロンプトを設定することをお勧めします。

[`login-password` プロンプトは、[プロンプトの partials を設定する](https://auth0.com/docs/api/management/v2/prompts/put-partials)エンドポイントを使用して設定できます。](#)
...
同様に、[プロンプトの partials を設定する](https://auth0.com/docs/api/management/v2/prompts/put-partials) エンドポイントを使用して、`login-passwordless` プロンプトを設定できます。
