...
サードパーティ製アプリケーションがスコープを要求すると、ユーザーには同意プロンプトが表示されます。デフォルトでは、このプロンプトはスコープの**名前**を使用してテキストを生成し、1 つのリソースに属するすべてのスコープをまとめて、そのリソースのアクションを 1 行で表示します。

たとえば保護者の同意のような特別な同意プロンプトが必要な場合は、独自のカスタム同意フォームを作成する必要があります。法律は国によって異なる点に注意してください。

たとえば、次のスコープが定義された Auth0 に登録済みの API があるとします。

* `read:messages`: メールメッセージを読む
* `write:messages`: メッセージを書く

同意プロンプトには **Messages: read and write your messages** と表示されます。

代わりに、定義したスコープの**説明**を使用してこのテキストを生成することもできます。この場合、同意ダイアログには **メールメッセージを読む**、**メッセージを書く** と表示されます。

この変更はテナント レベルで行われるため、そのテナント上のすべての API の同意プロンプトに影響します。

<div id="use-scope-descriptions-to-generate-consent-prompt-text">
  ## スコープの説明を使用して同意プロンプトのテキストを生成する
</div>
...
* [サンプルのユースケース: スコープとクレーム](/ja/docs/get-started/apis/scopes/sample-use-cases-scopes-and-claims)
...
* [メールアドレスに基づくスコープ割り当てに関する Auth0 セキュリティ情報](/ja/docs/secure/security-guidance/security-bulletins/2019-09-05-scopes)