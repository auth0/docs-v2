---
description: Event Streams を使用して、Auth0 のアイデンティティ変更に応じて CRM、請求、またはサードパーティシステムを更新します。
title: 外部システムとアイデンティティの変更を関連付ける
---
...
* Events が有効な Auth0 テナント。プランの提供状況の詳細については、[Event Stream を作成する](/ja/docs/customize/events/create-an-event-stream)を参照してください。
* 必要なイベントタイプをサブスクライブしている有効な Event Stream。詳細については、[Event Stream を作成する](/ja/docs/customize/events/create-an-event-stream)を参照してください。
...
以下のセクションでは、Auth0 のイベントを CRM プラットフォームに関連付ける方法を説明します。ハンドラー関数は、Event Stream の送信先にかかわらず共通です。[イベントをタイプ別にルーティングする](#route-events-by-type) セクションでは、webhook と Auth0 Action の両方の送信先にイベントをディスパッチする方法を示します。
...
<Tabs>
  <Tab title="Webhook">
    ...
    <Callout icon="file-lines" color="#0EA5E9" iconType="regular">
      HTTP `2XX` レスポンスは、できるだけ速やかに返してください。外部 API の呼び出しに時間がかかる場合は、イベントを内部キューに入れて非同期に処理してください。詳しくは、[イベントのベストプラクティス](/ja/docs/customize/events/events-best-practices) を参照してください。
    </Callout>
  </Tab>
...
HubSpot API キーを安全に保存するには、Action エディターで `HUBSPOT_TOKEN` という名前のシークレットを追加します。詳細については、[Action Secrets](/ja/docs/customize/actions/write-your-first-action#add-a-secret)を参照してください。
...
Event Stream のテストについて詳しくは、[Event Testing, Observability, and Failure Recovery](/ja/docs/customize/events/event-testing-observability-and-failure-recovery) を参照してください。
...
* [Event Stream の作成](/ja/docs/customize/events/create-an-event-stream)
