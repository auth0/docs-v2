...
`languageDictionary` は、Classic Login エクスペリエンスでの<Tooltip tip="多要素認証（MFA）: SMS による code など、username と password に加えて別の要素を使用するユーザー認証プロセス。" cta="用語集を表示" href="/ja/docs/glossary?term=MFA">MFA</Tooltip>プロセスのさまざまな箇所のテキストを上書きするために使用できます。設定方法の形式は次のとおりです。
...
...
  ...
  languageDictionary:{ // Language Dictionaryオプションを使用する
    smsEnrollmentConfirm:{  // 変更するカテゴリ/セクションを指定する
      headerText: "Some custom text - In order to confirm enrollment we need to confirm your phone. Please enter the received code." // 変更する属性に新しい値を指定する
    }
  }
...
利用可能なカテゴリとオプションの一覧は、以下を参照してください。
...
errorMessages: {
  ...
  // tenant_not_found | 関連するテナントが見つかりません。テナントを削除しない限り、通常は発生しません。
...
* [MFA Widget のテーマ オプション](/ja/docs/secure/multi-factor-authentication/customize-mfa/mfa-widget-theme-options)
* [Classic Login の MFA をカスタマイズ](/ja/docs/secure/multi-factor-authentication/customize-mfa/customize-mfa-classic-login)