---
description: 外部データベースからユーザーを削除するためのカスタムデータベースのアクション スクリプト テンプレートについて説明します。
title: 削除スクリプトテンプレート
---

Delete スクリプトは、定義された関数を実装し、指定したユーザーを外部データベースから削除します。この関数には `deleteUser` という名前を付けることを推奨します。

このスクリプトは、[レガシー認証シナリオ](/ja/docs/authenticate/database-connections/custom-db/overview-custom-db-connections)でのみ使用されます。Auth0 と外部データベースの両方から同じ操作でユーザーを削除する場合に必要です。
...
| ---------- | -------- | ------------------------------------ |
| `id`       | String   | ユーザーの識別子 `user_id`。`auth0` では始まりません。 |
| `callback` | Function | エラーまたはプロファイルデータをパイプラインに渡すために使用します。   |

### 例

これは、`deleteUser` 関数の実装方法を示す擬似 JavaScript の例です。言語固有の例については、[言語別のスクリプト例](/ja/docs/authenticate/database-connections/custom-db/templates/delete#language-specific-script-examples) を参照してください。
