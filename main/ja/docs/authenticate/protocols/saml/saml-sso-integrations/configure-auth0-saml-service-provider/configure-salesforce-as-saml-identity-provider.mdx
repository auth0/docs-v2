...
SSO のトラブルシューティングでは、やり取りの HTTP トレースを取得して HAR ファイルとして保存すると役立つことがよくあります。詳細については、[Generate and Analyze HAR Files](/ja/docs/troubleshoot/troubleshooting-tools/generate-and-analyze-har-files) を参照してください。
...
Salesforce のユーザープロファイルに、Salesforce IDプロバイダー経由でログインする権限があることを確認してください (上記のセクション 4 を参照) 。