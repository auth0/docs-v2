...
export interface MfaPhoneEnrollmentMembers extends BaseMembers {
   ...
   * @param payload 電話番号、タイプ、および追加のカスタムオプション。
   */
...
<ParamField body="continueEnrollment" type="Promise<void>">
  指定した電話番号と種別 (SMS または音声) を使用して、登録プロセスを続行します。

  <Expandable title="パラメータ">
    <ParamField body="payload" type={<span><a href="/ja/docs/libraries/acul/js-sdk/Screens/interfaces/MfaPhoneEnrollmentContinueOptions">MfaPhoneEnrollmentContinueOptions</a></span>}>
      電話番号、種別、および任意の追加のカスタムオプションです。
