...
Redeem Ticket 画面で使用できるデータを表すインターフェース。
...
export interface RedeemTicketMembers extends BaseMembers {
  ...
  /**
   * redeem-ticket 画面でデフォルトのアクションを実行します。通常はフローの続行を伴います。
   * @param {CustomOptions} [payload] - リクエストに含める追加のカスタムオプション（省略可能）。
...
<ParamField body="continue" type="Promise<void>">
  ...
  アクションが完了すると resolve される Promise です。
