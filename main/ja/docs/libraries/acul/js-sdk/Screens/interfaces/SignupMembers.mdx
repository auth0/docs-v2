...
export interface SignupMembers extends BaseMembers {
  ...
  /**
 * 現在のトランザクション状態に基づいて、有効な識別子（例：メールアドレス、電話番号、username）の
 * リストを返します。
 *
 * 識別子は、接続戦略および認証またはサインアップフロー中に提供された
...
<ParamField body="getSignupIdentifiers" type={<span><a href="/docs/libraries/acul/js-sdk/Screens/interfaces/Identifier">Identifier</a>[]</span>}>
  **`ユーティリティ機能`**
  ...
  現在のトランザクションの状態に基づいて、
  有効な識別子 (例: メールアドレス、電話番号、username) の一覧を返します。

  識別子は、接続戦略と
  認証またはサインアップフローで指定された設定に応じて、必須または任意になります。
