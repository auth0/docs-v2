...
export interface MfaWebAuthnRoamingChallengeMembers extends BaseMembers {
  ...
  /**
   * `screen.data.publicKeyChallengeOptions` に指定されたオプションを使用して `navigator.credentials.get()` を呼び出し、
   * WebAuthn チャレンジを開始します。
   * 成功した場合、`action: "default"` とともに結果のクレデンシャルを Auth0 に送信します。
   ...
   *
   * @param {VerifySecurityKeyOptions} [options] - 検証プロセスのオプションパラメーター（`rememberDevice` などのカスタムオプションを含む）。
   * @returns {Promise<void>} 検証の試行が送信されたときに解決される Promise。
   *                          成功した操作は通常リダイレクトで終わるため、データを直接返しません。
   ...
   *   // 成功した場合、Auth0 がリダイレクトを処理します。
   * } catch (error) {
   *   console.error("Security key verification failed:", error);
   *   // ページがリロードされた場合は sdk.transaction.errors でサーバー側の検証メッセージを確認してください。
   * }
   * ```
   */
  ...
  /**
   * Reports a specific WebAuthn API エラー to Auth0.
...
<ParamField body="reportWebAuthnError" type="Promise<void>">
  ...
  `verify()` で自動的に処理されない形で失敗した場合や、別のアクションを試す前に
  ...
  フォームの送信に失敗した場合、エラーをスローします。

  ```typescript Example
  // 'sdk' が MfaWebAuthnRoamingChallenge のインスタンスであるとします
  ...
  <Expandable title="パラメーター">
    <ParamField body="options" type={<span><a href="/ja/docs/libraries/acul/js-sdk/Screens/interfaces/MfaWebAuthnRoamingChallengeReportErrorOptions">MfaWebAuthnRoamingChallengeReportErrorOptions</a></span>}>
      WebAuthn API の `name` と `message` を含む
      `error` オブジェクトと、その他のカスタムオプションが含まれます。
      ...
      **プロパティ**
      ...
      <ParamField body="error" type={<span><a href="/ja/docs/libraries/acul/js-sdk/Screens/interfaces/WebAuthnErrorDetails">WebAuthnErrorDetails</a></span>}>
        報告対象の WebAuthn API エラーオブジェクトです。
      </ParamField>
    </ParamField>
  </Expandable>
</ParamField>
...
<ParamField body="tryAnotherMethod" type="Promise<void>">
  ユーザーが WebAuthn チャレンジを中止し、別の MFA 方法を選択できるようにします。
  このアクションは `action: "pick-authenticator"` を Auth0 に送信し、
  ユーザーを MFA 要素の選択画面に移動させます。
  ...
  <Expandable title="パラメーター">
    <ParamField body="options?" type={<span><a href="/ja/docs/libraries/acul/js-sdk/Screens/interfaces/MfaWebAuthnRoamingChallengeTryAnotherMethodOptions">MfaWebAuthnRoamingChallengeTryAnotherMethodOptions</a></span>}>
...
<ParamField body="verify" type="Promise<void>">
  ...
  <Expandable title="パラメーター">
    <ParamField body="options?" type={<span><a href="/ja/docs/libraries/acul/js-sdk/Screens/interfaces/VerifySecurityKeyOptions">VerifySecurityKeyOptions</a></span>}>
      検証プロセスに指定する任意のパラメーターです。
      たとえば `rememberDevice` やその他のカスタムオプションがあります。

      **プロパティ**

      <ParamField body="rememberDevice?" type="boolean">
        任意です。`true` の場合、今後の MFA チャレンジに備えてブラウザーを記憶しようとします。
