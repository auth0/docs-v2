...
mfa-push-enrollment-qr 画面で利用可能なメソッドとプロパティを定義するインターフェース
  ...
  <Expandable title="パラメータ">
    <ParamField body="options" type={<span><a href="/ja/docs/libraries/acul/js-sdk/Screens/type-aliases/MfaPollingOptions">MfaPollingOptions</a></span>}>
      <ParamField body="MfaPollingOptions" type="object">
        ...
        ポーリングセッションの開始時にこれらのオプションを指定すると、チャレンジエンドポイントへの
        問い合わせ頻度や、成功時・失敗時の処理方法を制御できます。

        <Expandable title="プロパティ">
          <ParamField body="intervalMs?" type="number">
            連続するポーリングリクエストの間隔を**ミリ秒**単位で指定する省略可能な値です。

            * 省略した場合は、SDK の内部既定値 (通常は 5000 ms) が使用されます。
            * 応答性とレート制限への配慮のバランスを考慮して値を選択してください。
          </ParamField>

          <ParamField body="onCompleted()?" type="() => void">
            MFA プッシュのチャレンジが正常に承認され、
            ポーリングが完了した時点で実行される省略可能なコールバックです。

            * **1 回だけ**呼び出され、その後ポーリングは自動的に停止します。
            * ログインフローを先に進める場合 (例: `continue()` の呼び出し) に便利です。
          </ParamField>

          <ParamField body="onError()?" type="(error) => void">
            ポーリング中にエラーが発生した場合に呼び出される省略可能なコールバックです。

            次の内容を含む [ULError](/ja/docs/libraries/acul/js-sdk/Screens/interfaces/Error) オブジェクトを受け取ります。
            ...
            * エラーメッセージの表示や、カスタムの再試行 / 中止ロジックのトリガーに使用します。
