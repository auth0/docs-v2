...
<ParamField body="client" type={<span><a href="/ja/docs/libraries/acul/js-sdk/Screens/interfaces/ClientMembers">ClientMembers</a></span>}>
  `mfa-webauthn-platform-challenge` 画面に対して、`id`、`name`、`logoUrl` などのクライアント関連の構成を提供します。
</ParamField>
...
<ParamField body="user" type={<span><a href="/ja/docs/libraries/acul/js-sdk/Screens/interfaces/UserMembers">UserMembers</a></span>}>
  `username`、`email`、`roles` などを含む、アクティブなユーザーの詳細です。
</ParamField>
...
<ParamField body="reportBrowserError" type="Promise<void>">
  ...
  **メソッドパラメーター**
  ...
  <Expandable title="パラメーター">
    <ParamField body="options">
      [MfaWebAuthnPlatformChallengeReportErrorOptions](/ja/docs/libraries/acul/js-sdk/Screens/interfaces/MfaWebAuthnPlatformChallengeReportErrorOptions)。
    </ParamField>
  ...
  **メソッドパラメーター**
  ...
  <Expandable title="パラメーター">
    ...
    <ParamField body="rememberDevice?" type="boolean">
      `true` の場合、今後のMFAチャレンジでこのブラウザーを記憶します。これは `screen.showRememberDevice` が `true` の場合にのみ適用されます。
    </ParamField>
  </Expandable>
</ParamField>