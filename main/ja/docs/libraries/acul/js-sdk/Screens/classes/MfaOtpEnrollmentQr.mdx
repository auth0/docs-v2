...
description: "Universal Login の `mfa-otp-enrollment-qr` 画面をカスタマイズするために利用できる、すべてのプロパティとメソッドについて説明します。"
---

MfaOtpEnrollmentQr クラスは、`mfa-otp-enrollment-qr` 画面の機能を実装します。この画面には、MFA の登録時にユーザーが認証アプリでスキャンするための QR コードが表示されます。

<Frame>
  <img style={{maxHeight:"400px"}} src="/docs/images/cdy7uua7fh8z/xgACud9b5L3UQ6OKH5rf8/8c3cce8d90c0b9d2162e2441df85bb01/Screenshot_2025-03-26_at_00.00.27.png" alt="ACUL MFA OTP 登録 QR" />
</Frame>
...
MFA OTP 登録用 QR 画面マネージャーのインスタンスを作成します。
...
<ParamField body="client" type={<span><a href="/ja/docs/libraries/acul/js-sdk/Screens/interfaces/ClientMembers">ClientMembers</a></span>}>
  `mfa-otp-enrollment-qr` 画面の `id`、`name`、`logoUrl` など、クライアント関連の設定を提供します。
</ParamField>
...
<ParamField body="transaction" type={<span><a href="/ja/docs/libraries/acul/js-sdk/Screens/interfaces/TransactionMembers">TransactionMembers</a></span>}>
  アクティブな識別子やフローの状態など、`mfa-otp-enrollment-qr` 画面に関するトランザクション固有のデータを提供します。
</ParamField>
...
<ParamField body="untrustedData" type={<span><a href="/ja/docs/libraries/acul/js-sdk/Screens/interfaces/UntrustedDataMembers">UntrustedDataMembers</a></span>}>
  MFA OTP の登録時におけるユーザー入力など、SDK に渡される信頼できないデータを扱います。
</ParamField>
...
<ParamField body="continue" type="Promise<void>">
  このメソッドは、登録プロセスを続行するために OTP コードを送信します。
  ...
  **メソッド パラメーター**
  ...
  <Expandable title="パラメーター">
    <ParamField body="options">
      [MfaOtpEnrollmentQrContinueOptions](/ja/docs/libraries/acul/js-sdk/Screens/interfaces/MfaOtpEnrollmentQrContinueOptions)。
    </ParamField>
