...
BruteForceProtectionUnblock クラスは、`brute-force-protection-unblock` 画面の機能を実装します。この画面は、ログイン試行の失敗が複数回続いたためにユーザーのアカウントがブロックされ、ブロック解除リクエストを送信する必要がある場合に表示されます。

<Frame>
  <img style={{maxHeight:"400px"}} src="https://mintlify.s3.us-west-1.amazonaws.com/auth0/docs/images/cdy7uua7fh8z/ACUL/brute-force-protection-unblock.png" alt="ACUL ブルートフォース保護のブロック解除" />
</Frame>
...
BruteForceProtectionUnblock 画面マネージャーのインスタンスを作成します。
...
<ParamField body="client" type={<span><a href="/ja/docs/libraries/acul/js-sdk/Screens/interfaces/ClientMembers">ClientMembers</a></span>}>
  `brute-force-protection-unblock` 画面の `id`、`name`、`logoUrl` など、クライアントに関する構成を提供します。
</ParamField>
...
<ParamField body="screen" type={<span><a href="/ja/docs/libraries/acul/js-sdk/Screens/interfaces/ScreenMembers">ScreenMembers</a></span>}>
  設定やコンテキストなど、`brute-force-protection-unblock` 画面に固有の詳細が含まれます。
</ParamField>
...
<ParamField body="transaction" type={<span><a href="/ja/docs/libraries/acul/js-sdk/Screens/interfaces/TransactionMembers">TransactionMembers</a></span>}>
  アクティブな識別子やフローの状態など、`brute-force-protection-unblock` 画面に関するトランザクション固有のデータを提供します。
</ParamField>
