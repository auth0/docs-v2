...
BruteForceProtectionUnblockSuccess クラスは、`brute-force-protection-unblock-success` 画面の機能を実装します。この画面は、ブルートフォース保護によるアカウントのブロック解除リクエストが正常に送信された後に表示されます。

<Frame>
  <img style={{maxHeight:"400px"}} src="https://mintlify.s3.us-west-1.amazonaws.com/auth0/docs/images/cdy7uua7fh8z/ACUL/brute-force-protection-unblock-success.png" alt="ACUL ブルートフォース保護のブロック解除成功" />
</Frame>
...
BruteForceProtectionUnblockSuccess 画面マネージャーのインスタンスを生成します。
...
<ParamField body="client" type={<span><a href="/ja/docs/libraries/acul/js-sdk/Screens/interfaces/ClientMembers">ClientMembers</a></span>}>
  `brute-force-protection-unblock-success` 画面に対して、`id`、`name`、`logoUrl` などのクライアント関連の構成を提供します。
</ParamField>
...
<ParamField body="screen" type={<span><a href="/ja/docs/libraries/acul/js-sdk/Screens/interfaces/ScreenMembersOnBruteForceProtectionUnblockSuccess">ScreenMembersOnBruteForceProtectionUnblockSuccess</a></span>}>
  設定やコンテキストなど、`brute-force-protection-unblock-success` 画面に固有の詳細を含みます。
</ParamField>
...
<ParamField body="transaction" type={<span><a href="/ja/docs/libraries/acul/js-sdk/Screens/interfaces/TransactionMembers">TransactionMembers</a></span>}>
  アクティブな識別子やフローの状態など、`brute-force-protection-unblock-success` 画面に固有のトランザクションデータを提供します。
</ParamField>
