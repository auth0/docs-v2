...
description: "Universal Login の `mfa-webauthn-roaming-challenge` 画面をカスタマイズするために使用できる、すべてのプロパティとメソッドについて説明します。"
---
...
<ParamField body="client" type={<span><a href="/ja/docs/libraries/acul/js-sdk/Screens/interfaces/ClientMembers">ClientMembers</a></span>}>
  `mfa-webauthn-roaming-challenge` 画面用のクライアント関連の構成 (`id`、`name`、`logoUrl` など) を提供します。
</ParamField>
  ...
  **メソッドパラメーター**
  ...
  <Expandable title="パラメーター">
    ...
    <ParamField body="rememberDevice?" type="boolean">
      `true` の場合、今後の MFAチャレンジのためにこのブラウザーを記憶します。これは `screen.showRememberDevice` が `true` の場合にのみ適用されます。
    </ParamField>
  </Expandable>
</ParamField>