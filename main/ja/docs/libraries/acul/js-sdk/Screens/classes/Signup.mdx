---
title: "サインアップ"
description: "Universal Login の `signup` 画面をカスタマイズするために利用できる、すべてのプロパティとメソッドについて説明します。"
---

Signup クラスは、`signup` 画面の機能を提供します。この画面では、ユーザーの識別子とパスワードを入力します。テナントによっては、この識別子はメールアドレス、電話番号、またはユーザー名になります。
...
<ParamField body="client" type={<span><a href="/ja/docs/libraries/acul/js-sdk/Screens/interfaces/ClientMembers">ClientMembers</a></span>}>
  `signup` 画面で使用する、`id`、`name`、`logoUrl` などのクライアント関連の設定を提供します。
</ParamField>
...
<ParamField body="screen" type={<span><a href="/ja/docs/libraries/acul/js-sdk/Screens/interfaces/ScreenMembersOnSignup">ScreenMembersOnSignup</a></span>}>
  設定やコンテキストなど、`signup` 画面固有の詳細を含みます。
</ParamField>
...
<ParamField body="federatedSignup" type="Promise<void>">
  このメソッドは、ソーシャルサインアップフォームの送信を処理します。[signup](#signup) メソッドと似ていますが、ソーシャルサインアップ用です。
...
<ParamField body="getErrors" type={<span><a href="/ja/docs/libraries/acul/js-sdk/Screens/interfaces/Error">Error</a>[]</span>}>
  このメソッドは、コンテキストからトランザクションエラーの配列を取得します。エラーがない場合は空の配列を返します。返されるのは、トランザクションコンテキスト内のエラーオブジェクトの配列です。
</ParamField>
...
<ParamField body="getSignupIdentifiers" type={<span><a href="/ja/docs/libraries/acul/js-sdk/Screens/interfaces/Identifier">Identifier</a>[] | null</span>}>
  このメソッドは、サインアップフォームで有効な識別子の一覧を返し、トランザクション設定に基づいて各項目が必須か任意かを示します。識別子オブジェクトの配列です (例: email、phone、username) 。
...
<ParamField body="pickCountryCode" type="Promise<void>">
  このメソッドは、ユーザーを国コードの選択リストにリダイレクトし、そこで電話番号の国コードプレフィックスを更新できるようにします。
...
<ParamField body="validateUsername" type={<span><a href="/ja/docs/libraries/acul/js-sdk/Screens/interfaces/UsernameValidationResult">UsernameValidationResult</a></span>}>
  このメソッドは、指定されたユーザー名を、トランザクションコンテキストで定義されている現在のユーザー名ポリシーに照らして検証します。返されるオブジェクトには、ユーザー名が有効かどうかと、その理由が示されます。
