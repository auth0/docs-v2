...
description: "Universal Login の `device-code-activation-allowed` 画面をカスタマイズするために使用できる、すべてのプロパティとメソッドについて説明します。"
---

DeviceCodeActivationAllowed クラスは、`device-code-activation-allowed` 画面の機能を実装します。この画面は、デバイスコード有効化が正常に承認されたときに表示されます。
...
<ParamField body="client" type={<span><a href="/ja/docs/libraries/acul/js-sdk/Screens/interfaces/ClientMembers">ClientMembers</a></span>}>
  `device-code-activation-allowed` 画面の `id`、`name`、`logoUrl` などのクライアント関連の構成を提供します。
</ParamField>
...
<ParamField body="screen" type={<span><a href="/ja/docs/libraries/acul/js-sdk/Screens/interfaces/ScreenMembers">ScreenMembers</a></span>}>
  構成やコンテキストなど、`device-code-activation-allowed` 画面に固有の詳細が含まれます。
</ParamField>
...
<ParamField body="transaction" type={<span><a href="/ja/docs/libraries/acul/js-sdk/Screens/interfaces/TransactionMembers">TransactionMembers</a></span>}>
  アクティブな識別子やフローの状態など、`device-code-activation-allowed` 画面のトランザクション固有のデータを提供します。
</ParamField>
