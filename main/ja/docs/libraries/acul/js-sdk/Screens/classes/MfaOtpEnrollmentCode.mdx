...
description: "Universal Login の `mfa-otp-enrollment-code` 画面をカスタマイズするために利用できる、すべてのプロパティとメソッドについて説明します。"
---

MfaOtpEnrollmentCode クラスは、`mfa-otp-enrollment-code` 画面の機能を実装します。この画面は、ユーザーが MFA 登録時に受け取った OTP コードを入力する必要がある場合に表示されます。

<Frame>
  <img style={{maxHeight:"400px"}} src="/docs/images/cdy7uua7fh8z/5x7zKvx9RtgMzJqkIgvE5Z/17d50fc705c6e0bf0bd75444d2b486ff/Screenshot_2025-03-26_at_00.01.21.png" alt="ACUL MFA OTP 登録コード" />
</Frame>
...
<ParamField body="client" type={<span><a href="/ja/docs/libraries/acul/js-sdk/Screens/interfaces/ClientMembers">ClientMembers</a></span>}>
  `mfa-otp-enrollment-code` 画面の `id`、`name`、`logoUrl` など、クライアント関連の構成を提供します。
</ParamField>
...
<ParamField body="screen" type={<span><a href="/ja/docs/libraries/acul/js-sdk/Screens/interfaces/ScreenMembersOnMfaOtpEnrollmentCode">ScreenMembersOnMfaOtpEnrollmentCode</a></span>}>
  設定やコンテキストなど、`mfa-otp-enrollment-code` 画面に固有の詳細を含みます。
</ParamField>
...
<ParamField body="transaction" type={<span><a href="/ja/docs/libraries/acul/js-sdk/Screens/interfaces/TransactionMembers">TransactionMembers</a></span>}>
  アクティブな識別子やフローの state など、`mfa-otp-enrollment-code` 画面のトランザクション固有のデータを提供します。
</ParamField>
...
<ParamField body="untrustedData" type={<span><a href="/ja/docs/libraries/acul/js-sdk/Screens/interfaces/UntrustedDataMembers">UntrustedDataMembers</a></span>}>
  MFA OTP の登録時のユーザー入力など、SDK に渡される信頼できないデータを処理します。
</ParamField>
...
<ParamField body="continue" type="Promise<void>">
  ...
  **メソッドパラメーター**
  ...
  <Expandable title="パラメーター">
    <ParamField body="options">
      [MfaOtpContinueOptions](/ja/docs/libraries/acul/js-sdk/Screens/interfaces/MfaOtpContinueOptions)。
    </ParamField>
