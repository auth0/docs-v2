...
description: "Universal Login の `brute-force-protection-unblock-failure` 画面をカスタマイズするために利用できる、すべてのフックとメソッドについて説明します。"
---

`brute-force-protection-unblock-failure` 画面は、ユーザーアカウントのブロック解除の試行が失敗した場合に表示され、ブロック解除リクエストを完了できなかったことをユーザーに通知します。
...
`brute-force-protection-unblock-failure` 画面で Auth0 のコンテキストデータへの読み取り専用アクセスを提供する、画面スコープのフックです。`@auth0/auth0-acul-react/brute-force-protection-unblock-failure` からインポートします。
...
<ParamField body="useClient" type={<span>() =&gt; <a href="/ja/docs/libraries/acul/react-sdk/API-Reference/Types/interfaces/ClientMembers">ClientMembers</a></span>}>
  このフックは、`brute-force-protection-unblock-failure` 画面の `id`、`name`、`logoUrl` などのクライアント関連設定を提供します。
...
<ParamField body="useScreen" type={<span>() =&gt; <a href="/ja/docs/libraries/acul/react-sdk/API-Reference/Types/interfaces/ScreenMembersOnBruteForceProtectionUnblockFailure">ScreenMembersOnBruteForceProtectionUnblockFailure</a></span>}>
  このフックには、設定やコンテキストなど、`brute-force-protection-unblock-failure` 画面に固有の詳細が含まれます。
...
<ParamField body="useTransaction" type={<span>() =&gt; <a href="/ja/docs/libraries/acul/react-sdk/API-Reference/Types/interfaces/TransactionMembers">TransactionMembers</a></span>}>
  このフックは、アクティブな接続や現在のフロー状態など、`brute-force-protection-unblock-failure` 画面のトランザクション固有のデータを提供します。
...
<ParamField body="useBruteForceProtectionUnblockFailure" type={<a href="/ja/docs/libraries/acul/react-sdk/API-Reference/Types/interfaces/BruteForceProtectionUnblockFailureMembers">BruteForceProtectionUnblockFailureMembers</a>}>
