...
`email-verification-result` 画面は、ユーザーがメールアドレスの確認プロセスを完了した後に表示されます。確認が成功したか失敗したかが示されます。

<Frame>
  <img style={{maxHeight:"400px"}} src="/docs/images/cdy7uua7fh8z/fNvOxLFlyoL8mWRazjxJe/b0651fc03d10a5e0825a55cc7d9142a9/Screenshot_2025-05-15_at_21.04.13.png" alt="ACUL メールアドレス確認結果" />
</Frame>
...
各画面には、それぞれ固有のフックとメソッドが用意されています。SDK では、各画面について **部分インポート** と **ルートインポート** をサポートしています。

* 部分インポートを使用すると、特定のユースケースに必要なコードだけを取り込めます。
* ルートインポートを使用すると、単一のバンドルからすべての画面を読み込めます。これは、考えられるすべての画面に対応する統一ビルドを使用したい場合に便利です。
...
`email-verification-result` 画面で Auth0 のコンテキストデータへの読み取り専用アクセスを提供する、画面スコープのフックです。`@auth0/auth0-acul-react/email-verification-result` からインポートします。
...
<ParamField body="useClient" type={<span>() =&gt; <a href="/docs/libraries/acul/react-sdk/API-Reference/Types/interfaces/ClientMembers">ClientMembers</a></span>}>
  このフックは、`email-verification-result` 画面の `id`、`name`、`logoUrl` など、クライアント関連の設定を提供します。
...
<ParamField body="useOrganization" type={<span>() =&gt; <a href="/docs/libraries/acul/react-sdk/API-Reference/Types/interfaces/OrganizationMembers">OrganizationMembers</a></span>}>
  このフックは、メールアドレスの確認が組織スコープの場合に、ユーザーの組織に関する情報を提供します。組織コンテキストが存在しない場合は `null` を返します。
...
<ParamField body="useTransaction" type={<span>() =&gt; <a href="/docs/libraries/acul/react-sdk/API-Reference/Types/interfaces/TransactionMembers">TransactionMembers</a></span>}>
  このフックは、アクティブな接続や現在のフローの state など、`email-verification-result` 画面のトランザクション固有のデータを提供します。
...
<ParamField body="useEmailVerificationResult" type={<a href="/docs/libraries/acul/react-sdk/API-Reference/Types/interfaces/EmailVerificationResultMembers">EmailVerificationResultMembers</a>}>
  このフックは、`email-verification-result` 画面で利用可能なすべてのメソッドとコンテキストを返します。
</ParamField>
