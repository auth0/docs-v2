...
description: "Universal Login の `mfa-push-enrollment-qr` 画面をカスタマイズするために使用できる、すべての Hooks とメソッドについて説明します。"
---
...
各画面には、それぞれ固有の Hooks とメソッドのセットがあります。SDK は、各画面で**部分インポート**と**ルートインポート**をサポートしています。
...
`mfa-push-enrollment-qr` 画面で Auth0 のコンテキストデータへの読み取り専用アクセスを提供する、画面スコープのHooksです。`@auth0/auth0-acul-react/mfa-push-enrollment-qr` からインポートします。
...
<ParamField body="useBranding" type={<span>() =&gt; <a href="/docs/libraries/acul/react-sdk/API-Reference/Types/interfaces/BrandingMembers">BrandingMembers</a></span>}>
  このフックは、`mfa-push-enrollment-qr` 画面に表示されるロゴ、色、テーマ設定などのブランディング設定を提供します。
...
<ParamField body="useClient" type={<span>() =&gt; <a href="/docs/libraries/acul/react-sdk/API-Reference/Types/interfaces/ClientMembers">ClientMembers</a></span>}>
  このフックは、`mfa-push-enrollment-qr` 画面の `id`、`name`、`logoUrl` などのクライアント関連の設定を提供します。
...
<ParamField body="useScreen" type={<span>() =&gt; <a href="/docs/libraries/acul/react-sdk/API-Reference/Types/interfaces/ScreenMembersOnMfaPushEnrollmentQr">ScreenMembersOnMfaPushEnrollmentQr</a></span>}>
  このフックには、設定やコンテキストなど、`mfa-push-enrollment-qr` 画面固有の情報が含まれます。
...
<ParamField body="useTransaction" type={<span>() =&gt; <a href="/docs/libraries/acul/react-sdk/API-Reference/Types/interfaces/TransactionMembers">TransactionMembers</a></span>}>
  このフックは、現在の MFA フローの状態など、`mfa-push-enrollment-qr` 画面のトランザクション固有のデータを提供します。
...
<ParamField body="useMfaPushEnrollmentQr" type={<a href="/docs/libraries/acul/react-sdk/API-Reference/Types/interfaces/MfaPushEnrollmentQrMembers">MfaPushEnrollmentQrMembers</a>}>
  このフックは、`mfa-push-enrollment-qr` 画面で使用できるすべてのメソッドとコンテキストを返します。
</ParamField>
...
<ParamField body="continueMethod" type="Promise<void>">
  このメソッドは、ユーザーが Auth0 Guardian アプリで QR コードをスキャンした後に、プッシュの登録を完了します。
  ...
  **メソッドパラメーター**
  ...
  <Expandable title="パラメーター">
    ...
    <ParamField body="rememberDevice?" type="boolean">
      `true` の場合、このデバイスが記憶され、このデバイスでは MFA が再度要求されなくなります。
    </ParamField>
  </Expandable>
</ParamField>

<ParamField body="pickAuthenticator" type="Promise<void>">
  このメソッドは認証方法の選択画面に移動し、ユーザーが登録する別の MFA 方法を選択できるようにします。
...
<ParamField body={<a href="/docs/libraries/acul/react-sdk/API-Reference/Hooks/useMfaPolling">useMfaPolling</a>} type="Hooks">
  このフックは、非同期のプッシュ通知応答を待機する画面のポーリング状態を管理します。
</ParamField>
