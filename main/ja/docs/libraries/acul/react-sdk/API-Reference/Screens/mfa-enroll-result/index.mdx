...
description: "Universal Login の `mfa-enroll-result` 画面をカスタマイズするために使用できる、すべてのフックとメソッドについて説明します。"
---

`mfa-enroll-result` 画面には、MFA の登録試行の結果が表示され、認証要素が正常に登録されたかどうかを確認できます。
...
* ルートインポートを使用すると、単一のバンドルからすべての画面を読み込めます。これは、想定されるすべての画面に対応するために、統一されたビルドを使用したい場合に便利です。
...
<ParamField body="useClient" type={<span>() =&gt; <a href="/ja/docs/libraries/acul/react-sdk/API-Reference/Types/interfaces/ClientMembers">ClientMembers</a></span>}>
  このフックは、`mfa-enroll-result` 画面の `id`、`name`、`logoUrl` など、クライアントに関する設定を提供します。
...
<ParamField body="useScreen" type={<span>() =&gt; <a href="/ja/docs/libraries/acul/react-sdk/API-Reference/Types/interfaces/ScreenMembersOnMfaEnrollResult">ScreenMembersOnMfaEnrollResult</a></span>}>
  このフックには、設定やコンテキストなど、`mfa-enroll-result` 画面に固有の詳細が含まれます。
...
<ParamField body="useTransaction" type={<span>() =&gt; <a href="/ja/docs/libraries/acul/react-sdk/API-Reference/Types/interfaces/TransactionMembers">TransactionMembers</a></span>}>
  このフックは、現在の MFA フローの state など、`mfa-enroll-result` 画面のトランザクション固有のデータを提供します。
...
<ParamField body="useMfaEnrollResult" type={<a href="/ja/docs/libraries/acul/react-sdk/API-Reference/Types/interfaces/MfaEnrollResultMembers">MfaEnrollResultMembers</a>}>
  このフックは、`mfa-enroll-result` 画面で利用できるすべてのメソッドとコンテキストを返します。
</ParamField>
