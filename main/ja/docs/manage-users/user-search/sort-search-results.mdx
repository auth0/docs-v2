---
description: sort パラメーターに field:order 値を指定して、検索結果を並べ替える方法について説明します。
title: 検索結果の並べ替え
---
...
ユーザー検索結果を並べ替えるには、リクエストを送信する際に `sort` パラメーターに `field:order` の値を渡します。`field` は並べ替えの基準となるフィールドの名前で、order には昇順の場合は `1`、降順の場合は `-1` を設定できます。

`app_metadata` または `user_metadata` での並べ替えはサポートされていません。

たとえば、`created_at` フィールドでユーザーを昇順に並べ替えるには、`sort` パラメーターに `created_at:1` を渡します。
...
`sort` やその他のパラメーターについて詳しくは、[Management API Explorer](https://auth0.com/docs/api/management/v2#!/Users/get_users) のドキュメントを参照してください。