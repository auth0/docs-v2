---
description: get-users-by-email エンドポイントを使用してユーザーの一覧を取得する方法を学びます。
title: Get Users by Email エンドポイントを使用してユーザーを取得する
---
...
[`GET /api/v2/users-by-email` エンドポイント](https://auth0.com/docs/api/management/v2#!/Users_By_Email/get_users_by_email)を使用すると、メールアドレスでユーザーを検索できます。検索では、指定したメールアドレスに完全一致するものが対象となり、大文字と小文字は区別されます。

このエンドポイントは**即時整合性**があるため、次の用途での使用を推奨します。
