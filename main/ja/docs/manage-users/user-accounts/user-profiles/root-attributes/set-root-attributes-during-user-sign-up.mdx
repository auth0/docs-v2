---
description: Auth0 Management API を使用して、ユーザーのサインアップ時にルート属性を設定する方法について説明します。
...
サインアップ時に、Auth0 の <Tooltip tip="Management API: 顧客が管理タスクを実行できるようにする製品です。" cta="用語集を見る" href="/ja/docs/glossary?term=Management+API">Management API</Tooltip> を使用して、ユーザーのルート属性を設定できます。これにより、ユーザー作成時にルート属性を設定するために必要な API 呼び出しの回数を最小限に抑えられます。

[Create a User endpoint](https://auth0.com/docs/api/management/v2#!/Users/post_users) に対して `POST` リクエストを送信します。`MGMT_API_ACCESS_TOKEN`、`CONNECTION_NAME`、`EMAIL_VALUE`、`PASSWORD_VALUE`、`GIVEN_NAME_VALUE`、`FAMILY_NAME_VALUE`、`NAME_VALUE`、`NICKNAME_VALUE`、`PICTURE` の各プレースホルダー値は、それぞれ Management API の <Tooltip tip="Access Token: API へのアクセスに使用される認可資格情報で、不透明な文字列または JWT の形式です。" cta="用語集を見る" href="/ja/docs/glossary?term=Access+Token">アクセストークン</Tooltip>、接続名、メールアドレス、パスワード、名、姓、名前、ニックネーム、画像 URL に必ず置き換えてください。
...
| 値                       | 説明                                                                                                                               |
| ----------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| `MGMT_API_ACCESS_TOKEN` | `create:users` [スコープ](/ja/docs/glossary?term=scope)を持つ [Management API のアクセストークン](https://auth0.com/docs/api/management/v2/tokens)。 |
...
| `PICTURE_VALUE`         | 作成するユーザーの画像 URL。                                                                                                                 |

<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  ユーザー登録に Lock または [公開サインアップエンドポイント](https://auth0.com/docs/api/authentication#signup) を使用している場合も、同じ方法でルート属性を設定できます。
</Callout>
