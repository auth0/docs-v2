---
title: "バックチャネルログイン"
description: "Back-Channel Login エンドポイントを使用すると、アプリケーションは、ユーザーがアプリをインストール済みで、[Guardian SDK を使用したプッシュ通知](https://auth0.com/docs/secure/multi-factor-authentication/auth0-guardian#enroll-in-push-notifications)に登録している場合、ユーザーの電話番号または認証デバイスに認証リクエストを送信できます..."
---

`POST /bc-authorize`

Back-Channel Login エンドポイントを使用すると、アプリケーションは、ユーザーがアプリをインストール済みで、[Guardian SDK を使用したプッシュ通知](https://auth0.com/docs/secure/multi-factor-authentication/auth0-guardian#enroll-in-push-notifications)に登録している場合、ユーザーの電話番号または認証デバイスに認証リクエストを送信できます。

Back-Channel Login エンドポイントは、次のようなユースケースでユーザーを認証するために使用します。
