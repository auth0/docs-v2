...
Auth0 では現在、[GET /api/v2/device-credentials エンドポイント](https://auth0.com/docs/api/management/v2/#!/Device_Credentials/get_device_credentials) を使用する際に `user_id` の指定が必須です。リクエストに `user_id` が含まれていない場合は、400 ステータスコードが返されます。この非推奨化の影響を受けるかどうかを確認するには、テナントログの `depnote` を確認してください。詳細については、[非推奨エラーを確認する](/ja/docs/troubleshoot/basic-issues/check-deprecation-errors) を参照してください。
