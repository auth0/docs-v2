---
mode: wide
sidebarTitle: .NET para Android e iOS
...
<Accordion title="Usa IA para integrar Auth0" icon="microchip-ai" iconType="solid" defaultOpen>
  ...
  **Luego, pídele a tu asistente de IA:**
  ...
  Tu asistente de IA creará automáticamente tu aplicación en Auth0, obtendrá las credenciales, instalará el SDK Auth0 OidcClient, configurará opciones específicas de cada plataforma e implementará los flujos de inicio y cierre de sesión. [Documentación completa de agent skills →](/es/docs/quickstart/agent-skills)
</Accordion>
...
<Steps>
  <Step title="Crear un proyecto nuevo de .NET" stepNumber={1}>
  ...
  <Step title="Instalar el SDK de Auth0" stepNumber={2}>
    ...
    <Tabs>
      <Tab title="Consola del Administrador de paquetes">
        Abre la Consola del Administrador de paquetes (**Ver** → **Otras ventanas** → **Consola del Administrador de paquetes**) e instala el paquete adecuado:
      ...
      <Tab title="Visual Studio para Mac">
        1. En el **Panel de soluciones**, haz Ctrl+clic (o clic derecho) en la carpeta **Packages**
  ...
  <Step title="Configura tu aplicación de Auth0" stepNumber={3}>
    ...
    **Allowed Callback URLs:**
    ...
    <Tabs>
      ...
      <Tab title="iOS">
        ...
        * `YOUR_BUNDLE_IDENTIFIER` por el identificador del paquete de tu aplicación (por ejemplo, `com.mycompany.myapp`)
        * `{yourDomain}` por tu dominio de Auth0 (por ejemplo, `dev-abc123.us.auth0.com`)
    ...
    **Allowed Logout URLs:**
    ...
    Usa las mismas URL que para las callback URLs.
    ...
    <Info>
      **Allowed Callback URLs** son esenciales para la seguridad: garantizan que los usuarios vuelvan de forma segura a tu aplicación después de autenticarse. Sin una URL coincidente, el proceso de inicio de sesión fallará.

      **Allowed Logout URLs** ofrecen una experiencia fluida cuando los usuarios cierran sesión, ya que los redirigen de vuelta a tu aplicación en lugar de dejarlos en una página de Auth0.
    </Info>
  </Step>
  ...
  <Step title="Inicializar el cliente de Auth0" stepNumber={4}>
    ...
    <Tabs>
      ...
      <Tab title="iOS - AppDelegate">
        Primero, registre el esquema de URL en `Info.plist`:
        ...
        Luego, configure `AppDelegate.cs`:
        ...
        <Warning>
          Reemplace `YOUR_BUNDLE_IDENTIFIER`, `{yourDomain}` y `{yourClientId}` por sus valores reales.
        </Warning>
      </Tab>
    </Tabs>
  ...
  <Step title="Implementar el inicio y cierre de sesión" stepNumber={5}>
    Agrega métodos para gestionar la autenticación del usuario.
  ...
  <Step title="Ejecuta tu aplicación" stepNumber={6}>
    ...
    <Tabs>
      ...
      <Tab title="Visual Studio for Mac">
        1. Selecciona la plataforma de destino (Android/iOS) en el menú desplegable de dispositivos
...
<Check>
  **Punto de control**
  ...
  Ahora tiene una experiencia de inicio de sesión con Auth0 completamente funcional en su aplicación .NET para Android o iOS. La aplicación usa el navegador del sistema para una autenticación segura y gestiona automáticamente el flujo de devolución de llamada.
</Check>
...
<Info>
  Los claims exactos que se devuelven dependen de los alcances solicitados. Para obtener más información, consulta [Uso de los alcances](https://auth0.github.io/auth0-oidc-client-net/documentation/advanced-scenarios/scopes.html) en la documentación de Auth0 OIDC Client.
</Info>
...
<CardGroup cols={2}>
  <Card title="Configurar proveedores de identidad" icon="key" href="/es/docs/authenticate/identity-providers" iconType="solid">
    Agregue proveedores de inicio de sesión social como Google, Facebook y GitHub
  </Card>

  <Card title="Habilitar la autenticación multifactor" icon="shield-halved" href="/es/docs/secure/multi-factor-authentication" iconType="solid">
  ...
  <Card title="Personalizar la experiencia de inicio de sesión" icon="paintbrush" href="/es/docs/customize/universal-login-pages" iconType="solid">
