...
description: Esta guía muestra cómo integrar Auth0 con cualquier aplicación ASP.NET OWIN nueva o existente mediante el paquete NuGet Microsoft.Owin.Security.OpenIdConnect.
...
Auth0 te permite agregar autenticación a tu aplicación ASP.NET OWIN en minutos. Esta guía te muestra cómo agregar el inicio de sesión, el cierre de sesión y la visualización del perfil del usuario a una aplicación clásica de ASP.NET OWIN.

Al final de esta guía, tu aplicación podrá:
...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  Esta guía está dirigida a aplicaciones **clásicas de ASP.NET (.NET Framework)** que usan OWIN. Si tu aplicación ya se ejecuta en **ASP.NET Core**, usa el SDK [`Auth0.AspNetCore.Authentication`](https://auth0.com/docs/quickstart/webapp/aspnet-core) en su lugar.
</Callout>
...
<Steps>
  <Step title="Configure su aplicación de Auth0">
    Toda aplicación que utilice Auth0 debe registrarse en el Auth0 Dashboard. Auth0 genera un **ID de cliente** y un **dominio** que tu aplicación usa para comunicarse con Auth0.

    Puedes crear y configurar la aplicación de Auth0 automáticamente con la Auth0 CLI, o de forma manual desde el Dashboard:
    ...
    <Tabs>
      <Tab title="CLI">
        ...
        Copia las dos líneas `<add>` de la salida a la sección `<appSettings>` de tu `Web.config`:
      ...
      <Tab title="Dashboard">
        ...
        7. Copia los valores de **Domain** y **Client ID** de la sección **Basic Information** y agrégalos a `Web.config`:
    ...
    <Info>
      Si tu aplicación se ejecuta en un puerto distinto, reemplaza `3000` por el número de puerto real en cada una de las URL anteriores.
    </Info>
  </Step>

  <Step title="Instale paquetes NuGet">
    Agrega a tu proyecto los dos paquetes de middleware de OWIN necesarios:

    | Paquete                                 | Propósito                                                                            |
    | --------------------------------------- | ------------------------------------------------------------------------------------ |
    | `Microsoft.Owin.Security.OpenIdConnect` | Controla el flujo de autenticación de OpenID Connect (OIDC) con Auth0                |
    | `Microsoft.Owin.Security.Cookies`       | Conserva la sesión del usuario en una cookie del navegador después de iniciar sesión |
  ...
  <Step title="Configure el middleware de OWIN">
    El middleware de OWIN se registra en una clase de inicio. Si su proyecto ya tiene una clase de inicio de OWIN (normalmente `App_Start/Startup.Auth.cs`), actualice el método `ConfigureAuth`. Si no, cree el archivo ahora.

    Se requieren tanto el **middleware de cookies** como el **middleware de OpenID Connect**, y deben registrarse exactamente en este orden:

    1. Middleware de cookies - almacena la sesión del usuario autenticado
    2. Middleware de OpenID Connect - controla el flujo de inicio y cierre de sesión de Auth0
    ...
    public partial class Startup
    {
        public void ConfigureAuth(IAppBuilder app)
        {
            ...
            // El middleware de cookies debe registrarse primero
  ...
  <Step title="Agregar acciones de inicio de sesión, cierre de sesión y perfil">
  ...
  <Step title="Agregar una vista de perfil">
  ...
  <Step title="Añade enlaces para iniciar y cerrar sesión a tu diseño">
...
<AccordionGroup>
  <Accordion title="La URI de redirección no coincide después de iniciar sesión">
    ...
    **Solución:** La URI de redirección que tu aplicación envía a Auth0 debe coincidir exactamente con una de las **Allowed Callback URLs** de la configuración de tu aplicación en Auth0. Verifica si hay diferencias en el protocolo (`http` vs `https`), el número de puerto, la ruta y las barras finales.
  </Accordion>
  ...
  <Accordion title="Bucle de inicio de sesión: la aplicación sigue redirigiendo a Auth0">
    ...
    **Solución:** Asegúrate de que el middleware esté registrado en el orden correcto y de que la canalización de OWIN esté inicializada:
    ...
    * El atributo `[assembly: OwinStartup(typeof(Startup))]` debe estar presente para que la canalización de OWIN se inicialice correctamente.
    ...
    ```csharp App_Start/Startup.Auth.cs
    app.SetDefaultSignInAsAuthenticationType(CookieAuthenticationDefaults.AuthenticationType); // Debe ser la primera llamada

    app.UseCookieAuthentication(...);          // Middleware de cookies antes de OIDC
    app.UseOpenIdConnectAuthentication(...);   // Middleware de OIDC después de cookies
    ```
  </Accordion>

  <Accordion title="El usuario no vuelve a la aplicación después de cerrar sesión">
    **Problema:** Al hacer clic en **Cerrar sesión**, se cierra la sesión del usuario en Auth0, pero no regresa a tu aplicación.

    **Solución:** Agrega un parámetro de consulta `returnTo` a la URL de cierre de sesión de Auth0 en la notificación `RedirectToIdentityProvider`. La URL de retorno también debe estar incluida en **Allowed Logout URLs** de la configuración de tu aplicación en Auth0:
  ...
  <Accordion title="La imagen de perfil o el correo electrónico están vacíos">
    **Problema:** `Model.ProfileImage` o `Model.EmailAddress` es `null` después de iniciar sesión.

    **Solución:** Verifica que `Scope` en `OpenIdConnectAuthenticationOptions` incluya `"openid profile email"`. El scope `profile` proporciona el nombre y la imagen; el scope `email` proporciona la dirección de correo electrónico.

    ```csharp App_Start/Startup.Auth.cs
    app.UseOpenIdConnectAuthentication(new OpenIdConnectAuthenticationOptions
    {
        Scope = "openid profile email",  // Se requieren los tres alcances
        ...
    });
    ```
  </Accordion>

  <Accordion title="Los valores de dominio o del ID de cliente son null al iniciar">
    **Problema:** La aplicación genera una excepción de referencia nula o de configuración al iniciarse.

    **Solución:** Confirma que tanto `auth0:Domain` como `auth0:ClientId` existan en `<appSettings>` en `Web.config` y que estés ejecutando la configuración de compilación correcta (Debug/Release), que carga la transformación adecuada de `Web.config`.
...
<Accordion title="Personalizar parámetros de inicio de sesión">
  ...
  RedirectToIdentityProvider = notification =>
  {
      if (notification.ProtocolMessage.RequestType == OpenIdConnectRequestType.Authentication)
      {
          ...
          // Establecer una configuración regional específica para la interfaz
...
<Accordion title="Llamar a una API en nombre del usuario">
...
<CardGroup cols={2}>
  <Card title="Aplicación de ejemplo" icon="github" href="https://github.com/auth0-samples/auth0-aspnet-owin-mvc-samples/tree/master/Quickstart/Sample">
    Ejemplo completo y funcional de esta guía de inicio rápido
  </Card>
