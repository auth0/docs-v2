---
mode: wide
description: Esta guía muestra cómo integrar Auth0, añadir autenticación y mostrar información del perfil de usuario en una aplicación de Python con el SDK de Auth0 para FastAPI.
sidebarTitle: FastAPI
title: Añade inicio de sesión a tu aplicación FastAPI
---
...
<Note>
  ...
  * **[Python](https://www.python.org/downloads/)** 3.9 o posterior (se recomienda 3.11 o superior)
  * **[pip](https://pip.pypa.io/)** 21+ o **[Poetry](https://python-poetry.org/)** 1.2+
  * **[OpenSSL](https://www.openssl.org/)**: necesario para generar secretos de sesión
...
Esta guía de inicio rápido muestra cómo añadir la autenticación de Auth0 a una aplicación web de Python con FastAPI. Crearás una aplicación web segura con funciones de inicio de sesión, cierre de sesión y perfil de usuario mediante el [SDK de Auth0 para FastAPI](https://github.com/auth0/auth0-fastapi).
...
<Steps>
  ...
  <Step title="Instala el SDK de Auth0 para FastAPI" stepNumber={2}>
    <Tabs>
      <Tab title="pip">
        ```shellscript
        pip install auth0-fastapi "uvicorn[standard]" python-dotenv itsdangerous
  ...
  <Step title="Configura tu aplicación en Auth0" stepNumber={3}>
    ...
    <Tabs>
      ...
      <Tab title="Dashboard">
        Antes de empezar, cree un archivo `.env` en el directorio raíz de su proyecto:
        ...
        5. Reemplaza los valores del archivo `.env` por los valores de **Domain**, **Client ID** y **Client Secret** del Auth0 Dashboard
        ...
        **Orígenes web permitidos:**
        ...
        <Info>
          ...
          **Allowed Logout URLs** son esenciales para ofrecer una experiencia fluida al cerrar sesión. Sin una URL que coincida, los usuarios no volverán a ser redirigidos a tu aplicación después de cerrar sesión y, en su lugar, se quedarán en una página genérica de Auth0.
  ...
  <Step title="Configura el SDK de Auth0 para FastAPI" stepNumber={4}>
    ...
    <Info>
      **SESSION&#95;SECRET** se usa para cifrar las cookies de sesión y debe ser criptográficamente seguro. Sin un secreto sólido (mínimo 32 bytes), las sesiones de la aplicación pueden verse comprometidas. Genera un secreto seguro con `openssl rand -hex 64` y nunca lo subas al control de versiones.

      **SessionMiddleware** debe agregarse antes de usar el SDK. Sin él, FastAPI no puede leer ni establecer cookies, y todos los intentos de autenticación fallarán sin mostrar errores.

      **HTTPS in Production** es obligatorio para las cookies seguras (`secure=True`). Sin HTTPS, los navegadores no enviarán las cookies de sesión y la sesión de los usuarios se cerrará repetidamente después de cada solicitud.
    </Info>
  </Step>
...
<Check>
  **Punto de control**
  ...
  Ahora deberías tener una página de inicio de sesión de Auth0 totalmente funcional ejecutándose en tu [localhost](http://localhost:3000/).
</Check>
...
<Accordion title="Llamada a APIs protegidas con tokens de acceso">
...
<Accordion title="Las sesiones no persisten / A los usuarios se les cierra la sesión repetidamente">
  **Problema**: Los usuarios inician sesión, pero las sesiones no persisten entre solicitudes.
  ...
  **Posibles causas y soluciones**:
  ...
  2. **HTTP en producción con cookies seguras**
...
<Accordion title="Error de Auth0: Callback URL mismatch">
  **Problema**: Después de hacer clic en &quot;Log In&quot;, Auth0 muestra este error: &quot;Callback URL mismatch&quot;

  **Causa**: La URL de callback no está registrada en la configuración de tu aplicación de Auth0.
...
<Accordion title="ImportError o problemas con async/await">
  **Problema**: Errores relacionados con funciones asíncronas o bucles de eventos.

  **Causa**: FastAPI es un framework asíncrono y todos los métodos del SDK deben esperarse con await.
...
<Accordion title="HTTPS obligatorio en producción">
  **Problema**: Las sesiones funcionan localmente, pero no en producción.

  **Causa**: Las cookies seguras requieren HTTPS en producción. La marca `secure=True` evita que las cookies se envíen a través de conexiones HTTP sin cifrar.
...
Para ver funciones más avanzadas y opciones de configuración, consulta la [documentación del SDK de Auth0 FastAPI](https://github.com/auth0/auth0-fastapi/blob/main/README.md).