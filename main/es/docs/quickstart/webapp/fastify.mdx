---
title: Agrega inicio de sesión a tu aplicación Fastify
sidebarTitle: Fastify
description: Esta guía muestra cómo integrar Auth0, agregar autenticación y mostrar información del perfil del usuario en una aplicación web Fastify con el SDK de Auth0 para Fastify.
mode: wide
---
...
<Accordion title="Usa IA para integrar Auth0" icon="microchip-ai" iconType="solid" defaultOpen>
  ...
  **Luego, pídele a tu asistente de IA:**
  ...
  Tu asistente de IA creará automáticamente tu aplicación en Auth0, obtendrá las credenciales, instalará `@auth0/auth0-fastify`, configurará el plugin y creará todas las rutas y vistas necesarias. [Documentación completa sobre agent skills →](/es/docs/quickstart/agent-skills)
</Accordion>
...
<Steps>
  ...
  <Step title="Instala el SDK de Auth0 para Fastify" stepNumber={2}>
  ...
  <Step title="Configura tu aplicación de Auth0" stepNumber={3}>
    A continuación, debe crear una nueva aplicación en su inquilino de Auth0 y agregar las variables de entorno a su proyecto.

    Tiene tres opciones para configurar su aplicación de Auth0: usar la herramienta Quick Setup (recomendada), ejecutar un comando de la CLI o configurarla manualmente desde el Dashboard:
    ...
    <Tabs>
      ...
      <Tab title="CLI">
        ...
        <CodeGroup>
          ```shellscript Mac
          # Instale Auth0 CLI (si aún no está instalado)
          brew tap auth0/auth0-cli && brew install auth0

          # Configure la aplicación de Auth0 y genere el archivo .env
        ...
        <Note>
          ...
          1. Verificar si ya inició sesión (y pedirle que inicie sesión si es necesario)
          2. Crear una aplicación web regular de Auth0 configurada para `http://localhost:3000`
          3. Generar un archivo `.env` con `AUTH0_DOMAIN`, `AUTH0_CLIENT_ID`, `AUTH0_CLIENT_SECRET`, `SESSION_SECRET` y `APP_BASE_URL`
        </Note>
      </Tab>

      <Tab title="Dashboard">
        1. Vaya al [Auth0 Dashboard](https://manage.auth0.com/dashboard/)
        2. Vaya a **Applications** → **Create Application**
        3. Introduzca un nombre para su aplicación (por ejemplo, &quot;My Fastify App&quot;)
        4. Seleccione **Regular Web Applications** y haga clic en **Create**
        5. En la pestaña **Settings**, configure lo siguiente:
        ...
        6. Desplácese hacia abajo y haga clic en **Save Changes**
        7. Copie los valores de **Domain**, **Client ID** y **Client Secret** de la sección **Basic Information**

        Cree su archivo `.env` con los siguientes valores:
        ...
        <Warning>
          Reemplace `YOUR_AUTH0_DOMAIN` por el dominio de su inquilino de Auth0 (por ejemplo, `dev-abc123.us.auth0.com`), `YOUR_CLIENT_ID` por el Client ID de su aplicación y `YOUR_CLIENT_SECRET` por el Client Secret de su aplicación en el Dashboard.
        </Warning>

        Genere un secreto seguro para cifrar la sesión:

        ```bash
        openssl rand -hex 64
        ```

        Copie la salida y úsela como valor de `SESSION_SECRET` en su archivo `.env`.
      </Tab>
    </Tabs>
  ...
  <Step title="Crear plantillas de vista" stepNumber={5}>
...
<Accordion title="Llamadas a API protegidas con tokens de acceso">
...
<Accordion title="Uso de TypeScript">
...
<AccordionGroup>
  <Accordion title="Problemas comunes y soluciones">
    ...
    **Problema:** El valor de `state` no coincide entre la solicitud de autenticación y el callback.
    ...
    **Solución:** Asegúrate de que el plugin de Auth0 se registre antes de acceder a los métodos de sesión:
...
* **[Autenticación de API con Fastify](/es/docs/quickstart/backend/fastify)** - Protege los endpoints de tu API mediante la validación de JWT
* **[Personaliza Universal Login](https://auth0.com/docs/customize/universal-login-pages)** - Adapta la experiencia de inicio de sesión a tu marca
* **[Agregar conexiones sociales](https://auth0.com/docs/connections/social)** - Habilita el inicio de sesión con Google, GitHub y otros proveedores sociales
