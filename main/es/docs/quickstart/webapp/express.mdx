---
mode: wide
description: En esta guía se muestra cómo integrar Auth0, agregar autenticación y mostrar información del perfil de usuario en una aplicación web de Express.js mediante el SDK express-openid-connect.
sidebarTitle: Express
title: Agrega el Login a tu aplicación de Express
---
...
<Accordion title="Usa IA para integrar Auth0" icon="microchip-ai" iconType="solid" defaultOpen>
  ...
  **Luego, pídele a tu asistente de IA:**
  ...
  Tu asistente de IA creará automáticamente tu aplicación de Auth0, obtendrá las credenciales, instalará `express-openid-connect`, configurará el middleware y preparará tus rutas. [Documentación completa de agent skills →](/es/docs/quickstart/agent-skills)
</Accordion>

<Note>
  **Requisitos previos:** Antes de comenzar, asegúrate de tener instalado lo siguiente:

  * [Node.js](https://nodejs.org/) 18 LTS o una versión posterior
  * [npm](https://www.npmjs.com/) 10+ o [yarn](https://yarnpkg.com/) 1.22+
  * [jq](https://jqlang.github.io/jq/) - Requerido para configurar la CLI de Auth0 (opcional)
...
A continuación, debes crear una aplicación nueva en tu inquilino de Auth0 y agregar las variables de entorno a tu proyecto.
...
<Tabs>
  ...
  <Tab title="Dashboard">
    ...
    3. Ingresa un nombre para tu aplicación (por ejemplo, &quot;My Express App&quot;)
    ...
    <Warning>
      Reemplaza `YOUR_AUTH0_DOMAIN` por el dominio de tu inquilino de Auth0 (por ejemplo, `dev-abc123.us.auth0.com`) y `YOUR_CLIENT_ID` por el ID de cliente de tu aplicación en el dashboard.
    </Warning>

    Genera un secreto seguro para cifrar sesiones:
...
Agrega el middleware de Auth0 a tu aplicación de Express. El middleware `auth()` se encarga de gestionar las sesiones y crea automáticamente las rutas `/login`, `/logout` y `/callback`.
...
Ahora agregue rutas para mostrar enlaces para iniciar y cerrar sesión, y una página de perfil protegida.
...
<Check>
  **Verificación**
  ...
  Ahora deberías tener una página de Login de Auth0 totalmente funcional. Cuando:
...
<AccordionGroup>
  <Accordion title="Proteger rutas específicas con requiresAuth()">
  ...
  <Accordion title="Llamar a APIs protegidas con tokens de acceso">
    Para llamar a APIs externas que requieren un token de acceso, configure el SDK para solicitar uno:
  ...
  <Accordion title="Uso de la autorización basada en claims">
    ...
    <Note>
      Los claims como `role` deben agregarse a los tokens mediante Rules o Actions de Auth0. [Obtén más información sobre cómo agregar claims personalizados](https://auth0.com/docs/customize/actions/flows-and-triggers/login-flow/add-user-roles-to-id-and-access-tokens).
    </Note>
  </Accordion>

  <Accordion title="Almacén de sesiones personalizado (Redis)">
...
<AccordionGroup>
  <Accordion title="Problemas comunes y soluciones">
    ...
    **Solución:** Asegúrate de que se llame a `app.use(auth(config))` antes de cualquier ruta que acceda a `req.oidc`:
    ...
    **Problema:** Los datos de la sesión del usuario superan los límites de tamaño de las cookies.
...
* **[Agregar autorización](https://auth0.com/docs/manage-users/access-control/rbac)** - Implementa el control de acceso basado en roles
* **[Llamar a API protegidas](https://auth0.com/docs/secure/tokens/access-tokens/get-access-tokens)** - Usa tokens de acceso para llamar a tus API de backend
* **[Personalizar Universal Login](https://auth0.com/docs/customize/universal-login-pages)** - Personaliza la experiencia de inicio de sesión con tu marca
* **[Agregar conexiones sociales](https://auth0.com/docs/connections/social)** - Habilita Google, GitHub y otros inicios de sesión con cuentas sociales
