---
mode: wide
description: Esta guía muestra cómo integrar Auth0, agregar autenticación y mostrar la información del perfil de usuario en una aplicación de una sola página (SPA) que usa JavaScript puro mediante el SDK de SPA de Auth0.
sidebarTitle: JavaScript
title: Agrega el inicio de sesión a tu aplicación de JavaScript
---
...
<Note>
  ...
  **Herramienta de compilación:** Esta guía de inicio rápido usa **Vite** para el desarrollo. También puedes usar el SDK a través de un CDN para una configuración sin necesidad de una herramienta de compilación.
</Note>
...
Este inicio rápido muestra cómo agregar la autenticación de Auth0 a una aplicación JavaScript sin framework. Crearás una aplicación moderna de una sola página con una función de inicio de sesión segura usando JavaScript puro y el SDK de SPA de Auth0.

export const localEnvSnippet = `VITE_AUTH0_DOMAIN={yourDomain}
VITE_AUTH0_CLIENT_ID={yourClientId}`;

<Steps>
  <Step title="Crear un proyecto" stepNumber={1}>
    Crea un nuevo proyecto de JavaScript para esta guía de inicio rápido

    ```shellscript
    mkdir auth0-vanillajs && cd auth0-vanillajs
    ```

    Inicializar el proyecto, instalar un servidor de desarrollo local y configurar scripts
  ...
  <Step title="Instala el SDK SPA JS de Auth0" stepNumber={2}>
    ```shellscript
    npm install @auth0/auth0-spa-js
    ```
  </Step>
  ...
  <Step title="Configura tu aplicación de Auth0" stepNumber={3}>
    ...
    Tienes tres opciones para configurar tu aplicación de Auth0: usar la herramienta Quick Setup (recomendada), ejecutar un comando de la CLI o configurarla manualmente desde el Dashboard:

    <Tabs>
      <Tab title="Quick Setup (recomendado)">
        Crea una aplicación de Auth0 y copia el archivo `.env` pregenerado con los valores de configuración correctos.
      ...
      <Tab title="Dashboard">
        ...
        5. Sustituye `YOUR_AUTH0_APP_DOMAIN` y `YOUR_AUTH0_APP_CLIENT_ID` en el archivo `.env.local` por los valores de **Domain** y **Client ID** del dashboard
        ...
        **Allowed Web Origins:**
        ...
        <Info>
          ...
          **Allowed Logout URLs** es esencial para ofrecer una experiencia fluida al cerrar sesión. Sin una URL coincidente, los usuarios no volverán a ser redirigidos a tu aplicación después de cerrar sesión y, en su lugar, se quedarán en una página genérica de Auth0.
  ...
  <Step title="Cree la estructura HTML y la lógica de la aplicación" stepNumber={4}>
...
<Check>
  **Punto de control**
  ...
  Ahora deberías tener una página de inicio de sesión de Auth0 totalmente funcional ejecutándose en tu [localhost](http://localhost:5173/)
</Check>
...
<Accordion title="Obtener un token de acceso para llamar a una API">
  Si necesitas llamar a una API protegida, puedes obtener un token de acceso:
...
<Accordion title="Usar el inicio de sesión con ventana emergente">
