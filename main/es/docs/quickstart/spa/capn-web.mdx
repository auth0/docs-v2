---
mode: wide
description: Esta guía muestra cómo integrar Auth0, añadir autenticación y mostrar información del perfil del usuario en una aplicación de página única (SPA) que usa Cap'n Web RPC mediante el SDK de Auth0 para SPA.
...
<Steps>
  ...
  <Step title="Instalar dependencias" stepNumber={2}>
    ...
    <Info>
      **@auth0/auth0-spa-js** se usa en el cliente para gestionar la autenticación de usuarios, los flujos de inicio de sesión y la gestión de tokens en el navegador.

      **@auth0/auth0-api-js** se usa en el servidor para verificar tokens de acceso y validar firmas JWT mediante el JWKS de Auth0.
    </Info>
  ...
  <Step title="Configura tu aplicación de Auth0" stepNumber={3}>
    ...
    <Tabs>
      ...
      <Tab title="Panel">
        Antes de empezar, crea un archivo `.env` en el directorio raíz del proyecto
        ...
        **Orígenes web permitidos:**
        ...
        <Info>
          ...
          **Allowed Logout URLs** son esenciales para ofrecer una experiencia fluida al cerrar sesión. Sin una URL que coincida, los usuarios no volverán a tu aplicación después de cerrar sesión, sino que permanecerán en una página genérica de Auth0.
        ...
        **Obligatorio: crea una API de Auth0**

        Debes crear una API de Auth0 para que la validación de tokens funcione:

        1. En el Auth0 Dashboard, ve a **APIs** &gt; **Create API**
        2. Configura estos valores:
           * **Nombre**: `Cap'n Web API`
           * **Identifier**: `https://capnweb-api.yourproject.com` (usa tu propio identificador único)
           * **Signing Algorithm**: `RS256` (predeterminado)
        3. Haz clic en **Create**
        4. En la pestaña **Scopes**, agrega estos alcances:
           * `read:profile` - Leer los datos del perfil del usuario
           * `write:profile` - Actualizar los datos del perfil del usuario
        5. Actualiza tu archivo `.env` con el identificador de la API como `AUTH0_AUDIENCE`

        <Warning>
          Si no creas una API de Auth0, recibirás un error &quot;access&#95;denied&quot; durante el inicio de sesión. El identificador de la API debe coincidir exactamente con tu variable de entorno `AUTH0_AUDIENCE`.
        </Warning>
      </Tab>
    </Tabs>
  </Step>

  <Step title="Crear el servidor" stepNumber={5}>
  ...
  <Step title="Crear la interfaz del cliente" stepNumber={6}>
...
<Accordion title="Seguridad de RPC en el servidor">
...
<Accordion title="Integración con base de datos">
