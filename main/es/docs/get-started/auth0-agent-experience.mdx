---
title: Revisar el Agent Experience Score de Auth0
description: Conozca cómo Auth0 evalúa el rendimiento de los agentes de IA en distintos modelos y frameworks, incluidas las dimensiones, el sistema de calificación y la metodología del Agent Experience Score.
---

El [Agent Experience Score](https://auth0.com/agent-experience) mide qué tan bien los agentes de IA implementan Auth0 en diferentes modelos y frameworks. Le permite comparar las puntuaciones actuales de agentes que implementan servicios y funcionalidades de Auth0, como la autenticación multifactor (MFA) o Auth0 Actions, en entornos de desarrollo y pruebas, para evaluar cómo las herramientas de Auth0 mejoran el rendimiento de los agentes.

Use este recurso para conocer la metodología de puntuación, incluido cómo se calculan las puntuaciones, qué dimensiones se miden y cómo se asignan las calificaciones.

<div id="test-specifications">
  ## Especificaciones de las pruebas
</div>

Los agentes de IA — Claude Code, GitHub Copilot y Gemini CLI — ejecutan tareas de integración con Auth0 en entornos de desarrollo aislados. Cada agente usa las mismas herramientas que usaría un desarrollador en un entorno real: un espacio de trabajo, una shell y herramientas para archivos como Auth0 CLI. Los prompts son breves y realistas: &quot;agrega autenticación a mi aplicación de Next.js&quot;, no recetas paso a paso.

Cada modelo se prueba con y sin herramientas de Auth0 ([MCP Server](/es/docs/get-started/build-with-ai-tools#auth0-docs-mcp-server) y [Agent Skills](/es/docs/quickstart/agent-skills)). La diferencia entre esos resultados es el impacto medible de las herramientas de IA de Auth0 en la experiencia del desarrollador.
...
Cada evaluador verifica el código generado, no la prosa ni las explicaciones. Los evaluadores comprueban que el código compile, importe paquetes reales, invoque métodos reales del SDK y no introduzca vulnerabilidades de seguridad.
...
El costo se calcula a partir del total de tokens consumidos durante la ejecución de la evaluación (tokens de entrada + tokens de salida), multiplicado por el precio por token publicado por el proveedor del modelo. Auth0 no cobra por ejecutar evaluaciones; el costo refleja lo que usted pagaría a su proveedor de modelos por un uso equivalente de tokens.

El precio de los tokens varía según el modelo y el proveedor. Para consultar las tarifas actuales, consulte la página de precios de su proveedor:
...
El tiempo es la duración real de la ejecución de la evaluación, desde el envío del prompt hasta el resultado final. Incluye toda la actividad del agente: leer archivos, hacer llamadas a herramientas, esperar respuestas de la API y escribir código.
...
* [Servidor MCP de Auth0](/es/docs/get-started/build-with-ai-tools#auth0-docs-mcp-server)
