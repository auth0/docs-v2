---
description: Describe las plantillas de scripts de acciones de base de datos personalizadas para la creación de usuarios.
title: Plantillas del script Create
---

El script Create implementa la función definida cuando se crea un usuario. Recomendamos llamar a esta función `create`.

Este script es opcional. Si está habilitado, cuando un usuario se registra a través de <Tooltip tip="Universal Login: Su aplicación redirige a Universal Login, alojado en el Servidor de autorización de Auth0, para verificar la identidad de un usuario." cta="Ver glosario" href="/es/docs/glossary?term=Universal+Login">Universal Login</Tooltip> o se crea a través de <Tooltip tip="Auth0 Dashboard: El producto principal de Auth0 para configurar sus servicios." cta="Ver glosario" href="/es/docs/glossary?term=Auth0+Dashboard">Auth0 Dashboard</Tooltip> o Auth0 <Tooltip tip="Auth0 Dashboard: El producto principal de Auth0 para configurar sus servicios." cta="Ver glosario" href="/es/docs/glossary?term=Management+API">Management API</Tooltip>, Auth0 ejecutará el script para crear el registro de usuario correspondiente en la base de datos externa.

Cuando se crea un usuario en Auth0, Auth0 invoca una serie de scripts:

1. [Get User](/es/docs/authenticate/database-connections/custom-db/templates/get-user): Verifica que el usuario no exista ya en Auth0 ni en la base de datos externa.
2. Create: Crea el usuario en la base de datos externa.
3. [Login](/es/docs/authenticate/database-connections/custom-db/templates/login): Verifica que el usuario se haya creado correctamente.
...
| ---------- | ---------------------------------------------------------------------------------------------------------------------- |
| `user`     | Objeto. Contiene [datos del perfil del usuario](#user-object-parameter) obtenidos del proceso de creación del usuario. |
| `callback` | Función. Se usa para pasar datos de error a través del pipeline.                                                       |

### Ejemplo

Este es un ejemplo en pseudo-JavaScript de cómo podría implementar la función `create`. Para ver ejemplos específicos de cada lenguaje, consulte [Ejemplos de scripts específicos de cada lenguaje](#language-specific-script-examples).
...
El objeto de error personalizado `ValidationError` le permite pasar datos que se mostrarán en los [registros del Tenant](/es/docs/deploy-monitor/logs).
...
| `tenant`        | El nombre del inquilino de Auth0.                                                                                                                                                                        |
| `email`         | La dirección de correo electrónico del usuario.                                                                                                                                                          |
| `password`      | La contraseña del usuario en texto sin formato.                                                                                                                                                          |
| `username`      | El username del usuario. Solo es obligatorio si la conexión de base de datos personalizada tiene habilitada la opción [**Requires Username**](/es/docs/authenticate/database-connections/require-username). |
| `connection`    | El nombre de la conexión de Auth0.                                                                                                                                                                       |
| `user_metadata` | Contiene las propiedades del objeto `user_metadata` en el perfil de Auth0 del usuario, si existe.                                                                                                        |
| `app_metadata`  | Contiene las propiedades del objeto `app_metadata` en el perfil de Auth0 del usuario, si existe.                                                                                                         |

<div id="username-property">
  ### Propiedad `username`
</div>

Si tu conexión de base de datos personalizada tiene habilitada la opción [Requires Username](/es/docs/authenticate/database-connections/require-username), los scripts de [Login](/es/docs/authenticate/database-connections/custom-db/templates/login) y [Get User](/es/docs/authenticate/database-connections/custom-db/templates/get-user) deben admitir la propiedad `username`, por lo que debes almacenarla en tu base de datos externa.
...
Si [crea y usa campos personalizados](/es/docs/libraries/custom-signup#using-the-api) durante el proceso de registro, se incluirán en el objeto `user`.
...
* [Plantillas de scripts para obtener un usuario](/es/docs/authenticate/database-connections/custom-db/templates/get-user)
