---
description: Describe las plantillas de scripts de acción de base de datos personalizada para la verificación del correo electrónico del usuario.
title: Plantillas del script Verify
---

El script Verify implementa la función que se ejecuta para marcar el estado de verificación de la dirección de correo electrónico de un usuario en una base de datos externa. Recomendamos llamar a esta función `verify`.

Este script solo se utiliza en escenarios de autenticación heredada y es necesario para admitir la verificación de la dirección de correo electrónico del usuario. Una dirección de correo electrónico verificada es fundamental para varios flujos de trabajo en Auth0, e implementar este script proporciona compatibilidad con ellos de forma predeterminada.
...
| `callback` | Function | Se utiliza para pasar errores o datos del perfil por el flujo de procesamiento. |

### Ejemplo

Este es un ejemplo en pseudo-JavaScript de cómo podría implementar la función `login`. Para ver ejemplos específicos de cada lenguaje, consulte [Ejemplos de scripts específicos de cada lenguaje](/es/docs/authenticate/database-connections/custom-db/templates/verify#language-specific-script-examples).
...
| `verified` | Boolean | Opcional    | Contiene un valor que representa el estado de verificación del usuario en la base de datos externa (`true` o `false`). Solo es obligatorio si el valor es `true`. |
...
<Warning>
  El script Verify no cambia el valor del atributo `email_verified` en el perfil de Auth0 del usuario, independientemente del valor que devuelva la función `callback`.

  Para actualizar el atributo `email_verified` en el perfil de Auth0 del usuario, debe incluir el atributo `email_verified` y su valor en el objeto de perfil del usuario que devuelven los scripts [Login](/es/docs/authenticate/database-connections/custom-db/templates/login) y [Get User](/es/docs/authenticate/database-connections/custom-db/templates/get-user).
</Warning>
...
* [Plantillas de scripts de obtención de usuarios](/es/docs/authenticate/database-connections/custom-db/templates/get-user)
...
* [Plantillas de scripts de cambio de contraseña](/es/docs/authenticate/database-connections/custom-db/templates/change-password)
* [Plantilla de script de cambio de correo electrónico](/es/docs/authenticate/database-connections/custom-db/templates/change-email)