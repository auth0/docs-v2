---
description: Describe las plantillas de script de acción de base de datos personalizadas para la búsqueda de usuarios.
title: Plantillas de script de Get User
---
...
Este script es obligatorio para la [migración automática](/es/docs/manage-users/user-migration/configure-automatic-migration-from-your-database) y, de forma condicional, para la autenticación heredada, según las operaciones configuradas para la conexión. Auth0 recomienda establecer un `user_id` permanente en el perfil de usuario devuelto para evitar la creación de usuarios duplicados.

Si la migración automática está configurada para la conexión y el perfil de usuario aún no se ha creado, el script se ejecuta cada vez que se realiza una de las siguientes operaciones:
...
Este es un ejemplo en seudo-JavaScript de cómo podría implementar la función `getUser`. Para ver ejemplos específicos de cada lenguaje, consulte [Ejemplos de scripts específicos por lenguaje](#language-specific-script-examples).
...
Si se encuentra el usuario, pasa un valor `null` al parámetro `error` y los datos del perfil del usuario al parámetro `profile` en [formato normalizado](/es/docs/manage-users/user-accounts/user-profiles/normalized-user-profile-schema). Además de los campos estándar, puedes incluir los campos `user_metadata`, `app_metadata` y `mfa_factors`.
...
Si se produce un error, pasa los datos del error al parámetro `error` con información relevante sobre lo que falló. Para obtener más información, consulta [Solucionar problemas de bases de datos personalizadas](/es/docs/authenticate/database-connections/custom-db/error-handling).
