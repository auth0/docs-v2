---
description: Describe cómo implementar la autenticación sin contraseña con inicio de sesión integrado en aplicaciones web tradicionales.
title: Inicio de sesión integrado sin contraseña en aplicaciones web tradicionales
---
...
Para usar las API de <Tooltip tip="sin contraseña: Forma de autenticación que no depende de una contraseña como primer factor." cta="Ver glosario" href="/es/docs/glossary?term=Passwordless">sin contraseña</Tooltip> integradas en aplicaciones web tradicionales, asegúrate de habilitar la concesión **Passwordless OTP** en [Auth0 Dashboard &gt; Applications &gt; Applications](https://manage.auth0.com/#/applications), en la configuración de tu aplicación, en **Advanced Settings** &gt; **Grant Types**.

La autenticación sin contraseña para aplicaciones web tradicionales consta de dos pasos:
...
Ten en cuenta que, al usar enlaces mágicos, no necesitas llamar a `/oauth/token`. El usuario hará clic en el enlace mágico y será redirigido a la URL de devolución de llamada de la aplicación.
...
El endpoint `/passwordless/start` tiene un [límite de tasa](/es/docs/troubleshoot/customer-support/operational-policies/rate-limit-policy) de 50 solicitudes por hora por IP. Si llama a la API desde el lado del servidor, la IP de su backend puede alcanzar fácilmente estos límites. Para obtener más información sobre cómo abordar este problema, lea la sección Limitación de tasa en endpoints sin contraseña de [Uso de las API sin contraseña](/es/docs/authenticate/passwordless/implement-login/embedded-login/relevant-api-endpoints).

<div id="customize-mfa">
  ## Personalizar MFA
</div>

<ReleaseStageNotice feature="MFA personalizable con los flujos de Resource Owner Password Grant, integrado o de Token de actualización" stage="ea" terms="true" contact="Soporte de Auth0" />

Personaliza <Tooltip tip="Autenticación multifactor (MFA): proceso de autenticación del usuario que utiliza un factor adicional además de username y contraseña, como un code enviado por SMS." cta="Ver glosario" href="/es/docs/glossary?term=MFA">MFA</Tooltip> con flujos embebidos. Usa la API de MFA para permitir que los usuarios se inscriban y completen desafíos con los factores compatibles con tu aplicación que elijan.

Al usar [Lock for Web](/es/docs/libraries/lock#2-authenticating-and-getting-user-info), el endpoint `oauth/token` devuelve el error `mfa_required` e incluye el `mfa_token` que necesitas para usar la API de MFA, así como el parámetro `mfa_requirements` con una lista de los autenticadores que tu aplicación admite actualmente:
...
Usa el `mfa_token` para llamar al endpoint [`mfa/authenticator`](https://auth0.com/docs/api/authentication/muti-factor-authentication/list-authenticators) a fin de listar todos los factores en los que el usuario se ha inscrito y hacer coincidir el `type` con el que admite tu aplicación. También debes obtener el `authenticator_type` correspondiente para emitir desafíos:
...
Continúe aplicando la comprobación de MFA llamando al endpoint [`request/mfa/challenge`](https://auth0.com/docs/api/authentication/muti-factor-authentication/request-mfa-challenge).
