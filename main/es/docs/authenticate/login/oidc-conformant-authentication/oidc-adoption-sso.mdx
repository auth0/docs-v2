---
description: Aprende cómo el flujo compatible con OIDC afecta el uso del inicio de sesión único (SSO).
title: Inicio de sesión único con OIDC
---

En el contexto del flujo de autenticación compatible con OIDC, el inicio de sesión único (<Tooltip tip="Single Sign-On (SSO): Servicio que, después de que un usuario inicia sesión en una aplicación, inicia automáticamente la sesión de ese usuario en otras aplicaciones." cta="Ver glosario" href="/es/docs/glossary?term=SSO">SSO</Tooltip>) debe producirse en el <Tooltip tip="Single Sign-On (SSO): Servicio que, después de que un usuario inicia sesión en una aplicación, inicia automáticamente la sesión de ese usuario en otras aplicaciones." cta="Ver glosario" href="/es/docs/glossary?term=authorization+server">servidor de autorización</Tooltip> (es decir, Auth0), y no en la aplicación, lo que significa que debes usar <Tooltip tip="Authorization Server: Servidor centralizado que contribuye a definir los límites del acceso de un usuario. Por ejemplo, tu servidor de autorización puede controlar los datos, las tareas y las funciones disponibles para un usuario." cta="Ver glosario" href="/es/docs/glossary?term=Universal+Login">Universal Login</Tooltip> y redirigir a los usuarios a la página de inicio de sesión. Para obtener más información, consulta [Universal Login](/es/docs/authenticate/login/auth0-universal-login) y [Single Sign-On](/es/docs/authenticate/single-sign-on).

En términos generales, al realizar SSO:

1. Si el usuario no ha iniciado sesión localmente, debes redirigirlo a la página de inicio de sesión de Auth0 (`/authorize`) para autenticarse mediante un flujo basado en redirección, como [Flujo de código de autorización](/es/docs/get-started/authentication-and-authorization-flow/authorization-code-flow) o [Implicit Flow](/es/docs/get-started/authentication-and-authorization-flow/implicit-flow-with-form-post), según el tipo de aplicación.
