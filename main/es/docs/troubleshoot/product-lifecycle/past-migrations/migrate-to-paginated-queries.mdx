...
title: Migración a consultas paginadas de endpoints de Management API v2
---

Después del **26 de enero de 2021** (o de la versión 2202 de Private Cloud), las solicitudes a los endpoints de <Tooltip tip="Management API: Un producto que permite a los clientes realizar tareas administrativas." cta="Ver glosario" href="/es/docs/glossary?term=Management+API">Management API</Tooltip> v2 devolverán un máximo de 50 elementos en los inquilinos de Public Cloud. Para obtener más elementos, debe incluir los parámetros `page` y `per_page`. A partir del **21 de julio de 2020** (o de la versión 2108 de Private Cloud), Auth0 mostrará registros del inquilino y un interruptor de migración para ayudarle a prepararse para este cambio.
...
Las llamadas a los siguientes endpoints de Management API v2 están afectadas:
...
Los avisos de deprecación se registrarán en los registros del inquilino para todas las solicitudes sin opciones de paginación que actualmente devuelven más de 1 elemento, una vez por hora, por cada cliente y endpoint diferentes.
...
2. Confirme que ya no aparezcan avisos de deprecación en los registros del inquilino. Verifique si alguna solicitud devolvió más de 50 elementos. Revise el campo `details.size_exceeded` y compruebe si es `true`.

   1. Use la siguiente consulta de logs para devolver todas las llamadas sin opciones de paginación con más de 1 resultado:
      `type:depnote AND description:*Unpaginated*`
   2. Use la siguiente consulta de logs para devolver todas las llamadas sin opciones de paginación con más de 50 resultados:
...
3. Deshabilite las solicitudes sin paginación de Management API para su inquilino. Vaya a [Dashboard &gt; Configuración del Tenant &gt; Advanced &gt; Migration](https://manage.auth0.com/#/tenant/advanced). Esto simulará el comportamiento esperado una vez que se cierre la ventana de migración, por lo que las llamadas a los endpoints afectados devolverán hasta 50 resultados. Podrá volver a habilitar las solicitudes sin paginación en cualquier momento antes de esa fecha.

<div id="update-extensions">
  ### Actualizar extensiones
</div>

Es posible que debas actualizar [Auth0 Extensions](/es/docs/customize/extensions) y las extensiones personalizadas desde versiones anteriores a las más recientes para asegurarte de que solo realicen consultas paginadas.
...
2. Ve a [Dashboard &gt; Extensiones](https://manage.auth0.com/#/extensions), selecciona **Extensiones instaladas** y haz clic en el enlace **Actualizar** de la extensión, si está disponible.

<div id="update-deploy-cli-tool">
  ### Actualice la herramienta Deploy CLI
</div>

Si utiliza la herramienta `auth0-deploy-cli`, debe actualizarla a la versión **5.3.0 o posterior**. Para obtener más información, consulte [Deploy CLI Tool](/es/docs/deploy-monitor/deploy-cli-tool).