---
description: Describe las opciones de configuración de la página alojada de MFA para personalizar las propiedades del tema en las páginas de MFA.
title: Diccionario de idioma del tema de MFA
---
...
`languageDictionary` se puede usar para sobrescribir el texto en varias áreas del proceso de <Tooltip tip="Autenticación multifactor (MFA): proceso de autenticación de usuarios que usa un factor además del nombre de usuario y la contraseña, como un código por SMS." cta="Ver glosario" href="/es/docs/glossary?term=MFA">MFA</Tooltip> en la experiencia de Classic Login. Para ello, use el siguiente formato:
...
Aquí tienes un ejemplo en el que se modifica el atributo `headerText` en `smsEnrollmentConfirm`:
...
...
  ...
  languageDictionary:{ // Usa la opción del diccionario de idioma
    smsEnrollmentConfirm:{  // Indica qué categoría/sección estás modificando
      headerText: "Some custom text - In order to confirm enrollment we need to confirm your phone. Please enter the received code." // Proporciona el nuevo valor para el atributo específico que deseas modificar
    }
  }
...
A continuación, encontrarás una lista de las categorías y opciones disponibles.

<div id="languagedictionary-options">
  ### Opciones de languageDictionary
</div>
...
smsEnrollmentAddPhoneNumber: {
  ...
  // countryCodes: { 'US': 'Traducción de Estados Unidos', ... '<country code>': '<translation>' }
},
