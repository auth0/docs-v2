...
title: Inscripción y desafío de autenticadores SMS y de voz
---
...
Auth0 proporciona un flujo integrado de inscripción y autenticación con <Tooltip tip="Autenticación multifactor (MFA): proceso de autenticación del usuario que utiliza un factor adicional al username y la contraseña, como un code por SMS." cta="Ver glosario" href="/es/docs/glossary?term=MFA">MFA</Tooltip> mediante [Universal Login](/es/docs/authenticate/login/auth0-universal-login). Sin embargo, si quiere crear su propia interfaz de usuario, puede usar la [MFA API](/es/docs/secure/multi-factor-authentication/multi-factor-authentication-developer-resources/mfa-api) para hacerlo.

<div id="prerequisites">
  ## Requisitos previos
</div>

Antes de poder usar las API de MFA, debe habilitar el tipo de concesión de MFA para su aplicación. Vaya a [Auth0 Dashboard &gt; Applications &gt; Advanced Settings &gt; Grant Types](https://manage.auth0.com/#/applications) y seleccione **MFA**.

* [Configure el teléfono como factor](/es/docs/secure/multi-factor-authentication/multi-factor-authentication-factors/configure-sms-voice-notifications-mfa) en el Dashboard o mediante la [Management API](https://auth0.com/docs/api/management/v2#!/Guardian/put_factors_by_name).
...
Según cuándo actives la inscripción, puedes obtener un <Tooltip tip="Token de acceso: credencial de autorización, en forma de una cadena opaca o JWT, que se utiliza para acceder a una API." cta="Ver glosario" href="/es/docs/glossary?term=access+token">token de acceso</Tooltip> para usar la API de MFA de distintas maneras:
...
Realice una solicitud `POST` al endpoint MFA Associate para inscribir el autenticador del usuario. El token bearer requerido por este endpoint es el token de MFA obtenido en el paso anterior.
...
Si recibe un error `User is already enrolled`, el usuario ya tiene inscrito un factor de MFA. Antes de asociar otro factor al usuario, debe solicitarle un desafío con el factor existente.
...
Obtén un token de MFA siguiendo los pasos que se describen en [Autenticación con Resource Owner Password Grant y MFA](/es/docs/secure/multi-factor-authentication/authenticate-using-ropg-flow-with-mfa).

<div id="retrieve-enrolled-authenticators">
  ### Recuperar autenticadores inscritos
</div>

Para desafiar al usuario, necesita el `authenticator_id` del factor que quiere desafiar. Puede listar todos los autenticadores inscritos mediante el endpoint de MFA Authenticators:
...
<Warning>
  La MFA personalizable en los flujos Resource Owner Password Grant, Embedded o Refresh Token está en EA. Al usar esta función, acepta los términos aplicables de la prueba gratuita del [Master Subscription Agreement](https://www.okta.com/legal/) de Okta. Para obtener más información sobre las fases de lanzamiento de Auth0, consulte [Etapas de lanzamiento del producto](/es/docs/troubleshoot/product-lifecycle/product-release-stages). Para participar en el acceso anticipado, póngase en contacto con [Soporte de Auth0](https://support.auth0.com/).
</Warning>

La MFA personalizable permite a los usuarios inscribirse y realizar desafíos con los factores de su elección que admite su aplicación.
...
Use `mfa_token` para llamar al endpoint [`mfa/authenticator`](/es/docs/api/authentication/muti-factor-authentication/list-authenticators) y obtener una lista de todos los factores en los que el usuario se ha inscrito, y busque uno del mismo tipo que admite su aplicación. También debe obtener el `authenticator_type` correspondiente para emitir desafíos:
...
Exige el desafío de MFA llamando al endpoint [`request/mfa/challenge`](/es/docs/api/authentication/muti-factor-authentication/request-mfa-challenge).
...
* [Inscripción y desafío de autenticadores de correo electrónico](/es/docs/secure/multi-factor-authentication/authenticate-using-ropg-flow-with-mfa/enroll-and-challenge-email-authenticators)
