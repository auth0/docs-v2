...
Este artículo te guía por todo el proceso de aprobación de una transferencia bancaria. El mismo <Tooltip tip="Flujo de autorización: concesión de autorización (o flujo de trabajo) especificado en el marco de OAuth 2.0." cta="Ver glosario" href="/es/docs/glossary?term=authorization+flow">flujo de autorización</Tooltip> transaccional se puede aplicar a otros casos de uso.
...
Antes de comenzar, siga las instrucciones de [Configurar solicitudes de autorización enriquecidas](/es/docs/get-started/apis/configure-rich-authorization-requests) para registrar `authorization_details.types` para su API o <Tooltip tip="Resource Server: servidor que aloja recursos protegidos. Los Resource Server aceptan solicitudes de recursos protegidos y responden a ellas." cta="Ver glosario" href="/es/docs/glossary?term=resource+server">Resource Server</Tooltip>.
...
2. Aplique una política dinámica después de que el usuario se autentique. Con [Actions](/es/docs/customize/actions), puede decidir dinámicamente los siguientes pasos en función de los detalles de la transacción y de otra información que pueda obtener de fuentes como API externas. Para obtener más información, consulte [Aplicar una política dinámica](#apply-dynamic-policy).
...
El usuario accede por primera vez a su aplicación web después de autenticarse con Auth0. Siguiendo nuestro caso de uso de ejemplo, el usuario solicita luego una transferencia de dinero a uno de sus contactos.
...
En el caso de las notificaciones push, la aplicación móvil es responsable de mostrar al usuario los detalles de la transacción para que la apruebe de forma explícita. Al enviar una notificación push, puedes deshabilitar la opción de recurrir al ingreso manual de un OTP agregando la opción `otpFallback: false`.

Para mostrar `authorization_details` al usuario, la aplicación móvil debe recuperarlos del parámetro `txlnkid`. El [SDK de Auth0 Guardian](/es/docs/secure/multi-factor-authentication/auth0-guardian) pasa el parámetro `txlnkid` del inquilino a la aplicación móvil mediante una notificación push.
            ...
            void onFailure(Throwable exception) {
              if (exception instanceof GuardianException) {
                ...
                if (guardianException.isResourceNotFound()) {
                  // no hay ningún consentimiento asociado con la transacción
                }
              }
              // algo salió mal
            }
...
Para completar la operación, Auth0 utiliza el estándar [Flujo de código de autorización](/es/docs/get-started/authentication-and-authorization-flow/authorization-code-flow). Si se aprueba la transacción, el navegador del usuario se redirige a tu aplicación con un código de autorización, que luego se intercambia por un <Tooltip tip="Token de acceso: Credencial de autorización, en forma de una cadena opaca o un JWT, utilizada para acceder a una API." cta="Ver glosario" href="/es/docs/glossary?term=access+token">token de acceso</Tooltip> cifrado mediante [JSON Web Encryption](/es/docs/secure/tokens/access-tokens/json-web-encryption). El token de acceso contiene los `authorization_details` que enviaste originalmente. El siguiente ejemplo de código muestra el contenido de un token de acceso descifrado:
