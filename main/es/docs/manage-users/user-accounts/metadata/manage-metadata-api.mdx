---
description: Aprende a crear, actualizar, fusionar y eliminar metadatos con la Management API.
...
Haga la siguiente llamada `POST` al <Tooltip tip="Management API: Un producto que permite a los clientes realizar tareas administrativas." cta="Ver glosario" href="/es/docs/glossary?term=Management+API">Management API</Tooltip> endpoint [`/post_users`](https://auth0.com/docs/api/management/v2#!/Users/post_users) para crear el usuario y establecer los valores de las propiedades:
...
Puede actualizar los metadatos de un usuario realizando una llamada `PATCH` al endpoint [`/patch_users_by_id`](https://auth0.com/docs/api/management/v2#!/Users/patch_users_by_id) de la Management API.

Supongamos que creó un usuario con los siguientes valores de metadatos:
...
Realizarías la siguiente llamada `PATCH`:
...
El perfil del usuario ahora se mostrará de la siguiente manera:
...
Los metadatos del cliente se pueden actualizar mediante el endpoint [`PATCH /api/v2/clients/{id}`](https://auth0.com/docs/api/management/v2#!/Users/patch_users_by_id), proporcionando un objeto de aplicación con la propiedad `clientMetadata property`, cuyo valor es un objeto que contiene los metadatos que desea cambiar.
...
Las claves de los metadatos del cliente pueden eliminarse mediante una solicitud PATCH, como se describe en &quot;Actualizar app&#95;metadata&quot; más arriba, pero asignando `null` al valor de la clave. Este comportamiento coincide con el de las propiedades `user_metadata` y `app_metadata` en el endpoint `PATCH` [/api/v2/users/{id}](https://auth0.com/docs/api/management/v2#!/Users/patch_users_by_id).
