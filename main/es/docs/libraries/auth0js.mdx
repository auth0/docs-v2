---
description: Cómo instalar, inicializar y usar auth0.js v9
title: Referencia de Auth0.js v9
---

import {AuthCodeBlock} from "/snippets/es/AuthCodeBlock.jsx";

Auth0.js es una biblioteca de JavaScript del lado del cliente para Auth0. Admite tanto el [inicio de sesión alojado como el inicio de sesión integrado](/es/docs/authenticate/login/universal-vs-embedded-login).

La documentación completa de la API de la biblioteca está [aquí](https://auth0.github.io/auth0.js/index.html).
...
El [directorio de ejemplo](https://github.com/auth0/auth0.js/tree/master/example) de la biblioteca auth0.js es una aplicación lista para usar que puede ayudarte a probar auth0.js de forma rápida y sencilla. Para ejecutarla, sigue estos pasos:
...
Ahora, comencemos a integrar auth0.js en tu proyecto. Veremos los [métodos de instalación](#installation-options), [cómo inicializar auth0.js](#initialization), [registro](#signup), [inicio de sesión](#login), [cierre de sesión](#logout) y mucho más.
...
Al implementar el inicio de sesión integrado, la biblioteca usará solicitudes entre orígenes dentro de iframes ocultos para realizar la autenticación. Para garantizar que esto pueda hacerse de forma segura, Auth0 necesita conocer los dominios donde alojarás tus aplicaciones.
...
Después de instalar el módulo `auth0-js`, tendrás que incluirlo en el paquete junto con todas sus dependencias o importarlo con:

```js lines
import auth0 from 'auth0-js';
```

Como alternativa, incluye el script desde nuestro CDN:
...
<Card title="Ejecutar Auth0.js de forma local">
...
Nuestra recomendación es usar Universal Login en lugar de configurar SSO en escenarios de inicio de sesión integrado. Universal Login es la forma más fiable y estable de realizar SSO, y es la única manera de hacerlo si debes usar varios dominios para tus aplicaciones o [aplicaciones de terceros](/es/docs/get-started/applications/third-party-applications/configure-third-party-applications).
...
Para usar la autenticación sin contraseña, deberás inicializar auth0.js con un `redirectUri` y establecer `responseType: 'token'`.
...
Para usar `passwordlessLogin`, las opciones `redirectUri` y `responseType` deben especificarse al inicializar WebAuth por primera vez.
...
| ------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `accessToken` | Un <Tooltip tip="Access Token: Credencial de autorización, en forma de una cadena opaca o JWT, utilizada para acceder a una API." cta="Ver glosario" href="/es/docs/glossary?term=Access+Token">Access Token</Tooltip> para la API especificada por `audience` |
...
Sin embargo, el navegador **debe** tener habilitadas las cookies de terceros. De lo contrario, **checkSession()** no podrá acceder a la sesión actual del usuario (lo que hace imposible obtener un nuevo token sin mostrar nada al usuario). Lo mismo ocurrirá si los usuarios tienen [la ITP de Safari habilitada](/es/docs/troubleshoot/authentication-issues/renew-tokens-when-using-safari).
...
El usuario recibirá entonces un correo electrónico con un enlace para restablecer su contraseña.
