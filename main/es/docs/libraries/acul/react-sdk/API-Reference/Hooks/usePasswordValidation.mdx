---
title: "usePasswordValidation"
---

```tsx
usePasswordValidation(
  password: string,
  options?: { includeInErrors?: boolean },
): PasswordValidationResult
```

<ParamField body="usePasswordValidation" type={<span><a href="/es/docs/libraries/acul/react-sdk/API-Reference/Types/interfaces/PasswordValidationResult">PasswordValidationResult</a></span>}>
  Este hook de React valida una contraseña según la política de contraseñas actual de Auth0
  y devuelve un resultado estructurado que indica si la contraseña cumple cada una de las reglas.

  Opcionalmente, puede enviar los resultados de la validación al administrador global de errores para que
  los componentes de error del formulario se actualicen automáticamente.

  ### Características principales

  * **Validación basada en políticas** — comprueba la contraseña según las reglas de la política de contraseñas de Auth0 configuradas para el inquilino.
  * **Integración con el administrador de errores** — opcionalmente muestra automáticamente los errores de validación en los componentes de error del formulario.

  ## Parámetros

  <ParamField body="password" type="string">
    La contraseña que se va a validar.
  </ParamField>

  <ParamField body="options" type="{ includeInErrors?: boolean }">
    Configuración opcional del hook.

    <Expandable title="propiedades">
      <ParamField body="includeInErrors" type="boolean">
        Si es `true`, los errores de validación se almacenan en el administrador global de errores en el campo `password`. El valor predeterminado es `false`.
      </ParamField>
    </Expandable>
  </ParamField>

  ## Devuelve

  [`PasswordValidationResult`](/es/docs/libraries/acul/react-sdk/API-Reference/Types/interfaces/PasswordValidationResult)

  Un objeto [PasswordValidationResult](/es/docs/libraries/acul/react-sdk/API-Reference/Types/interfaces/PasswordValidationResult) que contiene:

  * `isValid` — `true` si la contraseña cumple todas las reglas configuradas.
  * `results` — una lista de resultados por regla con `code`, `label`, `status` e `isValid`.

  ## Pantallas compatibles

  * `signup`
  * `signup-password`
  * `reset-password`

  ```tsx Example
  import { usePasswordValidation } from '@auth0/auth0-acul-react/signup';

  const { isValid, results} = usePasswordValidation(password, { includeInErrors: true });

  if (!isValid) {
    console.log(results);
  }
  ```

  ## Observaciones

  * Llame a `usePasswordValidation` en el nivel superior del componente; no lo llame de forma condicional ni dentro de controladores de eventos.
  * La ruta de importación debe coincidir con la pantalla; use `signup`, `signup-password` o `reset-password`, según corresponda.
</ParamField>