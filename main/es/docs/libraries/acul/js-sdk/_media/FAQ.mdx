...
A continuación, se incluyen varias preguntas o problemas que han surgido al usar el SDK.
...
1. **Método `getErrors` para errores específicos de la pantalla**
   Puede usar la función de nivel superior `getErrors` para recuperar errores de autenticación detallados cuando falla la validación del backend.
   ```javascript
      import Login from "@auth0/auth0-acul-js/login";
...
2. **Errores específicos de la pantalla**
   Cada pantalla incluye una propiedad `transaction.errors` que devuelve una lista de errores específicos de esa pantalla. Estos errores se generan después de la validación final de la solicitud, lo que le ayuda a identificar los problemas que requieren atención.

Al usar estos métodos, puede rastrear y resolver fácilmente los errores a lo largo del flujo del SDK.

<div id="can-i-use-this-sdk-from-a-cdn">
  ## ¿Puedo usar este SDK desde una CDN?
</div>

Actualmente, este SDK solo puede instalarse a través del registro de npm y no se puede usar directamente desde una CDN.

Para instalar el SDK, ejecute el siguiente comando:

```sh
npm install @auth0/auth0-acul-js
```

Una vez instalado, puede importarlo y usarlo en proyectos de JavaScript, TypeScript o ReactJS.

<div id="can-i-build-a-reactjs-application-with-this-sdk">
  ## ¿Puedo crear una aplicación de ReactJS con este SDK?
</div>

Sí, este SDK se puede integrar fácilmente en una aplicación de ReactJS.

Aunque el SDK está escrito en JavaScript puro, es totalmente compatible con todos los frameworks basados en JavaScript, incluidos React, Angular, Vue y otros. Puedes usar el SDK sin problemas dentro de tus componentes o hooks de React.