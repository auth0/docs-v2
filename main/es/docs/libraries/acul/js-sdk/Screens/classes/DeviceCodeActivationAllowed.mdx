...
description: "Describe todas las propiedades y los métodos disponibles para personalizar la pantalla `device-code-activation-allowed` de Universal Login."
---

La clase DeviceCodeActivationAllowed implementa la funcionalidad de la pantalla `device-code-activation-allowed`. Esta pantalla se muestra cuando la activación del código del dispositivo se ha autorizado correctamente.
...
Cree una instancia del administrador de la pantalla DeviceCodeActivationAllowed:
...
<ParamField body="client" type={<span><a href="/es/docs/libraries/acul/js-sdk/Screens/interfaces/ClientMembers">ClientMembers</a></span>}>
  Proporciona configuraciones del cliente, como `id`, `name` y `logoUrl`, para la pantalla `device-code-activation-allowed`.
</ParamField>
...
<ParamField body="untrustedData" type={<span><a href="/es/docs/libraries/acul/js-sdk/Screens/interfaces/UntrustedDataMembers">UntrustedDataMembers</a></span>}>
  Gestiona datos no confiables que se pasan al SDK, como la entrada del usuario durante el flujo de activación del código del dispositivo.
</ParamField>
