...
Interfaz que define los métodos y las propiedades disponibles de la pantalla `brute-force-protection-unblock`.

```ts Example
export interface BruteForceProtectionUnblockMembers extends BaseMembers {
  screen: ScreenMembers;
  /**
   * Desbloquea la cuenta del usuario después de que se activó la protección contra fuerza bruta.
...
<ParamField body="unblockAccount" type="Promise<void>">
  Desbloquea la cuenta del usuario una vez que se activa la protección contra fuerza bruta.
