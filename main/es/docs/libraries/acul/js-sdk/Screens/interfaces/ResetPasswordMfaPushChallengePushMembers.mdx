---
title: "ResetPasswordMfaPushChallengePushMembers"
---

Interfaz que define los métodos y propiedades disponibles para la pantalla reset-password-mfa-push-challenge-push

```ts Example
export interface ResetPasswordMfaPushChallengePushMembers extends BaseMembers {
  screen: ScreenMembersOnResetPasswordMfaPushChallengePush;

  /**
   * Continúa con el desafío de notificación push
   * @param payload Opciones personalizadas opcionales para incluir en la solicitud
   */
  continue(payload?: CustomOptions): Promise<void>;

  /**
   * Reenvía la notificación push
   * @param payload Opciones personalizadas opcionales para incluir en la solicitud
   */
  resendPushNotification(payload?: CustomOptions): Promise<void>;

  /**
   * Cambia a la introducción manual del código de verificación
   * @param payload Opciones personalizadas opcionales para incluir en la solicitud
   */
  enterCodeManually(payload?: CustomOptions): Promise<void>;

  /**
   * Permite probar otro método de autenticación
   * @param payload Opciones personalizadas opcionales para incluir en la solicitud
   */
  tryAnotherMethod(payload?: CustomOptions): Promise<void>;

  /**
   * Permite realizar polling para verificar si el desafío de notificación push ha sido aprobado.
   * @param options de tipo {@link MfaPollingOptions}
   */
  pollingManager(options?: MfaPollingOptions): MfaPushPollingControl;
}
```

<div id="properties">
  ## Propiedades
</div>

<ParamField body="branding" type={<span><a href="/es/docs/libraries/acul/js-sdk/Screens/interfaces/BrandingMembers">BrandingMembers</a></span>} />

<ParamField body="client" type={<span><a href="/es/docs/libraries/acul/js-sdk/Screens/interfaces/ClientMembers">ClientMembers</a></span>} />

<ParamField body="organization" type={<span><a href="/es/docs/libraries/acul/js-sdk/Screens/interfaces/OrganizationMembers">OrganizationMembers</a></span>} />

<ParamField body="prompt" type={<span><a href="/es/docs/libraries/acul/js-sdk/Screens/interfaces/PromptMembers">PromptMembers</a></span>} />

<ParamField body="screen" type={<span><a href="/es/docs/libraries/acul/js-sdk/Screens/interfaces/ScreenMembersOnResetPasswordMfaPushChallengePush">ScreenMembersOnResetPasswordMfaPushChallengePush</a></span>} />

<ParamField body="tenant" type={<span><a href="/es/docs/libraries/acul/js-sdk/Screens/interfaces/TenantMembers">TenantMembers</a></span>} />

<ParamField body="transaction" type={<span><a href="/es/docs/libraries/acul/js-sdk/Screens/interfaces/TransactionMembers">TransactionMembers</a></span>} />

<ParamField body="untrustedData" type={<span><a href="/es/docs/libraries/acul/js-sdk/Screens/interfaces/UntrustedDataMembers">UntrustedDataMembers</a></span>} />

<ParamField body="user" type={<span><a href="/es/docs/libraries/acul/js-sdk/Screens/interfaces/UserMembers">UserMembers</a></span>} />

<div id="methods">
  ## Métodos
</div>

<ParamField body="continue" type="Promise<void>">
  Continúa con el desafío de notificación push

  <Expandable title="Parámetros">
    <ParamField body="payload?" type={<span><a href="/es/docs/libraries/acul/js-sdk/Screens/interfaces/CustomOptions">CustomOptions</a></span>}>
      Opciones personalizadas opcionales para incluir en la solicitud
    </ParamField>
  </Expandable>
</ParamField>

<ParamField body="enterCodeManually" type="Promise<void>">
  Pasa a la introducción manual del código de verificación

  <Expandable title="Parámetros">
    <ParamField body="payload?" type={<span><a href="/es/docs/libraries/acul/js-sdk/Screens/interfaces/CustomOptions">CustomOptions</a></span>}>
      Opciones personalizadas opcionales para incluir en la solicitud
    </ParamField>
  </Expandable>
</ParamField>

<ParamField body="pollingManager" type={<span><a href="/es/docs/libraries/acul/js-sdk/Screens/interfaces/MfaPushPollingControl">MfaPushPollingControl</a></span>}>
  Permite hacer polling para saber si se ha aprobado el desafío de notificación push.

  <Expandable title="Parámetros">
    <ParamField body="options?" type={<span><a href="/es/docs/libraries/acul/js-sdk/Screens/type-aliases/MfaPollingOptions">MfaPollingOptions</a></span>} />
  </Expandable>
</ParamField>

<ParamField body="resendPushNotification" type="Promise<void>">
  Vuelve a enviar la notificación push

  <Expandable title="Parámetros">
    <ParamField body="payload?" type={<span><a href="/es/docs/libraries/acul/js-sdk/Screens/interfaces/CustomOptions">CustomOptions</a></span>}>
      Opciones personalizadas opcionales para incluir en la solicitud
    </ParamField>
  </Expandable>
</ParamField>

<ParamField body="tryAnotherMethod" type="Promise<void>">
  Permite probar otro método de autenticación

  <Expandable title="Parámetros">
    <ParamField body="payload?" type={<span><a href="/es/docs/libraries/acul/js-sdk/Screens/interfaces/CustomOptions">CustomOptions</a></span>}>
      Opciones personalizadas opcionales para incluir en la solicitud
    </ParamField>
  </Expandable>
</ParamField>