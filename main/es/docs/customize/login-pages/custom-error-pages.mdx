---
description: Aprende a crear una página de error personalizada para errores de autorización.
...
* Faltan parámetros obligatorios al llamar al [endpoint Login](https://auth0.com/docs/api/authentication#login) de la Authentication API de Auth0.
* El usuario abre un enlace de restablecimiento de contraseña caducado (cuando se usa la experiencia de Classic Login).
* El usuario abre una página de Login guardada en marcadores y no se especifica una [ruta de Login predeterminada](/es/docs/authenticate/login/auth0-universal-login/configure-default-login-routes).
...
| `connection`        | Conexión usada en el momento del error.                                       |
| `lang`              | Idioma configurado para usar en el momento del error.                         |
...
1. Vaya a [Auth0 Dashboard &gt; Tenant Settings](https://manage.auth0.com/#/tenant/).
...
Use el endpoint [Actualizar la configuración del tenant](https://auth0.com/docs/api/management/v2/#!/Tenants/patch_settings) de la <Tooltip tip="Management API: Un producto que permite a los clientes realizar tareas administrativas." cta="Ver glosario" href="/es/docs/glossary?term=Management+API">Management API</Tooltip>. Reemplace el valor del marcador de posición `{mgmtApiAccessToken}` por su <Tooltip tip="Token de acceso: Credencial de autorización, en forma de una cadena opaca o JWT, utilizada para acceder a una API." cta="Ver glosario" href="/es/docs/glossary?term=Access+Token">Token de acceso</Tooltip> de la Management API y actualice el valor del campo `url` en el cuerpo JSON para que apunte a la página de error.
...
| `show_log_link`         | Indica si se mostrará un enlace al error en los registros de su inquilino. Los valores válidos son `true` y `false`.             |
| `url`                   | Ubicación de la página de error personalizada a la que quiere redirigir.                                                         |
...
| ----------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| `MGMT_API_ACCESS_TOKEN` | [Token de acceso para la Management API](https://auth0.com/docs/api/management/v2/tokens) con el scope `update:tenant_settings`. |
| `show_log_link`         | Indica si se debe mostrar un enlace al error en los registros de tu inquilino. Los valores válidos son `true` y `false`.         |
