...
#
description: Obtenga información sobre el objeto de evento del trigger custom-token-exchange de Actions, que proporciona información contextual sobre la ejecución del trigger.
title: 'Triggers de Actions: custom-token-exchange - Objeto de evento'
validatedOn: 2026-05-22
---

El objeto `event` del trigger custom-token-exchange de Actions proporciona información contextual sobre la ejecución del trigger.
...
<ResponseField name="event.resource_server" type="object">
  Información sobre el servidor de recursos al que se solicita acceso.
...
<ResponseField name="event.transaction" type="dictionary">
  ...
  <Expandable title="propiedades de la transacción" defaultOpen>
    <ResponseField name="actor_token" type="string" post={["optional"]}>
      El token del actor proporcionado en la solicitud de intercambio de tokens.
    </ResponseField>

    <ResponseField name="actor_token_type" type="string" post={["optional"]}>
      El tipo del token del actor proporcionado en la solicitud de intercambio de tokens.
    </ResponseField>

    <ResponseField name="actor_token_user" type="dictionary" post={["optional"]}>
      El usuario representado por el token de actor. Solo estará presente si el actor&#95;token&#95;type es urn:ietf:params:oauth:token-type:id&#95;token y el token de actor proporcionado en la solicitud de intercambio de tokens es un ID Token válido generado por Auth0.

      <Expandable title="Propiedades de actor_token_user" defaultOpen>
        <ResponseField name="app_metadata" type="dictionary">
          Campos personalizados que almacenan información sobre un usuario e influyen en su acceso, como el plan de soporte, los roles de seguridad o los grupos de control de acceso.
        </ResponseField>
        ...
        <ResponseField name="phone_verified" type="boolean" post={["optional"]}>
          Indica si el usuario verificó su número de teléfono.
        </ResponseField>
        ...
        <ResponseField name="user_metadata" type="dictionary">
          Campos personalizados que almacenan información sobre un usuario que no afecta a los recursos a los que puede o no puede acceder, como la dirección del trabajo, la dirección de casa o las preferencias del usuario.
        </ResponseField>
        ...
        <ResponseField name="enrolledFactors" type="array of objects" post={["optional"]}>
          Un array de factores de autenticación en los que el usuario se ha inscrito.
        ...
        <ResponseField name="multifactor" type="array of strings" post={["optional"]}>
          Lista de proveedores de autenticación multifactor (MFA) en los que el usuario está inscrito. Este array se actualiza cuando el usuario se inscribe en MFA y cuando un administrador restablece las inscripciones de MFA de un usuario.
        </ResponseField>
    ...
    <ResponseField name="requested_scopes" type="array of strings">
      Los alcances solicitados (si corresponde) que se proporcionan en la solicitud de intercambio de tokens.
    </ResponseField>

    <ResponseField name="requested_token_type" type="string">
      El tipo de token que debe generar Auth0. Por ejemplo: urn:ietf:params:oauth:token-type:access&#95;token.
    </ResponseField>

    <ResponseField name="subject_token" type="string">
      El token del sujeto proporcionado en la solicitud de intercambio de tokens.
    </ResponseField>

    <ResponseField name="subject_token_type" type="string">
      El subject&#95;token&#95;type que se proporciona en la solicitud de intercambio de tokens.
    </ResponseField>
  </Expandable>
</ResponseField>