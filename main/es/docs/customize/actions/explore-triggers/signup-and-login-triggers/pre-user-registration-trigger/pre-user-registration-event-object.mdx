...
#
description: Conozca el objeto `event` del trigger `pre-user-registration` de Actions, que proporciona información contextual sobre la ejecución del trigger.
title: 'Triggers de Actions: pre-user-registration - Objeto event'
...
<ResponseField name="event.authentication" type="dictionary" post={["optional"]}>
  Detalles sobre la autenticación obtenida durante el flujo previo al registro del usuario.
  ...
  <Expandable title="propiedades de authentication" defaultOpen>
    <ResponseField name="riskAssessment" type="dictionary" post={["optional"]}>
      Detalles sobre las evaluaciones de riesgo para distintos flujos.

      <Expandable title="propiedades de riskAssessment" defaultOpen>
        <ResponseField name="supplemental" type="dictionary" post={["optional"]}>
          Señales complementarias enviadas por proveedores externos para ayudar con las evaluaciones de riesgo.
                ...
                <ResponseField name="akamaiBot" type="object" post={["optional"]}>
                  ...
                  <Expandable title="propiedades de akamaiBot">
                    ...
                    <ResponseField name="action" type="string" post={["optional"]}>
                      La acción indicada en los resultados de Akamai Bot Manager.
                    </ResponseField>

                    <ResponseField name="botCategory" type="array of strings" post={["optional"]}>
                      La categoría de bot en los resultados de Akamai Bot Manager.
                    </ResponseField>

                    <ResponseField name="botScore" type="number" post={["optional"]}>
                      La puntuación de bot en los resultados de Akamai Bot Manager.
                    </ResponseField>

                    <ResponseField name="botScoreResponseSegment" type="string" post={["optional"]}>
                      El segmento de respuesta de la puntuación de bot en los resultados de Akamai Bot Manager.
                    </ResponseField>

                    <ResponseField name="botnetId" type="string" post={["optional"]}>
                      El ID de botnet en los resultados de Akamai Bot Manager.
                    </ResponseField>
                  </Expandable>
                </ResponseField>
...
<ResponseField name="event.connection" type="dictionary">
  Detalles sobre la Conexión que se utilizó para registrar al usuario.
...
<ResponseField name="event.request" type="dictionary">
  ...
  <Expandable title="propiedades de la solicitud" defaultOpen>
    <ResponseField name="body" type="dictionary">
      El cuerpo de la solicitud POST. Estos datos solo estarán disponibles durante los flujos de Token de actualización, Client Credential Exchange y en la Action PreUserRegistration.
    </ResponseField>
    ...
    <ResponseField name="method" type="string">
      El método HTTP usado para la solicitud
    </ResponseField>
...
<ResponseField name="event.security_context" type="object" post={["optional"]}>
  Un objeto que contiene firmas de huellas digitales. Solo estará disponible si el cliente usa Cloudflare. La huella digital JA3/JA4 puede ser nula o estar vacía en algunos casos. El caso más común se da en las solicitudes HTTP, porque JA3 y JA4 se calculan en TLS. También puede estar vacía porque el Worker envía solicitudes dentro de la misma zona o a una zona que no pasa por proxy (o a un tercero).
...
<ResponseField name="event.transaction" type="dictionary" post={["optional"]}>
  ...
  <Expandable title="propiedades de la transacción" defaultOpen>
    <ResponseField name="acr_values" type="array of strings">
      Los valores de acr&#95;values proporcionados en la solicitud de autenticación original.
    </ResponseField>
    ...
    <ResponseField name="prompt" type="array of strings" post={["optional"]}>
      Lista de instrucciones que indica si se le puede solicitar al usuario reautenticación y consentimiento.
    </ResponseField>
    ...
    <ResponseField name="requested_scopes" type="array of strings">
      Los alcances solicitados, si los hay, al iniciar este flujo de autenticación.
    </ResponseField>
...
<ResponseField name="event.user" type="dictionary">
  Un objeto que describe al usuario que está intentando registrarse.
