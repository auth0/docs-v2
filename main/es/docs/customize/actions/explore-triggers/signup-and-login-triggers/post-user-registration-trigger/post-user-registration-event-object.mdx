...
#
description: Obtenga información sobre el objeto event del desencadenador post-user-registration de Actions, que proporciona información contextual sobre la ejecución del desencadenador.
title: 'Desencadenadores de Actions: post-user-registration - objeto Event'
validatedOn: 2026-05-25
---

El objeto `event` del desencadenador post-user-registration de Actions proporciona información contextual sobre la ejecución del desencadenador.
...
<ResponseField name="event.authentication" type="dictionary" post={["optional"]}>
  Detalles sobre señales de autenticación adicionales obtenidas durante el flujo de registro.
    ...
    <ResponseField name="riskAssessment" type="dictionary" post={["optional"]}>
      ...
      <Expandable title="propiedades de riskAssessment" defaultOpen>
        <ResponseField name="supplemental" type="dictionary" post={["optional"]}>
          Señales adicionales enviadas por proveedores externos para ayudar en las evaluaciones de riesgo.

          <Expandable title="propiedades de supplemental">
            <ResponseField name="akamai" type="dictionary" post={["optional"]}>
              [Acceso anticipado] Evaluación de riesgo adicional. Solo está disponible si Akamai Account Protector está habilitado y Akamai reenvía los encabezados de la transacción.
...
<ResponseField name="event.connection" type="dictionary">
  Detalles sobre la Conexión utilizada para registrar al usuario.
...
<ResponseField name="event.security_context" type="object" post={["optional"]}>
  Un objeto que contiene firmas de huella digital. Esto solo estará disponible si el cliente usa Cloudflare. La huella digital JA3/JA4 puede ser nula o estar vacía en algunos casos. El caso más común es el de las solicitudes HTTP, ya que JA3 y JA4 se calculan en TLS. También puede estar vacía porque el Worker envía solicitudes dentro de la misma zona o a una zona que no está detrás de un proxy (o a un tercero).
...
<ResponseField name="event.transaction" type="dictionary" post={["optional"]}>
  ...
  <Expandable title="propiedades de la transacción" defaultOpen>
    ...
    <ResponseField name="prompt" type="array of strings" post={["optional"]}>
      Lista de instrucciones que indican si se le puede solicitar al usuario que vuelva a autenticarse y otorgue consentimiento.
    </ResponseField>
    ...
    <ResponseField name="requested_scopes" type="array of strings">
      Los alcances solicitados (si corresponde) al iniciar este flujo de autenticación.
    </ResponseField>
