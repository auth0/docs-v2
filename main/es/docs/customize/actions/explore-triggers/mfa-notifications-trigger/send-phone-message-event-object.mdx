...
#
description: Obtén información sobre el objeto `event` del desencadenador de Action send-phone-message, que proporciona información contextual sobre la ejecución del desencadenador.
title: 'Desencadenadores de Actions: send-phone-message - Objeto `event`'
validatedOn: 2026-04-28
---

El objeto `event` del desencadenador de Actions send-phone-message proporciona información contextual sobre la ejecución del desencadenador.
...
<ResponseField name="event.message_options" type="object">
  ...
  <Expandable title="propiedades de message_options" defaultOpen>
    <ResponseField name="action" type="string">
      El flujo que desencadenó esta acción.
    ...
    <ResponseField name="message_type" type="string">
      Cómo se entregará el mensaje, ya sea por &#39;sms&#39; o &#39;voice&#39;.
...
<ResponseField name="event.request" type="dictionary">
  ...
  <Expandable title="propiedades de la solicitud" defaultOpen>
    ...
    <ResponseField name="hostname" type="string" post={["optional"]}>
      El nombre de host que se utiliza en el flujo de autenticación.
    </ResponseField>
...
<ResponseField name="event.security_context" type="object" post={["optional"]}>
  Un objeto que contiene firmas de huella digital. Esto solo estará disponible si el cliente usa Cloudflare. La huella digital JA3/JA4 puede ser nula o estar vacía en algunos casos. El caso más común es en las solicitudes HTTP, porque JA3 y JA4 se calculan en TLS. También puede estar vacía porque el Worker envía solicitudes dentro de la misma zona o a una zona que no pasa por el proxy (o a un tercero).
...
<ResponseField name="event.transaction" type="object" post={["optional"]}>
  ...
  <Expandable title="propiedades de la transacción" defaultOpen>
    <ResponseField name="correlation_id" type="string" post={["optional"]}>
      El ID de correlación se puede proporcionar en la solicitud inicial de autenticación cuando la aplicación redirige a Universal Login. Puede usar este valor para correlacionar logs y solicitudes de su código de Action con el flujo del usuario.
    </ResponseField>
  </Expandable>
</ResponseField>
...
<ResponseField name="event.user" type="dictionary">
  ...
  <Expandable title="propiedades del usuario" defaultOpen>
    ...
    <ResponseField name="created_at" type="string">
      Marca de tiempo que indica cuándo se creó por primera vez el perfil de usuario.
    </ResponseField>
    ...
    <ResponseField name="picture" type="string" post={["optional"]}>
      URL que apunta a la [imagen de perfil del usuario](https://auth0.com/docs/users/change-user-picture).
    </ResponseField>
    ...
    <ResponseField name="identities" type="array of objects" post={["optional"]}>
      Contiene información obtenida del proveedor de identidad con el que el usuario se autentica originalmente. Los usuarios también pueden vincular su perfil a varios proveedores de identidad; esas identidades también aparecerán en este arreglo. El contenido de un objeto de proveedor de identidad individual varía según el proveedor.
