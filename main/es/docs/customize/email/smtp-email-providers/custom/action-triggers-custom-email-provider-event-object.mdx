...
#
description: Conozca el objeto de evento del desencadenador de Actions custom-email-provider, que proporciona información contextual sobre la ejecución de este desencadenador.
title: 'Desencadenadores de Actions: custom-email-provider - Objeto de evento'
validatedOn: 2026-04-28
---

El objeto `event` del desencadenador de Actions custom-email-provider proporciona información contextual sobre la ejecución de este desencadenador.
...
<ResponseField name="event.connection" type="dictionary" post={["optional"]}>
  ...
  <Expandable title="propiedades de la conexión" defaultOpen>
    ...
    <ResponseField name="metadata" type="dictionary" post={["optional"]}>
      Metadatos asociados con la conexión.
    </ResponseField>

    <ResponseField name="name" type="string">
      El nombre de la conexión que se usó para autenticar al usuario (como `twitter` o `some-g-suite-domain`).
    </ResponseField>
...
<ResponseField name="event.notification" type="object">
  <Expandable title="propiedades de la notificación" defaultOpen>
    ...
    <ResponseField name="html" type="string">
      Plantilla HTML generada.
    </ResponseField>

    <ResponseField name="locale" type="string">
      La configuración regional en la que se generó el mensaje; por ejemplo, `en_US`, tal como se define en la especificación BCP-47.
    </ResponseField>

    <ResponseField name="message_type" type="string">
      El tipo de mensaje que se está enviando, como `verify_email` o `welcome_email`.
...
<ResponseField name="event.secrets" type="secrets">
  Valores de secretos asociados de forma segura a esta Action.
</ResponseField>
...
<ResponseField name="event.tenant" type="object">
  <Expandable title="propiedades del inquilino" defaultOpen>
    ...
    <ResponseField name="home_url" type="string" post={["optional"]}>
      La URL de inicio del inquilino, si está definida y figura en su configuración.
    </ResponseField>
...
<ResponseField name="event.transaction" type="object" post={["optional"]}>
  ...
  <Expandable title="propiedades de la transacción" defaultOpen>
    <ResponseField name="correlation_id" type="string" post={["optional"]}>
      Un identificador único para correlacionar esta solicitud en varios servicios para la trazabilidad distribuida.
    </ResponseField>
  </Expandable>
</ResponseField>
