---
description: Aprende a configurar los SDK de Auth0 para que funcionen con varios dominios personalizados.
title: Configuración de SDK para varios dominios personalizados
---

import {AuthCodeBlock} from "/snippets/es/AuthCodeBlock.jsx";
import {AuthCodeGroup} from "/snippets/es/AuthCodeGroup.jsx";

Al usar [varios dominios personalizados](/es/docs/customize/custom-domains/multiple-custom-domains), debe configurar sus SDK de Auth0 para usar el dominio personalizado adecuado para la autenticación. Esta guía abarca la configuración de los SDK para distintas plataformas y escenarios.
...
Todos los SDK de Auth0 requieren un parámetro `domain` que especifica qué dominio de Auth0 se usará para la autenticación. Si usa dominios personalizados, establezca este parámetro en su dominio personalizado en lugar del dominio canónico de Auth0.
...
Para aplicaciones de página única que usan el [SDK SPA de Auth0](https://github.com/auth0/auth0-spa-js):
...
<AuthCodeGroup>
  ...
  function getAuth0Domain() {
    ...
    // Asocia los dominios de la aplicación con dominios personalizados de Auth0
...
Para aplicaciones de Next.js que usan el [SDK de Auth0 para Next.js](https://github.com/auth0/nextjs-auth0) (v4+):
...
<AuthCodeGroup>
  ...
  const auth0 = new Auth0Client({
    domain: ({ headers }) => {
      // Asigna los hosts de la aplicación a dominios personalizados de Auth0
  ...
  // Asigna identificadores de Tenant a dominios personalizados (desde una base de datos, caché, etc.)
  async function resolveTenantDomain(host: string): Promise<string> {
    // Extrae el identificador del Tenant del subdominio
    const subdomain = host.split('.')[0];

    // Busca el dominio personalizado de este Tenant (por ejemplo, en una base de datos)
  ...
  # Configuración de Auth0 (un solo Tenant con varios dominios personalizados)
...
Para aplicaciones React que usan el [SDK de React de Auth0](https://github.com/auth0/auth0-react):
...
Para aplicaciones Node.js que usan [express-openid-connect](https://github.com/auth0/express-openid-connect):
...
Uso de [Auth0.Android](https://github.com/auth0/Auth0.Android):
...
Los SDK de administración se utilizan para interactuar con la Management API de Auth0. Al usar dominios personalizados, es posible que deba incluir el encabezado `auth0-custom-domain` o usar el [dominio predeterminado](/es/docs/customize/custom-domains/multiple-custom-domains/default-domain).
...
<table class="table">
  ...
  <tbody>
    <tr>
      ...
      <td>La validación del token espera el dominio canónico, pero recibe un dominio personalizado</td>
      <td>Actualiza la validación del token para que acepte el dominio personalizado como emisor</td>
    </tr>

    <tr>
      <td>Error al obtener el JWKS</td>
      <td>El URI del JWKS apunta al dominio canónico</td>
      <td>Actualiza el URI del JWKS para usar el dominio personalizado: `https://custom-domain/.well-known/jwks.json`</td>
    </tr>

    <tr>
      <td>Incompatibilidad del URI de redirección</td>
      <td>La URL de callback no coincide con los URI de redirección configurados</td>
      <td>Agrega la URL de callback del dominio personalizado a la configuración de la aplicación</td>
    </tr>
    ...
    <tr>
      ...
      <td>Agrega el parámetro `configurationBaseUrl` con la URL regional del CDN</td>
    </tr>
  </tbody>
</table>
...
* [SDKs de Auth0](/es/docs/troubleshoot/customer-support/product-support-matrix#auth0-sdks)