...
Voici quelques questions ou problèmes fréquemment rencontrés lors de l’utilisation du SDK.
...
<details>
  <summary>Import de tous les autres écrans</summary>
...
1. **Méthode `getErrors` pour les erreurs propres à l’écran**
   Une fonction d’accès de niveau supérieur, `getErrors`, peut être utilisée pour récupérer des erreurs d’authentification détaillées lorsque la validation côté serveur échoue.
   ```javascript
      import Login from "@auth0/auth0-acul-js/login";
...
<div id="can-i-use-this-sdk-from-a-cdn">
  ## Puis-je utiliser ce SDK depuis un CDN ?
</div>

Actuellement, ce SDK est uniquement offert par l’intermédiaire du registre npm et ne peut pas être utilisé directement depuis un CDN.
...
Oui, ce SDK s’intègre facilement dans une application ReactJS.
