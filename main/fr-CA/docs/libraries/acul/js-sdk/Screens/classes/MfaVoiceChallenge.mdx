...
description: "Décrit toutes les propriétés et méthodes disponibles pour personnaliser l’écran `mfa-voice-challenge` dans Universal Login."
---
...
<ParamField body="continue" type="Promise<void>">
  ...
  **Paramètres de la méthode**
  ...
  <Expandable title="Paramètres">
    <ParamField body="options">
      [MfaVoiceChallengeContinueOptions](/fr-CA/docs/libraries/acul/js-sdk/Screens/interfaces/MfaVoiceChallengeContinueOptions).
    </ParamField>
    ...
    <ParamField body="rememberDevice?" type="boolean">
      Lorsque `true`, le navigateur est mémorisé pour les futurs défis MFA. S’applique uniquement si `screen.showRememberDevice` est `true`.
    </ParamField>
  </Expandable>
</ParamField>
  ...
  **Paramètres de la méthode**
  ...
  <Expandable title="Paramètres">
    ...
    <ParamField body="onTimeout?" type="() => void">
      Fonction de rappel invoquée lorsque le délai d’expiration du renvoi est écoulé et que l’option de renvoi devient disponible.
    </ParamField>

    <ParamField body="timeoutSeconds?" type="number">
      Le nombre de secondes avant que l’option de renvoi ne devienne disponible.
    </ParamField>
  </Expandable>
</ParamField>
