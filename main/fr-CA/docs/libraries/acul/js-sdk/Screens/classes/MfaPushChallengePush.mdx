...
description: "Décrit toutes les propriétés et méthodes disponibles pour personnaliser l’écran `mfa-push-challenge-push` dans Universal Login."
---
...
<ParamField body="pollingManager" type={<span><a href="/fr-CA/docs/libraries/acul/js-sdk/Screens/interfaces/MfaPushPollingControl">MfaPushPollingControl</a></span>}>
  Cette méthode lance et gère le polling d’un défi MFA push.

  Elle crée une session de polling qui vérifie de façon répétée le point de terminaison du défi MFA push à l’intervalle spécifié jusqu’à ce que la demande soit approuvée ou qu’une erreur survienne. Lorsque la condition d’approbation est remplie, la fonction de rappel [onCompleted](/fr-CA/docs/libraries/acul/js-sdk/Screens/type-aliases/MfaPollingOptions#oncompleted) fournie est appelée et le polling s’arrête automatiquement.

  Utilisez le [MfaPushPollingControl](/fr-CA/docs/libraries/acul/js-sdk/Screens/interfaces/MfaPushPollingControl) renvoyé pour démarrer, arrêter ou vérifier l’état du processus de polling à tout moment.
  ...
  **Paramètres de la méthode**
  ...
  <Expandable title="Paramètres">
    ...
    <ParamField body="intervalMs?" type="number">
      Intervalle facultatif, en millisecondes, entre les requêtes de polling consécutives. S’il est omis, l’intervalle interne par défaut du SDK est utilisé (généralement 5000 ms).
    </ParamField>

    <ParamField body="onCompleted()?" type="() => void">
      Fonction de rappel facultative exécutée une fois que le défi MFA push a été approuvé avec succès et que le polling est terminé. Elle est appelée exactement une fois, après quoi le polling s’arrête automatiquement.
    </ParamField>

    <ParamField body="onError()?" type="(error) => void">
      Fonction de rappel facultative appelée si une erreur survient pendant le polling. Reçoit un objet `ULError` contenant `status` (code d’état HTTP) et `responseText` (corps brut de la réponse).
    </ParamField>
  </Expandable>
</ParamField>
