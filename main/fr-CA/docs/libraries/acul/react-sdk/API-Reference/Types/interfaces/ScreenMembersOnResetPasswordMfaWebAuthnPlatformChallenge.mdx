...
export interface ScreenMembersOnResetPasswordMfaWebAuthnPlatformChallenge extends ScreenMembers {
  /**
   * Accesseur pratique pour les options de demande d'identifiants de clé publique (défi) nécessaires pour WebAuthn.
   ...
   * Dérivé de `screen.data.show_remember_device`. La valeur par défaut est `false` si non spécifiée.
   * @type {boolean}
   */
