...
description: "Décrit tous les hooks et toutes les méthodes disponibles pour personnaliser l’écran `mfa-voice-enrollment` d’Universal Login."
---

L’écran `mfa-voice-enrollment` permet à l’utilisateur d’inscrire un numéro de téléphone pour la MFA par appel vocal en le saisissant.
...
* L’utilisation de l’importation racine vous permet de charger tous les écrans à partir d’un seul bundle, ce qui est utile si vous voulez qu’une build unifiée prenne en charge tous les écrans possibles.
...
Hooks propres à l’écran qui fournissent un accès en lecture seule aux données de contexte d’Auth0 sur l’écran `mfa-voice-enrollment`. Importez-les depuis `@auth0/auth0-acul-react/mfa-voice-enrollment`.
...
<ParamField body="useClient" type={<span>() =&gt; <a href="/fr-CA/docs/libraries/acul/react-sdk/API-Reference/Types/interfaces/ClientMembers">ClientMembers</a></span>}>
  Ce hook fournit les configurations liées à l’application, comme `id`, `name` et `logoUrl`, pour l’écran `mfa-voice-enrollment`.
...
<ParamField body="useTransaction" type={<span>() =&gt; <a href="/fr-CA/docs/libraries/acul/react-sdk/API-Reference/Types/interfaces/TransactionMembers">TransactionMembers</a></span>}>
  Ce hook fournit des données propres à la transaction pour l’écran `mfa-voice-enrollment`, comme l’état actuel du flux MFA.
...
<ParamField body="useMfaVoiceEnrollment" type={<a href="/fr-CA/docs/libraries/acul/react-sdk/API-Reference/Types/interfaces/MfaVoiceEnrollmentMembers">MfaVoiceEnrollmentMembers</a>}>
  Ce hook renvoie toutes les méthodes et le contexte disponibles pour l’écran `mfa-voice-enrollment`.
</ParamField>
...
<ParamField body="continueMethod" type="Promise<void>">
  Cette méthode soumet le numéro de téléphone de l’utilisateur afin de finaliser l’inscription MFA par appel vocal.
  ...
  function EnrollPhoneForm() {
    ...
    return (
      <button onClick={() => continueMethod({ phone: '+11234567890' })}>
        Inscrire un numéro de téléphone
      </button>
    );
  }
  ```
  ...
  **Paramètres de la méthode**
  ...
  <Expandable title="Paramètres">
    <ParamField body="options">
      [MfaVoiceEnrollmentContinueOptions](/fr-CA/docs/libraries/acul/react-sdk/API-Reference/Types/interfaces/MfaVoiceEnrollmentContinueOptions).
    </ParamField>
...
<ParamField body="selectPhoneCountryCode" type="Promise<void>">
  Cette méthode ouvre l’écran de sélection du code de pays.
