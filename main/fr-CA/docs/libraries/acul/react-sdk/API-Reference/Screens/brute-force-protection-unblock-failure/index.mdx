...
description: "Décrit tous les hooks et les méthodes disponibles pour personnaliser l’écran `brute-force-protection-unblock-failure` d’Universal Login."
---

L’écran `brute-force-protection-unblock-failure` s’affiche lorsqu’une tentative de déblocage du compte d’un utilisateur échoue, afin d’informer l’utilisateur que la demande de déblocage n’a pas pu être traitée.

<Frame>
  <img style={{maxHeight:"400px"}} src="/docs/images/cdy7uua7fh8z/ACUL/brute-force-protection-unblock-failure.png" alt="ACUL Échec du déblocage de la protection contre les attaques par force brute" />
</Frame>
...
Hooks associés à l’écran qui fournissent un accès en lecture seule aux données de contexte d’Auth0 sur l’écran `brute-force-protection-unblock-failure`. Importez-les depuis `@auth0/auth0-acul-react/brute-force-protection-unblock-failure`.
...
<ParamField body="useBranding" type={<span>() =&gt; <a href="/fr-CA/docs/libraries/acul/react-sdk/API-Reference/Types/interfaces/BrandingMembers">BrandingMembers</a></span>}>
  Ce hook fournit les paramètres d’image de marque, comme le logo, les couleurs et les réglages du thème affichés sur l’écran `brute-force-protection-unblock-failure`.
...
<ParamField body="useClient" type={<span>() =&gt; <a href="/fr-CA/docs/libraries/acul/react-sdk/API-Reference/Types/interfaces/ClientMembers">ClientMembers</a></span>}>
  Ce hook fournit les configurations liées à l’application, comme `id`, `name` et `logoUrl`, pour l’écran `brute-force-protection-unblock-failure`.
...
<ParamField body="useOrganization" type={<span>() =&gt; <a href="/fr-CA/docs/libraries/acul/react-sdk/API-Reference/Types/interfaces/OrganizationMembers">OrganizationMembers</a></span>}>
  Ce hook fournit des informations sur l’Organisation de l’utilisateur si le flux de déblocage est associé à une Organisation. Renvoie `null` lorsqu’aucun contexte d’Organisation n’est disponible.
...
<ParamField body="useScreen" type={<span>() =&gt; <a href="/fr-CA/docs/libraries/acul/react-sdk/API-Reference/Types/interfaces/ScreenMembersOnBruteForceProtectionUnblockFailure">ScreenMembersOnBruteForceProtectionUnblockFailure</a></span>}>
  Ce hook contient des détails propres à l’écran `brute-force-protection-unblock-failure`, notamment sa configuration et son contexte.
...
<ParamField body="useUntrustedData" type={<span>() =&gt; <a href="/fr-CA/docs/libraries/acul/react-sdk/API-Reference/Types/interfaces/UntrustedDataMembers">UntrustedDataMembers</a></span>}>
  Ce hook gère les données non fiables transmises à l’écran, par exemple un `username` prérempli à partir des paramètres d’URL.
...
<ParamField body="useBruteForceProtectionUnblockFailure" type={<a href="/fr-CA/docs/libraries/acul/react-sdk/API-Reference/Types/interfaces/BruteForceProtectionUnblockFailureMembers">BruteForceProtectionUnblockFailureMembers</a>}>
  Ce hook renvoie toutes les méthodes et tout le contexte disponibles pour l’écran `brute-force-protection-unblock-failure`.
</ParamField>
...
<ParamField body={<a href="/fr-CA/docs/libraries/acul/react-sdk/API-Reference/Hooks/useAuth0Themes">useAuth0Themes</a>} type="Hooks">
  Ce hook récupère les options de thème actuelles avec la configuration aplatie issue du contexte d’image de marque.
</ParamField>
