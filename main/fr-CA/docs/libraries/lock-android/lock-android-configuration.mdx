...
Les options de configuration sont ajoutées au générateur Lock selon le format suivant :
...
* **withAudience(String)** : Modifie l’<Tooltip tip="Audience : identifiant unique de l’audience d’un jeton émis. Dans un jeton, elle est désignée par aud; sa valeur contient l’ID soit d’une application (ID client) pour un ID Token, soit d’une API (identifiant d’API) pour un Jeton d’accès." cta="Voir le glossaire" href="/fr-CA/docs/glossary?term=audience">audience</Tooltip> demandée lors d’une requête d’authentification.
...
* **withSignUpFields(List)**: Affiche un deuxième écran avec des champs supplémentaires à remplir après avoir saisi le nom d’utilisateur ou le courriel ainsi que le mot de passe dans l’écran d’inscription. Les valeurs soumises de cette façon peuvent être stockées dans le profil de l’utilisateur à l’aide d’un attribut racine ou de l’attribut `user_metadata`. Pour en savoir plus, consultez [Lock Android: Champs personnalisés à l’inscription](/fr-CA/docs/libraries/lock-android/lock-android-custom-fields-at-signup).
...
* **withAuthStyle(String, int)**: Personnalisez l’apparence d’une connexion donnée (nom) à l’aide d’un style précis. Consultez [ce document sur les connexions OAuth personnalisées](/fr-CA/docs/libraries/lock-android/lock-android-custom-theming) pour en savoir plus.
* **withConnectionScope(String, String...)**: Permet de spécifier des scopes supplémentaires pour un nom de connexion donné, qui seront demandés avec ceux définis dans les paramètres de la connexion dans [Auth0 Dashboard](https://manage.auth0.com/#). Les scopes ne sont validés d’aucune manière et doivent être reconnus par le fournisseur d’authentification indiqué. Pour en voir la liste, consultez [Auth0 Dashboard](https://manage.auth0.com/#) dans les paramètres de la connexion en question.
...
* **useCode()** : Envoie un code au lieu d’un lien par courriel/SMS pour l’authentification <Tooltip tip="Passwordless : forme d’authentification qui ne repose pas sur un mot de passe comme premier facteur." cta="Voir le glossaire" href="/fr-CA/docs/glossary?term=Passwordless">Passwordless</Tooltip>.
