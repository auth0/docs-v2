---
description: Décrit comment créer vos propres flux MFA à l’aide de notification push comme facteur d’authentification.
title: Inscrire des authentificateurs par notification push et lancer des demandes de vérification
---
...
Auth0 fournit un flux intégré d’inscription à la <Tooltip tip="Authentification multifacteur (MFA) : processus d’authentification qui utilise un facteur supplémentaire en plus du nom d’utilisateur et du mot de passe, comme un code envoyé par SMS." cta="Voir le glossaire" href="/fr-CA/docs/glossary?term=MFA">MFA</Tooltip> et d’authentification avec [Universal Login](/fr-CA/docs/authenticate/login/auth0-universal-login). Toutefois, si vous souhaitez créer votre propre interface utilisateur, vous pouvez utiliser l’[API MFA](/fr-CA/docs/secure/multi-factor-authentication/multi-factor-authentication-developer-resources/mfa-api) pour le faire.

Vous pouvez inscrire les utilisateurs et leur envoyer des demandes de vérification par notifications push avec l’application Guardian ou le SDK à l’aide de l’API MFA.

<div id="prerequisites">
  ## Prérequis
</div>

Avant de pouvoir utiliser les API de MFA, vous devez activer le type d’autorisation MFA pour votre application. Accédez à [Auth0 Dashboard &gt; Applications &gt; Advanced Settings &gt; Grant Types](https://manage.auth0.com/#/applications) et sélectionnez **MFA**.

* [Configurer la notification push](/fr-CA/docs/secure/multi-factor-authentication/multi-factor-authentication-factors/configure-push-notifications-for-mfa) comme facteur dans le Dashboard ou à l’aide de la [Management API](https://auth0.com/docs/api/management/v2#!/Guardian/put_factors_by_name).
...
Si vous obtenez l’erreur `User is already enrolled`, c’est que l’utilisateur a déjà un facteur MFA inscrit. Avant d’associer un autre facteur à l’utilisateur, vous devez d’abord lui demander de s’authentifier avec le facteur existant.
...
Pour confirmer l’inscription, l’utilisateur final devra scanner un code QR à l’aide du `barcode_uri` dans l’application Guardian, au cours des 5 prochaines minutes.

Une fois cela fait, l’application Guardian informera Auth0 que l’utilisateur s’est inscrit avec succès. Pour vérifier si cela s’est produit, interrogez le point de terminaison de jeton d’Auth0 <Tooltip tip="Point de terminaison de jeton : point de terminaison sur le serveur d’autorisation utilisé pour demander des jetons par programmation." cta="Voir le glossaire" href="/fr-CA/docs/glossary?term=Token+endpoint">point de terminaison de jeton</Tooltip> avec le `oob_code` renvoyé par l’appel au point de terminaison MFA Associate.
...
À ce stade, l’authentificateur est correctement associé et prêt à être utilisé, et vous disposez des jetons d’authentification de l’utilisateur.
...
Obtenez un jeton MFA en suivant les étapes décrites dans [S’authentifier avec le flux Resource Owner Password Grant et l’authentification MFA](/fr-CA/docs/secure/multi-factor-authentication/authenticate-using-ropg-flow-with-mfa).

<div id="retrieve-enrolled-authenticators">
  ### Récupérer les authentificateurs inscrits
</div>

Pour soumettre l’utilisateur à une demande de vérification, vous avez besoin de l’`authenticator_id` du facteur que vous voulez vérifier. Vous pouvez lister tous les authentificateurs inscrits à l’aide du point de terminaison MFA Authenticators :
...
Lorsque les utilisateurs s’inscrivent avec push, ils sont également inscrits à l’OTP, puisque Guardian prend en charge [la vérification par OTP](/fr-CA/docs/secure/multi-factor-authentication/authenticate-using-ropg-flow-with-mfa/enroll-and-challenge-otp-authenticators) dans les cas où l’utilisateur n’a pas de connexion.
...
Votre application doit interroger régulièrement le point de terminaison OAuth0 Token jusqu’à ce que l’utilisateur accepte la notification push.
...
La MFA personnalisable permet aux utilisateurs de s’inscrire et de faire des demandes de vérification au moyen des facteurs de leur choix pris en charge par votre application.

Lors de l’authentification au point de terminaison `oauth/token`, la réponse renvoie l’erreur `mfa_required`, qui inclut le `mfa_token` à utiliser avec l’API MFA, ainsi que le paramètre `mfa_requirements`, qui contient une liste d’authentificateurs :
...
Forcez la demande de vérification MFA en appelant le point de terminaison [`request/mfa/challenge`](/fr-CA/docs/api/authentication/muti-factor-authentication/request-mfa-challenge).
...
* [Lancer une demande de vérification avec des codes de récupération](/fr-CA/docs/secure/multi-factor-authentication/authenticate-using-ropg-flow-with-mfa/challenge-with-recovery-codes)
* [Inscrire les authentificateurs par courriel et lancer des demandes de vérification](/fr-CA/docs/secure/multi-factor-authentication/authenticate-using-ropg-flow-with-mfa/enroll-and-challenge-email-authenticators)
* [Inscrire les authentificateurs OTP et lancer des demandes de vérification](/fr-CA/docs/secure/multi-factor-authentication/authenticate-using-ropg-flow-with-mfa/enroll-and-challenge-otp-authenticators)
* [Inscrire les authentificateurs par SMS et appel vocal et lancer des demandes de vérification](/fr-CA/docs/secure/multi-factor-authentication/authenticate-using-ropg-flow-with-mfa/enroll-challenge-sms-voice-authenticators)