...
Le SDK Guardian fournit un client sans interface utilisateur graphique pour Guardian.
...
L’utilisation de `requestToken` ou de `ticket` dépend de la méthode d’authentification. `ticket` correspond à un [ticket d’inscription](/fr-CA/docs/secure/multi-factor-authentication/multi-factor-authentication-developer-resources/create-custom-enrollment-tickets) généré précédemment.

<div id="use-a-custom-domain">
  ### Utiliser un domaine personnalisé
</div>

Si un <Tooltip tip="Domaine personnalisé : domaine tiers avec un nom spécialisé ou personnalisé." cta="Voir le glossaire" href="/fr-CA/docs/glossary?term=custom+domain">domaine personnalisé</Tooltip> est configuré pour votre locataire Auth0, vous devrez mettre à jour la propriété `serviceUrl` afin qu’elle pointe vers le point de terminaison Guardian :
