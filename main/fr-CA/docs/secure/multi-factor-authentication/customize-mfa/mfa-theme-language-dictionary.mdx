---
description: Décrit les options de configuration des pages MFA hébergées permettant de personnaliser les propriétés de thème des pages MFA.
title: Dictionnaire de langue du thème MFA
---
...
Le `languageDictionary` peut être utilisé pour personnaliser le texte dans plusieurs sections du processus de <Tooltip tip="Authentification multifacteur (MFA) : processus d’authentification de l’utilisateur qui utilise un facteur en plus du nom d’utilisateur et du mot de passe, comme un code envoyé par SMS." cta="Voir le glossaire" href="/fr-CA/docs/glossary?term=MFA">MFA</Tooltip> de l’expérience Classic Login. Pour ce faire, utilisez le format suivant :
...
...
  ...
  languageDictionary:{ // Utiliser l'option dictionnaire de langue
...
Consultez ci-dessous la liste des catégories et options disponibles.
...
smsEnrollmentAddPhoneNumber: {
  ...
  // countryCodes: { 'US': 'Traduction de United States', ... '<country code>': '<traduction>' }
},
