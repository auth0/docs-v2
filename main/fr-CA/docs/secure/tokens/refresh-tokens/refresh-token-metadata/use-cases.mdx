---
description: Consultez des exemples de mise en œuvre de cas d’utilisation pour les métadonnées de jeton d’actualisation et les métadonnées de session. 
title: Cas d’utilisation des métadonnées de jeton d’actualisation et des métadonnées de session
validatedOn: 2026-03-06
---

Les [métadonnées de jeton d’actualisation](/fr-CA/docs/secure/tokens/refresh-tokens/refresh-token-metadata) et les [métadonnées de session](/fr-CA/docs/manage-users/sessions/session-metadata) vous permettent, ensemble, de créer et de stocker des données qui persistent durant tout le cycle de vie de la session Auth0 d’un utilisateur. Cet article présente des exemples pour les cas d’utilisation suivants :

* [Créer des claims personnalisées persistantes](/fr-CA/docs/secure/tokens/refresh-tokens/refresh-token-metadata/use-cases#create-persistent-custom-claims)
* [Créer un identifiant de session unique](/fr-CA/docs/secure/tokens/refresh-tokens/refresh-token-metadata/use-cases#create-a-unique-session-id)
* [Propager des identifiants de locataire](/fr-CA/docs/secure/tokens/refresh-tokens/refresh-token-metadata/use-cases#create-a-tenant-identifier)
* [Gérer les données temporaires provenant de fournisseurs d’identité (IdP) en amont](/fr-CA/docs/secure/tokens/refresh-tokens/refresh-token-metadata/use-cases#manage-transient-data-from-upstream-identity-providers-idps)
* [Améliorer la sécurité et la détection de fraude](/fr-CA/docs/secure/tokens/refresh-tokens/refresh-token-metadata/use-cases#enhance-security-and-fraud-detection)
...
Les métadonnées du jeton d’actualisation et celles de la session, combinées, vous permettent de créer des claims personnalisées persistantes afin d’étendre les informations contenues dans les jetons [ID](/fr-CA/docs/secure/tokens#id-tokens) et [d’accès](/fr-CA/docs/secure/tokens/access-tokens).

À l’aide de claims personnalisées persistantes, vous pouvez accéder à des données propres à l’application, comme :
...
Configurez un déclencheur [post-login](/fr-CA/docs/customize/actions/explore-triggers/signup-and-login-triggers/login-trigger#login-/-post-login) [Action](/fr-CA/docs/customize/actions) pour créer des claims personnalisées persistantes et les attribuer aux métadonnées du jeton d’actualisation à l’aide de l’objet `api.refreshToken.setMetadata()`.
...
* Permettre aux [API](/fr-CA/docs/get-started/apis#apis) d’offrir une journalisation granulaire, une limitation du taux de requêtes et des décisions d’autorisation contextuelles.
* Offrir une expérience utilisateur cohérente sur plusieurs cycles de vie de jetons.

Configurez un déclencheur Action [post-login](/fr-CA/docs/customize/actions/explore-triggers/signup-and-login-triggers/login-trigger#login-/-post-login) pour créer un identifiant de session unique et l’attribuer à la session de l’utilisateur à l’aide des objets `api.session.setMetadata()` et `api.refreshToken.setMetadata()`.
...
Les métadonnées du jeton d’actualisation et les métadonnées de session vous permettent, ensemble, de gérer les données temporaires et contextuelles provenant des IdP en amont tout au long de la session d’un utilisateur, sans les stocker de façon permanente dans son profil Auth0.

Avec les données temporaires, vous pouvez :
...
Configurez un déclencheur Action [post-login](/fr-CA/docs/customize/actions/explore-triggers/signup-and-login-triggers/login-trigger#login-/-post-login) pour identifier les données de profil utilisateur contenues dans les objets `event.request`, `event.user` et `event.context`.
...
Configurez un déclencheur Action [post-login](/fr-CA/docs/customize/actions/explore-triggers/signup-and-login-triggers/login-trigger#login-/-post-login) pour identifier les données contextuelles de l’utilisateur, qui peuvent inclure l’empreinte de l’appareil, l’emplacement géographique, les attributs réseau et les attributs comportementaux. Stockez ces données contextuelles dans la session de l’utilisateur à des fins de comparaison, à l’aide de l’objet `api.session.setMetadata()`.
...
Vous pouvez utiliser les points de terminaison `GET` [/api/v2/refresh-tokens/&#123;id&#125;](/fr-CA/docs/api/management/v2/refresh-tokens/get-refresh-token) et [/api/v2/sessions/&#123;id&#125;](/fr-CA/docs/api/management/v2/sessions/get-session) de la Management API d’Auth0 pour récupérer les données stockées dans les métadonnées d’un jeton d’actualisation ou d’une session.
...
* [Jetons d’actualisation](/fr-CA/docs/secure/tokens/refresh-tokens) : En savoir plus sur les jetons d’actualisation.
* [Sessions](/fr-CA/docs/manage-users/sessions) : En savoir plus sur les sessions.
* [Objets d’événement des Actions](/fr-CA/docs/customize/actions/explore-triggers/signup-and-login-triggers/login-trigger/post-login-event-object) : En savoir plus sur l’objet d’événement `post-login` et ses propriétés.
* [Objet d’API des Actions](/fr-CA/docs/customize/actions/explore-triggers/signup-and-login-triggers/login-trigger/post-login-api-object) : En savoir plus sur l’objet d’API `post-login` et ses méthodes.