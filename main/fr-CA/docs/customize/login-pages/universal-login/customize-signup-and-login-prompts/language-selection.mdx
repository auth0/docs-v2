---
description: Découvrez comment ajouter un menu de sélection de la langue de préférence dans les invites de Universal Login.
...
La sélection flexible de la langue est une fonctionnalité facultative qui permet aux utilisateurs de choisir leur langue préférée dans les invites <Tooltip tip="Universal Login : votre application redirige vers Universal Login, hébergé sur le serveur d’autorisation d’Auth0, pour vérifier l’identité d’un utilisateur." cta="Voir le glossaire" href="/fr-CA/docs/glossary?term=Universal+Login">Universal Login</Tooltip> associées à votre application, comme l’écran de connexion. Vous pouvez configurer cette fonctionnalité individuellement pour différentes invites Universal Login.

Après avoir mis en place cette fonctionnalité pour une invite donnée, un menu de sélection de la langue est ajouté à l’écran associé. Les utilisateurs peuvent choisir une langue préférée dans ce menu pour afficher automatiquement l’invite dans cette langue.

<Frame>![Deux invites Universal Login affichant le menu de sélection de la langue. ](/docs/images/cdy7uua7fh8z/4Z5pt3aH3vpRRYV0RDDnba/dc7a5abc8ee2cb462b2be4b43490393c/Language_Selector_Prompts.png)</Frame>
...
* Utilisez [Universal Login](/fr-CA/docs/authenticate/login/auth0-universal-login).

  * Cette fonctionnalité n’est pas offerte pour les pages de connexion personnalisées.
* Configurez un [domaine personnalisé](/fr-CA/docs/customize/custom-domains).
* Activez les langues que vous prévoyez ajouter à vos invites dans votre locataire Auth0.

  * Dans Auth0 Dashboard, accédez à [Settings &gt; General &gt; Languages](https://manage.auth0.com/#/tenant/general) et activez une ou plusieurs langues.

    * Pour en savoir plus, consultez [Internationalisation d’Universal Login](/fr-CA/docs/customize/internationalization-and-localization/universal-login-internationalization).
...
Pour commencer, ajoutez le script suivant à votre [modèle de page Universal Login](/fr-CA/docs/customize/login-pages/universal-login/customize-templates) pour faciliter la sélection de la langue.
...
Ce script permet aux invites Universal Login d’afficher un menu dynamique de sélection de la langue. Les utilisateurs peuvent accéder à ce menu pour appliquer leur langue préférée à l’invite configurée.

Pour ajouter ce script à votre modèle de page, utilisez le point de terminaison [Set Template for Universal Login](https://auth0.com/docs/api/management/v2/branding/put-universal-login).
...
Ensuite, ajoutez la sélection de la langue à une ou plusieurs invites Universal Login en configurant des Partial personnalisés pour les invites. Les Partial correspondent à du code personnalisé inséré à un point d’entrée d’un écran d’invite, comme l’écran de connexion. Pour en savoir plus, consultez [Personnaliser les invites d’inscription et de connexion](/fr-CA/docs/customize/internationalization-and-localization/universal-login-internationalization).

Pour ajouter la sélection de la langue à une invite précise, utilisez la <Tooltip tip="Management API : un produit qui permet aux clients d’effectuer des tâches administratives." cta="Voir le glossaire" href="/fr-CA/docs/glossary?term=Management+API">Management API</Tooltip> d’Auth0 pour configurer des Partial personnalisés d’invite avec les paramètres suivants :
...
| `persist`  | Obligatoire.<br /><br />Détermine si la langue doit être conservée durant la session.                                                                                                                                                                                                                                                                                         | `session`         |
| `action`   | Facultatif.<br /><br />Utilisez ce paramètre lorsque vous voulez permettre aux utilisateurs de mettre à jour leur langue sans faire progresser l’invite.<br /><br />Si une valeur est fournie, le formulaire n’est **pas** soumis automatiquement lorsqu’un utilisateur change de langue. Sinon, l’invite progresse automatiquement lorsqu’une autre langue est sélectionnée. | `change-language` |
...
<AccordionGroup>
  <Accordion title="Invite d’inscription : identifiant + mot de passe">
    Pour ajouter la sélection de langue à l&#39;invite `signup`, utilisez le point de terminaison [Set partials for a prompt](https://auth0.com/docs/api/management/v2/prompts/put-partials) :
  ...
  <Accordion title="Invite de connexion : identifiant d’abord">
  ...
  <Accordion title="Invite de connexion : Passwordless (courriel)">
