...
Lorsqu’une application tierce demande des scopes, les utilisateurs voient une invite de consentement. Par défaut, cette invite utilise le **nom** du scope pour générer le texte et regroupe tous les scopes d’une ressource, en affichant les actions de la ressource sur une seule ligne.

Si vous avez besoin d’une invite de consentement particulière, par exemple pour un consentement parental, vous devez créer votre propre formulaire de consentement personnalisé. Gardez à l’esprit que les lois varient d’un pays à l’autre.

Par exemple, supposons que vous avez une API enregistrée dans Auth0 avec les scopes définis suivants :

* `read:messages`: Lire vos courriels
* `write:messages`: Écrire des messages

L’invite de consentement affichera **Messages : lire et écrire vos messages**.

Vous pouvez plutôt utiliser la **description** du scope définie pour générer ce texte. Dans ce cas, la boîte de dialogue de consentement afficherait : **Lire vos courriels**, **Écrire des messages**.

Cette modification s’applique à l’échelle du locataire; elle affectera donc les invites de consentement pour toutes les API du locataire.

<div id="use-scope-descriptions-to-generate-consent-prompt-text">
  ## Utiliser les descriptions de scope pour générer le texte de l’invite de consentement
</div>

Définissez l’indicateur **use&#95;scope&#95;descriptions&#95;for&#95;consent** de votre locataire à `true` en effectuant l’appel d’API suivant :
...
<div id="use-scope-names-to-generate-consent-prompt-text">
  ## Utilisez les noms de scope pour générer le texte de l’invite de consentement
</div>
...
* [Scopes OpenID Connect](/fr-CA/docs/get-started/apis/scopes/openid-connect-scopes)
* [Scopes d’API](/fr-CA/docs/get-started/apis/scopes/api-scopes)
* [Bulletin de sécurité Auth0 sur l’attribution de scopes en fonction de l’adresse de courriel](/fr-CA/docs/secure/security-guidance/security-bulletins/2019-09-05-scopes)