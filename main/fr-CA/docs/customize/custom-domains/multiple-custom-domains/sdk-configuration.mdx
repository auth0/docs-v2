---
description: Découvrez comment configurer les SDK Auth0 pour qu’ils fonctionnent avec plusieurs domaines personnalisés.
...
Lorsque vous utilisez [plusieurs domaines personnalisés](/fr-CA/docs/customize/custom-domains/multiple-custom-domains), vous devez configurer vos SDK Auth0 pour qu’ils utilisent le domaine personnalisé approprié pour l’authentification. Ce guide explique la configuration des SDK sur différentes plateformes et dans différents scénarios.
...
Tous les SDK Auth0 exigent un paramètre `domain` qui indique quel domaine Auth0 utiliser pour l’authentification. Si vous utilisez des domaines personnalisés, définissez ce paramètre sur votre domaine personnalisé plutôt que sur votre domaine canonique Auth0.
...
Pour les applications à page unique qui utilisent le [SDK Auth0 SPA](https://github.com/auth0/auth0-spa-js) :
...
<AuthCodeGroup>
  ...
  // Détermine le domaine en fonction du contexte de l'application
  function getAuth0Domain() {
    const hostname = window.location.hostname;

    // Associe les domaines d'application aux domaines personnalisés Auth0
...
Pour les applications Next.js qui utilisent le [SDK Next.js d’Auth0](https://github.com/auth0/nextjs-auth0) (v4+) :
...
<AuthCodeGroup>
  ...
  ```typescript Configuration dynamique (SaaS B2B)
  import { Auth0Client } from '@auth0/nextjs-auth0/server';

  // Associer les identifiants de locataire aux domaines personnalisés (depuis la base de données, le cache, etc.)
  async function resolveTenantDomain(host: string): Promise<string> {
    // Extraire l’identifiant du locataire à partir du sous-domaine
    const subdomain = host.split('.')[0];

    // Rechercher le domaine personnalisé pour ce locataire (par exemple, dans la base de données)
  ...
  # Configuration Auth0 (un seul locataire avec plusieurs domaines personnalisés)
  AUTH0_DOMAIN=login.example.com           # Domaine personnalisé par défaut ou de repli
...
**Concepts clés de MCD avec Next.js :**
...
Pour les applications React utilisant le [SDK React d’Auth0](https://github.com/auth0/auth0-react) :
...
Les SDK de gestion permettent d’interagir avec la Management API d’Auth0. Si vous utilisez des domaines personnalisés, vous devrez peut-être inclure l’en-tête `auth0-custom-domain` ou utiliser le [domaine par défaut](/fr-CA/docs/customize/custom-domains/multiple-custom-domains/default-domain).
...
À l’aide de [python-jose](https://github.com/mpdavis/python-jose) :
...
<table class="table">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>L’URI du JWKS pointe vers le domaine canonique</td>
      <td>Mettez à jour l’URI du JWKS pour utiliser le domaine personnalisé : `https://custom-domain/.well-known/jwks.json`</td>
    </tr>

    <tr>
      <td>URI de redirection non concordante</td>
    ...
    <tr>
      <td>Erreurs d’origine croisée (CORS)</td>
    ...
    <tr>
      ...
      <td>Le paramètre `configurationBaseUrl` est absent</td>
...
* [SDK Auth0](/fr-CA/docs/troubleshoot/customer-support/product-support-matrix#auth0-sdks)