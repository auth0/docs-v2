---
description: Découvrez comment passer d’un domaine personnalisé unique à plusieurs domaines personnalisés dans Auth0.
...
Ce guide vous aide à passer d’une configuration avec un seul domaine personnalisé à [plusieurs domaines personnalisés](/fr-CA/docs/customize/custom-domains/multiple-custom-domains). Que vous ajoutiez des domaines pour différentes marques, régions ou segments de clientèle, ce guide fournit des instructions étape par étape pour assurer une transition harmonieuse.
...
Ajoutez vos nouveaux domaines personnalisés à l’aide de [Auth0 Dashboard](/fr-CA/docs/customize/custom-domains/multiple-custom-domains#configure-multiple-custom-domains) ou de [Management API](https://auth0.com/docs/api/management/v2).
...
Si vous voulez qu’un domaine soit utilisé par défaut pour les courriels et les appels à l’API, [définissez-le comme domaine par défaut](/fr-CA/docs/customize/custom-domains/multiple-custom-domains/default-domain):
...
<AccordionGroup>
  <Accordion title="Fonctionnement en parallèle (sans interruption)">
    ...
    **Avantages** : Aucune interruption, déploiement graduel, retour arrière facile
...
1. Consultez [Auth0 Community](https://community.auth0.com/) pour voir si des problèmes similaires ont été signalés
2. Consultez la [documentation de dépannage](/fr-CA/docs/troubleshoot/integration-extensibility-issues/troubleshoot-custom-domains)
3. Communiquez avec l’[assistance Auth0](https://support.auth0.com/) en fournissant :
