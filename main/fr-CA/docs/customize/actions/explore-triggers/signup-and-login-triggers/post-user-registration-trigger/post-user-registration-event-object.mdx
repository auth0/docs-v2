...
#
description: Découvrez l'objet `event` du déclencheur post-user-registration d'Actions, qui fournit des renseignements contextuels sur l'exécution du déclencheur.
title: "Déclencheurs Actions : post-user-registration - objet event"
validatedOn: 2026-05-25
---

L&#39;objet `event` du déclencheur post-user-registration d&#39;Actions fournit des renseignements contextuels sur l&#39;exécution du déclencheur.
...
<ResponseField name="event.authentication" type="dictionary" post={["optional"]}>
  Détails sur les signaux d’authentification supplémentaires obtenus pendant le flux d’inscription.

  <Expandable title="propriétés de authentification" defaultOpen>
    <ResponseField name="riskAssessment" type="dictionary" post={["optional"]}>
      Détails sur les renseignements d’évaluation des risques pour différents flux.
        ...
        <ResponseField name="supplemental" type="dictionary" post={["optional"]}>
          ...
          <Expandable title="propriétés de supplemental">
            <ResponseField name="akamai" type="dictionary" post={["optional"]}>
              [Accès anticipé] Évaluation supplémentaire des risques. Elle est offerte uniquement si Akamai Account Protector est activé et qu’Akamai transmet les en-têtes pour la transaction.

              <Expandable title="propriétés de akamai">
                <ResponseField name="akamaiBot" type="object" post={["optional"]}>
                  Les résultats de détection de robots transmis par Akamai Bot Manager.
                  ...
                  <Expandable title="propriétés de akamaiBot">
                    ...
                    <ResponseField name="botCategory" type="array of strings" post={["optional"]}>
                      La catégorie de robots des résultats d’Akamai Bot Manager.
                    </ResponseField>

                    <ResponseField name="botScore" type="number" post={["optional"]}>
                      Le score de robots des résultats d’Akamai Bot Manager.
                    </ResponseField>

                    <ResponseField name="botScoreResponseSegment" type="string" post={["optional"]}>
                      Le segment de réponse du score de robots des résultats d’Akamai Bot Manager.
                    </ResponseField>
...
<ResponseField name="event.connection" type="dictionary">
  Informations sur la connexion utilisée pour inscrire l’utilisateur.
...
<ResponseField name="event.security_context" type="object" post={["optional"]}>
  Objet contenant des signatures d’empreinte. Cet objet est disponible uniquement si l’application utilise Cloudflare. L’empreinte JA3/JA4 peut être nulle ou vide dans certains cas. Le cas le plus courant concerne les requêtes HTTP, car JA3 et JA4 sont calculées au niveau de TLS. Elle peut aussi être vide lorsque le Worker envoie des requêtes dans la même zone ou vers une zone qui n’est pas proxifiée (ou vers un tiers).
...
<ResponseField name="event.tenant" type="object">
  Détails du locataire associé à la transaction en cours.
...
<ResponseField name="event.transaction" type="dictionary" post={["optional"]}>
  ...
  <Expandable title="propriétés de la transaction" defaultOpen>
    <ResponseField name="acr_values" type="array of strings">
      Toutes les valeurs acr&#95;values fournies dans la demande d’authentification initiale.
    </ResponseField>

    <ResponseField name="locale" type="string">
      La langue à utiliser pour cette transaction, déterminée en comparant les langues demandées par le navigateur aux paramètres de langue du locataire.
    </ResponseField>
    ...
    <ResponseField name="response_type" type="array of strings" post={["optional"]}>
      Indique le type d’informations d’identification qu’Auth0 renverra.
    </ResponseField>

    <ResponseField name="state" type="string" post={["optional"]}>
      Une chaîne alphanumérique arbitraire et opaque que votre application ajoute à la demande initiale et qu’Auth0 inclut lors de la redirection vers votre application.
    </ResponseField>

    <ResponseField name="ui_locales" type="array of strings">
      Les valeurs ui&#95;locales fournies dans la demande d’authentification initiale.
    </ResponseField>
  </Expandable>
</ResponseField>
...
<ResponseField name="event.user" type="dictionary">
  ...
  <Expandable title="propriétés de l’utilisateur" defaultOpen>
    ...
    <ResponseField name="last_password_reset" type="string" post={["optional"]}>
      Horodatage indiquant la dernière fois que le mot de passe de l’utilisateur a été réinitialisé ou modifié. À la création de l’utilisateur, ce champ n’existe pas. Cette propriété est offerte uniquement pour les connexions de base de données.
    </ResponseField>
    ...
    <ResponseField name="user_metadata" type="dictionary">
      Champs personnalisés qui stockent des renseignements sur un utilisateur sans incidence sur ce à quoi il peut accéder, comme son adresse professionnelle, son adresse personnelle ou ses préférences.
    </ResponseField>
