...
<ResponseField name="options" type="customtokenexchangesetuserbyconnectionoptions">
  ...
  * `creationBehavior` - comportement à appliquer si aucun utilisateur avec le `user_id` spécifié n’existe dans la connexion.
    ...
    ou `'none'`, ce qui fera qu’aucun utilisateur ne sera créé et qu’une erreur sera renvoyée si aucun utilisateur n’existe.

  * `updateBehavior` - comportement à appliquer si un utilisateur avec le `user_id` spécifié existe déjà dans la connexion.
    Peut être `'replace'`, ce qui a pour résultat que les attributs de l’utilisateur existant sont remplacés par les attributs
    utilisateur spécifiés; ou `'none'`, ce qui signifie que l’utilisateur existant ne sera pas modifié.
