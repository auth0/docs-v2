...
#
description: "Découvrez l’objet event du déclencheur send-phone-message d’Actions, qui fournit des renseignements contextuels sur l’exécution du déclencheur."
title: "Déclencheurs Actions : send-phone-message - objet event"
validatedOn: 2026-04-28
---

L’objet `event` du déclencheur send-phone-message d’Actions fournit des renseignements contextuels sur l’exécution du déclencheur.
...
<ResponseField name="event.custom_domain" type="dictionary" post={["optional"]}>
  ...
  <Expandable title="Propriétés de custom_domain" defaultOpen>
    ...
    <ResponseField name="domain_metadata" type="dictionary">
      Métadonnées associées au domaine personnalisé, sous forme de paires clé-valeur.
    </ResponseField>
  </Expandable>
</ResponseField>
...
<ResponseField name="event.message_options" type="object">
  ...
  <Expandable title="propriétés de message_options" defaultOpen>
    ...
    <ResponseField name="code" type="string">
      Code à usage unique que l’utilisateur doit saisir dans le formulaire.
    </ResponseField>

    <ResponseField name="message_type" type="string">
      Mode d’acheminement du message, soit par &#39;sms&#39; ou par &#39;voice&#39;.
...
<ResponseField name="event.transaction" type="object" post={["optional"]}>
  ...
  <Expandable title="propriétés de la transaction" defaultOpen>
    <ResponseField name="correlation_id" type="string" post={["optional"]}>
      L’ID de corrélation peut être fourni dans la requête d’authentification initiale lorsque l’application redirige vers Universal Login. Vous pouvez utiliser cette valeur pour faire le lien entre les journaux et les requêtes de votre code Action et le flux utilisateur.
    </ResponseField>
  </Expandable>
</ResponseField>
...
<ResponseField name="event.user" type="dictionary">
  ...
  <Expandable title="propriétés de l’utilisateur" defaultOpen>
    ...
    <ResponseField name="identities" type="array of objects" post={["optional"]}>
      Contient les renseignements récupérés auprès du fournisseur d’identité avec lequel l’utilisateur s’authentifie initialement. Les utilisateurs peuvent aussi lier leur profil à plusieurs fournisseurs d’identité; ces identités figureront alors aussi dans ce tableau. Le contenu de chaque objet de fournisseur d’identité varie selon le fournisseur.
