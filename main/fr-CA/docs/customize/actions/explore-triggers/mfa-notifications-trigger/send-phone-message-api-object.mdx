...
Les valeurs stockées dans ce cache sont limitées au déclencheur d'action dans lequel elles
...
`ttl` ou `expires_at` spécifiée. Si aucune durée de vie n’est précisée, une durée de vie
...
disponibles lors de transactions ultérieures, même s’ils sont encore dans leur période de validité.
