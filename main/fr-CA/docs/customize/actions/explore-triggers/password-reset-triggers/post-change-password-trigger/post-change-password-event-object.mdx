...
#
description: L'objet `event` du déclencheur post-change-password d’Actions fournit des renseignements contextuels sur l’exécution du déclencheur.
title: 'Déclencheurs Actions : post-change-password - objet event'
validatedOn: 2026-05-25
---

L&#39;objet `event` du déclencheur post-change-password d’Actions fournit des renseignements contextuels sur l’exécution du déclencheur.
        ...
        <ResponseField name="supplemental" type="dictionary" post={["optional"]}>
          ...
          <Expandable title="propriétés de supplemental">
            <ResponseField name="akamai" type="dictionary" post={["optional"]}>
              [Accès anticipé] Évaluation supplémentaire des risques. Disponible uniquement si Akamai Account Protector est activé et qu’Akamai transfère les en-têtes pour la transaction.

              <Expandable title="propriétés de akamai">
                <ResponseField name="akamaiBot" type="object" post={["optional"]}>
                  Les résultats de détection de robots tels que transmis par Akamai Bot Manager.
