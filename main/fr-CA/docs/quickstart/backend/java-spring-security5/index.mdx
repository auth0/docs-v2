...
description: "Ajoutez l’authentification JWT Auth0 à une API Spring Boot avec des points de terminaison protégés"
---
...
<Accordion title="Utiliser l’IA pour intégrer Auth0" icon="microchip-ai" iconType="solid" defaultOpen>
  ...
  **Demandez ensuite à votre assistant IA :**
  ...
  Votre assistant IA créera automatiquement votre API Auth0, récupérera les informations d’identification, ajoutera la dépendance du SDK Auth0 Spring Boot API, configurera `application.yml` et implémentera une `SecurityFilterChain` avec validation des JWT et points de terminaison protégés. [Documentation complète sur les agent skills →](/fr-CA/docs/quickstart/agent-skills)
</Accordion>
...
<Note>
  ...
  **Compatibilité des versions de Java :** Ce guide de démarrage rapide est compatible avec **Java 17+** et **Spring Boot 3.2+**.
</Note>
...
Ce guide de démarrage rapide montre comment ajouter l’authentification JWT d’Auth0 à une API Spring Boot. Vous créerez une API sécurisée avec des points de terminaison protégés à l’aide du SDK Auth0 Spring Boot API.

<Steps>
  <Step title="Créer un nouveau projet" stepNumber={1}>
    Créez un nouveau projet d’API Spring Boot pour ce guide de démarrage rapide :
  ...
  <Step title="Ajouter le SDK Auth0" stepNumber={2}>
    Ajoutez le SDK d’API Auth0 Spring Boot aux dépendances de votre projet :
  ...
  <Step title="Configurez votre API Auth0" stepNumber={3}>
    Ensuite, créez une nouvelle API sur votre locataire Auth0 et ajoutez la configuration à votre projet.
    ...
    <Tabs>
      ...
      <Tab title="Dashboard">
        ...
        3. Entrez les renseignements de votre API :
           * **Name** : My Spring Boot API
           * **Identifier** : `https://my-springboot-api` (cela devient votre Audience)
           * **Signing Algorithm** : RS256
        4. Sélectionnez **Create**
        5. Remplacez `YOUR_AUTH0_DOMAIN` dans `application.yml` par votre **Domaine** à partir de l’onglet Test (par exemple, `your-tenant.auth0.com`)
        6. Remplacez `YOUR_AUTH0_API_IDENTIFIER` dans `application.yml` par votre **Identifier**. Par exemple, `https://my-springboot-api`.

        <Info>
          Votre **Domaine** ne doit pas inclure `https://`. Utilisez uniquement le domaine et la région. Par exemple : `your-tenant.auth0.com`.

          L’**Audience** (identifiant de l’API) est un identifiant unique pour votre API et peut être n’importe quel URI valide. Elle n’a pas besoin d’être une URL accessible publiquement.
        </Info>
      </Tab>
    </Tabs>
  </Step>
  ...
  <Step title="Lancez votre API" stepNumber={6}>
    ...
    **Gradle:**
    ...
    Votre API s’exécute maintenant à l’adresse `http://localhost:8080` (consultez la sortie de la console pour l’URL exacte).
  </Step>
</Steps>
...
<Check>
  **Vérification**
  ...
  Vous devriez maintenant avoir une API protégée par Auth0 pleinement fonctionnelle à l’adresse [localhost](http://localhost:8080/)
</Check>
...
<Accordion title="Accès aux claims JWT">
...
<Accordion title="Autorisation basée sur les scopes">
  Implémentez un contrôle d’accès granulaire à l’aide des scopes JWT pour renforcer la sécurité.
  ...
  * `read:users` - Lire les données des utilisateurs
  * `write:users` - Écrire des données d’utilisateur
  * `admin` - Accès administratif

  **2. Configurez des stratégies d’autorisation :**
...
<Accordion title="Sécurité renforcée DPoP">
  Activez DPoP (Demonstration of preuve de possession) afin de renforcer la sécurité des jetons en liant les jetons d’accès à des clés cryptographiques.
  ...
  * `DISABLED` : validation JWT Bearer standard uniquement
    ...
    <Info>
      Pour en savoir plus sur DPoP, consultez la [documentation Auth0 sur DPoP](https://auth0.com/docs/secure/sender-constraining/demonstrating-proof-of-possession-dpop).
    </Info>
</Accordion>
...
<AccordionGroup>
  <Accordion title="401 Non autorisé - audience invalide">
    ...
    **Solution :** Assurez-vous que `auth0.audience` correspond exactement à l’identifiant de votre API Auth0. La revendication audience du jeton doit correspondre à cette valeur.
  ...
  <Accordion title="401 Non autorisé - émetteur invalide">
    **Problème :** La validation de l’émetteur du jeton échoue.

    **Solution :** Vérifiez que votre Domaine est correct et n’inclut pas `https://`. Utilisez le domaine sans le préfixe `https://`.
  ...
  <Accordion title="Valeurs de configuration introuvables">
    ...
    **Solution :** Vérifiez la structure de `application.yml` et les noms des propriétés. Assurez-vous que la section auth0 contient les valeurs de Domaine et d’Audience.
  ...
  <Accordion title="Problèmes de connectivité réseau">
    **Problème :** Échec de récupération des JWKS ou délai d’expiration de connexion.
    ...
    *.auth0.com
    *.us.auth0.com  # Pour les locataires de la région US
    *.eu.auth0.com  # Pour les locataires de la région EU
    *.au.auth0.com  # Pour les locataires de la région AU
    ```
  </Accordion>
...
<CardGroup cols={3}>
  ...
  <Card title="Documentation sur DPoP" icon="shield" href="https://auth0.com/docs/secure/sender-constraining/demonstrating-proof-of-possession-dpop">
...
Une application exemple complète démontrant toutes les fonctionnalités est disponible dans le dépôt du SDK.
...
# Obtenir un jeton d'accès (remplacer par vos informations d'identification Auth0)
...
# Tester le point de terminaison protégé avec un jeton Bearer
