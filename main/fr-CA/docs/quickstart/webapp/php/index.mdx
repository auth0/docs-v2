...
<Info>
  **Vous débutez avec Auth ?** Découvrez [comment fonctionne Auth0](/fr-CA/docs/get-started/auth0-overview), comment il [s’intègre aux applications Web classiques](/fr-CA/docs/get-started/architecture-scenarios/sso-for-regular-web-apps) et quel [protocole](/fr-CA/docs/get-started/authentication-and-authorization-flow) il utilise.
</Info>
...
Créons un exemple d’application qui authentifie un utilisateur dans une application PHP. Nous allons adopter ici une approche simple, adaptée au format de cette documentation. Cela dit, consultez aussi l’application [Quickstart sur GitHub](https://github.com/auth0-samples/auth0-php-web-app/) qui l’accompagne pour un exemple plus complet.

<div id="installing-http-client-and-messaging-factories">
  ### Installation du client HTTP et des fabriques de messages
</div>

Le SDK PHP d’Auth0 prend en charge de nombreuses normes [PHP-FIG](https://www.php-fig.org), qui offrent des options d’interopérabilité avec votre architecture. Deux d’entre elles sont particulièrement importantes : [PSR-17](https://www.php-fig.org/psr/psr-17/) et [PSR-18](https://www.php-fig.org/psr/psr-18/). Ces normes vous permettent d’intégrer les composants réseau de votre choix pour gérer les messages et les requêtes HTTP. Vous devrez installer des bibliothèques compatibles dans votre projet pour que le SDK puisse les utiliser.

La bibliothèque réseau la plus répandue pour PHP est [Guzzle](https://docs.guzzlephp.org/), bien qu’il existe plusieurs autres options dans la communauté PHP. Utilisons Guzzle pour cet exemple d’application. Encore une fois, à partir du répertoire de votre projet, exécutez la commande shell suivante :
...
# L’URL du domaine de votre locataire Auth0
...
# Vous pouvez la générer à l’aide de \`openssl rand -hex 32\` dans votre terminal.
AUTH0_COOKIE_SECRET=
...
Enfin, gérons correctement la déconnexion de nos utilisateurs. La méthode `logout()` du SDK PHP d’Auth0 s&#39;occupe d&#39;effacer les témoins de session de notre application d&#39;exemple, de rediriger l&#39;utilisateur vers le [point de terminaison /logout](https://auth0.com/docs/logout) d&#39;Auth0 (ce qui met fin à la session Auth0 ainsi qu&#39;à toute session ouverte chez le fournisseur d&#39;identité), puis de le renvoyer vers la route d&#39;index `/`.
...
Ouvrez votre navigateur à l’adresse [http://127.0.0.1:3000](https://127.0.0.1:3000) et essayez l’application.
