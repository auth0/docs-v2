---
mode: wide
description: Ce tutoriel explique comment utiliser le module nginx-openid-connect pour ajouter l’authentification et l’autorisation à votre serveur NGINX.
title: NGINX Plus
---
...
<Recipe>
  <Content>
    ...
    votre serveur NGINX. Nous vous recommandons de vous connecter pour suivre ce guide de démarrage rapide avec des exemples configurés pour votre compte.
    ...
    <Section id={sections[1].id} title={sections[1].title} stepNumber="2">
      ...
      Une fois le module installé, vous devez l’activer dans NGINX en ajoutant la ligne suivante près du début de votre
    ...
    <Section id={sections[3].id} title={sections[3].title} stepNumber="4">
      ...
      Ensuite, ajoutez l’[URL de déconnexion de votre locataire](https://auth0.com/docs/api/authentication#auth0-logout) à votre
    ...
    <Section id={sections[6].id} title={sections[6].title} stepNumber="7">
      ...
      4. Passez à l’onglet **Credentials**.

      Dans la section **Application Authentication**, réglez **Authentication Method** sur None.
    </Section>

    <Section id={sections[7].id} title={sections[7].title} stepNumber="8">
      Ajoutez des en-têtes supplémentaires provenant du jeton d’identité (JWT) à la cible en amont dans votre
      fichier `/etc/nginx/conf.d/frontend.conf`.
    </Section>

    ## Prochaines étapes

    Excellent travail ! Si vous êtes rendu jusqu’ici, la connexion, la déconnexion et les renseignements de profil de l’utilisateur devraient maintenant fonctionner dans votre application.

    Cela conclut notre tutoriel de démarrage rapide, mais il reste encore beaucoup à explorer. Pour en savoir plus sur ce que vous pouvez faire avec Auth0, consultez :
