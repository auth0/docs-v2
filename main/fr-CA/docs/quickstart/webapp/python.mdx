---
mode: wide
description: 'Ajoutez l’authentification Auth0 à une application Web Flask avec une fonctionnalité de connexion, des routes protégées et des profils utilisateur'
sidebarTitle: Flask
title: 'Ajoutez la fonctionnalité de connexion à votre application Flask'
---
...
Ce guide de démarrage rapide explique comment ajouter l’authentification Auth0 à une application Flask. Vous créerez une application Web sécurisée avec une fonctionnalité de connexion, des routes protégées et l’accès au profil de l’utilisateur à l’aide du [SDK WebApp Python d’Auth0](https://github.com/auth0/auth0-server-python).
...
<Steps>
  ...
  <Step title="Configurez votre application Auth0" stepNumber={3}>
    ...
    <Tabs>
      ...
      <Tab title="Dashboard">
        ...
        7. Remplacez `YOUR_AUTH0_DOMAIN` dans `.env` par votre **Domaine** indiqué dans l’onglet **Settings** (par ex., `your-tenant.auth0.com`)
        8. Remplacez `YOUR_CLIENT_ID` dans `.env` par votre **ID client**
        9. Remplacez `YOUR_CLIENT_SECRET` dans `.env` par votre **Secret client**
        ...
        <Info>
          **Important :** Ne validez jamais votre fichier `.env` dans votre système de contrôle de version. Ajoutez-le à `.gitignore`.
        </Info>
      </Tab>
    </Tabs>
  </Step>

  <Step title="Créer la configuration d’authentification, les routes et les modèles" stepNumber={4}>
    Créer des fichiers
    ...
    <AuthCodeGroup>
      ...
      # Stockage simple en mémoire pour le développement
      ...
      class MemoryStateStore:
          ...
          async def delete_by_logout_token(self, claims, options=None):
              # Pour la prise en charge de la déconnexion par canal arrière
              pass
      ...
      # Initialiser le ServerClient Auth0
      auth0 = ServerClient(
  ...
  <Step title="Lancez votre application" stepNumber={5}>
    ...
    Votre application sera accessible à l’adresse [http://localhost:5000](http://localhost:5000). Le SDK Auth0 prend automatiquement en charge les routes d’authentification.
  </Step>
</Steps>
...
<Check>
  **Point de contrôle**
  ...
  Vous devriez maintenant avoir une page de connexion Auth0 entièrement fonctionnelle qui s’exécute sur votre [localhost](http://localhost:5000/)
</Check>
...
<Accordion title="Accès aux jetons d’API">
...
<Accordion title="Configuration d’un stockage personnalisé">
  Par défaut, le SDK utilise un stockage basé sur des cookies. Pour les environnements de production ayant des besoins particuliers (mise à l’échelle horizontale, partage de session entre services), vous pouvez configurer des backends de stockage personnalisés comme Redis ou PostgreSQL.
  ...
  **Exemple Redis :**
  ...
  <Info>
    Pour la plupart des applications, le stockage par défaut basé sur des cookies est suffisant. Le stockage personnalisé exige l’implémentation de l’interface `StateStore`. Consultez les [exemples du SDK](https://github.com/auth0/auth0-server-python/blob/main/examples/ConfigureStore.md) pour obtenir des implémentations détaillées.
  </Info>
</Accordion>
...
<AccordionGroup>
  <Accordion title="MissingRequiredArgumentError">
    ...
    **Cause :** La variable d’environnement `AUTH0_SECRET` est absente ou ne se charge pas correctement.
  ...
  <Accordion title="Erreur : URL de rappel invalide">
    ...
    **Cause :** L’URI de redirection dans votre code ne correspond pas à celle enregistrée dans Auth0 Dashboard.
  ...
  <Accordion title="Erreurs liées à la boucle d’événements AsyncIO">
    **Problème :** Vous voyez &quot;RuntimeError: This event loop is already running&quot; ou des erreurs asynchrones semblables

    **Cause :** La prise en charge de l’asynchrone dans Flask 2.0+ peut poser problème avec certaines configurations.

    **Solution :**

    Installez Flask avec la prise en charge de l’asynchrone :
  ...
  <Accordion title="Erreurs « Module introuvable »">
    **Problème :** Vous voyez &quot;ModuleNotFoundError: No module named &#39;auth0&#95;server&#95;python&#39;&quot; ou un message semblable
  ...
  <Accordion title="Erreur ClaimDecodingFailed">
    **Problème :** Vous voyez l’erreur &quot;ClaimDecodingFailed&quot; ou &quot;Failed to decode claims&quot; lors de l’authentification

    **Cause :** Le jeton d’identité ou le jeton d’accès reçu depuis Auth0 n’a pas pu être décodé correctement, souvent pour l’une des raisons suivantes :
  ...
  <Accordion title="Jeton expiré ou invalide">
    **Problème :** Vous voyez les erreurs &quot;Token has expired&quot; ou &quot;invalid&#95;token&quot;

    **Cause :** Le jeton d’accès ou le jeton d’identité a expiré, ou la session a pris fin.
  ...
  <Accordion title="CORS et erreurs inter-origines">
  ...
  <Accordion title="Limite de requêtes dépassée">
    **Problème :** Vous voyez des erreurs &quot;Trop de requêtes&quot; ou &quot;Rate limit exceeded&quot;

    **Cause :** Votre application a dépassé les limites de requêtes d&#39;Auth0 pour les demandes d&#39;authentification.
