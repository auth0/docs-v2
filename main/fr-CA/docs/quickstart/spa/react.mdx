...
sidebarTitle: React
description: Ce guide explique comment intégrer Auth0, ajouter l’authentification et afficher les informations du profil utilisateur dans une application monopage (SPA) utilisant React, à l’aide du SDK React d’Auth0.
mode: wide
---
...
<Accordion title="Utilisez l’IA pour intégrer Auth0" icon="microchip-ai" iconType="solid" defaultOpen>
  ...
  **Demandez ensuite à votre assistant IA :**
  ...
  Votre assistant IA créera automatiquement votre application Auth0, récupérera les identifiants, installera `@auth0/auth0-react`, configurera Auth0Provider et créera tous les composants nécessaires. [Documentation complète sur les agent skills →](/fr-CA/docs/quickstart/agent-skills)
</Accordion>
...
<Steps>
  ...
  <Step title="Installez le SDK React d’Auth0" stepNumber={2}>
    ```shellscript
  ...
  <Step title="Configurez votre application Auth0" stepNumber={3}>
    La prochaine étape consiste à créer une nouvelle application dans votre locataire Auth0 et à ajouter les variables d’environnement à votre projet.

    Vous avez trois options pour configurer votre application Auth0 : utiliser l’outil Quick Setup (recommandé), exécuter une commande CLI ou la configurer manuellement dans le Dashboard :
    ...
    <Tabs>
      ...
      <Tab title="Dashboard">
        ...
        5. Remplacez `YOUR_AUTH0_APP_DOMAIN` et `YOUR_AUTH0_APP_CLIENT_ID` dans le fichier `.env` par les valeurs **Domaine** et **ID client** du dashboard
        ...
        **Allowed Web Origins:**
        ...
        <Info>
          ...
          **Allowed Logout URLs** est essentiel pour offrir une expérience fluide lors de la déconnexion. Sans URL correspondante, les utilisateurs ne seront pas redirigés vers votre application après la déconnexion et resteront plutôt sur une page Auth0 générique.
  ...
  <Step title="Créer des composants de connexion, de déconnexion et de profil" stepNumber={5}>
    Créer des fichiers
...
<Accordion title="Protéger des routes avec React Router">
