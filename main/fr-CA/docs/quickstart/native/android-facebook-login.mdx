---
mode: wide
sidebarTitle: Facebook Login sur Android
title: Ajouter la connexion Facebook native à votre application Android
---
...
2. **Configurer Auth0 pour Facebook Native** - Configurez votre application Auth0 pour utiliser Facebook Native Sign In. Consultez [Add Facebook Login to Native Apps](/fr-CA/docs/authenticate/identity-providers/social-identity-providers/facebook-native).
...
<Steps>
  <Step title="Configurer les autorisations de Facebook Login" stepNumber={1}>
    Mettez à jour le bouton Facebook Login dans votre activité pour demander les bonnes autorisations.

    Votre application prend déjà en charge Facebook Login, mais pour obtenir un profil utilisateur plus complet, vous devez demander les autorisations `public_profile` et `email`. Vous ajouterez également une fonction de rappel pour démarrer le flux d&#39;authentification Auth0.
  ...
  <Step title="Installer et configurer le SDK Auth0" stepNumber={2}>
    Ajoutez le SDK Android Auth0 à votre projet et configurez les identifiants de votre application.

    **Ajoutez la dépendance dans `app/build.gradle.kts` :**
    ...
    **Ajoutez vos identifiants Auth0 dans `strings.xml` :**

    Accédez à la section **Applications** de l’[Auth0 Dashboard](https://manage.auth0.com/) et sélectionnez l’application dans laquelle vous avez activé Facebook Native Sign In. Copiez les valeurs **Domaine** et **ID client**.
    ...
    **Initialisez le SDK Auth0 dans votre activité :**
    ...
    <Note>
      ...
      Si vous prévoyez prendre en charge Web Authentication, consultez [Authentification par Universal Login](/fr-CA/docs/libraries/auth0-android#authentication-via-universal-login).
    </Note>
  </Step>

  <Step title="Récupérer le jeton d’accès de la session Facebook" stepNumber={3}>
    Une fois la connexion à Facebook réussie, récupérez un **jeton d’accès de session** à partir de l’API Facebook. Auth0 a besoin de ce jeton pour vérifier l’identité de l’utilisateur côté backend.
  ...
  <Step title="Échanger des jetons contre des identifiants Auth0" stepNumber={5}>
    Utilisez le jeton de session et le profil de l’utilisateur des étapes précédentes pour vous authentifier auprès d’Auth0 et obtenir des jetons Auth0.

    Appelez `loginWithNativeSocialToken` sur le `AuthenticationAPIClient`, en fournissant le jeton de session ainsi que le type de jeton de sujet Facebook :
  ...
  <Step title="Assembler l’ensemble" stepNumber={6}>
...
<Check>
  **Point de vérification**
  ...
  Vous devriez maintenant pouvoir vous authentifier nativement avec Facebook. Si l’application Facebook est installée sur l’appareil, l’authentification se fait directement dans l’application — aucun navigateur n’est nécessaire.
</Check>
...
<CardGroup cols={2}>
  ...
  <Card title="Référence du SDK Auth0 pour Android" icon="android" href="/fr-CA/docs/libraries/auth0-android" iconType="solid">
