---
title: Ajouter la connexion à votre application Android avec le SDK Auth0.Android
...
<Accordion title="Utiliser l’IA pour intégrer Auth0" icon="microchip-ai" iconType="solid" defaultOpen>
  ...
  **Ensuite, demandez à votre assistant IA :**
  ...
  Votre assistant IA créera automatiquement votre application Auth0, récupérera les identifiants, ajoutera la dépendance du SDK Auth0 Android, configurera les variables de substitution du manifeste et implémentera les flux de connexion et de déconnexion. [Documentation complète sur les agent skills →](/fr-CA/docs/quickstart/agent-skills)
</Accordion>
...
<Steps>
  ...
  <Step title="Ajouter le SDK Auth0 via Gradle" stepNumber={2}>
  ...
  <Step title="Configurez votre application Auth0" stepNumber={3}>
    Ensuite, vous devez créer une nouvelle application dans votre locataire Auth0 et ajouter la configuration à votre projet Android.
    ...
    4. Ouvrez l’onglet **Settings** de la page des détails de l’application
    5. Remplacez `{yourDomain}` et `YOUR_AUTH0_CLIENT_ID` dans le fichier `strings.xml` par les valeurs **Domaine** et **ID client** de l’Auth0 Dashboard
    ...
    **URLs de déconnexion autorisées :**
    ...
    <Info>
      ...
      **Allowed Logout URLs** sont essentielles pour offrir une expérience utilisateur fluide lors de la déconnexion. Sans URL correspondante, les utilisateurs ne seront pas redirigés vers votre application après la déconnexion et resteront plutôt sur une page Auth0 générique.
      ...
      Le schéma d’URL inclut le nom de votre package (`com.auth0.samples.android`) afin que le rappel soit acheminé vers votre application précise.
    </Info>
  ...
  <Step title="Mettre en œuvre la connexion et la déconnexion" stepNumber={5}>
    **Implémenter la connexion :** utilisez WebAuthProvider pour lancer la page Universal Login.
  ...
  <Step title="Lancez votre application" stepNumber={6}>
    ...
    **Déroulement attendu :**
    ...
    2. Touchez &quot;Se connecter&quot; → une Chrome Custom Tab s’ouvre → Terminez la connexion
...
<Check>
  **Point de contrôle**
  ...
  Vous devriez maintenant disposer d’une expérience de connexion à Auth0 entièrement fonctionnelle sur votre appareil Android ou votre émulateur. L’application utilise les onglets Chrome personnalisés pour assurer une authentification sécurisée et stocke automatiquement les identifiants.
</Check>
...
<Accordion title="Problèmes courants et solutions">
  ...
  **Solutions :**
  ...
  1. Vérifiez que **Allowed Callback URLs** dans Auth0 Dashboard correspond exactement à votre `applicationId`
  2. Vérifiez que les variables de substitution du manifeste dans `build.gradle.kts` sont correctes
  3. Assurez-vous que les URL HTTPS et les URL avec schéma personnalisé sont toutes les deux configurées
  4. Nettoyez et regénérez le projet : **Build** → **Clean Project** → **Rebuild Project**
  ...
  **Correctif :**
  ...
  1. Mettez à jour le plugin Android Gradle vers la version la plus récente dans `build.gradle` (niveau du projet)
...
<Accordion title="Configurer Android App Links">
  Les [Android App Links](https://developer.android.com/training/app-links) permettent à votre application de se définir comme gestionnaire par défaut des URL de rappel Auth0, pour une expérience d’authentification plus sécurisée et fluide. Sans App Links, Android peut afficher une boîte de dialogue de choix demandant à l’utilisateur de choisir entre votre application et un navigateur.

  <Note>
    Les App Links utilisent des URL de rappel vérifiées avec le schéma `https`. C’est plus sécuritaire que les schémas d’URL personnalisés, qui peuvent être vulnérables aux [attaques d’usurpation de client](https://datatracker.ietf.org/doc/html/rfc8252#section-8.6).
  </Note>
  ...
  Pour en savoir plus, consultez la documentation [Enable Android App Links Support](/fr-CA/docs/get-started/applications/enable-android-app-links-support) et le guide Android [Verify App Links](https://developer.android.com/training/app-links/verify-site-associations).
</Accordion>
...
<Accordion title="Utiliser un schéma d’URL personnalisé">
  ...
  1. Mettez à jour l’espace réservé `auth0Scheme` du manifeste dans votre `app/build.gradle.kts` :
  ...
  2. Mettez à jour les paramètres **Allowed Callback URLs** et **Allowed Logout URLs** de votre application dans l’[Auth0 Dashboard](https://manage.auth0.com/#/applications) pour utiliser le schéma personnalisé :
