...
<Steps>
  ...
  <Step title="Installer le SDK d’Auth0" stepNumber={2}>
    ...
    <Tabs>
      ...
      <Tab title="Visual Studio pour Mac">
        1. Dans le **Solution Pad**, faites Ctrl+clic (ou clic droit) sur le dossier **Packages**
  ...
  <Step title="Initialiser l’application Auth0" stepNumber={4}>
    ...
    <Tabs>
      ...
      <Tab title="iOS - AppDelegate">
        ...
        <Warning>
          Remplacez `YOUR_BUNDLE_IDENTIFIER`, `{yourDomain}` et `{yourClientId}` par vos valeurs.
        </Warning>
      </Tab>
    </Tabs>
  ...
  <Step title="Mettre en œuvre la connexion et la déconnexion" stepNumber={5}>
    Ajoutez des méthodes pour gérer l’authentification de l’utilisateur.
  ...
  <Step title="Lancez votre application" stepNumber={6}>
    ...
    <Tabs>
      ...
      <Tab title="Visual Studio pour Mac">
        ...
        2. Sélectionnez un émulateur, un simulateur ou un appareil connecté
...
<Check>
  **Point de contrôle**
  ...
  Vous disposez maintenant d’une expérience de connexion Auth0 entièrement fonctionnelle dans votre application .NET pour Android ou iOS. L’application utilise le navigateur système pour une authentification sécurisée et gère automatiquement le flux de redirection.
</Check>
...
<Info>
  Les revendications renvoyées dépendent exactement des scopes demandés. Pour en savoir plus, consultez [Utilisation des scopes](https://auth0.github.io/auth0-oidc-client-net/documentation/advanced-scenarios/scopes.html) dans la documentation du client OIDC Auth0.
</Info>
...
<CardGroup cols={2}>
  <Card title="Configurer des fournisseurs d’identité" icon="key" href="/fr-CA/docs/authenticate/identity-providers" iconType="solid">
    Ajoutez des fournisseurs de connexion sociale comme Google, Facebook et GitHub
  </Card>

  <Card title="Activer l’authentification multifacteur" icon="shield-halved" href="/fr-CA/docs/secure/multi-factor-authentication" iconType="solid">
  ...
  <Card title="Personnaliser l’expérience de connexion" icon="paintbrush" href="/fr-CA/docs/customize/universal-login-pages" iconType="solid">
