...
mode: wide
sidebarTitle: UWP
title: Ajouter Login à votre application UWP
...
Ce guide explique comment intégrer Auth0 à une application Universal Windows Platform (UWP) à l’aide du SDK Auth0.OidcClient.UWP. À la fin, votre application prendra en charge la connexion, la déconnexion et l’affichage des renseignements de profil de l’utilisateur.
...
<Steps>
  ...
  <Step title="Installer le SDK Auth0" stepNumber={3}>
    ...
    **À l’aide de l’interface utilisateur du gestionnaire de packages (recommandée) :**
  ...
  <Step title="Ajoutez la connexion et la déconnexion à votre page principale" stepNumber={4}>
    Ajoutez maintenant des boutons de connexion et de déconnexion à votre application. Mettez à jour `MainPage.xaml` et `MainPage.xaml.cs`.
...
<AccordionGroup>
  ...
  <Accordion title="Erreur : l’URL de rappel ne correspond pas">
  ...
  <Accordion title="Erreur de grant invalide lors de la connexion">
  ...
  <Accordion title="Aucune information sur l’utilisateur n’est affichée">
    **Problème :** Le scope `profile` n’a pas été demandé; les revendications de l’utilisateur ne sont donc pas disponibles.
...
<CardGroup cols={2}>
  <Card title="Appeler une API protégée" icon="plug" href="/fr-CA/docs/quickstart/backend/nodejs#call-your-api">
    Utilisez votre jeton d’accès pour authentifier les requêtes vers une API côté serveur
  </Card>

  <Card title="Jetons d’actualisation" icon="rotate" href="/fr-CA/docs/secure/tokens/refresh-tokens">
    Maintenez les sessions utilisateur d’un redémarrage de l’application à l’autre
  </Card>

  <Card title="Organisations" icon="building" href="/fr-CA/docs/manage-users/organizations">
    Prenez en charge les applications B2B multilocataires
  </Card>

  <Card title="Personnaliser Universal Login" icon="paintbrush" href="/fr-CA/docs/authenticate/login/auth0-universal-login">
    Adaptez la page de connexion d’Auth0 à votre image de marque
  </Card>

  <Card title="Contrôle d’accès basé sur les rôles" icon="shield-halved" href="/fr-CA/docs/manage-users/access-control/rbac">
...
<CardGroup cols={3}>
  <Card title="Client OIDC d’Auth0" icon="github" href="https://github.com/auth0/auth0-oidc-client-net">
    Code source et documentation du SDK sur GitHub
  </Card>

  <Card title="OpenID Connect" icon="book" href="/fr-CA/docs/authenticate/protocols/openid-connect-protocol">
    Découvrez le fonctionnement d’OpenID Connect avec Auth0
  </Card>

  <Card title="Bonnes pratiques relatives aux jetons d’accès" icon="key" href="/fr-CA/docs/secure/tokens/access-tokens">
