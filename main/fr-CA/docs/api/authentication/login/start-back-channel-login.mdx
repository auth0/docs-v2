---
title: "Login en canal arrière"
description: "Le point de terminaison Login en canal arrière permet aux applications d’envoyer une demande d’authentification au téléphone d’un utilisateur ou à son appareil d’authentification, à condition qu’une..."
---

`POST /bc-authorize`

Le point de terminaison Login en canal arrière permet aux applications d’envoyer une demande d’authentification au téléphone d’un utilisateur ou à son appareil d’authentification, à condition qu’une application y soit installée et qu’il soit inscrit aux [notifications poussées à l’aide du SDK Guardian](https://auth0.com/docs/secure/multi-factor-authentication/auth0-guardian#enroll-in-push-notifications).
...
La valeur `auth_req_id` doit être conservée telle quelle, car elle est utilisée plus tard dans le flux pour identifier la demande d’authentification.
