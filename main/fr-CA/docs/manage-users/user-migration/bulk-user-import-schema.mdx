---
description: Découvrez comment effectuer une importation groupée d’utilisateurs avec la Management API.
title: Schéma et exemples d’importation groupée d’utilisateurs
---
...
| `password_hash`        | string  | Mot de passe haché pour la connexion de l’utilisateur. Lorsqu’un utilisateur est créé, Auth0 utilise [bcrypt](https://auth0.com/blog/hashing-in-action-understanding-bcrypt/) pour sécuriser le mot de passe. L’importation de mots de passe hachés permet aux utilisateurs de conserver leurs mots de passe, pour une expérience plus fluide. Les mots de passe compatibles doivent être hachés à l’aide de bcrypt $2a$ ou $2b$ et utiliser 10 `saltRounds`. Cette propriété ne peut être fournie que lors de la première importation de l’utilisateur et ne peut pas être mise à jour par la suite. | Non                            |
...
Pour en savoir plus sur `app_metadata` et `user_metadata`, consultez [Comprendre le fonctionnement des métadonnées dans les profils utilisateur](/fr-CA/docs/manage-users/user-accounts/metadata).
...
L’objet `user.custom_password_hash` peut être utilisé à la place de la propriété `user.password_hash` lorsque le hachage du mot de passe de l’utilisateur a été créé avec un autre algorithme. Notez que ce champ et `password_hash` sont mutuellement exclusifs.
...
| `hash.encoding`     | chaîne | L’encodage du hachage fourni. Doit être l’un des suivants : <ul><li>`base64`</li><li>`hex`</li><li>`utf8`</li></ul>Les variantes hexadécimales en majuscules et en minuscules sont prises en charge, ainsi que le base64 encodé dans l’URL.                                                                                                                                                                                                                                                 |
...
| `salt.encoding`     | chaîne | L’encodage du sel fourni. Doit être l’un des suivants : <ul><li>`base64`</li><li>`hex`</li><li>`utf8`</li></ul> Les variantes hexadécimales en majuscules et en minuscules sont prises en charge, ainsi que le base64 encodé dans l’URL. Par défaut, `salt.encoding` est `utf8`.                                                                                                                                                                                                            |
...
| `cost`              | entier | Paramètre de coût CPU/mémoire utilisé pour le hachage scrypt. Doit être une puissance de deux supérieure à un. Par défaut, `cost` est 16384.<br />Ce paramètre est utilisé uniquement lorsque `algorithm` est défini sur `scrypt`.                                                                                                                                                                                                                                                          |
...
Pendant le processus d’importation groupée, vous pouvez mettre à jour le `custom_password_hash` si l’utilisateur ne s’est pas connecté à l’aide du `custom_password_hash` importé initialement. Par exemple, vous pouvez soumettre le JSON ci-dessous deux fois à l’endpoint [`/api/v2/jobs/users-imports`](https://auth0.com/docs/api/management/v2?_ga=2.169020323.346193905.1601302257-746691936.1587131255#!/Jobs/post_users_imports) avec des valeurs différentes pour `custom_password_hash`. Lors du deuxième envoi, définissez l’indicateur `upsert` sur `true`.
...
Vous pouvez utiliser le [Bcrypt Password Generator](https://www.browserling.com/tools/bcrypt) de browserling.com pour générer des hachages bcrypt de mots de passe.

<div id="supported-hash-algorithms">
  ### Algorithmes de hachage pris en charge
</div>

Auth0 prend actuellement en charge l’importation de mots de passe d’utilisateurs hachés avec :
...
Par exemple, la valeur suivante a été générée à partir de la chaîne `hello` à l’aide d’un paramètre de coût de 10 :
...
* `hash.value` doit respecter le format indiqué dans la [section 5.3 de la RFC-2307](https://tools.ietf.org/html/rfc2307#section-5.3) sur IETF Datatracker.
* Le schéma doit être l’un des suivants : `md5|smd5|sha*|ssha*` ; consultez [ce document](https://www.openldap.org/faq/data/cache/347.html) pour en savoir plus.
* Notez que le schéma [crypt](https://www.openldap.org/faq/data/cache/344.html) n’est **pas pris en charge** en raison d’un comportement qui dépend du système ou de l’implémentation. Pour en savoir plus, consultez [Open LDAP Admin Guide - 14.4.2. CRYPT password storage scheme](https://www.openldap.org/doc/admin24/guide.html#CRYPT%20password%20storage%20scheme).
...
* `hash.value` doit être au format de chaîne PHC, comme indiqué dans [P-H-C / phc-string-format](https://github.com/P-H-C/phc-string-format/blob/master/phc-sf-spec.md) sur GitHub.
...
Le tableau `user.mfa_factors` contient les inscriptions <Tooltip tip="Authentification multifacteur (MFA) : processus d’authentification de l’utilisateur qui utilise un facteur en plus du nom d’utilisateur et du mot de passe, comme un code envoyé par SMS." cta="Voir le glossaire" href="/fr-CA/docs/glossary?term=MFA">MFA</Tooltip> de l’utilisateur. Pour en savoir plus, consultez [l’authentification multifacteur dans Auth0](/fr-CA/docs/secure/multi-factor-authentication). L’importation des inscriptions évite aux utilisateurs d’avoir à se réinscrire à l’authentification multifacteur après l’importation. Les types d’inscription pris en charge sont :
...
* [Exportation groupée d’utilisateurs](/fr-CA/docs/manage-users/user-migration/bulk-user-exports)
