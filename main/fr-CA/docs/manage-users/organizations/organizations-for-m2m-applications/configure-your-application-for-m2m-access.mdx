---
description: Découvrez comment configurer votre application pour l’accès M2M à l’aide d’Auth0 Dashboard et de la Management API.
...
1. [Définissez le comportement de l’organisation](#define-organization-behavior) pour chaque API à laquelle vous devez accéder.
2. Si l’application ne doit accéder qu’à certaines organisations, c.-à-d. pas à toutes, [autorisez l’accès M2M](/fr-CA/docs/manage-users/organizations/organizations-for-m2m-applications/authorize-m2m-access) pour chaque API à laquelle vous devez accéder pour ces organisations précises.
...
Le tableau suivant explique les champs permettant de définir le comportement de l’organisation pour l’accès M2M :
...
| Champ                                                   | Description                                                                                              | Mappage API                                                                                                                                                                                                                                                                                                                                                                                                                   |
| ------------------------------------------------------- | -------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Organisation par défaut pour l’accès Machine-to-Machine | Définit l’organisation à appliquer aux requêtes Client Credentials qui ne contiennent pas d’organisation | L’organisation par défaut pour l’accès Machine-to-Machine correspond à l’objet `default_organization` avec les propriétés suivantes : <ul> <li>`organization_id` : ID de l’organisation à utiliser</li> <li> `flows` : Le flux d’authentification à utiliser pour l’organisation par défaut.<br /><br />Ce paramètre est masqué dans l’interface utilisateur et sa valeur par défaut est `["client_credentials"]` </li> </ul> |

<div id="set-default-organization-for-an-application">
  ### Définir l’organisation par défaut pour une application
</div>

Pour définir l’organisation par défaut d’une application, utilisez [Auth0 Dashboard](https://manage.auth0.com/) ou la [Management API](https://auth0.com/docs/api/management/v2).
...
<Tabs>
  ...
  <Tab title="Management API">
    ...
    L’exemple de code suivant met à jour une application avec une organisation par défaut :
