...
Si vous voulez que votre application web dispose d’une interface de connexion personnalisée, vous pouvez utiliser Auth0.js. Cette bibliothèque JavaScript sans interface utilisateur pour Auth0 déclenche le flux d’authentification (ainsi que d’autres tâches) et récupère en retour un objet profil utilisateur. Pour en savoir plus, consultez la [référence d’Auth0.js v9](/fr-CA/docs/libraries/auth0js).
