---
description: Découvrez comment ajouter des permissions à des rôles à l’aide d’Auth0 Dashboard ou de la Management API.
title: Ajouter des permissions à des rôles
---
...
Vous pouvez ajouter des permissions aux [rôles](/fr-CA/docs/manage-users/access-control/rbac) à l’aide de l’<Tooltip tip="Auth0 Dashboard : produit principal d’Auth0 pour configurer vos services." cta="Voir le glossaire" href="/fr-CA/docs/glossary?term=Auth0+Dashboard">Auth0 Dashboard</Tooltip> ou de la <Tooltip tip="Auth0 Dashboard : produit principal d’Auth0 pour configurer vos services." cta="Voir le glossaire" href="/fr-CA/docs/glossary?term=Management+API">Management API</Tooltip>. Les rôles et les permissions qui leur sont associées peuvent être utilisés avec le jeu de fonctionnalités API Authorization Core.
...
* Les rôles sont sélectionnés parmi des valeurs prédéfinies. Si votre liste de rôles est vide, vous devez [créer un rôle](/fr-CA/docs/manage-users/access-control/configure-core-rbac/roles/create-roles).
* [Configurez une API](/fr-CA/docs/get-started/auth0-overview/set-up-apis) dans l’Auth0 Dashboard.
* Les permissions sont sélectionnées parmi des valeurs prédéfinies. Si votre liste de permissions est vide, vous devez [ajouter des permissions](/fr-CA/docs/get-started/apis/add-api-permissions) à votre API.

<div id="dashboard">
  ## Auth0 Dashboard
</div>

1. Accédez à [Auth0 Dashboard &gt; Gestion des utilisateurs &gt; Rôles](https://manage.auth0.com/#/roles) et cliquez sur le nom du rôle pour l’afficher.
...
Effectuez un appel `POST` au [point de terminaison Add Role Permissions](https://auth0.com/docs/api/management/v2#!/Roles/post_role_permission_assignment). Assurez-vous de remplacer les valeurs d’espace réservé `ROLE_ID`, `MGMT_API_ACCESS_TOKEN`, `API_IDENTIFIER` et `PERMISSION_NAME` par l’id de votre rôle, le <Tooltip tip="Jeton d’accès : information d’identification d’autorisation, sous la forme d’une chaîne opaque ou d’un JWT, utilisée pour accéder à une API." cta="Voir le glossaire" href="/fr-CA/docs/glossary?term=access+token">jeton d’accès</Tooltip> de la Management API, l’identifiant de l’API (<Tooltip tip="Jeton d’accès : information d’identification d’autorisation, sous la forme d’une chaîne opaque ou d’un JWT, utilisée pour accéder à une API." cta="Voir le glossaire" href="/fr-CA/docs/glossary?term=audience">audience</Tooltip>) et le ou les noms de permission, respectivement.
...
* [Supprimer des permissions de rôles](/fr-CA/docs/manage-users/access-control/configure-core-rbac/roles/remove-permissions-from-roles)