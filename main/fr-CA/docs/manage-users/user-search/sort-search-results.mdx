---
description: Découvrez comment trier les résultats de recherche en indiquant une valeur field:order au paramètre sort.
...
Pour trier les résultats de recherche d’utilisateurs, transmettez une valeur `field:order` au paramètre `sort` lors de votre requête. `field` correspond au nom du champ selon lequel effectuer le tri, tandis que `order` peut être défini à `1` pour un ordre croissant et à `-1` pour un ordre décroissant.
