---
description: Décrit la syntaxe de la chaîne de requête pour la recherche d’utilisateurs dans Auth0.
title: Syntaxe de la chaîne de requête pour la recherche d’utilisateurs
---
...
* Un terme sans nom de champ ne correspondra pas au texte des champs de [métadonnées utilisateur](/fr-CA/docs/manage-users/user-accounts/metadata).
...
Vous pouvez rechercher des utilisateurs à l’aide de tous les [champs normalisés du profil utilisateur](/fr-CA/docs/manage-users/user-accounts/user-profiles/normalized-user-profile-schema) ainsi que des champs ci-dessous :
...
| `last_login`       | date heure                       | L’horodatage de la dernière connexion de l’utilisateur. Si cette propriété est évaluée dans une [Rule](/fr-CA/docs/customize/rules) à l’aide de l’objet `user`, la valeur sera associée à la connexion qui a déclenché la règle (puisque les règles s’exécutent après la connexion proprement dite). |
