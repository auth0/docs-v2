---
title: Création d’un fournisseur SSO
sidebarTitle: Créer de nouveaux fournisseurs SSO
description: Assistant en plusieurs étapes pour créer des fournisseurs SSO, avec sélection du fournisseur, configuration des détails et paramétrage de l’authentification
---

import Spinner from "/snippets/fr-CA/Spinner.jsx";
import { ComponentLoader } from "/snippets/fr-CA/ComponentLoader.jsx";

Le composant `SsoProviderCreate` fournit une interface unifiée pour ajouter de nouveaux fournisseurs [d’authentification unique (SSO)](/fr-CA/docs/authenticate/enterprise-connections/self-service-enterprise-configuration).

<ComponentLoader componentSelector="sso-provider-create" componentPreviewText="Aperçu du composant SSO Provider Create" />
...
<Tabs>
  ...
  <Tab title="Next.js">
    ...
    <Callout icon="file-lines" color="#0EA5E9" iconType="regular">
      **Configuration Auth0 requise** - Assurez-vous que votre locataire est configuré avec l’API
      ...
      export default function App() {
        ...
        return (
          <Auth0Provider
            ...
            clientId="your-client-id"
            authorizationParams={{ redirect_uri: window.location.origin }}
            interactiveErrorHandler='popup' // Obligatoire pour gérer les demandes d’authentification renforcée via la fenêtre contextuelle d’Universal Login
          >
            <Auth0ComponentProvider domain={domain}>
              <CreateSsoProviderPage />
            </Auth0ComponentProvider>
...
Personnalisez l’apparence à l’aide de variables CSS et de surcharges de classes. Prend en charge un styling adapté au thème.
...
<table class="table">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>Étape de configuration du nom et du nom affiché</td>
    </tr>
...
<table class="table">
  ...
  <tbody>
    <tr>
      ...
      <td>Logique de création du fournisseur et intégration à l’API</td>
    </tr>
  </tbody>
</table>