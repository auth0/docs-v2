...
2. Accédez à l’onglet **Permissions**, cliquez sur l’icône de corbeille à côté de la permission que vous souhaitez supprimer, puis confirmez.
   ...
   <Frame>![Écran Définir les permissions de l’API dans la section Ajouter des permissions à l’API des applications du Dashboard](/docs/images/cdy7uua7fh8z/32Pb185OFs2mC6z2fmunEw/2708c52c2869d016066cc456dd00b6a2/dashboard-applications-apis-permissions.png)</Frame>

<div id="use-the-management-api">
  ## Utiliser Management API
</div>

Effectuez un appel PATCH au [point de terminaison Update Resource Server](https://auth0.com/docs/api/management/v2#!/Resource_Servers/patch_resource_servers_by_id) qui inclut toutes les permissions que vous voulez conserver et exclut toutes celles que vous voulez supprimer. Remplacez les valeurs d’espace réservé `API_ID,` `MGMT_API_ACCESS_TOKEN`, `PERMISSION_NAME` et `PERMISSION_DESC` par l’ID de votre API, le <Tooltip tip="Jeton d’accès : information d’identification d’autorisation, sous la forme d’une chaîne opaque ou d’un JWT, utilisée pour accéder à une API." cta="Voir le glossaire" href="/fr-CA/docs/glossary?term=Access+Token">Jeton d’accès</Tooltip> de la Management API, le ou les noms de permission et la ou les descriptions de permission, respectivement.
...
| Valeur                  | Description                                                                                                                      |
| ----------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| `API_ID`                | ID de l’API dont vous souhaitez supprimer des permissions.                                                                       |
...
| `PERMISSION_DESC`       | Description(s) claire(s) de la ou des permissions que vous souhaitez conserver pour l’API indiquée.                              |
