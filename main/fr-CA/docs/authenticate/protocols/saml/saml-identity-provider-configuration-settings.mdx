...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  Si vous avez configuré un [domaine personnalisé](/fr-CA/docs/customize/custom-domains), vous devez utiliser le CNAME de votre domaine personnalisé au lieu du domaine Auth0. Pour en savoir plus, consultez [Configurer les fonctionnalités pour utiliser des domaines personnalisés](/fr-CA/docs/customize/custom-domains/configure-features-to-use-custom-domains#configure-saml-identity-providers).
</Callout>
...
Lorsque vous utilisez le <Tooltip tip="Authentification unique (SSO) : service qui, après la connexion d’un utilisateur à une application, l’authentifie automatiquement dans d’autres applications." cta="Voir le glossaire" href="/fr-CA/docs/glossary?term=SSO">SSO</Tooltip> initié par l’IdP, assurez-vous d’inclure le paramètre de connexion dans l’URL de retour :
...
Si vous utilisez la fonctionnalité [Organisations](/fr-CA/docs/manage-users/organizations), vous pouvez inclure, au besoin, un paramètre organization contenant l’id de l’organisation souhaitée :
...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  Pour que les utilisateurs puissent ouvrir une session avec cette méthode, la connexion doit être activée pour l’Organisation. De plus, vous devez soit configurer l’[ajout automatique](/fr-CA/docs/manage-users/organizations/configure-organizations/grant-just-in-time-membership) pour la connexion activée, soit vous assurer que les utilisateurs appartiennent à l’Organisation.
</Callout>
...
Vous pouvez créer un ID d’entité personnalisé à l’aide de la propriété `connection.options.entityId`. Pour en savoir plus, consultez [Specify a Custom Entity ID](/fr-CA/docs/authenticate/identity-providers/enterprise-identity-providers/saml#specify-a-custom-entity-id).

Vous pouvez obtenir la valeur de l’ID d’entité personnalisé à l’aide du point de terminaison Get a Connection :
...
C’est ici que le fournisseur d’identité SAML enverra les requêtes et les réponses de déconnexion :

export const codeExample13 = `https://{yourDomain}/logout`;

<AuthCodeBlock children={codeExample13} language="http" />

Les demandes de déconnexion SAML doivent être signées par le fournisseur d’identité.
...
Pour en savoir plus sur le SSO initié par l’IdP, consultez [Configurer le SSO SAML initié par l’IdP](/fr-CA/docs/authenticate/protocols/saml/saml-sso-integrations/identity-provider-initiated-single-sign-on).
...
* [Trouver l’ID ou le nom de la connexion](/fr-CA/docs/authenticate/identity-providers/locate-the-connection-id)
* [Personnaliser les assertions SAML](/fr-CA/docs/authenticate/protocols/saml/saml-configuration/customize-saml-assertions)
* [Dépanner les configurations SAML](/fr-CA/docs/troubleshoot/authentication-issues/troubleshoot-saml-configurations)