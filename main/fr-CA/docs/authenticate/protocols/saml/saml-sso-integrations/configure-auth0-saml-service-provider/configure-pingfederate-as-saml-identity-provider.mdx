...
* Liaison **HTTP-Redirect** pour la requête <Tooltip tip="Security Assertion Markup Language (SAML) : protocole normalisé permettant à deux parties d’échanger des informations d’authentification sans mot de passe." cta="Voir le glossaire" href="/fr-CA/docs/glossary?term=SAML">SAML</Tooltip>
...
Pour savoir comment configurer Auth0 afin de router les réponses SAML entrantes, consultez [Configurer le SSO initié par le fournisseur d’identité](/fr-CA/docs/authenticate/protocols/saml/saml-sso-integrations/identity-provider-initiated-single-sign-on).