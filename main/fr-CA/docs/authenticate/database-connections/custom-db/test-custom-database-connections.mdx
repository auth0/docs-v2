---
description: Décrit comment tester vos connexions de base de données personnalisées dans le tableau de bord Auth0.
title: Tester les connexions de base de données personnalisées
---
...
2. Donnez un nom à votre connexion source de test, activez **Requires Username**, puis sélectionnez **Create**.
   ...
   <Callout icon="file-lines" color="#0EA5E9" iconType="regular">
     Vous pouvez créer des utilisateurs pour vos tests en accédant à [User Management &gt; Users](https://manage.auth0.com/#/users). Après avoir sélectionné **Create** **Users** et rempli les champs requis, sélectionnez la base de données source dans le champ **Connection**.
   </Callout>
...
7. Mettez à jour les scripts d’action de base de données [Login](/fr-CA/docs/authenticate/database-connections/custom-db/test-custom-database-connections#login) et [Get User](/fr-CA/docs/authenticate/database-connections/custom-db/test-custom-database-connections#get-user) dans votre base de données cible. Pour en savoir plus sur les pratiques exemplaires pour l’utilisation des scripts d’action de base de données, consultez [Custom Database Connection and Action Scripts Best Practices](/fr-CA/docs/authenticate/database-connections/custom-db/custom-database-connections-scripts).
8. Sélectionnez **Save and Try** pour chaque script. Vous devriez surveiller la sortie `console.log` dans les journaux en temps réel d’Actions. Pour en savoir plus, consultez [Actions Real-time Logs](/fr-CA/docs/customize/actions/actions-real-time-logs).
...
1. Répétez les étapes de [Créer un locataire et une application de test](/fr-CA/docs/authenticate/database-connections/custom-db/test-custom-database-connections#create-a-test-tenant-and-application).
...
Le script [Get User](/fr-CA/docs/authenticate/database-connections/custom-db/templates/get-user) implémente une fonction exécutable qui détermine l’état actuel d’un utilisateur.

Lorsque **Import Users to Auth0** est activé, le script Get User s’exécute lorsqu’un utilisateur tente de s’inscrire pour vérifier si cet utilisateur existe déjà dans le répertoire d’utilisateurs externe.

Le script Get User s’exécute aussi lorsqu’un utilisateur tente de :

* Modifier l’adresse courriel d’un utilisateur ([Change Email](/fr-CA/docs/authenticate/database-connections/custom-db/templates/change-email) script)
* Se connecter ([Login](/fr-CA/docs/authenticate/database-connections/custom-db/templates/login) script)
* Modifier le mot de passe d’un utilisateur ([Change Password](/fr-CA/docs/authenticate/database-connections/custom-db/templates/change-password) script)

Lorsque **Import Users to Auth0** est désactivé, le script Get User s’exécute lorsqu’un utilisateur tente de s’inscrire pour vérifier si cet utilisateur existe déjà dans le répertoire d’utilisateurs externe. Si l’utilisateur existe déjà dans le répertoire d’utilisateurs externe, le script Create ne sera pas exécuté.

Le script Get User s’exécute aussi lorsqu’un utilisateur tente de :

* Créer un utilisateur ([Create](/fr-CA/docs/authenticate/database-connections/custom-db/templates/create) script)
* Modifier l’adresse courriel d’un utilisateur ([Change Email](/fr-CA/docs/authenticate/database-connections/custom-db/templates/change-email) script)
* Modifier le mot de passe d’un utilisateur ([Change Password](/fr-CA/docs/authenticate/database-connections/custom-db/templates/change-password) script)
...
Le [script de création](/fr-CA/docs/authenticate/database-connections/custom-db/templates/create) contient une fonction exécutable qui crée un enregistrement utilisateur correspondant dans la base de données externe lorsqu’un utilisateur s’inscrit au moyen de <Tooltip tip="Universal Login : votre application redirige vers Universal Login, hébergé sur le serveur d’autorisation d’Auth0, pour vérifier l’identité d’un utilisateur." cta="Voir le glossaire" href="/fr-CA/docs/glossary?term=Universal+Login">Universal Login</Tooltip>, ou est créé dans le <Tooltip tip="Universal Login : votre application redirige vers Universal Login, hébergé sur le serveur d’autorisation d’Auth0, pour vérifier l’identité d’un utilisateur." cta="Voir le glossaire" href="/fr-CA/docs/glossary?term=Auth0+Dashboard">tableau de bord Auth0</Tooltip> ou à l’aide de l’<Tooltip tip="Management API : un produit qui permet aux clients d’effectuer des tâches administratives." cta="Voir le glossaire" href="/fr-CA/docs/glossary?term=Management+API">API de gestion</Tooltip> d’Auth0.
...
Le [script de suppression](/fr-CA/docs/authenticate/database-connections/custom-db/templates/delete) exécute une fonction qui supprime un utilisateur d’Auth0 et de la base de données externe en une seule opération lorsque l’utilisateur est supprimé dans le tableau de bord Auth0 ou à l’aide de l’API de gestion Auth0.
...
Le [script de vérification](/fr-CA/docs/authenticate/database-connections/custom-db/templates/verify) implémente une fonction exécutable qui met à jour l’état de vérification de l’adresse courriel d’un utilisateur dans la base de données externe lorsque l’utilisateur clique sur le lien dans le courriel de vérification envoyé par Auth0.
...
Le [script de changement de mot de passe](/fr-CA/docs/authenticate/database-connections/custom-db/templates/change-password) implémente une fonction exécutable qui met à jour le mot de passe d’un utilisateur dans la base de données externe lorsqu’un processus de changement de mot de passe est lancé depuis le tableau de bord Auth0 ou l’API de gestion Auth0.
...
Le script [Change Email](/fr-CA/docs/authenticate/database-connections/custom-db/templates/change-email) exécute une fonction qui met à jour l’adresse courriel d’un utilisateur lorsque celui-ci modifie son adresse courriel ou que l’état de vérification de celle-ci change.

Ce script n’est pas disponible dans le tableau de bord Auth0. Vous devez appeler le point de terminaison [Update a connection](https://auth0.com/docs/api/management/v2/connections/patch-connections-by-id) de l’API de gestion et fournir la propriété `options.customScripts.change_email`.
...
* [Niveau de robustesse des mots de passe dans les connexions de base de données Auth0](/fr-CA/docs/authenticate/database-connections/password-strength)
* [Pratiques exemplaires pour l’environnement des scripts d’action des bases de données personnalisées](/fr-CA/docs/authenticate/database-connections/custom-db/custom-database-connections-scripts/environment)
* [Options de mot de passe dans les connexions de base de données Auth0](/fr-CA/docs/authenticate/database-connections/password-options)
* [Configurer la connexion du fournisseur d’identité pour les mises à jour du profil d’utilisateur](/fr-CA/docs/manage-users/user-accounts/user-profiles/configure-connection-sync-with-auth0)
* [Pratiques exemplaires pour les paramètres de connexion](/fr-CA/docs/authenticate/connection-settings-best-practices)
* [Créer des connexions à des bases de données personnalisées](/fr-CA/docs/authenticate/database-connections/custom-db/create-db-connection)