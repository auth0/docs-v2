---
description: Décrit les modèles de script Verify pour les actions de base de données personnalisée liées à la vérification du courriel de l’utilisateur.
title: Modèles de script Verify
---

Le script Verify implémente la fonction exécutée pour indiquer l’état de vérification du courriel d’un utilisateur dans une base de données externe. Nous recommandons de nommer cette fonction `verify`.

Ce script est utilisé uniquement dans les scénarios d’authentification hérités et il est nécessaire pour prendre en charge la vérification du courriel de l’utilisateur. Une adresse courriel vérifiée est essentielle à plusieurs flux de travail dans Auth0, et l’implémentation de ce script ajoute cette prise en charge d’emblée.

S’il est activé, ce script s’exécute lorsqu’un utilisateur clique sur le lien contenu dans le courriel de vérification envoyé par Auth0.
...
| `callback` | Function | Permet de transmettre les erreurs ou les données de profil dans le pipeline. |

### Exemple

Voici un exemple en pseudo-JavaScript illustrant comment vous pourriez implémenter la fonction `login`. Pour des exemples propres à chaque langage, consultez [exemple de script par langage](/fr-CA/docs/authenticate/database-connections/custom-db/templates/verify#language-specific-script-examples).
...
| `verified` | Boolean | Facultatif  | Contient une valeur qui indique l’état de vérification de l’utilisateur dans la base de données externe (`true` ou `false`). Obligatoire uniquement si la valeur est `true`. |
