---
description: Décrit l’implémentation de l’authentification Passwordless avec connexion intégrée dans les applications natives.
title: Connexion Passwordless intégrée dans les applications natives
---
...
Pour utiliser les API intégrées <Tooltip tip="Passwordless : forme d’authentification qui ne repose pas sur un mot de passe comme premier facteur." cta="Voir le glossaire" href="/fr-CA/docs/glossary?term=Passwordless">Passwordless</Tooltip> dans des applications natives, assurez-vous d’activer le type d’octroi **Passwordless OTP** dans [Auth0 Dashboard &gt; Applications &gt; Applications](https://manage.auth0.com/#/applications), dans les paramètres de votre application sous **Advanced Settings** &gt; **Grant Types**.
...
**Envoyer un mot de passe à usage unique par texto**
...
* [Lock.Android Passwordless](/fr-CA/docs/libraries/lock-android/lock-android-passwordless)
* [Auth0.Android Passwordless](/fr-CA/docs/libraries/auth0-android/auth0-android-passwordless)
* [Auth0.swift Passwordless](/fr-CA/docs/libraries/auth0-swift/auth0-swift-passwordless)
...
Personnalisez la <Tooltip tip="Authentification multifacteur (MFA) : processus d’authentification de l’utilisateur qui utilise un facteur en plus du nom d’utilisateur et du mot de passe, comme un code envoyé par SMS." cta="Voir le glossaire" href="/fr-CA/docs/glossary?term=MFA">MFA</Tooltip> avec des flux intégrés. Utilisez l’API MFA pour permettre aux utilisateurs de s’inscrire et de répondre aux demandes de vérification à l’aide des facteurs de leur choix pris en charge par votre application.

Lorsque votre application demande un <Tooltip tip="Jeton d’accès : justificatif d’autorisation, sous la forme d’une chaîne opaque ou d’un JWT, utilisé pour accéder à une API." cta="Voir le glossaire" href="/fr-CA/docs/glossary?term=access+token+at+the">jeton d’accès au</Tooltip> point de terminaison `oauth/token`, la réponse renvoyée inclut le `mfa_token` à utiliser avec l’API MFA, ainsi que le paramètre `mfa_requirements`, qui contient la liste des méthodes d’authentification actuellement prises en charge par votre application :
...
Personnalisez davantage votre flux MFA avec les Actions Auth0. Pour en savoir plus, consultez [Déclencheurs des Actions : post-challenge - objet API](/fr-CA/docs/customize/actions/explore-triggers/password-reset-triggers/post-challenge-trigger/post-challenge-api-object).