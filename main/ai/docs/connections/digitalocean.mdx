---
title: DigitalOcean
description: Connect your AI agent to a user's DigitalOcean account to automate the creation and management of servers and storage volumes.
og:title: "DigitalOcean Integration - AI Identity for B2B"
og:description: "Connect your AI agent to a user's DigitalOcean account to automate the creation and management of servers and storage volumes."
og:url: "https://auth0.com/ai/docs"
twitter:title: "DigitalOcean Integration - AI Identity for B2B"
twitter:description: "Connect your AI agent to a user's DigitalOcean account to automate the creation and management of servers and storage volumes."
validatedOn: 2026-08-27
---

import FindDomainInfoBlock from "/snippets/ai/common/find-domain-info-block.mdx";
import { Auth0SetupBlock } from "/snippets/ai/connections/Auth0SetupBlock.jsx";
import { IntegrationInfoBlock } from "/snippets/ai/connections/IntegrationInfoBlock.jsx";
import NextStepsBlock from "/snippets/ai/connections/next-step.mdx";
import { TokenVaultConfigBlock } from "/snippets/ai/connections/TokenVaultConfigBlock.jsx";

Connect your AI agent to a user's DigitalOcean account to automate the creation and management of servers (Droplets), configure networking, and manage storage volumes.

## Connect DigitalOcean to Auth0

<Steps>
  <Step title="DigitalOcean setup">
    1. Sign up for a [DigitalOcean account](https://cloud.digitalocean.com/registrations/new) if you don't already have one.
    2. Visit the [OAuth Applications page](https://cloud.digitalocean.com/account/api/applications) in your DigitalOcean control panel.
    3. Select **Register OAuth Application** to create a new OAuth application.
    4. Fill in the application details:
       - **Name**: Enter a descriptive name for your integration
       - **Homepage URL**: `https://YOUR_AUTH0_DOMAIN`
       - **Application description**: Brief description of your application
       - **Callback URL**: `https://YOUR_AUTH0_DOMAIN/login/callback`
       <FindDomainInfoBlock />
    5. Select **Register OAuth Application** and note your **Client ID** and **Client Secret**.

    To learn more about DigitalOcean OAuth setup, read the [DigitalOcean OAuth API documentation](https://docs.digitalocean.com/reference/api/oauth/).

  </Step>
  <Step title="Auth0 setup">
    <Auth0SetupBlock providerName="DigitalOcean" scopesName="Scopes" allowFreeFormScopes={true} />
  </Step>
</Steps>

<IntegrationInfoBlock providerName="DigitalOcean" />

## Token Vault configuration example

To configure the Token Vault for your DigitalOcean connection, you can use the following code snippet in your application:

<TokenVaultConfigBlock
  providerName="DigitalOcean"
  connectionName="digitalocean"
  scopes={["account:read"]}
  optionalScopes={["droplet:create", "kubernetes:read"]}
/>

<NextStepsBlock />
