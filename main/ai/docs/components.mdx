---
title: Components
---

## Language Selector

<Tabs>
  <Tab title="JavaScript" icon="/docs/images/ai/js.svg">
  ### Implement feature in JavaScript

This is how you can implement this GenAI feature in JavaScript:

```JavaScript
const foo = (bar) => return bar;
const baz = foo('ai');
console.log(baz);
```

  </Tab>
  <Tab title="Python" icon="python">
  ### Implement feature in Python

This is how you can implement this GenAI feature in Python

```Python
def foo(bar):
baz = foo('ai')
print(baz)
```

  </Tab>
</Tabs>

## Language Selector with Python disabled

<Tabs>
  <Tab title="JavaScript" icon="/docs/images/ai/js.svg">
  ### Implement feature in JavaScript

This is how you can implement this GenAI feature in JavaScript:

```JavaScript
const foo = (bar) => return bar;
const baz = foo('ai');
console.log(baz);
```

  </Tab>
  <Tab title="Python" icon="python" disabled={true}>
  ### Implement feature in Python

This is how you can implement this GenAI feature in Python:

```Python
def foo(bar):
baz = foo('ai')
print(baz)
```

  </Tab>
</Tabs>

## Card

<Card title="I am a Heading">

Auth0 for AI Agents is currently available in Early Access. Join today to start building secure AI agents and provide feedback to shape the platform's future.

{" "}
<a href="/intro">Link to same tab</a>

{" "}
<a href="https://auth0.com" target="_blank">
  Link to new tab
</a>

  <Card title="I am a Card with a Checked Checkbox" icon="circle-check" horizontal>

Auth0 for AI Agents is currently available in Developer Preview. Join today to start building secure AI agents and provide feedback to shape the platform's future.

  </Card>

  <Card title="I am a Card with an Unchecked Checkbox" icon="circle" horizontal>

Auth0 for AI Agents is currently available in Developer Preview. Join today to start building secure AI agents and provide feedback to shape the platform's future.

  </Card>

  <Card title="I am a Card with an icon" icon="js" horizontal iconType="solid">

Auth0 for AI Agents is currently available in Developer Preview. Join today to start building secure AI agents and provide feedback to shape the platform's future.

  </Card>

  <Card title="I am a Card with an icon with no border" icon="js" horizontal iconType="solid">

Auth0 for AI Agents is currently available in Developer Preview. Join today to start building secure AI agents and provide feedback to shape the platform's future.

  </Card>

  <Card title="I am a Card with a CTA" icon="python" horizontal iconType="solid" href="https://auth0.com">

Auth0 for AI Agents is currently available in Developer Preview. Join today to start building secure AI agents and provide feedback to shape the platform's future.

  </Card>

  <Card title="I am a Card with a disabled CTA" icon="python" horizontal iconType="solid">

Auth0 for AI Agents is currently available in Developer Preview. Join today to start building secure AI agents and provide feedback to shape the platform's future.

  </Card>

  <Card title="I am a Card with a loading CTA state" icon="python" horizontal iconType="solid">

Auth0 for AI Agents is currently available in Developer Preview. Join today to start building secure AI agents and provide feedback to shape the platform's future.

  </Card>

  <Card title="I am a Card with tags" icon="js" horizontal iconType="solid">

Auth0 for AI Agents is currently available in Developer Preview. Join today to start building secure AI agents and provide feedback to shape the platform's future.

`next.js` `fastapi`

  </Card>

</Card>

## Image Card

<Frame>
  <img src="/docs/images/ai/docusaurus-social-card.jpg" />
</Frame>

## Two Column Layout

<Columns cols={2}>
  <Card>
    ### 1. I am a heading
    I am some example content for testing the card component layout and styling.
    </Card>
    <Card>
    ```bash
    http://mockurl.com/auth/login
    ```
    </Card>
</Columns>
<Columns cols={2}>
  <Card>
  I am some more content without a heading.
  </Card>
  <Card>
    <img src="/docs/images/ai/docusaurus-social-card.jpg"/>
  </Card>
</Columns>
<Columns cols={2}>
    <Card>
    ### 3. I am another heading
    This is some example content for demonstrating
    the card layout and styling in a
    two-column configuration.
    </Card>
    <Card>

    ```typescript wrap
    // example code to show how a code block looks

    "use client";

    import { useUser } from "@auth0/nextjs-auth0";

    export default function Page() {
      // Extract the user object and loading state from Auth0.
      const { user, isLoading } = useUser();

      if (isLoading) return <div>Loading...</div>;

      // If user exists, show a welcome message and logout button.
      return (
        <main>
          <h1>Welcome, {user.name}!</h1>
          <p>
            <a href="/auth/logout">
              <button>Log out</button>
            </a>
          </p>
        </main>
      );
    }
    ```
    </Card>

</Columns>
