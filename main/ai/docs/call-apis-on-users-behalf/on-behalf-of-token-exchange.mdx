...
To enable your applications to call third-party APIs on the user's behalf, use [Token Vault](/ai/docs/call-apis-on-users-behalf/token-vault).
...
During an OBO exchange, the [middle-tier service](/ai/docs/call-apis-on-users-behalf/on-behalf-of-token-exchange) holds the token.
