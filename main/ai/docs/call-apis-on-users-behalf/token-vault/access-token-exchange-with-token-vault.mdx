...
When a Single-Page Application (SPA) calls a backend API, it only passes an Auth0 access token in the `Authorization` header. Because the backend API does not receive any refresh tokens issued to the SPA, it cannot use the [refresh token exchange](/ai/docs/call-apis-on-users-behalf/token-vault/refresh-token-exchange-with-token-vault) to access the Token Vault to call external APIs.
...
Before getting started, you must [configure the access token exchange with Token Vault](/ai/docs/call-apis-on-users-behalf/token-vault/configure-token-vault#configure-access-token-exchange).
...
The user logs in to the application via Google with the [Connected Accounts flow](/ai/docs/call-apis-on-users-behalf/token-vault/connected-accounts-for-token-vault#how-it-works), which uses the [My Account API](/docs/manage-users/my-account-api). If the application uses [Organizations](/docs/manage-users/organizations), the user signs in to the target organization before continuing.
...
For the access token exchange, you need to [create a Custom API Client](/ai/docs/call-apis-on-users-behalf/token-vault/configure-token-vault#create-custom-api-client) linked to the backend API. The Custom API Client has the same identifier as your backend API and has the Token Vault grant type enabled.
