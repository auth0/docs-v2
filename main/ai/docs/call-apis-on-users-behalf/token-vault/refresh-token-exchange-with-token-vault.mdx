...
Before getting started, you must [configure the refresh token exchange with Token Vault](/ai/docs/call-apis-on-users-behalf/token-vault/configure-token-vault#configure-refresh-token-exchange).
...
The user logs in to the application via Google with the [Connected Accounts flow](/ai/docs/call-apis-on-users-behalf/token-vault/connected-accounts-for-token-vault#how-it-works), which uses the [My Account API](/docs/manage-users/my-account-api). If the application uses [Organizations](/docs/manage-users/organizations), the user signs in to the target organization before continuing. After the My Account API validates and completes the Connected Accounts request, it stores the Google access and refresh tokens with the requested calendar scopes in the Token Vault.
