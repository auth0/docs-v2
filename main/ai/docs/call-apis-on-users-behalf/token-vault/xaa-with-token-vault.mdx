...
1. When using Token Vault with XAA, end users do not need to [connect their account](/ai/docs/call-apis-on-users-behalf/token-vault/connected-accounts-for-token-vault) with external applications. Your application does not need to provide a button to "Connect to [Third-party Application]" that makes a `POST` request to the My Account API's `/me/v1/connected-accounts/connect` endpoint.
