    ...
    <div className="custom-grid-container">
      <div className="custom-grid-search-container">
        <div className="custom-grid-search-filter-wrapper">
          <div className="custom-grid-search-input-wrapper">
            <input type="text" placeholder="検索..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="custom-grid-search-input" />
            ...
            <Card href={item.href} disabled={isComingSoon} key={item.id}>
              <div className="custom-card">
                ...
                <div className="custom-card-footer">
                  ...
                  {isComingSoon && <em className="custom-card-coming-soon">近日公開</em>}
                </div>
              </div>
            </Card>
          )
      ...
      {sortedItems.length === 0 && (
        <div className="custom-grid-no-results">
          ...
          <h3 className="custom-grid-no-results-title">結果が見つかりませんでした</h3>
          <p className="custom-grid-no-results-text">{searchTerm ? `「${searchTerm}」に一致する結果はありません。` : ""}検索キーワードやフィルター条件を変更してお試しください。</p>
        </div>
      )}
    </div>
  )
