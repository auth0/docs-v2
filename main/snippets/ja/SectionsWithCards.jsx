        ...
        <div className="px-5 md:px-6 pt-5 md:pt-6 pb-4">
          <div className="flex items-start justify-between gap-4">
            ...
            <div className="flex flex-col items-end gap-1 shrink-0">
              ...
              {!!date && <span className="text-sm text-gray-500 dark:text-gray-400">on {date.replace(/^on\s+/i, "")}</span>}
            </div>
          </div>
        </div>
    ...
    <main className="w-full flex justify-center px-6 py-12">
      <div className="w-full max-w-[1400px]">
        ...
        {filteredSections.length === 0 ? (
          <p className="text-gray-600 dark:text-gray-400">「{query}」の検索結果はありません。</p>
        ) : (
...
export const LibrariesCards = ({ cards }) => {
  ...
  const Card = ({ item }) => {
    ...
    const tertiary = quickstart || docs
    const tertiaryLabel = quickstart ? "クイックスタート" : docs ? "使ってみる" : ""
        ...
        <div className="flex flex-col gap-3 w-full">
          ...
          {tertiary && (
            ...
            >
              ...
              <span className="w-full">{tertiaryLabel === "Quickstart" ? "クイックスタート" : tertiaryLabel === "Get started" ? "はじめに" : tertiaryLabel}</span>
            </a>
          )}
        </div>
      </div>
