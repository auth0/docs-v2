...
export const SignUpForm = () => {
  ...
  /**
   * 認証フローデモ（外部UIライブラリなし）
   * - Tailwindのみ、本番環境対応の構造
   * - LocalStorageによるアプリ一覧 + アプリごとのURL設定
   * - 3つのフロー:
   *   1) メニュー: 作成 | 統合 | サンプル
   *   2) アプリケーションの作成 -> 一覧に追加後、統合フローに事前選択された状態でルーティング
   *   3) 既存との統合: アプリを選択してURLを保存 -> トースト通知
   */
  function LoggedInForm({ sampleApp }) {
    ...
    function loadApps() {
      ...
      // スクリーンショットに表示されている3件の初期データを投入する
    ...
    function Flows() {
      ...
      const onCreate = (name) => {
        const id = uid()
        const next = [...apps, { id, name: name || "無題" }]
        setApps(next)
        saveApps(next)
              ...
              <Card className="p-5 hover:shadow-md transition">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    ...
                    <h2 className="text-lg">新しいアプリケーションを作成する</h2>
                  </div>
              ...
              <Card className="p-5 hover:shadow-md transition">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    ...
                    <h2 className="text-lg">既存のアプリケーションと統合する</h2>
                  </div>
                  <RightChevron className="w-4 h-4 text-zinc-500" />
                </div>
              </Card>
              ...
              <Card className="p-5 hover:shadow-md transition">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    ...
                    <h2 className="text-lg">サンプルアプリケーションを確認する</h2>
                  </div>
                  <RightChevron className="w-4 h-4 text-zinc-500" />
                </div>
              </Card>
    ...
    function CreateForm({ onSave, onCancel }) {
      ...
      return (
        <div className="space-y-6">
          <Input id="app-name" label="アプリケーション名" placeholder="My App" value={name} onChange={setName} />
          <p className="text-sm text-zinc-500">この設定はアプリケーション設定で後から変更できます。</p>
    ...
    function IntegrateForm({ apps, selected, onSelect, saved, onSave, onCancel }) {
      ...
      return (
        <div className="space-y-6">
          <div>
            <span className="block text-sm text-zinc-600 dark:text-zinc-300 mb-1">アプリケーションを選択してください</span>
          ...
          >
            <Input id="callbacks" name="callbacks" label="コールバックURL" placeholder="http://localhost:3000" value={callbacks} onChange={setCallbacks} />
            <Input id="logout" name="allowed_logout_urls" label="ログアウトURL" placeholder="http://localhost:3000" value={logouts} onChange={setLogouts} />
            <Input id="origins" name="web_origins" label="許可されたWebオリジン" placeholder="http://localhost:3000" value={origins} onChange={setOrigins} />
