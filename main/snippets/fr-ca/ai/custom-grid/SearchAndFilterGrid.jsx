    ...
    <div className="custom-grid-container">
      <div className="custom-grid-search-container">
        <div className="custom-grid-search-filter-wrapper">
          <div className="custom-grid-search-input-wrapper">
            <input type="text" placeholder="Rechercher..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="custom-grid-search-input" />
          ...
          <div className="custom-grid-filter-buttons-wrapper">
            <div className="custom-grid-filter-buttons" ref={filterButtonsRef}>
              ...
              {filters.map((filterOption) => (
                <button key={filterOption} onClick={() => setFilter(filterOption)} className={`custom-grid-filter-button ${filter === filterOption ? "active" : ""}`}>
                  {filterOption}
                </button>
              ))}
              ...
              <div className="custom-card">
                <div className="custom-card-header">
                  <span className={`custom-card-icon ${isComingSoon ? "coming-soon" : ""}`}>
                    <img src={item.icon} alt={`${item.title} icon`} style={{ ...item.styles }} />
                  </span>
                  <h3 className={`custom-card-title ${isComingSoon ? "coming-soon" : ""}`}>{item.title}</h3>
                </div>

                <p className={`custom-card-description ${isComingSoon ? "coming-soon" : ""}`}>{item.description}</p>

                <div className="custom-card-footer">
                  <span className={`custom-card-type-tag ${isComingSoon ? "coming-soon" : ""}`}>{item.type}</span>

                  {isComingSoon && <em className="custom-card-coming-soon">Bientôt disponible</em>}
                </div>
              </div>
            </Card>
          )
      ...
      {sortedItems.length === 0 && (
        <div className="custom-grid-no-results">
          ...
          <h3 className="custom-grid-no-results-title">Aucun résultat trouvé</h3>
          <p className="custom-grid-no-results-text">{searchTerm ? `Aucun résultat pour « ${searchTerm} ». ` : ""}Essayez de modifier vos critères de recherche ou de filtrage.</p>
        </div>
      )}
    </div>
  )
