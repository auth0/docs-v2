...
export const SignUpForm = () => {
  ...
  useEffect(() => {
    ...
    function init() {
      setStoreReady(true)
      // Configura una suscripción reactiva al estado de autenticación
  ...
  /** Inicio de LoggedInForm */
  /**
   * Demostración de flujos de autenticación (sin bibliotecas de UI externas)
   * - Estructura lista para producción solo con Tailwind
   * - Lista de aplicaciones almacenada en LocalStorage + configuración de URL por aplicación
   * - 3 flujos:
   *   1) Menú: Crear | Integrar | Ejemplo
   *   2) Crear aplicación -> la agrega a la lista y luego la redirige al flujo Integrar, ya preseleccionado
   *   3) Integrar con una existente: selecciona una aplicación y guarda las URL -> toast
   */
  function LoggedInForm({ sampleApp }) {
    ...
    function loadApps() {
      ...
      // inicializa con las 3 que se muestran en las capturas de pantalla
      const seeded = [{ id: "{yourClientId}", name: "Aplicación predeterminada" }]
    ...
    function Flows() {
      ...
      const onCreate = (name) => {
        const id = uid()
        const next = [...apps, { id, name: name || "Sin título" }]
        setApps(next)
        saveApps(next)
    ...
    return (
      <div className="flex flex-col gap-2 items-center h-full">
        <img
          ...
          alt="Regístrate para obtener una cuenta de Auth0"
          style={{
            width: "250px",
            height: "250px"
          }}
  ...
  // // No renderizar nada hasta que el store esté listo
  // if (!storeReady) {
  //   return <SignUpFormInternal />;
  // }
  // return isAuthenticated ? <LoggedInForm /> : <SignUpFormInternal />;

  // Deshabilitar/ocultar temporalmente el formulario de registro hasta que la integración con el backend esté lista
  return <></>
}
