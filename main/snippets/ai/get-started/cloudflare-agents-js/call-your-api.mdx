import { Prerequisites } from "/snippets/ai/get-started/prerequisites/call-your-api.jsx";
import { AccountAndAppSteps } from "/snippets/ai/get-started/prerequisites/account-app-steps.jsx";
import ConfigureCloudflareKvStore from "/snippets/ai/common/configure-cloudflare-kv-store.mdx";
...
- `@auth0/ai-vercel`: [Auth0 Vercel AI SDK](https://github.com/auth0/auth0-ai-js/tree/main/packages/ai-vercel) to provide building blocks for using AI Identity for B2B with the Vercel AI SDK.
- `@auth0/ai-cloudflare`: [Auth0 Cloudflare AI SDK](https://github.com/auth0/auth0-ai-js/tree/main/packages/ai-cloudflare) to provide building blocks for using AI Identity for B2B with the Cloudflare Agents API.
...
The tool we are defining here uses the same access token to call Auth0's [`/userinfo`](/docs/api/authentication/user-profile/get-user-info) endpoint.
