...
export const SignUpForm = () => {
  ...
  useEffect(() => {
    ...
    function init() {
      setStoreReady(true)
      // Configurer un abonnement réactif à l'état d'authentification
  ...
  /** Début de LoggedInForm */
  /**
   * Démo des flux d'authentification (sans bibliothèques d'interface externes)
   * - Structure prête pour la production, Tailwind uniquement
   * - Liste d'applications sauvegardée dans LocalStorage + configuration d'URL par application
   * - 3 flux :
   *   1) Menu : Créer | Intégrer | Exemple
   *   2) Créer une application -> ajoute à la liste puis redirige vers le flux Intégrer présélectionné
   *   3) Intégrer avec l'existant : sélectionner une application + enregistrer les URL -> notification
   */
  function LoggedInForm({ sampleApp }) {
    ...
    function loadApps() {
      ...
      // amorcer avec les 3 affichés dans les captures d'écran
      const seeded = [{ id: "{yourClientId}", name: "Application par défaut" }]
    ...
    const RightChevron = ({ className = "w-5 h-5", ...props }) => (
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" stroke="currentColor" fill="none" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={className} {...props}>
    ...
    function Toast({ open, onClose, children }) {
      ...
      return (
        <div className={`fixed right-4 top-4 z-50 transition ${open ? "opacity-100 translate-y-0" : "opacity-0 -translate-y-2 pointer-events-none"}`}>
    ...
    function Flows() {
      ...
      const onCreate = (name) => {
        const id = uid()
        const next = [...apps, { id, name: name || "Sans titre" }]
        setApps(next)
        saveApps(next)
      ...
      return (
        <div>
          ...
          {route === "integrate" && (
            <IntegrateForm apps={apps} selected={selected} onSelect={selectApp} saved={cfg[selected]} onSave={(data) => onSaveCfg(selected, data)} onCancel={() => nav("menu")} />
          )}
              ...
              <Card className="p-5 hover:shadow-md transition">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    ...
                    <h2 className="text-lg">Créer une nouvelle application</h2>
                  </div>
              ...
              <Card className="p-5 hover:shadow-md transition">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    ...
                    <h2 className="text-lg">Intégrer à une application existante</h2>
                  </div>
                  <RightChevron className="w-4 h-4 text-zinc-500" />
                </div>
              </Card>
              ...
              <Card className="p-5 hover:shadow-md transition">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    ...
                    <h2 className="text-lg">Voir un exemple d'application</h2>
                  </div>
                  <RightChevron className="w-4 h-4 text-zinc-500" />
                </div>
              </Card>
    ...
    function CreateForm({ onSave, onCancel }) {
      ...
      return (
        <div className="space-y-6">
          <Input id="app-name" label="Nom de l'application" placeholder="Mon application" value={name} onChange={setName} />
          <p className="text-sm text-zinc-500">Vous pourrez modifier ce paramètre ultérieurement dans les paramètres de l'application.</p>
          <div className="flex gap-3">
            <Button onClick={() => onSave(name)}>Enregistrer</Button>
    ...
    function IntegrateForm({ apps, selected, onSelect, saved, onSave, onCancel }) {
      ...
      return (
        <div className="space-y-6">
          <div>
            <span className="block text-sm text-zinc-600 dark:text-zinc-300 mb-1">Sélectionnez votre application</span>
          ...
          >
            <Input id="callbacks" name="callbacks" label="URL de rappel" placeholder="http://localhost:3000" value={callbacks} onChange={setCallbacks} />
            <Input id="logout" name="allowed_logout_urls" label="URL de déconnexion" placeholder="http://localhost:3000" value={logouts} onChange={setLogouts} />
            <Input id="origins" name="web_origins" label="Origines Web autorisées" placeholder="http://localhost:3000" value={origins} onChange={setOrigins} />
    ...
    return (
      <div className="flex flex-col gap-2 items-center h-full">
        <img
          ...
          alt="S'inscrire à un compte Auth0"
          style={{
            width: "250px",
            height: "250px"
          }}
        ...
        >
          Inscrivez-vous pour obtenir un{" "}
          ...
          à votre compte existant pour vous intégrer directement à votre propre locataire.
        </span>
        <button
...
export const CreateInteractiveApp = ({
  ...
  const Button = ({ children, ...props }) => {
    return (
      <button className="bg-[--button-primary] text-[--foreground-inverse] px-[1.125rem] py-1.5 rounded-lg font-medium" {...props}>
        {children}
      </button>
    )
  }
      ...
      <Card className="flex-col items-start p-4 gap-3.75">
        ...
        <div className="w-full flex gap-2">
          <input
            ...
            "
            placeholder={`Mon application ${placeholderText}`}
            value={name}
