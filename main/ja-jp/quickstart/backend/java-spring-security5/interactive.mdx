...
<Recipe>
  <Content>
    ...
    Auth0 Dashboardを使って初めてAPIをセットアップする場合には、[使用の開始ガイド](/ja-jp/get-started/auth0-overview/set-up-apis)を確認してください。
    ...
    **Auth0を初めてご利用ですか？** [Auth0の仕組み](/ja-jp/overview)と、OAuth 2.0フレームワークを用いた[API認証と認可の実装](/ja-jp/api-auth)について説明します。
      ...
      <AuthCodeBlock icon="file-code" language="gradle">
      ...
      `APIController`という名前の新しいクラスを作成し、エンドポイントへの要求を処理します。`APIController`には、[APIエンドポイントを保護する](/ja-jp/quickstart/backend/java-spring-security5/interactive#configure-the-resource-server)セクションで定義されるように、3つのルートがあります。たとえば、`@CrossOrigin`の注釈からすべてのオリジンを許可します。実際のアプリケーションでは、ユースケースに対して`CORS`を構成する必要があります。
      </Section>
      ...
      <AuthCodeBlock icon="file-code" language="ps">
      AUTH0_SECRET='use [openssl rand -hex 32] to generate a 32 bytes value'
      APP_BASE_URL='http://localhost:3000'
      AUTH0_DOMAIN='https://{USER_DOMAIN}'
      AUTH0_CLIENT_ID='{USER_CLIENT_ID}'
