...
<Recipe>
  <Content>
    ...
    <Section id={sections[1].id} title={sections[1].title} stepNumber="2">
      ...
      - `domain`：Auth0テナントのドメインです。通常、Auth0 Dashboardにあるアプリケーションの設定の［Domain（ドメイン）］フィールドで確認できます。[カスタムドメイン](/ja-jp/custom-domains)を使用している場合は、その値を代わりに設定してください。
    ...
    <Section id={sections[4].id} title={sections[4].title} stepNumber="5">
      ユーザーがログインやログアウトできるようになったら、認証済のユーザーに関連付けられた[プロファイル情報](/ja-jp/users/concepts/overview-user-profile)を取得できるようにしたいと考えるはずです。たとえば、ログインしたユーザーの名前やプロファイル画像をプロジェクトに表示できるようになりたいかもしれません。
